"""Leg E — E4d: full open-borders flow with Saxony (alias-reachable neutral) + DP evidence."""
import sys, json
sys.path.insert(0, "smoke_logs")
from g4_legE_lib import get, post, save

st, ds0 = get("/debug/diplomatic_status")
print("DP before:", ds0.get("diplomatic_points"), "| rel Saxony:",
      ds0.get("nation_relations", {}).get("France|Saxony"))
...
st, r = post("/command", {"command": "propose open borders with Saxony"})
print("mount:", st, r.get("success"))
save("g4_legE_e4_openborders_saxony_mount.json", r)
...
if dd:
    print("type:", dd.get("type"), "| est:", dd.get("acceptance_estimate"),
          ...
          "| display:", dd.get("proposal_type_display"))
    st, r2 = post("/respond_to_diplomatic_dialogue", {"choice": "execute_proposal"})
    print("send:", st, r2.get("success"))
    save("g4_legE_e4_openborders_saxony_send.json", r2)
    print("message:", json.dumps(r2.get("message"), ensure_ascii=False)[:600])
    print("proposal_result:", json.dumps(r2.get("proposal_result"), indent=1, ensure_ascii=False)[:1400])
