...
w.marshals["Ney"].location = "Milan"; w.marshals["Ney"].strength = 16000
...
print("PLAYER ATTACK tail:", (r1.get("message") or "")[-330:])
print("counter_punch granted:", j.counter_punch_available, "| John:", j.strength,
      "| Ney:", w.marshals['Ney'].strength, "@", w.marshals['Ney'].location)
before = {n: w.marshals[n].strength for n in ("Ney","Massena","Lannes","Davout")}
print("Milan field:", sum(before.values()))
with contextlib.redirect_stdout(io.StringIO()):
...
for a in (ep.get("actions") or []):
    ...
    if "John" in s:
        print("\nENEMY PHASE:", s[:900])
print("\nMilan before:", before, "\nMilan after :", {n: w.marshals[n].strength for n in ("Ney","Massena","Lannes","Davout")})
