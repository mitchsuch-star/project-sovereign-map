...
class TestM13RedemptionCooldown:
    """Talleyrand redemption should have a 5-turn cooldown."""

    def _make_talleyrand(self, trust=15):
        """Create a mock Talleyrand diplomat."""
        class MockDiplomat:
            def __init__(self):
                self.personality = "schemer"
                self.trust = trust
                self.skill = 10
                self.name = "Talleyrand"
        return MockDiplomat()

    def test_redemption_fires_when_no_cooldown(self):
        """Redemption should fire when trust <= 20 and no cooldown."""
        from backend.commands.diplomatic_defiance import check_talleyrand_redemption
        world = _make_world()
        world.last_redemption_turn = 0
        world.current_turn = 10
        talleyrand = self._make_talleyrand(trust=15)
        assert check_talleyrand_redemption(talleyrand, world) is True

    def test_redemption_blocked_by_cooldown(self):
        """Redemption should NOT fire within 5 turns of last redemption."""
        from backend.commands.diplomatic_defiance import check_talleyrand_redemption
        world = _make_world()
        world.last_redemption_turn = 3
        world.current_turn = 5  # Only 2 turns since last redemption
        talleyrand = self._make_talleyrand(trust=15)
        assert check_talleyrand_redemption(talleyrand, world) is False

    def test_redemption_not_blocked_when_never_fired(self):
        """last_redemption_turn=0 means 'never fired' and should not block."""
        from backend.commands.diplomatic_defiance import check_talleyrand_redemption
        world = _make_world()
        world.last_redemption_turn = 0
        world.current_turn = 1  # Even on turn 1, 0 means never fired
        talleyrand = self._make_talleyrand(trust=15)
        assert check_talleyrand_redemption(talleyrand, world) is True

    def test_redemption_fires_after_cooldown_expires(self):
        """Redemption should fire again after 5 turns have passed."""
        from backend.commands.diplomatic_defiance import check_talleyrand_redemption
        world = _make_world()
        world.last_redemption_turn = 3
        world.current_turn = 8  # 5 turns since last redemption
        talleyrand = self._make_talleyrand(trust=15)
        assert check_talleyrand_redemption(talleyrand, world) is True

    def test_apply_redemption_sets_cooldown(self):
        """apply_redemption_choice should set last_redemption_turn."""
        from backend.commands.diplomatic_defiance import apply_redemption_choice
        world = _make_world()
        world.current_turn = 10
        talleyrand = self._make_talleyrand(trust=15)

        apply_redemption_choice("redemption_apologize", talleyrand, world)
        assert world.last_redemption_turn == 10

    def test_apply_redemption_replace_sets_cooldown(self):
        """Replace also sets the cooldown turn."""
        from backend.commands.diplomatic_defiance import apply_redemption_choice
        world = _make_world()
        ...
        assert world.last_redemption_turn == 7

    def test_redemption_loyalist_never_fires(self):
        """Loyalist personality should never trigger redemption."""
        from backend.commands.diplomatic_defiance import check_talleyrand_redemption
        world = _make_world()
        world.last_redemption_turn = 0
        world.current_turn = 100
        talleyrand = self._make_talleyrand(trust=5)
        talleyrand.personality = "loyalist"
        assert check_talleyrand_redemption(talleyrand, world) is False


# ═══════════════════════════════════════════════════════════════════════════
# m13 SERIALIZATION: last_redemption_turn round-trips
# ═══════════════════════════════════════════════════════════════════════════

class TestM13Serialization:
    """last_redemption_turn should serialize and deserialize."""

    def test_to_dict_includes_last_redemption_turn(self):
        """to_dict should include last_redemption_turn."""
        world = _make_world()
        world.last_redemption_turn = 7
        data = world.to_dict()
        assert "last_redemption_turn" in data
        assert data["last_redemption_turn"] == 7

    def test_from_dict_restores_last_redemption_turn(self):
        """from_dict should restore last_redemption_turn."""
        world = _make_world()
        world.last_redemption_turn = 12
        data = world.to_dict()

        restored = WorldState.from_dict(data)
        assert restored.last_redemption_turn == 12

    def test_from_dict_defaults_to_zero(self):
        """from_dict should default to 0 if field is missing."""
        world = _make_world()
        data = world.to_dict()
        # Remove the field to simulate old save
        data.pop("last_redemption_turn", None)

        restored = WorldState.from_dict(data)
        assert restored.last_redemption_turn == 0
