...
class TestFindWeightedPath:
    ...
    def setup_method(self):
        # Default map: Tyrol=mountains(2.0), Lyon=hills(1.2), Paris=urban(1.0)
        # Set up a scenario where BFS and Dijkstra differ.
        # Route from Bavaria to Milan:
        #   BFS: Bavaria -> Tyrol -> Milan  (2 hops)
        #   OR:  Bavaria -> Vienna -> Milan  (2 hops)
        #   BFS picks either (same hop count). Dijkstra should prefer
        #   Vienna(urban=1.0) + Milan(urban=1.0) = 2.0
        #   over Tyrol(mountains=2.0) + Milan(urban=1.0) = 3.0
        self.world = WorldState()

    def test_prefers_lower_attrition_route(self):
        """Dijkstra picks lower-cost route over equal-hop-count alternatives."""
        # Bavaria -> Milan: via Vienna (urban 1.0 + urban 1.0 = 2.0)
        # vs via Tyrol (mountains 2.0 + urban 1.0 = 3.0)
        path = self.world.find_weighted_path("Bavaria", "Milan")
        assert path is not None
        assert path[0] == "Bavaria"
        assert path[-1] == "Milan"
        # Should go through Vienna, not Tyrol
        assert "Vienna" in path
        assert "Tyrol" not in path
    ...
    def test_longer_hop_count_but_lower_cost(self):
        ...
        path = self.world.find_weighted_path("Paris", "Rhineland")
        assert path is not None
        assert "Lyon" in path
    ...
    def test_returns_none_for_unreachable(self):
        ...
        # Isolate Netherlands by blocking all its neighbors
        # Netherlands connects to Belgium, Waterloo, Hanover
        path = self.world.find_weighted_path("Netherlands", "Paris",
                                              avoid_regions=["Belgium", "Waterloo", "Hanover"])
        assert path is None
    ...
    def test_avoid_regions_doesnt_block_destination(self):
        ...
        path = self.world.find_weighted_path("Belgium", "Rhineland",
                                              avoid_regions=["Rhineland"])
        assert path is not None
        assert path[-1] == "Rhineland"
    ...
    def test_weighted_and_bfs_return_different_routes(self):
        ...
        bfs_path = self.world.find_path("Paris", "Rhineland")
        weighted_path = self.world.find_weighted_path("Paris", "Rhineland")
...
class TestGetWeightedDistance:
    ...
    def test_multi_hop_distance(self):
        ...
        # Bavaria -> Milan via Vienna (cheapest):
        # Vienna(urban=1.0) + Milan(urban=1.0) = 2.0
        dist = self.world.get_weighted_distance("Bavaria", "Milan")
    ...
    def test_returns_inf_for_unreachable(self):
        ...
        # Block all exits from Netherlands (Belgium, Waterloo, Hanover)
        dist = self.world.find_weighted_path("Netherlands", "Paris",
                                              avoid_regions=["Belgium", "Waterloo", "Hanover"])
        assert dist is None  # No path exists

        # get_weighted_distance doesn't have avoid_regions, so test
        # with actually unreachable case via invalid region name.
        # On the default 19-region map everything is connected.
    ...
    def test_weighted_greater_than_hop_count_for_mountains(self):
        """Weighted distance > hop count when path crosses non-plains terrain."""
        # Belgium to Bavaria: via Rhineland (2 hops)
        # Rhineland(river_crossing=1.5) + Bavaria(hills=1.2) = 2.7
        hop_dist = self.world.get_distance("Belgium", "Bavaria")
        weighted_dist = self.world.get_weighted_distance("Belgium", "Bavaria")
        assert weighted_dist > hop_dist  # 2.7 > 2
...
class TestAIRetreatTerrainAware:
    ...
    def test_retreat_avoids_mountains_when_tie(self):
        ...
        # Lyon is adjacent to: Paris, Bordeaux, Marseille, Rhineland, Milan
        # Set several as British-controlled
        self.world.get_region("Marseille").controller = "Britain"  # plains (1.0)
        self.world.get_region("Rhineland").controller = "Britain"  # river_crossing (1.5)

        # Capital is Netherlands. Weighted distance from Marseille/Rhineland to Netherlands
        ...
        dest = self.ai._find_retreat_destination(wellington, "Britain", self.world)
        assert dest is not None
        assert dest in ("Marseille", "Paris", "Bordeaux", "Rhineland", "Milan")
...
class TestTerrainDisplay:
    ...
    def test_scout_includes_terrain_name_and_defense(self):
        ...
        # Paris is adjacent to Normandy, Belgium, Lyon, Bordeaux
        # Lyon = hills (+15% defense)
        result = self.executor.execute(
            {"command": {
                "marshal": "Ney",
                "action": "scout",
    ...
    def test_scout_adjacent_includes_terrain(self):
        ...
        assert result.get("success")
        msg = result.get("message", "")
        # Paris is adjacent to Normandy(plains), Belgium(plains), Lyon(hills), Bordeaux(plains)
    ...
    def test_scout_event_includes_terrain_data(self):
        ...
        # Paris is adjacent to Lyon (hills)
        result = self.executor.execute(
            {"command": {
                "marshal": "Ney",
                "action": "scout",
    ...
    def test_status_map_data_includes_terrain(self):
        ...
        assert "Tyrol" in map_data
        assert map_data["Tyrol"]["terrain"] == "mountains"
...
class TestBFSUnchanged:
    ...
    def test_find_path_still_works(self):
        ...
        path = self.world.find_path("Paris", "Rhineland")
        assert path is not None
    ...
    def test_get_distance_still_works(self):
        ...
        dist = self.world.get_distance("Paris", "Rhineland")
        assert dist == 2

    def test_find_path_avoid_regions_still_works(self):
        """BFS find_path with avoid_regions still works."""
        path = self.world.find_path("Paris", "Rhineland", avoid_regions=["Belgium"])
        assert path is not None
        assert "Belgium" not in path
        assert "Lyon" in path
...
class TestWeightedPathfindingEdgeCases:
    ...
    def test_weighted_distance_adjacent_regions(self):
        ...
        # Paris -> Lyon: Lyon is hills (1.2)
        dist = self.world.get_weighted_distance("Paris", "Lyon")
        assert dist == pytest.approx(1.2)

    def test_empty_avoid_regions_same_as_none(self):
        """Empty avoid_regions list behaves same as None."""
        path_none = self.world.find_weighted_path("Paris", "Rhineland")
        path_empty = self.world.find_weighted_path("Paris", "Rhineland", avoid_regions=[])
