...
def _evaluate_settlement_eligibility(world, war_id: str) -> Dict[str, Any]:
    """SC-10 (v0.22): active hostile-pair eligibility payload with humanized
    disabled reasons. Callers that only need the boolean can use the
    `_evaluate_settlement_available(...)` wrapper below. The richer payload
    is preferred because it carries `error_display` /
    `disabled_reason_display` for surfaces that need to explain why
...
def _evaluate_settlement_available(world, war_id: str) -> bool:
    """SC-10 boolean wrapper preserved for legacy callers that only need
    the availability flag without the disabled-reason payload."""
