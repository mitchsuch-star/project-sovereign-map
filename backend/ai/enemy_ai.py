...
# HC-4b "The Admiralty's bill" (gate §5b) — flip lever for the
...
# affordability-gated at that price — the parity the dead table entry
...
class EnemyAI:
    ...
    def execute_admin_phase(self, nation: str, world, game_state: Dict) -> List[Dict]:
        ...
        while admin_ap > 0:
            ...
            if result.get("success"):
                # HC-4b "The Admiralty's bill" (gate §5b): the two naval
                # verbs bill at the `_action_costs` TABLE price (2 / 1)
                # against the admin budget — a descent consumes the whole
    ...
    def _pick_admin_action(self, nation: str, world, admin_ap: int, skip_actions: set = None) -> Optional[Dict]:
        ...
        if ("grant_dotation" not in skip_actions
                or "grant_pension" not in skip_actions):
        ...
        # HC-4b: the rung is affordability-gated at the TABLE price — a
