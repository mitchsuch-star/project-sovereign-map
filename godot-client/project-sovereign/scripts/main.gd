...
const DIPLO_FAMILY_KEYWORDS = [
	...
	"end agreement",
	"void treaty",
	# WO slice 11 (WO-20): the articled and alliance forms the parser's
	# `_mentions_treaty_break` now accepts — the Cabinet claims them too,
	# so a typed "break the alliance with Austria" reaches the wizard's
	# break flow (the one with a working breach preview) and never the
	# backend by the terminal (G1).
	"break the treaty",
	"cancel the treaty",
	"renounce the treaty",
	"end the treaty",
...
func _display_turn_change(event: Dictionary):
	...
	# IQ-2 (Sept 14, 2026): the collapse, stated beside the treasury. A France
	# holding one province — then none — read this banner as an ordinary turn
	# for four turns running. The backend sends "" while the realm stands (and
	# on the legacy world), so a standing realm renders nothing new here. A
	# null-safe read: str(null) would print "<null>".
	var collapse_line = event.get("collapse_line", "")
