...
- **Settlement Conversational Re-front** — **v0.6 APPROVED May 29, 2026** (audited five times before approval; Slice 0 LANDED, **Slice 1 is the active next slice**). `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` + audit `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_AUDIT.md` (next-up settlement priority; common peace as a conversational multi-party settlement table; per-court ratification gate; per-court baseline direction from `direct_scores` via the `select_direct_score` tuple-or-`None` contract; SC-32 D5 no-conference-copy boundary; remaining: Slices 1–3 + multi-court voice)
...
| Peace Deals / Imperial Settlement | **Active: `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` v0.6 (APPROVED May 29, 2026; Slice 0 LANDED, Slice 1 next).** Cleanup gate is now `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` **v0.32 GO** (SC-5R + SC-32/Slice G2 LANDED) with **Gate 4 smoke open**, then `docs/STATUS.md`. Older Imperial Settlement plan/spec docs remain historical anchors; do not route active work back to v0.19-v0.27, Slice F, Slice E, or a fresh G2-Slice-1 start. |
