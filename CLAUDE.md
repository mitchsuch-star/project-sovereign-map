...
- **Architecture Refactoring — IN PROGRESS.** Audit complete. Now iterating through `docs/ARCHITECTURE_REFACTORING_PLAN.md` (20 R-items, 22 sessions). **Session 1 (R3 conftest) COMPLETE** — `tests/conftest.py` with MarshalFactory/WorldFactory, 25 validation tests, 5 files migrated (253 tests). **Session 2A (R1 characterization) COMPLETE** — 25 tests pinning all 5 combat paths' post-combat behavior. **Session 2B (R1 pipeline extraction) COMPLETE** — `_post_combat_pipeline()` (14 steps), all 5 paths wired, 7 bugs fixed, 30 enforcement tests. **Session 3 (R2 war-state helpers) COMPLETE** — 5 query helpers on WorldState, centralized `set_diplomatic_state()` in diplomacy.py, 23 write sites migrated, 47 tests. **Session 4 (R4 response pipeline) COMPLETE** — `build_base_response()` + `_build_result_response()` in main.py, 11 endpoints migrated, "Bug 5" pattern eliminated, 45 tests. **Session 5 (R5 fog-filtered data access) COMPLETE** — `get_visible_enemies()` + `visibility_at_least()`, 8 sites migrated, 26 callers audited, 42 tests. **Session 6 (R7+R8 display names + campaign log) COMPLETE** — `display_names.py` registry (11 maps consolidated + 4 new), 16 silent event types added to campaign log, 99 enforcement tests. **Session 7 (R6 CooldownManager + PopupQueue) COMPLETE** — `cooldown_manager.py` with CooldownManager (5 registered cooldowns, `decrement_all()`) + PopupQueue (7 popup types, `pop_highest()`), 12 backward-compat properties, 64 tests. **Session 8 (R9+R20 Scaling Index + advance_turn Guard) COMPLETE** — `_marshals_by_region` index + `_get_marshals_in_region_indexed()`, 2 hot paths optimized (visibility + supply attrition), `_last_advanced_turn` idempotency guard serialized, 28 tests. **Session 9 (R18 Test Enforcement Suite) COMPLETE** — `test_enforcement_suite.py` (7 classes, 21 tests): VALID_ACTIONS ↔ AP costs/display names/parser/executor, proposal types ↔ display, diplomatic states ↔ display, free_actions sync. 11 latent gaps fixed. **Session 10A (R10A Executor Split: Combat Core) COMPLETE** — created `combat_executor.py` (CombatExecutor class, ~3,700 lines): extracted _execute_attack, _execute_bombardment, _execute_glorious_charge, _execute_charge, _execute_form_square, _execute_break_square, _resolve_garrison_combat, _post_combat_pipeline + 13 combat helpers. executor.py 14,802→11,101 lines. `__getattr__` backward compat. 3 test fixes (monkeypatch target, enforcement suite scan, _current_sortie delegation). **Session 10B (R10B audit 10A + coordination extraction) COMPLETE** — Phase 1 audit clean (10 delegated methods verified, no circular imports). Phase 2: extracted 13 coordination methods + 6 auto-dispatch methods from executor.py to combat_executor.py. executor.py 11,101→10,153 lines, combat_executor.py 3,752→4,713 lines. 31 methods in _COMBAT_DELEGATED. 2 test fixes. **Session 11 (R11 Executor Split Phase 2: Diplomatic + Strategic) COMPLETE** — created `strategic_executor.py` (StrategicExecutor, ~1,833 lines, 10 methods) and `diplomatic_executor.py` (DiplomaticExecutor, ~2,303 lines, 19 methods). executor.py 10,153→6,148 lines (-4,005). 10 methods in _STRATEGIC_DELEGATED, 19 in _DIPLOMATIC_DELEGATED. 7,707 tests passing. **Session 12A (R12 DialogueManager Foundation) COMPLETE** — created `dialogue_manager.py` (DialogueManager class: push/replace/pop/peek/is_blocking/clear_stale/promote_if_empty/remove_matching, priority queue, serialization), transparent backward-compat properties on WorldState, 48 tests (13 characterization + 35 unit). 7,755 tests passing. **Session 12B (R12 Core Migration: diplomatic_executor.py) Phase 0 audit COMPLETE, Phase 2 migration NEXT** — 73 ops classified: 22 SETs → `replace()`, 41 CLEARs → `pop()`, 2 CLEARs → DELETE (Pattern 3), 8 READs unchanged. ALL SETs use `replace()` (player-initiated = overwrite, not queue). Migration audit: `docs/R12B_MIGRATION_AUDIT.md`. Spec: `docs/R12_DIALOGUE_MANAGER_SPEC.md`. **Session 12C (R12 Consolidation)** after 12B — migrate remaining 12 files, remove duplicate auto-pop, lock down properties. Methodology: Characterization Testing (pin behavior → restructure → fix bugs). Priority order: R3→R1→R2→R4→R5 (first 5 deliver ~80% of value).
