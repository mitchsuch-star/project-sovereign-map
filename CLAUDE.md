...
- **Settlement UI Cleanup G2-Slice-5 — LANDED + AUDIT-FIXED May 7, 2026 (Presentation & Metadata).** Branch `codex/g2-slice5-audit-fixes` adds audit fix commit `8144ea9` and regression commit `f513f38` on top of the original Slice 5 landing. Active next gate is **Gate 4 manual Godot smoke** (10-step script in `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` §Gate 4), then Slice G AI/ally settlement agency.

- **G2-Slice-5 closure summary:** Closes SC-15 / SC-15b / SC-16 / SC-17 / SC-19 / SC-20 / SC-23 / SC-24 / SC-25 per spec v0.17 after the audit follow-up. Backend additions in `backend/game_logic/settlement_presentation.py`: `build_applied_clauses_preview` (structural mutation rows by clause type with region/payer/recipient/amount/turns/`includes_continental_system`/`projected_threat_delta`/`pair_state_transition`), `build_beneficiaries_preview`, `build_shut_out_allies_preview`, `build_third_party_reaction_preview`, `build_forced_alliance_threat_preview_display` (consumes `compute_forced_alliance_threat_preview(...)` output via `component_debug.forced_alliance_threat_preview`), and `build_peace_settlement_history` (single reverse-chronological merged surface, `PEACE_HISTORY_DEFAULT_ROWS = 5`, `peace:` / `settlement:` distinct route-id namespaces). Audit fixes make the preview/mutation fields align for gold indemnity, gold-per-turn, forced-alliance, vassalage/subjugation, liberation, and archived acceptance rendering; shut-out ally preview now excludes leaders/rewarded/opposite-side rows; live preview filters enemy-side display through `world.get_visible_enemies(...)`; failed ratification now actually calls `route_settlement_reactions(success=False, mutated=False)` and returns `skipped_reason="failed_ratification"` without history emission; merged peace/history caps after merge; bilateral peace row meta preserves the `peace:{participants_signature}:{turn}:{seq}` route id; `diplomatic_ledger.py` reads common-peace harshness through `get_treaty_harshness_for_consumer(..., consumer="diplomatic_ledger")`; SC-19 voice families are wired for live review, blocked ratification, rescore-after-staging, discard, collision, reopen-cap, and foreign-observer copy. `proposal_confirm_popup.gd` heading reads `talleyrand_text` directly; legacy `(phrase)` duplicate render is gone (SC-20). Test surface: `tests/test_settlement_ui_cleanup_g2_slice5.py` now has **31 behavior tests** including audit regressions for archived acceptance and shut-out filters. Verification after audit fixes: Slice 5 test `31 passed`; settlement focused bundle `175 passed`; serialization enforcement `19 passed`; `ruff check backend tests` clean. **Next:** Gate 4 manual Godot smoke → Slice G AI/ally settlement agency → Final Gate test bundle → 8.EVAL → Phase 8.5.
...
- **Peace Deals - Phase A COMPLETE, Gate 1 PASSED. WB-A/WB-B/WB-C/WB-D COMPLETE. Imperial Settlement Slices A1–E + Slice F UI routing LANDED. Settlement UI Cleanup Gate 2 Slices 1–5 LANDED and audit-fixed.** `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.17 now points to **Gate 4 manual Godot smoke** as the active next step before Slice G, not more Gate 2 cleanup coding.
