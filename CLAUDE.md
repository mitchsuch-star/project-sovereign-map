...
- **Settlement Conversational Re-front** — **v0.6 finished + audited five times (3rd & 4th runs Codex NO-GO; 5th run synthesis of a Claude NO-GO + Codex GO → all findings folded), NEEDS APPROVAL. DO NOT CODE.** `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` + audit `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_AUDIT.md` (next-up settlement priority; common peace as a conversational multi-party settlement table; per-court ratification gate; per-court baseline direction from `direct_scores` via the `select_direct_score` tuple-or-`None` contract; SC-32 D5 no-conference-copy boundary; on approval → Slices 0–3 + multi-court voice)
