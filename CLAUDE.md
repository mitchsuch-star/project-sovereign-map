...
- **AUTHORITATIVE Imperial Settlement handoff (May 7, 2026):** G2-Slice-1 Foundation plus Codex audit fixes are landed on `codex/settlement-spec-readiness-tightening` at commit `97a5fc2` (`Fix settlement foundation audit gaps`). Confidence is high for the backend foundation/audit surface: canonical clause validation rejects empty drafts, unknown/alias keys, non-dict clauses, invalid actors/sides, conflicts, and >8 clauses; submitted `settlement_terms` are forwarded and revalidated before staging; `Ratify Settlement` is acceptance/hard-stop gated and rescored from live world state before mutation; `Revise Terms` is hidden/disabled until a real editor exists; Back Out clears persisted drafts; `pending_settlement_drafts` and `settlement_route_seq` serialize/deserialize; same-war same-turn route ids now use the staged sequence and are carried into reaction events. Verification: `ruff check backend tests` clean and full suite `9833 passed, 1 skipped`. This is not full Settlement UI Cleanup closure: no Slice G / AI-ally settlement agency yet, and the next coding target is the next Gate 2 cleanup slice, starting with SC-5/SC-7 incoming-offer defer-and-hide leak closure, SC-10/SC-12 entry safety, SC-13 selected-target continuity, SC-14 active-vs-archived route continuity, and SC-18 settlement-family fallback safety. Do not follow the older Slice E / Final Gate paragraph below as active routing.
