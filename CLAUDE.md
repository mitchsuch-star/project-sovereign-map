...
- **Imperial Settlement current handoff:** B3 lifecycle + Slice C1a side-pressure foundation + Slice C1b common-peace acceptance formula + Slice C2 preview foundation + Slice C2 ratification mutation + **Slice D1/D2 settlement & cross-war reaction routing** are landed. `ratify_settlement_confirm` in `backend/game_logic/settlement_preview.py` now invokes `route_settlement_reactions(world, war_id=..., proposer_side=..., accepting_side=..., covered_enemy_participants=..., settlement_terms=..., resolved_pairs=..., applied_clauses=..., pre_cleanup_snapshots=..., war_ended=..., balance_projection=...)` AFTER cache invalidation (`invalidate_war_instance_indexes` + `invalidate_bloc_members_cache` + `invalidate_active_nations_cache`) but BEFORE the `settlement_confirm` dialogue is popped, satisfying spec §11 line 1239 ordering. New module `backend/game_logic/settlement_reactions.py` ships: (1) `settlement_memories: Dict[str, List[Dict]]` field on `WorldState` (round-trip + `prune_expired_settlement_memories()` wired into `advance_turn`), (2) `_add_settlement_memory(...)` writer with refresh semantics for `settlement_gratitude` / `sold_out_by_war_leader` per spec §14.3 line 1480 / §14.6 line 1524, (3) proposer-side reactions writing `they_chose_us` durable + `settlement_gratitude` transient (only when `material_contribution_points > 0` per spec §14.3 line 1478) for rewarded allies, plus `settlement_shut_out` grievance flag through the existing `_add_grievance_flag(..., source_episode_type="settlement_reaction")` machinery for shut-out allies (zero-material `major` with `seat` standing reduced to relation-only per spec §14.2 line 1469), (4) enemy-side reactions writing `sold_out_by_war_leader` memory + relation hits vs. France and the leader gradient by severity, only for material-loss families per spec §14.6 Tilsit threshold line 1511, (5) WB-B integration routing French claim awarded to a different beneficiary through the existing `breach_bargain` pipeline (lifecycle pass still owns auto-fulfillment when France controls the claim region), (6) cross-war scan bounded by `world.get_war_instances_by_participant(...)` per spec §11.5 line 1241 — writes `rival_strengthened` grievance flags through the same `_add_grievance_flag` path so subsequent acceptance reads it via `grievance_modifier` per spec line 1245, (7) `settlement_summary` + `settlement_digest` event emission with the spec §11.6 line 1290 settlement route metadata (`event_family="settlement"`, `review_target="settlement_review"` while war active else `"diplomatic_ledger"`, deterministic `route_id`), and (8) `settlement_gratitude_mod()` acceptance hook `+5` for rewarded-ally subsequent deep-treaty / war-entry / war-bargain / ally-entry proposals — wired into `calculate_acceptance` AFTER `political_subtotal_clamped` and BEFORE `deal_balance` per spec §14.3 line 1482, with `display_names.FEEDBACK_STRINGS` entry. Verification: 31 new tests in `tests/test_settlement_reactions_d1_d2.py` covering memory storage / refresh / prune / round-trip, gratitude hook gating, proposer-side rewarded + shut-out + zero-material-major bands, enemy-side sold-out severity gradient, bargain breach detection, cross-war scan via `war_instances_by_participant`, dispatch event emission + digest cap, and the `ratify_settlement_confirm` ordering invariant. Full suite **`9669 passed, 1 skipped`** (was `9629`); ruff clean. **Remaining: Slice E presentation surface (settlement review / war status panel rows / Talleyrand voice copy / advisory warnings / ledger surfacing for the new memories + reaction summaries) + serial-bilateral aggregate warning + `sold_out_by_war_leader` posture gate for deep-treaty asks from the former leader (spec §14.6 line 1526) + named-diplomat voice copy per Voice Bible.**
...
- **Peace Deals - Phase A COMPLETE, Gate 1 PASSED. WB-A/WB-B/WB-C/WB-D COMPLETE. Imperial Settlement / Ally Participation Slices A1 + A2 + A3 + B1 + B2 (ORDERING GUARD + BATTLE EMITTER WIRING + NON-BATTLE EMITTERS) + B3 (LIFECYCLE + STAYING POWER + ARCHIVE COMPACTION + DEBUG LIFECYCLE REVIEW FIX) LANDED; SLICE C IS NEXT.** B3 wires `open_episode` / `close_episode_for_exit` into every WAR-entry / exit seam, adds per-turn `+5` staying-power capped at `10` qualifying turns, compacts contribution scores into `archived_war_contribution_scores`, and now covers debug WAR exits through the same lifecycle expectations. Verification: `9522 passed, 1 skipped`; ruff clean. Common-peace term legitimacy + War Bargain settlement classification + Slice C/D reaction routing are the next gates.
