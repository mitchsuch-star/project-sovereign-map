...
- **Architecture Refactoring — IN PROGRESS.** 20 R-items, 22 sessions. See `docs/ARCHITECTURE_REFACTORING_PLAN.md`. **Sessions 1-12C COMPLETE** (R3 conftest, R1 post-combat pipeline, R2 war-state helpers, R4 response pipeline, R5 fog-filtered access, R7+R8 display names + campaign log, R6 CooldownManager + PopupQueue, R9+R20 scaling index + advance_turn guard, R18 test enforcement suite, R10A+R10B executor split combat, R11 executor split diplomatic + strategic, R12A+R12B+R12C DialogueManager). executor.py 14,802→6,148 lines. 7,755 tests passing. **NEXT: R13** (Executor Split Phase 3 — 6 remaining modules, executor.py→~1,500 lines), **R17** (HTTP timeout + api_client consolidation, CRITICAL), **R15** (utils.gd + PopupBase), **R14a-d** (AI Fog, 4 sessions, CRITICAL). See plan for full details.
