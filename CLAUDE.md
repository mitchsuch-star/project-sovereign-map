...
- **Peace Deals - Phase A COMPLETE, Gate 1 PASSED. WB-A/WB-B/WB-C/WB-D COMPLETE. Imperial Settlement / Ally Participation READY FOR SLICE A1.** BPH-A through BPH-D + WPS-A through WPS-D all landed. Gate 1 verified (9/9 smoke criteria). `threat_coalition` retired (merged into `balance_of_europe`). WB-A (data model + creation + validation), WB-B (lifecycle: fulfillment + breach + void), WB-C (war-entry contract: `war_entry_score`, hard blocks, `join_opportunity`, declaration preview, counter-bargains, `repudiate_bargain`, AI rules, ledger integration), WB-D (presentation: commitments routing, notifications, witness scope, voiced templates, response routes, ledger badges) all landed with ~160 tests. Active settlement work starts from `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.13 and `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.16 after the full-Europe audit synthesis closure for side-scoped leader-source metadata, off-map elimination exemption, confirm revalidation, material-share guards, corrected combat call-site inventory, access/supply cap scope, cache timing, and final stress fixtures. See `docs/PEACE_DEALS_UMBRELLA_SPEC.md` for historical context.
