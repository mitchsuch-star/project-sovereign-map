...
- **Command Robustness (`COMMAND_ROBUSTNESS_SPEC.md` v0.2, ACTIVE — scope blessed July 3, 2026):** ~~CR-0~~ + ~~CR-1~~ **✅ LANDED July 3, 2026** — parser rosters live-world derived (all 7 French 1805 marshals commandable; `test_command_robustness_cr0_parser_rosters.py`) and the eval harness is the phase's standing regression gate (`tests/data/parser_golden_corpus.json` 246 entries + `backend/ai/parser_eval.py` engine/CLI + `test_command_robustness_cr1_eval_harness.py`; typed `status` wire connected; new-action checklist step 12 = corpus entry). ~~CR-2~~ **✅ LANDED July 4, 2026** — marshal-aware confidence + one forced LLM retry on fuzzy errors; support-object/condition-clause executor demotions ("support ney" no longer self-supports); the unified `command_clarification` LOCAL_PLANNING question ("Which marshal, Sire?" — `backend/commands/clarification.py`, registered ONLY from main.py's player path, typed answers deterministic, options carry full reissue commands through the existing popup); silent-marshal-drop fixed; sequential "then" orders parse the first clause + report the tail; parse failures with candidates surface as clarifications instead of the generic Berthier shrug (`test_command_robustness_cr2_clarification.py`, 63 tests incl. the 7 pre-commit adversarial-review fixes). **Next slice: CR-3 (LLM modernization) → CR-4/CR-5.** CR-6 (conversational objection negotiation) needs its own design gate; spec §4 parks the gate-review candidates.
...
- **Command Robustness** — v0.2 scope BLESSED July 3, 2026 (CR-0..CR-5); CR-6 needs a dedicated gate; CR-5 needs its scope blessing at the §4 gate review. `docs/COMMAND_ROBUSTNESS_SPEC.md`
- **Economy Revisit** — v0.1 DRAFTED July 2, 2026; EC-2 gold-sink gate + EC-5/EC-6/EC-7 user decisions. `docs/ECONOMY_REVISIT_SPEC.md`
