...
- **AUTHORITATIVE Imperial Settlement handoff (May 8, 2026):** Settlement UI Cleanup G2-Slice-1 through G2-Slice-5 are treated as landed for current routing; do not send implementers back to G2-Slice-1. The active gate is `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.20 repair plus Gate 4 manual smoke. v0.20 resolves the rejected/losing-side contract as normative spec text: blocked ratification omits `confirm_settlement` from `options[]`, `Revise Terms` is absent unless it opens a real editor route, enemy-offer waiting remains hidden while SC-5 is deferred, failed settlement review routes to `Open War Detail` rather than inventing a popup armistice action, and losing-side concession authoring is owned by SC-1 offer-mode on the standard treaty editor. Slice G / AI-ally settlement agency remains blocked until v0.20 checks pass and Gate 4 smoke evidence, including rejected/losing concession smoke, is recorded. Do not follow older v0.19 or Slice E / Final Gate paragraphs as active routing.

  Verification focus: blocked settlement confirmation must not collapse to a lone `Back Out`; losing a war must still permit peace-seeking agency through concessionary treaty authoring; `Open War Detail` is the recovery route for bilateral peace / armistice choices; enemy-offer waiting remains hidden unless SC-5 is explicitly reversed; and `Revise Terms`, incoming offers, ratification, and War Detail recovery must remain distinct surfaces.
...
| Peace Deals / Imperial Settlement | Current gate: `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.20 repair + Gate 4 smoke, then `docs/STATUS.md`. Older Imperial Settlement plan/spec docs remain historical anchors; do not route active work back to v0.19, Slice F, Slice E, or a fresh G2-Slice-1 start. |
