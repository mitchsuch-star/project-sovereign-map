...
- **Imperial Settlement current handoff:** B3 lifecycle + Slice C1a side-pressure foundation + Slice C1b common-peace acceptance formula + Slice C2 preview foundation + **Slice C2 ratification mutation** are landed. `settlement_confirm.confirm` now mutates state through `ratify_settlement_confirm(world, dialogue)` in `backend/game_logic/settlement_preview.py` (4 helpers: `_build_pair_ratification_plan` for per-pair `PEACE`-vs-`ALLIANCE` resolution; `_capture_pre_cleanup_snapshots` for per-pair `_capture_pre_cleanup_war_data` BEFORE any cleanup; `_apply_settlement_terms` for package-level territory / gold / liberation outcomes; `_resolve_pair_state_transitions` for the per-pair `set_diplomatic_state(PEACE)` → `cleanup_war_end(conclude_objectives=True)` → optional re-set `ALLIANCE` + Continental System + `+15` threat + `alliance_origins="forced"` + `forced_alliance_imposed` event flow). Covered active hostile pairs (WAR or ARMISTICE) move to `resolved_diplo_keys` with `pair_status="resolved"` / `resolved_turn` stamped; uncovered hostile / armistice pairs stay active in the same `war_id`; `ended_turn` / `end_reason="all_pairs_resolved"` stamps only when the LAST hostile pair resolves. Cache invalidation per spec §11 line 1239: `invalidate_war_instance_indexes()` + `invalidate_bloc_members_cache()` + `invalidate_active_nations_cache()` happen BEFORE the dialogue is popped so reaction readers in the next slice see fresh state. Verification: 14 new tests in `tests/test_common_peace_c2_ratification.py`. Full suite **`9629 passed, 1 skipped`** (was `9615`); ruff clean. **Settlement / cross-war reaction routing (D1/D2) is intentionally NOT wired in C2** — the returned summary exposes `resolved_pairs`, `applied_clauses`, and `pre_cleanup_snapshots` so the next slice can route them. **Remaining: D1/D2 reactions (settlement-side ally / enemy-leader / Europe-at-large reactions reading the C2 ratification summary plus fresh war-instance indexes; cross-war reaction scan bounded by `affected_nations` per spec §11.5 line 1241; settlement memories / grievance flag writes through the existing `grievance_modifier` path per spec line 1245; settlement_summary / settlement_digest dispatch + campaign log + ledger payloads per §11.6), then E presentation surface.**
