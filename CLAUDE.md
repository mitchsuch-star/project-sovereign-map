...
~~Gate 4 visual-half confirmation~~ (✅ PASSED July 3, 2026) → ~~Slice H gate + landing~~ (✅ LANDED July 3, 2026) → ~~**Command Robustness** CR-0..CR-5~~ (✅ COMPLETE July 3-7, 2026; `docs/COMMAND_ROBUSTNESS_SPEC.md`) → ~~**CR-5b (Flavor Echoing)**~~ (✅ LANDED July 7, 2026; entry gate CLEARED — non-parroting floor + register gate) → **▶ Comprehensive Codebase Audit** — RUNS NEXT (re-sequenced July 9, 2026; `docs/AUDIT_GUIDELINE.md`; Fable-led, fix-as-you-find + creative/fun-factor pass, decide-and-act authority inside the §3 boundary) → **Econ eval** (`docs/ECONOMY_REVISIT_SPEC.md §0.7`; Fable's independent pre-build read of every economy rec — concur/dissent/simplify — feeding the EC-2 gate) → **Economy Revisit build** (`docs/ECONOMY_REVISIT_SPEC.md`; EC-0 + ledger floor landed, EC-6 = sandbox, gate-ready pass-1 plan §0.6; build session runs §0.6.3 gate-free Track 1 then the EC-2 ES-2+ES-7 pair behind the design gate) → **Marshal Content Pass** gate (`docs/MARSHAL_CONTENT_PASS_SPEC.md`; needs USER design gate) → DEF-1 voices + DEF-13 UI scale → 8.EVAL → Phase 8.5.
...
- **▶ Comprehensive Codebase Audit (`AUDIT_GUIDELINE.md`, Fable-led) — RUNS NEXT (re-sequenced July 9, 2026):** component-by-component sweep (core mechanics; state/surface/hygiene; Map & rendering) — fix safe defects in place, escalate genuine judgment calls, close with a creative/fun-factor memo (§8). Fable has **decide-and-act authority** inside the fix-vs-escalate boundary (guideline §3) and owns the sweep's shape. Component coverage + phasing completeness-checked July 9 (3 stand-alone subsystems — `diplomatic_defiance.py`, `war_contribution.py`, `settlement_reactions.py` — folded into §7.4/§7.5). **Flows into the econ eval** (`ECONOMY_REVISIT_SPEC.md §0.7`): Fable carries whole-codebase context into an independent read of every economy rec (concur/dissent/simplify), producing a memo that feeds the EC-2 gate — before any economy code. Sequence: **audit → econ eval → EC build.**
