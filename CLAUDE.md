...
- **Memory and Pressure v2.4.3 (Hegemony refactor + deep-audit fixes) — audit blocks in progress.** Substrate shipped (betrayal_history, episode_id, witness scoping, hard-reject). v2.4 collapses static concern seed into per-turn hegemony engine reading bloc shares. v2.4.3 deep-audit fixes add: §7.1 recursive vassal-chain walk, §7.3 deterministic hegemon tie-break + non-France-hegemon guard, §7.2 drops runtime `nation_power_tiers` map (reads authored scenario record directly), §8.6.1a grievance-variant Make Amends, §8.8.7a existing-alliance terminates on defensive refusal, §9.3 conditional composite floor (-60 with DG-4), §11.1 Balance-of-Europe five-case state machine (including COOLDOWN). **Block 1 doc cleanup is complete. Next:** `docs/audits/MP_V243_BLOCK2_SUBSTRATE.md`, then `docs/audits/MP_V243_BLOCK3_BLOC_NAMING.md`, then the remaining implementation slices: B-Hegemony + B-B1-lite + B-B3 + B-B4 (DG-4 call-to-arms + grievance_modifier + composite floor reintroduction) + B-B7 + trimmed C-lite presentation (~71-82 tests including parallel B-B4, ~2 implementation sessions after the audit blocks). Merge gate: B-B4 must land AT or AFTER B-B1-lite so the composite-floor reintroduction catches the `grievance_modifier` stacking. See `docs/RELIABILITY_COMMITMENTS_SPEC.md` v2.4.3 + `docs/RELIABILITY_IMPLEMENTATION_PLAN.md`.
