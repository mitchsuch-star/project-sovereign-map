...
- **Peace Deals — BPH-A through BPH-D + WPS-A + WPS-B COMPLETE.** Term ownership, peace preview panel, fallout/commitment conflicts, ratification summary + dispatch, War Purpose popup + objective state + ticking score, vassalage power cap are landed. Next: **WPS-C** (forced alliance + liberation). See `docs/PEACE_DEALS_UMBRELLA_SPEC.md` + `docs/WAR_PURPOSE_SCORE_SEMANTICS_SPEC.md`.
