...
| `backend/commands/combat_executor.py` | Combat execution: attack, bombardment, charge, garrison, form_square, post-combat pipeline (~3.7k lines, R10A) |
...
| Combat execution (attack/bombard/charge) | `combat_executor.py` (CombatExecutor: all combat _execute_* methods, post-combat pipeline, garrison, forced retreat, capture). Accesses non-combat executor methods via `self._executor.X` |
