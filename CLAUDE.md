...
- **Command Robustness (`COMMAND_ROBUSTNESS_SPEC.md` v0.2, ACTIVE — scope blessed July 3, 2026):** ~~CR-0~~ **✅ LANDED July 3, 2026** — parser rosters are live-world derived (all 7 French 1805 marshals commandable; typed vassal commands repaired; tests in `test_command_robustness_cr0_parser_rosters.py`). **Next slice: CR-1 (parser eval harness — land before behavior changes) → CR-2..CR-5.** CR-6 (conversational objection negotiation) needs its own design gate; spec §4 parks the gate-review candidates.
