...
Casualty Distribution (Session 62: `resolve_battle(apply_casualties=False)` deferred-casualty contract, proportional distribution by strength, Hostile exclusion with SUPPORT override per D3, uniform morale deltas, primary-only recklessness/counter-punch, per-participant forced retreat, C2 projected-strength victor, 63 tests). Post-S62 Hotfix: artillery AI frontline avoidance (`_score_artillery_position` -50/-30 penalty + +15 behind-screen bonus), artillery 50% casualty reduction in combined arms (`ARTILLERY_CASUALTY_FACTOR`), 22 tests. AI Coordination Enhancements (Session 63: P4.6 coordinated attack setup, P4.75 relationship-filtered ally support, P4.76 co-location persistence guard, P4.77 cross-nation adjacency scoring, P4.78 defensive reinforcement positioning, +8% attack threshold per co-located ally, artillery stagnation override, combined arms awareness, 35 tests). Full Battle Reports + Berthier Coordination Observations (Session 65: 7 new observation categories P0.5-P15, `_fill()` `.replace()` rewrite with 5 coordination placeholders, two-pass observation re-pick, reinforcement notification messages, 24 tests). Gate 4 Fixes (5 issues: coordination stats removed from modifier snapshots for narrative voice, auto-assign attack delegates to _execute_attack for coordination, artillery reinforcement no-advance + coordination gap fix so artillery still counts as adjacent ally, general_attack delegates to _execute_attack for full Phase 7 coordination, reinforcer retreat on stalemate via `not atk_won`/`not def_won`, Berthier names all reinforcers in mixed arrival/failure observations, 23 tests). Godot UI + Integration Audit (Session 66: coordination readiness tooltip, relationship display in marshal tooltip, inline-dramatic reinforcement display, first-time coordination tutorial with `coordination_tutorial_shown` field, enemy phase reinforcement display, backend relationship/co-location data in game state summary, cross-system confidence report ≥97% all areas, aggregate ally casualty display, narrative reinforcement messages, 32 tests).
