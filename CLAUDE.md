...
- **Marshal Content Pass (`MARSHAL_CONTENT_PASS_SPEC.md` — ✅ GATE BLESSED July 10, 2026, §4 gate record authoritative; ▶ BUILD IN PROGRESS: ~~MC-1a~~ + ~~MC-1b~~ + ~~MC-1c~~ ✅ ALL LANDED July 10, 2026 — the TEN blessed abilities are LIVE, landing records spec §5 + §6, 15 + 7 review findings fixed pre-commit; MC-1c = Davout "Iron Resolve", the set's only T2: serialized `iron_resolve_stacks`, fortify tick coils +1/turn cap 3, next attack consumes all at +8%/stack in `get_attack_modifier` only, survives unfortify, clears on move/rout/capture, accrual guarded to STANDING marshals; NEXT = MC-2 skills/trust):** 10 abilities (7 French / Charles / Kutuzov / Moore), MC-2 skills/trust table (admin+command per Q3 below), MC-3 13-pair relationship web, MC-4 doc surgery (defer balanced/loyal; canonize Soult-literal), MC-V personality-assurance rider. **Q3 amendment LANDED same day — `command` is WIRED: "The Rally"** (single source `marshal.py` `get_rally_stages_per_turn`/`get_retreat_stage_penalty`, constants in-band tunable: command ≥ 8 → recovery 2 stages/turn at the single `_process_tactical_states` tick — retreat 3→2 turns, broken 4→2, GR5-symmetric both sides; command ≤ 3 → retreat penalties 10pp deeper (−55/−40/−25) at unchanged duration, shown = applied; command 4–7 byte-identical — the shipped 1805 roster is flat-5 until MC-2 lands authored values, while the LEGACY fixture roster's Ney 8/Davout 9/Wellington 9 hit the fast tier deliberately; EVERY recovery-number surface is command-aware — dispatch ETA, executor block messages, voluntary-retreat + forced-retreat + shattered copy, and the map tooltip via derived `retreat_penalty`/`broken_turns_left` in `tactical_state` — post-landing review swept them, spec §4). **`administration` is UNWIRED and HIDDEN from the marshal card** (backend `marshal_overview` filter + `marshal_management.gd` skill-row list — do not re-add without the slice) **behind the owned MC-2b row** (recruit-seam wiring + un-hide; re-opens at the MC exit review). Prerequisite for the Jealousy gate.
