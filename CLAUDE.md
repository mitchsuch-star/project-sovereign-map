...
- **Imperial Settlement current handoff:** B3 lifecycle is landed. War-entry seams (`_create_skeleton_instance`, `attach_pair_to_war_instance`, `attach_participant_to_war_instance`) call `_open_contribution_episode_for_participant(...)` so every joiner gets a fresh episode at the current attach turn — a re-entered participant whose previous episode was closed gets a new episode_id at the re-entry turn (spec §7.5). Exit seams close episodes via `_close_contribution_episode_for_participant(...)`: `mark_participant_eliminated_in_all_wars` stamps elimination exits per spec §7.4, and `resolve_pair_to_resolved` now exits participants whose last active pair just resolved (separate-peace exit_path) or every still-active participant when `end_reason="all_pairs_resolved"` stamps (war-end exit_path). `cleanup_war_end(conclude_objectives=True)` (PEACE outcomes) calls `resolve_pair_to_resolved(...)` so player- and AI-driven peace ratifications drive episode closure through the same path that armistice expiration already used (idempotent against the existing armistice-expired-peace call). Per-turn staying power is `accrue_staying_power_all_wars(world, current_turn=...)` wired into `process_diplomacy_turn` BEFORE step 8 (armistice expiration); each active episode gains `STAYING_POWER_PER_TURN = 5` raw points capped at `STAYING_POWER_TURN_CAP = 10` qualifying turns (50 raw); the per-`(war_id, nation, current_turn)` idempotency guard sits on `last_staying_power_turn` / `staying_power_credited_turns`. Archive compaction runs inside `archive_terminal_war_instances`: when a war_instance clears the 10-turn retention window, `compact_war_contribution_for_archive(...)` collapses episode detail into per-nation finals (battle/occupation/staying_power/support/material_total/total + episode_count + first_joined_turn + last_exited_turn) and moves the record to `world.archived_war_contribution_scores[war_id]`; the active store is cleared post-compaction. New fixtures in `tests/helpers/full_europe_settlement_fixtures.py`: `build_three_theater_full_europe_fixture(...)`, `build_concurrent_war_lifecycle_fixture(...)`, `build_archive_retention_fixture(...)`. Tests: 27 new in `tests/test_war_contribution_scores.py` covering staying-power per-turn / cap / idempotency / closed-episode skip / ended-war skip, all-wars walker, open-episode wiring at every attach seam (skeleton, pair attach, participant attach, idempotent re-attach, post-exit re-entry), close-episode wiring at every exit seam (elimination, separate-peace, all-pairs-resolved war-end, cleanup_war_end PEACE, cleanup_war_end ARMISTICE no-op), same-turn separate-peace battle-credit ordering, per-turn staying-power same-turn ordering, concurrent-war independence (staying-power, exit), three-theater distant-front exclusion, archive retention boundary (under 10 turns / at 10 turns / multiple-episodes-per-nation compaction / save-load round trip / no-op when no record). Verification: `9520 passed, 1 skipped`; ruff clean. **Remaining: presentation surface (Slice C/D) for the new lifecycle data — settlement standing readers, dispatch / ledger surfaces, paradox resolution.**
...
- **Peace Deals - Phase A COMPLETE, Gate 1 PASSED. WB-A/WB-B/WB-C/WB-D COMPLETE. Imperial Settlement / Ally Participation Slices A1 + A2 + A3 + B1 + B2 (ORDERING GUARD + BATTLE EMITTER WIRING + NON-BATTLE EMITTERS) + B3 (LIFECYCLE + STAYING POWER + ARCHIVE COMPACTION) LANDED; SLICE C IS NEXT.** B3 wires `open_episode` / `close_episode_for_exit` into every WAR-entry / exit seam (declaration, cascade attach, participant attach, elimination, separate peace, all-pairs-resolved war end, cleanup_war_end PEACE), adds per-turn `+5` staying-power capped at `10` qualifying turns (idempotent per `(war_id, nation, current_turn)`), and compacts contribution scores into `archived_war_contribution_scores` once a war_instance clears the 10-turn retention window. Verification: `9520 passed, 1 skipped`; ruff clean. Common-peace term legitimacy + War Bargain settlement classification + Slice C/D reaction routing (presentation surfaces consuming the new lifecycle data) are the next gates.
