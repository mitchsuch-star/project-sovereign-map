...
- **Peace Deals Phase A — BPH-A COMPLETE + audit follow-up CLOSED.** Term ownership + display labels: `annotate_peace_terms()`, `peace_ratified` event, Godot annotated-terms rendering, ownership/label edge-case follow-up verified. Next: **BPH-B** (Peace preview panel + war context). See `docs/PEACE_DEALS_UMBRELLA_SPEC.md` + `docs/BILATERAL_PEACE_HARDENING_SPEC.md`.
