...
- **Peace Deals - Phase A COMPLETE, Gate 1 PASSED. WB-A/WB-B/WB-C/WB-D COMPLETE. Imperial Settlement / Ally Participation Slice A1 + A2 LANDED; A3 MERGE/ARCHIVE/LEADER IS THE NEXT GATE.** BPH-A through BPH-D + WPS-A through WPS-D all landed. Gate 1 verified (9/9 smoke criteria). `threat_coalition` retired (merged into `balance_of_europe`). WB-A/B/C/D landed with ~160 tests. **Slice A2 (May 1, 2026) — war-entry threading.** `backend/game_logic/settlement_helpers.py` now exports `CascadeContext` (transaction object so `_process_war_cascade()` does not exceed nine positional args), pre-commit `validate_war_declaration(...)` (`war_instance_side_conflict` / `war_instance_merge_required` hard-stops), `ensure_war_instance_for_pair(...)` (skeleton allocation + pair / participant reuse + ARMISTICE → WAR resumption reusing the same `war_id`), `attach_pair_to_war_instance(...)`, `attach_participant_to_war_instance(...)`, `mark_pair_armistice(...)`, and `resolve_pair_to_resolved(...)`. Live wiring threads `war_id` through `declare_war()` (validates and allocates BEFORE `set_diplomatic_state(..., "WAR", ...)`; the war-entry ledger now uses the real allocated `war_id`, not the legacy `war_{episode_id}` placeholder), every cascade attach point inside `_process_war_cascade()` (defensive, offensive, vassal defensive auto-join, vassal offensive auto-join), `backend/game_logic/vassal.py::check_vassal_rebellion()` and `release_vassal(rebellion=True)`, `resolve_join_opportunity(..., "accept")`, `accept_counter_bargain(...)`, and `_process_armistice_expiration(...)` (both ARMISTICE → WAR same-`war_id` reuse and ARMISTICE → PEACE pair-status='resolved' transition). 31 new A2 tests in `tests/test_war_settlement_instances.py` (48 total settlement tests including A1) cover the durable `WAR_ENTRY_SEAMS_UNDER_TEST` checklist plus side-conflict / merge-required hard-stop guards, concurrent-war independence, recursive-cascade single-`war_id` proof, full combined cascade + vassal fixture invariant pass, and CascadeContext legacy-kwarg propagation. **Slice A1 (foundation, landed earlier May 1, 2026):** `WorldState.next_war_instance_id`/`war_instances`/`archived_war_instances` containers with `1`/`{}`/`[]` save/load defaults; empty-safe `WorldState.get_war_instances_by_leader()` + `get_war_instances_by_participant()` with single `invalidate_war_instance_indexes()` dirty-flag hook + lazy rebuild; `_is_nation_eliminated` refactored onto cached `world.get_nation_regions(...)`; `assert_war_instance_invariants(...)` + `WarInstanceInvariantError`; synthetic 20-active-`war_instance` fixture in `tests/helpers/full_europe_settlement_fixtures.py`. Active settlement work continues from `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.21 Slice A3 "merge/archive/leader invariants" bullets and `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.24 §7. A3 implements transitive connected-component merge, multi-objective preservation, contribution-reference rewrites, leader replacement, elimination exit, terminal retention + archive compaction, and promotes `assert_war_instance_invariants(..., context="post_merge")` into an always-on post-merge assertion. Future nations enter settlement one by one through `docs/ADDING_CONTENT.md` plus a focused settlement readiness fixture when scenario/map data adds them; synthetic full-Europe fixtures must extend the active roster before adding future-nation regions/controllers. See `docs/PEACE_DEALS_UMBRELLA_SPEC.md` for historical context.
...
| Peace Deals (umbrella + sub-specs) | `docs/PEACE_DEALS_UMBRELLA_SPEC.md` (dependency graph, gates, cross-cutting decisions), `BILATERAL_PEACE_HARDENING_SPEC.md`, `WAR_PURPOSE_SCORE_SEMANTICS_SPEC.md`, `WAR_BARGAIN_SPEC.md`, `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` (active Imperial Settlement / Ally Participation handoff — Slice A1 foundation landed, A2 war-entry threading is next). New: `backend/game_logic/settlement_helpers.py` (`assert_war_instance_invariants`, `WarInstanceInvariantError`); `tests/helpers/full_europe_settlement_fixtures.py` (synthetic 13-nation + 20-active-`war_instance` fixture). |
