...
- **Memory and Pressure v2.4 (Hegemony refactor) — ACTIONABLE NOW.** Substrate shipped (betrayal_history, episode_id, witness scoping, hard-reject). v2.4 collapses static concern seed into per-turn hegemony engine reading bloc shares. Remaining: B-Hegemony (engine + bloc helpers + Balance of Europe headline) + B-B1-lite (collapsed acceptance formula) + B-B3 (paradox rename) + B-B7 (Make Amends) + trimmed C-lite presentation (~35-42 tests, ~1 session — down from v2.3 68-74 / 3 sessions). See `docs/RELIABILITY_COMMITMENTS_SPEC.md` v2.4 + `RELIABILITY_IMPLEMENTATION_PLAN.md`.
...
| Memory and Pressure substrate (hegemony / betrayal memory / paradox / reliability) | `docs/RELIABILITY_COMMITMENTS_SPEC.md` (v2.4.2 — §8.8 holds the DG-4 call-to-arms episode contract; `docs/SCALE_READINESS_PLAN.md` §DG-4 Amendment is the source of truth), `docs/RELIABILITY_IMPLEMENTATION_PLAN.md`, `diplomacy.py`, `world_state.py` (`betrayal_history`, `next_episode_id`), `commitments` logic within `diplomatic_templates.py`, `campaign_log.py`, `coalition.py` (hegemony engine when landed) |
...
| C3-lite presentation (Memory and Pressure final slice) | `docs/COMMITMENTS_PRESENTATION_SPEC.md` (v0.5 — v2.4 hegemony-aligned; Slice C trims cut spotlight-tier card variant, split-voice `attributed_lines[]`, N+1 Talleyrand aside), `docs/COMMITMENTS_PRESENTATION_DESIGNER_AUDIT.md` (historical), `docs/DIPLOMAT_VOICE_BIBLE.md`, `diplomatic_templates.py` (`commitments_spotlight_*` / `commitments_notice_*` families), `notifications.py`, `notification_bar.gd`, `dispatch.py`. Any `speaker="envoy"` / `speaker="foreign_office"` template MUST resolve to a named diplomat per Voice Bible — anonymous voice is disallowed. Three live notice events (not spotlights): `diplomatic_treaty_broken` (french_breach), `hard_reject_posture_triggered`, `commitment_paradox`. |
...
| Memory and Pressure (substrate + presentation) | `docs/RELIABILITY_COMMITMENTS_SPEC.md` (v2.4.2), `RELIABILITY_IMPLEMENTATION_PLAN.md`, `COMMITMENTS_PRESENTATION_SPEC.md` (v0.5 C3-lite hegemony-aligned), `COMMITMENTS_PRESENTATION_DESIGNER_AUDIT.md` (historical) |
