...
- **Peace Deals - Phase A COMPLETE, Gate 1 PASSED. WB-A/WB-B/WB-C/WB-D COMPLETE. Imperial Settlement / Ally Participation READY FOR SLICE A1 FOUNDATION GATE.** BPH-A through BPH-D + WPS-A through WPS-D all landed. Gate 1 verified (9/9 smoke criteria). `threat_coalition` retired (merged into `balance_of_europe`). WB-A (data model + creation + validation), WB-B (lifecycle: fulfillment + breach + void), WB-C (war-entry contract: `war_entry_score`, hard blocks, `join_opportunity`, declaration preview, counter-bargains, `repudiate_bargain`, AI rules, ledger integration), WB-D (presentation: commitments routing, notifications, witness scope, voiced templates, response routes, ledger badges) all landed with ~160 tests. Active settlement work starts from `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.20 and `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.23. Slice A1 must begin with mapped-nation capital helper safety, settlement containers, old-save defaults, empty index/cache scaffolding, cache invalidation hooks, and a 20-`war_instance` cache/index proof before any A2 `war_id` threading or later settlement behavior. Future nations enter settlement one by one through `docs/ADDING_CONTENT.md` plus a focused settlement readiness fixture when scenario/map data adds them; synthetic full-Europe fixtures must extend the active roster before adding future-nation regions/controllers. The v1.23 review sharpening requires pre-A2 `CascadeContext` guidance, explicit common-peace tuning escalation in Slice C, a spec-naive manual 4+ participant settlement-review comprehension gate before D1, always-on post-merge war-instance invariant assertions in A3, a B2 `record_battle()` ordering guard, concrete `request_revision` cooldown/no-counter-loop semantics, and D2 profiling for 5+ directly affected cross-war reactions. See `docs/PEACE_DEALS_UMBRELLA_SPEC.md` for historical context.
