...
- **Imperial Settlement current handoff:** Slices A1-A3, B1-B3, C1a/C1b, C2, D1/D2, E, and Slice F UI routing are all landed (commit `0b9289e`). **Next: Settlement UI Cleanup Spec** (`docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.17). The cleanup spec is a mandatory quality gate — common peace must become a real treaty-authoring system, not a white-peace lever with misleading controls. Implementation proceeds through Gate 1 (spec/doc readiness) then Gate 2's five ordered slices: **(1) Foundation** (SC-1–4: inline clause editor, canonical clause schema, draft persistence, acceptance-gated ratify, submit revalidation), **(2) Entry Safety** (SC-8/8b/10–13: multi-war disambiguation, eligibility cache, selected-target threading), **(3) Continuity** (SC-14/14b–e/7b/26: route-id format, stale recovery caps, collision protection, active-vs-archived routing), **(4) Incoming Offers** (SC-5/6/7: defer+hide by default unless explicitly reversed), **(5) Presentation & Metadata** (SC-15/15b/16/17/19/20/23–25: clause preview, reaction preview, acceptance snapshots, vocabulary humanization, Voice Bible §16.1). After cleanup: manual Godot smoke → Slice G AI/ally settlement agency → Final Gate test bundle → 8.EVAL → Phase 8.5.
