...
~~Gate 4 visual-half confirmation~~ (✅ PASSED July 3, 2026) → ~~Slice H gate + landing~~ (✅ LANDED July 3, 2026) → ~~**Command Robustness** CR-0..CR-5~~ (✅ COMPLETE July 3-7, 2026; `docs/COMMAND_ROBUSTNESS_SPEC.md`) → ~~**CR-5b (Flavor Echoing)**~~ (✅ LANDED July 7, 2026; entry gate CLEARED — non-parroting floor + register gate) → **▶ Comprehensive Codebase Audit correctness sweep** — RUNS NEXT (re-sequenced July 9, 2026; `docs/AUDIT_GUIDELINE.md` §5–§7; Fable-led, fix-as-you-find, decide-and-act authority inside the §3 boundary) → **Econ eval** (`docs/ECONOMY_REVISIT_SPEC.md §0.7`; Fable's independent pre-build read of every economy rec — concur/dissent/simplify — feeding the EC-2 gate) → **Economy Revisit build** (`docs/ECONOMY_REVISIT_SPEC.md`; EC-0 + ledger floor landed, EC-6 = sandbox, gate-ready pass-1 plan §0.6; build session runs §0.6.3 gate-free Track 1 then the EC-2 ES-2+ES-7 pair behind the design gate) → **§8 creative/fun-factor capstone** (split off to run AFTER EC so it judges the improved economy; `AUDIT_GUIDELINE.md` §8) → **Marshal Content Pass** gate (`docs/MARSHAL_CONTENT_PASS_SPEC.md`; needs USER design gate) → DEF-1 voices + DEF-13 UI scale → 8.EVAL → Phase 8.5.
...
- **▶ Comprehensive Codebase Audit (`AUDIT_GUIDELINE.md`, Fable-led) — RUNS NEXT (re-sequenced July 9, 2026):** component-by-component **correctness sweep** (core mechanics; state/surface/hygiene; Map & rendering) — fix safe defects in place, escalate genuine judgment calls. Fable has **decide-and-act authority** inside the fix-vs-escalate boundary (guideline §3) and owns the sweep's shape. Component coverage + phasing completeness-checked July 9 (3 stand-alone subsystems — `diplomatic_defiance.py`, `war_contribution.py`, `settlement_reactions.py` — folded into §7.4/§7.5). **Closes into the econ eval** (`ECONOMY_REVISIT_SPEC.md §0.7`): Fable carries whole-codebase context into an independent read of every economy rec (concur/dissent/simplify), producing a memo that feeds the EC-2 gate — before any economy code. **The §8 creative/fun-factor capstone is SPLIT OFF to run AFTER the EC build** (so it judges the improved economy, not the current flat 4.5/10 one; then feeds the MC/Jealousy/8.EVAL gates). **Two apertures: the *fix* aperture is conservative (safe hygiene only); the *recommendation* aperture is WIDE — the user is explicitly open to EXPANSIONS that add depth, so the creative memo (§8) + econ eval (§0.7) should propose new mechanics / deeper systems ambitiously (recommendations behind a design gate, not autonomous code).** Sequence: **audit sweep → econ eval → EC build → §8 capstone → MC.**
- **Economy Revisit (`ECONOMY_REVISIT_SPEC.md` v0.4 — design DONE + gate-recorded; the BUILD is QUEUED BEHIND the audit + econ eval, re-sequenced July 9, 2026):** EC-0 (AP-reset) + PRE-EC ledger floor LANDED; EC-6 DECIDED = sandbox (impl EC-6a pending). **EC-2 audit + gate decisions RECORDED July 8; the gate-ready pass-1 spec is §0.6** — ES-7 REFRAMED to "The Cost of Success" — mechanically unchanged (recurring income-skim + no-trust-bump + grace-turn erosion) but its **player-facing surface is now "endow the marshal with an ESTATE and a province-derived TITLE"** (e.g. "Endow Ney with the Duchy of Swabia" — the Domaine Extraordinaire framing; internal action id stays `grant_dotation`; ES-8 title *flavor* rides pass 1 for free, the ES-8 stat-bonus mechanic stays deferred): success raises a marshal's reward *expectation*; unmet/revoked expectation erodes loyalty via `modify_trust`; **paying stops the bleed, never buys trust**; explicitly OUT of the `modify_relationship` Jealousy graph; fires AI-side, GR5. Pass-1 = **ES-2 Occupation Upkeep + ES-7** matched pair, **ES-1** manpower fix as gate-free prereq; ES-4 → pass 2; **ES-10 CUT**. Build order = §0.6.3 (gate-free ES-1 re-key+retune / EC-6a toggle / ledger-GR8 fix, THEN the EC-2 pair). All balance numbers (band E1 + regen/upkeep/ES-7 constants E2–E6) ESCALATE to the EC-2 gate. **EC-5 self-cost = Option B / EC-7 (ES-6) timing = dated-trigger / sandbox soft-goal = open-ended all RESOLVED July 8 (cont.); only the blessed numbers remain open** (§0.6.6).
...
- **Economy Revisit** — v0.4 (July 8, 2026); **EC-2 gate-ready pass-1 spec in §0.6** (ES-2 + ES-7-reframed pair — ES-7 surface = "endow marshal with estate + province-derived title"; band + constants E1–E6 escalate). **Re-sequenced July 9, 2026: the Comprehensive Codebase Audit + a Fable-led econ eval (§0.7 — Fable's independent read of every economy rec) run BEFORE this gate; the eval's memo feeds the gate (recommends the blessed numbers; the user still blesses them).** EC-2 design gate = sign off §0.6.2 + numbers; EC-5 self-cost (=Option B) / EC-7 timing (=dated-trigger) / sandbox soft-goal (=open-ended) RESOLVED July 8 (cont.) — only the blessed numbers (E1–E6) remain at the gate (§0.6.6). EC-6 = sandbox (decided). `docs/ECONOMY_REVISIT_SPEC.md`
