...
- **Map Readiness Closure Pass — Phase 3 COMPLETE (§§3.1-3.4) + §4.1 COMPLETE.** Phase 3: §3.1 (nation config factory — `DEFAULT_NATION_DEFAULTS` fallback + `create_marshals_from_data` / `create_diplomat_from_data` factories; adding a new nation now only needs a capital + diplomat + optional override), §3.2 (shared topology endpoint — backend `GET /map_topology`), §3.3 (centralize nation colors), §3.4 (prompt/parser fallback de-hardcoding). §4.1: province registry schema v2 in `session8_placeholder_provinces.json` — every region carries `province_id`, four per-feature anchors (`unit_anchor` / `label_anchor` / `garrison_anchor` / `building_anchor`), and `wired` / `interactive` flags; `map_renderer_base.gd` parses new fields, gates hover/click on `interactive`, pre-filters `update_all_regions()` on `wired`, and now has direct guardrail tests for the non-interactive hover fallback plus unwired gameplay-state erasure. Remaining non-art map-readiness items are §§4.2-4.4. Next implementation slice: §4.2 external bitmap loading. See `docs/STATUS.md` and `docs/SCALE_READINESS_PLAN.md` §§3-4.
