...
- **Memory and Pressure v2.4.3 (Hegemony refactor + deep-audit fixes) — ACTIONABLE NOW.** Substrate shipped (betrayal_history, episode_id, witness scoping, hard-reject). v2.4 collapses static concern seed into per-turn hegemony engine reading bloc shares. v2.4.3 deep-audit fixes add: §7.1 recursive vassal-chain walk, §7.3 deterministic hegemon tie-break + non-France-hegemon guard, §7.2 drops runtime `nation_power_tiers` map (reads authored scenario record directly), §8.6.1a grievance-variant Make Amends, §8.8.7a existing-alliance terminates on defensive refusal, §9.3 conditional composite floor (-60 with DG-4), §11.1 Balance-of-Europe four-case state machine. Remaining: B-Hegemony (engine + bloc helpers + Balance of Europe headline) + B-B1-lite (collapsed acceptance formula) + B-B3 (paradox rename) + B-B7 (Make Amends) + trimmed C-lite presentation (~45-54 tests, ~1.5 sessions — down from v2.3 68-74 / 3 sessions). Merge gate: B-B4 must land AT or AFTER B-B1-lite so the composite-floor reintroduction catches the `grievance_modifier` stacking. See `docs/RELIABILITY_COMMITMENTS_SPEC.md` v2.4.3 + `RELIABILITY_IMPLEMENTATION_PLAN.md`.
...
| Memory and Pressure substrate (hegemony / betrayal memory / paradox / reliability) | `docs/RELIABILITY_COMMITMENTS_SPEC.md` (v2.4.3 — §8.8 holds the DG-4 call-to-arms episode contract, §8.6.1a authors the Make Amends grievance variant, §8.8.7a authors the existing-alliance termination on defensive refusal; `docs/SCALE_READINESS_PLAN.md` §DG-4 Amendment is the source of truth), `docs/RELIABILITY_IMPLEMENTATION_PLAN.md`, `diplomacy.py`, `world_state.py` (`betrayal_history`, `next_episode_id`), `commitments` logic within `diplomatic_templates.py`, `campaign_log.py`, `coalition.py` (hegemony engine when landed) |
...
| Memory and Pressure (substrate + presentation) | `docs/RELIABILITY_COMMITMENTS_SPEC.md` (v2.4.3), `RELIABILITY_IMPLEMENTATION_PLAN.md`, `COMMITMENTS_PRESENTATION_SPEC.md` (v0.5.1 C3-lite hegemony-aligned, non-normative bulk trimmed), `COMMITMENTS_PRESENTATION_DESIGNER_AUDIT.md` (historical) |
