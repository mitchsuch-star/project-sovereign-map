...
- **Map Readiness Closure Pass — §3.2 + warm-up bundle COMPLETE.** Phase 3 §3.2 (shared topology endpoint — backend `GET /map_topology`; Godot `map.gd` no longer hardcodes `REGION_CONNECTIONS`), §3.3 (centralize nation colors), and §3.4 (prompt/parser fallback de-hardcoding) are done. Remaining non-art map-readiness items are §3.1 (nation config factory), then Phase 4 map-art pipeline work. See `docs/STATUS.md` and `docs/SCALE_READINESS_PLAN.md` §§3-4.
