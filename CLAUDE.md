...
- **Imperial Settlement current handoff:** B2 battle emitter call-site wiring is landed and review-fixed. `war_contribution.detect_battle_theater(world, *, battle_region, attacker_nation, defender_nation, attacker_marshal_name, defender_marshal_name, war_id, attacker_pre_battle_strength, defender_pre_battle_strength)` resolves `war_id`, validates side membership, walks `world.marshals` once across the battle region plus one-hop adjacent regions, and preserves same-nation secondary marshal strength by replacing only the named primary combatant's post-battle strength with captured pre-battle strength. The three `record_diplo_battle` callers — `_post_combat_pipeline()` (glorious-charge / bombardment / garrison), `_execute_attack()` inline, and `WorldState._process_reckless_cavalry_turn_start()` — forward primary marshal names plus `war_id` / `attacker_participants` / `defender_participants` / `nation_theater_strength` into `record_battle()`. Tests cover helper multi-marshal stacking, source-order guards, glorious-charge, garrison, bombardment, theater-`None` fallback to the legacy adapter, distant-front exclusion, inline attack, and auto-dispatch charge. Verification: `9457 passed, 1 skipped`; ruff clean. **Remaining B2 work is non-battle emitters/support attribution: occupation events, support/treaty support, British coalition subsidy attribution, and dedupe fixtures. B3 lifecycle / retention / full-Europe fixtures come after that.**
...
- **Peace Deals - Phase A COMPLETE, Gate 1 PASSED. WB-A/WB-B/WB-C/WB-D COMPLETE. Imperial Settlement / Ally Participation Slices A1 + A2 + A3 + B1 + B2 ORDERING GUARD + B2 BATTLE EMITTER CALL-SITE WIRING LANDED; REMAINING B2 NON-BATTLE EMITTERS ARE NEXT.** B2 battle wiring review fixes preserve same-nation secondary marshal theater strength, cover glorious-charge / garrison / bombardment pipeline variants, inline attack, auto-dispatch charge, distant-front exclusion, and theater-None fallback. Verification: `9457 passed, 1 skipped`; ruff clean. Remaining B2 scope is occupation events, support/treaty-support ingestion, British coalition subsidy support attribution, and support dedupe fixtures. B3 lifecycle / retention / full-Europe fixtures follow after remaining B2 is green.
