...
- **Imperial Settlement current handoff:** B3 lifecycle and its review fix are landed. War-entry seams (`_create_skeleton_instance`, `attach_pair_to_war_instance`, `attach_participant_to_war_instance`) call `_open_contribution_episode_for_participant(...)` so every joiner gets a fresh episode at the current attach turn; re-entered participants whose previous episode was closed get a new episode_id at the re-entry turn (spec §7.5). Exit seams close episodes via `_close_contribution_episode_for_participant(...)`: elimination, separate peace, and all-pairs-resolved war end are wired. `cleanup_war_end(conclude_objectives=True)` drives PEACE episode closure through `resolve_pair_to_resolved(...)`; review follow-up `be93a32` also closes the debug `set_diplo_state` lifecycle seam so debug WAR to PEACE resolves pairs/closes episodes and debug WAR to ARMISTICE suspends the pair without closing episodes. Per-turn staying power is `accrue_staying_power_all_wars(world, current_turn=...)` before armistice expiration; each active episode gains `+5` raw points capped at 10 qualifying turns / 50 raw. Archive compaction moves terminal records into `world.archived_war_contribution_scores[war_id]` after the 10-turn retention window. Verification: `9522 passed, 1 skipped`; ruff clean. **Remaining: Slice C presentation/mechanics surface for the new lifecycle data - common-peace term legitimacy, settlement standing readers, dispatch / ledger surfaces, and paradox routing.**
