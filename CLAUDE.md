...
- **NEXT UP — Settlement Conversational Re-front (May 28, 2026) — DESIGN GATE, NEEDS APPROVAL, DO NOT CODE. Spec is v0.3 (twice-audited); awaiting user approval:** `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` is now **v0.3** (v0.2 resolved all seven §8 open questions; v0.3 folds a second independent audit run) and audited in `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_AUDIT.md` (audited twice: v0.2 verdict **GO-with-changes**, 4 items folded; v0.3 second independent run verdict **GO**, 6 more items folded — per-court call signature, Slice-1 fixture bound, ratify cite `:4776`→`:4583`, no-baseline floor, `balance_projection` rewire). **Immediate next action: USER APPROVAL of v0.3** — no implementation until approved. Gate 4 smoke surfaced that the SC-5R-2 editor diverged from the conversational bilateral-diplomacy pattern (only the rawest clause-assembly tier, a blank unguided form allowing invalid combos like self-referential `liberation` rejected only at Submit). The re-front makes common peace work like a bilateral peace proposal: Talleyrand proposes a valid baseline (Tier 1), you steer by intent (Harsher / More generous, whole-table or focused court — Tier 2) with **live per-court acceptance**, and the structured picker editor becomes the **opt-in valid-by-construction deep tier** (Tier 3, reached via `Adjust terms`) — extended to **multi-party** (a peace conference: one settlement covering a set of enemy courts, each scored independently). Mostly wiring existing machinery (`generate_suggested_terms`, `calculate_common_peace_acceptance` — which already accepts memoized score inputs, the concession-baseline generator, Voice Bible) **plus one deliberate, user-approved mechanics change**: the ratification gate moves from single-`accepting_leader` to **per-covered-court** (`carries` iff every covered court ≥ threshold; holdouts block but offer one-click Ease/Drop), chosen for UX + historical analogue (separate Napoleonic peaces; holdouts fight on). On approval → Slices 0–3 + a conference-voice slice (Voice Bible has no multi-court family yet); `DWL-SET-SC5R-3` inline merge-conflict controls fold into Tier 3 (Slice 3). The SC-5R-2 editor and the `suspend_settlement_editor` Back Out fix remain LANDED — this re-fronts the front door, it does not revert them. **Interim band-aid = Slice 0 (independent de-risk):** land the picker valid-by-construction contract that `SETTLEMENT_UI_CLEANUP_SPEC.md:601` already promised but never wired (`_build_clause_control_schema_for_review` hardcodes `enabled=True`; liberation falls back to all-nations at `:2959`; the line-618 test is absent). Slice G AI-ally agency stays separate and later.
...
- **Settlement Conversational Re-front** — **v0.3 finished + twice-audited (GO), NEEDS APPROVAL. DO NOT CODE.** `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` + audit `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_AUDIT.md` (next-up settlement priority; common peace as a conversational multi-party peace conference; per-court ratification gate; on approval → Slices 0–3 + conference voice)
...
| Peace Deals (umbrella + sub-specs) | **Next-up: `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` v0.3 + audit `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_AUDIT.md` (conversational multi-party re-front — DESIGN GATE, finished + twice-audited GO, needs user approval).** Current gate: `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.27 and `docs/STATUS.md`. Historical anchors: `docs/PEACE_DEALS_UMBRELLA_SPEC.md`, `BILATERAL_PEACE_HARDENING_SPEC.md`, `WAR_PURPOSE_SCORE_SEMANTICS_SPEC.md`, `WAR_BARGAIN_SPEC.md`, `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md`, and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md`. Treat older Slice B / Final Gate notes as historical unless STATUS points back to them. |
