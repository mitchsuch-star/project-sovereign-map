...
- **AUTHORITATIVE Imperial Settlement handoff (May 7, 2026):** G2-Slice-1 Foundation, G2-Slice-2 Entry Safety, and Codex audit fixes are landed on `codex/settlement-spec-readiness-tightening` through commit `32b868c` (`Fix settlement entry safety audit gaps`). Confidence is high for the backend foundation + entry-safety surface: canonical clause validation rejects empty drafts, unknown/alias keys, non-dict clauses, invalid actors/sides, conflicts, and >8 clauses; submitted `settlement_terms` are forwarded and revalidated before staging; `Ratify Settlement` is acceptance/hard-stop gated and rescored from live world state before mutation; `Revise Terms` is hidden/disabled until a real editor exists; Back Out clears persisted drafts; `pending_settlement_drafts` and `settlement_route_seq` serialize/deserialize; same-war same-turn route ids carry into reaction events; multi-war diplomacy-wizard and typed/free-text entry no longer auto-pick a war; war-status rows use active hostile-pair eligibility and fail closed on probe errors; rows expose `settlement_eligibility` plus disabled reasons for Godot; `selected_target_nation` is staged/reopened/executor-threaded and validated against covered enemy scope. Verification: `ruff check backend tests` clean, `tests/test_settlement_entry_safety_slice.py` `25 passed`, settlement foundation/UI behavior bundle `62 passed`, and full suite `9858 passed, 1 skipped`. This is not full Settlement UI Cleanup closure: no Slice G / AI-ally settlement agency yet. The next active task is **spec review only**, not coding: re-audit the new rejected-settlement / losing-side peace addition in `docs/SETTLEMENT_UI_CLEANUP_SPEC.md`, decide whether the design is correct, and look for similar settlement cleanup misses where UI exposes incomplete actions, hides strategic next steps, or conflates preview/revision/ratification/offer handling. Do not implement the next Gate 2 cleanup slice until that spec review reaches GO and updates `docs/STATUS.md` if needed. Do not follow the older Slice E / Final Gate paragraph below as active routing.

  Spec-review focus: blocked settlement confirmation must not collapse to a lone `Back Out`; losing a war must still permit peace-seeking agency; concession/surrender/ask-for-terms routes must be designed as treaty-authoring agency rather than passive waiting for enemy AI; and `Revise Terms`, armistice fallback, incoming offers, and ratification must remain distinct surfaces.
...
- **Imperial Settlement current handoff:** B3 lifecycle + Slice C1a/C1b + Slice C2 preview/ratification + Slice D1/D2 settlement & cross-war reaction routing + **Slice E presentation, ledger, and logs** are all landed. Slice E ships `backend/game_logic/settlement_presentation.py` (settlement-only `SETTLEMENT_ROUTES` distinct from `COMMITMENTS_ROUTES` per spec §11.6 line 1290; `is_settlement_event_visible` for the §11.6 fog rule; `cap_settlement_dispatch_lines` + dispatch-side `_enforce_settlement_primary_beat_cap` for the spec §11.6 line 1279 top-four cap with digest overflow; `apply_warning_cap` for spec §16.3; `build_settlement_review` for spec §16.2 Terms / Allies / Warnings / Acceptance sectioning at compact / medium / verbose density; `detect_awe_set_pieces` for spec §20.8; `settlement_notification_meta` for the rail; `recent_settlement_summaries` for the diplomatic ledger; `build_contribution_share_rows` for the war status panel). `dispatch.py` now routes `settlement_summary` / `settlement_digest` events through their own fog filter + formatter and enforces the per-ratification top-four cap on dispatch lines. `campaign_log.py` adds a settlement fog branch routed through the new presentation filter. `diplomatic_ledger.py` exposes `recent_settlements` in the ledger payload. `war_status.py` decorates each war entry with `contribution_share[]` + `contribution_overflow_count` + `war_instance_id`. Godot: `top_bar.gd` wires `ledger_settlements` into `open_diplomatic_ledger_review`; `diplomatic_ledger.gd` adds `open_to_settlements()` and a Recent Settlements block inside the Treaties tab (CanvasLayer 50, one-screen rule preserved); `main.gd` routes `settlement_review` / `diplomatic_ledger` / `ledger_settlements` review targets to the layer-50 ledger; `war_status_panel.gd` extends the war tooltip with the top-five standing list + overflow count. Diplomat Voice Bible §16.1 settlement families (`settlement_advisory_common_peace_*` / `settlement_advisory_defensive_*` / `settlement_acceptance_*` / `settlement_rejection_*` / `settlement_sold_out_by_leader_*` / `settlement_rewarded_ally_*` / `settlement_excluded_ally_*`) remain the contract for final committed copy authored inside the Final Gate. Verification: `tests/test_settlement_presentation.py` **`65 passed`**; settlement focused suite (presentation + reactions + projection + foundation + instances + merge + side_pressure + common_peace_acceptance + common_peace_harshness + war_status) **`304 passed`**; full suite **`9736 passed, 1 skipped`** (was `9671`); ruff clean across `backend/`. **Next: Final Gate** (`docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` line 400 — bundled `pytest tests/test_war_settlement_foundation.py tests/test_war_settlement_instances.py tests/test_war_contribution_scores.py tests/test_common_peace_acceptance.py tests/test_settlement_term_legitimacy.py tests/test_settlement_reactions.py tests/test_settlement_presentation.py` + WB/WPS/BPH companion bundle + manual Godot smoke on settlement review / war status / notification / ledger routes against current 19-region map and the synthetic full-Europe payload + per-cast voice copy passes against Voice Bible §16.1). **After Final Gate:** no Slice F is currently defined; broader closeout/release audit is next.
...
- **Peace Deals - Settlement UI Cleanup v0.19 coding is up next.** Earlier Imperial Settlement / Ally Participation slices through Slice E landed, but the current player-readiness gate is `docs/SETTLEMENT_UI_CLEANUP_SPEC.md`: full treaty authoring, edit/review mode controls, acceptance and hard-stop mutation gates, SC-4b resource-cost contract, route safety, incoming-offer hide-or-complete behavior, presentation/metadata cleanup, behavior tests, and manual smoke. Begin with Gate 1 doc scans/consolidation, then G2-Slice-1 Foundation. Slice G remains blocked until this cleanup is implemented, behavior-tested, and smoked.
...
| Peace Deals / Imperial Settlement | Current gate: `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.19, then `docs/STATUS.md`. Older Imperial Settlement plan/spec docs remain useful anchors, but current implementation starts with cleanup Gate 1 and G2-Slice-1 Foundation before any Slice G / AI-ally settlement agency work. |
