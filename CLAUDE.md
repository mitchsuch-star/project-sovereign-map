...
- **AUTHORITATIVE Imperial Settlement handoff (May 8, 2026, post-merge with codex/settlement-smoke-start):** Settlement UI Cleanup G2-Slice-1 through G2-Slice-5 are landed on this branch (`b699f4c`, `4310903`, `052c5d0`, `4884e93`, `b2a4d7b`) and the v0.22 spec/smoke-start preset/audit-fix work was merged in from `origin/codex/settlement-smoke-start` on May 8. Do not send implementers back to G2-Slice-1. The active gate is `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.22 plus the Deferred Work Landing Ledger, SC-27 scans, SC-28 / SC-28b rejected/losing recovery verification, post-merge full-suite + ruff re-run, and Gate 4 manual smoke. v0.22 resolves the rejected/losing-side contract as normative spec text: blocked ratification omits `confirm_settlement` from `options[]`, `Revise Terms` is absent unless it opens a real editor route, enemy-offer waiting remains hidden while SC-5 is unshipped, failed settlement review routes to `Open War Detail` rather than inventing a popup armistice action, valid below-threshold drafts can Submit into blocked REVIEW but cannot Ratify, and losing-side concession authoring is owned by SC-1 offer-mode on the standard treaty editor. Hidden settlement affordances are not open-ended: `Seek Armistice Instead` / `Seek Bilateral Peace` land in SC-29 / G2-Slice-7, `Wait for Enemy Offer` / `Ask for terms` land in SC-30 / Slice G1, `Surrender terms` lands in SC-31 / G2-Slice-8, and broader settlement agency lands or is explicitly removed in SC-32 / Slice G2. Slice G / AI-ally settlement agency remains blocked until v0.22 checks pass and Gate 4 smoke evidence, including rejected/losing concession smoke, is recorded. Do not follow older v0.17-v0.21 or Slice E / Final Gate paragraphs as active routing.
  ...
  Manual smoke shortcuts: set `SOVEREIGN_SMOKE_START=settlement_multilateral` before Gate 4 manual settlement UI smoke for the shared France vs Britain + Prussia `war_instance` with Britain as defender leader. Use `SOVEREIGN_SMOKE_START=settlement_losing` or `settlement_rejected` for the v0.22 blocked-popup, SC-28 / SC-28b recovery, scoped-draft, no-direct-pair-action, and concession path. Leave the env var unset for the canonical campaign start.
...
- **Historical Imperial Settlement Slice E handoff (superseded by v0.22 gate above):** B3 lifecycle + Slice C1a/C1b + Slice C2 preview/ratification + Slice D1/D2 settlement & cross-war reaction routing + **Slice E presentation, ledger, and logs** are all landed. Slice E ships `backend/game_logic/settlement_presentation.py` (settlement-only `SETTLEMENT_ROUTES` distinct from `COMMITMENTS_ROUTES` per spec §11.6 line 1290; `is_settlement_event_visible` for the §11.6 fog rule; `cap_settlement_dispatch_lines` + dispatch-side `_enforce_settlement_primary_beat_cap` for the spec §11.6 line 1279 top-four cap with digest overflow; `apply_warning_cap` for spec §16.3; `build_settlement_review` for spec §16.2 Terms / Allies / Warnings / Acceptance sectioning at compact / medium / verbose density; `detect_awe_set_pieces` for spec §20.8; `settlement_notification_meta` for the rail; `recent_settlement_summaries` for the diplomatic ledger; `build_contribution_share_rows` for the war status panel). `dispatch.py` now routes `settlement_summary` / `settlement_digest` events through their own fog filter + formatter and enforces the per-ratification top-four cap on dispatch lines. `campaign_log.py` adds a settlement fog branch routed through the new presentation filter. `diplomatic_ledger.py` exposes `recent_settlements` in the ledger payload. `war_status.py` decorates each war entry with `contribution_share[]` + `contribution_overflow_count` + `war_instance_id`. Godot: `top_bar.gd` wires `ledger_settlements` into `open_diplomatic_ledger_review`; `diplomatic_ledger.gd` adds `open_to_settlements()` and a Recent Settlements block inside the Treaties tab (CanvasLayer 50, one-screen rule preserved); `main.gd` routes `settlement_review` / `diplomatic_ledger` / `ledger_settlements` review targets to the layer-50 ledger; `war_status_panel.gd` extends the war tooltip with the top-five standing list + overflow count. Diplomat Voice Bible 16.1 settlement families remain the contract for final committed copy. Verification at that time: `tests/test_settlement_presentation.py` **`65 passed`**; settlement focused suite **`304 passed`**; full suite **`9736 passed, 1 skipped`**; ruff clean across `backend/`. Treat the old Final Gate routing in this paragraph as historical; the active route is v0.22 repair verification plus Gate 4 smoke.

- **G2-Slice-5 closure summary:** Closes SC-15 / SC-15b / SC-16 / SC-17 / SC-19 / SC-20 / SC-23 / SC-24 / SC-25 per spec v0.17. Backend additions in `backend/game_logic/settlement_presentation.py`: `build_applied_clauses_preview` (structural mutation rows by clause type with region/payer/recipient/amount/turns/`includes_continental_system`/`projected_threat_delta`/`pair_state_transition`), `build_beneficiaries_preview`, `build_shut_out_allies_preview`, `build_third_party_reaction_preview`, `build_forced_alliance_threat_preview_display` (consumes `compute_forced_alliance_threat_preview(...)` output via `component_debug.forced_alliance_threat_preview`), and `build_peace_settlement_history` (single reverse-chronological merged surface, `PEACE_HISTORY_DEFAULT_ROWS = 5`, `peace:` / `settlement:` distinct route-id namespaces). `build_settlement_review` exposes `applied_clauses_preview[]` / `beneficiaries_preview[]` / `shut_out_allies_preview[]` / `third_party_reaction_preview[]` / `forced_alliance_threat_preview` at top level; blocked acceptance payload sets `total=null` / `threshold=null` plus `band_display="Blocked"` and `blocker_display`. `build_settlement_preview` now plumbs `forced_alliance_threat_preview` and live `awe_tags` via `detect_awe_set_pieces(...)`. `_humanize_term` delegates to `clause_display_name` for known types; `war_scope_display` "Bilateral row" → "Bilateral war"; `talleyrand_text` resolves through `resolve_settlement_voice_line(...)` and contains zero raw acceptance verdict enums. `proposal_confirm_popup.gd` heading reads `talleyrand_text` directly; legacy `(phrase)` duplicate render is gone (SC-20). `diplomatic_ledger.py` exposes `peace_settlement_history`; `diplomatic_ledger.gd` renders the merged `PEACE & SETTLEMENT HISTORY` surface with type pills. SC-19 ships seven new committed voice families in `SETTLEMENT_VOICE_TEMPLATES` (`settlement_review_heading_talleyrand`, `settlement_observed_foreign_court_chancery`, `settlement_blocked_for_ratification_talleyrand`, `settlement_rescored_after_staging_talleyrand`, `settlement_discard_confirm_talleyrand`, `settlement_collision_active_review_talleyrand`, `settlement_reopen_cap_exhausted_talleyrand`). SC-24 ships `_record_common_peace_treaties` writing both `raw_harshness` (unclamped) and `clamped_harshness` plus `source="common_peace"`, with new `get_treaty_harshness_for_consumer(treaty, *, consumer=...)` selector returning raw for the five named common-peace consumers. Failed-ratification guard: `route_settlement_reactions(success=False, mutated=False)` short-circuits with `summary_event=None` and `skipped_reason="failed_ratification"`. `_emit_settlement_summary_event` carries `acceptance_snapshot` (fresh ratification-time payload) and `acceptance_at_staging` (audit context). Test surface: new `tests/test_settlement_ui_cleanup_g2_slice5.py` (29 behavior tests + required inversions for `debug_action_ids` / "Will %s accept this settlement?" / "Settlement payload incomplete" / `band_phrase` duplicate render). Verification: full suite **`9936 passed, 1 skipped`** (was `9904`, +32 net); `ruff check backend tests` clean. **Next:** Gate 4 manual Godot smoke → Slice G AI/ally settlement agency → Final Gate test bundle → 8.EVAL → Phase 8.5.

- **Imperial Settlement current handoff:** Slices A1-A3, B1-B3, C1a/C1b, C2, D1/D2, E, Slice F UI routing (commit `0b9289e`), Settlement UI Cleanup G2-Slice-1 (Foundation, commit `b699f4c`), G2-Slice-2 (Entry Safety, commit `4310903`), G2-Slice-3 (Continuity, May 7, 2026), G2-Slice-4 (Incoming Offers — default defer-and-hide, May 7, 2026), and G2-Slice-5 (Presentation & Metadata, May 7, 2026) are all landed.
...
- **Peace Deals - Settlement UI Cleanup v0.22 repair + Deferred Work Landing Ledger + Gate 4 smoke is up next.** G2-Slice-1 through G2-Slice-5 are landed on this branch and the v0.22 spec/smoke-start preset/audit-fix work was merged in from `origin/codex/settlement-smoke-start` on May 8. The remaining player-readiness gate is to consume the v0.22 contract in `docs/SETTLEMENT_UI_CLEANUP_SPEC.md`, verify SC-27 scans, verify the SC-29 / SC-30 / SC-31 / SC-32 landing ownership check, verify Voice Bible 16.1 anchors, re-run the full pytest + `ruff check backend tests` on the merged tree, then run Gate 4 manual smoke including the rejected/losing concession path. Slice G remains blocked until those checks and smoke evidence are recorded.
