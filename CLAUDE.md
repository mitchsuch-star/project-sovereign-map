...
- **Settlement Conversational Re-front** — **v0.4 finished + thrice-audited (3rd run NO-GO → all findings folded), NEEDS APPROVAL. DO NOT CODE.** `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` + audit `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_AUDIT.md` (next-up settlement priority; common peace as a conversational multi-party settlement table; per-court ratification gate; per-court baseline direction from `direct_scores`; SC-32 D5 no-conference-copy boundary; on approval → Slices 0–3 + multi-court voice)
...
| Peace Deals (umbrella + sub-specs) | **Next-up: `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` v0.4 + audit `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_AUDIT.md` (conversational multi-party re-front — DESIGN GATE, finished + thrice-audited, 3rd-run findings folded, needs user approval).** Current gate: `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.27 and `docs/STATUS.md`. Historical anchors: `docs/PEACE_DEALS_UMBRELLA_SPEC.md`, `BILATERAL_PEACE_HARDENING_SPEC.md`, `WAR_PURPOSE_SCORE_SEMANTICS_SPEC.md`, `WAR_BARGAIN_SPEC.md`, `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md`, and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md`. Treat older Slice B / Final Gate notes as historical unless STATUS points back to them. |
