...
- **Peace Deals — Phase A COMPLETE, Gate 1 PASSED. WB-A/WB-B/WB-C COMPLETE.** BPH-A through BPH-D + WPS-A through WPS-D all landed. Gate 1 verified (9/9 smoke criteria). `threat_coalition` retired (merged into `balance_of_europe`). WB-A (data model + creation + validation), WB-B (lifecycle: fulfillment + breach + void), WB-C (war-entry contract: `war_entry_score`, hard blocks, `join_opportunity`, declaration preview, counter-bargains, `repudiate_bargain`, AI rules, ledger integration) all landed with ~130 tests. Next: WB-D (presentation extension). See `docs/PEACE_DEALS_UMBRELLA_SPEC.md` + `docs/WAR_BARGAIN_SPEC.md`.
