...
- **Peace Deals — Phase A COMPLETE, Gate 1 PASSED. WB-A COMPLETE.** BPH-A through BPH-D + WPS-A through WPS-D all landed. Gate 1 verified (9/9 smoke criteria). `threat_coalition` retired (merged into `balance_of_europe`). WB-A (war bargains data model + creation + validation + acceptance modifiers + region_observer activation) landed with 23 tests. Next: WB-B (lifecycle: fulfillment + breach + void). See `docs/PEACE_DEALS_UMBRELLA_SPEC.md` + `docs/WAR_BARGAIN_SPEC.md`.
