...
- **Imperial Settlement current handoff:** B2 non-battle emitters and support attribution are landed with review fixes. Treaty support attribution now emits AP support only for AP actually removed from the payer, and treaty support event ids include the source clause index so duplicate same-type one-time/per-turn clauses accrue separately while same-turn replays remain idempotent. Wired call sites remain `world_state.capture_region()`, `world_state._ratify_treaty()`, `world_state._process_treaty_clauses()`, and `coalition._process_british_subsidy()`. Tests now cover duplicate `gold_lump`, duplicate `gold_per_turn`, AP-floor blocked payment, occupation events, support formulas/dedupe/caps, subsidy attribution branches, and treaty/capture end-to-end paths. Verification: `9493 passed, 1 skipped`; ruff clean. **Remaining: B3 lifecycle / per-turn staying-power / war-entry seam wiring of `open_episode` / `close_episode_for_exit` / archive compaction / retention / full-Europe fixtures.**
...
- **Peace Deals - Phase A COMPLETE, Gate 1 PASSED. WB-A/WB-B/WB-C/WB-D COMPLETE. Imperial Settlement / Ally Participation Slices A1 + A2 + A3 + B1 + B2 (ORDERING GUARD + BATTLE EMITTER WIRING + NON-BATTLE EMITTERS) LANDED; B3 LIFECYCLE IS NEXT.** B2 non-battle emitters cover occupation event accrual (capture path + treaty cede + vassal liberation), support event accrual (one-time + per-turn treaty clauses + British coalition subsidy), episode-id dedupe across the new occupation/support paths, duplicate same-type treaty clauses, and access/supply per-`(war_id, supporter, support_kind)` cap of 5 raw points. Verification: `9493 passed, 1 skipped`; ruff clean. B3 lifecycle / per-turn staying-power / war-entry seam open/close / archive compaction / retention / full-Europe fixtures follow next.
