...
- **Command Robustness (`COMMAND_ROBUSTNESS_SPEC.md` v0.2, ACTIVE — scope blessed July 3, 2026):** ~~CR-0~~ + ~~CR-1~~ **✅ LANDED July 3, 2026** — parser rosters live-world derived (all 7 French 1805 marshals commandable; `test_command_robustness_cr0_parser_rosters.py`) and the eval harness is the phase's standing regression gate (`tests/data/parser_golden_corpus.json` 226 entries + `backend/ai/parser_eval.py` engine/CLI + `test_command_robustness_cr1_eval_harness.py`; typed `status` wire connected; new-action checklist step 12 = corpus entry). **Next slice: CR-2 (clarification dialogue) → CR-3..CR-5.** CR-6 (conversational objection negotiation) needs its own design gate; spec §4 parks the gate-review candidates.
...
- **Command Robustness** — v0.1 DRAFTED July 2, 2026; scope blessing needed; CR-6 needs a dedicated gate. `docs/COMMAND_ROBUSTNESS_SPEC.md`
