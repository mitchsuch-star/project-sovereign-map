...
- **Imperial Settlement current handoff:** B3 lifecycle + Slice C1a/C1b + Slice C2 preview/ratification + **Slice D1/D2 settlement & cross-war reaction routing** are landed and re-audited GO. D1/D2 now routes settlement reactions after cache invalidation and before dialogue pop, stores/prunes `settlement_memories`, writes proposer-side `they_chose_us` / material-gated `settlement_gratitude` / `settlement_shut_out` grievance flags, writes enemy-side `sold_out_by_war_leader`, routes active French WB claim breaches, scans cross-war fallout via `world.get_war_instances_by_participant(...)`, emits `settlement_summary` / `settlement_digest`, and wires `settlement_gratitude_mod` into `calculate_acceptance` after `political_subtotal_clamped` and before `deal_balance`. Audit-fix commit `5a9614d` closed expired-gratitude, over-broad affected-nation, non-French bargain breach, and invalidation-ordering test gaps. Verification: `tests/test_settlement_reactions_d1_d2.py` **`33 passed`**; full suite **`9671 passed, 1 skipped`**; ruff clean. **Next: Slice E presentation, ledger, and logs** (`dispatch.py`, `campaign_log.py`, `diplomatic_ledger.py`, Godot settlement review / war status / notification / ledger routes, settlement voice copy, warning caps, top-four dispatch beats plus digest overflow). **After E:** no Slice F is defined; run the implementation-plan Final Gate before broader closeout/release audit.
...
- **Peace Deals - Phase A COMPLETE, Gate 1 PASSED. WB-A/WB-B/WB-C/WB-D COMPLETE. Imperial Settlement / Ally Participation Slices A1 + A2 + A3 + B1 + B2 + B3 + C1a + C1b + C2 + D1/D2 LANDED; SLICE E IS NEXT.** Current gate is presentation, ledger, logs, and Godot surfacing for common-peace settlement review and reaction aftermath. After Slice E, run the Final Gate in `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md`; no Slice F is currently defined.
