...
> **▶ LIVE STATE (September 26, 2026). Everything below this block is historical — the bullets that follow are per-phase records kept for detail, not a to-do list.**
>
> **▶ ▶ ▶ ROW SR CHUNK 1 "PARIS MUST BE REACHABLE" — IN PROGRESS September 26, 2026: ~~SR-1a~~ ✅ + ~~SR-1b~~ ✅ LANDED** (landing records `SCORE_MANDATE_PLAN.md` §2 Chunk 1; rules `SYSTEMS_REFERENCE.md` §70): **status quo is a cession** — `game_end.title_status_quo_retentions` at the ONE state setter on every SIGNED road (`SIGNED_PEACE_REASONS`), both directions, a `retained` treaty record that counts and breaks like a signature but is NEVER reconciled (the ceder's designs still covet it — recognition is bought at the table); the AAR's four Vienna provinces title at the turn-9 peace (35 → 39 the same turn); **the client's war is the lord's war** — `diplomacy.follow_the_lord` from the four treaty roads (never the setter), both directions, a client's truce thawing or collapsing with the lord's, the surface naming who follows, and (AAR-28) an incoming offer's paradox a WARNING that says what it does; quick win **AAR-12** (the last admin spend leaves the day to the player). Four-arm attribution `tools/_sr1_series_arms.py`: `BASELINE_SERIES` byte-identical with the counts measured (one retention the AI cannot read at the Britain–Spain exit; the cascade called twice, moving nothing); M1–M7 byte-identical; one pin consciously flipped (the Tilsit carve's Silesia is a retained title). ~~**SR-1c**~~ ✅ LANDED the same day — the gate line teaches the road: `game_end.title_roads` (quiet / peace / loyalty / signature, each with its turn) on the CONGRESS tab, the Territories tab, the war room, the desk and the Moniteur's near-miss column; the alarm term names its decay; the blocker and the counsel name the DP price of the road they recommend (`diplomatic_price_quote`); rules §70.4, pins `test_sr1c_the_gate_line_teaches_the_road.py` 20; parse harness EXIT=0, boot 0 SCRIPT ERROR. ~~**SR-1d**~~ ✅ LANDED the same day — PR-D1b as ruled (`coalition_break_ranks_reason` + `league_offer_gate` on the offering leader, producer loop + both `request_terms` checkpoints, mediation and the player's proposal untouched; the `long_war` limb found dead and recorded): **the league's offer t4 → t9 / t10 / t11, France 28 / 25 / 28 at turn 40** on the commanded arm (`prd1b-cmd-*`); six arms byte-identical; the four pin families re-seated as ruled (the driver gains `--lever MODULE:NAME=0|1`); **PB-7** eight rows closed with evidence, CA9-F3 measured still open on the wire → SR-9, every ownerless row owned by pillar; rules §70.5–§70.6, pins `test_sr1d_the_league_treats_when_spent.py` 30. ~~The reserve~~ ✅ DRAWN (AAR-12, AAR-15 a truce told as a truce, the GE-V §4 price nits, the School's card XVII "The Congress of Paris" — 18 → 19; rules §70.3 + §70.7). ~~**SR-1e**~~ ✅ RUN — the three roads re-driven SCRIPTED from the archived typed orders (the chunked hand-play was not possible this session; said in the record): **the AAR road 37 at turn 40, opening A 31 (three titling on turn 44), opening B 39 at turn 9 → 16 in its unattended tail — no road reaches 45; 45 is NOT moved; the §68.6 levers are SR-D3's first questions with the numbers** (plan §4 Q0); archives `sr1e-*`, probe `tools/sr1e_titled_probe.py`, the xfail reads all five roads. **CHUNK 1 CLOSED — re-scored FOR USER CONFIRMATION: the ending 5.5 → 6.5 (target NOT met; residue SR-D3), diplomacy 6.0 → 6.25, directional ≈7.0. NEXT = Chunk 2 DIPLOMACY, or SR-D3's gate first (the user's call — §4 slots it right after Chunk 1 when SR-1e stops short).**
>
> **▶ ▶ ▶ NEXT = ROW SR "THE SCORE MANDATE" — `docs/SCORE_MANDATE_PLAN.md` IS THE ROUTING AUTHORITY (RULED September 26, 2026, by the user's direction; the "NEXT = UPDATE 1" line in the EP paragraph below is SUPERSEDED — Updates 1–5 are absorbed by category, §6).** The mandate: every pillar rises and none falls, weakest first (the ending 5.5, diplomacy 6.0, then the 6.5s), chunks by category with 30% of each reserved for quick wins from the standing bank, player reports still jump the queue, nothing under §4 built before its gate. **Chunk 1 = THE ROAD TO THE CONGRESS** (the reach was a parked "user's gate" line and is now a slice): SR-1a status quo is a cession · SR-1b the client's war is the lord's war (AAR-1) · SR-1c the gate line teaches the road and the DP price of the counsel · SR-1d PR-D1b + PB-7 · SR-1e the re-measure against the standing xfail, with the §68.6 levers handed to the AP/DP gate by name if a played road still stops short. Then 2 DIPLOMACY → 3 FIRST CONTACT & COMMAND → 4 COMBAT LEGIBILITY & MARSHAL DRAMA → 5 ECONOMY & NAVAL → 6 NARRATION → 7 AI ALIVENESS & LIVING BALANCE → 8 VASSALS & AGENDAS → 9 UI/UX → 3b the rest of the CRT queue → the ROADMAP spine unchanged. **§4 NEEDS DESIGN, after the chunks: SR-D1 Technology ("reforms, not research"), SR-D2 Asymmetry & national flavor (three doctrines first, ruled with HC-6), SR-D3 the AP and DP gaps.** The scoreboard (§1) is re-scored at every chunk exit; done when every pillar meets its target and the directional reads ≥ 7.5. **PROGRESS: ~~Chunk 1~~ ✅ CLOSED September 26, 2026 (commits `759414f9` + `f023d0b3`: SR-1a status quo is a cession, SR-1b the client's war is the lord's war, SR-1c the gate line teaches the road, SR-1d PR-D1b + PB-7, the reserve, SR-1e the re-measure — the best re-driven road peaks at 39 of 45 and erodes, so 45 is NOT moved and the §68.6 levers are SR-D3's Q0 by name; exit FOR USER CONFIRMATION: the ending 5.5 → 6.5, diplomacy 6.0 → 6.25). Chunk 2 IN PROGRESS: ~~SR-2a~~ ✅ LANDED September 26 (AAR-2 the table seats the covered leader and the per-court table is the gate; AAR-3 the revision route carries consent and the letter stands; AAR-7 one harsh transform + the collapsed menu; AAR-26 the petition's derived arm, routed refusal and expiry; AAR-27 the live-pair label — rules `SYSTEMS_REFERENCE.md` §71.1, pins `test_sr2a_one_verdict_per_screen.py` 47); ~~SR-2b~~ ✅ LANDED September 26 (the recall words, the nation list claims only the court, the Cabinet's rules on the typed road — rules §71.2, pins `test_sr2b_the_talleyrand_verbs.py` 26); ~~SR-2c~~ ✅ LANDED September 26 (WO-32 a refused arm keeps the decision; PR-D1c the spend is the war's own; PR-D1d lever (c) — a war France declared is gated on its leader's break-ranks clause — rules §71.3, pins the two files the rows name + `test_sr2c_a_refusal_keeps_the_decision.py`); ~~the Chunk 2 reserve~~ ✅ DRAWN September 26 (AAR-22 / AAR-16 / AAR-30 / AAR-18 / AAR-14 / AAR-13 — rules §71.4, pins `test_sr2_quick_wins.py` 32); ~~the Chunk 2 EXIT re-score~~ ✅ RUN September 26 — **CHUNK 2 CLOSED** (memo `docs/audits/SR_CHUNK2_EXIT_2026_09_26.md`: the table read at the wire as victor and as loser, a mediation answered on the Tilsit arm, Austria courted to an alliance; **diplomacy 6.25 → 6.75 FOR USER CONFIRMATION, target NOT met — residue SR-2d = SR-2-X2..X5**; directional ≈7.0); ~~SR-2d~~ ✅ **LANDED September 26** at the head of Chunk 3 (the letter subtracts what it carves; the PL-14 net names a ratified settlement off the ratifier's own feedback + the letter's rail row leaves with the letter; X4 NOT REPRODUCED — Δ = 0 on three routes, the exit's two figures were two runs' letters — closed with its invariant pinned; the Arbiter's Offer names its mediator on the rail, the dispatch event, the mailbox and the review — rules `SYSTEMS_REFERENCE.md` §71.5, pins `test_sr2d_the_letter_tells_the_truth.py` 26, sweep 18/18); **Chunk 3 IN PROGRESS: ~~SR-3a part (i)~~ ✅ LANDED September 26 — CRT-3 "a question never orders" (the Cabinet reads the shared question verdict + the client door mirrors the subject rule; the comma tail stands down only on an order verb; a hedge is a question; a leading run of up to six non-order words hides no deliberative question; the interjections are never an address; a court is a subject) + CX5-L5-F2 (the retreat noun takes a possessive) + CQ-30 (a near miss of a commander's surname asks fog-honestly) — landing record `COMMAND_ROBUSTNESS_SPEC.md` §12.8, rules `SYSTEMS_REFERENCE.md` §72.1, pins 72 + 21 + 13, sweep 18/18, ONE `.gd` (parse EXIT=0, boot 0 SCRIPT ERROR); two pins flipped consciously (IQ7-X7's `maybe accept`, the door mirror); ~~**SR-3a part (ii)**~~ ✅ LANDED September 26 — CRT-7 "the desk answers what the order would do" + AAR-17 / AAR-19 / AAR-23 / AAR-29 / AAR-31 (every desk answer asks the seam that would refuse or charge the order: seven new desk kinds off the war banner / `are_allies` / the intel store / the truce's clock / the dispatch, the what-if fog-honest and refusing like the order, the price as `recruit_quote`, the income sentence built from `NET_GOLD_COMPONENTS`, who-is-winning off `build_active_wars`, the router on whole words, the counsel reading the action points / CN-4's refusals / the crossing gate and spreading its orders, the insist arm's price on the button, Berthier's live reply read back through the fast parser, the strategic objection naming its order, our own fallen answered; rider: the truce refusal's "turns remaining" read a never-decremented cooldown — landing record `COMMAND_ROBUSTNESS_SPEC.md` §12.9, rules `SYSTEMS_REFERENCE.md` §72.2, pins `test_crt7_the_desk_reads_the_order.py` 110, sweep 39/39, ONE `.gd`, two pins re-seated consciously — the CX-2 levy line on the quote, the session-3 armistice refusal on the clock; **SR-3a COMPLETE**); ~~**SR-3b**~~ ✅ LANDED September 26 — CRT-2 "the name is never replaced" (CQ-17 an addressed reward goes to its OBJECT, a fallen object answered with the desk's sentence; CQ-29 accents never hide a name, the named ground of recruit / build / repair is kept, the executor reads an unresolved province through the march road's region matcher and discloses the reading on every answer — never the capital; rider: a hold the game placed claims no reading) — landing record `COMMAND_ROBUSTNESS_SPEC.md` §12.10, rules `SYSTEMS_REFERENCE.md` §72.3, pins `test_crt2_the_name_is_never_replaced.py` +49 (62), corpus 791/791, sweep 20/20, zero `.gd`; CQ-37 → reserve, CQ-38 → CRT-11; ~~**SR-3c**~~ ✅ LANDED September 26 — L-1 "the prompt turns around" (the parse prompt static first: the same order one battle apart shares 84 → 15,771 characters, ≥ 15,921 over twelve turns once the examples were made board-independent; lever down = the shipped prompt byte for byte, attributing the re-stamp of 16 IQ-9 cassettes; the addressee params wired) + IQ9-X1 (the retry's live reading of the marshal stands) + IQ9-X3 (a failure names its road) — landing record `COMMAND_ROBUSTNESS_SPEC.md` §12.11, rules `SYSTEMS_REFERENCE.md` §72.4, pins 29 + 14, sweep 18/18, zero `.gd`; prompt caching stays OFF (its obstacle is gone — the user's call); **SR-3 COMPLETE**; ~~**the Chunk 3 reserve**~~ ✅ DRAWN September 26 — the boot help teaches the desk ("who am I fighting", "is Paris safe", "what happened last turn", free); CQ-37 ("at once" is the adverb in both condition readers; "hold fast / firm / steady" a bare hold); AAR-25 (the counter-punch announced the morning it opens, every word the executor's — the foe in reach, `unfortify_is_free` the one source for the price of breaking camp, silent for a man locked in drill); IQ10-X closed on evidence — rules `SYSTEMS_REFERENCE.md` §72.5, pins `test_sr3_quick_wins.py` 34, sweep 22/22, TWO `.gd` (parse EXIT=0, boot 0 SCRIPT ERROR); ~~NEXT = the Chunk 3 exit re-score~~ **⚑ RE-RULED September 26, 2026 (evening), docs only** (`SCORE_MANDATE_PLAN.md` header/§4/§5): SR-D3's premise corrected (the AI pays action points; its rosters are 1–3 corps against 2–4 actions, France's 8 against 4) and RULED (c) first — **SR-2e "standing orders reliable"** (SUPPORT at 1 action, a march keeps its tail through a reinforcement, SR-4b folded in); no free AP scaling, new points only through SR-D1's Staff reform on both boards; **L-D APPROVED**; SR-D1 → Chunk 5's gate, SR-D2 (+ HC-6) → Chunk 7's; measurement = ONE exit per session + ONE residue slice, **§1 re-scored ONCE, fully, at the END of the mandate** (the Chunk 3 exit re-score was NOT run); **NEXT = SR-2e → L-D → onward → one session exit + one residue slice; no re-score until the end.**
>
> **▶ ▶ ▶ (RECORD) ROW EP "THE ENDGAME PROGRAM" — `docs/ENDGAME_PLAN.md` was the routing authority until September 26, 2026; ~~first slice F1~~ ✅ F1 "The first ten minutes" LANDED September 23, 2026 → ~~F3 "The client layout pass" + F4 "The fuse is longer"~~ ✅ LANDED September 24, 2026 (taken ahead of F2 by the user's direction; landing records `ENDGAME_PLAN.md` §1 F3 / §1 F4, rules `SYSTEMS_REFERENCE.md` §59 + §60) → ~~F2 "The display-name pass" + F5 "Bohemia is not empty" + F6 "Settled once, reopened"~~ ✅ LANDED September 25, 2026 in one commit (landing records `ENDGAME_PLAN.md` §1 F2 / §1 F5 / §1 F6, rules `SYSTEMS_REFERENCE.md` §61–§63; **the six fixes F1–F6 are ALL LANDED and all 22 LV rows are CLOSED**) → ~~GE-1 "The Verdict and the Fall"~~ ✅ LANDED September 25, 2026 (landing record `ENDGAME_PLAN.md` §6 GE-1, rules `SYSTEMS_REFERENCE.md` §64; **with the user's three September 25 asks: the Emperor's death as a third defeat arm "The Eagle Falls" (a seeded 15% roll at the ONE removal seam, battlefield causes only), the generals' death-odds memo `docs/audits/GENERALS_DEATH_ODDS_2026_09_25.md` (recommend-only — GE-D1 awaits the user's ruling), and the exile story `game_end.build_exile_story` on the Fall's summary**; **its adversarial review round landed the same day — 38 surviving findings, 4 P1, all fixed and pinned (`tests/test_ge1_review_round.py`, rules §64.1): the war is closed at ONE seam (`game_end.close_campaign` at `build_base_response` — the popup/typed/objection death roads had ended the war silently with no Final save), the captor's terms no longer overwritten by P1, a ticking clock outranks a paused one, a renewed war breaks a signed title, no court at peace holds the Emperor**; **and a verification round on those fixes — 27 more, all fixed (`tests/test_ge1_verification_round.py`, rules §64.2): no question after the Fall on any road, a truce on its last turn is a dated ticking clock, no court OUT OF WAR holds the Emperor (a vassal treaty or release frees him at the setter), every title record carries its `house` so only the signatories unsign it and a carve is a treaty record, a Fall is always the eclipse**) → ~~GE-2 "the client"~~ ✅ LANDED September 25, 2026 (landing record `ENDGAME_PLAN.md` §6 GE-2, rules `SYSTEMS_REFERENCE.md` §65, pins `tests/test_ge2_the_client.py` 60 DRIVEN on the real `main.tscn`: **`campaign_end.tscn` (layer 122) — ONE scene, FOUR registers, ONE payload** (the crimson Fall with THE EXILE / THE FUNERAL, THE RECORD and the eclipse and Load / Main Menu with the command line closed for good; the crimson-grey Humbled Peace and the parchment Verdict with Continue; the gold Imperial Peace declared now as `game_end.CAUSE_IMPERIAL_PEACE` so GE-3's stamp needs no client change); **raised from every road by the NA-6b stash-and-raise** — `api_client.response_received` sees every 200-OK body before its handler, a cause named only in `game_state.endings` (the Humbled Peace on the ratify road) is fetched off `GET /campaign_end`, the six `game_over` roads end at ONE seam, each cause once per session, a world swap adopts marked endings as history and keeps a terminal one so `/load` of a Final save lands on the card; **the command-road death (GE1-RV10) FIXED** — `Napoleon, attack Mack` at 100 raises the funeral register from the `/command` response, pinned driven; the legacy text de-legacied and kept as the scrollback record + fallback; **the clock line from ONE source `fall.clock_line`** on the end-turn banner, the R screen, the Territories tab (`fall_clock`) and the war room — one line per HELD arm, a paused clock names no date, `severity` decided once; **the driver** prints the END SCREEN's blocks under each ending, `--stop-on-ending`, two STAGED fixtures, **the four arms archived** (`ge2-eagle-falls` Fall t31 funeral · `ge2-chains` Fall t10 · `ge2-soil-or-sword` Fall t5 · `ge2-verdict` Verdict t44 contested); **GE-D2 measured** (the Eagle-Falls arm dies with no Guard question asked — recorded, the floor not built, the user's ruling); 8 IQ-10 frames × 2 scales dated 2026_09_25; parse harness EXIT=0, 0 SCRIPT ERROR, sweep 23/23 0 INERT at close, `BASELINE_SERIES` + M1–M7 byte-identical, two pins consciously flipped — the GE-1 driver-note count and the FA-94 ESC census) → → ~~GE-3 "The Congress of Paris"~~ ✅ LANDED September 25, 2026 (landing record + review-round addendum `ENDGAME_PLAN.md` §6 GE-3, rules `SYSTEMS_REFERENCE.md` §66, pins `tests/test_congress_of_paris.py` 117 + `tests/test_congress_review_round.py` 118 + `tests/test_ge3_the_client.py` 9: `backend/game_logic/congress.py` with ONE serialized `world.congress`; the summons at 50 TITLED provinces — 2 DP + 1 admin action and every rente ×1.5 from that end turn; the public recognition table, every court's stance, reason and PRICE (each lever the formula's own move, applied); the eight-turn sitting under the seven-condition HOLD with refusal's teeth — intent, the coalition gate at 40, the War of the Congress via `coalition.join_coalition` only where `march_blocker` is empty, London's purse — and the bills; THE IMPERIAL PEACE through GE-2's declared register, else the dissolution; E1 the Universal Monarchy; the CONGRESS ledger tab (key 7), the wizard row, the war room, the clock line on three surfaces, the beats, the Moniteur column. **Arms staged and DRIVEN: Pressburg wins on turn 36 on 4 of 4 seeds, Premature dissolves on turns 4–6 on 3 of 3.** A 69-finding review round, all fixed — the P1s: only a peace signed while it sits, or a BEATEN court's, latches recognition; a court that answers the Congress or holds a truce is never marched while it sits; `SIEGE_ENDS_WITH_THE_WAR` (every nation); every pressure reader asks the LIVE answer; the summons is read only at a line's head. `BASELINE_SERIES` + M1–M7 byte-identical, NO re-record (the arms count 0 Congress writes). ⚠ open for the user: the Congress surfaces' feel, GE-D1, GE-D2) → ~~GE-V "the played campaign"~~ ✅ RUN September 25, 2026 (landing record `ENDGAME_PLAN.md` §6 after GE-3's; memo `docs/audits/GE_V_PLAYED_CAMPAIGN_2026_09_25.md`; rules `SYSTEMS_REFERENCE.md` §66 (45) + §67; pins `tests/test_gev_played_campaign.py` 31 + `tests/test_ge_d1_generals_mortality.py` 22; 17 archived digests `gev-*`: **the reach from the boot is NOT on the road** — two hand-played openings and three scripted arms all stalled at 35 titled by turn 12 (best 41), so §2.8's rule fired and **`hold_titled` is 45**; the blocker is the campaign layer (six measured mechanics — the muster preview overstates arrival by ~⅓, four AP for eight corps, the supply cap, the marshals' own systems, 3–5k landings, satellite bleed) → **GEV-D1 "The Road to Forty-Five"**, the user's ruling; the table/prices/sitting confirmed on the driven arms + frames; **GEV-1 P1 fixed** — `vassalize Austria` on turn 1 with no battle subjugated a great power (`vassal.subjugation_refusal`: beaten = capital held · war score ≤ −40 · no corps standing; the signed clause bypasses at the ratify seam) + GEV-2/3/4; **GE-D2 RULED YES + built** (`GUARD_SPENT_FLOOR` 1,000 — the Emperor is asked before the fatal battle) and **GE-D1 RULED (a) + built as VP-M1 "The Fortunes of War"** (`fortunes_of_war.py`: the losing lead of a real battle at ≥ 25% lost rolls once through the campaign seed, killed 1% / wounded 8% for 3 turns on ONE field `wounded_until_turn`, a death passes the men through the ONE removal seam, both boards, log 167 → 168 flipped consciously); `BASELINE_SERIES` byte-identical with the lever up, M1–M7 byte-identical; **the ending re-scored 3.0 → 5.5, ≥ 7 NOT met**) → ~~VP-R1 "The Road to Forty-Five"~~ ✅ LANDED September 25, 2026 (the GEV-D1 follow-on, taken ahead of the build by the user's direction; landing record `DESIGN_REFINEMENT.md` GEV-D1, probes memo `docs/audits/VP_R1_PROBES_2026_09_25.md`, rules `SYSTEMS_REFERENCE.md` §68, pins `tests/test_vp_r1_the_road_to_forty_five.py`: **three probes before any code, two overriding the brief** — the muster's committed figure was already the honest expectation (74 of 74 within 20% of the Monte-Carlo mean; the label and the rows lied — 35 of 100 strikes printed WILL JOIN for a corps priced at 0%), the 60% shut-out was unreachable from play (**`cs_shutout_pct` 60 → 50 moved first**), and two of three autonomous glory charges fired below the 0.7 floor (a defect); **built behind levers** — (a) the preview names the expected arrival and every row's odds with the SUPPORT lever named at zero, (b) a raiding party under 5,000 holds no homeland at the walk-in / the attack's undefended exit / the landing / the AI's rungs (`BASELINE_SERIES` re-recorded ONCE, this lever the sole mover; the passive France ends turn 40 with 17 provinces instead of 4), (c) the glory attack obeys the odds on both boards, (d) the objection's trust option says "attack Archduke Charles at Tyrol"; **the re-measure did NOT reach 45** (B 36 at turn 40; A the Fall) — the number is NOT moved a second time, **the deeper gate is the user's: four action points for eight corps, the 37,500 supply cap, the capture cascade, the accepting dial** (§68.6); VP-R1-X1 filed) → ~~THE RELEASE BUILD (ROADMAP 10)~~ ✅ **LANDED September 25, 2026** (one commit; landing record `ROADMAP.md` position 10, rules `SYSTEMS_REFERENCE.md` §69, pins `tests/test_release_build_2026_09_25.py`: `deploy/build.bat` stamps, freezes, SMOKES the frozen server — it plays a turn — exports RELEASE, verifies the pack and zips; the zip `deploy/dist/ink_iron_<stamp>.zip` is on the dev PC for the user's itch.io upload; PB-1 … PB-6 FIXED; the parked patch and stash retired) → **▶ NEXT = UPDATE 1 (PR-D1b + the PB-7 doc housekeeping; build note `docs/NEXT_SESSION_PROMPT.md`).** (F2 = `display_names.plural` + `Utils.plural` retire the `(s)` hedge on both sides under census pins; the humaniser at the scout/scan/covering/muster seams and `Utils.display_marshal_name` on the diorama and the piece label; printed treaty states and courts, the incoming envoy without hints, `command_display` + int `turns_remaining` on every report row, the advance toll beside the locked figure, the defence line split into labelled rows, one arrow, " (was X)" on both capture branches; **the first cut MOVED the series** — a function-local import of the humaniser inside `_execute_attack` shadowed the module-level name and raised on every AI attack against a just-retreated corps; six local imports removed, AST-pinned; `combat.py` + `ledger.py` stay NPC-12's. F5 = one walk-in per corps per turn + the literal takes the cautious check — measured, Bavaria takes at most one Austrian province on turn 1 on three seeds; **`BASELINE_SERIES` re-recorded ONCE, four-arm attributed** (arm 0 byte-identical; cap alone diverges at 7, check alone at 13); the blocker names the unbeaten courts and their scores and the court to press alone, and a covered court at ≥ 50 gets "Separate peace with <court>" routed to the pair-substitute tier — pinned through the scorer seam, no shipped board reaches 50; frame `IQ10_SETTLEMENT_THREE_COURTS_2026_09_25.png`. F6 = the review's contradiction was two grievances — the battle settled one by action and the pass re-fired the pair the same turn; the card now opens "Settled once at Tyrol — and reopened by Ney's laurels since." off the `jealousy_resolved` event's new `location` (all three resolver seams, AST-pinned), a stale card for the pair is replaced; zero series movement. Gates: sweep 33/33 0 INERT at close, parse harness EXIT=0, 0 SCRIPT ERROR, M1–M7 byte-identical; tests 49 + 28 + 10.) (F4 = the reward curve is DEED-keyed: a decisive victory as LEAD — `dotation.is_decisive_victory`, the war score's own exchange test now named `battle_scale.is_decisive_exchange` — or an earned glory-RANK rise, one rise per marshal per 4 turns, none before turn 6, the collective petition at turn ≥ 12 and ≥ 300g, the UNMET block within 2 turns of erosion; ONE write `dotation.raise_expectation` both sides pass; three serialized fields `expectation_steps` / `last_expectation_rise_turn` / `glory_rank_seen`, pre-F4 saves backfilled from `battles_won`; **measured on the review's own board: first rise Lannes turn 7, no petition in 14 turns**; `BASELINE_SERIES` byte-identical across a SEVEN-arm attribution with the reason measured — 8 AI grants + 4 petitions on the ambient board became 0 + 0 and none reaches a threat-bearing decision — so NO re-record; 111 fixtures in 22 files rewritten `battles_won` → `expectation_steps`. F3 = LV-5 the petition's closed arm under the header + the body fits; LV-14(b) one Press/Ease/Drop row per court; LV-15 wrapping chips, reason on its own line, no sideways scroll; LV-16 glyph prefixes; LV-20 one-line step-1 prompt + the honest Sponsor copy (hidden while at war with a court aimed at us); LV-22 the capture route is `result_first` and `_display_result` stamps `_result_rendered` (driven: report once, message once, then the question, both roads); LV-D3 the recap modal RETIRED — terminal block + MARSHAL STATUS ETA from `strategic.order_turns_remaining` / `order_eta_phrase`; six IQ-10 frames `docs/audits/IQ10_*_2026_09_24.png` clean at both scales; `tools/ep_f3_client_layout_harness.gd` drives the real `main.tscn`; tests `test_ep_f3_the_client_layout_pass.py` 36 + `test_ep_f4_the_fuse_is_longer.py` 34.) (F1 = LV-1 a campaign is born with a turn-1 briefing — `build_morning_dispatch(world, boot=True)`, pure, carried by `/new_game` + `/load`, printed with the help after every world swap, with a TODAY section of orders the board takes; LV-12 a modal the command did not ask for waits behind its result (`result_first` routes); LV-13 one fog sentence; LV-7 "The courts at war hold 28 regions."; LV-8 the homeland purpose is one sentence from `war_status.objective_target_summary`; LV-22 filed → F3; landing record `ENDGAME_PLAN.md` §1 F1, rules `SYSTEMS_REFERENCE.md` §56, pins `tests/test_ep_f1_the_first_ten_minutes.py` driven through `tools/ep_f1_first_ten_minutes_harness.gd`.) RULED September 23, 2026 (evening) under the user's grant (*"make all decisions … a challenge to force a new status quo … hold the win state for X turns with a mechanic that makes it fun … start with fixes, end with end state"*): **F1 → F6** (every live-review row decided — the first-ten-minutes fixes, the display-name pass, the client layout pass, the reward curve's longer fuse, Bavaria's walk-ins capped, the legitimacy sentence, the vindication contradiction) **then GE-1 → GE-2 → GE-3 → GE-V** (R1's two defeat clocks + the marked "Humbled Peace"; the four-register `campaign_end.tscn`; **"THE CONGRESS OF PARIS"**: at 50 titled provinces the Emperor summons the Congress, every great power answers on a public table with reason and PRICE, it sits 8 turns while refusers are driven to war and the bills come due, Britain is shut out by a 60% Continental System, full recognition = THE IMPERIAL PEACE, failure dissolves and can be re-summoned; the Universal Monarchy when no great power remains), **then the release build, then Updates 1–5.** `GAME_END_SPEC.md` §8 records the ruling. Every slice's commit marks the plan's table + STATUS + the rows + this line.
>
> **▶ ▶ NUI-2 "THE FLEET RIDES AT ANCHOR" — ✅ LANDED September 24, 2026** (user-directed between F1 and F2; **NEXT is still F2**; landing record `docs/NAVAL_SPEC.md` §18, rules `SYSTEMS_REFERENCE.md` §57): the coast audit found three dockyards on provinces the map draws inland — **Holland Amsterdam → Friesland, France Flanders → Normandy (Flanders stays a camp), Russia Estonia → Livonia** — and three wrong flags (**Flanders, White Russia, Volhynia → inland**); an inland yard is now a validation ERROR and old saves migrate on load (`save_manager.load_game` → `reconcile_saved_registry_corrections`). Every coastal province carries a registry **`port_anchor`** (derived from the art by `tools/gen_port_anchors.py`; **run `--audit` and `--check` after any map-art or flag change**) and the map draws the fleet there. THE ADMIRALTY opens with its orders and the expedition's road is buttons (commission → march to a yard → land; `naval.expedition_road_chips`). Filed **DEF-14 "The Names Match the Map"** (built the same day, next line). A yard also needs open water to moor at (a registry `port_anchor`: Estonia keeps its coastal flag as a sea-link end). Tests `test_nui2_the_fleet_rides_at_anchor.py` 44; sweep 31/31; `BASELINE_SERIES` + M1–M7 byte-identical; the WO-13 gates-DOWN counterfactual arm moved (Paget's turn-16 descent now lands at Provence, not Flanders; Flanders' flag alone, measured), with four pins re-recorded plus four conscious flips (`NAVAL_SPEC.md` §18.3).
>
> **▶ ▶ DEF-14 "THE NAMES MATCH THE MAP" — ✅ LANDED September 24, 2026** (user-directed right after NUI-2; **NEXT is still F2**; record `docs/MAP_IMPLEMENTATION_PLAN.md` DEF-14, rules `SYSTEMS_REFERENCE.md` §58): nine provinces renamed to where the art paints them, with owners, shapes, borders, yards and capitals unchanged — **Bergen → Jutland, Oslo → Schleswig, Jutland → Holstein, Dalarna → Scania, Scania → Stralsund, Gothland → Norrland, Norrland → Uleaborg, Toledo → Cartagena, La Mancha → Andalusia**. `tools/audit_province_names.py --check` is the standing name audit (every outlier is renamed or recorded in its `STYLISED` table). Old saves are renamed on load, simultaneously, before `from_dict` (`world_state.RENAMED_PROVINCES` — **after renaming any province, add it there and move its gazetteer entry**). Filed **DEF-15 "The Low Countries"** (inland Amsterdam and the interleaved Dutch/Hanoverian provinces need a capital move — the user's call). The parser now reads a word after "of" as a title's territory ("Prince of Moskowa" had been refused properly only because "Moskowa" fuzzy-matched "Oslo"), and the 14 authored IQ-9 cassettes were re-stamped (the prompt lists every province — **re-stamp them after any province rename**). Tests `test_def14_the_names_match_the_map.py` 42; sweep 13/13; `BASELINE_SERIES` + M1–M7 byte-identical.
>
> **▶ ▶ THE CREATIVE AAR PLAYTEST — September 25, 2026 (evening; nothing built).** *"Play the game, review it, be creative"* — eighteen turns of France/1805 played BY HAND at `POST /command` on a fresh port-8007 backend (shipped defaults, live parser configured; **141 of 142 orders read OFFLINE at 0.80–0.95, prose included — the model consulted once**), every popup answered through the client's own endpoints; memo **`docs/audits/PLAYTEST_CREATIVE_AAR_2026_09_25.md`** (the AAR diary + the review: *7/10, an empire contested*; directional ≈6.9 held — narration/AI aliveness/first contact ▲, diplomacy/combat legibility/naval ▼, UI/UX unscored); **32 defect rows `BUG_FIXES.md` §Creative AAR Playtest AAR-1 … AAR-32** (**P1 AAR-1: a lord's separate peace leaves its vassal at war with no army — Austria, at peace with France, ate the Kingdom of Italy and stormed Milan on turn 17**; AAR-2 the settlement editor's "Will sign" beside "cannot be ratified"; AAR-3 the revision table pricing Britain's own offer at −32; AAR-4 the garrison-blind scout; AAR-6 `recruit` refused at zero military AP though admin-priced; AAR-7 "Harsh demands" == "Generous peace"; AAR-11 vindication on the wrong battle); **8 design rows `DESIGN_REFINEMENT.md` §Creative AAR AAR-D1 … AAR-D8** (the client's war is the lord's war; **status quo is a cession** — the Congress does not title what a signed treaty retains; the jealousy cadence; DP as the bottleneck; the paying peace from the player's chair). Two passive driver arms took Britain's turn-4 gold and reached **THE VERDICT at turn 44: "AN EMPIRE CONTESTED — the question of 1805, still open"** (119,401 gold, threat 0). Limits: no client pass (the user was at the machine); the hand-played campaign was lost at turn 18 by `tools/playtest_driver.py --http`'s `POST /new_game` (save first — in memory). **NEXT stays Update 1; AAR-1 + AAR-6 ride beside it as player reports.**
>
> **▶ ▶ THE LIVE REVIEW — September 23, 2026 (evening; nothing built).** The user's ask — *play the game for real using commands, look at the game, find remaining bugs, do a Steam review, determine how the end can work ("holding X for X", "stabilization of new status quo", defeat, global elimination)* — was done as: seven turns of France/1805 played BY HAND in the real client on port 8006 with the live parser (Ulm turn 1, Mack captured, Vienna stormed turn 6; 0 server errors, 0 parser misreads on 19 typed orders); memo **`docs/audits/PLAYTEST_LIVE_REVIEW_2026_09_23.md`** (the Steam review: *7/10, recommended once it has an ending*; directional ≈6.9 with two new honest pillars, first contact 5.5 and the ending 3.0); **21 defect rows `BUG_FIXES.md` §Live Review LV-1 … LV-21**, every producer verified (headlines: LV-1 a fresh campaign and every load open on an EMPTY terminal and no turn-1 dispatch exists; LV-12 the battle report swallowed by envoy dialogs on the attack's tail; LV-14 the settlement table telling a player who holds Vienna that a white peace "claims a victory the field has not delivered"; the display-name family LV-2/3/4/9/19); **5 design rows `DESIGN_REFINEMENT.md` §Live Review LV-D1 … LV-D5** (the reward curve firing the collective petition on turn 7 of a winning campaign; Bavaria's turn-1 triple walk-in re-opening FA-D13; the strategic-orders recap modal; the legitimacy blocker; PB-D1's evidence); **three end-state probes** (the commanded-accept arm: peace on turn 4 then 0 enemy attacks in 56 turns; a fiat conquest of 91 provinces pinned at alarm 97 and bleeding back; a REAL global elimination through `_eliminate_nation`: the game plays on at alarm 99 "Brewing" with nobody left to alarm); and **the victory design — `GAME_END_SPEC.md` §7 "THE IMPERIAL PEACE": hold 51 provinces by TITLE (homeland / ceded by treaty / 12 quiet turns; ONE new serialized `province_title`) for 8 consecutive turns with no great power at war, no coalition, alarm < 60, satellites loyal; the Universal Monarchy fires it when no great power remains; rulings E1–E6 for global elimination; R1's two defeat clocks confirmed plus a marked non-terminal "Humbled Peace". ⚠ AWAITING THE USER'S RULING on §7.8 Q1–Q6 — GE-1 opens on it.** The order (GE → the release build → Updates 1–5) stands; routing = `docs/STATUS.md` ▶ NEXT UP.
>
> **▶ ▶ THE PLAN — September 23, 2026 (revised twice the same day). ROUTING AUTHORITY = `docs/STATUS.md` ▶ NEXT UP, top block — START THERE.** A read-only pre-build review at HEAD `289697ef` (six seeded driver campaigns with 0 server errors and 0 unhandled popups; a build-pipeline audit; a defect census with reproductions; a decisions census; a game-end research fleet; a PR-D1b simulation), then the user's *"make calls please and then separate out game end and defeat state as session after these"*, *"i mean people should be able to add keys and stuff lets just release a playable game"* and *"thought we wanted end state first?"*. Every call was taken by Claude under the delegated grant (three revised by the user's own word), recorded with its re-open condition. **▶ NEXT = row GE "The Verdict and the Fall"** (spec **`docs/GAME_END_SPEC.md`**, RULED: defeat = two warned clocks — ≤1 province or no army and no affordable commission for 5 turns, or the Emperor captive and unfreed for 10; game end = the Verdict of History at a scenario-authored turn 44 (Tilsit), marked and continued; WO-D10's exile commissioning) → **THE RELEASE BUILD** (ROADMAP 10), **whose part 1 is written and SAVED** — the local `git stash` "release-build part 1 …" and the committed patch **`deploy/parked/release_build_part1.patch`** (apply it first: PB-1's fix — **the frozen server could not find `europe_1805.json`, because PyInstaller's entry-script `__file__` is `_internal\main.py` (measured with a probe build)** — the runtime log + transcript, the build stamp, the API-key check + `key_status` + parser notice (call C1: keys SHIP), `launch.bat`, the help/debug copy); the rest = boot help + README + `build.bat` (frozen boot + release export) + tests + the live-key smoke → the user uploads to itch.io → **fix Updates** (player reports re-order them): **1** PR-D1b (the gate is P1's own coalition break-ranks clause, NOT `effective_peace_threshold`) + doc housekeeping → **2** CRT-3 + WO-32 → **3** L-D + CX5-L5-F2 + CQ-30 → **4** petition Antechamber B1 → **5** PR-D1c + PR-D1d → CRT-2…CRT-11 · L-1 · the Victory & Objectives gate (ROADMAP 12, which owns PB-D1 "the long peace"). Every "FOR USER CONFIRMATION" ruling before Sept 23 is **CONFIRMED AS BUILT** (call C6).
>
> **✅ FIRST CONTACT + THE SCHOOL OF WAR REFRESH — LANDED September 23, 2026** (user-directed, after CRT-1 + row NUI; record `docs/audits/FIRST_CONTACT_AND_SCHOOL_2026_09_23.md`, rules `SYSTEMS_REFERENCE.md` §54/§55, rows `BUG_FIXES.md` §First Contact): `backend/ai/first_contact.py` = ONE source for greeting / Esc-menu / undo / stuck / goal answers (the keyless report's one canned shrug is gone; every shrug names `what can I do` / `status` / `help`); the place-only shrug walks the road law and **the tactical `move to` belt now reads it too** (`Ney, move to London` was ACCEPTED while `march to London` was refused — FA-46 one verb over; AI byte-identical); `destroy Austria` → the nation refusal (attack-verb inflections in the target scan's skip list); example provinces come from the world; `own_army` + `is X strong` answered. **School of War = 18 cards**, mirrored in `tutorial_state.py`, driven headless by `tools/tutorial_overlay_harness.gd`: VII the Cabinet (real F1 mission via the `open_cabinet` signal — typed diplomatic verbs are G1-redirected), IX the Marshalate, XVI the Wooden Wall, pushback/defiance on IV/V; **unbreakable** = refused-suggestion release (`main.gd` `note_sent` at three send sites) + per-step Skip + the catch-up floor; driver `missions: begin|decline` dial. Tests `test_first_contact_keyless.py` 114 + `test_tutorial_unbreakable_2026_09_23.py` 23 (driven). Full IQ-10 re-shoot dated 2026_09_23. **NEXT = ROADMAP position 10, the shippable build.**
>
> **▶ ▶ NEXT = THE COMMAND-ROAD QUEUE (opened September 20, 2026). ROUTING AUTHORITY = `docs/STATUS.md` ▶ NEXT UP, top block — START THERE, it holds the numbered build order, the six rulings and the four memos.** Opened by user direction (*"examine feasibility for local llm for parser… lay out plan for multi step or conditional commands and assure the parser and autofill are in good form and sensical… you can click recruit cavalry etc for every general"*, then *"make decisions and assure build order is documented… make sure a fresh session knows what is next and knows to update status and progress when next commit and push"*). A 53-agent read-only fleet (18 recon → 18 adversarial refuters → 7 plans → 6 judges), everything driven at the real `POST /command` on an unmodified 1805 boot at HEAD `15c498cb`. **Memos of record:** `docs/audits/RECRUIT_ARM_UX_2026_09_20.md` (row **CN**) · `COMPOUND_CONDITIONAL_COMMANDS_2026_09_20.md` (row **CR-7**) · `PARSER_AUTOFILL_ASSURANCE_2026_09_20.md` (row **CX-R**) · `LOCAL_PARSER_FEASIBILITY_2026_09_20.md` (row **HC-L**; **amends `HEALTH_CHECK_DESIGN_GATE_2026_08_14.md` §7b — do NOT follow the old §7b contract**).
>
> **BUILD ORDER (decided, take in order):** ~~**1. CR-7-1** "the tail stops eating the head" (0.5 · **P1** — `Ney, fortify then attack Mack` marches, fights, loses 1,950 men and is *not* fortified; **40 of 40** non-movement compound shapes swallowed, silently)~~ ✅ **LANDED September 22, 2026** (landing record = `COMPOUND_CONDITIONAL_COMMANDS_2026_09_20.md` §CR-7-1 LANDING RECORD; `SYSTEMS_REFERENCE.md` §4 Stage 2a is the rule: a tail fuses onto a head ONLY if the head can carry an arrival — MOVE_TO/PURSUE with a destination derived from `STRATEGIC_KEYWORDS`, or a standing order carrying `until`; **40 → 0**, riders `and then`/`;`/the FA-50 `and` arm/compound-only `move to`→`march to` promotion; `test_cr7_1_the_tail_stops_eating_the_head.py` 106, sweep 14/14, `BASELINE_SERIES` + M1–M7 byte-identical, corpus 688/688 BOTH arms = not evidence; CQ-1 + CQ-9 FIXED, **CQ-10** filed — the bare-comma boundary — to CR-7-3) → ~~**2. CX-R1** "the unbound name spends nothing" (0.5 · **P1** — without a comma, `Zorglub build ships` spends **400 gold** and `Zorglub vassalize Austria` **subjugates a great power**)~~ ✅ **LANDED September 22, 2026** (landing record = `PARSER_AUTOFILL_ASSURANCE_2026_09_20.md` §CX-R1 LANDING RECORD; rules `SYSTEMS_REFERENCE.md` §51: re-measured wider than filed — **~30 forms** mutated for a name nobody has, incl. `Zorglub retire` marching eight corps back at 0 AP, and the comma was NOT the discriminator for the vassal/instrument family, the rewards or a clause comma; the executor's gate covers **every** order (reads exempt) and accepts only a name that takes the order — our marshals, the desk on an order of state (never a field order, FA-22), the foreign minister, the admiral on the fleet's orders; **the verb set is GENERATED from the parser's routing branches** — `tools/gen_routed_order_words.py` → `backend/ai/routed_order_words.py`, 283 words, generated not import-harvested because the frozen build carries no source, a census fails on drift — **after changing the parser's keywords, regenerate it**; an unmarked address is the name at its head (L2-4, L2-3 epithet); `test_cx_r1_*.py` 426; CQ-2 + L2-1/L2-2/L2-4 FIXED, **CQ-17** filed, **CX-X3 (a bare `vassalize`) re-routed to the CR-6 triage** — taken out of scope when the user asked why it was being worked on) → **▶ 3. CN-1..CN-4** "The Chip Names the Man" (2.0 · the user's reported bug) — ✅ **CN-1 + CN-2 (backend) LANDED September 22, 2026** (landing record = `RECRUIT_ARM_UX_2026_09_20.md` §CN-1 + CN-2 LANDING RECORD: where the game chooses the man the requested arm is a SELECTION KEY — `find_nearest_marshal_to_region(..., arm=)`, no-arm rule byte-identical on 126 provinces × 9 boards — wrong-arm chips **14 → 0**; an arm nobody in range commands refuses free and names the remedy derived from the board, incl. R2 item 9's gun marshal (`commission Marmont (4500g)` → then `'Marmont, recruit artillery'`); the NAMED road stays PF-7's surfaced correction, **ruling D1's "closes both roads" corrected**; ONE quote `economy_executor.recruit_quote` = the executor's own steps, drift-pinned on every province × arm, shipped as `map_data[p]["recruit_here"]`; `recruit_price_here` made true; AI recruit producers all name a marshal so the series cannot move; `test_cn_*.py` 32, sweep 19/19) → ~~**CN-3**~~ ✅ **LANDED September 22, 2026** (landing record = `RECRUIT_ARM_UX_2026_09_20.md` §CN-3 LANDING RECORD: the row renders where the backend quoted it, one chip per arm, ENABLED only where the levy will be made and stating its terms, else dimmed beside the backend's `short`; **the ground speaks first** — `economy_executor.recruit_ground_refusal`, one source for the executor and the quote (CQ-18: `recruit artillery in Milan` had offered a 4,500g commission on soil where no levy is raised); **a remedy names an order the game takes** (CQ-19); riders — the marshal row names the arm, the bench tags every arm and pool, the ordinance line says its multiplier, the result names every priced term via `_recruit_cost_terms`; **driven**: `tools/cn3_region_panel_harness.gd` runs the real panel headless and every `do:recruit` url is sent through `/command` — 12 enabled all raise their labelled arm, 0 enabled-and-refusing (was 78), 0 wrong arm; `test_cn3_*.py` 26, sweep 30/30 0 INERT, parse 49 scripts EXIT=0, boot 0 SCRIPT ERROR, series + M1–M7 byte-identical) → ~~**CN-4**~~ ✅ **LANDED September 22, 2026 — ROW CN IS DONE** (landing record = `RECRUIT_ARM_UX_2026_09_20.md` §CN-4 LANDING RECORD: a DRIVEN census of every chip the client composes — the real panel and Generals cards rendered headless on a staged board, 175 chips sent through `/command`, the wizard's 26 templates run by the REAL `_build_command`, the Vassals tab, the reward dialog, the Admiralty chips; the four siblings plus five more of the class fixed as single sources — `economy_executor.substitute_quote` (the chip names the first infantryman and states the men after the field cap and the gold), `tactical_executor.drill_refusal`/`fortify_refusal` read by the pre-objection battery, both executors and the payloads (**CQ-23**: an objection had spoken for an order the executor refuses), `at_war_with_player` (the attack chip is an order, not a declaration — "Attack Deroy" beside an ally was always refused), printed names, `naval.build_ships_refusal` for both keel chips, the cede echo names the province, the white-peace echo disclosed as display copy and kept off the up-arrow; **CQ-22**: the executor's drill road keeps no stance gate because giving it one MOVED `BASELINE_SERIES` (flip-proven) — filed; CQ-20 + CQ-21 filed to the CR-6 triage; `test_cn4_*.py` 26, sweep 33/33 0 INERT at close, series + M1–M7 byte-identical) → ~~**4. CX-R2** "the offer is reachable" (0.5 · the completer's 280/280 lines all parse and **59.3% are refused by the executor**)~~ ✅ **LANDED September 23, 2026 — THE FOUR SLICES AHEAD OF THE BUILD ARE ALL LANDED** (landing record = `PARSER_AUTOFILL_ASSURANCE_2026_09_20.md` §CX-R2 LANDING RECORD; rules `SYSTEMS_REFERENCE.md` §53: every completer pool is the executor's own answer, nearest first over the lawful road — slot letters in `main.gd` `_MARSHAL_VERBS` (`E` at war · `R` the march's lawful reach · `A` `move_open` · `S` `scout_range` · `H` his own province · `M` our marshals on the map), every no-target verb behind its `tactical_state.<verb>_refusal`; **not client-only, by decision** — `enemies[].at_war_with_player` + `passable_nations` ride the summary (display-only, 0.9 ms) because a second copy of `is_at_war` in the client is the CA9 through-line; `_execute_garrison` refuses a named province that is not his (the latent substitution closed); **driven** by `tools/cx_r2_completer_harness.gd`, every offered line sent to `/command` — boot **169 of 280 refused → 0 of 272**, turn-10 0 of 243, only CQ-24's marshal-state class left on turn-20 and a staged board; **a new completer verb gets a slot letter whose pool is the executor's own answer, or a `<verb>_refusal` — and the driven census**; CQ-5 FIXED, CQ-25/26/27 found and closed, **CQ-24 + CQ-28 filed** to the CR-6 triage; `test_cx_r2_*.py` 75, sweep 39/39 0 INERT at close, series + M1–M7 byte-identical) → **→ ROADMAP position 10, THE SHIPPABLE BUILD** (**NEXT** — CRT-1 and row NUI both landed September 23, 2026, rows 6a/6b below) → ~~**5. CR-7-2..8** (4.5)~~ ✅ **LANDED September 22, 2026 — pulled AHEAD of slices 2–4 by user direction** (*"continue work on multi step commands make sure costs and contradictions are smoothes out make all decisions"*; landing record = `COMPOUND_CONDITIONAL_COMMANDS_2026_09_20.md` §CR-7-2..8 LANDING RECORD; rules `SYSTEMS_REFERENCE.md` §4 Stages 2b–2e; **the relay** `backend/commands/relay.py` — a dropped tail rides every arm, is handed back to the line for the seal only when coherent, a CONTRADICTORY tail is named not filled, a tail behind a live march is held for its moment, a refused head cancels its tail, and it comes back re-judged after every objection/interrupt answer; **one condition vocabulary** `backend/ai/condition_grammar.py` — 15 of 15 phrasings honest, unmeetable referents refused free by cause, the echo names every condition from the Ledger's own sentence; **the third verdict** — `when|if|once|as soon as <friendly marshal> arrives, hold …` = the engine's own `until`, fails closed, 0 pinned refusals flip; **the arrival object** `StrategicOrder.arrival_target` + `strategic.pick_contact_enemy` (kill gate run first: Mack was engaged when Charles was named); the completer's `_CONTINUATIONS` + help + School; **the cross-turn order queue RETIRED BY CONTRACT** `COMMAND_ROBUSTNESS_SPEC.md` §11.1 with two instrumented re-open conditions (0 completions / 0 breaks on the commanded arm); costs decided — a standing order pays once, a relayed tail pays its own AP when sent, a refused condition costs 0; CQ-6/7/10 FIXED, CQ-11/12 found and closed; sweep 33/33 0 INERT at close; the hook's full run found 7 more — 4 legacy until-battle-won fixtures stamped + a suite-wide hazard closed (an instance-level monkeypatch on the executor singleton blinds every later class-level patch: 10 sites converted, static census + session-end guard in conftest); corpus 706/706; harness EXIT=0; boot 0 SCRIPT ERROR; series + M1–M7 byte-identical) → ~~**5b. CR-7-9** "The conditions say what they mean"~~ ✅ **LANDED September 22, 2026** (the user's follow-up question on the landed row; landing record = `COMPOUND_CONDITIONAL_COMMANDS_2026_09_20.md` §CR-7-9; rules `SYSTEMS_REFERENCE.md` §4 Stage 2f: `and` = every arm — `StrategicCondition.require_all` latched on `StrategicOrder.condition_progress` with a progress beat and the Ledger ticking arms off — `or`/none = whichever comes first, SAID on the echo and the Ledger; a bare second clause reads without its own `until`; ONE timer rule `strategic.count_order_turns` — a HOLD counts the turn it was given, a SUPPORT the turn after arrival, the Ledger never reads 0 on a live order; a stashed tail is let go WITH A WORD (`relay_let_go`) and the typed interrupt answer brings it back; CQ-14/15/16 found and closed; `test_cr7_9_*.py` 37; corpus 709/709; series + M1–M7 byte-identical; zero `.gd`) → ~~**6. CR-6 triage**~~ ✅ **HELD September 23, 2026 (docs only).** Record and build contract = `docs/COMMAND_ROBUSTNESS_SPEC.md` §12; each row's disposition is on the row in `docs/BUG_FIXES.md`.
> - **Method.** Every row that named the retired "CR-6 proper", the seven intake rows (CQ-17/20/21/24/28/29, CX-X3) and CR-7's orphans (CQ-8, R158 and the rest) were re-measured at HEAD `5fec6a9c` on `POST /command`. Four read-only agents covered the review verdicts; every P1, and every P2 that spends or leaks, was re-confirmed by hand.
> - **Outcome.** Each row is now homed to **CRT-1 … CRT-11** ("Command-Road Triage", numbered in build order — not `CX-R3+`, which `docs/audits/cx_recon_2026_09_19/` already uses) or closed or struck with its reason. CQ-30 … CQ-36 are filed.
> - **⛔ FOUR P1s on ONE seam**, each doing the OPPOSITE of what the player typed:
>   - `Ney, retreat as they attack` fights (CQ-32);
>   - `Nobody retreat` retreats the whole army, and `No one attack Mack` fights (CQ-34 — CX-7's collective list re-opened it);
>   - `couldn't we attack Mack` fights (CXR1-3 — filed P1 on Sept 19 and left unowned);
>   - `I would not accept` SIGNS the treaty (CQ-35).
>
> ~~**▶ 6a. CRT-1 "What the sentence forbids is never the order" — NEXT, before position 10**~~ ✅ **LANDED September 23, 2026** (landing record = `COMMAND_ROBUSTNESS_SPEC.md` §12.7; rules `SYSTEMS_REFERENCE.md` §4 Stage 2g): re-measured first (all four families reproduced at `POST /command`), then built as the contract's three items in `clause_guards` — the modal / perfect / past-copula / contracted negatives + `ought we` / `have we` as markers (the lead of `is_question` untouched; `Davout, don't advance on our left, fortify` still fortifies, pinned), the negative indefinites as markers that LEFT `_COLLECTIVE` (with the vocative comma consumed: `Nobody, retreat` is `Nobody retreat`), and ONE subtractive reason-clause guard sited after the question and condition guards — applied to all THREE readers of the raw text (the mock chain, the strategic layer's read, and the parser's fuzzy target scan; the third found while driving: `Ney, retreat, Mack is attacking` had bound Mack as the retreat's DESTINATION); foes handed in BOTH registers (`Archduke Charles is attacking` attacked him with the keys alone); a reason-only sentence refused by name (`refusal == "reason"`); `'d not` closed too (11 of 11 — the row had written it off). `test_crt1_what_the_sentence_forbids.py` 103 (+13 / +18 in the two contract-named files), sweep `_sweep_crt1.json` 14/14 0 INERT, corpus 723/723 (12 `crt1-*` rows; 711/711 in both arms before them = not evidence), series + M1–M7 byte-identical, zero `.gd`. **CQ-32 / CQ-34 / CQ-35 / CXR1-3 / CX5-L5-F1 FIXED.**
>
> ~~**▶ NEXT = row NUI "The Admiralty on the Map"**~~ ✅ **LANDED September 23, 2026** (user-directed — *"add naval ui and make the ux and ui work and while at it assure ux and ui is good in other key areas"*; landing record = `NAVAL_SPEC.md` §17): every fleet in commission stands on the map as the ship piece at its senior yard with its sail count (`naval.fleet_pieces` → `naval_overlay.fleets`, public counts by the §9 ruling; `map_renderer_base._update_fleet_pieces`, DIFF-updated like the corps); a sea crossing answers a hover over open water with THE ADMIRALTY's own sentence — `naval.crossing_line` is ONE string for the ledger's Crossings row, the tooltip and the region panel's new THE SEA block (parity pinned); the top bar carries the Admiralty chip (`naval.player_naval_summary`, crimson under blockade, gold in a window, hidden on a naval-less world); **three doors, one room** — a fleet piece, a crossing, THE SEA's link and the chip all `open_ledger_to_tab(6)`. **It named and took "the next UI slice": IQ10-X1 CLOSED** (`top_bar._fit_bar` — compact below 1,180 logical px, icon-only nav with the letter as fallback, the Cabinet line hidden; driven at 800×450 = no BaseButton off the viewport) **and IQ10-X2 CLOSED** (the petition's "Grant unavailable" line under the header, above the fold). Driven, not read: `tools/nui_top_bar_harness.gd` + `tools/nui_map_capture.gd` (both in the parse check's TOOL_SCRIPTS) + the CN-3 harness; `test_nui_the_admiralty_on_the_map.py` 31, sweep `_sweep_nui.json` 9/9 killed, 0 INERT, 0 BROKEN, parse harness EXIT=0, CN-4's inventory reviews the new chip; evidence `docs/audits/NUI_MAP_FLEETS_CROP_WEST_2026_09_23.png` + `NUI_MAP_FLEETS_CROP_NORTH_2026_09_23.png` (crops of the windowed capture — the 4.5 MB full frame is not committed) + the 2026_09_23 IQ-10 frames (the IQ-10 road gained `top_bar_admiralty`). Recorded not built (§17.3): naval rows on the notice rail; fleets drawn at sea. ⚠ open for the user: the in-game feel of the pieces and tooltips.
>
> ~~**▶ NEXT = ROADMAP position 10, THE SHIPPABLE BUILD** → then CRT-2 … CRT-11 in `COMMAND_ROBUSTNESS_SPEC.md` §12.3's order.~~ **⚑ Superseded September 23, 2026 by the plan above: NEXT = row GE (game end and defeat), then the release build.**
>
> **→ position 10** → **7. HC-L L-0/L-1/L-2** (1.5; **L-1 is unconditional and may ride any earlier session**, and it now carries the triage's riders IQ9-X1 + IQ9-X3) → **8. CRT-2 … CRT-11**, in §12.3's order after the build (≈ 8.5–9 sessions; Round 0 may re-order them):
> - CRT-2: the name is never replaced;
> - CRT-3: a question never orders;
> - CRT-4: the road law read where it is quoted;
> - CRT-5: an answer is read closed;
> - CRT-6: the retreat is a word;
> - CRT-7: the desk answers what the order would do;
> - CRT-8: the Cabinet's rules on every road;
> - CRT-9: the state speaks first;
> - CRT-10: the suggestion is honest — lands WITH or AFTER CRT-1;
> - CRT-11: the second name is heard.
>
> **Flagged, not in scope:** "the next UI slice" and NPC-12's census carry "CR-6 proper"'s shape. Dated: the next `.gd` session names the first; CRT-7's builder names the second.
>
> **Rulings taken under the delegated grant** (arguments + re-open conditions in the STATUS block): **D1** which surface the user clicked is **moot by construction** — the typed per-general form has the identical defect (`Murat, recruit infantry` → **cavalry at 1,504g**) and CN-1/CN-2 are backend slices closing both roads (⚠ **corrected by the CN-1 + CN-2 landing record**: they close the province road; the NAMED road stays PF-7's surfaced correction, with a re-open condition) · **D2** the order above · **D3** **"CR-6 proper" is RETIRED as a routing destination** (47 routed rows, no spec section, no gate — a GR9 violation; compound/conditional → CR-7, state-mutating → CX-R1, remainder triaged the session after CR-7-8 — **✅ held Sept 23, 2026, homed to CRT-1…CRT-11** (`COMMAND_ROBUSTNESS_SPEC.md` §12); **nothing new routes there**; CR-6 the *feature* keeps its USER DESIGN GATE) · **D4** **no artillery marshal is authored onto the 1805 board** (there are zero for any nation; authoring one moves `BASELINE_SERIES` and this is a UX row — the chip becomes an honest refusal naming its remedy, `commission Marmont` 4,500g, which also un-deadens Paris; re-open at the next scenario-balance gate) · **D5** **recruiting does NOT open on ally soil** (the executor is right; CN-3 gates the recruit row on the recruit gate — IQ-10's H1 one verb over, resolved the opposite way, deliberately) · **D6** the local model gets a **dated 1.5-session decision package, not the 8.5-session build** (9 of 9 measured "the game acts on an order you did not give" cells parse at **0.90–1.00** against a **0.70** gate and never reach any model tier — so a local model is *not* what makes the game "just work"; what it would close is real but narrow, **CR-5 delegation is 100% dead for keyless players** via `delegation.py:398`). **⚠ D6 AMENDED the same day — `LOCAL_PARSER_FEASIBILITY_2026_09_20.md` §6 supersedes that memo's own §4 build order:** the delegation feature is **already fully deterministic in mock** (`detect_delegation` matched 7 of 7 rows, recovering marshal + personality + target; `classify_arm` is personality × boolean and nothing else), so the whole three-way split is gated on ONE boolean — new slice **L-D "The Boolean Road"** (~0.5 session, **behind a user gate** because guardrail (e) is part of CR-5's blessed scope) may close the local model's single justification without a model. Order becomes **L-1 → L-D → (only if needed) L-0 + L-2**. ⛔ Deleting the `mode == "mock"` clause is a **measured NO-OP** — a mock delegation parse fails at clauses 1 and 3 and never reaches the mode check; the gate must read the `DelegationMatch`, not the parse.
>
> **⛔ EVERY SLICE'S COMMIT MUST ALSO CARRY:** the STATUS ▶ NEXT UP block updated (strike what landed, name what is next) · the landing record appended to that row's memo · **this** LIVE STATE block updated · `docs/BUG_FIXES.md` §Command-Road Queue disposed. **A slice is not landed until those four are in the commit.**
>
> **▶ IQ-10 "THE CLIENT PASS" — LANDED September 19, 2026. THE IMPROVEMENT QUEUE IS CLOSED.** (memo `docs/audits/IQ10_CLIENT_PASS_2026_09_19.md`; landing record `IMPROVEMENT_QUEUE_SPEC.md` §1.9; rules `SYSTEMS_REFERENCE.md` §49). **UI/UX was unmeasured because there was no instrument**; the row builds one and commits it — `tools/iq10_capture_payloads.py` (payloads off STAGED boards) → `tools/iq10_surface_screenshot.gd` (ONE generic offscreen capture: the real scene, its real entry method, `content_scale_factor`, and a machine record of every visible string, every button outside the logical viewport, every label taller than its box) → `tools/iq10_run_captures.py`. Two commands re-shoot everything. **80 surfaces × 2 Interface Scales = 160 committed frames**, 0 `SCRIPT ERROR`. **7 defects fixed, 2 routed:** H1 — the region panel gated every action row on `controller == _PLAYER_NATION` while its own comment claimed the substitute market rendered on ally soil, and the backend had been pricing that ground all along (Amsterdam 598g / 3,193g); the EMPTY letter-book skipped its clamp and the diorama's `custom_minimum_size` floor defeated it, so **both were unreadable at Interface Scale 2.0** (a tableau fits by SCALING, never by reflowing); a CAPTIVE Emperor's card advertised The Presence from a cell; "PRISONER of Kingdom of Italy" wanted its article; and **the game's own printed sentence was not typable** (`gather intelligence on Austria` shrugged while `gather intel on` worked — now a drift pin). ⚠ **UI/UX 7.5 HELD, FOR USER CONFIRMATION**; the top bar's 2.0 overflow is routed (IQ10-X1) and the eyes-on half stays the user's. `tests/test_iq10_client_pass.py` (25).
>
> **▶ IQ-9 "THE KEYLESS PARSER GATE" — LANDED September 18, 2026** (landing record = `IMPROVEMENT_QUEUE_SPEC.md` §1.8; rules = `SYSTEMS_REFERENCE.md` §48; technical record `COMMAND_ROBUSTNESS_SPEC.md` §9). **The finding was wider than "no gate":** the escalation path's two production functions were referenced by ZERO test files, and three test ids built env-derived clients that escalated to the REAL API on any checkout whose `.env` said `LLM_MODE=anthropic` — so the T0 floor landed FIRST (`LLM_MODE=mock` as a MODULE-LEVEL assignment in `tests/conftest.py` — an autouse fixture alone leaves `backend.main`'s import-time parser singleton live — plus the autouse pin and a network guard that refuses everything but loopback). ONE production seam `AnthropicProvider.bind_sdk_client` (`_client()` untouched); `tests/_parser_replay.py` (a cassette miss is a `BaseException`; the key is `(kind, utterance, world)`, never the prompt hash); seventeen AUTHORED cassettes the USER promotes with the opt-in recorder `tools/record_parser_cassettes.py` (the suite never records); `parser_eval --replay`; **112 keyless pins that each count live calls** (`tests/test_iq9_keyless_parser_gate.py`). Routed to CR-6 proper: IQ9-X1 (the CR-2 retry cannot rescue the word-scan family), IQ9-X2 (a fogged enemy in the fuzzy suggestion), IQ9-X3 (a live-road failure stamps `parse_mode: "mock"`). Corpus count corrected 681 → 686. **NEXT = IQ-10 "The Client Pass"** (recon done; the harness is the build).
>
> **▶ IQ-8 "THE HARNESS TELLS THE TRUTH" — LANDED September 18, 2026** (landing record = `IMPROVEMENT_QUEUE_SPEC.md` §1.7; rules = `SYSTEMS_REFERENCE.md` §47; the instrument's contract = `docs/PLAYTESTING.md`). **The recon overturned the premise:** the hash seed does NOT move the board (commanded and ambient runs at `PYTHONHASHSEED` 0 / 1 / 12345 are the same game; the one order-dependent site was a naval display walk, `naval._tracked_links_for`, now sorted by `_link_key` — the row's one lever-less change), and PR-D4's 20 / 24 / 22 has no archive on either side, so it closes as cause unrecoverable and provenance becomes mandatory instead. **PR-X5 was wider than filed** — four request values recorded as resolutions — and closes behind five driver levers: `meta.json` = `requested` + `resolved` (seed, `dice_label`, scenario, map, env read AFTER the backend import) + `platform` + `engine_revision` (git commit, a scoped `dirty` flag, an LF-normalised content hash); the from-save seed rule (the save's seed plays, an explicit `--seed` is the dice only and is WARNED); the driver SETS the board environment instead of popping it; `driver_revision` LF-normalised; `ap_available` / `ap_spent` / `cmd_refused` off the `/ledger` read (**85 / 80 / 76 of 160 spent, France 28 / 28 / 29** on the `iq8-cmd-*` archives). The published table marks the un-archived figure UNCITABLE (`TestTheTableRule`). **Routed items closed here:** IQ6-X1 (the Berthier rotation is emptied at `WorldState.__init__`; a loaded campaign restarts it — display-only, never serialized) and IQ6-D4 (`tools/ai_v_sweep.py --script france_soil` restores the scene-4 positive on 3 of 3 seeds). Gates: `test_iq8_the_harness_tells_the_truth.py` 49 (29 of 31 pins shown RED first), sweep 52 / 52, `BASELINE_SERIES` + M1–M7 byte-identical. IQ-9 followed (above).
>
> **▶ IQ-7 "THE SATELLITES HAVE A POSITION" — LANDED September 16, 2026** (landing record = `IMPROVEMENT_QUEUE_SPEC.md` §1.6; rules = `SYSTEMS_REFERENCE.md` §46; memo `docs/audits/IQ7_SATELLITES_2026_09_16.md`). **The premise flipped at HEAD:** IQ-3 keeps a commanded France at peace from turn 5 to 29, and at peace the satellite web's only live loyalty term is the −2 drift, so a well-played France lost **2 / 3 / 3** of its three satellites by turns 30–33 and saw nothing but the rebellion modal. **"The Client's Petition":** a LOYAL satellite (≥60, grace 5, cadence 8) petitions for a province (the VS-3 grant) or eight collections of tribute relief; granting binds it (relation +20, cap 40 — two honoured petitions cancel the drift), refusing or a lapse spends its standing (−10 loyalty, −20 relation, never an AI-3 refusal); an AI lord answers by the same rule. Two vassal-row keys (`petitioned_turn`, `remission_left`); ONE tribute source `vassal_tribute_owed` for all four readers; levers `vassal.THE_CLIENT_PETITIONS` / `AN_UNANSWERED_PETITION_IS_REFUSED` / `COURTING_SPARES_THE_LORDS_ALLIES` / `THE_WAVERING_LINE_IS_HONEST` + the driver's `THE_DIGEST_SEES_THE_WEB`. Riders: the wavering line stops promising regiments the satellites do not have, a lord's allies stop courting its satellites, the Garrison option says what it does, the digest gains a `vassals` bit + `--client-petition`. **Measured: GRANT holds 3/3/3 satellites at turn 40 with 0 rebellion modals; REFUSE loses 0/0/0 by turns 21–23; HEAD held 1/0/0.** Fixed while integrating: the PL-14 safety net mislabelled every delivered `proposal_result` ("Diplomatic Action", a granted petition read REJECT — IQ7-X4); the rail's raw vassal tags (IQ7-X6). `BASELINE_SERIES` re-recorded ONCE (arm 0 byte-identical; the lapse lever is the sole mover, diverging at [20]); 13 log-count pins 164 → 165. Routed: VD-C the Contingent (`VASSAL_DEEPENING_SPEC.md` §9), IQ7-D2 the Suitor (declined), IQ7-D3, IQ7-X1..X3 → VD-C, IQ7-X5 → IQ-10. **Pillar vassals 6.5 → 7.0 ⚠ FOR USER CONFIRMATION.** **Review round (Sept 18, 2026): 39 of 43 findings survived, ALL FIXED or RECORDED over four passes — `BUG_FIXES.md` IQ7-RV1..RV33 + IQ7-X7.** Two P1s on the ordinary route: the game's own `cede` verb honoured the ask and was charged as a REFUSAL (now `fulfilled` on both arms, `THE_DEED_HONOURS_THE_PETITION`); a typed `accept the petition` signed the letter ahead of it in the queue (the matter noun is an addressee at both routing seams). The contract's T11(d) was a dodge (a DP-short Grant now STANDS, `THE_LORD_PAYS_TO_GRANT`); a relief rode into the next lord (`THE_TRANSFER_SHEDS_THE_REMISSION`); quote ≠ applied at every read (ONE re-pricer on the fixed subject); the province price was ~4× too high and wrong in SIGN on a fresh conquest; the lapse's price reached no surface the player was on (priced aloud, display only). IQ7-X5 closed here. **Passes 2–4:** the verifiers' twelve findings built (RV17..RV23); a second verifier then showed pass 2's strip-the-filler rule had created a P2 (`grant them relief instead` at a PROVINCE petition ceded the province), so the typed answer to a client petition is now a CLOSED literal grammar keyed to the petition's own subject that FAILS CLOSED (`dialogue_routing.petition_plain_answer`; an unaccepted phrasing is a re-prompt by design, re-prompted IN PLACE so nothing mounts over the petition; the auxiliaries answer in STATEMENT order only, so `then shall we grant it` claims nothing). Closed on the way: a QUESTION signed a treaty — and an inverted one RATIFIED a settlement — on every family (`A_QUESTION_NEVER_ANSWERS`); the button route never read the court; **the suite had been overwriting the developer's Continue slot — `tests/conftest.py` sets `INK_IRON_SAVE_DIR` at import now** (`tests/test_suite_save_floor.py`). Routed: **IQ7-X7** (a deferred answer still signs an ordinary letter → CR-6 proper, pinned as current). ⛔ **The lesson: a rule built by stripping what you recognise is only as safe as the list it strips — for an irreversible priced answer, write the allowlist out and fail closed.** ⚠ the seven-seed ambient re-read breaches the ±1 band on eylau (3 vs 6) FOR USER CONFIRMATION. IQ-8 followed (above).
>
> **▶ IQ-6 "EUROPE SPEAKS ITS MIND" — LANDED September 16, 2026 (built September 14)** (landing record = `IMPROVEMENT_QUEUE_SPEC.md` §1.5; rules = `SYSTEMS_REFERENCE.md` §45). **Two zeros, two answers.** The Stage-F narration's zero (PR-X4) was the INSTRUMENT: the lines are MEDIUM/LOW dispatch rows and the driver's digest printed HIGH/CRITICAL only (30 of 122 diplomatic rows reached it); the producer fires on every board (8 unique routine lines on the ambient run, ≤2 a dispatch). `tools/playtest_driver.py` now reads the whole dispatch (`COURTS`/`DIPLO` lines, `dispatch_row` jsonl, `dispatch_type_counts`; lever `THE_DIGEST_READS_THE_WHOLE_DISPATCH`), no backend change, the rescore memo's three "0 in twelve runs" claims struck. **The volte-face's zero was REAL**: the predicate wanted a courtship to 40 inside 15 turns of the peace and the game's own best lever reaches 40 at best 16 turns after it; four lever-gated rulings in `emergent_designs.py`/`ai_diplomacy.py` — window 15 → 20 (ceiling 25 pinned), a separate peace counts as the war's end (a bilateral peace never stamped `exited_turn`), the courier skips the routine ask cooldown, the exhaustion arm RETIRED under GR9 (R49 zeroes it at the peace; struck from `AI_INTENT_SPEC.md` §3.6-4/§18 with a re-open condition), and Talleyrand + the war room NAME the open door with the turns left (`volte_face_failing_clauses` single source, boolean byte-identical on a 1,152-cell grid). Measured: the courting script `volte_court_austria.json` fires `volte_face` once (turn 21), the plain arm 0. `BASELINE_SERIES` + M1–M7 byte-identical (no France–great-power peace on the ambient board); zero `.gd`. **Routed: IQ6-D1..D4.** IQ-7 followed (above).
>
> **▶ IQ-5 "BOTH SIDES OF THE BUTCHER'S BILL" — LANDED September 14, 2026** (landing record = `IMPROVEMENT_QUEUE_SPEC.md` §1.4; rules = `SYSTEMS_REFERENCE.md` §44; defects = `BUG_FIXES.md` §Both Sides of the Butcher's Bill). **Display only — no mechanical casualty figure moves; M1–M7 + `BASELINE_SERIES` byte-identical.** The row was too narrow: the enemy-phase dialog (where the player defends), the campaign log ("/ 500" for a 12,866-man loss), the morning dispatch (mauled the lead with the army's losses) and co-located stacks all misreported a reinforced side. **Both sides now carry `*_casualties_scope`** — the predicate is the casualty DISTRIBUTION (a lone side is never labelled; a naive raw-vs-share mirror labels one on 3 of 8 seeds); event side dicts carry `casualties_scope: "army"` + `lead_remaining`. **Trust names its price:** `combat_executor.pair_contribution_breakdown` is the single source for the arithmetic AND the copy (drift-pinned over 1,000 cells); the muster row branches on cause (relationship arm verbatim); `battle_report["trust_note"]` both sides of the field; diorama `faith`. Levers `CombatExecutor.BOTH_SIDES_NAME_THEIR_SCOPE` / `TRUST_NAMES_ITS_PRICE`. **Routed: IQ5-R1** — `_distribute_casualties` splits by FULL strength (a mechanical change, owned by the next combat-mechanics row). **Review round (Sept 16, 2026): 20 of 21 findings survived and ALL FIXED (IQ5-RV1..RV9)** — every faith sentence is RELATIVE (`_faith_share_clause`: the absolute "half his weight" was false on every non-neutral pair), the jealousy card prices the grievance's increment (`pair_contribution_breakdown(without_grievance=True)`), one `_reinf_name` display function over every reinforcement name (IQ5-10's rename is behind the lever now — the record's byte-identity claim was false), the co-located attacker is named, a rubbled reinforcer reports his realised loss, a lone destroyed corps carries `applied_casualties`, the enemy-phase colours follow the side, the caption sits under the corps; tests 185, sweep 140 of 140/0 INERT. IQ-6 followed (above).
>
> **▶ IQ-4 "THE CABINET IS VISIBLE" — LANDED September 14, 2026** (landing record = `IMPROVEMENT_QUEUE_SPEC.md` §1.3; rules = `SYSTEMS_REFERENCE.md` §43; defects = `BUG_FIXES.md` §The Cabinet Is Visible). **ONE source `diplomatic_dialogue.mission_status`** for every surface that speaks of a running mission — the Strategic Ledger's Cabinet block, one `diplomatic_mission` rail row with a Recall, the `diplomatic_mission_ended` log type (163 → 164), the help block, tutorial step XIV, the Talleyrand tab. **PR-D3 "The Court's Favour" ⚠ RULED, FOR USER CONFIRMATION:** +2 a funded turn (cap +10) on the player's cooperative offers to the courted court; COURT is the quicker, dearer road and **HOLDS at the ceiling** (the favour ends with the mission); Talleyrand's counsel prices both relation missions by `forecast_mission_to_accept` (the formula's own relation-free sum, rounded once — 674/674 forecasts match real ticks after the review round, IQ4-R1..R15). Also fixed: MS-9b (IMPROVE/REASSURE parked at 99 forever), COURT's decay exemption leaking to every pair, **a pre-existing P1 — a failed counter-offer stranded Talleyrand IN_TRANSIT for the rest of the campaign**, settlement gratitude never reaching an alliance offer (case). **Completion MET:** the counsel arm (`tools/playtest_driver.py --missions advisor`) ticks **3 / 3 / 4** mission types on three seeds, the control **0**. The Godot binary exists on the user's machine (`C:\Users\User\Downloads\Godot_v4.4.1-stable_win64.exe\...`) — XR-1 parse harness + boot smoke ran. IQ-5 followed (above).
>
> **▶ IQ-3 "THE COALITION IS RARE" — LANDED September 14, 2026** (landing record = `IMPROVEMENT_QUEUE_SPEC.md` §1.2; measurements `docs/audits/IQ3_COALITION_CADENCE_2026_09_14.md`; rules = `SYSTEMS_REFERENCE.md` §42). PR-D1's revolving door — 6–8 coalitions in 40 turns on the commanded arm, every one dissolved by France ratifying the league's own peace offer while the alarm stood at 90+, the ≥90 override cancelling the cooldown — is closed by **"The League Is Spent"**: a TREATY that dissolves a league halves Europe's alarm against its target (`coalition.THE_LEAGUE_SPENDS_ITS_ALARM`; `LEAGUE_SPENT_DIVISOR = 2` ⚠ FOR USER CONFIRMATION), the treaty's own annexation/vassalization alarm kept whole — derived, no timer, zero serialized fields, GR5. Measured A ambient / B accepts / C + `--declare-war proceed` / D + declarations / E levers down = 1·1·1 / 1·1·1 / **2·1·2** / 3·2·2 / 8·4·5. Rider: Talleyrand's declare-war objection reads the projection (`diplomacy.declaration_alarm` is the single source). **The driver's `--declare-war` flag was DEAD since FA-S17-D6** — fixed in `resolve_policy`. Routed: PR-D1b (the AI settlement offer at war-age 2). The review round (four lenses plus refuters) fixed a confirmed P1 — breaking a truce bought the spend (`diplomacy.UNILATERAL_PEACE_REASONS`) — plus treaty eliminations (`_eliminate_nation(by_treaty=)`), the cap-clip pair order (`add_threat` stamps `applied`) and the projection's pre-declaration count; routed PR-D1c. **NEXT = IQ-4 "The Cabinet Is Visible".**
>
> **▶ IQ-2 "THE COLLAPSE IS LEGIBLE" — LANDED September 14, 2026** (landing record = `BUG_FIXES.md` §Collapse Legibility (IQ-2); rules = `SYSTEMS_REFERENCE.md` §41). The user's row: an annihilated France was told the winds favoured it and played on at zero provinces. **Scope note binding: legible, never terminal** — win/defeat stays with the Victory Pass. **A P1 sat underneath:** a landless France fell off `get_active_nations()` and fielded a FREE army (no upkeep/bankruptcy/desertion); `world_state.PLAYER_NEVER_LEAVES_THE_ROSTER` closes it. **ONE source `backend/game_logic/collapse.py`** (`get_collapse_state`, sandbox, ≤1 province) feeds every surface that speaks of the collapse — new standing headline `empire_reduced`, a Berthier collapse rung, Talleyrand, the sandbox arm of `get_defeat_imminent_state`, the end-turn banner, war room, ledgers, status report, Le Moniteur, chronicle; never add a second collapse predicate. ~20 general legibility fixes ride behind their own levers. This is row IQ-2 of the improvement queue (PR-X1) — CLOSED, its completion item pinned on a staged zero-province board. IQ-3 followed (above; the queue in `docs/STATUS.md` ▶ NEXT UP is the routing authority).
>
...
>
> **▶ THE IMPROVEMENT QUEUE (row IQ) IS THE LIVE ROUTING AUTHORITY — opened September 12, 2026 by user direction** (*"improve everything but win conditions one by one — let the agents make decisions on how; start with a comprehensive econ fix that finally makes it work better"*). **The queue lives in `docs/STATUS.md` ▶ NEXT UP and is authoritative for what is built next.** Ten rows, taken one at a time, each opened by a read-only decision fleet whose rulings become the build contract: ~~**IQ-1 the economy that bites**~~ (✅ **CLOSED Sept 14, 2026** — absorbed the DECLINED FA-S17-D2 gold-sink ruling, re-opened here by direction, plus IGR-X9 and the live half of `ECONOMY_REVISIT_SPEC.md` Track 3) → ~~**IQ-2 the collapse is legible**~~ (✅ **LANDED Sept 14, 2026** — PR-X1 closed; landing record `BUG_FIXES.md` §Collapse Legibility (IQ-2), filed in `IMPROVEMENT_QUEUE_SPEC.md` §1) → ~~**IQ-3 the coalition is rare**~~ (✅ **LANDED Sept 14, 2026** — PR-D1 closed, the league is spent; `IMPROVEMENT_QUEUE_SPEC.md` §1.2) → ~~**IQ-4 the cabinet is visible**~~ (✅ **LANDED Sept 14, 2026** — PR-D2/D3 closed, "The Court's Favour" ⚠ FOR USER CONFIRMATION; `IMPROVEMENT_QUEUE_SPEC.md` §1.3) → ~~**IQ-5 both sides of the butcher's bill**~~ (✅ **LANDED Sept 14, 2026** — PR-X2/X3 closed; `IMPROVEMENT_QUEUE_SPEC.md` §1.4) → ~~**IQ-6 Europe speaks its mind**~~ (✅ **LANDED Sept 16, 2026** — PR-X4 was the instrument's blindness; the volte-face's 15-turn window could not fit the game's own courtship; `IMPROVEMENT_QUEUE_SPEC.md` §1.5) → ~~**IQ-7 the satellites have a position**~~ (✅ **LANDED Sept 16, 2026** — "The Client's Petition": the premise had flipped after IQ-3; GRANT holds 3/3/3 satellites, REFUSE 0/0/0; `IMPROVEMENT_QUEUE_SPEC.md` §1.6) → ~~**IQ-8 the harness tells the truth**~~ (✅ **LANDED Sept 18, 2026** — the hash seed never moved the board; PR-D4 closes as cause unrecoverable and provenance becomes mandatory; PR-X5, IQ6-X1 and IQ6-D4 closed; `IMPROVEMENT_QUEUE_SPEC.md` §1.7) → ~~**IQ-9 the keyless parser gate**~~ (✅ **LANDED Sept 18, 2026** — the suite was keyless by discipline, not construction; ONE seam, authored cassettes, 112 pins counting live calls; `IMPROVEMENT_QUEUE_SPEC.md` §1.8) → ~~**IQ-10 the client pass**~~ (✅ **LANDED Sept 19, 2026** — the instrument is committed and is the deliverable; 160 frames at two Interface Scales; 7 fixed + 2 routed; UI/UX 7.5 HELD ⚠ FOR USER CONFIRMATION; `IMPROVEMENT_QUEUE_SPEC.md` §1.9). **THE IMPROVEMENT QUEUE IS CLOSED — routing returns to `docs/STATUS.md` ▶ NEXT UP and the ROADMAP.** **⛔ WIN CONDITIONS ARE EXCLUDED BY DIRECTION** — `sandbox_mode` suppresses victory AND defeat on every Europe world and that stays owned by the Victory & Objectives Pass, ROADMAP positions 12–13; no IQ row builds it or half-builds it, and IQ-2's scope note states in writing that making the collapse legible does not make it terminal.
>
...
> loop's `lord == player` invariant). **Everything that round left flagged
> was then BUILT rather than filed:** WO-D12 (both objection dicts stamp a
> display-only `trust_gain_modifier` off the new single source
> `authority.objection_trust_modifier`; the dialog spends it on the
> authority line — *"they have taken your measure (trust rewards x0.50)"*);
> the unpinnable guard became the pure predicate
> `courtier_is_of_the_same_house`, pinned where a non-player lord is
> reachable; and a yield rider (own lever) makes NA-2's agenda bias outrank
> the roster — measured inert on the ambient board for a STATED reason
> (nobody covets Switzerland), so pinned by construction. The redistribution
> worry was measured on the shipped board and does NOT bind: 40 turns,
> uncapped 19 events / 38 DP / 95 nominal vs capped **4 / 8 / 20**.
> **A claim of mine was falsified by tracing it:** the capped satellite does
> not "recover instead of rebelling" — it bleeds 47·34·21·8 and rebels
> three turns later, which is why the series re-converges at [31] (that
> index IS the delayed rebellion). ✅ **UX23-R9 CLOSED** on measurement:
> `reveille` cuts at 51.6% of peak and the music RISES to 85% under the
> 0.8 s fade, `to_the_color`'s next phrase hits 90.5% inside it, and no cap
> in 3.0–9.0 s is better — so the FADE was the lever, not the cap; `fade_s`
> is now per-cue and the two condemned bugles get a 2.0 s decrescendo
> (`fanfare` left alone at 22.5%).
> `tests/test_wo_slice9_the_courting_cap.py` (55); **35 mutations, 35 killed,
> 0 inert at close**; suite **19,055 / 4 skipped**; parse harness EXIT=0,
> boot smoke 0 SCRIPT ERROR.
>
> **▶ ROW REV'S FOLLOW-UPS — three of four CLOSED August 31, 2026.**
> Landing record = `docs/BUG_FIXES.md` §THE FOLLOW-UPS (August 31,
> 2026); owner file `docs/REV_FOLLOWUPS.md` struck to match. **REV-F1**
> settled by experiment and REAL, not masked — a mid-turn save/load
> bought a second 2× GLORIOUS CHARGE on a pair already engaged; fixed as
> the FIFTH documented non-clear in `load_game`. **REV-V3** — the rail's
> unmapped tail joined, and an AST census over every producer found the
> filed 33 wrong by seven in BOTH directions (four live types shipped as
> bare literals; three of the 33 have no producer at all and now stand in
> `notifications.RAIL_EXEMPT_TYPES`); the floor pin is a two-directional
> census. **REV-V4** staged and shot — and the row's own recipe could
> never have worked (a standing march auto-secures, IGR-X5). `tests/test_rev_followups_2026_08_31.py` (22); **20 mutations, 20
> killed, 0 inert at close** (one INERT found and repaired); suite
> **18,996 / 4 skipped**; M1–M7 + `BASELINE_SERIES` byte-identical.
> ✅ **UX23-R9 CLOSED September 1, 2026** (in the slice-9 session) — the
> phrase-boundary framing was a dead end for BOTH bugles, not just
> `to_the_color`. The answerable question is how loud the cue is when the
> fade starts: `reveille` cuts at 51.6% of peak and RISES to 85% under the
> 0.8 s fade. No cap in 3.0-9.0 s does better than ~60%, so the FADE was
> the lever — `fade_s` is now a per-cue registry field and the two loud
> cuts take a 2.0 s decrescendo. Record = `BUG_FIXES.md` §UX23-B.
>
> **ruff clean · Godot parse harness EXIT=0 (47 scripts) · boot smoke 0 SCRIPT ERROR.**
>
...
> harness EXIT=0.
> ✅ **UX23-R9 CLOSED Sept 1, 2026** — phrase-completeness turned out to be
> the wrong question (no cap in 3.0-9.0 s ends either bugle on a phrase);
> the answerable one is loudness at the cut, and the FADE is the lever.
> ⚠ OPEN: the structural answer to "too early" =
...
> SPEC'D + GATE RULED August 15, 2026 (seventh session that day; rulings
> at the recommended defaults under the user's delegated grant): fix
> design + gate record = `docs/PETITION_POPUP_REVISIT_SPEC.md` v1.1,
> authoritative for the whole grievances-and-popups revisit slice** (F1
> "The Antechamber" tier split + nine structural fixes; §6 RULED
> Q1(a)/Q2(a)/Q3/Q4/Q5 with the Q1 L1-liveness re-open condition;
> **BUILD-READY per §9**, slots at the user's discretion around row NP),
> PC15-16/-18 are client `.gd` surfaces for the next visual pass
...
> `start_date`, legacy byte-identical) → HC-1 blockade war-score
> component (±15 on the PT-J2 ledger, SPARSE third key) → HC-2 ledger
> narration (war-room rung + 25k battle-report clause) → HC-3 crowned-
> name envoy clauses (flavor half; HC-D1 filed) → HC-4 the naval-balance
> duo (shore_supply_state presence semantics + AI table-price billing;
> **the ONE sanctioned BASELINE_SERIES re-record, 4-arm flip experiment,
> parity sole cause at index 12; Britain's descent cadence byte-identical
> both arms**) → HC-5 tutorial step XIV honesty → HC-G Le Moniteur (the
> deterministic Gazette: serialized `gazette_issues` cap 20, 5-turn
