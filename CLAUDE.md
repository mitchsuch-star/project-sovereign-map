...
- **Peace Deals - Settlement UI Cleanup `DWL-SET-SC5R` repair is up next, split into SC-5R-1 (LANDED) + SC-5R-2 (NEXT).** SC-5R-1 backend EDIT payload contract + scoped `draft_key` persistence + `author_gold_indemnity_terms` schema fix + tampered/cut clause-type pre-stage and pre-ratify revalidation is landed (22 new tests, full suite `10204 passed, 1 skipped`). SC-5R-2 ships the Godot EDIT-mode editor popup (clause add/edit/remove controls, covered-enemy picker, Submit for Review button, draft round-trip on Back Out / War Detail reopen), Godot result/notification routing for active-vs-archived settlement review targets, and incoming-offer action label/copy alignment. Player-quality gate remains **NO-GO** until SC-5R-2 lands. G2-Slice-1 through G2-Slice-5 plus G2-Slice-G2a-G2f remain landed. Slice G AI-ally settlement agency stays blocked until SC-5R-2 ships and Gate 4 manual smoke (including rejected/losing concession path) is recorded.
