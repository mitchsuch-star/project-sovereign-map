...
- **Settlement UI Cleanup G2-Slice-4 — LANDED May 7, 2026 via default defer-and-hide package.** Active next gate is **G2-Slice-5 Presentation & Metadata (SC-15/15b/16/17/19/20/23–25)**.

- **G2-Slice-4 audit follow-up:** `tests/test_incoming_offer_deferral_no_leaks.py` now behavior-checks that deferred handler and stale-save API paths do not emit notification rail items, dispatch events, or backend popup payloads; `main.gd` now routes malformed `diplomatic_dialogue.type == "incoming_settlement_offer"` responses to the deferred-message branch before the generic proposal-confirm route.

- **Imperial Settlement current handoff:** Slices A1-A3, B1-B3, C1a/C1b, C2, D1/D2, E, Slice F UI routing (commit `0b9289e`), Settlement UI Cleanup G2-Slice-1 (Foundation, commit `b699f4c`), G2-Slice-2 (Entry Safety, commit `4310903`), G2-Slice-3 (Continuity, May 7, 2026), and G2-Slice-4 (Incoming Offers — default defer-and-hide, May 7, 2026) are landed. `INCOMING_OFFERS_DEFERRED = True` lives in `backend/game_logic/settlement_preview.py`; `handle_incoming_settlement_offer_action(...)` short-circuits with `incoming_offer_deferred` BEFORE pop/stage/mutate; `incoming_settlement_offer` is dropped from `CURRENT_TURN_OFFER_TYPES` / `MAILBOX_SUMMARY_LABELS` / `DIALOGUE_PRIORITY` (kept in `SETTLEMENT_FAMILY_DIALOGUE_TYPES` per SC-18 v0.17 for stale-save defensive guards); `/mailbox`, `/pending_envoy`, and `/mailbox/activate` defensively strip / reject any stale-save records of the type; Godot `PROPOSAL_CONFIRM_DIALOGUE_TYPES`, `SETTLEMENT_DIALOGUE_ACTIONS`, the `proposal_confirm_popup.gd` settlement match arm, the `_show_confirm_dialogue_from_response` payload fallback, and the two pending-envoy/mailbox-activate result branches all drop `incoming_settlement_offer`. New `tests/test_incoming_offer_deferral_no_leaks.py` covers the v0.16 §SC-5 deferral assertion list (mailbox count, mailbox/pending-envoy/mailbox-activate API, handler short-circuit, 50-turn AI soak, backend producer grep, Godot surface absence). **Next: Settlement UI Cleanup G2-Slice-5 (Presentation & Metadata, SC-15/15b/16/17/19/20/23–25)** per `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.17. After cleanup: manual Godot smoke → Slice G AI/ally settlement agency → Final Gate test bundle → 8.EVAL → Phase 8.5.
