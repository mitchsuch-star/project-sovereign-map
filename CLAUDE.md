...
- **Imperial Settlement current handoff:** B3 lifecycle + Slice C1a side-pressure foundation are landed. New module `backend/game_logic/settlement_scoring.py` exports `compute_direct_scores_by_enemy(world, war_instance, *, proposer_side, covered_enemy_participants)`, `select_direct_score(per_member)` (max with alphabetical tie-break, names `direct_score_source`), and `compute_side_pressure_score(...)` — the spec §6.3 power-weighted average (`{"major": 3, "secondary": 2, "minor": 1}` with `DEFAULT_POWER_TIER = "secondary"` fallback) using traditional half-away-from-zero `round()` so the Pressburg worked example (line 1153) reproduces exactly. The aggregator clamps to `[-100, 100]` (spec line 239), accepts a caller-supplied memoized `direct_scores` map (spec line 238 — settlement preview / confirm reuse the same map for side pressure, direct-score gates, burden penalties, advisory rows, and `weak_pressure_penalty` checks), and surfaces hard stops `no_covered_enemy_participants` / `no_direct_war_score_for_covered_enemy` per spec line 235 / 237. C1a is pure (no mutation of world / war_instance, no ratification, no live wiring into `diplomacy.py` or `diplomatic_templates.py`). **C1a foundation only — the common-peace acceptance formula (`base_side_pressure`, `term_harshness_penalty`, leader-own-loss clamp, burden penalty, projected hegemony, war-objective alignment, war exhaustion, abandoned-by-ally) is the C1b sub-gate.** Verification: 15 new C1a tests in `tests/test_settlement_side_pressure.py`; full suite `9537 passed, 1 skipped`; ruff clean. **Remaining: C1b acceptance formula + 11+ tuning-gate fixtures + tuning escalation order, then C2 endpoints/dialogue/advisory/Godot routing, then D1/D2 reactions, then E presentation surface.**
