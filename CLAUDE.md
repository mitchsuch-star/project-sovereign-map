...
- **NEXT UP — Settlement Conversational Re-front (May 28, 2026) — DESIGN GATE, NEEDS APPROVAL, DO NOT CODE:** `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` v0.1 (vision). **Immediate next action: finish the spec (resolve its §8 open questions into a v0.2) and audit it before any code or approval.** Gate 4 smoke surfaced that the SC-5R-2 editor diverged from the conversational bilateral-diplomacy pattern — it ships only the rawest clause-assembly tier as a blank, unguided form that allows invalid combos (e.g. self-referential `liberation`: France liberates France from France, rejected only at Submit). The re-front makes common peace work like a bilateral peace proposal: Talleyrand proposes a valid baseline, you steer by intent (Harsher / More generous) with **live per-court acceptance**, and the structured picker editor becomes the **opt-in valid-by-construction deep tier** — extended to **multi-party** (one settlement covering a set of enemy courts, each with its own acceptance). It is mostly wiring existing machinery (`generate_suggested_terms` pattern, `calculate_common_peace_acceptance`, the concession-baseline generator, Voice Bible). On approval → v0.2 detailed spec → Slices 1–3; the deferred `DWL-SET-SC5R-3` inline merge-conflict controls fold into Tier 3. The SC-5R editor and the `suspend_settlement_editor` Back Out fix remain LANDED — this re-fronts the front door, it does not revert them. Optional interim band-aid (independent): filter the dependency-clause pickers (`vassalage`/`subjugation`/`liberation`) so invalid combos cannot be authored, to de-risk the current Gate 4 pass. Slice G AI-ally agency stays separate and later.
...
- **Settlement Conversational Re-front** — v0.1 vision drafted, NEEDS APPROVAL. DO NOT CODE. `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` (next-up settlement priority; make common peace work like bilateral diplomacy, multi-party aware)
...
| Peace Deals (umbrella + sub-specs) | **Next-up vision: `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` (conversational multi-party re-front — DESIGN GATE, needs approval).** Current gate: `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.27 and `docs/STATUS.md`. Historical anchors: `docs/PEACE_DEALS_UMBRELLA_SPEC.md`, `BILATERAL_PEACE_HARDENING_SPEC.md`, `WAR_PURPOSE_SCORE_SEMANTICS_SPEC.md`, `WAR_BARGAIN_SPEC.md`, `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md`, and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md`. Treat older Slice B / Final Gate notes as historical unless STATUS points back to them. |
