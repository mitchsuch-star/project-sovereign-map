...
- **Map Readiness Closure Pass — Phase 3 COMPLETE (§§3.1-3.4).** §3.1 (nation config factory — `DEFAULT_NATION_DEFAULTS` fallback + `create_marshals_from_data` / `create_diplomat_from_data` factories; adding a new nation now only needs a capital + diplomat + optional override), §3.2 (shared topology endpoint — backend `GET /map_topology`), §3.3 (centralize nation colors), and §3.4 (prompt/parser fallback de-hardcoding) are all done. Remaining non-art map-readiness items are the Phase 4 map-art pipeline (§§4.1-4.4). See `docs/STATUS.md` and `docs/SCALE_READINESS_PLAN.md` §§3-4.
