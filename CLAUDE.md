...
- **AUTHORITATIVE Imperial Settlement handoff (May 7, 2026):** `docs/STATUS.md` and `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.19 supersede the older Slice E / Final Gate notes below. Settlement UI Cleanup v0.19 is the next coding target. Start with Gate 1 doc scans/consolidation, then code Gate 2 in order beginning with G2-Slice-1 Foundation (SC-1 through SC-4b). Do not run the old bundled Final Gate as the active closeout, and do not start Slice G / AI-ally settlement agency first. Full Treaty Settlement remains approved: player-authored war-scoped terms, live consequence/acceptance preview, real revise/re-author flow, incoming-offer package preservation if offers ship, acceptance/hard-stop mutation gates, and SC-4b's explicit zero-cost default for settlement entry/editor/refusal/ratification paths unless a later product decision records a non-zero `confirm_settlement` cost in both spec and STATUS.
