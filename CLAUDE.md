...
- **Settlement Guided Terms** — v0.1 DRAFT, NEEDS APPROVAL. DO NOT CODE. **NEXT UP (June 9, 2026 re-sequencing): fold amendments GT-A1..4 from `docs/SETTLEMENT_GATE4_PREFLIGHT_AUDIT.md` §7, then user approval — it now precedes the (end-of-queue) Gate 4 smoke.** `docs/SETTLEMENT_GUIDED_TERMS_SPEC.md` — replaces the Tier-3 freeform "Adjust terms" editor (identity-picker model defaults `France`/`France`, reads as vague, diverges from the bilateral guided `terms_guidance` flow) with **guided per-court demand authoring** folded into the per-court rows (direction implied; valid-by-construction; no submit-blob so the merge/replace tension dissolves). Two Gate-4 editor transport/validation bugs already fixed at `067e431`.
