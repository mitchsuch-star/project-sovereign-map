...
- **Peace Deals - Phase A COMPLETE, Gate 1 PASSED. WB-A/WB-B/WB-C/WB-D COMPLETE. Imperial Settlement / Ally Participation Slice A1 FOUNDATION GATE COMPLETE; A2 IS THE NEXT GATE.** BPH-A through BPH-D + WPS-A through WPS-D all landed. Gate 1 verified (9/9 smoke criteria). `threat_coalition` retired (merged into `balance_of_europe`). WB-A/B/C/D landed with ~160 tests. **Slice A1 (May 1, 2026) — foundation only.** Landed: `WorldState.next_war_instance_id`/`war_instances`/`archived_war_instances` containers with `1`/`{}`/`[]` save/load defaults; empty-safe `WorldState.get_war_instances_by_leader()` + `get_war_instances_by_participant()` with single `invalidate_war_instance_indexes()` dirty-flag hook + lazy rebuild; touched `_is_nation_eliminated` refactored from raw `world.regions.values()` scan onto cached `world.get_nation_regions(...)`; `backend/game_logic/settlement_helpers.py` exporting `assert_war_instance_invariants(world, *, context=...)` + `WarInstanceInvariantError`; synthetic 20-active-`war_instance` fixture in `tests/helpers/full_europe_settlement_fixtures.py` covering the canonical 13 DG-1 roster + 6+ participant coalition side; 13 new foundation tests in `tests/test_war_settlement_foundation.py` (17 total). Strictly foundation — no behavioral common-peace, no contribution accrual, no settlement reactions. Active settlement work continues from `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.21 Slice A2 "war-entry threading" bullets and `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.24 §7. A2 must thread `war_id` through every WAR-entry seam (`ensure_war_instance_for_pair(...)`, `attach_pair_to_war_instance(...)` / `attach_participant_to_war_instance(...)`, `validate_war_declaration(...)`, cascade via a `CascadeContext`, and the live `backend/game_logic/vassal.py` rebellion call site). Future nations enter settlement one by one through `docs/ADDING_CONTENT.md` plus a focused settlement readiness fixture when scenario/map data adds them; synthetic full-Europe fixtures must extend the active roster before adding future-nation regions/controllers. See `docs/PEACE_DEALS_UMBRELLA_SPEC.md` for historical context.
