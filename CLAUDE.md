...
- **Settlement Conversational Re-front** — **v0.6 APPROVED May 29, 2026** (audited five times before approval; Slice 0 + Slice 1 + REFRONT-V voice LANDED May 30 at `ec1081d`; **Slices 2 + 3 IMPLEMENTED May 30 + Slice 3 audit fold (CRITICAL ratify-gate V1–V5 defense-in-depth; MAJOR role-aware pickers; MAJOR OQ#4 panel re-homed as REFRONT-9) — all implementation slices done except the REFRONT-9 OQ#4 focused breakdown, Gate 4 manual settlement smoke is the remaining gate**). `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` + audit `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_AUDIT.md` (next-up settlement priority; common peace as a conversational multi-party settlement table; per-court ratification gate; per-court baseline direction from `direct_scores` via the `select_direct_score` tuple-or-`None` contract; SC-32 D5 no-conference-copy boundary; remaining: Gate 4 manual smoke, then Slice G)
- **Settlement Guided Terms** — v0.1 DRAFT, NEEDS APPROVAL. DO NOT CODE. `docs/SETTLEMENT_GUIDED_TERMS_SPEC.md` — replaces the Tier-3 freeform "Adjust terms" editor (identity-picker model defaults `France`/`France`, reads as vague, diverges from the bilateral guided `terms_guidance` flow) with **guided per-court demand authoring** folded into the per-court rows (direction implied; valid-by-construction; no submit-blob so the merge/replace tension dissolves). Sequenced **after the Gate 4 smoke**. Two Gate-4 editor transport/validation bugs already fixed at `067e431`.
...
| Peace Deals / Imperial Settlement | **Active: `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` v0.6 (APPROVED May 29, 2026; Slices 0–3 + REFRONT-V voice IMPLEMENTED/LANDED (Slice 3 + audit fold), OQ#4 panel owned as REFRONT-9, Gate 4 manual smoke next).** Cleanup gate is now `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` **v0.32 GO** (SC-5R + SC-32/Slice G2 LANDED) with **Gate 4 smoke open**, then `docs/STATUS.md`. Older Imperial Settlement plan/spec docs remain historical anchors; do not route active work back to v0.19-v0.27, Slice F, Slice E, or a fresh G2-Slice-1 start. |
...
| Peace Deals (umbrella + sub-specs) | **Active: `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` v0.6 + audit `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_AUDIT.md` (conversational multi-party re-front — APPROVED May 29, 2026; Slices 0–3 + REFRONT-V voice IMPLEMENTED/LANDED (Slice 3 + audit fold), OQ#4 panel owned as REFRONT-9, Gate 4 manual smoke next).** Current cleanup gate: `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` **v0.32 GO** (Gate 4 smoke open), then `docs/STATUS.md`. Historical anchors: `docs/PEACE_DEALS_UMBRELLA_SPEC.md`, `BILATERAL_PEACE_HARDENING_SPEC.md`, `WAR_PURPOSE_SCORE_SEMANTICS_SPEC.md`, `WAR_BARGAIN_SPEC.md`, `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md`, and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md`. Treat older Slice B / Final Gate notes as historical unless STATUS points back to them. |
