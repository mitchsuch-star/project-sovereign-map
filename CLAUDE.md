...
- **Peace Deals - Phase A COMPLETE, Gate 1 PASSED. WB-A/WB-B/WB-C/WB-D COMPLETE. Imperial Settlement / Ally Participation Slices A1 + A2 + A3 + B1 + B2 ORDERING GUARD LANDED; B2 EMITTER CALL-SITE WIRING IS NEXT.** **Slice B2 ordering guard (May 2, 2026) — `accrue_battle_contribution()` entrypoint + `record_battle()` ordering pin.** New `backend/game_logic/war_contribution.py::accrue_battle_contribution(world, *, attacker_nation, defender_nation, winner_nation, attacker_casualties, defender_casualties, location, war_id, attacker_participants, defender_participants, nation_theater_strength, turn)` is the canonical battle-bucket accrual entrypoint per spec §9.2 / §9.4. Resolves `war_id` via `world.get_war_instances_by_participant()` (cached) when omitted, validates side membership (no-op when both nations land on same side), uses `adapt_legacy_battle_record(...)` to fill theater defaults so legacy single-attacker/single-defender callers work unchanged, and accrues `episode["battle"]` + `episode["total"]` distributed by `nation_theater_strength` share with floor-1 per spec §9.4 line 622. Internal `_battle_side_raw` implements the spec §9.2 formula `inflicted // 100 + suffered // 250 + decisive * 25`; `_is_decisive_battle` mirrors the war-score decisive criterion (ratio > 2:1 AND total > 10,000); no per-war decisive cap on settlement side (episode boundary already bounds it). `backend/game_logic/diplomacy.py::record_battle()` calls the entrypoint AFTER the `is_at_war` precondition but BEFORE the 1000-casualty war-score early return, so sub-1000 battles still accrue settlement contribution per spec §9.4 line 713. 8 new B2 ordering-guard tests in `tests/test_war_contribution_scores.py`: source-order assertion via `inspect.getsource` proving the accrue call appears before the gate, behavioral sub-1000 fixture proving accrual happens with empty `battle_records`, regression test proving above-1000 battles still produce both records, and 4 function-safety tests (no-op when no active war_instance, no-op same-side, skip participants without active episode, theater-strength distribution when explicit). Verification: `9439 passed, 1 skipped`; ruff clean. **B2 emitter call-site wiring (theater-aware updates to `_post_combat_pipeline()` / `_execute_attack()` inline path / auto-dispatch charge / glorious-charge pipeline, plus occupation events, treaty-clause / British-subsidy support events, and one-hop adjacency participant detection) is the next sub-gate.** **Slice B1 (May 1, 2026) — contribution store + share math + standing classifier.** New module `backend/game_logic/war_contribution.py` ships the spec §9.1 data shape, episode helpers (`canonical_episode_id`, `open_episode`, `close_episode_for_exit`, `current_episode`, `iter_active_episodes`), current-episode math (`current_episode_total`, `current_episode_material_total`, `total_side_current_episode_contribution`, `total_side_material_contribution`, `contribution_share`, `material_contribution_share` — all zero-safe per spec §9.1 zero-denominator rules), the §9.6 old-record adapter (`adapt_legacy_battle_record`), and the pure §8.2 standing classifier (`classify_standing(...)` + composite `compute_standing_inputs` / `standing_for_participant`). Standing classifier accepts term-derived booleans (`has_active_bargain_stake`, `has_survival_stake`, `is_named_beneficiary`, `has_direct_territorial_interest`, `is_treaty_ally_materially_involved`, `rival_strengthened`, `is_vassal_auto_join`) explicitly with `False` defaults so B1-only callers and tests never invent Slice C/D reaction state; vassal auto-join cap blocks all promotion paths except independent material thresholds (§8.2 line 506); `rival_strengthened` for minor tier requires `material_contribution_points > 0` to promote past `no_standing` (§8.2 line 497); staying-power-only contribution can never seat or consult (`material_contribution_points = battle + occupation + support` only). `WorldState.war_contribution_scores: Dict[str, Dict[str, Dict]] = {}` with deep-copy `to_dict()` / `from_dict()` round-trip + `{}` default for pre-B1 saves. Settlement-helpers `_rewrite_absorbed_war_id_in_contribution` now handles the real B1 shape (per-nation dicts with `current_episode_id` / `episodes` / `historical_total`): episodes union with survivor-wins on collision, historical_total accumulates, current_episode_id prefers survivor. `war_leader_score` reads contribution share via `material_contribution_share` (no-op until B2 emitters accrue). 55 new tests in `tests/test_war_contribution_scores.py`; 2 A3 tests migrated in `tests/test_war_settlement_merge.py` (no-longer-absent `war_contribution_scores` container). Verification: `9431 passed, 1 skipped`; ruff clean. **Slice A3 (May 2, 2026) — merge/archive/leader invariants.** BPH-A through BPH-D + WPS-A through WPS-D all landed. Gate 1 verified (9/9 smoke criteria). `threat_coalition` retired (merged into `balance_of_europe`). WB-A/B/C/D landed with ~160 tests. **Slice A3 (May 2, 2026) — merge/archive/leader invariants.** `backend/game_logic/settlement_helpers.py` adds `merge_war_instances(world, *, candidate_war_ids, ctx=None)` (full transitive merge transaction per spec §7.6: connected-component walk, side validation without mutation, oldest-`created_sequence` survivor, participant/pair/episode union with multi-objective preservation, side-scoped leader replacement, absorbed-`war_id` rewrite on `diplomatic_commitments` + `pending_dispatch_events`, no-op-safe hooks for absent Slice B/C containers `war_contribution_scores` / `pending_settlement_dialogues` / `settlement_route_payloads`, atomic swap, index invalidation, always-on post-merge `assert_war_instance_invariants` assertion); `war_leader_score(world, nation, *, war_id, side)` settlement-specific scorer; `_choose_leader_for_side` side-scoped chooser delegating coalition sources to `coalition.select_coalition_leader`; `mark_participant_eliminated_in_all_wars` (no separate-peace reaction); `archive_terminal_war_instances` (10-turn retention via `ARCHIVE_RETENTION_TURNS=10`). `validate_war_declaration` returns `merge_required=True` advisory; `ensure_war_instance_for_pair` runs merge then re-validates; `attach_pair_to_war_instance` triggers merge when pair owned by a different active instance; `create_war_bargain_commitment` snapshots `war_id` / `side_at_creation` / `side_leader_at_creation` (rewritten on merge but `side_*` preserved per spec §11.3 line 1573); `_eliminate_nation` calls `mark_participant_eliminated_in_all_wars` BEFORE the diplomatic-state teardown; `_advance_turn_internal` calls `archive_terminal_war_instances` once per turn after the idempotency guard; `resolve_pair_to_resolved` stamps `ended_turn` + `end_reason="all_pairs_resolved"` when the last active pair resolves. 23 new tests in `tests/test_war_settlement_merge.py`; 3 new fixtures (`build_three_instance_chain_merge_fixture`, `build_multi_objective_merge_fixture`, `build_side_scoped_leader_source_fixture`); 4 A2 hard-stop tests migrated in `tests/test_war_settlement_instances.py` (true `side_conflict` cases stay hard-stops; cross-instance attach now triggers merge and retargets to the survivor). Verification: `9373 passed, 1 skipped`; ruff clean. **Slice A2 (May 1-2, 2026) — war-entry threading.** `backend/game_logic/settlement_helpers.py` exports `CascadeContext` (transaction object so `_process_war_cascade()` does not exceed nine positional args), pre-commit `validate_war_declaration(...)`, `ensure_war_instance_for_pair(...)` (skeleton allocation + pair / participant reuse + ARMISTICE → WAR resumption reusing the same `war_id`), `attach_pair_to_war_instance(...)`, `attach_participant_to_war_instance(...)`, `mark_pair_armistice(...)`, `resolve_pair_to_resolved(...)`. Live wiring threads `war_id` through `declare_war()`, every cascade attach point inside `_process_war_cascade()`, vassal rebellion / vassal-release rebellion, `resolve_join_opportunity(..., "accept")`, `accept_counter_bargain(...)`, debug/scripted `set_diplo_state WAR`, combat auto-war fallback, and `_process_armistice_expiration(...)`. **Slice A1 (foundation, May 1, 2026):** `WorldState.next_war_instance_id`/`war_instances`/`archived_war_instances` containers with `1`/`{}`/`[]` save/load defaults; empty-safe `WorldState.get_war_instances_by_leader()` + `get_war_instances_by_participant()` with single `invalidate_war_instance_indexes()` dirty-flag hook + lazy rebuild; `_is_nation_eliminated` refactored onto cached `world.get_nation_regions(...)`; `assert_war_instance_invariants(...)` + `WarInstanceInvariantError`; synthetic 20-active-`war_instance` fixture in `tests/helpers/full_europe_settlement_fixtures.py`. Active settlement work continues from `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` Slice B contribution tracker bullets and `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` §8. Future nations enter settlement one by one through `docs/ADDING_CONTENT.md` plus a focused settlement readiness fixture when scenario/map data adds them; synthetic full-Europe fixtures must extend the active roster before adding future-nation regions/controllers. See `docs/PEACE_DEALS_UMBRELLA_SPEC.md` for historical context.
...
| Peace Deals / Imperial Settlement | `docs/PEACE_DEALS_UMBRELLA_SPEC.md` (dependency graph, gates, cross-cutting decisions — read FIRST), then the relevant landed sub-spec: `BILATERAL_PEACE_HARDENING_SPEC.md`, `WAR_PURPOSE_SCORE_SEMANTICS_SPEC.md`, `WAR_BARGAIN_SPEC.md`. Slice A (war identity, war-entry threading, merge/archive/leader invariants) is fully landed; active settlement work continues into Slice B contribution tracking from `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` §"Slice B" and `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` §8. WB §7.2/§9.2/§9.3 have been reconciled; if future wording drifts, umbrella §4.2 remains the live acceptance source of truth. |
...
| Peace Deals (umbrella + sub-specs) | `docs/PEACE_DEALS_UMBRELLA_SPEC.md` (dependency graph, gates, cross-cutting decisions), `BILATERAL_PEACE_HARDENING_SPEC.md`, `WAR_PURPOSE_SCORE_SEMANTICS_SPEC.md`, `WAR_BARGAIN_SPEC.md`, `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` (Slices A1 + A2 + A3 landed; Slice B contribution tracker is the next gate). `backend/game_logic/settlement_helpers.py` exports `merge_war_instances`, `mark_participant_eliminated_in_all_wars`, `archive_terminal_war_instances`, `war_leader_score`, `assert_war_instance_invariants` (post-merge always-on), and the A2 war-entry plumbing. `tests/helpers/full_europe_settlement_fixtures.py` exposes the synthetic 13-nation + 20-active-`war_instance` fixture plus `build_three_instance_chain_merge_fixture`, `build_multi_objective_merge_fixture`, `build_side_scoped_leader_source_fixture`. |
