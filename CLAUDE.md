...
- **Settlement Conversational Re-front** — **v0.6 APPROVED May 29, 2026** (audited five times before approval; Slice 0 + Slice 1 + REFRONT-V voice LANDED May 30 at `ec1081d`; **Slices 2 + 3 IMPLEMENTED May 30 + Slice 3 audit fold; Gate-4 pre-flight fix slices PF-1/PF-2/PF-3 LANDED June 9, 2026 (D1–D6 fixed) — all implementation slices done except the REFRONT-9 OQ#4 focused breakdown (folds into Guided Terms per GT-A4); the Gate 4 manual smoke is RE-SEQUENCED to the end of the settlement queue (user decision June 9)**). `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` + audit `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_AUDIT.md` (common peace as a conversational multi-party settlement table; per-court ratification gate; per-court baseline direction from `direct_scores` via the `select_direct_score` tuple-or-`None` contract; SC-32 D5 no-conference-copy boundary; remaining: end-of-queue Gate 4 manual smoke, then Slice G)
- **Settlement Guided Terms** — **v0.2 (audit folded June 10, 2026), NEEDS APPROVAL. DO NOT CODE.** **NEXT UP: user approval of v0.2 — it precedes the (end-of-queue) Gate 4 smoke. The GT-A1..4 amendments + the Run #1 audit (`docs/SETTLEMENT_GUIDED_TERMS_AUDIT.md`, GO-with-changes, 0 CRITICAL / 9 MAJOR / 6 MINOR, all folded) are IN v0.2; the one fold-changed decision to confirm explicitly is D4 (mixed-direction rows).** `docs/SETTLEMENT_GUIDED_TERMS_SPEC.md` — replaces the Tier-3 freeform "Adjust terms" editor (blind while authoring — no live acceptance until REVIEW; identity-picker model defaults `France`/`France`; diverges from the bilateral guided `terms_guidance` flow) with **guided per-court demand authoring** folded into the per-court rows (direction-led menus per option, both demand/offer groups per D4; table-scoped valid-by-construction with a visible `treasury_line` per GT-A1; no submit-blob so the merge/replace tension dissolves — though D7's replacement-stage orphan class survives and stays cured by CH-5). Slice order GT-1 → 2 → V (voice, before/with 3) → 3 → 4. Two Gate-4 editor transport/validation bugs already fixed at `067e431`.
...
| Peace Deals / Imperial Settlement | **Active: `docs/SETTLEMENT_GUIDED_TERMS_SPEC.md` design gate next (v0.2 audit-folded June 10, 2026 + audit `docs/SETTLEMENT_GUIDED_TERMS_AUDIT.md`; user approval before code — confirm D4), then the queued settlement items; `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` v0.6 (APPROVED May 29, 2026; Slices 0–3 + REFRONT-V voice + PF-1/2/3 IMPLEMENTED/LANDED, OQ#4 owned as REFRONT-9 → folds into Guided Terms).** Cleanup gate is `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` **v0.32 GO** with the **Gate 4 smoke open, re-sequenced to the end of the settlement queue (June 9, 2026)**, then `docs/STATUS.md`. Older Imperial Settlement plan/spec docs remain historical anchors; do not route active work back to v0.19-v0.27, Slice F, Slice E, or a fresh G2-Slice-1 start. |
...
| Peace Deals (umbrella + sub-specs) | **Active: `docs/SETTLEMENT_GUIDED_TERMS_SPEC.md` design gate next (v0.2 + audit `docs/SETTLEMENT_GUIDED_TERMS_AUDIT.md` — folded June 10, 2026; user approval pending, confirm D4); `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_SPEC.md` v0.6 + audit `docs/SETTLEMENT_CONVERSATIONAL_REFRONT_AUDIT.md` (conversational multi-party re-front — APPROVED May 29, 2026; Slices 0–3 + REFRONT-V voice + Gate-4 pre-flight PF-1/2/3 IMPLEMENTED/LANDED, OQ#4 panel owned as REFRONT-9 → folds into Guided Terms); `docs/SETTLEMENT_GATE4_PREFLIGHT_AUDIT.md` owns the PF/DC/UX/CH/DW ledgers.** Current cleanup gate: `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` **v0.32 GO** (Gate 4 smoke open — re-sequenced June 9, 2026 to one end-of-queue run), then `docs/STATUS.md`. Historical anchors: `docs/PEACE_DEALS_UMBRELLA_SPEC.md`, `BILATERAL_PEACE_HARDENING_SPEC.md`, `WAR_PURPOSE_SCORE_SEMANTICS_SPEC.md`, `WAR_BARGAIN_SPEC.md`, `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md`, and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md`. Treat older Slice B / Final Gate notes as historical unless STATUS points back to them. |
