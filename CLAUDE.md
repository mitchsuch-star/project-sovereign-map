...
- **Peace Deals — Phase A COMPLETE, Gate 1 PASSED. WB-A/WB-B/WB-C/WB-D COMPLETE.** BPH-A through BPH-D + WPS-A through WPS-D all landed. Gate 1 verified (9/9 smoke criteria). `threat_coalition` retired (merged into `balance_of_europe`). WB-A (data model + creation + validation), WB-B (lifecycle: fulfillment + breach + void), WB-C (war-entry contract: `war_entry_score`, hard blocks, `join_opportunity`, declaration preview, counter-bargains, `repudiate_bargain`, AI rules, ledger integration), WB-D (presentation: commitments routing, notifications, witness scope, voiced templates, response routes, ledger badges) all landed with ~160 tests. See `docs/PEACE_DEALS_UMBRELLA_SPEC.md` + `docs/WAR_BARGAIN_SPEC.md`.
