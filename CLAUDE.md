...
- **Imperial Settlement current handoff:** B2 non-battle emitters and support attribution are landed. `war_contribution.accrue_occupation_event(...)` accrues 20/40/15/15 raw points for `enemy_region_captured` / `enemy_capital_captured` / `allied_region_restored` / `liberated_region_restored` (`treaty_transfer` logged with 0 points per spec §9.2 line 641); `emit_capture_occupation_event(...)` is the capture-path wrapper that classifies via `NATION_CAPITALS` + `get_starting_controllers()`. `accrue_support_event(...)` covers gold/subsidy `// 100`, AP `* 5`, manpower `// 500`, access/supply `1/turn` capped at 5 raw per `(war_id, supporter, support_kind)` per spec §9.2 line 674, with deterministic episode-id dedupe and an unattributed-`war_id` logging-only branch. `resolve_british_subsidy_war_id(...)` returns `(war_id, source_detail)` per the impl-plan tie-break order: `unique_eligible` → `matching_coalition_target` → `highest_overlap` → `oldest_sequence` → `unattributed_subsidy`. Wired call sites: `world_state.capture_region()` (occupation BEFORE `_eliminate_nation`), `world_state._ratify_treaty()` (`gold_lump` support, `territory_cede` allied restoration when `to_nation != proposer` AND `get_starting_controllers()[region] == to_nation`, vassal liberation per-region using the captured pre-release region list), `world_state._process_treaty_clauses()` (per-turn gold/manpower/AP), `coalition._process_british_subsidy()` (subsidy + attribution + event payload now carries `war_id` / `subsidy_source_detail`). Tests: 33 new in `tests/test_war_contribution_scores.py` covering the four occupation kinds, treaty_transfer no-accrual, dedupe, episode-turn-window filter, `emit_capture_occupation_event` capital + non-war guards, `capture_region` end-to-end (region capture + capital capture), support kind/source rejection, opposite-side filter, dedupe + zero-raw dedupe, access/supply cap, AP / manpower formulas, all four subsidy attribution branches, `_process_british_subsidy` end-to-end, `gold_lump` + `territory_cede` + per-turn gold + per-turn AP treaty paths, and same-turn replay dedupe. Verification: `9490 passed, 1 skipped`; ruff clean. **Remaining: B3 lifecycle / per-turn staying-power / war-entry seam wiring of `open_episode` / `close_episode_for_exit` / archive compaction / retention / full-Europe fixtures.**
...
- **Peace Deals - Phase A COMPLETE, Gate 1 PASSED. WB-A/WB-B/WB-C/WB-D COMPLETE. Imperial Settlement / Ally Participation Slices A1 + A2 + A3 + B1 + B2 (ORDERING GUARD + BATTLE EMITTER WIRING + NON-BATTLE EMITTERS) LANDED; B3 LIFECYCLE IS NEXT.** B2 non-battle emitters cover occupation event accrual (capture path + treaty cede + vassal liberation), support event accrual (one-time + per-turn treaty clauses + British coalition subsidy), episode-id dedupe across the new occupation/support paths, and access/supply per-`(war_id, supporter, support_kind)` cap of 5 raw points. Verification: `9490 passed, 1 skipped`; ruff clean. B3 lifecycle / per-turn staying-power / war-entry seam open/close / archive compaction / retention / full-Europe fixtures follow next.
