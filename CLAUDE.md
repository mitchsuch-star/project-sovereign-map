...
- **Peace Deals — BPH-A through BPH-D + WPS-A AUDIT FOLLOW-UP COMPLETE.** Term ownership, peace preview panel, fallout/commitment conflicts, ratification summary + dispatch, War Purpose popup + objective state + ticking score are landed; WPS-A audit follow-up fixed ticking persistence, armistice cleanup, campaign-log visibility, Godot objective rendering, combat auto-war staging, parser validation, coalition liberation, and AI-AI default objectives. Next: **WPS-B** (power cap gating). See `docs/PEACE_DEALS_UMBRELLA_SPEC.md` + `docs/WAR_PURPOSE_SCORE_SEMANTICS_SPEC.md`.
