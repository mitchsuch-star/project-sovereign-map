...
- Historical default-start WAR-pair probe language is **SUPERSEDED BY `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-1 / SC-8 / SC-13 / SC-27**. Cleanup-time normal UI must not mount EDIT or REVIEW from `war_id=None` / `would_backfill=True`; click-time resolution must return a concrete `war_id`, `selected_target_nation`, and `covered_enemy_participants`, or show humanized no-action/disambiguation copy.
...
135. Wizard availability for default-start WAR pairs is read-only and does not mutate `world.war_instances`; normal UI may not mount Settlement EDIT/REVIEW from `war_id=None` and must resolve a concrete `war_id` before editor open or show humanized no-action copy per `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-1 / SC-8 / SC-13.
...
147. Post-cleanup Slice G AI/ally settlement agency is explicitly tracked: AI war-leader `incoming_settlement_offer` remains the common-peace offer path only after cleanup SC-5 is explicitly reversed, while non-leader allies use petitions/advisories rather than co-authoring treaties.
