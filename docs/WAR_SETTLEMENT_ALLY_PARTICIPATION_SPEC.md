...
> **Status:** v1.18 PRE-A1 MAPPED-NATION FOUNDATION GATE - A1 is foundation-only and must start with mapped-nation capital helper safety, settlement containers, old-save defaults, empty index/cache scaffolding, and cache invalidation hooks before any behavioral settlement work. There is no off-map nation model in A1: current settlement participants must exist in scenario/map data; future nations enter one by one when the map/scenario adds them. A2 owns `war_id` threading; B owns contribution-before-war-score-gate hooks; C owns raw harshness / settlement dialogue taxonomy; D owns ratification cache-ordering. Coding may start from `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.15.
...
- Do not infer off-map status from `None`.
...
110. Live-code Britain fixture proves `NATION_CAPITALS["Britain"] == "Netherlands"` is treated as the current-map configured home/spawn region and can later be replaced by normal scenario data without creating an off-map settlement identity.
