...
- `war_id` resolves to an active `war_instance` with `ended_turn is None`, or the current player/target pair is an existing `WAR` diplomatic state that would lazy-backfill a valid settlement `war_instance` if the player clicked the action.
...
Wizard availability must be read-only. It must not allocate a `war_instance`, attach pairs, open contribution episodes, invalidate caches, or stage a dialogue. Use a pure probe that can report `would_backfill=True` for default-start WAR pairs; the click/command path remains the only lazy-backfill mutation path. This probe cannot be a thin wrapper around an eligibility helper that requires an existing `war_id`; `war_id=None` is a valid F1/bilateral entry, not an error.
