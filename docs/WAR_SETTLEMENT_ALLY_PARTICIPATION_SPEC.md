...
> **Status:** v1.33 A1-E LANDED / SECOND SLICE F AUDIT SYNTHESIZED AND READY FOR CODING - A through E backend/presentation work is green, but final player-readiness now requires a dedicated UI/UX closure slice before smoke. Common peace must be reachable from the normal diplomacy wizard and war-score/detail surfaces, not only typed command; default-start WAR pairs without `war_instance` must be shown as settlement-openable through a read-only backfill probe; `open_settlement` is free to open and not DP-gated; `settlement_confirm` must render a readable sectioned popup with humanized display fields; war-detail and coalition-detail surfaces must expose settlement CTAs with a mandatory war-scoped entry contract; ledger, HUD, popup, dispatch, rail, and campaign-log surfaces must humanize term/standing/warning/acceptance/error enums; incoming AI settlement offers need minimal backend response handlers plus Godot routing before final smoke; and final smoke is invalid until these gates close. The synthesized Slice F audit also pins implementer-level contract details before coding: one typed action schema for settlement dialogue responses with no legacy `actions[]` in player payloads, concrete `reopen_target` routing for stale/revise paths, a named settlement result feedback payload, staging-time stable non-empty `route_id` threading from confirm through ledger/dispatch/rail, wizard/detail entry signatures that preserve `war_id`, row/detail routing that emits `war_instance_id`, a popup fallback guard so settlement payloads never render as "DIPLOMATIC PROPOSAL - Unknown", side/outcome-first settlement comprehension, cross-war settlement dialogue collision behavior, notification review metadata continuity, player-defender open-settlement coverage, and raw-enum source guards. After Slice F/final smoke, the next explicitly tracked quality phase is AI/ally settlement agency: AI war leaders can already send `incoming_settlement_offer` per §16.5, while non-leader allies need a separate petition/advisory layer that lets them ask, warn, request spoils, or demand redress without getting a veto or conference turn. Current settlement participants must exist in scenario/map data; future nations enter one by one through the `ADDING_CONTENT.md` nation checklist plus settlement readiness fixture when the map/scenario adds them. Coding may start from `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.31.
