...
> **Status:** v1.33 A1-E LANDED / SECOND SLICE F AUDIT SYNTHESIZED - SUPERSEDED FOR CLEANUP READINESS BY `SETTLEMENT_UI_CLEANUP_SPEC.md` v0.27. This parent spec remains useful for settlement mechanics, but cleanup-time player readiness, incoming-offer exposure, route-id shape, rejected/losing recovery, and Gate 4 smoke are governed by the cleanup spec. Incoming AI settlement offers are defer-and-hide during cleanup unless SC-5 is explicitly reversed with producer, payload, mailbox/pending-envoy UI, package-preserving actions, incoming voice, and behavior tests together. Current settlement participants must exist in scenario/map data; future nations enter one by one through the `ADDING_CONTENT.md` nation checklist plus settlement readiness fixture when the map/scenario adds them.
...
## 3. Historical Pre-A1 Baseline (April 27, 2026)

This section is retained as historical context for why the Imperial Settlement work was needed. It is not the current implementation-readiness source. A1-E have landed; cleanup-time player readiness is governed by `SETTLEMENT_UI_CLEANUP_SPEC.md` v0.27 plus `STATUS.md`.
...
| Area | Current status | Active source for implementation/tests |
|------|----------------|----------------------------------------|
| War grouping | Landed via `war_instances`, active/resolved pair metadata, participant indexes, and archive retention | This spec Section 7 plus live `WorldState` / settlement helpers |
| Contribution tracking | Landed via episode-scoped `war_contribution_scores`, material contribution gates, standing helpers, and archive compaction | This spec Section 9 plus live contribution helpers |
| Common peace | Landed mechanically through settlement preview/review/ratification paths; cleanup owns current action availability and recovery behavior | This spec Sections 10-13 plus `SETTLEMENT_UI_CLEANUP_SPEC.md` |
| Ally beneficiary terms | Landed for common-peace settlement terms; War Bargains remain France-claim-scoped by design | This spec Sections 13 and 15 |
| Settlement standing | Landed through contribution share, power-tier rules, and side-leader standing presentation | This spec Sections 8-9 and presentation helpers |
| Settlement reaction pass | Landed through settlement memories, gratitude/sold-out/shut-out reactions, and cross-war reaction routing | This spec Section 14 plus live reaction helpers |
| Incoming AI settlement offers | Deferred and hidden during cleanup unless SC-5 is explicitly reversed with producer, payload, UI, voice, and behavior tests together | `SETTLEMENT_UI_CLEANUP_SPEC.md` v0.27 plus `STATUS.md` |
| Rejected/losing settlement recovery | Active cleanup gate, not this historical baseline | `SETTLEMENT_UI_CLEANUP_SPEC.md` v0.27 plus `STATUS.md` |
...
- Common-peace incentive guard: Slice C must also compare narrow coverage, full common peace, and serial bilateral/separate peace against the same war state. If serial settlements gain a large pressure advantage that the serial fallout penalty cannot plausibly offset, C1/C2 may not lock on the assumption that players will choose common peace because it exists. Follow the tuning escalation order in this spec, including the last-resort `common_peace_coverage_bonus`, or raise serial-fallout constants before lock. The headline feature must remain a rational option after decisive coalition victories, not only a roleplay choice.
- Full-Europe incentive gate: before Slice C constants lock, run a synthetic full-Europe fixture with at least 6 participants per side, at least 4 covered enemy participants available for settlement, and identical military/occupation/contribution state evaluated three ways: (1) narrow common peace against one covered enemy, (2) full common peace covering at least four enemies, and (3) serial separate/bilateral settlement against the same four enemies over the same five-turn lookback window. At least one decisive-victory fixture must show full common peace as strictly strategically attractive: total projected relation damage plus durable grievance/posture cost from the full common peace must be lower than or equal to the serial path after serial fallout, and the full package must meet the accept/near-accept target under the locked constants. If that cannot be achieved without breaking mixed-strength anti-farming fixtures, Slice C must stop and tune; it may not ship with common peace justified only as flavor or roleplay.
...
- Merge operations also preserve the surviving instance's `war_id` and older `created_sequence` so the unique `war_id` allocator remains monotonic and archived references stay stable. Current A3 rewrite targets are live `diplomatic_commitments[*]["war_id"]` / bargain context fields when present, `world.pending_dispatch_events` payloads, recent `event_log` war-entry/ledger payloads, and any dispatch or ledger route payload that stores an absorbed `war_id`. Future-safe rewrite hooks must tolerate absent Slice B/C containers (`war_contribution_scores`, `pending_settlement_dialogues`, settlement route payloads) without creating those fields early. SUPERSEDED BY `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-5 / SC-26 for cleanup queue semantics: these hooks are technical reference repair only and do not authorize incoming-offer player exposure or cross-war settlement queue/defer.
- After every merge transaction, run `assert_war_instance_invariants(world, *, context="post_merge")` before returning reader-visible state. The assertion verifies: every live `diplomatic_states[diplo_key] == "WAR"` appears in exactly one active `war_instance.active_diplo_keys` with `diplo_key_meta[diplo_key]["pair_status"] == "war"`; no active instance claims a non-`WAR` pair as `pair_status == "war"`; no absorbed `war_id` remains in live `diplomatic_commitments`, optional `war_contribution_scores`, optional `pending_settlement_dialogues`, dispatch route payloads, or ledger payloads; and `side_by_nation` matches attacker/defender membership with no nation on both sides. Normal merge paths must fail fast in tests and debug if this assertion fails; explicit load-repair/debug-import tooling may downgrade it to a structured repair report only when that tooling owns the repair. SUPERSEDED BY `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-5 / SC-26 for cleanup queue semantics: invariant coverage may detect stale technical references, but must not revive incoming-offer exposure or cross-war settlement queue/defer.
- **Merge transaction order:** (1) compute the connected component and validate all side assignments without mutating state; (2) choose the surviving oldest `war_id`; (3) merge `participant_meta`, active/resolved pair keys, side lists, and episode records into an in-memory merged shape; (4) choose leaders per section 7.4; (5) rewrite current bargain references; (6) rewrite current dispatch/event-log/ledger references; (7) run no-op-safe rewrite hooks for absent future Slice B/C containers; (8) atomically replace the survivor record and remove absorbed active instances; (9) mark `war_instances_by_leader` and `war_instances_by_participant` dirty before any reader uses the merged state. Any conflict before step 8 aborts the whole merge. The connected-component visit is bounded by active instance count, not participant count, and must not recursively rebuild indexes during the merge.
...
Forced-alliance ratification through common peace follows the same resolved-pair lifecycle as peace, with a different final diplomatic state. When a covered `WAR` or `ARMISTICE` pair resolves through a `forced_alliance` clause, the pair's diplomatic state becomes the WPS forced-alliance result (`ALLIANCE` plus forced-origin metadata), `diplo_key_meta[pair]["pair_status"] = "resolved"`, the pair moves from `active_diplo_keys` to `resolved_diplo_keys`, and `resolved_turn` is stamped in the same transaction. The newly forced-aligned nation remains available to settlement summary, reaction, bargain, and contribution snapshot readers as historical context for that `war_id`, but it is no longer an unresolved hostile/suspended pair. It appears in `war_instances_by_participant` only if it still has another active `WAR` or `ARMISTICE` pair in the same or another active `war_instance`. Battle, occupation, support, and staying-power contribution accrual for the resolved pair stops after the ratification snapshot; no post-ratification contribution accrues merely because the former enemy is now an ally. Cross-war reactions must read the snapshot and resolved-pair metadata rather than infer active-war participation from the final `ALLIANCE` state.

When a `war_instance` ends:

- Set `ended_turn = world.current_turn`.
- Set `end_reason = "all_pairs_resolved"` or `"no_active_participants"`.
- Keep the terminal record in `world.war_instances` while `current_turn - ended_turn < 10` so contribution, settlement, dispatch, and ledger readers can resolve recent references.
- Archive when `current_turn - ended_turn >= 10`: move the record to `world.archived_war_instances` and remove it from active `war_instances`.
...
- Historical proposer-side leader auto-void is **SUPERSEDED BY SETTLEMENT_UI_CLEANUP_SPEC.md SC-3 / SC-27**. If either proposer-side or accepting-side leader changed, recompute standing, direct-score gates, acceptance components, hard stops, warnings, and term beneficiary validity against the live leaders before resolving.
- Neither side's leader change automatically causes `must_reopen` while the rescored package still passes. If the fresh rescore fails, returns a hard stop, or invalidates leader-owned beneficiaries, block mutation with humanized recovery per `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-3 / SC-28.
...
Slice B1 owns the contribution store, current-episode math, material-share helpers, and a pure standing-input classifier that accepts future settlement-term booleans explicitly. B1 must default term-derived inputs such as direct beneficiary, territorial interest, active bargain relevance, survival stake, and `rival_strengthened` to `False` when no settlement package is being evaluated. Slice C/D own the code that derives those booleans from common-peace terms, War Bargain classification, and reaction routing.
...
Typed commands such as `propose common peace with Britain` may remain as parser/debug fallback, but they are not the primary player UX. The F1/Diplomacy wizard, war-score detail popup, and coalition/detail surfaces must all be able to lead the player into the same settlement flow.

**Open Settlement eligibility / grey-out rules:**

The **Open settlement** option is enabled only when all are true:

- `war_id` resolves to an active `war_instance` with `ended_turn is None`. Historical missing-`war_id` / true-`would_backfill` editor entry is **SUPERSEDED BY SETTLEMENT_UI_CLEANUP_SPEC.md SC-1 / SC-8 / SC-13 / SC-27** for normal UI; if a default-start WAR pair lacks a concrete settlement `war_id`, click-time resolution must create/return a concrete `war_id` before EDIT opens or show humanized no-action copy.
...
- No other active `settlement_confirm` dialogue is currently blocking settlement review. `incoming_settlement_offer` only participates in this blocker set after `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-5 is explicitly reversed; during cleanup defer-and-hide, normal gameplay must not expose, queue, count, notify, or block on incoming settlement offers.

Wizard availability must be read-only. It must not allocate a `war_instance`, attach pairs, open contribution episodes, invalidate caches, or stage a dialogue. Historical pure-probe true-`would_backfill` / missing-`war_id` normal-UI entry is **SUPERSEDED BY SETTLEMENT_UI_CLEANUP_SPEC.md SC-1 / SC-8 / SC-13 / SC-27**: the normal player UI must not mount EDIT or REVIEW from missing war scope. A click path may resolve/create a war instance only if it returns a concrete `war_id`, `selected_target_nation`, and `covered_enemy_participants` before the settlement editor opens; otherwise it displays humanized no-action/disambiguation copy.

Required probe contract:

```python
probe_settlement_eligibility(
    world,
    *,
    player_nation: str,
    target_nation: str,
    ...
    "would_backfill": bool,
    "error": str | None,
    "disabled_reason_display": str,
    "debug_reason": str | None,
}
```

`war_id` is preferred for war-detail / coalition-detail entry and required before EDIT or REVIEW mounts. If both `war_id` and `target_nation` are present, war-scoped settlement opening prefers `war_id` after validating that the target belongs to that active instance. The probe and the mutating open path must distinguish "no instance yet but resolvable before editor mount" from true validation failure; they must not expose a player-facing settlement editor from a missing `war_id`.
...
```python
{
    ...
    "uncovered_enemy_participants_display": [
        {"nation": "Britain", "label": "Britain", "role_display": "Still at war"},
    ],
...
Acceptance component ids must be humanized before reaching any visible surface. Required component display coverage includes `base_side_pressure`, `term_harshness_penalty`, `leader_own_losses`, `burdened_participant_penalty`, `projected_hegemony_mod`, `war_objective_alignment`, `war_exhaustion`, `abandoned_by_ally_acceptance_mod`, and `settlement_tier_legitimacy`. The label should be phrased from the accepting side's perspective where possible, e.g. "Their leader's losses" rather than `leader_own_losses`.
...
2. Header phrased as the decision question, e.g. "Will Austria accept this settlement?", with `war_label` and a **Whole war** / **Partial settlement** / **Separate front** badge.
...
4. Participants: compact "Your side" and "Their side" rows with `side_label`, top-three `participant_summary`, `standing_display`, material share, `standing_source`, and one outcome stamp: `leader`, `rewarded`, `consulted`, `ignored`, `shut_out`, `sold_out`, or `forced_aligned`. This appears before terms so the player sees who is being rewarded, sold out, forced into alignment, or ignored before parsing clause details.
...
Godot must route `settlement_confirm` to a dedicated settlement content builder before any generic proposal fallback. A malformed settlement payload may show a dev diagnostic in debug builds, but it must not silently render as "DIPLOMATIC PROPOSAL - Unknown" or as the bilateral peace preview.
...
- `confirm_settlement`: revalidates current leaders, active pair keys, hard stops, and acceptance components against live state. Historical proposer-side leader auto-void is **SUPERSEDED BY SETTLEMENT_UI_CLEANUP_SPEC.md SC-3 / SC-27**; proposer-side and accepting-side leader changes both trigger fresh rescore and neither side automatically reopens if the rescored package still passes. On success, ratifies and returns `{"success": True, "settlement_summary": {...}, "settlement_result_popup": {...}}`. On rejection, returns `{"success": False, "rejected": True, "feedback": [...]}` without mutating state.
...
Forced-alliance lifecycle rule: even though the final diplomatic state is `ALLIANCE`, the covered hostile pair is resolved for this `war_instance`. Common-peace confirmation must snapshot the pair's war-instance data and contribution linkage, apply the forced-alliance treaty state, move the pair from `active_diplo_keys` to `resolved_diplo_keys`, set `diplo_key_meta[pair]["pair_status"] = "resolved"`, stamp `resolved_turn`, and invalidate `war_instances_by_leader` / `war_instances_by_participant` before settlement or cross-war reactions read indexes. The forced-aligned nation does not keep accruing contribution in the resolved pair and does not remain an active participant solely because the final treaty state is `ALLIANCE`. If that nation still has another unresolved hostile/suspended pair in the same `war_instance`, it remains indexed through that other pair only.
...
War-exhaustion exploit guard: Slice C must include a fixture where the accepting leader has accumulated exhaustion across two or more simultaneous wars, including at least one cheap or low-stakes parallel war that is not materially represented in the settlement package. The common-peace debug output must expose both the raw per-nation exhaustion and the settlement component. If unrelated parallel wars can make the real settlement absurdly cheap, cap the settlement component by war relevance before constants lock. The first allowed cap is `war_exhaustion = min(20, relevant_war_exhaustion // 3 + unrelated_war_exhaustion // 10)`, where relevant exhaustion comes from the current `war_id` or directly covered enemies and unrelated exhaustion keeps only a muted coalition-system echo. If this cap is adopted, document it in tests, implementation notes, and `SYSTEMS_REFERENCE.md`.
...
If either side's leader changes between proposal construction and acceptance evaluation, re-evaluate the package against live leaders before resolving. Historical proposer-side leader auto-void is **SUPERSEDED BY SETTLEMENT_UI_CLEANUP_SPEC.md SC-3 / SC-27**; neither side automatically requires reopening when the rescored package still passes. If accepted, all covered active hostile pairs resolve; uncovered hostile pairs remain active under section 10.5. If rejected, the whole package fails. The player can then re-author when the cleanup spec's SC-2 / SC-28 edit-capable route exists, including concession-baseline recovery for rejected losing multi-party packages; otherwise the cleanup spec's SC-28 recovery/history/terminal-close routes apply. Rejection feedback must identify the top 1-2 objectionable terms or acceptance components by absolute penalty so the player knows whether the problem was Silesia, forced alliance, insufficient direct pressure, a bargain conflict, or accumulated harshness. **SUPERSEDED FOR PLAYER-FACING REJECTED-REVIEW ACTIONS BY `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-2 / SC-28.**
...
|-------|-----------------|-----------|----------|----------------|
| `settlement_summary` | `{war_id, covered_enemy_participants, proposer_side, accepting_side, terms_summary, participant_reactions, warnings, balance_projection, turn}` | "Settlement of {war_name}: {terms_summary[0]}; {participant_1}, {participant_2}, {participant_3}{+N more} react." | Visible to France; visible to any court that can see at least one covered participant, resulting territorial/alignment change, or participant reaction involving itself. | `event_family="settlement"`, `review_target="settlement_review"` when the war remains active, otherwise `review_target="diplomatic_ledger"`, `route_id` is the staged settlement id, e.g. `settlement:{war_id}:{turn}:{seq}` per `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-14c. |
| `settlement_digest` | `{war_id, hidden_reaction_count, top_reaction_types, turn, digest_marker}` | "{hidden_reaction_count} additional courts register the settlement aftermath." | Same as `settlement_summary`, filtered to courts that can see at least one hidden reaction. | `event_family="settlement"`, `review_target="diplomatic_ledger"`, `route_id` equals the same staged settlement id used by the parent `settlement_summary`, e.g. `settlement:{war_id}:{turn}:{seq}`; historical `settlement-digest:{war_id}:{turn}:{seq}` is SUPERSEDED BY `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-14c. |
...
Acceptance hook: expose this as `settlement_gratitude_mod = +5` when an active `settlement_gratitude` memory exists with `actor="France"` and `subject=<proposal_target>`, and the proposal is a deep treaty (`DEFENSIVE_ALLIANCE` or `ALLIANCE`), a war-entry request, or a war-bargain / ally-entry proposal where the rewarded ally is being asked to support France again. The modifier is an upside component only, added after `political_subtotal_clamped` and before `deal_balance`, matching `PEACE_DEALS_UMBRELLA_SPEC.md` §4.2; it is not part of the clamped political subtotal and cannot bypass hard posture gates. Debug output and proposal previews name the component as `settlement_gratitude_mod`.

Design intent for the small `+5` value: gratitude is a nudge that makes a recently rewarded ally easier to ask again, not a substitute for trust repair or a way to erase durable grievances. Betrayal costs are intentionally larger than fulfillment rewards because the campaign arc is about the political cost of overreach. If D1 playtests show rewarded allies feel invisible even with the immediate relation bonus and presentation beat, tune the relation reward or presentation first; raise `settlement_gratitude_mod` only after proving it does not let players farm token ally rewards to bypass the political-pressure floor.
...
| Density | Default use | Visible by default | Hidden by default |
|---------|-------------|--------------------|-------------------|
| `compact` | First attempt for 6+ participant common peace | Outcome band, top blocker/cost, top affected ally/enemy, primary action, top 3 terms/allies, hard stops, top warning | Full acceptance debug, low-salience allies, secondary warnings, per-term marginal rows |
| `medium` | Default if compact hides too much context | Top 5 terms/allies, hard stops, top 2 warnings, headline acceptance components, bargain status rows | Full debug, low-salience overflow, INFO warnings |
| `verbose` | Debug/reviewer expansion | All section data reachable with scrolling/pagination | Nothing mechanically relevant, but still grouped by section |

If the spec-naive reviewer cannot identify the top blocker or political cost, the main affected ally/enemy, and the available next action within roughly 30 seconds, C2 must first try `compact` ordering and copy reduction before changing mechanics. Slice E may refine styling, but C2 owns the fallback payload shapes and default ordering.

### 16.2a Final UI/UX closure before smoke
...
- The diplomacy wizard can preserve war context through `open_for_nation(nation, war_id: String = "")` or an equivalent `open_for_war(war_id, nation)` entry point. War-detail and coalition-detail entry must not discard `war_id`.
- Historical default-start WAR-pair probe language is **SUPERSEDED BY `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-1 / SC-8 / SC-13 / SC-27**. Cleanup-time normal UI must not mount EDIT or REVIEW from missing-`war_id` / true-`would_backfill` probe state; click-time resolution must return a concrete `war_id`, `selected_target_nation`, and `covered_enemy_participants`, or show humanized no-action/disambiguation copy.
...
- Notification/dispatch review routes must open the relevant settlement review target and focus the matching settlement row by the staged route id. Historical "exact auto-expand can be post-F polish" wording is **SUPERSEDED BY `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-14c / SC-14d / SC-27**; cleanup-time focused rows cannot be trimmed or open blank focus.
...
- AI defender packages in player-initiated wars often have only the WPS auto-`defense` objective. They must not rely on a guaranteed `+15` `war_objective_alignment` score. Package construction must tolerate `war_objective_alignment` of `0` or `+5` for defensive objectives and still evaluate whether side pressure, war exhaustion, leader-own-loss avoidance, concessions, and abandoned-ally pressure make the package acceptable.
- AI-defender viability gate: before C1 constants lock, add a losing-defender fixture where an AI side with only a `defense` objective and `war_objective_alignment <= +5` can construct at least one concrete acceptable package under the common-peace acceptance formula. The package may use occupied-region returns, limited concessions, leader-own-loss avoidance, war exhaustion, and abandoned-ally pressure; it may not use a hidden objective bonus or bypass acceptance. If no package can reach the accept threshold without breaking other fixtures, add an explicit `defender_concession_bonus` tuning knob capped at `+10`, visible in debug output and available only when the defender is offering concrete concessions or restoring occupied territory. If the knob is added, document it in tests and `SYSTEMS_REFERENCE.md`. Before C2 closes, route the same viable package shape through the `settlement_confirm` executor to prove dialogue validation preserves the C1 formula result.
...
### 16.6 Post-F ally settlement agency
...
- Non-leader allies may send settlement petitions only when they are active participants in the relevant `war_instance`, have standing/direct stake/material contribution/active bargain basis, or are directly affected by a proposed or likely term.
- Petition types should include at minimum: `request_open_settlement`, `request_consultation`, `request_reward_or_restoration`, `warn_against_sellout`, `demand_bargain_honor`, and `request_redress_after_settlement`.
- Ally petitions are advisory/current-turn diplomatic items, not hard stops. Accepting one may open or focus the settlement wizard/review with prefilled context or highlight the relevant warning row; rejecting/dismissing records memory, relation, or posture consequences only where existing settlement-memory rules justify it.
- Petitions must share the settlement anti-spam model: per-`war_id` / per-ally cooldowns, at most one active settlement-agency item from a war at a time unless the mailbox explicitly batches them, and deterministic salience ordering when multiple allies qualify.
- Non-leader allies do not create common-peace packages, do not ratify, do not bypass side-leader validation, and do not gain a universal veto. This preserves §20.3: France/the side leader owns the final settlement choice; allies create visible political pressure.
- Presentation must use named diplomat voice where available and must not surface raw standing or petition-type enums.
...
Settlement-memory per-turn cleanup removes inactive or expired memory entries where `current_turn > expires_on_turn`. Run cleanup after any settlement-memory acceptance modifiers or ledger queries for that turn have read active records, and before dispatch / presentation payload construction. Cleanup must not mutate existing `betrayal_history` grievance flags. Cleanup is deterministic from `current_turn` and stored record fields only; save/load at any point in the turn lifecycle must produce the same active/expired result after cleanup runs. Historical `pending_settlement_dialogues` expiry / retry-loop behavior is SUPERSEDED BY `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-5 / SC-26 during cleanup; deferred incoming offers cannot surface or block turns, and cross-war outgoing settlement collisions do not queue/defer.
...
- **A2 war-entry threading:** create skeleton `war_instance` before `_process_war_cascade()`; allocate `war_id` from `next_war_instance_id`; pass `war_id` through the existing recursive cascade / vassal-entry / ally-entry paths and append resolved participants as they join. Before A2 starts, introduce a small `CascadeContext` dataclass if threading `war_id` would push `_process_war_cascade(...)` past nine positional arguments or make the recursive cascade call sites hostile to safe migration. The context should carry `war_id`, `root_episode_id`, `war_entry_entries`, `ally_entry_decisions`, and any existing cascade transaction metadata that already travels together; tests should verify old call sites route through the context instead of positional-argument drift.
...
- Before B2 starts, add a regression guard for `backend/game_logic/diplomacy.py::record_battle()` ordering. The guard must fail if settlement contribution accrual moves after the existing 1000-casualty war-score early return. It can be a source-order assertion plus a behavioral sub-1000-casualty fixture, but it must prove a valid theater-participant battle with `war_id` accrues settlement contribution even when no pairwise war-score battle record is added.
...
- Active cleanup behavior for `incoming_settlement_offer`: do not register, queue, count, notify, route, dispatch, or expose player-facing incoming settlement offers unless `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-5 is explicitly reversed with producer, payload, UI, package-preserving actions, incoming voice, and behavior tests together.
- Post-cleanup Slice G appendix behavior, inactive until SC-5 reversal: `incoming_settlement_offer` may become a current-turn offer / mailbox dialogue, not a hard stop; accepting it promotes to `settlement_confirm` and does not mutate until the confirm executor succeeds; dialogue priority, mailbox label, pending-envoy route, and Godot activation tests must ship in that same owner row.
...
- **C1 done when:** backend acceptance/scoring helpers are pure, full-Europe tuning fixtures pass, territory legitimacy/direct-score gates are covered, and no endpoint or Godot route depends on unimplemented C2 payload fields. Estimated tests: ~30-34.
- **C2 done when:** preview/confirm/back-out/revise routes serialize the C1 outputs, pair-specific treaty-state resolution and leader-change handling are covered, settlement advisory payloads are Godot-renderable, and the C2 Godot smoke gate below passes. Estimated tests: ~26-30.
...
- Slice C total estimate: ~56-64 tests split across C1/C2 as above. Do not start C2 until C1's scoring/legitimacy gate is stable.
...
- Before D2 ships, profile a synthetic full-Europe settlement where one ratification directly affects at least five other active wars: for example, a forced-alliance term crossing two existing wars plus a bargain target in a third. The profile must prove directly affected wars are evaluated through `war_instances_by_participant`, no broad all-nation scan is introduced, and only secondary adjacency/sphere-only scans remain capped at three.
...
**Estimated total: ~280 tests across A1/A2/A3, B, C1/C2, D1/D2, and E.** Slice boundaries are implementation gates: each slice owns its data fields, serialization/defaults, save-format documentation, focused backend tests, synthetic full-Europe fixtures, and any UI payload tests listed above.
...
59. `settlement_confirm` fresh-rescores when either proposer-side or accepting-side leader changes; historical proposer-side auto-void is superseded by `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-3 / SC-27.
...
115. Active cleanup test: `incoming_settlement_offer` has no player-facing dialogue priority, mailbox summary label, mailbox count, pending-envoy route, notification, dispatch, popup queue entry, or Godot route unless `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-5 is explicitly reversed. Post-cleanup priority/label/route coverage belongs in the Slice G appendix row that reverses SC-5.
...
123. Active cleanup test: `incoming_settlement_offer.accept_settlement_offer`, `incoming_settlement_offer.reject_settlement_offer`, and `incoming_settlement_offer.request_revision` are absent from player-facing payloads while SC-5 remains deferred. Post-cleanup request-revision cooldown/no-counter-loop behavior belongs in the Slice G appendix row that reverses SC-5.
...
127. War-exhaustion exploit fixture proves unrelated low-stakes parallel wars cannot make a real settlement absurdly cheap; if needed, Slice C adopts the documented relevance cap and exposes raw/relevance-split debug inputs.
128. Settlement review supports compact, medium, and verbose density payloads before the spec-naive comprehension gate; C2 tries compact ordering/copy reduction before changing mechanics.
129. `settlement_gratitude_mod` is documented and tested as a small future-ask nudge, applied after the political subtotal and never as trust repair or hard-stop bypass.
130. Slice E presentation smoke covers at least two §20.8 awe set-piece targets and proves the extraordinary choice, projected cost, and aftermath are legible without debug output.
131. Forced-alliance common-peace pairs end in `ALLIANCE` while their `diplo_key_meta[pair]["pair_status"]` becomes `resolved`, the pair moves to `resolved_diplo_keys`, contribution accrual stops after snapshot, and participant indexes include the former enemy only through other unresolved pairs.
132. The full-Europe narrow/full/serial fixture uses at least 6 participants per side and at least 4 covered enemy participants, and constants cannot lock unless full common peace is strictly attractive in at least one decisive-victory case.
...
135. Wizard availability for default-start WAR pairs is read-only and does not mutate `world.war_instances`; normal UI may not mount Settlement EDIT/REVIEW from a missing `war_id` and must resolve a concrete `war_id` before editor open or show humanized no-action copy per `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-1 / SC-8 / SC-13.
136. Historical read-only probe property tests are **SUPERSEDED BY `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-1 / SC-8 / SC-13 / SC-27** for cleanup readiness. Normal UI must not mount settlement from missing war scope; click-time resolution must return concrete scope or humanized no-action/disambiguation copy. The old non-mutation probe remains historical implementation reference only.
...
140. Confirm-time stale-state failures return `error_display` / `message_display` plus `reopen_target`; no silent close or nation-picker dump passes. Incoming-offer promote-time stale-state coverage is conditional on explicit SC-5 reversal in `SETTLEMENT_UI_CLEANUP_SPEC.md`; while SC-5 remains deferred, cleanup tests prove stale/debug-injected incoming offers are dropped or rejected without surfacing, counting, blocking, or promoting.
...
142. Settlement ledger, HUD, popup, wizard, dispatch, rail, campaign-log, and settlement voice surfaces do not leak raw term, standing, warning, acceptance-band, acceptance-component, route-target, awe-tag, or disabled-reason enums.
...
146. Active cleanup final smoke uses the no-exposure branch for `incoming_settlement_offer`: no Godot route, no pending-envoy activation, no mailbox activation, no generic proposal fallback, and no enemy-offer waiting control unless SC-5 is explicitly reversed.
147. Post-cleanup Slice G AI/ally settlement agency is explicitly tracked: AI war-leader `incoming_settlement_offer` remains the common-peace offer path only after `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-5 is explicitly reversed, while non-leader allies use petitions/advisories rather than co-authoring treaties.
...
150. `proposal_confirm_popup.gd` routes `settlement_confirm` before generic proposal fallback and never renders malformed settlement payloads as "DIPLOMATIC PROPOSAL - Unknown"; while SC-5 remains deferred, injected `incoming_settlement_offer` payloads are dropped/rejected without surfacing or falling through to generic proposal UI.
151. Active cleanup pending-envoy and mailbox-activation paths prove `incoming_settlement_offer` no-exposure: no activation, no settlement-capable surface, no generic pending text, no route mutation, and no turn-blocking unless SC-5 is explicitly reversed.
152. Proposer-side and accepting-side leader changes share the same fresh-rescore rule; neither side automatically dumps the player unless live validation fails, a hard stop appears, or the package is no longer acceptable.
...
### 19.1 Post-cleanup Slice G incoming-offer appendix (inactive until SC-5 reversal)

These rows are not active cleanup requirements. They become implementation requirements only when `SETTLEMENT_UI_CLEANUP_SPEC.md` and `STATUS.md` explicitly reverse SC-5 with a producer, payload, mailbox/pending-envoy UI, package-preserving actions, incoming voice, and behavior tests in the same owner row.

- `incoming_settlement_offer` has its own dialogue priority, mailbox summary label, pending-envoy activation path, and Godot route.
- Accepting an incoming settlement offer preserves stable offer identity and exact offered terms through live re-preview before promotion to `settlement_confirm`.
- Rejecting or requesting revision applies the per-`war_id` AI settlement cooldown and does not imply an immediate counter-offer loop.
- `proposal_confirm_popup.gd` routes active `incoming_settlement_offer` payloads before generic proposal fallback and never renders malformed settlement payloads as generic unknown diplomatic proposals.
- Full Slice G AI/ally settlement agency also owns non-leader ally petitions/advisories; petitions may focus settlement UI but never ratify, veto, or bypass side-leader validation.
...
This design call does not mean allies are silent. Non-leader allies may petition, warn, request consultation, demand bargain honor, or seek redress through the post-F ally-agency layer, but those beats are pressure and memory, not a separate settlement turn or veto.
...
### 20.8 Awe set-piece targets

Imperial Settlement should not only be correct; it should create a few visible Napoleonic "I cannot believe the game let me do that" moments. Slice E presentation, campaign-log copy, and manual playtest scripts should target at least these set pieces:

1. **Triple forced alignment:** one common peace forces two or three enemies into alignment, projects `+30` or `+45` forced-alliance threat, crosses a Balance of Europe threshold, and still lets the player confirm after Talleyrand names the risk.
2. **Tilsit bargain-and-betrayal settlement:** France fulfills one War Bargain for itself while awarding a different region to an ally and knowingly shutting out a high-contribution major, producing both gratitude and grievance in one ratification.
3. **Defensive coalition preservation:** France as defender-side leader ends an enemy offensive war by restoring occupied allied territory, preserving the coalition, and making a limited concession that feels like statesmanship rather than conquest.
4. **Serial peace as a weapon:** France peels one enemy out of a larger war, the remaining enemy leader receives `abandoned_by_ally_acceptance_mod`, and the preview makes clear that the shortcut carries separate-peace political fallout.

These are presentation targets, not extra mechanics. If the mechanical result exists but the review panel, notification, dispatch, or campaign log does not make the moment legible, Slice E is incomplete.

---

## 21. Changelog

- **May 5, 2026 - v1.33 second Slice F audit synthesis.** Folded the follow-up Slice F audit into the spec and companion plan before runtime coding: incoming settlement offer accept/reject/revision handlers are now an explicit Slice F backend dependency; cross-war settlement hard-stop vs incoming-offer collision behavior is pinned; `war_status_panel.card_clicked` must emit `war_instance_id`; partial-settlement and uncovered-enemy display fields are required; side/outcome rows include `standing_source` and `forced_aligned`; awe banners get a `settlement_awe_banner_*` voice/display family; revise preserves draft terms and same-war context; route ids are pinned at staging time; notification review routing must carry route id/war id; player-defender Open Settlement, heterogeneous coalition hiding, raw-enum scans, and result-popup tests are mandatory. Historical changelog only; SUPERSEDED BY `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-5 for cleanup-time incoming-offer exposure. The companion implementation plan is v1.31.
- **May 5, 2026 - v1.32 Slice F pre-implementation audit fold.** Folded the latest Claude/Codex Slice F audit into the spec before coding: the read-only probe is explicitly not a wrapper around existing-war eligibility; wizard/detail row routing must preserve `war_instance_id`; `settlement_confirm` must delete or debug-rename legacy `actions[]`; `proposal_confirm_popup.gd` must guard against generic fallback rendering; side/outcome rows move above acceptance/terms in compact comprehension; acceptance components, review targets, and awe tags require display/voice labels; incoming settlement offers must route through pending-envoy and mailbox activation; route ids must be non-empty; and tests now cover action-schema drift, incoming-offer routing, accepting-leader rescoring, concurrent coalition disambiguation, 6+ participant humanization, voice-template raw-enum leaks, and awe banner voice routing. Historical changelog only; SUPERSEDED BY `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-5 for cleanup-time incoming-offer exposure. The companion implementation plan is v1.30.
...
- **May 2, 2026 - v1.26 A2 landed / A3 readiness cleanup.** Updated the status header after A2 review fixes landed and tightened the A3 boundary: connected-component merge owns absorbed-reference rewrites against current containers (`diplomatic_commitments`, dispatch/event-log/ledger payloads) plus no-op-safe hooks for absent future Slice B/C containers, without inventing contribution accrual or settlement-dialogue behavior early. Terminal war-instance retention is now exact: keep while `current_turn - ended_turn < 10`, archive at `>= 10`.
- **May 1, 2026 - v1.25 A2 handoff cleanup.** Updated the status header after A1 landed and clarified the next implementation boundary: A2 owns `war_id` create/reuse/attach threading across every WAR-entry seam, while full transitive connected-component merge, archive/leader/elimination behavior, and post-merge absorbed-reference rewrites remain A3.
- **May 1, 2026 - v1.24 future-audit hardening.** Closed the post-A1 audit blockers before implementation: forced-alliance common-peace pairs now resolve to `pair_status="resolved"` with `active_diplo_keys` / `resolved_diplo_keys` behavior defined; Slice C now has a hard full-Europe narrow/full/serial incentive gate; common-peace war-exhaustion farming gets an exploit fixture and optional relevance cap; AI defender common peace must prove an acceptable defense-objective package or add a capped `defender_concession_bonus`; C2 now owns compact/medium/verbose settlement-review payload fallbacks before the spec-naive comprehension gate; `settlement_gratitude_mod` intent is documented as a nudge rather than trust repair; and §20 names explicit awe set-piece targets for Slice E.
- **April 30, 2026 - v1.23 review sharpening.** Added the final follow-up hardenings before coding: pre-A2 `CascadeContext` guidance, B2 `record_battle()` ordering guard, concrete `request_revision` cooldown/no-counter-loop semantics, a stronger commitment to use the common-peace tuning escalation when narrow/full/serial fails, a spec-naive reviewer requirement for the manual comprehension gate, and D2 profiling for 5+ directly affected cross-war reactions. The companion implementation plan is v1.20.
...
- **April 29, 2026 - v1.21 mapped-foundation readiness closure.** Closed the full-Europe scale-readiness audit follow-ups: synthetic fixture helpers must extend the active roster before adding future-nation regions/controllers, A1 cache fixtures must prove 20 active `war_instance` records before A1 closes, C2 owns serialized `ai_settlement_cooldowns` and `pending_settlement_dialogues`, dialogue overflow retries are deterministic and bounded, and already-landed settlement helper/dialogue scaffolding is marked for verification rather than re-addition. Historical changelog only; SUPERSEDED BY `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-5 / SC-26 for cleanup queue semantics. The companion implementation plan is v1.18.
