...
> **Status:** v1.23 PRE-A1 REVIEW SHARPENING - A1 is foundation-only and must start with mapped-nation capital helper safety, settlement containers, old-save defaults, empty index/cache scaffolding, and cache invalidation hooks before any behavioral settlement work. Current settlement participants must exist in scenario/map data; future nations enter one by one through the `ADDING_CONTENT.md` nation checklist plus settlement readiness fixture when the map/scenario adds them. A2 owns `war_id` threading and should introduce a `CascadeContext` before expanding `_process_war_cascade(...)` into a ten-argument signature; A3 owns always-on post-merge war-instance invariant assertions; B owns contribution-before-war-score-gate hooks plus a B2 ordering guard for `record_battle()`; C owns raw harshness, explicit common-peace tuning escalation, settlement dialogue taxonomy, settlement cooldown serialization, settlement dialogue overflow retry storage, and concrete `request_revision` semantics; C2 must pass a spec-naive manual settlement-review comprehension gate before D1 starts; D owns ratification cache-ordering and D2 cross-war reaction profiling. Coding may start from `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.20.
...
- **A2 war-entry threading:** create skeleton `war_instance` before `_process_war_cascade()`; allocate `war_id` from `next_war_instance_id`; pass `war_id` through the existing recursive cascade / vassal-entry / ally-entry paths and append resolved participants as they join. Before A2 starts, introduce a small `CascadeContext` dataclass if threading `war_id` would otherwise create a ten-argument `_process_war_cascade(...)` signature. The context should carry `war_id`, `root_episode_id`, `war_entry_entries`, `ally_entry_decisions`, and any existing cascade transaction metadata that already travels together; tests should verify old call sites route through the context instead of positional-argument drift.
...
121. A2 uses a `CascadeContext` or equivalent transaction object before `_process_war_cascade(...)` grows into a ten-argument positional signature.
