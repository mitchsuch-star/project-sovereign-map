...
**Fold status: ALL FINDINGS + GT-A1..4 FOLDED into spec v0.2 on June 10, 2026 (same-day fold session; see §11). Spec v0.2 APPROVED June 10, 2026 — the one fold-changed decision (GT-R1-1 → spec D4, mixed-direction rows) was EXPLICITLY CONFIRMED by the user at approval.** Implementation is unblocked (GT-Slice-1 first).
