...
| W6-4 | ~~Muster preview + standing orders surfaced (EXP-C1 + E-CA-4)~~ **✅ LANDED July 10, 2026** — muster block (reason ladder mirroring eligibility + Grouchy Rule, `MUSTER_REASON_DISPLAY`, fog-banded target, shared-casualty note, serialized `muster_hint_shown` latch); odds band via the extracted CR-5 single source (`inferred_attack_effective_ratio`); even/unfavorable → `muster_confirm` interrupt (carries `marshal`) resolved via `/strategic_response`, favorable resolves with the block prepended; AI/strategic/counter-punch/post-objection bypass; SUPPORT creation confirms the doctrine; Grouchy-Rule §6.4 pin; `test_w6_muster_preview.py` (17); live-verified (audit's Ney-vs-Mack fires the confirm listing Soult's reason + teach line) | **combat legibility 4.5→7** | M | W6-1 |
| W6-5 | ~~The Literal Doctrine *(user addition — hone literal)*~~ **✅ LANDED July 10, 2026** — never-objects pinned (R59/R153 triggers → doctrine comment, DESIGN_REFINEMENT rows SUPERSEDED); order echo + completion quote `original_command` (voice bank `marshal_voice.py`, deterministic rotation); `literal_fidelity` event (adjacent-battle-held / quarry-moved / destination-flipped; campaign log + dispatch; cap 1/marshal/turn); doctrine on card + "(to the letter)" dispatch tell + 1-AP precision caption; no-march line moved into the bank; `test_w6_literal_doctrine.py` (20) | marshal drama | M | W6-4 (muster rows) |
| W6-6 | ~~Enemy marshals speak (EXP-M2)~~ **✅ LANDED July 10, 2026** — `backend/game_logic/enemy_voice.py` (personality × situation bank, 2+ variants each; named overrides Mack/Kutuzov/Archduke Charles/Wellington/Blücher; deterministic rotation via `battle_counts`); attaches `battle_report.enemy_voice` + campaign-log suffix, player-involving field battles only; display-only, no serialization; `test_w6_enemy_voice.py` (18) | marshal drama / aliveness | S | W6-2 |
| W6-7 | ~~Marshal Fates — capture, ransom, last stand (EXP-M1)~~ **✅ LANDED July 10, 2026 (both commits)** — commit 1: fate trigger (strength <5k / desperation-only / encircled-outright) at the `_apply_forced_retreat_or_break` seam, escape 60/40 seeded roll, last-stand interrupt (carries `marshal`) + AI home-ground rule, serialized `captured_by`/`captured_turn`, prisoner exclusions everywhere, manpower half-return, ES-7 freeze, `marshal_captured`/`last_stand` events (headline 95). Commit 2: `prisoner_return` clause (treaty demand naming the marshal; `marshal` key carried through the treaty-clause build; applied in `_ratify_treaty`; AI valuation 500g/800g-major in the acceptance demand walk; display+template rows) + `release_captured_marshal`/`release_mutual_prisoners` (capital, 5,000, morale 50) + the **`set_diplomatic_state` chokepoint auto-return** on any WAR/ARMISTICE→PEACE (covers bilateral treaties, settlements, armistice expiry) + `marshal_released` event. **Decide-in-session record:** ransom rides treaty demands + the peace auto-return — no new typed phrasing landed, so no corpus row needed; recorded cuts stand (no escape mechanic pass 1; AI accepts but never initiates). `test_w6_marshal_fates.py` (25) | **marshal drama 6→7.5** | L (2 commits) | W6-0, W6-1 |
| W6-8 | ~~The Spoils of War — estate confiscation (EXP-E1)~~ **✅ LANDED July 10, 2026 (second session)** — second capture choice on enemy-estate soil (same `capture_choice` pipeline, `stage:"estate"`, `dialogue_id` minted from the W6-0 counter via `mint_dialogue_id`; typed tokens `confiscate`/`respect` in the pending-question router; stale-id refused): CONFISCATE = 2× effective-income windfall (popup-promised number = charged number) + strip from `dotation_regions` (existing ES-7 erosion does the rest) + −10 relations + capturer's cautious marshals −1 trust + region endowable ("Endow Ney with the Duchy of Swabia" named test); RESPECT = serialized `world.respected_estates` `{region, marshal, nation, respecter}` — prune-skip + satisfaction keeps counting + `respected_estate_mod` +5 acceptance (cap 1/nation, settlement-gratitude template in `calculate_acceptance`); AI rule (GR5, both capture seams): confiscate at war with holder, respect otherwise, player-victim HIGH notification; events `estate_confiscated`/`estate_respected` through the campaign-log checklist; stage-aware Godot dialog + response chaining; `test_w6_estate_confiscation.py` (26) | economy/drama | S–M | — (rides ES-7 as landed) |
| W6-9 | ~~"What does Europe intend?" — assessment verb (EXP-D1 + R132 + R117 + coalition posture)~~ **✅ LANDED July 10, 2026 (second session)** — mock-parser assess/situation/state-of-Europe branch (never the nation-picker; 3 corpus rows + `diplomatic_advisory` added to the coverage gate) → `ADVISORY_KEYWORDS` `assess_situation` arm (named target downgrades to nation assessment); `_assess_situation` war room: per-war trajectory prose from `build_active_wars`, coalition POSTURE surfaced (stored `strategic_posture` when active, `get_coalition_posture` else), top-3 `threat_sources_this_turn` humanized, vassal loyalty + drift trend + W6-3 `reason` cause, ONE recommendation by the §11 priority table ending in an executable option — `expand_options` for openings, NEW `execute_suggestion` dialogue arm for request_terms/invest_vassal (same executors + self-charged costs as typed; "do it"/"proceed"/"yes" typed keywords); R117 LANDED, R132 80/20 LANDED; `test_w6_assessment_verb.py` (25) | diplomacy/aliveness | M | — |
| W6-10 | ~~Incoming diplomacy: voice + variety + territorial legibility (E-CA-6 + E-CA-5)~~ **✅ LANDED July 10, 2026 (second session)** — (1) `diplomat_line` on every incoming proposal (deterministic register bank in `diplomatic_templates.compose_incoming_diplomat_line`: named overrides Castlereagh/Hardenberg/Metternich/Einsiedel + hawk/schemer/dove defaults + chancery fallback for loyalist/unknown courts, pinned; every name via `resolve_named_diplomat`; motive = decision_reason voiced in-character — the Godot popup renders the line and SUPPRESSES the raw "Court rationale" tag; typed surface weaves it into the arrival text; 2-variant rotation on turn+nation, GR6); (2) anti-monotony: `TYPE_REJECTION_COOLDOWN` 5→**6** + NEW `apply_lapse_type_cooldown` (6) at the turn_manager lapse seam keyed on the STABLE P-rule label (`_extract_lapse_info` fixed to prefer `context.proposal_type` — the documented trap; the counter-fail reject at the same trap fixed too) + the P3 hegemony ask diversified by relation band via `_hegemony_ask_candidates` (>30 ladder / 0..30 non_aggression-led / <0 `friendly_gift` = gift-sweetened legal low-tier ask; all candidates legality-checked, cooldown-skipped → consecutive approaches differ); (3) E-CA-5: settlement `terms_summary` appends the derived "Status quo: X retains …" line (`_derive_status_quo_lines`, controller vs `nation_starting_regions` across opposing participants, cached `get_nation_regions`, fog-free); `test_w6_incoming_voice.py` (19) | **incoming 4→6.5** | M | — |
| W6-11 | ~~Balance duo: morale symmetry + war-priced recruitment (E-CA-1 + E-CA-3)~~ **✅ LANDED July 10, 2026 (second session — deliberately AFTER the §0 re-score measurement)** — (1) E-CA-1: casualty-scaled morale loss on BOTH sides in every outcome, winner delta = bonus − the same curve the loser pays in that arm, in BOTH combat copies (normal + `_build_deferred_result`); `DEFENDER_MORALE_CURVE_FACTOR = 1.0` (band floor 0.75); acceptance = the audit battle-2 replay (the 50k "holds the line" defender's delta moves +5 → −5, ≥8 below pre-change; attacker byte-identical) + the Mack three-battle replay ≤85 (was 95); counter-punch/forced-retreat/battles_won untouched; (2) E-CA-3: `_calculate_recruit_cost(nation=...)` ×3 at war (band 2–4) composed with ×(1+overage) above the force limit, Europe-scoped (N1 — legacy boots at war, zero pins moved), caller passes `acting_nation`, the AI's two `_pick_admin_action` pre-checks now price through the SAME helper (GR5); two-sided 1805 at-war solvency check (E1-band style) + Spent-line visibility; `test_w6_balance_duo.py` (14) | fun of fighting | S–M | W6-4 (measure legibility first) |
...
- **Design.** When a capture resolves in a region present in any **enemy** marshal's `dotation_regions`, the existing plunder/secure capture choice (`capture_executor`) gains context + two more options:
  - **Confiscate the estate** — windfall = **2× the region's effective income** (blessed; band 1.5–3×) — **⚠ AMENDED July 31, 2026 (IGR-X4, user delegated grant): the base is `income_value × (1 − war_damage)`, single-sourced in `dotation.confiscation_windfall`.** The effective-income read ran AFTER stage 1 left stability ≤ 25 (modifier 0.0), so the blessed number had priced **exactly 0 gold on every province in the game** since landing; the 2× multiplier and its band stand, only the structurally-zero base was re-keyed (the IGR-E plunder precedent), and the war-damage term keeps plunder-then-confiscate worth ~35% less than secure-then-confiscate — the original order-honesty promise, now true; the enemy marshal loses the region from `dotation_regions` (his satisfaction drops → the AI nation's own ES-7 erosion machinery does the rest — no new erosion code); **−10 relations** with his nation (blessed; band −5..−15); each of the player's **cautious** marshals loses 1 trust (property is sacred — one-time, capped); the region becomes endowable by the player (normal ES-7 path).
...
- **Blessed-numbers ledger (for tuning sessions):** Great-battle 80k (60–100k) · fate floor 5k (3–8k) · escape 60%/capture 40% (±15) · last stand +25% (15–35) · ransom 500g/800g · confiscation 2× base income × (1 − war damage) (1.5–3×; base re-keyed by IGR-X4 July 31, 2026 — effective income was structurally 0), −10 relations (−5..−15), respect +5 acceptance · proposal type-cooldown 6 turns (4–10) · war recruit ×3 (2–4) · defender morale curve 100% (≥75%).
...
> **✅ WAVE DONE — July 10, 2026 (two build sessions).** All 12 slices landed W6-0 → W6-11 in order; every BUG-CA row FIXED; DESIGN_REFINEMENT rows (EXP-N1..D1, E-CA-1..6, R117, R132-80/20, R59/R153-superseded) all point here with dates. **The §0 re-score addendum (memo §9, measured LIVE post-W6-10, pre-W6-11 by design): narration 3.5 → 7.5 ✓ · combat legibility 4.5 → 7 ✓ · incoming diplomacy 4 → 7 ✓ · marshal drama 6 → 7.5 ✓ — both required pillars MET, measured not assumed.** The addendum's one flagged spot-recheck (E-CA-1 defender-morale asymmetry still visible at measurement time) is CLOSED by W6-11's symmetry landing. Queue advanced: **▶ NEXT = the Marshal Content Pass DESIGN GATE** (`docs/MARSHAL_CONTENT_PASS_SPEC.md`, user-gated — the capstone and the §9 drama score both name roster content as the binding constraint).
