...
| W6-7 | Marshal Fates — capture, ransom, last stand (EXP-M1) — **commit 1 (capture core) ✅ LANDED July 10, 2026**: fate trigger (strength <5k / desperation-only / encircled-outright) at the `_apply_forced_retreat_or_break` seam, escape 60/40 seeded roll, last-stand interrupt (carries `marshal`) + AI home-ground rule, serialized `captured_by`/`captured_turn`, prisoner exclusions (rosters/dispatch/muster/attrition-sweep/AI scans), manpower half-return, ES-7 expectation freeze, `marshal_captured`/`last_stand` through the full event checklist (headline 95 pre-wired); `test_w6_marshal_fates.py` (19). **Commit 2 (ransom & release) — REMAINING.** | **marshal drama 6→7.5** | L (2 commits) | W6-0, W6-1 |
