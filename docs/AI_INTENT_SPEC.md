...
`europe.json`'s `sea_links` contains the Channel crossing — **London ↔ Normandy** (`[116, 47]`; it was London ↔ Flanders `[116, 109]` until NV-8c cut that edge on August 2, 2026) — the registry folds sea
...
| 13 | §3.4 | Britain's core "sits behind the moat"; "a land threat cannot touch the core" | `europe.json` `sea_links` contains the Channel crossing (London ↔ **Normandy** since NV-8c; London ↔ Flanders before it), sea links fold into `adjacent`, and the far shore boots **French**. DEF-6 kept the crossing walkable deliberately, gated by the 12k depot; since NV-4's host rule it is expedition-only while a hostile fleet covers it. §0 of this spec records Britain taking the beach by land | §3.4 Britain row + paragraph, derived rule; §5 pin 10 |
