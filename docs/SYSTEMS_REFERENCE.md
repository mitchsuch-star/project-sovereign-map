...
- **AI-4a steps 1-4 (§4.4a):** `world.threat_by_target` + `threat_level` as a property over the player's slot (the `gold` idiom); `add_threat`/`reduce_threat` optional ACTOR target; source entries carry `target`. Byte-identical against the pre-migration baseline (the PYTHONHASHSEED-pinned 40-turn subprocess harness, `test_ai_intent_threat_migration.py`); steps 5-6 (producer migration + decay re-key) are Stage D — every non-player slot is structurally 0 until then.
