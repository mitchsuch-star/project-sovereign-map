...
- Personality skew: 14 cautious / 4 aggressive / 3 literal; `balanced`/`loyal` remain unimplemented placeholders (ADDING_CONTENT.md:94-95; ARCH plan finding #29).
...
| Slice | Scope | Gate |
|-------|-------|------|
| **MC-0 (bug fix, independent)** | Fix/test-pin the `marshal_overview` name-keyed ability display ("None" shown as an active ability); optionally restore Ney/Davout's existing wired abilities via scenario JSON `ability` dicts (small, mechanics already live). | None |
...
| **MC-4** | Personality coverage decision: implement `balanced`/`loyal` (trigger tables, V2 evaluators, AI behavior) or re-author the skewed roster within the existing three types. ROADMAP Phase 10's "evaluate before 1805" note — the condition arrived. | User decision |
