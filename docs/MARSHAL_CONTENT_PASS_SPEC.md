...
- Personality skew: **13 cautious / 4 aggressive / 4 literal** (was 14/4/3 until Soult was reassigned `cautious → literal` at the July 5, 2026 CR-5 gate — see MC-4 + `COMMAND_ROBUSTNESS_SPEC.md` §6.1); `balanced`/`loyal` remain unimplemented placeholders (ADDING_CONTENT.md:94-95; ARCH plan finding #29).
...
| **MC-4** | Personality coverage decision: implement `balanced`/`loyal` (trigger tables, V2 evaluators, AI behavior) or re-author the skewed roster within the existing three types. ROADMAP Phase 10's "evaluate before 1805" note — the condition arrived. **One assignment already pulled forward:** Soult was reassigned `cautious → literal` at the July 5, 2026 CR-5 gate (to give the player a commandable literal marshal; `europe_1805.json`, pinned by `test_cr5_literal_arm_player_reachable`) — MC-4 inherits current distribution 13 cautious / 4 aggressive / 4 literal and should not re-litigate Soult without cause. Note: literal's objection-trigger set is thin (MC-1/MC-2 own deepening it). | User decision |
