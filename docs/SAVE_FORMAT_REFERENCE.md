...
```json
{
  ...
  "game_over": false,
  "victory": null,
  "campaign_end": {"verdict_turn": 44, "fall_grace_turns": 5,
                   "captivity_grace_turns": 10, "title_turns": 12},
  "fall_clock": {"chains": {"turns": 3, "since": 31, "captor": "Britain", "captured_turn": 31, "paused": false}},
  "endings": [],
  "campaign_totals": {"battles_fought": 0, "battles_won": 0, "opening": {"provinces": 28}},
  "province_title": {"Franconia": {"kind": "conquest", "since": 12, "from": "Bavaria", "holder": "France", "house": "France"}},
...
| `campaign_end` | dict | {} | **GE-1 (`GAME_END_SPEC.md` R7, landed September 25, 2026):** the scenario's authored `campaign_end` block, copied through `from_dict` (the scenario key and the save key are one). Its PRESENCE arms the endings — the derived flag `WorldState.endings_armed` / `game_end.endings_armed` (never `sandbox_mode`). `{}` = unarmed: the bare flag world, the tutorial, the legacy map. Keys read by this build: `verdict_turn`, `fall_grace_turns`, `captivity_grace_turns`, `title_turns`. **A pre-GE-1 save carries none**; `save_manager.load_game` backfills the authored block for a known scenario (`_BACKFILL_SCENARIOS` — read from the scenario file, never a copy of the numbers). |
| `fall_clock` | dict | {} | **GE-1 R1:** the PLAYER's fall clocks, keyed by arm — `soil_or_sword` `{turns, since, realm, sword, paused}` and `chains` `{turns, since, captor, captured_turn, paused}`. ONE writer, `fall.tick_fall_clocks`, once per end turn after `advance_turn`. An arm whose predicate lifts is dropped (its clock resets); the chains arm PAUSES (`paused: true`) while France is not at war with the captor and resets on release or a fresh capture (an entry whose `captor` or `captured_turn` differs from the live captivity starts over — review round; a pre-review entry without `captured_turn` adopts the live one); the soil arm PAUSES while France's only quarrels stand in a truce (review round) and resets at a peace with everyone. Verification round: the `chains` entry also counts `paused_turns` — the ticks a truce held the clock — so the cause line names "N of them at war with his captor" only when the pauses account for the rest of his captivity (a clock that merely STARTED late, on a pre-GE-1 save loaded with the Emperor already a prisoner, never paused). Sparse and never nation-keyed (only the player's Fall ends the game — GR5 lives in the predicate, `fall.get_fall_state(world, nation)`). |
| `endings` | list | [] | **GE-1 R7:** every ending stamped, in order — `{kind, cause, register, title, cause_line, turn, calendar_label, terminal, nation, detail, summary}` (+ `final_save`, the Final save's path — stamped BEFORE the write, so the Final file names itself, cleared on a failed write; + `closed`, set once by `game_end.close_campaign` when a terminal record's summary is rebuilt on the finished field — review round). `detail` gains `held` (the provinces still held, the realm arm) and `captured_turn` / `held_turns` (the chains arm — how long he was held, which a truce makes longer than the clock). ONE writer, `game_end.record_ending`; each cause is stamped once. Causes: `soil_or_sword` / `chains` / `eagle_falls` (terminal — they set `game_over`), `humbled_peace` and `verdict` (marked — never `game_over`). `summary` is `build_campaign_summary` AT THE MOMENT (the Verdict's inputs are never stored beyond the stamped result, R2), carrying the `epilogue` (the exile story) for a Fall or a Humbled Peace. The spec's `ending` is PLURAL here because §7.10 requires a Humbled Peace and a Verdict to stand together. |
| `campaign_totals` | dict | {} | **GE-1 R4:** the display-only accumulator (GR6 — no mechanic reads it), written at the moment each thing happens because nothing the engine keeps is durable (the event log rolls at 500, the war ledgers pop at peace): `battles_fought/won/lost/drawn`, `men_lost/inflicted`, `battles_in_person`, `greatest_victory`/`worst_defeat` `{name, turn, region, enemy, inflicted, suffered, side}`, `provinces_taken/lost`, `lost_to`/`taken_from` `{nation: n}`, `provinces_ceded`/`provinces_gained_by_treaty`, `enemy_marshals_taken`/`own_marshals_taken`, `marshals_fallen`/`enemy_corps_destroyed`, `coalitions_faced` + `coalition_names` (last 12), `peaces_signed` + `treaties` (last 12 `{turn, with, source, war_ending, ceded, gained, vassal_of}`), and `opening` `{turn, provinces, satellites}` seeded at boot by `from_scenario`. Review round: `lost_regions` (the DISTINCT provinces lost, for the epilogue's count) and `record_since_turn` — set only when a pre-GE-1 save is armed at load (`game_end.backfill_record`, which also seeds `opening` there), carried on the summary so the end screen says where the record begins. Verification round: `lost_region_to` `{region: court}` — the court that took each lost province LAST, so the epilogue ranks the courts in the same unit as the count; and `lost_regions_partial` — set when a save written before the distinct list existed (975f1f13) had already lost provinces, so the epilogue counts the LOSSES, the unit that record kept. Player-scoped. |
| `province_title` | dict | {} | **GE-1 / ENDGAME_PLAN §2.2 (the Congress's substrate):** `{region: {kind, since, from, holder, house[, carve]}}` — `kind` `conquest` (written at `capture_region` and the auto-charge bypass; titled after `title_turns` quiet turns) or `treaty` (written at the three ratify seams and an ultimatum yield; titled at once). A province returning to its own homeland court is POPPED (homeland is title by construction). A record whose holder no longer holds the province is dropped by the per-turn pass `game_end.reconcile_province_titles`, which also restarts a conquest's quiet clock when a hostile army stands on it or its former owner is at war with its holder. Review round: a record whose province passes to another court INSIDE the holder's bloc is re-homed (`holder` rewritten, kind and clock kept); every entry into WAR turns a `treaty` record between the two sides into a `conquest` record from that turn (`game_end.break_signed_titles`); a pre-GE-1 save armed at load gets a `conquest` record, `since` = the loaded turn, for every province held off its holder's homeland. Region-keyed, so DEF-14's rename walk migrates it. Written for every nation (GR5). Verification round (supersedes the review round's `carve_broken`, retired): `house` is the court the title BELONGS to — the SIGNATORY of a treaty, the conquering bloc's leader for a conquest (a legacy record is stamped by the per-turn pass). A hand-off follows it (the record is re-homed to any holder of the same house — a grant, and the lord's reclaim from a rebel), and a signature is broken only by the two courts that signed it (`from` and `house`): a renewed war between them, or a treaty repudiated without war (`diplomacy.break_treaty`, the paradox choice) — never a satellite's own war, and never the settlement's ARMISTICE→WAR→VASSAL bookkeeping hop. GE-3 review #12 (display only): a record a renewed war breaks carries `reopened_by` (the ceder) and `reopened_turn`, so the Congress's hold names the war that reopened a cession. A Tilsit carve writes a `treaty` record with `carve: true` for each CARVED province (`game_end.record_carve_titles`, from `formations.apply_create_client_clause`), so it reconciles, re-homes, breaks and outlives the client's release like any signed cession. **SR-1a (Score Mandate Chunk 1, Sept 26 2026):** a `treaty` record may carry `retained: true` — a status-quo retention written at the ONE state setter on a SIGNED war-ending peace (`game_end.title_status_quo_retentions`; `house` = the signatory's lord): it counts and breaks like any signature but is NEVER reconciled (`reconciled_regions` skips it). Old saves need no migration (the key absent = an ordinary treaty record). |
| `congress` | dict \| null | null | **GE-3 "The Congress of Paris" (`ENDGAME_PLAN.md` §2; `backend/game_logic/congress.py`) — the ONE new field.** `null` until a great power signs a war-ending treaty with the player or the Congress is first summoned. Keys: `status` (`sitting` / `dissolved` / `concluded` / `ended` — a Fall closed it while it sat, `ended_turn`), `number` (the Congresses summoned), `summoned_turn`, `ends_turn`, `answers` (`{court: {stance, by, reason, score, refusing_since}}` — the last tick's answers; the table itself is always re-derived live), `strip` (`[{turn, day, held, titled, stances}]` — one per end turn of the sitting), the hold's latches `lost` (titled provinces taken by force), `declared` (courts France declared on), `joined` (`{court: ally}` — an ally's offensive war France entered during the sitting; it breaks the hold as a declaration does), `rebellions` (satellites that broke free or defected) + `defections` (`{satellite: the court it defected to}`), `withdrawn` (`{court: reason}` — a recognizer that withdrew this sitting), `sweeteners` (`{court: gold}` paid at the table), `shut_out_broken` (the ports fell short at an end turn — SHUT OUT is spent for this Congress), `subsidized`/`paid_this_turn` (London's purse), `warned` + `warned_turn` (`{court: turn}` — the War of the Congress fore-warning; the war comes at least one end turn after it), `signed_at_table` (`{court: turn}` — a recognition at the table, latched for the sitting), `sued` (`{court: turn}` — the sue envoy's cadence), `petition_owed`, `signed` (`{court: {turn, broken, kind}}` — the TREATY RECOGNITION: a war-ending peace the player ratified with a great power WHILE the Congress sits (`kind: "table"`), or one in which the court ceded provinces to the player's bloc (`kind: "beaten"`), recognizes the order until a new war between them or the EMPEROR's own capture from its bloc or of its covets breaks it; written at `game_end.note_ratification` (GE-3 review #0: any other peace feeds the formula), and after a dissolution `dissolved_turn`, `dissolve_reason`, `dissolve_key`, `refusers` (the REFUSERS only — they bear the grudge), `sued_at_the_end` (courts still suing at the eighth turn, named apart), `expectation_raised`; after the Imperial Peace `concluded_turn`, `route`. Province names inside are migrated by DEF-14's rename walk. A pre-GE-3 save loads `null`. |
...
| `gold_spent_this_turn` | dict | {} | Per-nation gold spending tracker for turn summary. **DISPLAY ONLY** — its readers are `ledger._build_economy` (`Spent`), the typed `economy` report and the two end-turn banner snapshots; no pricer reads it. Reset at start of each turn. ⚠ **IQ1-2 correction:** this said "(recruit, build, repair)". It records **player purchases**, which since IQ1-2 also means fleets, buy-offs, reparations, ultimatum payments, vassal investment and vassal bribes. Three outflows are permanently excluded with reasons at their call sites (the EC-W3 materiel bill, its reckless-cavalry twin, and vassal tribute, which debits the vassal); five recurring obligations are excluded pending IQ1-3a, which gives them signed Net lines instead. The exact set is pinned as `UNRECORDED_OUTFLOWS` in `tests/test_iq1_sw0_chest_speaks.py`, with an AST census that reds on a new one. |
...
| `active_diplomatic_mission` | dict/null | null | **Session 3.** Talleyrand's ongoing mission: {type, target, turns_active, paused, paused_turns, started_turn, initial_relation}. UNDERMINE_ALLIANCE adds `target_ally` and `initial_pair_relation` (the target↔ally baseline, MS-7b). `completed: true` marks a finished mission that is kept as a RECORD (MS-1) — the surfaces read it for "Last mission", it holds no desk (`mission_is_live`). **IQ-4 (Sept 14, 2026): no new keys** — the Strategic Ledger's `cabinet` block, the notice rail row and the `diplomatic_mission_ended` log row are all derived from these fields; the rail's last beat lives in the notification row's `details`. |
...
| `threat_sources_this_turn` | list | [] | **Session 7.** Per-turn threat source log. Each entry: {source: str, amount: int, target?: str, applied?: int}. **IQ-3 review (Sept 14, 2026):** `applied` is written only when the 100 cap clipped the add (the change that actually reached the slot; `amount` stays the act's own figure) — the league spend reads it to recover the alarm that stood before a treaty's own add. **AI-4a step 3 + Stage D step 6:** every writer stamps `target` (the actor) except the player-slot legacy decay writer; readers default missing targets to the player, and player-facing panels filter to player-targeted entries. Cleared at turn start. Used by dispatch for threat breakdown. |
...
| `command_history` | list | [] | Last 50 parsed commands (sliding window). Each entry: `{raw_input, marshal, action, target, turn}` (+ `relayed: true` — CR-7-3, Sept 22 2026 — when the client sent a relayed compound tail UNCHANGED from its fill; the CR-7-8 queue re-open instrument, optional, absent otherwise). Recorded in BOTH mock and live modes (CR-4). Feeds the live LLM repetition prompt AND CR-4 context carryover ("again"/"same target"/"him"/"there"/"not you, X"). `target` added CR-4. Round-trips as a shallow dict-copy list — no per-field schema enforcement. ⚠ `WorldState._pending_relay` (CR-7-3, the tail waiting behind a question) is TRANSIENT BY CONTRACT — never in `to_dict`, cleared at the turn boundary; a serialized relay would be the retired cross-turn queue in disguise. |
| `event_log` | list | [] | Structured game event history. Each entry is a dict with `type`, `turn`, and event-specific fields. Accumulates across full game, never cleared. Used by Campaign Log, Gazette. DG-4 `call_to_arms_refused_defensive` entries may include `coalition_threat_partners_at_refusal` and `severity_factors`; these are event payload fields, not separate top-level save keys. Future Ally Participation `settlement_summary` entries store one common-peace log record with `war_id`, `terms_summary`, and structured `participant_reactions`, not one event per participant. **IQ-5 (Sept 14, 2026):** `battle` entries from a coordinated battle also carry display-only `attacker_field_before` / `defender_field_before` (int, each side's summed participant strength before the resolve — the campaign log's casualty clamp reads it first) and `attacker_participant_losses` / `defender_participant_losses` (list of `{marshal, casualties, strength_before}` — the morning dispatch's `own_mauled` names the man who bled). Stamped only with `CombatExecutor.BOTH_SIDES_NAME_THEIR_SCOPE` up; every reader falls back when they are absent, so pre-IQ-5 saves render exactly as before. **IQ-5 review round (Sept 16, 2026):** a `battle` entry whose LONE side the rubble rule or the overkill cap destroyed also carries display-only `attacker_applied_casualties` / `defender_applied_casualties` (int — the loss actually applied, which the campaign-log one-liner prefers on that side); the `*_participant_losses` casualties are the REALISED loss (share + rubble). The mechanical `attacker_casualties` / `defender_casualties` never move. **F6 (row EP, Sept 25, 2026):** `jealousy_resolved` entries carry `location` (str — the battle region for a battle-time resolution, `""` otherwise; display-only, read by `jealousy.settled_once_clause` for the reopened card's first line; absent on pre-F6 entries and read as `""`). |
...
| `last_morning_dispatch` | dict | {} | Last morning dispatch dict for dispatch re-read screen (Session A). Stored by `build_morning_dispatch()`. Contains turn, situation, marshals, intelligence, turn_events, berthier_note, diplomatic_events. All primitives, no circular refs. **Row EP F1 (Sept 23, 2026):** no new field — a campaign is now BORN with it filled (the boot briefing, `build_morning_dispatch(world, boot=True)`, built by `main._ensure_first_morning` at creation and on `/load` of a save without one), and the boot briefing alone carries a `today` dict (`orders`, `doors`, `cabinet`). `situation.enemy_regions_are_at_war` (Europe only) says the region count is the courts at war. A pre-F1 save with `{}` loads unchanged and is given the boot briefing at `/load`. |
...
| `is_coastal` | bool | Whether the region borders the sea. Default false for backward compat. **NUI-2 (September 24, 2026):** on a Europe registry world the flag follows the painted map; `save_manager.load_game` re-applies `world_state.SAVE_COAST_CORRECTIONS` (Flanders, White Russia, Volhynia → false) to a save written before the correction — TARGETED, not a blanket registry reconcile, because a mod may author `is_coastal` through `region_overrides`. |
...
**Within version 3 — load-time corrections (no version bump):** the NV-9 adjacency reconcile (`_reconcile_saved_adjacency`, in `from_dict`), the EB-2 `overseas_income` backfill, **NUI-2's coast + dockyard correction** (`reconcile_saved_registry_corrections`, in `save_manager.load_game`, September 24, 2026), and **DEF-14's province rename** (`rename_provinces_in_save_data`, in `save_manager.load_game` on the RAW world dict BEFORE `from_dict`, September 24, 2026: nine provinces renamed to where the art paints them — Bergen → Jutland, Oslo → Schleswig, Jutland → Holstein, Dalarna → Scania, Scania → Stralsund, Gothland → Norrland, Norrland → Uleaborg, Toledo → Cartagena, La Mancha → Andalusia; applied SIMULTANEOUSLY because three names were reused; every string equal to an old name, and each part of a `|`-joined crossing key, takes the new name; prose is left as written; a save is recognised as old by a name that exists only before the rename). Each is scoped to the Europe registry world and is a no-op on a save written after it. The rename runs first because the registry reconciles only run on a world whose every province the registry knows.
