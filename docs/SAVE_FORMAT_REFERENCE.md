...
| `war_intents` | dict | {} | **AI-3 Stage D (AI_INTENT_SPEC §4.3, §17, July 24, 2026).** The war council's open crises. Keys: coveter nation. Values: `{coveter, target, design_id, want_title, opened_turn, foregrounded, foregrounded_turn, coerce_recorded_turn, treaty_broken_turn, stall_turns?, buyoff_attempted?, guarantee_considered?}`. Serialized so fore-warning TENURE survives a save (a re-derived crisis would skip beat 2's clock and beat 7's owed ending). One foregrounded crisis world-wide. Pre-Stage-D saves read {}. War instances additionally carry `ai_initiated`/`design_id`/`stated_reason`/`third_party_peace_attempt_turn`/`broker_ask_turn` and design-war objectives carry `design` — all inside containers already serialized wholesale. |
