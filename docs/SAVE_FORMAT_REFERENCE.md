...
| `pending_settlement_drafts` | dict[str, list[dict]] | {} | **Settlement UI Cleanup G2-Slice-1 (landed).** Same-turn authored settlement draft clauses keyed by `war_id`. Drafts are written only after successful POST preview, round-trip through save/load, restore within the same turn, and are discarded on turn end, ratification, or explicit discard-confirm. Clauses use `SETTLEMENT_UI_CLEANUP_SPEC.md` canonical settlement clause schema; old saves default to `{}`. |
...
| `settlement_route_seq` | dict[str, dict[int, int]] | {} | **Settlement UI Cleanup G2-Slice-3 (landed).** Per-`war_id`, per-turn settlement route sequence used to produce stable `settlement:{war_id}:{turn}:{seq}` route ids. Serialized keys may round-trip through JSON strings and must be normalized back to integer turn keys on load; old saves default to `{}`. |
