...
| `pending_settlement_dialogues` | list[dict] | [] | Slice C2 retry list for `settlement_confirm` payloads deferred by dialogue queue capacity, with `war_id`, `dialogue_type`, locked payload, `created_turn`, `retry_after_turn`, and `expires_on_turn`. **Settlement UI Cleanup G2-Slice-4 v0.22 amendment:** while the SC-5 default-defer-and-hide decision is in force (`backend/game_logic/settlement_preview.INCOMING_OFFERS_DEFERRED = True`), any entry with `dialogue_type == "incoming_settlement_offer"` is treated as inactive on load: it cannot count toward mailbox totals, activate from `/mailbox/activate`, surface through `/pending_envoy`, emit notifications or dispatch lines, or stage a Godot popup. Stale incoming-offer records are dropped defensively when read or routed. The field stays in the schema for the active `settlement_confirm` retry behavior and so the post-SC-5-reversal landing slice can repopulate incoming-offer entries with payloads that match the v0.22 review schema. |
