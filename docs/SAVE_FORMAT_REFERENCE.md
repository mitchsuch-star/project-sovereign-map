...
| `pending_settlement_drafts` | dict[str, dict] | {} | **SETTLEMENT_UI_CLEANUP_SPEC G2-Slice-1 / SC-1 (foundation landed).** Per-war authored draft term packages keyed by `war_id`, value carries `war_id`, `proposer_side`, `selected_target_nation`, `covered_enemy_participants`, and the canonical `settlement_terms[]` clause schema. Written only after successful POST settlement preview. Discarded on turn end, ratification, or explicit discard-confirm. Save/load round-trips deepcopy-safely so editor mutations cannot bleed through the serialized payload. Non-dict top-level containers load as `{}`; non-dict draft entries are skipped on load. Pre-cleanup saves default to `{}`. Behavior tests for the editor / preview / ratification gating land in subsequent G2-Slice-1 sub-gates. |
| `settlement_route_seq` | dict[str, dict[int, int]] | {} | **SETTLEMENT_UI_CLEANUP_SPEC G2-Slice-1 / SC-14c (foundation landed).** Per-`(war_id, turn)` sequence counter so same-turn settlement events for one war get distinct `settlement:{war_id}:{turn}:{seq}` route ids. Outer key `war_id`; inner key `turn` (int). JSON serializes inner int keys as decimal strings; `from_dict` converts them back. Non-dict top-level containers load as `{}`; non-int turn keys, non-int seq values, and non-dict per-turn maps are dropped on load. Pre-cleanup saves default to `{}`. |
