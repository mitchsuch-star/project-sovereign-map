...
**Within version 3 — load-time corrections (no version bump):** the NV-9 adjacency reconcile (`_reconcile_saved_adjacency`, in `from_dict`), the EB-2 `overseas_income` backfill, and **NUI-2's coast + dockyard correction** (`reconcile_saved_registry_corrections`, in `save_manager.load_game`, September 24, 2026). Each is scoped to the Europe registry world and is a no-op on a save written after it.
