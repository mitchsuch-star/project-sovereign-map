...
| `threat_by_target` | dict | {player: 0} | **AI-4a steps 1-4 (AI_INTENT_SPEC §4.4a, July 24, 2026).** Per-target threat store. Keys: the ACTOR nation accruing threat (never the victim). Values: int 0-100. `threat_level` is a view over the player's slot. NO producer passes a non-player target until Stage D (steps 5-6) — every non-player slot is structurally 0 until then. Written alongside the legacy scalar for one release; the dict is authoritative when present. |
| `threat_sources_this_turn` | list | [] | **Session 7.** Per-turn threat source log. Each entry: {source: str, amount: int, target?: str}. **AI-4a step 3:** API writers stamp `target` (the actor); legacy entries (and the coalition decay writer, migrating in Stage D) omit it — readers default to the player. Cleared at turn start. Used by dispatch for threat breakdown. |
