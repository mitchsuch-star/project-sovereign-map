...
| `pending_settlement_drafts` | dict[str, list[dict]] | {} | **G2-Slice-1 (landed).** Unratified settlement draft store keyed by `war_id`. Each entry is a list of canonical clause dicts `{type, from?, to?, region?, amount?, turns?, includes_continental_system?}`. Drafts are persisted within a turn so reopening the same `war_id` after Back Out restores the draft in EDIT mode; `world.advance_turn` discards every draft. G2-Slice-3 SC-26 same-war restage merges authored draft terms into the active draft through this store. |
| `settlement_route_seq` | dict[str, dict[int, int]] | {} | **G2-Slice-3 SC-14c (landed).** Per-`(war_id, turn)` monotonic counter persisted on world state so two settlement events in one turn mint distinct `settlement:{war_id}:{turn}:{seq}` route ids. Outer key is `war_id`; inner dict maps `turn` to the last allocated `seq`. The staged dialogue mints the id; reaction event, dispatch, ledger, notification meta, and result feedback consume it verbatim. Pre-Slice-3 saves load with `{}` defaulted. |
