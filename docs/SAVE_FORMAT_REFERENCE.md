...
| `betrayal_history` | dict | {} | **Memory and Pressure.** Pair-keyed betrayal memory store tracking active strikes, grievance flags, witness scoping, and episode lineage. Keys: pair IDs. Values: structured per-pair history records. Per-pair shape: `{strikes: list[StrikeRecord], grievance_flags: list[GrievanceFlag], categories: list[str], last_turn: int}`. **B-B4 §8.8.4** adds `grievance_flags` to the canonical record shape (durable victim-grade flags; do NOT decay under §8.6 passive rules; cleared only via Make Amends grievance variant). Each grievance flag: `{grievance_type: str, episode_id: str, turn: int, source_episode_type: str}`. Pre-B-B4 saves load with `grievance_flags: []` defaulted. |
...
| `war_entry_reroll_memory` | dict | {} | **Peace Deals WB-C (landed).** Reroll-key → `{input_hash, score, band, counter_bargain}` for deterministic counter-bargain reroll. Keys are `"join_opp_{id}"` format. Cleared when input hash changes (world state mutated between evaluations). Pre-WB-C saves load with `{}` defaulted. |
| `pending_ally_entry_opportunities` | list[dict] | [] | **Peace Deals WB-C (landed).** Queue of unresolved ally-entry join opportunities surfaced by `build_declaration_preview()`. Each carries `id`, `beneficiary`, `named_enemy`, `request_type`, `promiser`, `origin_episode_id`, `band`, `score`, `counter_bargain` (if 25-49 band). Consumed by `resolve_join_opportunity()`. Pre-WB-C saves load with `[]` defaulted. |
