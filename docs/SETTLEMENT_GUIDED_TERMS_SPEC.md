...
**Status:** ✅ **v0.2 APPROVED — June 10, 2026 (user approval; D4 mixed-direction rows EXPLICITLY CONFIRMED). Implementation COMPLETE June 10, 2026 — ALL GT slices landed: GT-Slice-1 + companion CH-5 (`4c32e59`); GT-Slice-2 (`dc40012`, user-approved); GT-Slice-V + GT-Slice-3 (one session, per the §9 voice-before/with-3 ordering — incl. the re-homed REFRONT-9 expanded-row breakdown, UX-2, UX-5, and the D4-lesson HTTP-boundary dispatch fix); GT-Slice-4 (the §9 retirement inventory executed — the freeform editor is RETIRED). This doc is now the historical implementation reference and the normative contract for the guided per-court authoring surface.** v0.2 folds the Run #1 design-gate audit (`docs/SETTLEMENT_GUIDED_TERMS_AUDIT.md` — GO-with-changes; 0 CRITICAL / 9 MAJOR / 6 MINOR, all folded) plus the four mandatory pre-flight amendments **GT-A1..4** (`docs/SETTLEMENT_GATE4_PREFLIGHT_AUDIT.md` §7). D4 (mixed-direction rows — §2) was the one fold-changed decision; the user confirmed it at approval after reviewing the probe-verified sweetener rationale.
**Sequencing (historical — queue complete):** the GT-A1..4 amendments and the audit findings are folded (this version); all GT slices landed June 10, 2026. The Gate 4 manual smoke, **re-sequenced to run ONCE at the END of the settlement queue**, RAN July 2, 2026 — it smoked the guided surface, not the editor it replaced. (v0.1's "tackle after the Gate 4 smoke completes" predated the June 9 re-sequencing and is superseded.)
...
| **D4** | **Mixed-direction rows: every court row exposes BOTH option groups** — demands (court → France) *and* offers/sweeteners (France → court) — with the court's war-score direction choosing which group leads and is pre-expanded. **⚠ Changed by the audit fold — CONFIRMED by the user at approval (June 10, 2026).** Probe-verified rationale: a France-pays sweetener on a demand-direction court is legal, validates clean, and is mechanically rewarded by the live `concession_credit` component (probe: Prussia 35 → 43); the re-front's own §17 worked example performs exactly this move in Tier 3, and with Tier 3 retired no other surface can author it (the focused-Ease dial seed fires only on a court with no live clauses). A one-direction-per-row model would silently delete that agency. | Audit GT-R1-1, 2026-06-10 (recommendation applied; **user CONFIRMED at approval, June 10, 2026**) |
...
**GT-A5 (June 11, 2026, user-approved — dial territory escalation, the OQ#7 crossing recorded in the re-front spec):** the dial remains a tuning verb FIRST — but when a scoped court's gold lever is exhausted (grow at the payer-budget/hard cap, or the G4F-5 unpressed-court seed unfundable), the click escalates ONCE into the court's suggestion-engine territory clause in the dial direction (press → `Take <region>` as a demand; ease → `Offer <region>` as a France-paid concession), tagged `"authored_by": "talleyrand"` so later opposite-direction sweeps may drop it like any suggested line. Anti-balloon guards: at most one escalated territory clause per court per direction (further land is an explicit row click — the bilateral `modify_harsh` `has_territory` guard, direction-scoped); identity always from the same selectors the row suggestions show (never dial-invented); `MAX_SETTLEMENT_CLAUSE_COUNT` + promised-region dedupe honored; vassalage/subjugation/liberation-class NEVER auto-authored; every escalation voiced via an `authoring_voice_beats` `talleyrand_line` ("…I have asked {court} for {region} instead."), with the DC-4 concede-court guard still firing on escalated demands. Ease-side symmetry is deliberate (Building Blocks / Golden Rule #5 — the dial must serve Slice G1's AI offer producer unchanged). Exhausted-and-already-escalated courts fall back to the G4F-5 ceiling notes. (`tests/test_settlement_gate4_leg1_fixes.py::TestDialTerritoryEscalation`, GT-Slice-5.)
...
> **Approval granted June 10, 2026 (D4 confirmed) — slices are unblocked, in order GT-Slice-1 → 2 → V → 3 → 4.** The Gate 4 manual smoke runs ONCE at the END of the settlement queue, after these slices (June 9 re-sequencing). Voice lands **before or with GT-Slice-3** (the REFRONT-V lesson — copy never lands ahead of its resolver rule).
...
- **GT-Slice-4 — Retire the freeform editor. ✅ LANDED June 10, 2026.** Execute the full §5 inventory (removals + re-points); repurpose retained candidate/eligibility helpers; remove the dead `CommandRequest` settlement fields after the verify-dead pass; close REFRONT-7 as moot. **Editor-test disposition (enumerated):** in `tests/test_settlement_sc5r2_godot_editor.py` and adjacent files — *migrate* the draft-lifecycle/round-trip/suspend-reopen tests onto the guided surface; *delete* the picker-schema and editor-widget pins; *keep* backend validity-contract tests that don't reference the editor. **UX-3 closes as moot with the editor** (pre-flight audit §8; UX-8 also closed moot — the mount routing died with its producers). **Completion (met):** no freeform editor or identity-picker schema remains; every §5 re-point routes correctly (absence + routing tests in `tests/test_settlement_gt_slice4.py` + re-homed suites); suite green; no dead code. **Tests (landed):** absence tests; concession-authoring-routes-to-guided-flow (`test_apply_concession_baseline_replacement_stages_guided_propose_not_editor` + the re-author arm pins in `test_settlement_concession_baseline.py`); incoming-offer-counter-routes-to-guided-propose (`test_request_revision_routes_to_guided_propose_seeded_from_offer` + the continuity twin); re-author-arms-stage-guided-propose-not-editor; carry-hint-copy-points-at-row-offer; plus a verify-dead HTTP regression (the old editor POST body is stripped at the Pydantic boundary and can never stage a submitted blob). **Implementation note (same-war refresh):** retiring the merge call site replaced `merge_same_war_settlement_drafts` with the §6 single-source-of-truth rule — a same-war same-scope refresh re-shows the MOUNTED draft (caller terms only seed an empty mounted draft); the `same_war_merge_conflict` dead end is gone (helper deleted, no caller remained).
...
**Approved June 10, 2026** — the `docs/STATUS.md` Active Settlement Gate carries the per-slice tracker (`GT-Slice-1 → 2 → V → 3 → 4`), and the `CLAUDE.md` Design Gate entry reads APPROVED. Implementation proceeds GT-Slice-1 first; each slice lands with its named tests on master.

### Changelog

- **GT-Slice-V + GT-Slice-3 LANDED (2026-06-10, one session — voice with the UI that fires it, per §9's ordering):** **V** — the 11 `settlement_guided_reason_*_talleyrand` committed templates now resolve every suggestion `reason_display` (the in-character pass); the DC-4 guard line lands VERBATIM and fires as a one-shot `authoring_voice_beats` entry on both D5 triggers (demand-group `Add demand` AND the focused-Harsher dial seed on a concede-direction court — the seed reports through a new `seeded_events` collector); affected courts react through `settlement_multi_court_demand_received` / `offer_received` (named diplomat via `resolve_named_diplomat`, chancery fallback pinned on Russia); the OQ-6 `recommendation_voice` extends `settlement_budget_bound_constraint_talleyrand` in the advisory slot; the §5 incoming-offer revision copy retargeted off the editor; Voice Bible §16.1a updated; the SC-32 D5 test extended over all 17 new templates. **3** — `proposal_confirm_popup` renders the guided table in a three-region layout (header / UX-5 ScrollContainer table / footer): §3.4 treasury line, `current_demands[]` lines with per-line Remove + clamped magnitude steppers, the inline `Add demand` expansion (§3.3 lead group pre-expanded, trailing collapsed, dead-band both collapsed; OQ-1 dropdowns; Continental System sub-link; GT-Slice-V reason lines), all emitted through `send_dialogue_response_with_params` with the suggestion's exact `action_params`; **REFRONT-9 LANDED** (rows carry a display-ready `component_breakdown` from the existing score pass; clicking a court name round-trips presentation-only `settlement_focus_court` and the focused row expands the full 10-component table incl. `concession_credit`); **UX-2** (server: dial/holdout affordances attach on PROPOSE only; client: STAGED FOR REVIEW banner + promoted blockers + PROPOSE-gated affordance rail — the slice-1 holdout test re-pinned to `Return to terms` as the no-dead-end route); **UX-5** scroll container. The §9 **HTTP-boundary test caught a real D4-lesson gap**: the executor's structured dispatch keyed only on `action_params["action"]`, but the Godot wire carries the verb in `choice` — fixed with a `choice` fallback. +19 tests (12 V + 7 slice-3 incl. the re-homed `test_tier3_focused_court_expands_full_component_breakdown` and the HTTP-boundary test); full suite 10466 passed / 1 skipped; ruff clean; Godot 4.4.1 parse harness 0 failures + scene-instantiation check. **Remaining: GT-Slice-4.**

- **v0.2 APPROVED (2026-06-10)** — user approval granted; **D4 (mixed-direction rows) explicitly confirmed** after review of the probe-verified `concession_credit` sweetener rationale. Implementation unblocked; GT-Slice-1 next.
