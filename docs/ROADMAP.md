...
| **HC** *(inserted Aug 14, 2026 — runs BEFORE position 10)* | **THE HEALTH-CHECK DESIGN PROGRAM — row HC, ▶ NEXT UP.** Gate held Aug 14, 2026 under the user's delegated grant; **gate record + build contracts = `docs/audits/HEALTH_CHECK_DESIGN_GATE_2026_08_14.md` (authoritative); the ordered queue = `docs/STATUS.md` ▶ NEXT UP.** Six slices in order: **HC-1** The Silver Blockade (naval denial becomes a capped ±15 war-score component on the PT-J2 ledger substrate — the A2 strangulation arc stops reading 0) → **HC-2** The Butcher's Ledger Speaks (stateless war-room/battle-report lines voicing the per-war dead the popup already shows) → **HC-3** The Crowned Name Abroad (envoy lines name the ★ marshal; mechanical term deferred to HC-D1/Victory gate) → **HC-4** The Lifeline and the Bill (RN shore-supply arm reusing the 1.5× multiplier + AI expedition AP parity; the ONE sanctioned `BASELINE_SERIES` re-record, flip-attributed) → **HC-5** The School Names the Fleet (tutorial step XIV honesty) → **HC-6** Seasons & Weather SPEC + **USER GATE** (deliberately not ruled under the delegation — the biggest balance change since naval gets the user's own eyes). Then the played 20-turn campaign (PT row-2 acceptance + naval pillar + all visual sign-offs), then position 10. | ~4–6 sessions | The health check (Aug 14) proved these are the systems that compute honestly and stay silent — the same through-line CA9 named, one layer up. HC-1/HC-2 must land BEFORE the played campaign or its naval evidence is taken against a bar that reads 0. |
...
**Cut to post-EA, each with a named owner row** (memo §4): Events System · Gazette "Le Moniteur" · Imperial Governance (merge the duplicated `:548`/`:725` rows first) · ~~Voice-to-Text~~ *(un-cut Aug 7, 2026 by user direction — now position 8 after the second renumbering, approach-gated)* · the Advisors stat half.
