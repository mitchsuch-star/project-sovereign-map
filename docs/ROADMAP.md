...
| **4** *(new Aug 7 — was 8)* | **MUSIC & SOUND (CORE)** — audio-manager autoload (there is none), Master/Music/SFX/UI buses, volume sliders on a minimal Settings surface (position 8's main menu inherits it — the old co-location note on the build row is superseded), wire the two orphaned parchment WAVs, license-verify the three unopened zips, source + wire the core cue set (turn resolve, battle open/close, notification, UI clicks) around the existing battle-diorama audio. ⚠ the cleared CC0 packs are SFX, not orchestral — PD *scores* and PD *recordings* are separate questions. | multi-session | **User-ordered forward (Aug 7): "we have no sound."** Largest perceived-quality jump per session available; Round 0 should hear a game, not a spreadsheet; and the true trailer blocker starts falling now instead of at old-8. |
