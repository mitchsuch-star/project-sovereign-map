...
## ▶ THE ROAD TO EARLY ACCESS — the sequence (re-planned August 3, 2026; **AMENDED August 7, 2026 by user direction**)
...
>
> **⚖ AMENDED August 7, 2026** — the user, after the CA8 close-out: *"lets add some
> things before build, we have no sound[,] econ is still unbalanced, voice to text would
> be cool, consider any other items worth adding or fixing before build."* Four positions
> now precede the shippable build: **Music & Sound (Core)** pulled up from old-8, a
> materialized **Econ Balance pass** (row EC-P3 + CA8-D1/D5), a new **Voice-to-Text**
> row with an approach gate, and **Marshal Voice Tier 1** pulled up from old-9 (the
> recommendation the user's "consider other items" invited — the cheapest lever on the
> one pillar still under target). The Aug-3 memo's decision #1 ("packaging before
> content") is consciously partially overruled by the owner of the plan; its reasoning
> (build size is the largest variance) survives as the reason the build sits immediately
> after these four, not later. Positions renumbered; old numbers noted per row.
>
> **⚖ AMENDED AGAIN August 7, 2026 (second user direction, at the econ hand-off):**
> *"after econ fix we make main menu and tutorial next."* Two positions are pulled
> forward: **MAIN MENU** (carved out of the shippable-build row, where it was a
> clause) is the new **6**, and the **TUTORIAL** (old 14) is the new **7**. The old
> tutorial row's "late on purpose — a tutorial teaches *final* mechanics" reasoning
> is **consciously overruled by the owner of the plan**; its cost survives on the
> row: content authored at 7 may need touch-ups when Victory (now 12–13) changes
> what a campaign is *for*. Downstream positions renumbered; old numbers per row.
>
> **Honest total: ~25–33 build sessions plus two non-coding tracks.** The largest single
> variance is the build (now position 10) — everything cheap and loud lands before it so
> Round 0 tests a game with sound, a working economy, a front door, a teacher, and
> marshals who speak.
...
| ~~**3.75**~~ | **CREATIVE AUDIT — ✅ HELD August 4, 2026. Record = `docs/audits/CREATIVE_AUDIT_2026_08_04.md`, authoritative (§2a pillar→remediation map, §7 routing, §8 the argued recommendation).** User direction: *"play the gam,e do a creative audit"*. A **17-turn live France/1805 campaign** (`LLM_MODE=anthropic`, `SOVEREIGN_SEED=historical`, master `e450b02`) + an **80-agent find→refute fleet**. **Every re-scored pillar moved DOWN or held flat and NOTHING REGRESSED** — combat 8.0→**6.5** · marshal drama 8.5→**7.5** · economy 7.5→**6.5** · diplomacy 7.0→**6.5** · narration **6.0 flat** (still under its 6.5 target) · command 7.5→**7.0** · aliveness 7.5→**7.0**; directional ≈**6.9** vs ≈7.4 (Jul 25). Every scorer independently attributed the drop to **campaign LENGTH**: these are accumulation failures a 9-turn pass cannot load. **Position 3 is vindicated and insufficient** — all six PC rows verify live (zero duplicate holds, zero fortify/unfortify pairs, names unique and escalating), and the pillar still reads 6.0, because the slice fixed a layer below the problem. Verdict: *France won every battle it fought, lost 83,000 men to hunger, ended 44× richer than it started, and could not end the war — and the game narrated none of those four facts as connected to each other.* **28 defects → `BUG_FIXES.md` §Creative Audit; 6 gates → `DESIGN_REFINEMENT.md` §Creative Audit (CA8-D1..D6). No production code touched.** Headline P1s, all code-proven and gate-free: **CA8-1** two surfaces disagree on French casualties by up to **15×** and the mass/ally-casualty lines are attacker-only · **CA8-2** the `supply_strain` headline is wrong four ways and led 6 of 12 dispatches · **CA8-5** headline+sub-beats can be one event thrice, and `own_broken` is structurally unreachable. Design calls: **CA8-26** the dispatch has **no headline class for a French success** (17 `_add()` sites, all wounds) · **CA8-27** the peace generator cedes territory whenever `relation < -50`, i.e. every war. | play + review session | The queue had never been measured by a LONG active campaign — positions 1 and 3 both ran quiet-France. Length is the variable that exposes this game's real failure class, and it cost nothing to find. |
| ~~**3.75-open**~~ | **✅ RESOLVED by events, August 4–7, 2026.** The question ("does any CA8 row preempt position 4?") was answered by the user doing both: CA8 sweeps 1–4 (Aug 4) closed every gate-free row including §8's CA8-1 and §2a's CA8-15, and the Aug-7 directive (*"make design gate decisions and finish CA sweep"*) closed the gated remainder BEFORE the build — see 3.8. The default ("sweep at 13") was superseded by the ruling, not taken. | user ruling | Recording the question rather than answering it: position 3 was itself promoted by a re-score arbitrating a dissent, so "an audit re-orders the queue" is a precedent this project has already set once — which is exactly why it should be the user's call and not an agent's. |
| ~~**3.8**~~ | ~~**THE CA8 CLOSE-OUT GATES**~~ ✅ **HELD + BUILT August 7, 2026** (user-delegated: *"make design gate decisions and finish CA sweep and assure Ca fixes are good"*). **Gate record = `CREATIVE_AUDIT_2026_08_04.md` §10 (authoritative); landing record = `BUG_FIXES.md` §Creative Audit close-out block. The section is CLOSED: 25 of 28 fixed · 1 refuted · CA8-19 closed by ruling · CA8-25's garrison half homed with a completion definition.** D2 (war-level leverage + the strict cession gate), D6 (four success headline classes — the briefing can finally lead with a victory), the CA8-17 reduced build behind the granted Voice Bible §16.1a amendment, D4 (24 non-fear third variants), and the CA8-19 parity REJECTION (canonized, pinned). `test_ca8_gate_closeout_2026_08_07.py` (47); suite 16,381/3; a 4-lens adversarial review fleet ran post-commit (findings record in STATUS). | session | The user's call, and the right one: the three leverage/narration defects it killed were the audit's argued top of the list, and every one was invisible to the suite until played. |
| ~~**4**~~ *(new Aug 7 — was 8)* | **MUSIC & SOUND (CORE)** — **✅ SOURCING HALF LANDED August 7, 2026** (record = `docs/MUSIC_SOUND_SPEC.md` §0–§2): 75 cue-named license-verified files force-tracked under `assets/audio/{music,ui,battle,ambient}/` — the game's first 18 music tracks (Musopen PD *Eroica* I/II/IV + *Coriolan* + calms; US Navy Band Marseillaise; Old Guard fife & drum ×5; four bugle calls; Marine Band *Marche militaire française*), the quill-scribble text-streaming loop + command-send flick + wax seal + bells + naval + battle sets, the two orphaned parchment WAVs assigned, the three zips license-confirmed and curated (28 extractions). The "PD scores vs PD recordings" question the row flagged was resolved the right way: **every music file is a PD/CC0 RECORDING, adversarially verified**. ONE new CC-BY credit (musket volley — `THIRD_PARTY_LICENSES.md`). **✅ WIRING HALF LANDED the same day** (user-directed "wire all sounds"; record spec §3): `audio_manager.gd` as a class_name static singleton (autoload rejected — the parse harness can't resolve autoload globals; recorded deviation), four buses, pause-menu volume sliders (position 6's main menu inherited them Aug 8 via the shared SettingsPanel), the WHOLE §2 cue map live at ~75 call sites across 27 scripts incl. war-driven music (new war → La Marseillaise), the command-round-trip quill scribble, and the diorama's beds/stings; parse harness extended to 42 scripts EXIT=0, boot smoke 0 SCRIPT ERROR. ✅ **§3.5 AUDITION GATE PASSED August 7, 2026 (user)** — the live listen was held, no trims or swaps requested; pool rows stay unwired variants. **THE ROW IS COMPLETE.** | multi-session (✅ done) | **User-ordered forward (Aug 7): "we have no sound."** Largest perceived-quality jump per session available; Round 0 should hear a game, not a spreadsheet; and the true trailer blocker starts falling now instead of at old-8. |
| ~~**5**~~ *(new Aug 7)* | ~~**THE ECON BALANCE PASS — row EC-P3's gate + build.**~~ ✅ **GATED + BUILT August 7, 2026, one session under the user's delegated grant** (*"make it actually engaging and good… economy shouldn't go crazy unless you are doing very well and stable… abstract England's relative wealth"*). **Gate + landing record = `docs/audits/ECON_BALANCE_GATE_2026_08_07.md` (authoritative).** A 7-agent research fleet measured the disease first: treasury runaway is a PEACETIME disease (Prussia +298/turn and Spain +1,010/turn linearly forever — the only brake, the WE tax, switches off exactly when a nation does well) and condition-blind at war. **EB-1 THE CHARGES OF EMPIRE** — the WE tax generalized into ONE condition-priced treasury draw (WE + crown + war + ill + unrest + grip named terms; hoard floor 2,000 = boot-byte-neutral by construction); golden peace ~1.2% (may grow rich — the user's carve-out, literally), war ≈20k plateau, collapse ≈11k and falling. **EB-2** authored `overseas_income` (Britain 500/Spain 250/Holland 150/Portugal 150; France none — its arm is extraction) sea-power-modulated + subsidy tier 4 — the CS finally has an economic target. **EB-3** tier-scaled infra (a market on a city flips −3 → +17/turn) + **a ruin bills nothing** (IGR-X9 DECIDED — razing no longer dominates securing) + XR-4. **EB-4** threat headroom: boot 70/[65,75], region_control gates recentered 30/40/50% Europe-scoped, defensive wins no longer feed the alarm, the mirror's coalition-at-war arm — **CA8-D5 answered, new terms are measurable**. **EB-5** Requisitions of War (EWC-D1 built — Austria +37 from Mack@Swabia at boot, historically exact) + EWC-F2; the plunder dissent untouched (re-ran green at ×4). After-probe: France converges 22.1k / Austria 13.4k / Russia 14.7k; `BASELINE_SERIES` re-recorded ONCE with a three-cause flip-experiment attribution. Suite 16,427/3, zero new serialized fields. CA8-D1's answer recorded in the gate §3 EB-3.4 (ES-4 development stays the pass-2 anchor at its own gate). | gate + session (✅ done) | **User-ordered (Aug 7): "econ is still unbalanced."** The deferred-decision owner row got its date, and every downstream measurement now reads a conditional economy. |
| ~~**6**~~ *(new Aug 7, second amendment)* | ~~**THE MAIN MENU**~~ ✅ **BUILT August 8, 2026, one session** (user direction: *"make main menu … make it pretty use a painting or something and get good font and motion if it works maybe a slide of battle paintings"*). **Landing record = `docs/STATUS.md` top entry (authoritative); evidence `docs/audits/MAIN_MENU_2026_08_08.png` + `MAIN_MENU_SETTINGS_2026_08_08.png`; tests `tests/test_main_menu_and_ux_pass.py` (35).** The front door is a **Ken Burns slideshow of six license-verified PD battle paintings** (Gérard's Austerlitz opens; Commons API `LicenseShortName` checked per file; credits in `THIRD_PARTY_LICENSES.md`) under the title in Cinzel 700 + IM Fell, with crossfade + drift, entrance animation, per-painting caption, and the §2 title theme on a new AudioManager "menu" mood. Campaign column = Begin (confirm-guarded when anything would be replaced) / Continue (newest save, turn shown) / Load / Settings / Quit, all **honest-availability** against a live `GET /test` poll that names the real launch command when the war office is down. **The Settings surface is PROMOTED into a shared `settings_panel.tscn`** embedded by BOTH the menu and the pause menu (Interface Scale + battle toggle + four volume sliders + credits — the pause menu builds nothing itself now), and gains **THE PARSER (AI): the in-client Anthropic API-key field** — stored in `user://ui_settings.cfg`, pushed to the NEW backend `/config/llm` (BYOK swaps `parser.llm` via the existing `LLMClient.create` seam; empty key reverts to `.env`; key never echoed) at every campaign start. Scene hand-off = `MenuBoot` class-name statics consumed once by main.gd's connection test — New/Continue/Load reuse the EXACT pause-menu world-swap flows, zero second hydration path; the pause menu gains a **Main Menu** exit ("Return to the War Room" re-enters without a load). Shipping identity: `config/name="Ink & Iron"`, generated crossed quill-&-sabre `icon.png` (`tools/gen_app_icon.py`, original procedural art), navy boot splash (default logo off). Fixed in passing: **both /saves handlers gated on a `success` field the endpoint never sends**; menu timers are child nodes (a SceneTreeTimer would fire on the freed menu after every campaign start). Harness 48 scripts + 5 scenes EXIT=0, boot smoke 0 SCRIPT ERROR. The build row (10) now *inherits* this menu. **✅ Visual sign-off PASSED August 8, 2026 (user) — the row is CLOSED.** | session (✅ done) | **User-ordered (Aug 7, second): the front door comes right after the economy works.** Round 0's first ten minutes start here; every session until EA opens on this screen, so it pays back daily. |
| ~~**7**~~ *(new Aug 7, second amendment — was 14)* | ~~**TUTORIAL SYSTEM + R159**~~ ✅ **BUILT August 8, 2026, one session — "THE SCHOOL OF WAR" + the NV-12 naval-clarity rider** (user: *"create the tutorial and while doing it make the ui for naval more clear like naval expeditions etc"*). **Landing record = `docs/STATUS.md` top session entry (authoritative); the naval half = `NAVAL_SPEC.md` §16.** ⚠ **This row's "legacy 19-region on-ramp, zero code change" premise was FALSE on the client** — `map.gd:8` retired the 19-circle path at the July cutover and five weeks of UI postdate it; put to the user Aug 8, who ruled for an **authored scenario on the real 1805 map**: `tutorial_1805.json` "The Danube Lesson" (Ulm in miniature — France+Bavaria ALLIANCE vs Austria only, one-way front by movement law, naval/agendas dormant by authoring), launched from the menu via allowlisted `POST /new_game {"scenario": "tutorial"}`, taught by `tutorial_overlay.gd` (Berthier's non-modal tutor card, CanvasLayer 90, observe-only, suggest chips that FILL the command line; every suggest mock-parse-pinned, every beat precondition pinned as arithmetic, the whole lesson driven green over live HTTP 20/20 — which caught four defects the suite could not, incl. the AI rotating its corps off the scripted anchors → beats re-ordered + retuned). R159 = nine screens name their mechanic. The old "late on purpose" overruling + the Victory-touch-up cost stand as recorded. | multi-session (✅ one) | **User-ordered (Aug 7, second): the teacher comes with the front door.** Round 0 now measures "did you know what to do in ten minutes" against a game that actually teaches. |
| ~~**8**~~ *(was 6)* | ~~**VOICE-TO-TEXT — approach gate + v1.**~~ ✅ **GATE HELD + v1 BUILT August 8, 2026** (user-delegated: *"what works best for steam or itch release"* / *"use best decision"*). **Ruling = option (a): OS dictation (Win+H into the command line) as the supported v1** — best Steam/itch sequencing: zero build size, keeps positions 9–10 moving, and once the spoken check passes the store page may honestly claim *"speak your orders."* **(b) embedded STT (whisper.cpp GDExtension) stays DEFERRED behind the named re-open condition: Round-0 testers actually using dictation (or the store page needing a stronger claim than OS dictation supports)** — if it re-opens, it lands as a post–Round-0 slice (push-to-talk, in-game mic button, local model in the build; completion = a spoken order lands in the command line with no OS feature involved; the fill-never-auto-submit rule carries over). **(c) cloud STT REJECTED** — a second vendor key breaks the single-key BYOK Settings design. **Landed:** the SPOKEN ORDERS section on the shared `settings_panel.gd` (menu + pause both inherit; review-then-send wording — dictation FILLS the box, Enter sends, the PARSE-NEG discipline) + the README_TESTER.txt SPEAK COMMANDS block (the build row's regeneration source) + `tests/test_voice_dictation_v1.py` (incl. a GR6 structural pin: the voice section is display-only, and a GR9 pin: this row must keep naming the re-open condition). GR6 holds trivially — dictation is Windows typing into the same `LineEdit`; the deterministic parser never learns the text was spoken. **✅ the live spoken check PASSED August 8, 2026 (user: "sign off done", the tutorial sign-off pass) — position 14's *"or speak them"* store claim is UNGATED.** | gate + short session (✅ done) | **User-ordered (Aug 7): "voice to text would be cool."** Cut to post-EA on Aug 3 as a feature; re-scoped as an approach gate because v1 was nearly free — and it was: one hint section, one README block, one pin file. |
| ~~**9**~~ *(was 7, orig 9)* | ~~**MARSHAL VOICE TIER 1 + XR-5 (+ the CA8-D3 gate rides this slot)**~~ ✅ **GATED + BUILT August 8, 2026, one session under the user's delegated grant.** **CA8-D3 HELD, both YES (gate record = `JEALOUSY_SPEC.md` §0.5, authoritative):** Q1 rival memory — `find_jealousy_target` re-fixes on the man his envy already fired on while he still stands above (no history = one-rung-up byte-identical; `JEALOUSY_RIVAL_MEMORY` flip flag; GR5 single source) · Q2 the §6 confrontation latch widened to once per (pair, escalation level), legacy bare keys = level-0, bounded 4/pair/campaign; **M1–M7 + `BASELINE_SERIES` byte-identical without re-record**; `test_ca8_d3_rival_permanence.py` (19). **TIER 1 BUILT** — `marshal_voice.py` completes the trio on the enemy_voice pattern: the post-battle mirror (5 own-perspective situations × 3 registers × 3 lines + named rows Ney/Davout/Murat) riding `battle_report.marshal_voice` through the ONE shared propagation seam (player battles AND the enemy phase), rendered in `main.gd` + `enemy_phase_dialog.gd`; aggressive/cautious ack banks (4 order families) + completion banks at the two W6-5 seams — the literal's verbatim-quote doctrine untouched (banks return "" for him, pinned); ONE pin consciously flipped (`test_w6_literal_doctrine.py`: the boundary is now "only the literal QUOTES", not "only the literal speaks"). **XR-5 FIXED** (append-only bank growth; Mack's four banks ≥3; three-battle grind = three distinct lines; index-0 anchored for the serialized rotation). Plus **TUT-F3** from the sign-off pass's live report (`bool(null)`/`bool(Dictionary)` are GDScript constructor errors — null-safe `_truthy` at all five payload-bool sites in `tutorial_overlay.gd`, regex-pinned). `test_marshal_voice_tier1.py` (44). | session–multi (+ gate) (✅ done) | **The "consider any other items" pick (Aug 7), argued:** narration is the last pillar under its target (6.0 vs 6.5) and this is the cheapest lever on it. Pre-build so Round 0's testers meet marshals who speak in three voices, not one. |
| **HC** *(inserted Aug 14, 2026 — runs BEFORE position 10)* | **THE HEALTH-CHECK DESIGN PROGRAM — ✅ BUILT THROUGH HC-G August 14, 2026 (second session), + the same-day review round (gate record §9.9: 8 fixed incl. the mojibake recovery, 2 accepted).** HC-0..HC-5 + HC-G all landed (landing records §9.0–§9.8; suite 17,567/3; the ONE sanctioned `BASELINE_SERIES` re-record flip-attributed to HC-4b alone). **HC-6 SEASONS: spec authored, gate RETURNED same day — BUILD DEFERRED PAST ROUND 0** (user: "push it out"; ruling = `docs/SEASONS_WEATHER_SPEC.md` header, authoritative; landing slot = **the first post-Round-0 content slice, "The General Winter" EA-update candidate**, so Round-0 tester data on supply-pressure feel tunes Q1 before it is spent; SW-0 display-only season names = the recorded pull-forward option, user's word only). **Remaining in-row: HC-L only** (spike-first, dedicated session). *(Original routing, kept:)* Gate held Aug 14, 2026 under the user's delegated grant; **gate record + build contracts = `docs/audits/HEALTH_CHECK_DESIGN_GATE_2026_08_14.md` (authoritative); the ordered queue = `docs/STATUS.md` ▶ NEXT UP.** **Amended Aug 14 second session by user direction ("include gazette … figure out how long a turn is"): HC-0 + HC-G inserted.** Eight slices in order: **HC-0** The Calendar (**RULED: one turn = HALF A MONTH, ~15 days** — pinned by march rates/Ulm-to-Austerlitz/AGEOD precedent, gate record §2a; display-only `calendar_label`, turn 1 = "Late September 1805", legacy world byte-identical) → **HC-1** The Silver Blockade (naval denial becomes a capped ±15 war-score component on the PT-J2 ledger substrate — the A2 strangulation arc stops reading 0) → **HC-2** The Butcher's Ledger Speaks (stateless war-room/battle-report lines voicing the per-war dead the popup already shows) → **HC-3** The Crowned Name Abroad (envoy lines name the ★ marshal; mechanical term deferred to HC-D1/Victory gate) → **HC-4** The Lifeline and the Bill (RN shore-supply arm reusing the 1.5× multiplier + AI expedition AP parity; the ONE sanctioned `BASELINE_SERIES` re-record, flip-attributed) → **HC-5** The School Names the Fleet (tutorial step XIV honesty) → **HC-G** Le Moniteur (**the Gazette UN-CUT** — deterministic v1, zero LLM, dated by HC-0, `gazette_issues` archive cap 20, gate record §7a; the Aug-3 cut's objections honored by re-scope) → **HC-6** Seasons & Weather SPEC + **USER GATE** (deliberately not ruled — **consumes HC-0: 6-turn seasons, winter ~turn 6**; the biggest balance change since naval gets the user's own eyes) → **HC-L** Smart Parsing — Offline (**slotted Aug 14 by user direction "after all fixes and weather"** — the memo's v2.1 local-model flagship promoted from post-EA; spike-first with a measured corpus score as the ship/no-ship gate, build only on a pass; gate record §7b; pins: parsing only, never silently replace BYOK). ~~Then the played 20-turn campaign (PT row-2 acceptance + naval pillar + all visual sign-offs)~~ **(RAN August 15, 2026 as the COMPREHENSIVE PLAYTEST — memo docs/audits/PLAYTEST_COMPREHENSIVE_2026_08_15.md: directional 6.3 to 6.7, PT row-2 arm DISCHARGED, naval scored 6.5 on played evidence, all owed visual surfaces walked with screenshots; ~~Round-0 gate = the PC15 P1 trio in BUG_FIXES.md~~ **✅ the PC15 fix slice LANDED August 15, 2026 — 13 rows incl. the whole P1 trio+1; Round-0's gate is CLEARED** — THEN position 10)**, then position 10. | ~6–8 sessions | The health check (Aug 14) proved these are the systems that compute honestly and stay silent — the same through-line CA9 named, one layer up. HC-1/HC-2 must land BEFORE the played campaign or its naval evidence is taken against a bar that reads 0. |
| **NP** *(inserted Aug 15, 2026 by user direction — runs AFTER the PC15 fix slice, BEFORE position 10)* | **NAPOLEON — "The Emperor Takes the Field."** The player embodied as a marshal — *"the biggest item yet"* (user). **Spec + gate record = `docs/NAPOLEON_SPEC.md` v0.2, §14.1 authoritative — ✅ GATE RETURNED August 15, 2026** (Q2 capture-only + the Guard-buys-escape refinement · Q4 Shadow ×0.5 + the Petition for Independent Command · Q5 Seat +1 DP · Q8 ruled as the user's middle path: no Crowned Heads gate, field monarchs only, capture-worth the headline, owned slice NP-6 post-NP-V strikeable · Q1/Q3/Q6/Q7/Q9 at defaults unobjected). Design spine: `"sovereign"` as the 4th implemented personality (MC-4 guard consciously re-opened; `is_sovereign` derived — **zero new serialized fields**); boots at Paris, 10,000 Guard carved from Soult (economy pins byte-identical); the Presence aura (registered combat transient, own factor outside the coordination caps) + enemy-AI fear (`_evaluate_target_ratio` term) + discipline dampening; glory Shadow over co-located marshals; capture-only Peril on the existing prisoner machinery (encirclement the only road, captivity = Brétigny peace leverage, "The Captive Eagle" ±15 sparse war-score component); Seat +1 DP; emperor map-piece arm from the house generator. Slices NP-0 substrate → NP-1 hand → NP-2 presence → NP-3 court → NP-4 peril → NP-5 stage/seat → NP-V measured pass (`BASELINE_SERIES` re-record ONCE, flip-attributed) → NP-6 The Three Emperors. **Sequencing pin: ✅ SATISFIED August 15, 2026 — the PC15-1 silent-death fix (+ the prisoner-pop sibling) LANDED in the PC15 fix slice** (`WorldState.destroy_marshal` is the one removal seam NP-4 builds on; `world.fallen_marshals` tombstones serialized). The owed played 20-turn campaign runs AFTER this row (Q9), so it evaluates the game with its Emperor. | 2–3 sessions (+NP-6) | **User-ordered (Aug 15): Napoleon before the build.** The game asserts "You are Napoleon Bonaparte" at boot and never embodies it; the roster already models everything an embodied Emperor needs (skills, relationships, glory, capture/ransom, pieces, portraits) — the one man the court orbits is the one man not in `world.marshals`. Presence-as-scarce-resource is the design: the aura, the Shadow, the Peril and the Seat are all consequences of "one man, one place." |
| **EP** *(inserted Sept 23, 2026 evening by user direction — "create a plan to fix all of these, make all decisions … a challenge to force a new status quo … hold the win state for X turns with a mechanic that makes it fun … start with fixes, end with end state"; **▶ NEXT — the routing authority is `docs/ENDGAME_PLAN.md`**)* | **EP — THE ENDGAME PROGRAM: the live-review fixes, then "The Congress of Paris".** Plan + rulings = **`docs/ENDGAME_PLAN.md`** (RULED under the delegated grant; build-ready).<br>• **F1 → F6** (~3.25): the first-ten-minutes fixes (the empty first turn, the swallowed battle report, nine fog lines, the 98-regions count, the 28-name purpose) · the display-name pass (NPC-12's first slice) · the client layout pass (the clipped petition, the settlement overlap, the wizard chips, the recap modal retired) · the reward curve's longer fuse · Bavaria's walk-ins capped + the legitimacy sentence named · the vindication contradiction.<br>• **GE-1 → GE-2 → GE-3 → GE-V** (~4): row GE's backend and screens, then **"THE CONGRESS OF PARIS"** — at **50 titled provinces** (homeland / ceded by treaty / 12 quiet turns; ONE serialized `province_title`) the Emperor summons the Congress; every great power answers on a public table with its reason and PRICE, re-answered every turn; the Congress **sits 8 turns** while refusers are driven to war (+15 intent/turn), Britain funds them, the coalition gate drops to 40, the marshals and satellites present their bills, and every titled province, Paris and the Emperor's freedom must be held; Britain is shut out by a 60% Continental System; full recognition on the eighth turn = **THE IMPERIAL PEACE** (gold, mark-and-continue with Retire); a failed Congress dissolves (alarm +15, 10-turn cooldown) and can be summoned again; the Universal Monarchy when no great power remains. Defeat = R1's two clocks + the marked "Humbled Peace". One `campaign_end.tscn`, four registers.<br>• **Then the release build (10), then Updates 1–5.** | ~7.25 sessions | The live review (`docs/audits/PLAYTEST_LIVE_REVIEW_2026_09_23.md`) measured the three ways the game fails to end — the turn-4 paying peace (0 attacks in 56 turns), raw occupation (alarm 97, bleeding back), total elimination (alarm 99 with nobody left). The Congress keys victory to SETTLED conquest and makes the hold a crisis the player manages, not a timer they wait out. |
| **GE** *(inserted Sept 23, 2026 by user direction — "separate out game end and defeat state as session after these", then "thought we wanted end state first?"; **⚑ evening: ABSORBED into row EP as its last four slices GE-1..GE-3 + GE-V — build from `docs/ENDGAME_PLAN.md`**)* | **GE — GAME END & DEFEAT STATE, "The Verdict and the Fall."** Spec + gate record = **`docs/GAME_END_SPEC.md`** (RULED Sept 23 under the delegated grant; build-ready).<br>• **Defeat — "The Fall of the Empire", two warned clocks** (Paris alone never triggers them, PL-31):<br>&nbsp;&nbsp;– ≤1 province, or no free corps and no affordable commission, for 5 turns;<br>&nbsp;&nbsp;– the Emperor captive and unfreed for 10 turns.<br>• **Game end — "The Verdict of History" at a scenario-authored turn 44** (Early July 1807, Tilsit). It is marked and then the campaign continues (EC-6 kept).<br>• A `campaign_end.tscn` end screen, the saves stamped, and WO-D10's exile commissioning.<br>• The rules arm only where a scenario authors them, so the bare flag world and the tutorial never do.<br>• Slices GE-1 (backend, ~1) and GE-2 (client, ~1). The release build carries the result.<br>• **⚑ Sept 23 (evening), after the live review (`docs/audits/PLAYTEST_LIVE_REVIEW_2026_09_23.md`): spec §7 PROPOSES the victory arm the user asked for — "THE IMPERIAL PEACE", hold 51 provinces by TITLE (homeland / ceded by treaty / 12 quiet turns) for 8 consecutive quiet turns, the Universal Monarchy when no great power remains, six global-elimination rulings, R1 confirmed plus a marked "Humbled Peace". AWAITING THE USER'S RULING on §7.8 Q1–Q6; GE-1 opens on it. If ruled, add GE-3 (~1) — row GE ≈ 3 sessions.** | ~2 sessions | Carved from positions 12–13: it takes the loss condition and a game end. VP-1's objectives and the rest of VP-2 stay there. It precedes the release by the user's direction: a playable game has an ending. |
| **10** *(was 8, orig 4)* | ✅ **LANDED September 25, 2026 (one commit; rules `SYSTEMS_REFERENCE.md` §69; pins `tests/test_release_build_2026_09_25.py` (86); rows PB-1 … PB-6 FIXED, PB-7 stays Update 1's). `deploy/build.bat` is the whole build: ONE stamp (`tools/build_stamp.py`) → PyInstaller from the regenerated spec → `tools/release_smoke.py` boots the frozen exe from a foreign folder and PLAYS a turn (stops the build otherwise) → a RELEASE export → `tools/list_pck.py` reads the pack's directory for the maps and scenes → one versioned zip. Measured on the dev PC (the pre-commit preflight of this same tree): the frozen server answers `/test` in under 1 second and plays, with Python stripped from PATH too; the exported client boots against it with 0 SCRIPT ERROR; the pack 881 files / 253 MB, the export 17 s; the zip 293 MB in 38 s; the live-key smoke put one hard phrasing through the live parser (`parse_mode` `anthropic`). The first outside launch is the no-Python confirmation; the user uploads `deploy/dist/ink_iron_<stamp>.zip` to itch.io. ⚠ The parked part 1 was NOT suite-green as parked (17 failures: a help pin convention, three source pins on `execute_command`, the GR6 provenance census) — fixed and folded into the one commit rather than committed as its own step.** **THE SHIPPABLE BUILD — "it runs on someone else's computer"** — **⚑ AMENDED September 23, 2026 — THE RELEASE BUILD, right after row GE** (the user: *"lets just release a playable game"*, then *"thought we wanted end state first"*; review record = `BUG_FIXES.md` §Pre-Build Review; plan = `STATUS.md` ▶ NEXT UP):<br>• **Part 1 is written and SAVED** — the local `git stash` "release-build part 1 …" and the committed patch `deploy/parked/release_build_part1.patch`. Apply it first (`git apply --3way`), then finish the rest listed in `STATUS.md` ▶ NEXT UP.<br>• **First items: PB-1 … PB-6**, headlined by PB-1: the frozen server cannot find `europe_1805.json`, because PyInstaller's entry-script `__file__` is `_internal\main.py`. This was measured, and it would stop a frozen boot. `build.bat` must boot the frozen exe and hit `/test` before zipping.<br>• **Call C1, revised by the user: the key section SHIPS**, working and honest. The four LLM touchpoints below land IN this row, plus a live-key smoke on the user's own key before release. PB-3's false privacy sentence goes.<br>• **The release:** itch.io. The user uploads and chooses public or restricted; the first players are Round 0 (row 11).<br>• **Clean-machine run:** a fresh local Windows account on the dev PC comes first; the first outside player's launch is the no-Python confirmation.<br>— frozen-bundle data path; regenerate the spec (it predates the July-18 SDK migration); **a Godot client export of the CURRENT game** (see the correction opposite — an export pipeline already exists and has run); client supervises the backend (child process, negotiated port, honest boot screen); ~~saves under `%APPDATA%` *(+ wire the Continue/Load arms of position 6's menu to it)*~~ ✅; ~~a real main menu + in-client API-key field + shipping name/icon~~ **INHERITED from position 6 (Aug 7 second amendment)**; run it on a machine with no Python. **Done = a stranger unzips it and plays in mock mode without editing a text file.** *(Aug 7: the old "+½ session audio shell" clause is SUPERSEDED — position 4 lands the autoload/buses/sliders and this row inherits them; the main menu + `%APPDATA%` integration of that Settings surface stays here.)* ~~**+~1 hour (added Aug 3):** re-gate the cheat surface on explicit debug — `meta_executor.py:2037-2043` computes `live_client_armed = key_source != "none"`, so a keyless build leaves cheats OPEN; making mock the shipped default is exactly what arms it. **Do NOT patch `deploy/launch.bat`** — the build row regenerates it.~~ **✅ THE PRE-BUILD FIX PASS LANDED August 15, 2026** (user: *"finish fixes for prebuild"*; record = `docs/STATUS.md` top entry; pins = `tests/test_prebuild_fixes_2026_08_14.py`, 28): cheat surface re-gated on explicit debug (`game_state["debug_mode"]` / `DEBUG_MODE=true` env — key_source retired from the gate); saves resolve to `%APPDATA%\InkAndIron\saves` when frozen (menu Continue/Load follow via `/saves` for free); **launch.bat regenerated** mock-default with a 30s `GET /test` health poll + honest failure + stale-server reuse guard (the Aug-3 "do not patch" note is DISCHARGED — this was the regeneration, pulled forward); README_TESTER rewritten for the 1805 game; config.txt template reframed OPTIONAL; build.bat demands a fresh export + keyless mock smoke; the 265MB `movies.avi` moved out of res://. **This row's remainder: the fresh export itself + verify JSONs in the .pck, the v1 LLM touchpoints below, client-side supervision beyond the launcher's, and the clean-machine run.** **+ THE LLM-ACCESS v1 TOUCHPOINTS (added Aug 14 — this row owns the memo's §5 v1 list, `docs/audits/LLM_MONETIZATION_RESEARCH_2026_08_14.md`):** rename every player-facing surface off "API key" → **"Smarter Parsing (optional) — Connect your Anthropic account"** with the three-fears copy (cost "<$2 a campaign, billed by Anthropic to you" / security "stored on this PC, sent only to Anthropic" / setup link); live status line (Connected / Not connected / Key rejected); the once-ever non-modal Berthier hint on first keyless campaign start (latched, reactive-but-discoverable discipline); the quiet once-per-session "clerks could not reach the wire" notice on live-parse failure (mock fallback already structural). **NOTHING IS SOLD — no purchase surface exists in v1 by ruling** (token packs NEVER — the AI2U retreat; a bounded dev-paid courtesy tier is a v2 QUESTION gated on EA telemetry, owned at position 14 with the store-page disclosure). | multi-session | ⚠ **CORRECTED August 4, 2026 — this row previously read "Nobody has ever produced a build of this game," and that is FALSE.** The claim was written into the Aug-3 re-plan as a bolded gloss on a true supporting sentence, and was repeated downstream before anyone checked it. **A complete build exists at `deploy/dist/ink_iron_server/`, dated March 10, 2026** — verified, not inferred: `InkAndIron.exe` (95 MB) and `ink_iron_server.exe` (10.5 MB) both carry `MZ` PE headers, `InkAndIron.pck` carries the `GDPC` Godot-pack magic, and it ships with `_internal/` (31 MB), `launch.bat`, `config.txt`, a `saves/` dir and a 16 KB `README_TESTER.txt`, zipped to a 52 MB `ink_iron_server.zip`. `export_presets.cfg` exists and its `export_path` points at exactly that exe. **So the pipeline exists and has run end-to-end.** What is TRUE and still makes this row real: (1) it is **untracked** — `.gitignore:20` ignores `dist/`, so only the *scripts* are in the repo and nothing demonstrates the build still reproduces; (2) it **predates the July-2 map cutover by four months**, and the `.pck` is **337 KB**, far too small to hold the current fonts, portraits and map bitmaps — **strong evidence, not proof**, that it carries the pre-cutover world (the server's Python is compiled into a PyInstaller archive, so grepping the binary settles nothing either way, and this has NOT been verified); (3) `ink_iron.spec` predates the July-18 Anthropic SDK migration. **Why nothing caught it:** `deploy/dist/` is gitignored, the pre-commit hook runs ruff over `backend/` and pytest over `tests/`, and the Godot harness parses `.gd` scripts — every check this project runs reads tracked source, and the artifact sits outside all of them. The row is therefore **smaller than billed**: it starts from a working pipeline, not from nothing. Whether that is one session or two weeks *is the schedule*. Measure it while nothing is promised. Also collapses the BYOK/monetization row. |
| **UPD** *(Sept 23, 2026 — the update queue after the release; player reports re-order it)* | **THE FIX UPDATES** — ⚑ **ABSORBED September 26, 2026 by row SR below (`docs/SCORE_MANDATE_PLAN.md` §6: Update 1 → SR-1d, Update 2 → SR-3a + SR-2c, Update 3 → SR-3a, Update 4 → SR-4c, Update 5 → SR-2c; nothing dropped)** — by the user's direction (*"lets just release a playable game"*): the defects the pre-build review found ship known and land here.<br>• **Update 1** — PR-D1b "The League Treats When Spent" (+ PB-7 doc housekeeping).<br>• **Update 2** — "A question never orders" (CRT-3 + WO-32).<br>• **Update 3** — "The word is the order" (L-D + CX5-L5-F2 + CQ-30).<br>• **Update 4** — petition Antechamber B1.<br>• **Update 5** — PR-D1c + PR-D1d.<br>Order and rulings = `STATUS.md` ▶ NEXT UP. | ~6 sessions | A defect a player actually hits jumps the queue. Row GE and the release build come first. |
| **SR** *(inserted Sept 26, 2026 by user direction — "make the mandate raising the score in all sections, especially the weakest, and order the chunks by category, leaving bandwidth for quick wins; then technology, asymmetry / national flavor, AP and DP gaps — needs design")* | **THE SCORE MANDATE — `docs/SCORE_MANDATE_PLAN.md` IS THE ROUTING AUTHORITY.** Every pillar rises and none falls, weakest first; nine chunks by category (1 THE ENDING — the road to the Congress, which no prior plan contained: status quo is a cession, the client's war is the lord's war, the gate line teaches the road, PR-D1b, the re-measure → 2 DIPLOMACY → 3 FIRST CONTACT & COMMAND → 4 COMBAT LEGIBILITY & MARSHAL DRAMA → 5 ECONOMY & NAVAL → 6 NARRATION → 7 AI ALIVENESS & LIVING BALANCE → 8 VASSALS & AGENDAS → 9 UI/UX → 3b the rest of CRT), each with 30% reserved for the quick-win bank; then three design gates (SR-D1 technology as reforms, SR-D2 asymmetry / national flavor, SR-D3 the AP and DP gaps). **Absorbs row UPD's Updates 1–5 by category (plan §6); the spine from 11 onward is unchanged.** **⚑ Re-ruled September 26, 2026 (evening) (plan header, §4, §5):** SR-D3 RULED — (c) first as SR-2e "standing orders reliable", no free AP scaling, new points only through SR-D1's Staff reform on both boards; L-D approved; SR-D1 at Chunk 5's gate, SR-D2 (+ HC-6) at Chunk 7's; ONE exit per session and ONE full re-score at the end of the mandate. | ≈22.5 sessions + 3 gates | The two played reviews of September 23 and 25 scored the ending at 5.5 and diplomacy at 6.0 with the road to the Congress not on the board; the mandate is to raise every score, weakest first, and to put that road in the plan by name. |
| **11** *(was 9, orig 5)* | **Playtest Round 0** — 3–5 testers on the exported build, **three** questions: *could you launch it*, *did you know what to do in ten minutes*, and — added Aug 3 — **keep your typed transcript**: roughly what fraction of your orders were misread, and paste the worst five. *(Aug 7: the build they get now carries sound, a balanced economy, a main menu, a tutorial, dictation if 8(a) verified, and marshals in three voices — the answers measure the game, not its absences.)* Zero build cost. Do **not** set a pass bar; PARSE-NEG shows it is already breached. | non-coding | Every measurement in this project's history has been the developer or an agent. Cheapest possible falsification of positions 12–17. Expect the unsigned-executable SmartScreen flag here — no test in the suite can see it. |
| **12** *(was 10, orig 6)* | **⚑ Sept 23, 2026:**<br>• **Carved out:** the loss condition and a date-based game end now belong to row **GE** (`docs/GAME_END_SPEC.md`).<br>• **New gate item:** this gate also owns **PB-D1 "the long peace"** (`DESIGN_REFINEMENT.md`). Even with PR-D1b, 29 quiet turns follow the opening war on the commanded-accept arm; France's designs are the first answer.<br>— **Victory & Objectives — GATE, then VP-1 "The Emperor's Designs"** (France's own agenda deck on the NA substrate). Into the gate: objective-shaped **not** a region race; 2–3 paths; branch irreversible; **mark the triumph and continue** (protects EC-6); defeat rides existing W6-7 attrition. Carry the churn number: **42 test files, 115 `game_over` assertions.** *(Aug 7: CA8-D2's settlement half is already landed at 3.8 — the war-score→terms→ending chain this gate inherits now starts from war-level leverage.)* | gate + multi-session | **The game cannot end** — `turn_manager.py:1099` returns `game_over: False` on every Europe world. Unblocked one day ago: `NATION_AGENDAS_SPEC` §9's own blocker was *"design the pass after DEF-5 lands"*, and DEF-5 landed Aug 2. Unblocks All-Dead Loss, Napoleon Comparison and 8.5 Campaign Objectives — and honest store copy. |
| **13** *(was 11, orig 7)* | **VP-2 "The Ending"** — **⚑ Sept 23, 2026: row GE (`docs/GAME_END_SPEC.md`) takes this row's loss condition and a date-based game end** ("The Verdict of History", marked and continued).<br>• **What remains here:** the objective-shaped triumph, the decisive-peace victory, the Napoleon Comparison (which may anchor on GE's Tilsit date), HC-D1, NPC-D2, NP-D7 and the Emperor's death.<br>• **Reuse:** VP triumphs reuse GE's `record_ending` seam.<br>• **Churn count corrected:** it is 20 test files / 97 lines, not 42 / 115 (that figure counted `__pycache__` and fixtures).<br>— un-gate `_check_victory_conditions` for **design satisfaction only**, triumph screen, keep-playing arm, Napoleon Comparison, the chosen loss condition. **Surface the replay frame:** Stage A's seed + authored variance bands already make every Europe open differently and the game has never told anyone. | multi-session | The arc closes. Split from VP-1 so each lands green against those 115 assertions. Three replay drivers, two already paid for. |
| **14** *(was 12, orig 10)* | **STEAM PAGE GOES LIVE.** **Written constraint (added Aug 3):** the copy describes the build that exists at **14**, not the one CR-6 delivers at 15. Headline verb is *"command your marshals by typing orders"* — which is true; *"or speak them"* is ALSO true (the 8(a) live spoken check passed Aug 8, 2026). *"Argue with your generals"* is **not** true at 12 (CR-5b flavour is live-only; `delegation.py` mode-gates personality bias). The LLM is a **named optional mode**, disclosed. **AI provisioning/monetization plan of record = `docs/audits/LLM_MONETIZATION_RESEARCH_2026_08_14.md`** (Aug 14, 2026, 5-agent research workflow over shipped Steam AI games + Valve policy + cost model + sentiment): **v1 = mock-default + reframed BYOK ("Connect your Anthropic account", never "API key"), sell NOTHING AI-related in EA; token packs are NEVER built (the AI2U retreat); a parser subscription is never built (AI Dungeon's review profile); local-model parsing is the v2 flagship; a bounded dev-paid courtesy tier only if EA telemetry shows BYOK friction.** The memo carries the store-page AI-disclosure draft, the Steam Content Survey guardrail story, the three Settings-copy fear-answers, and six §6 owner questions (BYOK on day one? lead with "no AI required"? telemetry? local-model gate timing? flavor-line toggle? Anthropic-terms re-check). | non-coding | All four real prerequisites now exist: Steamworks account (2), a build that runs elsewhere (8), a sentence about what a campaign is *for* (10–11), screenshots from the shipping export (8) — now with sound under them. |
| **15** *(was 13, orig 11)* | **CR-6 *proper* — Conversational Objection Negotiation.** Gate first: may the LLM classify a free-text rebuttal into the **existing deterministic** Insist/Trust/Compromise buckets? That is the GR6 boundary case and the user's call. Mock fallback = today's three buttons. | gate + multi-session | The most VISION-central unbuilt feature in the repo. Placed after the voiced build deliberately: if the gate returns *no*, the trailer is already made of positions 4 + 7 and nothing is stranded. **Note: the CR-6 gate slot was consumed once by the S5-D1 bare-attack mini-gate — this is the other CR-6.** **And (September 23, 2026):** the typed-road defect rows that were routed to "CR-6 proper" (IQ9-X1..X3, IQ7-X7, the September-19 review verdicts) were never this feature's work. The CR-6 triage re-homed every one to slices **CRT-1 … CRT-11** (`COMMAND_ROBUSTNESS_SPEC.md` §12). This row is the feature and its gate only. |
| ~~**14-old**~~ | ~~**Tutorial System**~~ **MOVED to position 7 by the Aug-7 second amendment** (user direction: main menu + tutorial immediately after econ). The old rationale ("late on purpose: a tutorial teaches *final* mechanics") is recorded on the new row as consciously overruled. | — | Provenance row only. |
| **16** *(was 15, orig 13)* | **Pre-EA hardening + Playtest Round 1** — ~~UI-2d-1 first (the only open soft-lock class)~~ *(PB-7, Sept 26 2026: UI-2d-1 CLOSED by F3 + IQ-10 — the decision modals clamp; the soft-lock class is empty)*; *(Aug 7: the CA8 backlog clause is DISCHARGED — the section closed at 3.8; IGR-X9 + EWC-F1/F2 moved to position 5's gate)*; S5-4; sweep NAD-1..4, VP-D2/D3, S5-D3; complete Settings; **code signing** so an unsigned PyInstaller server does not trip SmartScreen; 8–12 external testers. | multi-session | The open defect list is genuinely small and all P2/P3 — this codebase is in unusually good repair. |
| **17** *(was 16, orig 14)* | **Trailer** — commissioned at 9, delivered here, onto a page that has been accumulating wishlists since 14. | non-coding | Its true prerequisites — audio (4), personality lines (9), a payoff moment, a validated loop — are all landed by here. |
| **18** *(was 17, orig 15)* | **EARLY ACCESS** | — | Ships a game that runs on a stranger's computer, ends, teaches itself, has sound and music, opens on a real front door, can take spoken orders, and whose marshals speak in three voices. |

**The three decisions this encodes** (each overrulable — memo §2):
1. **Packaging before content, at 4.** Almost everything left is content on a proven engine; exactly one item's size is genuinely unknown and it is invisible to every planning doc in the repo. **⚖ PARTIALLY OVERRULED by the user, Aug 7, 2026** — four content positions (sound, econ balance, voice-to-text, marshal voice) now precede the build, which moved to **8**; **the second Aug-7 amendment added the main menu (6) and tutorial (7) in front of it too, moving the build to 10**. The decision's *reasoning* survives: the build's unknown size is still why it sits immediately after them rather than later.
...
**Cut to post-EA, each with a named owner row** (memo §4): Events System · ~~Gazette "Le Moniteur"~~ *(**un-cut Aug 14, 2026 by user direction — now slice HC-G of row HC**, re-scoped DETERMINISTIC so the Aug-3 objection — "first `game_logic` LLM call" — no longer applies; gate record `docs/audits/HEALTH_CHECK_DESIGN_GATE_2026_08_14.md` §7a)* · Imperial Governance (merge the duplicated `:548`/`:725` rows first) · ~~Voice-to-Text~~ *(un-cut Aug 7, 2026 by user direction — now position 8 after the second renumbering, approach-gated)* · the Advisors stat half.
...
| **EC-P3** | **Economy pass 3 (deferred-decision owner + peacetime economy)** | **✅ THE ECON BALANCE PASS LANDED Aug 7, 2026 (position 5)** — gate + landing record `docs/audits/ECON_BALANCE_GATE_2026_08_07.md`: the Charges of Empire (conditional treasury fixed point), the overseas pools + subsidy tier 4, tier-scaled infra + ruins-bill-nothing (IGR-X9), threat headroom (CA8-D5), Requisitions of War (EWC-D1), EWC-F2, XR-4. The row's remaining unscheduled residue: **ES-4 province development + ES-7b confer_title stay behind EC-2 pass 2's own gate**; EWC-D2 (casualty→pool drain) stays at the Pre-EA Balance Pass; the Spain idle-hoard note + the opulence-band recut are named dials in the gate record §8 if a future review wants them. Prior research: `PEACETIME_ECONOMY_RESEARCH_2026_08_03.md` (position 3.5 landed its §6 slice Aug 4). |
...
| **Gazette System** | ~~Period newspaper every 3-5 turns, LLM-generated~~ **SUPERSEDED Aug 14, 2026 → slice HC-G of row HC** (deterministic v1, every 5 turns + forced specials, dated by the HC-0 calendar; contract = `docs/audits/HEALTH_CHECK_DESIGN_GATE_2026_08_14.md` §7a) | Medium | **▶ Queued (row HC)** |
...
> **SUPERSEDED Aug 14, 2026 — the live contract is `docs/audits/HEALTH_CHECK_DESIGN_GATE_2026_08_14.md` §7a (slice HC-G): DETERMINISTIC composition, no LLM in v1** (LLM polish is a later BYOK-gated slice owned there). The paragraph below is the original LLM-based sketch, kept as history.
