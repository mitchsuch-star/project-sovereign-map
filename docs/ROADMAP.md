...
| **8** *(was 6)* | **VOICE-TO-TEXT — approach gate + v1.** The game is typed-command-native, so dictation is the natural fit. Gate question: **(a)** OS dictation as supported v1 (Win+H into the command box — zero code beyond focus/IME-safety verification + a Settings hint + a README line; ships in hours) · **(b)** embedded STT (whisper.cpp GDExtension — real multi-session work: platform binaries, latency budget, mic permissions) · **(c)** cloud STT (needs a second vendor key — conflicts with the single-key BYOK design; effectively out). **Recommended default: (a) now as v1, with (b) named behind a re-open condition (post–Round-0, if testers actually use dictation).** GR6 stands either way — STT feeds the SAME deterministic parser; nothing here touches mechanics. | gate + short session | **User-ordered (Aug 7): "voice to text would be cool."** Cut to post-EA on Aug 3 as a feature; re-scoped here as an approach gate because v1 may be nearly free — and "speak your orders" is store-page material position 14 can use honestly if (a) verifies. |
