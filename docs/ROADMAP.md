...
| **VD** | **Vassal Depth — raise Vassals 6.5 → 7+** (the Sweep-4 recommendations, now the committed build order) | Post-Sweep-4 vassal build queue (STATUS "Vassal Depth Queue"): **(1) VS-3 Land Grants via the F1 Diplomacy Wizard ✅ BLESSED July 15, 2026** ("assure we build diplo stuff" — the *positive* grip lever, cede a province → worth-scaled loyalty, lord forfeits income/vassal tributes it; spec `VASSAL_DEEPENING_SPEC.md §1`, `test_vassal_land_grant.py`) → **(2) "The Defection" coalition-join (GR9)** — a spiralling satellite switches to the enemy coalition, not just independence (the marquee "Coalition Dynamics" beat; §2.7) → **(3) garrison lever wire-or-remove (VP-D1, P0)** → **(4) enemy-AI grip-awareness (VP-D6)**. Two Sweep-4 friction fixes already LANDED July 15 (the "grant X more autonomy" parse shrug + VP-D5 tribute legibility). Runs alongside the CO program's Sweep 5 → exit; interleave-vs-after is the user's call at build time. | ~~VS-3 gate~~ **✅ BLESSED July 15, 2026** (build at §1 recommended defaults, in-band tunable); "The Defection" is a GR9 slice | **`VASSAL_DEEPENING_SPEC.md`** |
