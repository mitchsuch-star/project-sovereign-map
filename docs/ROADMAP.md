...
| **CR** | **Command Robustness** ("Talk to Your Marshals") — **ACTIVE (scope blessed July 3, 2026)** | Promotes the Post-Diplomacy Command Layer Queue (below) to a numbered phase. ~~CR-0~~ **✅ LANDED July 3, 2026** (the P0 parser roster gap is FIXED — parser rosters live-world derived; BUG_FIXES.md entry closed); **next: CR-1 eval harness**, then clarification dialogue, LLM modernization, context carryover, personality-biased disambiguation. | CR-6 needs its own design gate | `COMMAND_ROBUSTNESS_SPEC.md` v0.2 |
