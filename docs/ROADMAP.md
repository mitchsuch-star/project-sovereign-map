...
| **EC** | **Economy Revisit** (1805-scale economy + campaign feel) — **▶ NOW ACTIVE** | ~~EC-0 fixes the **AP-reset defect**~~ **✅ LANDED July 4, 2026** (world-scoped `base_nation_actions` snapshot — Austria holds 4 AP, Europe-only treaty AP penalties apply-then-release instead of compounding); **PRE-EC ledger floor ✅ landed July 7** (economy tab now reconciles to Net); **EC-6 victory DECIDED July 7 = sandbox** (remove hard win/lose; real win-conditions → the new Pre-Ship Victory & Objectives Pass); **EC-1/EC-3/EC-4 gate-free track ready**; gold sinks at ~3.8k/turn NET scale (Province Development anchor — EC-2, USER GATE); manpower/garrison retunes (EC-3: dead artillery-regen bug + cavalry runaway + garrison cap); enemy AP (EC-4); Continental System decision (EC-5); DG-3 supply/overextension (EC-7). | EC-2 design gate; EC-5/EC-7 user decisions | `ECONOMY_REVISIT_SPEC.md` |
...
**Open design decisions parked for the user:** EC-2 sink set · EC-5 Continental System upgrade-vs-trim · ~~EC-6/DG-5 victory conditions~~ (**DECIDED July 7, 2026 = sandbox**; real win-conditions → Pre-Ship Victory & Objectives Pass) · MC-1 ability set · MC-4 personality types · CR-6 gate (CR-5 scope BLESSED July 5, 2026 — spec §6) · DEF-12 map modes · Jealousy v3.1 (after MC + v3.2 addendum). *(The Slice H gate was approved + landed July 3, 2026.)*
