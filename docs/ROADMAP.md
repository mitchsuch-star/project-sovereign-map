...
| **AUD** | **Comprehensive Codebase Audit** (component-by-component, fix-as-you-find) — **▶ RUNS NEXT (re-sequenced July 9, 2026)** | Fable-led **correctness sweep** across every component (core mechanics; state/surface/hygiene; Map & rendering) — fix safe defects in place, escalate genuine judgment calls — **closing into the econ eval** (`ECONOMY_REVISIT_SPEC.md §0.7`) on a known-green build. **The §8 creative / fun-factor capstone is SPLIT OFF to run *after* the EC build** (so it judges the improved economy, not the current flat 4.5/10 one — then feeds the MC / Jealousy / 8.EVAL design gates). **The correctness sweep is sequenced BEFORE the EC build** (was "after EC") so Fable's whole-codebase context carries into an independent read of the economy recs before that code is written. Fable operates with **decide-and-act authority** inside the fix-vs-escalate boundary (guideline §3); component coverage + phasing re-verified July 9 (completeness check — 3 stand-alone subsystems folded into §7.4/§7.5). | none for the audit (correctness fixes land directly; escalations + the post-EC creative memo → user / design gate); **the econ eval feeds the EC-2 gate** | `AUDIT_GUIDELINE.md` (+ `ECONOMY_REVISIT_SPEC.md §0.7`) |
| **EC** | **Economy Revisit** (1805-scale economy + campaign feel) — **▶ QUEUED BEHIND AUD + ECON EVAL** | ~~EC-0 fixes the **AP-reset defect**~~ **✅ LANDED July 4, 2026** (world-scoped `base_nation_actions` snapshot — Austria holds 4 AP, Europe-only treaty AP penalties apply-then-release instead of compounding); **PRE-EC ledger floor ✅ landed July 7** (economy tab now reconciles to Net); **EC-6 victory DECIDED July 7 = sandbox** (remove hard win/lose; real win-conditions → the new Pre-Ship Victory & Objectives Pass); **EC-2 audit + gate decisions RECORDED July 8; the gate-ready pass-1 spec (ES-7 "Cost of Success" mechanic + ordered plan + item reconciliation + escalations) is `ECONOMY_REVISIT_SPEC.md` §0.6.** Pass-1 = **ES-2 Occupation Upkeep + ES-7 (REFRAMED — success makes a marshal more expensive; *paying stops the bleed, never buys trust*; player-facing surface = endow the marshal with an ESTATE + a province-derived TITLE, e.g. "Endow Ney with the Duchy of Swabia" — internal action id stays `grant_dotation`; ES-8 title *flavor* rides pass 1 free, ES-8 stat-bonus mechanic stays deferred) + ES-1 manpower-fix prereq**; ES-4 → pass 2; ES-10 CUT; band ~55–70% of the whole net incl. the diplomatic economy. **The build runs AFTER the audit + econ eval (§0.7):** §0.6.3 gate-free Track 1 (ES-1 re-key+retune / EC-6a sandbox toggle / ledger-GR8 fix) → EC-2 pair behind the gate. Also EC-3 super-linear upkeep; EC-4 enemy AP; Continental System (EC-5); DG-3 supply/overextension (EC-7). | **Econ eval (§0.7) first** → EC-2 design gate (§0.6.2 + band E1 + blessed numbers E2–E6 only); EC-5 self-cost scope / EC-7-ES-6 supply timing / sandbox soft-goal all RESOLVED July 8 (cont.) — §0.6.6 | `ECONOMY_REVISIT_SPEC.md` |
...
| **AUD** | **Comprehensive Codebase Audit** (Fable, fix-as-you-find) | **▶ RUNS NEXT** (re-sequenced July 9, 2026 — before the EC *build*, feeding the econ eval; after CR, before MC). `AUDIT_GUIDELINE.md` |
