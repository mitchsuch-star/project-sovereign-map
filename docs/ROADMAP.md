...
| **6** *(new Aug 7, second amendment)* | **THE MAIN MENU** — carved out of the shippable-build row by user direction (*"after econ fix we make main menu and tutorial next"*). A real front door: title scene (New Game / Continue / Load / Settings / Quit), the Settings surface promoted out of the pause menu (inherits position 4's audio sliders + UI scale + text size), the in-client API-key field that retires the `config.txt` ritual (BYOK stays optional — mock is the default arm), shipping name + icon (`project.godot` still says "Project Sovereign"), and an honest boot/loading screen. The build row (10) now *inherits* this menu instead of building it. | session | **User-ordered (Aug 7, second): the front door comes right after the economy works.** Round 0's first ten minutes start here; every session until EA opens on this screen, so it pays back daily. |
...
**Cut to post-EA, each with a named owner row** (memo §4): Events System · Gazette "Le Moniteur" · Imperial Governance (merge the duplicated `:548`/`:725` rows first) · ~~Voice-to-Text~~ *(un-cut Aug 7, 2026 by user direction — now position 6, approach-gated)* · the Advisors stat half.
