...
| **IGR** | **In-Game Review Fixes & Improvements** — ✅ **REVIEW HELD + SPEC QUEUED July 25, 2026** (evidence `docs/audits/INGAME_REVIEW_2026_07_25.md`; spec `docs/INGAME_REVIEW_FIXES_SPEC.md`, **awaiting the §5 gate — 4 questions**). The live pass scored overall ≈7.4 and **fixed 5 defects in-session** (commit `bdeb17c`) — two P1s: declaring war on a treaty partner soft-locked in an infinite modal loop, and every AP-priced marshal-petition arm arrived permanently disabled. What was ROUTED becomes this row: **IGR-A** honest copy (raw `no_participation_path` key leak, the duplicated "Political Context" block, nation-name-as-region suggestions, silent vassal release) · **IGR-B** the campaign log becomes readable (Q1 — 24 of 25 turn-9 events were AI-AI `X rebuffs Y` lines, burying Russia adopting *The Gulf and the Straits*) · **IGR-C** Talleyrand's unreachable "designs held in check" rung (Q3, GR9) · **IGR-D** the carve becomes completable (Q2 — the Proclamation card has still never been sighted in-client; the historical case is Tilsit, a separate peace with Prussia alone) · **IGR-E** plunder earns its prompt (Q4, blessed number — 87g against 3,085/turn income) · **IGR-F** the minor-court envoy digest · **IGR-G** settlement viewport + map-stack legibility. Build order A → pause → B → C → G → D → F → E. | Raises the two weakest measured pillars (narration 6.0, UI/UX 6.5) and closes the review's GR9 debts. Polish pass, not a systems phase. |
