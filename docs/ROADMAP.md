...
Standing deferred rows that survive this staging with owners: DEF-4 (behind the 15× tripwire), DEF-5 naval — **✅ BUILT COMPLETE NV-0..NV-3 August 2, 2026 (`NAVAL_SPEC.md` §14 landing record; the user's "start coding the navy" directive = the §12 gate at recommended defaults incl. Q5 EA promotion; NV-V's live half = the next play session)**, ~~Jealousy v3.1 (gate after MC)~~ **✅ LANDED July 11, 2026 as v3.2 (row JV above)**, Historical Precision P1 ministers (EA), Post-EA table.
...
| 11 | Britain (naval/subsidy) & Imperial Governance | Planned — vassal core LANDED in Phase 8; **the naval half is BUILT (DEF-5 NV-0..NV-3, Aug 2, 2026 — blockade/crossing gate/CS 2.0 closure/subsidy-era Britain pressure all live)**; remaining rows are the governance promotion + whatever the NV-V live pass routes |
