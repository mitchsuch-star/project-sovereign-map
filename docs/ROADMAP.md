...
| # | Item | Size | Why here |
|---|------|------|----------|
| **0** | **⚠ PARSE-NEG — fix the negation guard. BEFORE the road.** The fast parser is confidently WRONG above the LLM escalation gate: `Ney, never attack Mack` → **attack** at 0.90, `Ney, don't attack` → **attack** at 0.90, `how do I attack?` → **attack** at 0.80, `hold until Davout arrives then attack` → **attack now**, `if Mack advances fall back to Alsace` → **move now** at 0.95. All above 0.7, so `should_use_llm` never escalates — **an API key does not fix it.** Fix: negation pre-check · interrogative pre-check · `until`/`if`/`unless`/`when` join the CR-2 clause-boundary family · demote confidence below 0.7 so the LLM is actually consulted · golden-corpus rows for every line above. **Do NOT raise the threshold** — that makes the game LLM-dependent, the opposite of the EA framing. Row: `BUG_FIXES.md` §PARSE-NEG. | session | **A correctness bug, not a scope decision, and it needs no gate.** It is the only defect found in the whole EA-scope review that can lose a player their campaign because of a word they used correctly — and it is invisible to a key, invisible to the golden corpus, and sitting in the repo's own `live_phrasing_backlog` with an owner who never closed it. | Position 1's played campaign measures a parser that is not lying to it; Round 0 (5) gets a misread-rate worth reporting; and position 11 (CR-6) inherits a clean base rather than this. |
...
| **3** | ~~**Asset backup**~~ ✅ **DONE August 3, 2026** (this commit) + campaign triage | session | `europe_visual.png` + `europe_lookup.png` were **untracked** (`.gitignore:40`) and existed on exactly one disk. Force-added. **Also back up the three CC0 audio zips** — free insurance, but ranked well below the map: they are re-downloadable in two minutes, the commissioned map is not. ⚠ **The source PSD is still not in the repo — a user action.** |
