...
| **CR** | **Command Robustness** ("Talk to Your Marshals") — **ACTIVE (scope blessed July 3, 2026)** | Promotes the Post-Diplomacy Command Layer Queue (below) to a numbered phase. ~~CR-0~~ + ~~CR-1~~ **✅ LANDED July 3, 2026** (the P0 parser roster gap is FIXED — rosters live-world derived; the eval harness — 226-entry golden corpus over both worlds + action-coverage gate — is the phase's standing regression gate; typed `status` wire connected); **next: CR-2 clarification dialogue**, then LLM modernization, context carryover, personality-biased disambiguation. | CR-6 needs its own design gate | `COMMAND_ROBUSTNESS_SPEC.md` v0.2 |
