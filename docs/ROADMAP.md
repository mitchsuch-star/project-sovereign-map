...
| **3** | **THE COMPOSITION SLICE — promoted here by position 1's re-score, per §5's own dissent clause.** ~~Asset backup~~ ✅ done Aug 3 (the slot's previous occupant; the source PSD is still a user action). **(a) What position 1 already fixed** (`1df5a36`): duplicate phase lines 30→0, fortify/wait thrash 41→0, `estate_eroding` headline share 51%→30%. **(b) What is left and is why this row exists:** PC-4 a marshal fights up to five separately-*named* battles in one phase ("Sixth Battle of Swabia" by turn 3), PC-5 the flanking line names a province the attacker's side does not hold, PC-6 the diorama observation contradicts its own contingent list, PC-8 the bad-odds interrupt prices solo strength while the muster commits neighbours, PC-9 square tutorial copy addressed to the player inside the *enemy's* report — all routed OPEN with seams in `BUG_FIXES.md` §Quiet-France. **(c) The balance half of PC-3**, with its experiment already done: the producer-side fortify latch diverges `BASELINE_SERIES` *and* collapses the AI-V §4.7 variance signature, attribution verified by flipping it off. **(d) RECOMMENDED, NOT YET APPROVED — bundle "The Levy and the Camp"** (row EC-P3): the econ memo's first slice, 1 session, ONE serialized field, **zero new enemy-phase rungs**, which is why it belongs with a slice whose whole point is that pillar. ⚠ **Gated on the user's ruling on memo §6 Q1–Q6** — if that ruling does not come, (a)–(c) ship alone and (d) waits. | session (+1 if (d) is approved) | The plan's own strongest dissent, upheld by measurement: *"the measured floor is not packaging, it is the enemy phase as theater"* — the one surface the player sees every turn, for the whole campaign, forever. |
...
| **EC-P3** | **Economy pass 3 (deferred-decision owner + peacetime economy)** | **▶ RESEARCHED Aug 3, 2026** — memo `docs/audits/PEACETIME_ECONOMY_RESEARCH_2026_08_03.md`. **Six open questions await a user ruling (§6).** First slice ("The Levy and the Camp") is **recommended for position 3**, bundled with the composition slice; the rest of the row (ES-4, the five inherited items) stays unscheduled behind EC-2 pass 2's own gate. |
