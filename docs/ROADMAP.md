...
| **CR** | **Command Robustness** ("Talk to Your Marshals") — **ACTIVE (scope blessed July 3, 2026)** | Promotes the Post-Diplomacy Command Layer Queue (below) to a numbered phase. ~~CR-0~~ + ~~CR-1~~ **✅ LANDED July 3, 2026** (the P0 parser roster gap is FIXED — rosters live-world derived; the eval harness — golden corpus over both worlds + action-coverage gate, 246 entries post-CR-2 — is the phase's standing regression gate; typed `status` wire connected); ~~CR-2~~ **✅ LANDED July 4, 2026** (marshal-aware confidence + LLM retry on fuzzy errors; the unified `command_clarification` question — "Which marshal, Sire?" — riding the existing popup with deterministic typed answers; silent-marshal-drop + self-support + condition-hijack + sequential-"then" fixes; parse failures with candidates surface as clarifications instead of the Berthier shrug); **next: CR-3 LLM modernization** (carries the routed July-3 audit items), then context carryover, personality-biased disambiguation. | CR-6 needs its own design gate | `COMMAND_ROBUSTNESS_SPEC.md` v0.2 |
