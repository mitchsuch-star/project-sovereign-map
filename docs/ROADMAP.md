...
| **5** *(new Aug 7)* | **THE ECON BALANCE PASS — row EC-P3's gate + build.** Into the gate: **CA8-D1** (what does gold buy after turn 6 — 13 building slots for the whole game, measured 44× treasury growth with 88% of income unspendable), **CA8-D5** (the threat bar boots at 91–97 — nothing new is measurable on a bar at its ceiling), **IGR-X9** (the razing/EC-U2 interaction published as multiplier-invariant), **EWC-D1/EWC-F2**, **XR-4/IGR-X4 residuals**, and the standing PLUNDER dissent (attempts used: 1 of 2 — if the acceptance test fails at a second multiplier, re-open at stability-vs-authority). Gate FIRST (these have blessed-number consequences), then the build session(s). | gate + session–multi | **User-ordered (Aug 7): "econ is still unbalanced."** Every downstream measurement — Round 0, Victory tuning, the Steam claims — reads a broken economy until this lands. The deferred-decision owner row finally gets its date. |
