...
| **13** | **Pre-EA hardening + Playtest Round 1** — UI-2d-1 first (the only open soft-lock class), then **the CA8 backlog** (`BUG_FIXES.md` §Creative Audit — 28 rows, of which the P1s CA8-1/2/5 should land earlier if the 3.75-open ruling says so), IGR-X9, EWC-F1/F2, S5-4; sweep NAD-1..4, VP-D2/D3, S5-D3; complete Settings; **code signing** so an unsigned PyInstaller server does not trip SmartScreen; 8–12 external testers. | multi-session | The open defect list is genuinely five rows, all P2/P3 — this codebase is in unusually good repair, so this is small. |
