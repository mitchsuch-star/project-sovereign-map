...
Ordering below is the recommended sequence; A-items are small and interleave freely. User priorities anchored this staging: (1) command-by-text robustness, (2) economy revisit, (3) docs that cover the bases for a great game. **Re-sequenced July 9, 2026:** the **Comprehensive Codebase Audit (AUD) now runs before the EC *build***, feeding a Fable-led **econ eval** (`ECONOMY_REVISIT_SPEC.md §0.7`) of the economy recs before any of that code is written — CR (done) → **AUD → econ eval → EC build** → MC. The EC *design* work is already done and gate-recorded; only its build moved behind the audit.
...
| **AUD** | **Comprehensive Codebase Audit** (component-by-component, fix-as-you-find) — **▶ RUNS NEXT (re-sequenced July 9, 2026)** | Fable-led sweep across every component (core mechanics; state/surface/hygiene; Map & rendering) — fix safe defects in place, escalate genuine judgment calls — closing with a creative / systems & fun-factor assessment pass (§8) and **flowing into the econ eval** (`ECONOMY_REVISIT_SPEC.md §0.7`) on a known-green build. **Sequenced BEFORE the EC build** (was "after EC") so Fable's whole-codebase context carries into an independent read of the economy recs before that code is written, and play-feel isn't muddied by economy churn. Fable operates with **decide-and-act authority** inside the fix-vs-escalate boundary (guideline §3); component coverage + phasing re-verified July 9 (completeness check — 3 stand-alone subsystems folded into §7.4/§7.5). | none for the audit (correctness fixes land directly; escalations + creative memo → user / design gate); **the econ eval feeds the EC-2 gate** | `AUDIT_GUIDELINE.md` (+ `ECONOMY_REVISIT_SPEC.md §0.7`) |
