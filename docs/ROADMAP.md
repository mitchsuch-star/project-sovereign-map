...
| **VD** | **Vassal Depth — raise Vassals 6.5 → 7+** (dependency-ordered build queue) | Post-Sweep-4 vassal build queue (STATUS "Vassal Depth Queue"; full design `VASSAL_DEEPENING_SPEC.md §1, §5–§8): **(1) VS-3 Land Grants via the F1 Diplomacy Wizard ✅ BLESSED July 15** (the positive grip lever; §1) → **(2) VS-4 loyalty-gated call-to-arms** (a disaffected vassal < ~35 loyalty **won't send troops to your wars**; wavering drags its feet; GR5; §5) → **(3) VS-5 vassal creation & transfer in peace deals** (assure the settlement `vassalage`/`liberation` clauses are reachable on the guided/wizard surface + ADD a `vassal_transfer` clause; §6) → **(4) VS-6 "The Defection" — coalition-flip as a BRIBE** (the coalition must offer a concession the vassal *wants*; outcomes = **free → guaranteed at WAR with the former lord** OR **transfer to the bribing coalition member** (reuses VS-5); supersedes the §2.7 stub; §7) → **(5) cleanup/enablers: garrison wire-or-remove (VP-D1) + enemy-AI grip-awareness (VP-D6)**. Two Sweep-4 friction fixes already LANDED July 15 (the "grant X more autonomy" parse shrug + VP-D5 tribute legibility, `7908227`). Runs alongside the CO program's Sweep 5 → exit; interleave-vs-after is the user's call. | ~~VS-3 gate~~ **✅ BLESSED July 15**; VS-4/VS-5/VS-6 numbers escalate per-slice; VS-6 depends on VS-5 | **`VASSAL_DEEPENING_SPEC.md`** |
