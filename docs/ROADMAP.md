...
| 8.EVAL | Pre-8.5 Evaluation Gate | **✅ HELD July 16, 2026** — `docs/audits/EVAL_8_2026_07_16.md` |
| 8.5 | Events, Goals & National Identity | **▶ IN PROGRESS** — **Batch Q Chunk 1 ✅ LANDED July 16, 2026** (S5-1/2/3/5 + S5-4 docstring + AUD-g/AUD-e; `tests/test_batch_q_fixes.py`) + **Chunk 2 ✅ LANDED July 16, 2026** (AUD-b zero-combat armistice guard + AUD-c war-score-aware offer direction + E7 authority-banded defiance floor + Metternich DD8 small build + the VS-5 live ratification exercise (no defects) + map CITY/COUNTRY serif label fonts; suite **13,614/3**; pre-push find→verify Workflow zero findings — see STATUS top entry). Remaining: the **8.5 design gate** (Nation Agendas core) → the **Battle Diorama (Tier A)** (row BD — ✅ queued July 17, 2026, sequenced after the agendas gate; scope = `docs/audits/BATTLE_DIORAMA_EVAL_2026_07_17.md` §6); the ~~CR-6 mini-gate~~ ✅ landed July 16, 2026 (`COMMAND_ROBUSTNESS_SPEC.md` §7); see the 8.EVAL record §3 |
