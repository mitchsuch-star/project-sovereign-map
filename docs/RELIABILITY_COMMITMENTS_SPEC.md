...
- **Case 2 — Hegemon exists, no coalition:** one or two composed lines. Hegemon phrasing resolves through `describe_hegemon_bloc` per §8.1a (below 50% = *"{Hegemon} leads a widening {descriptive_label} ({share}%)"*; at 50%+ = *"The {bloc_label} commands {share}% of Continental power"*). At `70%+`, the same Case-2 line may take one tonal lift without creating a fourth beat or a new label family (for example, *"The French System commands 72% of Continental power; the continent no longer offers neutral ground."*). In v0.1 the headline may still name a non-French hegemon for descriptive forward-compat, even though passive coalition-pressure accrual remains gated on `hegemon == world.player_nation` per §7.3 / R5; that asymmetry is intentional until D2 Coalition Generalization.
  ...
  **Non-player-hegemon honesty clause (v0.1 edge case).** When `hegemon != world.player_nation` AND `threat_level < THREAT_TENSION_MIN` (so the Tension/Murmurs flavor line above does not fire), append exactly one honesty clause so the ledger does not land as a single flat hegemon line: *"No court has yet moved to contest it."* This is composition-only — it does not retarget any scalar, does not re-enable the coalition-pressure sub-line suppressed by §11.1 L1166, and fires only in this narrow edge. It exists because the v0.1 scalar asymmetry would otherwise leave a dominant foreign hegemon visibly uncontested on the headline, which reads as "Europe is asleep" rather than as the intentional forward-compat state it is.
