...
> **April 28, 2026 repo reality:** B-Hegemony, B-B1-lite, B-B4, B-B7, and C-lite are landed. Live code has the `hegemony_target_mod` acceptance term, Balance of Europe bloc-share memory, `balance_of_europe_shifted` beats, commitment presentation routing, and ledger/dispatch surfaces. Any older note below saying those pieces are not built is historical.

> **Repo reality check (April 22, 2026).** The §7.3 engine, `describe_hegemon_bloc` helper, `balance_of_europe_shifted` beat family, and §11.1 Balance of Europe headline are **not yet built**. Live code in `backend/game_logic/coalition.py`, `backend/game_logic/diplomatic_ledger.py`, and the Godot Diplomatic Ledger still runs the older `threat_coalition` payload, the legacy anonymous `Diplomatic Tension` / `European Courts Concerned` notifications (coalition.py:1098-1132), and the pre-v2.4.3 `NATION OVERVIEW` Nations tab. `backend/game_logic/diplomacy.py` + `backend/display_names.py` already relabel the AI `decision_reason` to `hegemony_pressure`, but the mechanical term `hegemony_target_mod` and the corresponding proposal-preview `hegemony` warning are **not yet implemented** — "hegemony pressure" is currently a label rendered over legacy coalition-threat math. B-Hegemony + B-B1-lite + Slice C-lite are therefore a **substrate swap** (retire the legacy clue chain, replace the Threat & Coalition ledger tab payload, wire the real `hegemony_target_mod`), not additive copy layered on top of something that already works. Every surface below assumes the swap lands together; do not half-land the naming layer over the old mechanics. The v0.1 non-France-hegemon guards (§7.3 `add_threat` guard bullet and §7.4 "Coalition target and leader selection") and the anti-`world.player_nation` scalar are forward-compat intent that D2 Coalition Generalization will eventually fix; `coalition.py` `form_coalition`, `qualifies_for_coalition`, and `coalition_leadership_score` still anchor on France in code today, and the headline / contrast-copy suppression rules below are written to stay honest under that anchoring until D2 lands.
...
- The first time the **current hegemon's bloc** crosses **33%**, **50%**, or **60%** of Continental power, fire a same-turn **Balance of Europe** beat as a **named-diplomat notification** (the persistent-alert event family in `notifications.py`). The beat surface MUST NOT be (a) the Diplomatic Ledger Balance-of-Europe headline itself or (b) the Morning Dispatch's Balance-of-Europe summary line — both of those surfaces re-present the *current state*, and using either as the threshold beat collapses signal and state into one clue, defeating the "headline is not the first clue" rule. The notification is the upstream event; the headline and dispatch reflect the new state on the same turn.
- New-game bootstrap seeds `world.hegemony_signal_high_water` from the opening bloc share's surfaced band and `world.hegemony_signal_hegemon` from the opening hegemon after scenario load. Europe may begin the 1805 campaign already watching France warily; turn 1 must not stage that inherited state as a fresh diplomatic revelation. The first earned beat is the first *new* band crossing after play begins. Scenarios that open above `33%` therefore do **not** stage the noticed beat unless share first falls below `33%` and later rises back through it; this is intentional, because the opening is inherited political weather, not a new revelation.
...
- At `33-49%`, the beat is intentionally warning / tutorial first. On a cold board it may only arrest decay rather than create visible continental hardening from zero. `50-59%` remains the reveal band rather than a guaranteed hardening gate; `60%+` is the first band expected to produce net self-propelling pressure under the pacing baseline.
- Upward beats fire only on first crossings into a higher band. If one same-turn seam jumps multiple upward bands at once (for example `28% -> 51%` after a major treaty or vassal change), emit only the **highest** newly reached band beat on that seam and set `world.hegemony_signal_high_water` to that highest reached band; do not stack a noticed + reveal double line in the same breath. A hegemon change inside an already surfaced band counts as a fresh political arrangement rather than a silent continuation: if France cedes the lead to Russia at `52%`, Europe must get one new beat for the Russian-led arrangement rather than letting the headline become the first clue. Downward crossings from `60% -> 59%` or `50% -> 49%` are not allowed to be totally silent: emit one same-turn Talleyrand advisory aside in his existing bloc-naming register as a **turn-end dispatch aside** (not a rail notice, not a dispatch headline, not a popup, no counter-play hint) so expensive compliance is visibly rewarded. The line uses the **current-share label after the drop**: `50% -> 49%` names the descriptive alignment because the proper noun has receded; `60% -> 59%` keeps the proper noun and softens only the frame. Dedupe is owned by `world.hegemony_relaxation_bands_fired: Set[int]`, which records the surfaced bands (`2` for alarming, `3` for crisis) whose relaxation has already been voiced in the current hegemon / equilibrium epoch. Add the band to the set when the aside fires; do not replay while it remains there; clear the set when the hegemon changes or share falls below `33%`. On a multi-band downward collapse (for example `65% -> 48%` across a single turn, crossing both the `60%` and `50%` boundaries at once), emit only **one** aside — the one whose label matches current end-of-turn share (at `48%` that is the band-2 *alarming receding* aside, naming the descriptive alignment) — and record **all** crossed-out surfaced bands (`2` and `3` in this example) in `hegemony_relaxation_bands_fired` so later oscillations in the same epoch cannot re-fire either relaxation. This mirrors the upward highest-only rule above and keeps Talleyrand's aside register quiet instead of double-narrating two receding bands on one seam.
- **Relaxation-aside trigger contract (normative).** The relaxation aside fires **iff** end-of-turn share is in a strictly lower band than `world.hegemony_signal_high_water` at the moment `process_coalition_turn` evaluates the aside, the current hegemon still matches `world.hegemony_signal_hegemon`, **and** that lower band has not yet been recorded in `world.hegemony_relaxation_bands_fired` for the current hegemon epoch. The evaluation is done ONCE per turn inside `process_coalition_turn`, not on mid-turn crossings. Mid-turn oscillation (e.g. `52 → 49 → 51` within a single player turn through two ratification actions) that returns to the starting band by turn end MUST NOT fire the aside and MUST NOT touch `hegemony_relaxation_bands_fired`. This prevents (a) awkwardly narrating a "name receding" on a turn the share ended right back in the alarming band, and (b) spuriously poisoning the dedupe set, which would later suppress a genuine downward crossing. The corresponding upward-beat behavior is unchanged: upward band crossings may fire same-turn at the ratification seam (so the player hears *before* the headline refreshes), but the relaxation aside, by construction, reflects the net turn-over-turn reality.
- The below-`33%` reset is intentional rather than "reset on every lower band": Europe should not theatrically rename the same camp every time share oscillates between `45%` and `55%`; only a real return to equilibrium wipes public memory and re-arms the full upward-beat sequence. Re-crossings back into `50%` or `60%` within the same hegemon epoch therefore do **not** re-fire the reveal / crisis beat; until share drops below `33%` or the hegemon changes, the refreshed headline alone carries the returned state.
- Speaker selection is deterministic: use the highest-weight non-bloc major court first, where **weight = `power_score(court, world)` among non-bloc majors**. If that court has an authored named diplomat, use them. If **no** non-bloc major court exists at all (the hegemon bloc contains every authored major), fall straight to Talleyrand's advisory register rather than to a generic chancery. If the selected non-bloc major does **not** have an authored envoy register, prefer a Talleyrand advisory line in his existing bloc-naming register (the same register used for his other beats — no separate `hegemony_beat_talleyrand_*` family is required, per `DIPLOMAT_VOICE_BIBLE.md`) over an unauthored generic chancery; generic non-cast chanceries are the last resort, not the first fallback. No anonymous `system` speaker is allowed on this surface. Per-court register at each band lives in `DIPLOMAT_VOICE_BIBLE.md` (`hegemony_beat_*_{noticed,alarming,crisis}` minimum coverage) and is parameterized by `{hegemon_label}` rather than hard-wired to France-only copy. **Authoritative fallback chain:** the strict three-step chain (envoy → Talleyrand advisory → chancery) is owned by `COMMITMENTS_PRESENTATION_SPEC.md` §8.1 routing table; the prose above derives from it and must not disagree. If this spec and the routing table ever appear to diverge, the routing table wins.
- Every upward beat about the **player's own hegemonic rise** carries one counter-play hint. The hint must be (a) capability-aware: only suggest bloc-shrinking / treaty-lapse actions that are actually legal in the current shipped slice, never bilateral repair verbs as if they reduced bloc share, AND (b) **causally specific**: name *which* members of the hegemon's bloc account for the largest share-contribution slice, not generic levers. *"Saxony accounts for the decisive non-French slice of your bloc share — releasing it shrinks the share immediately; Europe's passive pressure eases the following turn"* is legible in the v0.1 roster; *"consider releasing a vassal"* reads as random advice. When no stronger legal lever is available, the floor hint is the always-legible restraint line: *"Hold the bloc where it stands; another major ally would harden Europe further."* At the `33%` noticed beat, the alternative tutorial floor line is explicitly allowed: *"Another major alliance would lift Europe past fifty percent and harden the courts into camp."* If the current hegemon is **not** the player in the v0.1 forward-compat edge case, the beat uses a deliberate no-hint descriptive variant rather than inventing a fake French lever over another court's bloc geometry. Multi-minor Confederation-of-the-Rhine examples are 13-nation forward-compat illustrations, not v0.1 assumptions. Compute the contributor breakdown by sorting `bloc_members` by their individual `power_score` contribution to the bloc total, **excluding the hegemon nation itself from the sort** — the hint is about reducing the bloc around the hegemon, not dissolving the hegemon, and a naive sort that does not exclude the hegemon would name France as its own decisive contributor in every 5-nation starting position. Only surface a named contributor when the matching shrink lever is actually actionable in the shipped slice; if the top non-hegemon contributor cannot be released, lapsed, or otherwise reduced by any current legal lever, fall through to the restraint-floor line rather than naming an impossible move.
- `_hegemony_signal_band(share)` and `world.hegemony_signal_high_water` / `world.hegemony_signal_hegemon` are deliberately different things. The helper is a pure current-share reader used for labels and band comparisons; the stored fields are public-memory / dedupe state seeded from scenario start and cleared only by a real return below `33%` or a change of hegemon. Wiring them together would either refire beats on every oscillation, suppress a new hegemon's reveal, or make labels lag behind the actual share.
...
- **v0.1 non-player-hegemon guard on the named target (mirrors §7.3 `add_threat` guard).** When `_calculate_hegemony_pressure` returns a non-empty dict but `hegemon != world.player_nation`, the named target still resolves to `world.player_nation`, not the foreign hegemon. Rationale: the `threat_level` scalar remains anti-`world.player_nation` in v0.1 by §7.3 / R5, so naming the coalition against `{foreign_hegemon}` while the scalar records anti-France event-threat would publish a geometry the engine cannot back up on downstream surfaces (coalition declaration popup, coalition dispatch entry, war status panel, any commitments event that reads `coalition.target`). The §11.1 "Composition rules" non-France-hegemon suppression bullet handles the Balance-of-Europe headline; this guard handles every other surface that reads the named target. Generalization — letting the named target track the actual hegemon — lands with D2 Coalition Generalization at the same time the scalar becomes per-target.
...
| **Player surface** | `balance_of_europe_shifted` beat + Balance of Europe headline + `hegemony` warning category in proposal preview | Coalition popup, war declaration, war HUD, post-war terms |
...
- **Case 1 — No hegemon (max bloc share < 33%):** single standalone line. (Threshold matches `_calculate_hegemony_pressure`'s pressure-floor return per §7.3 — the engine returns no hegemon below 33% even though the §9.1 acceptance-formula penalty starts at 30%.)
  ...
  The equilibrium line is standalone. If coalition pressure is independently brewing from event-based threat (battles, captures, vassalization) with no hegemon present, a BREWING line from Case 3 may still render below it; composable. When Case 1 + Case 3 compose, the second line anchors the threat target to the player nation (`world.player_nation`) rather than a bloc — there is no bloc to name, and the brewing was triggered by aggression, not by share dominance. Both facts coexist and are intentional.
  Worked example (equilibrium + brewing from event threat):
  >
  > *"Balance of Europe — no bloc commands decisive power. The continent remains in equilibrium, though France's recent actions have drawn coordinated attention."*
  >
  > *"Coalition pressure against France: Brewing (64/100). Qualifying: Austria, Britain, Prussia."*

- **Case 2 — Hegemon exists, no coalition:** one or two composed lines. Hegemon phrasing resolves through `describe_hegemon_bloc` per §8.1a (below 50% = *"{Hegemon} leads a widening {descriptive_label} ({share}%)"*; at 50%+ = *"The {bloc_label} commands {share}% of Continental power"*). At `70%+`, the same Case-2 line may take one tonal lift without creating a fourth beat or a new label family (for example, *"The French System commands 72% of Continental power; the continent no longer offers neutral ground."*). In v0.1 the headline may still name a non-French hegemon for descriptive forward-compat, even though passive coalition-pressure accrual remains gated on `hegemon == world.player_nation` per §7.3 / R5; that asymmetry is intentional until D2 Coalition Generalization.
  >
  > *"European courts have taken note, and private consultations are quickening, but no coordinated response has yet formed."* (present during the `Tension` / `Murmurs` tiers per `COALITION_SPEC.md` §3a.)

  **Non-player-hegemon honesty clause (v0.1 edge case).** When `hegemon != world.player_nation` AND `threat_level < 60` (i.e. below the BREWING threshold), the Tension/Murmurs flavor line above is **also suppressed** and replaced by exactly one honesty clause so the ledger does not land as a single flat hegemon line: *"No court has yet moved to contest it."* The flavor line "European courts have taken note, and private consultations are quickening" reads as though the consultations are about the named hegemon, which the v0.1 anti-`world.player_nation` scalar cannot honestly back up; suppressing it across the whole 0-59 threat band (instead of only `< THREAT_TENSION_MIN`) closes the voice-intent mismatch where mid-band threat would otherwise publish a consultations-are-quickening line the scalar does not support. This is composition-only — it does not retarget any scalar, does not re-enable the coalition-pressure sub-line suppressed by the §11.1 Composition-rules non-France-hegemon suppression bullet, and fires only in this narrow edge. It exists because the v0.1 scalar asymmetry would otherwise leave a dominant foreign hegemon visibly uncontested on the headline, which reads as "Europe is asleep" rather than as the intentional forward-compat state it is.

- **Case 3 — Coalition BREWING (no leader yet per COALITION_SPEC §3-§4):** hegemon line + brewing line. Hegemon phrasing follows the §8.1a contract above. Bloc-reference resolves through the helper chain `{bloc_label or descriptive_label or hegemon}` so the second line always uses the same noun the hegemon line used — proper noun at `50%+`, descriptive phrase at `33-49%`, bare hegemon name only as last-resort fallback. See §8.1a.6 for the normative rule that `bloc_label` is `None` below `50%`. When no hegemon exists and Case 1 + Case 3 compose, the brewing target falls back to `world.player_nation` per Case 1's composition rule above rather than attempting to render a bloc label.
  >
  > *"Coalition pressure against the {bloc_label or descriptive_label or hegemon}: Brewing ({threat}/100). Qualifying: {nation_list}."*
  ...
  >
  > *"Coalition pressure against the French-led alignment: Brewing (64/100). Qualifying: Austria, Britain, Prussia."*
...
- **Case 4 — Coalition DECLARED (leader selected):** hegemon line + leader line + formal coalition line. Coalition declaration copy contrasts the formal coalition against the named bloc (e.g. *"Britain's coalition marches against the French System"*) per §8.1a surface contract.
  >
  > *"{Coalition leader's named diplomat} has begun private consultations."*
  >
  ...
  On the declaration turn only, the coalition-declaration popup / dispatch carries the §8.1a contrast copy (for example, *"Britain's coalition marches against the French System"*). This contrast copy is the **peak dramatic moment** for the bloc-naming contract — louder than the Case 4 ledger line, which is steady-state presence. Author the declaration copy with at least the same voice budget as `breach_lead_*` lines (named diplomat, single committed sentence, no hedging); do not let the ledger echo and the declaration popup read at equal weight. The headline keeps the composed three-line form above. See `COALITION_SPEC.md` §3f for the full four-surface contrast enumeration, including the declaration-turn campaign log entry. Playtest gate: if declaration turns read as crowded, suppress the Case 4 flavor line on the declaration turn itself and restore it on later declared turns.

  **Non-France-hegemon guard on declaration contrast copy (mirrors the §11.1 Composition-rules non-France-hegemon suppression bullet ledger suppression).** When `coalition.target == world.player_nation` (per §7.4) but `hegemon != world.player_nation`, the declaration copy targets the coalition target — bare `world.player_nation`, no bloc contrast — not the named hegemon bloc. Rendering *"Britain's coalition marches against the Russian Alignment"* while the scalar and coalition target are actually anti-France would publish the same visible lie the ledger sub-line suppression is written to prevent. Bloc-contrast form returns when D2 Coalition Generalization makes the scalar per-target. See `COMMITMENTS_PRESENTATION_SPEC.md` §8.1a.4 for the presentation-side mirror of this rule.
...
- The Case 4 flavor line uses the coalition leader's named diplomat. Britain keeps the subsidy register (*"Castlereagh has begun assembling subsidies"*); non-British leaders should instead use consultations / ambassadors / war-council language in the same single-clause slot rather than assigning them a British subsidy function, or else rely on the declaration-turn suppression path above if the turn is already crowded. If the leader has no named diplomat in `diplomat.py`, fall back to a court-level version of the same register rather than forcing a subsidy line onto a non-British court.
- **Qualifying-courts flourish (optional, lawful extension).** Authors MAY append a second clause to the Case 4 flavor line that names up to two of the top non-leader qualifying courts (e.g. *"Berlin and Vienna are listening."*). When rendered, the flourish is selected deterministically from the qualifying list sorted by `power_score` descending with alphabetical tie-break, capped at two names, dropped entirely when fewer than two non-leader qualifying courts exist. The flourish does not add a new line — it extends the existing Case 4 flavor line. This is an extension point, not a required render; the minimal contract is the single-clause form in the worked example. When in doubt, omit.
- **Headline line cap (hard).** The Balance-of-Europe headline is capped at THREE lines total. On compound states — e.g. (a) Case 5 COOLDOWN with residual pressure active (hegemon/equilibrium + cooldown + residual-flavor); (b) a turn where a threshold beat also fires but routes to the rail without stacking into the headline (ledger stays in whichever case applies); (c) a Case 4 DECLARED ledger with the qualifying-courts flourish extension on the Case 4 flavor line (hegemon + extended-flavor + leader-leads); (d) Case 2 hegemon + the non-player-hegemon honesty clause above — suppress the lowest-priority optional line when rules would otherwise stack. Priority order is (1) hegemon/equilibrium line, (2) coalition-structural line (Case 3 brewing / Case 4 declared / Case 5 cooldown), (3) flavor / subsidy / residual / honesty line. Never render four lines even when rules would otherwise stack.
- In the v0.1 non-France-hegemon edge case, Case 3 / Case 4 MUST suppress **both** the Case 3 brewing sub-line AND the Case 4 *"{leader} leads the coalition against the {bloc_label}"* formal-coalition line entirely. ("Coalition-pressure sub-line" in this rule means both — not just the `(threat/100)` scalar fragment, which Case 4 has already dropped per the Case 4 status-line rule above.) Do not retarget either line to France as a workaround: the `threat_level` scalar is anti-`world.player_nation` in v0.1, so rendering *"Coalition pressure against France: 64"* while the player IS France is confusing, and rendering *"Coalition pressure against the Russian Alignment: 64"* while `64` is the anti-France scalar is a visible lie. Suppression is the only safe render. The coalition-pressure sub-line returns when D2 Coalition Generalization makes the threat scalar per-target; until then, Case 3 / Case 4 composite states with a non-France hegemon render only the hegemon line + any applicable flavor line. Case 4 retains the hegemon line and the Case 4 flavor line; the bloc-vs-coalition contrast structure returns with D2. **Reassurance to the reader:** in this composite the ledger necessarily goes quiet on brewing context, but the coalition-declaration popup, rail `balance_of_europe_shifted` beat, and war status HUD still carry the brewing state against `world.player_nation` per §7.4 "Coalition target and leader selection"; the ledger surface re-acquires the full BREWING / DECLARED lines at D2 Coalition Generalization.
...
- **Same-band hegemon swap also fires a beat.** Per §7.3, if the hegemon identity changes while share stays in the same surfaced band (e.g. France `52%` → Russia `52%`), a fresh `balance_of_europe_shifted` beat fires for the new arrangement and `world.hegemony_signal_hegemon` updates. The headline must never become the first clue just because the band number stayed the same.
...
Per-nation **bloc membership badges** (`[French System]`, `[Coalition Member]`, `[Neutral]`, `[Vassal of Saxony]`) are **deferred out of v2.4.3** and tracked as a later-slice item in `RELIABILITY_IMPLEMENTATION_PLAN.md` Slice D3. The locked v2.4.3 naming layer is concentrated on the Balance of Europe headline, `balance_of_europe_shifted` threshold beats, proposal-preview hegemony warnings, and coalition-declaration contrast copy. Spreading the naming layer across the whole Nations tab in v2.4.3 would weaken the headline as the main dramatic owner and make "hegemon bloc" vs "formal coalition" easier to blur, so the row-stamp surface waits for a later playtest pass that explicitly asks for it.
...
- one immediate counter-play lever when one is legible from current state. For `hegemony` warnings, the lever is bloc-shrinking only (release a vassal, avoid a new major ally, let an alliance lapse). For bilateral betrayal / chancery-state blockers, `Make Amends` with a wronged court may appear once B-B7 is live.
...
- on a turn that crosses a new hegemony band (as defined by `_hegemony_signal_band` in `coalition.py` — `0 / 1 / 2 / 3` for below-threshold / noticed / alarming / crisis, corresponding to the `33% / 50% / 60%` beat thresholds rather than the ladder gates), reserve one inline slot for the new `hegemony` warning if the current proposal would intensify that pressure. This reservation keys off the **current-share** band helper, not the stored `hegemony_signal_high_water` dedupe field, so a proposal preview opened later on the same turn as a ratification seam sees the upgraded warning immediately. If the inline cap is already saturated by `critical` warnings, the hegemony warning displaces the lowest-severity slot rather than overflowing — a hegemony band-crossing turn is the dramatic moment where dropping it silently would be worst.
- In the `30% < share < 33%` pre-noticed zone, `hegemony` preview warnings use private-tally wording only (*"Several courts have begun tallying French allies privately."*), with no bloc label and no proper name. Descriptive alignment language begins only once `_hegemony_signal_band(share) >= 1`; the proper bloc name still waits until `_hegemony_signal_band(share) >= 2`.
...
- `balance_of_europe_shifted` (same-turn named-diplomat notice / dispatch **event line** on upward `33 / 50 / 60` crossings — a one-shot turn-N callout, **distinct** from the Morning Dispatch's Balance-of-Europe **state-summary** line which re-presents current share/hegemon every morning. Both surfaces can coexist on the same morning: the event line is the upstream clue, the summary line is the refreshed state. Per §7.3's threshold-crossing signal contract, the event beat MUST NOT collapse into the state summary. Quieter downward relaxations at `60 -> 59` and `50 -> 49` stay advisory-only per §7.3.)
- betrayal recorded
...
- `hegemony_signal_high_water: int` — stored public-memory field for upward `33 / 50 / 60` beat dedupe; seeded from the opening share-derived band on new game, default `0` on load, reset below `33%`; serialized through save/load
- `hegemony_signal_hegemon: Optional[str]` — hegemon identity paired with `hegemony_signal_high_water` so a same-band hegemon swap (for example France `52%` -> Russia `52%`) still earns a fresh upstream beat instead of making the headline the first clue; seeded from the opening hegemon alongside that opening band on new game, default `None` on load, reset below `33%`; serialized through save/load
- `hegemony_relaxation_bands_fired: Set[int]` — surfaced bands whose downward relaxation aside has already fired in the current hegemon / equilibrium epoch (`2` = alarming, `3` = crisis); cleared when the hegemon changes or share falls below `33%`; serialized through save/load
- `positive_threat_delta_this_turn: bool` — transient per-turn flag (not serialized) set `True` when `add_threat()` applies any positive increment during the current turn and cleared after turn-end / ledger evaluation; backs the `residual_pressure_active` AND-gate in §11.1 Case 5 and `COMMITMENTS_PRESENTATION_SPEC.md` §11 so quiet cooldown turns above Tension do not loop the residual-pressure line

On save-load migration from a pre-v2.4.3 file with no hegemony-memory fields, the first B-Hegemony / ledger bootstrap may reseed `hegemony_signal_high_water` and `hegemony_signal_hegemon` from the loaded current share before evaluating beats. Inherited political weather should not stage a stale turn-one notice just because the older save serialized no memory.
...
**Coalition `threat_level` scalar (`coalition.py`) — ONE-TURN lag exists.** The passive hegemony threat contribution is added via `add_threat()` during `process_coalition_turn` (end-of-turn), not at ratification moment. When France ratifies an alliance, the coalition threat scalar jumps on turn N+1, not turn N. The Balance of Europe **headline label/share** and same-turn `balance_of_europe_shifted` beat update on the ratification turn; the lag applies to scalar-driven states such as `Tension`, `Murmurs`, `BREWING`, and declaration pressure.
...
After the §7.3 realignment that moved the ladder gates to `33 / 50 / 60 / 70`, the inconsistency direction inverted. The §9.1 acceptance-formula penalty returns `0` at exactly `share = 0.30` and only reaches `-1` once `share` crosses `~0.3167` (integer truncation of `(share - 0.30) * 60`); it scales from there to `-18` at exactly `0.60` and clamps at `-20` from `~0.6334` onward. `_hegemony_pressure_for_share` returns `0` until `share >= 0.33`. Between `~31.67%` and `33%` bloc share, a pair sees a `-1` acceptance-formula penalty on cross-bloc proposals but no per-turn pressure accrual and no `balance_of_europe_shifted` beat. At exactly `share = 0.30`, both sides return `0` (no silent tax on the boundary). At exactly `share = 0.33`, the ladder returns `1` (per-turn pressure begins) while `hegemony_target_mod` has already returned `-1` for `~1.33` percentage points. This is intentional per §7.8.1 — per-pair friction begins one band before continental consensus, mirroring how foreign offices privately count an opponent's allies before public clamor. The earlier R11 wording (`-1 to -1.8` across the 30-33% band) was a pre-integer-truncation artifact and has been corrected to match the §9.1 formula at L898-903.

**Mitigation:** documented design call. The ~31.67%-33% gap surfaces in proposal-preview `warnings[]` (the per-pair friction is visible when the player attempts a relevant cross-bloc proposal — not silent), even though no headline / beat fires yet. If playtest shows the pre-noticed band reading as "the engine penalizes me for a bloc Europe hasn't noticed," shift the formula floor from `0.30` to `0.33` to merge the two thresholds; the slope `int((share - X) * 60)` then re-anchors automatically. The other direction (lowering the ladder gate) is forbidden because it would re-create the silent-tax band the realignment closed.

**Integer-bucket test lock (required for B-B1-lite):** the `hegemony_target_mod` test list must pin the truncation buckets, not just the endpoints: `share=0.30 → 0`, `share=0.316 → 0`, `share=0.317 → -1`, `share=0.333 → -1`, `share=0.334 → -2`, `share=0.50 → -12`, `share=0.60 → -18`, `share=0.70 → -20`. "Scales correctly" without these buckets hides the R11 boundary artifact.
...
- **April 20, 2026 — v2.4.3 Deep-audit fixes.** Pre-implementation deep audit (`MEMORY_AND_PRESSURE_V2_4_2_DEEP_AUDIT.md`) raised 14 findings — 2 CRITICAL, 7 MAJOR, 5 MINOR — against v2.4.2. v2.4.3 applies the full action list. No new mechanical features; several contracts tightened, one new sub-section (§8.6.1a), one new sub-section (§8.8.7a), three new risks (R9/R10/R11). Edits by section:
  ...
  - **§11.1 — Balance of Europe state machine.** Specified five composition cases: no hegemon (equilibrium line), hegemon without coalition, coalition BREWING (no leader yet per COALITION_SPEC §3-§4), coalition DECLARED, coalition COOLDOWN. Fixes the prior example's assumption that brewing has a named leader. (Audit A12, B6.)
