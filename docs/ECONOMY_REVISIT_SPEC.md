...
| Slice | Scope | Gate |
|-------|-------|------|
| **EC-0 (defect)** | Fix the `advance_turn` AP reset to be world-scoped; regression tests (Austria holds 4 AP; `ap_per_turn` penalties apply-then-release correctly for Europe-only nations); re-baseline any Slice-8 balance pin the fix moves. | None |
