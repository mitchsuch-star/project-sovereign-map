...
> **Status:** v1.29 A1-E LANDED / SLICE F AUDIT SYNTHESIZED / UI-UX CONTRACT READY FOR CODING / POST-F ALLY AGENCY TRACKED
> **Last Updated:** May 5, 2026
> **Source spec:** `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.31
...
- Pick one settlement dialogue action schema and test it. `settlement_confirm` should use the typed `options[].action` values that Godot popups already emit (`confirm_settlement`, `revise_settlement_terms`, `back_out_settlement`) and the backend response handler must accept those exact ids. Do not leave a second divergent `actions=["confirm", ...]` contract in the same payload unless it is debug-only and proven ignored by Godot.
...
- Extend the war-detail entry signature before adding CTAs. `diplomacy_wizard.gd::open_for_nation(nation, war_id=null)` or a sibling `open_for_war(war_id, nation)` must exist before coalition/detail buttons are wired, so war-scoped entry cannot collapse back to ambiguous pair lookup.
...
- Extend wizard entry to preserve war context. `open_for_nation(...)` must accept an optional `war_id` or delegate to `open_for_war(...)`; the preview request and any displayed Open Settlement action should carry that `war_id` through when launched from war detail or coalition detail.
...
- Make `settlement_confirm` the primary comprehension surface. Compact popup order is: awe banner when applicable; header as an action question ("Will {accepting_leader} accept this settlement?") with war label and whole-war/separate-front badge; covered-enemy chips with uncovered enemies shown as still at war; Acceptance verdict band + `score/threshold` + top-blocker callout; Warnings with all hard stops plus the top warning and severity-count overflow (`+1 hard stop, +3 warnings`, not just `+4 concerns`); Terms top rows grouped by payer/beneficiary and split into Bargains vs Rewards whenever a bargain target exists; side/outcome rows for "Your side" and "Their side" with `standing_display`, `standing_phrase`, material share, and one outcome stamp (`leader`, `rewarded`, `consulted`, `shut out`, `sold out`, or `ignored`); then `Ratify Settlement`, `Revise Terms`, and `Back Out` actions. Ledger remains the durable detail surface; war detail remains an entry/context surface.
...
- Polish standing/overflow text after the functional route passes. Tooltip and popup overflow rows should use the same marker/indent style as participant rows so "View all participants" / `+N concerns` reads as an intentional affordance, not a dangling debug suffix.
...
- Add backend tests proving repeated probes do not backfill, while executing `propose common peace with Britain` backfills exactly one `war_instance` and stages `settlement_confirm`. Include a property-style probe test that 100 probes leave `world.war_instances`, `world.war_contribution_scores`, dialogue stack, and cache invalidation counters unchanged.
- Add backend tests proving `open_settlement` is available at `DP=0`; typed command and wizard path have parity; disabled reasons expose player-facing display text and retain raw diagnostics only in debug fields.
...
- Add source-level Godot tests proving `diplomacy_wizard.gd` maps `open_settlement`, `proposal_confirm_popup.gd` has a `settlement_confirm` content path, war-detail/coalition detail expose distinct settlement CTAs/signals, and `main.gd` routes `incoming_settlement_offer`.
...
- Add action-schema tests proving `settlement_confirm` uses the action ids Godot emits and `/respond_to_diplomatic_dialogue` accepts; do not permit drift between `actions` and `options`.
...
- Add raw-enum negative tests/source guards across wizard, settlement popup, war tooltip, war detail, ledger Recent Settlements expansion, notification rail, dispatch one-liner, and campaign log.
...
- Only after Slice F passes may manual Godot smoke begin. Required smoke:
  1. F1/Diplomacy button -> Britain -> Open Settlement at `DP=0` -> `settlement_confirm` with awe banner if applicable, war label, whole/separate badge, covered-enemy chips, acceptance band as score/threshold, top blocker, warnings with severity-count overflow, terms, side/outcome rows, and no raw enums -> Ratify Settlement -> required result feedback -> ledger Recent Settlements expanded/expandable.
  2. War-score row -> war detail -> Open Settlement -> `settlement_confirm` -> Back Out leaves no pending settlement dialogue.
  ...
  5. Raw enum scan across F1 wizard, settlement popup, war-status tooltip, war detail, coalition detail, diplomatic ledger, notification rail, dispatch, and campaign log. Any visible settlement/standing/error enum fails the gate.
