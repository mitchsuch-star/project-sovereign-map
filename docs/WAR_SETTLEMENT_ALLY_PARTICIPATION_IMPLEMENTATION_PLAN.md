...
- Add cross-war settlement collision tests: stage `settlement_confirm` on war A, activate or generate `incoming_settlement_offer` for war B, assert the active hard-stop is not overwritten, the offer queues/defers through the documented settlement-owned route, and a humanized "resolve current settlement first" beat is surfaced. SUPERSEDED BY SETTLEMENT_UI_CLEANUP_SPEC.md SC-5 - incoming-offer collision applies only if SC-5 is explicitly reversed; otherwise incoming offers remain hidden, and outgoing cross-war collision follows SC-26.
