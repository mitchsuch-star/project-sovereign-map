...
> **Cleanup readiness:** Superseded by `SETTLEMENT_UI_CLEANUP_SPEC.md` v0.27 for player-facing settlement readiness, incoming-offer exposure, route-id shape, rejected/losing recovery, and Gate 4 smoke.
...
- Historical `incoming_settlement_offer` response contracts and dialogue taxonomy are inactive during cleanup. While `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-5 remains deferred, do not register a current-turn offer type, mailbox-browseable row, action ids, or player-facing response path. Post-cleanup Slice G1 owns the producer, stable offer identity, `accept_settlement_offer` / `reject_settlement_offer` / `request_settlement_revision` actions, live revalidation into `settlement_confirm`, cooldown behavior, mailbox/pending-envoy UI, and tests after SC-5 is explicitly reversed.
...
- Do not add `DialogueManager.DIALOGUE_PRIORITY["incoming_settlement_offer"]`, `MAILBOX_SUMMARY_LABELS["incoming_settlement_offer"]`, mailbox ordering, activation routes, or Godot rows while SC-5 remains deferred. Slice G1 must add those together with producer/action behavior after explicit SC-5 reversal; cleanup-time tests instead prove no mailbox count, row, activation, route, or generic fallback is reachable.
...
- AI-to-player common-peace offers remain inactive during cleanup. They may create an incoming settlement-review dialogue only after SC-5 is explicitly reversed in the cleanup spec and STATUS, at which point Slice G1 must route acceptance through the same confirm executor rather than direct mutation.
- Post-cleanup Slice G1 AI package-construction guard: AI common peace can surface only after it builds a concrete package that scores `>= 50` through the accepting side's common-peace acceptance formula. Losing-side AI offers must include enough concessions/returns to pass that check or record `package_unacceptable` and stay silent. This is not a cleanup-time exposure requirement.
- Post-cleanup Slice G1 AI common-peace anti-spam: one active incoming settlement offer at a time, one new player-facing settlement offer per turn, and a 3-turn per-`war_id` cooldown after reject/request-revision/expiry/live-state void. Store this in a common-peace namespace separate from bilateral proposal cooldowns unless the action also sends a bilateral proposal. During cleanup, no player-facing settlement offer may surface, count, activate, notify, dispatch, or block turns.
...
- Cleanup-time AI-to-player common-peace offer tests prove deferred offers cannot surface, count, activate, notify, dispatch, route, show generic fallback text, or block turns unless SC-5 is explicitly reversed. Current-turn/mailbox taxonomy, accept/reject/request-revision response shapes, confirm-executor promotion, cooldown, and one-active-offer tests move to Slice G1.
- Cleanup-time mailbox tests prove `incoming_settlement_offer` has no `DIALOGUE_PRIORITY`, no `MAILBOX_SUMMARY_LABELS` row, no Godot route, and no activation path while deferred. Slice G1 adds the positive mailbox route tests only after explicit SC-5 reversal.
...
Active cleanup override: this historical Slice F plan is subordinate to `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-5. While SC-5 remains deferred, Slice F must prove incoming settlement offers have no player-facing exposure. It must not add producer, mailbox, pending-envoy, popup, accept/reject/request-revision, notification, dispatch, or turn-blocking behavior for `incoming_settlement_offer`. Those behaviors belong only to post-cleanup Slice G1 after SC-5 is explicitly reversed in the cleanup spec and `docs/STATUS.md`.
...
- Enforce the SC-5 no-exposure branch in Slice F. Do not add incoming-offer response handlers, mailbox labels, pending-envoy routes, popup routes, notification/dispatch rows, or turn-blocking behavior in cleanup-time Slice F. If SC-5 is later reversed, post-cleanup Slice G1 must land the producer, payload identity, package-preserving accept/reject/request-revision actions, stale-state promotion checks, cooldowns, mailbox/pending-envoy UI, and behavior tests together.
- Pin the cross-war settlement collision rule. Only one settlement hard-stop may own the active popup at a time. Cross-war restaging during an active settlement-family hard stop is rejected with `error="cross_war_settlement_collision"` and humanized "resolve current settlement first" copy; it is not queued through the mailbox or `pending_settlement_dialogues`, and it leaves the active draft/dialogue unchanged. Prior queue/defer behavior is replaced by SETTLEMENT_UI_CLEANUP_SPEC.md SC-26. AI-driven cross-war retries follow the cleanup spec SC-26 retry contract only if SC-5 is reversed.
...
- Make the diplomacy wizard the primary settlement entry point. Historical `diplomacy_wizard.gd::_build_command()` mapping of `open_settlement` to typed `propose common peace with {nation}` is SUPERSEDED BY SETTLEMENT_UI_CLEANUP_SPEC.md SC-1 / SC-13 / SC-27: cleanup-time Godot entry must use the structured v0.27 cleanup payload (`war_id`, `selected_target_nation`, `covered_enemy_participants`, `settlement_terms`, `draft_key`, `caller_kind`) and must not synthesize natural-language command text for normal player settlement authoring. Typed command remains a fallback/dev path only and rejects multi-war ambiguity.
...
- Keep `incoming_settlement_offer` hidden in Slice F. Cleanup-time source and behavior checks must reject or drop injected incoming-offer payloads without generic proposal fallback, mailbox activation, pending-envoy activation, popup exposure, notification, dispatch, route mutation, or turn blocking. The former minimum incoming-offer routing requirement is inactive until SC-5 is explicitly reversed; the restored implementation then belongs to Slice G1, not this Slice F cleanup path.
...
- Add source-level Godot tests proving `diplomacy_wizard.gd` routes `open_settlement` through the structured v0.27 cleanup settlement payload rather than typed command synthesis, `proposal_confirm_popup.gd` has a `settlement_confirm` content path, war-detail/coalition detail expose distinct settlement CTAs/signals, `card_clicked` carries `war_instance_id`, and row/detail routing preserves `war_instance_id`. SUPERSEDED BY SETTLEMENT_UI_CLEANUP_SPEC.md SC-5 - incoming-offer Godot routing must be hidden/no-exposure by default unless SC-5 is explicitly reversed.
...
- Add dialogue/mailbox/source tests proving the cleanup no-exposure branch: injected `incoming_settlement_offer` payloads cannot surface, count, activate, notify, dispatch, route, show generic pending text, expose accept/reject/request-revision actions, or block turns while SC-5 remains deferred. Current-turn offer taxonomy, mailbox labels, activation routes, response handlers, and package-preserving action tests move to Slice G1 after explicit SC-5 reversal.
