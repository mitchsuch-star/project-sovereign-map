...
> **Status:** v1.30 A1-E LANDED / SLICE F PRE-IMPLEMENTATION AUDIT FOLDED / UI-UX CONTRACT READY FOR CODING / POST-F ALLY AGENCY TRACKED
> **Last Updated:** May 5, 2026
> **Source spec:** `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.32
...
- Thread one stable `route_id` end to end. The id produced by the confirmation/summary event must be the same value used by dispatch, notification metadata, ledger `settlement:<idx>:<route_id>` links, and any post-F auto-expand work.
- Extend the war-detail entry signature before adding CTAs. `diplomacy_wizard.gd::open_for_nation(nation, war_id=null)` or a sibling `open_for_war(war_id, nation)` must exist before coalition/detail buttons are wired, and war-status row clicks must preserve `war_instance_id` into the detail popup. War-scoped entry cannot collapse back to ambiguous pair lookup.
...
- Extend wizard entry to preserve war context. `open_for_nation(...)` must accept an optional `war_id` or delegate to `open_for_war(...)`; the preview request and any displayed Open Settlement action should carry that `war_id` through when launched from war detail or coalition detail. `war_status_panel.gd` must either emit `war_instance_id` with `card_clicked` or `main.gd` must look up and pass it from the cached row before opening `war_detail_popup.gd`.
...
- Make `settlement_confirm` the primary comprehension surface. Compact popup order is: awe banner when applicable; header as an action question ("Will {accepting_leader} accept this settlement?") with war label and visually loud whole-war/separate-front/partial badge; covered-enemy chips with uncovered enemies shown as still at war; side/outcome rows for "Your side" and "Their side" with `standing_display`, `standing_phrase`, material share, and one outcome stamp (`leader`, `rewarded`, `consulted`, `shut out`, `sold out`, or `ignored`); Acceptance verdict band + `score/threshold` + top-blocker callout; Warnings with all hard stops plus the top warning and severity-count overflow (`+1 hard stop, +3 warnings`, not `+4 concerns` or `more concerns`); Terms top rows grouped by payer/beneficiary and split into Bargains vs Rewards whenever a bargain target exists; then `Ratify Settlement`, `Revise Terms`, and `Back Out` actions. Ledger remains the durable detail surface; war detail remains an entry/context surface.
- Render awe set pieces through settlement voice/display helpers, not raw tag labels. Tags such as `triple_forced_alignment`, `tilsit_bargain_and_betrayal`, `defensive_coalition_preservation`, and `balance_of_europe_crossing` should produce a one-line Talleyrand-register banner via the same voice pathway as the rest of settlement presentation.
...
- Define stale-state and revise behavior. `revise_settlement_terms` should preserve context by reopening the settlement review/popup, reopening the wizard at the selected nation/war with prior diagnostics and draft terms, or returning an explicit current-turn "Resume settlement" beat. It must not dump to the nation picker. Confirm-time `must_reopen=True` failures must produce `error_display` and `reopen_target` back to the same nation/war context. If Slice F does not implement a real terms editor, the Revise button copy or result message must honestly say where it will send the player; a no-op or nation-list dump fails the slice.
- War-detail surfaces must offer a settlement affordance. `war_detail_popup.gd::show_war()` exposes `Open Settlement` when the row has `war_instance_id` or probe eligibility says `would_backfill=True`. `show_coalition()` exposes `Open Whole-War Settlement` when visible member rows share one `war_instance_id`. Existing Target/Negotiate buttons remain secondary and mean bilateral/separate-peace intent.
...
- War status standing must not silently disappear for default-start WAR pairs. When no active `war_instance` exists, `war_status.py` should emit `standing_status="no_instance"` and `standing_status_display` such as "Settlement standing appears once you open settlement on this war." When a `war_instance` exists but no material contribution is accrued, emit `standing_status="no_contribution_yet"`; real rows use `standing_status="available"`. `war_status_panel.gd` must render this placeholder and add a short legend such as "Higher contribution means a stronger voice in settlement."
...
- Add source-level Godot tests proving `diplomacy_wizard.gd` maps `open_settlement`, `proposal_confirm_popup.gd` has a `settlement_confirm` content path, war-detail/coalition detail expose distinct settlement CTAs/signals, war status row/detail routing preserves `war_instance_id`, and `main.gd` routes `incoming_settlement_offer` from both pending-envoy and mailbox-activation paths.
- Add presentation tests proving `build_settlement_confirm_dialogue(...)` carries every required field: `war_label`, `war_scope_display`, compact/medium `review_sections`, covered-enemy display chips, acceptance display fields including `band_display` and threshold text, top blocker/fixable labels, warning display fields, awe tag display labels, route metadata, settlement tier display, and humanized term/standing labels.
...
- Add dialogue/mailbox/source tests proving `incoming_settlement_offer` has current-turn offer taxonomy, mailbox label, activation route, and does not silently fall into generic pending text.
- Add stale-state tests proving confirm-time and incoming-offer promote-time `must_reopen=True` returns a humanized reason and `reopen_target` back to the same war/nation context without mutation.
...
- Add route-id continuity tests proving confirmation -> `settlement_summary` event -> dispatch -> notification meta -> ledger row all preserve the same `route_id`.
- Add raw-enum negative tests/source guards across wizard, settlement popup, war tooltip, war detail, ledger Recent Settlements expansion, notification rail, dispatch one-liner, campaign log, and settlement voice templates. Include acceptance component ids, route targets, awe tags, and the legacy popup strings `more concern` / `more concerns`.
...
- Add coalition disambiguation tests for concurrent coalition wars: whole-war settlement from coalition A must emit coalition A's `war_id`, and bilateral war-detail Open Settlement must emit the row war id, not the coalition-wide id.
- Add awe banner tests proving at least one set-piece is rendered through settlement voice/display helpers and never displays raw `triple_forced_alignment` / `tilsit_bargain_and_betrayal` style tags.
...
- Only after Slice F passes may manual Godot smoke begin. Required smoke:
  ...
  3. Coalition header/detail -> Open Whole-War Settlement when rows share `war_instance_id` -> popup covers all expected enemy participants and uses a friendly whole-war label, not raw `war_id`.
