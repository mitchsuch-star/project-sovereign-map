...
- Add dialogue/mailbox/source tests proving `incoming_settlement_offer` has current-turn offer taxonomy, mailbox label, activation route, backend response handlers for accept/reject/request-revision, and does not silently fall into generic pending text.
- Add cross-war settlement collision tests: stage `settlement_confirm` on war A, activate or generate `incoming_settlement_offer` for war B, assert the active hard-stop is not overwritten, the offer queues/defers through the documented settlement-owned route, and a humanized "resolve current settlement first" beat is surfaced.
