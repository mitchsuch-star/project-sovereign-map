...
> **Status:** v1.28 A1-E LANDED / SLICE F UI-UX CONTRACT TIGHTENED BEFORE IMPLEMENTATION / POST-F ALLY AGENCY TRACKED
> **Last Updated:** May 5, 2026
> **Source spec:** `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.30
...
- Widen `build_settlement_confirm_dialogue(...)` to include the full popup contract: friendly `war_label`, staged leaders, `settlement_terms`, compact and medium `review_sections` from `build_settlement_review(...)`, acceptance payload with `band_display`, top blocker/top fixable component labels, `hard_stops`, warnings with `code_display`, settlement tier display, `awe_tags`, and route/review metadata. The dialogue may preserve raw ids for diagnostics, but every player-rendered row needs a display sibling.
...
- Make `settlement_confirm` the primary comprehension surface. Compact popup order is: header with war label and whole-war/separate-front badge; Acceptance verdict band + score + top-blocker callout; Warnings with all hard stops plus the top warning and `+N concerns`; Terms top rows grouped by payer/beneficiary and split into Bargains vs Rewards when both exist; Allies top rows with `standing_display`, `standing_phrase`, material share, and one outcome stamp (`leader`, `rewarded`, `consulted`, `shut_out`, `sold_out`, or `ignored`); then `Ratify Settlement`, `Revise Terms`, and `Back Out` actions. Ledger remains the durable detail surface; war detail remains an entry/context surface.
- Promote immediate ratification feedback to required. After `confirm_settlement`, show "Settlement Ratified" with `war_label`, resolved-pair count, and a Review Settlement route. The rail/ledger remain the full review surfaces; the popup/result beat is required so confirmation does not feel silent.
- Define stale-state and revise behavior. `revise_settlement_terms` should preserve context by reopening the settlement review/popup or returning the player to the selected nation's action view with prior diagnostics intact; it must not dump to the nation picker. Confirm-time `must_reopen=True` failures must produce human text and a route back to the same nation/war context.
...
- Add a war-scoped entry contract for detail/coalition launches. New Godot signal/payload should carry `war_id` and `target_nation`; backend settlement opening should accept either and prefer `war_id` when provided. This prevents a coalition/detail click from reopening the wrong war when pair-based resolution is ambiguous.
...
- War status standing must not silently disappear for default-start WAR pairs. When no active `war_instance` exists, `war_status.py` should emit `standing_status="no_instance"` and `standing_status_display` such as "Settlement standing appears once you open settlement on this war." When a `war_instance` exists but no material contribution is accrued, emit `standing_status="no_contribution_yet"`; real rows use `standing_status="available"`.
...
- Thread settlement review route identity far enough that notification/dispatch review clicks can open the ledger settlement section; exact row auto-expand by `route_id` is preferred in F but may be documented as post-F polish if the functional path is otherwise complete.
- Add minimal `incoming_settlement_offer` Godot routing in Slice F. Full AI/ally agency remains Slice G, but `main.gd` must not fall back to a generic "Pending diplomatic matter" line when the backend returns an incoming settlement offer. Minimum F behavior is to open the settlement/proposal review popup with accept/reject/request-revision options or an explicit humanized current-turn offer surface that can promote to `settlement_confirm`.
...
- Add backend tests proving two consecutive probes do not backfill, while executing `propose common peace with Britain` backfills exactly one `war_instance` and stages `settlement_confirm`.
...
- Add presentation tests proving `build_settlement_confirm_dialogue(...)` carries `war_label`, compact/medium `review_sections`, acceptance display fields, warning display fields, awe tags, route metadata, and humanized term/standing labels.
...
- Add stale-state tests proving confirm-time `must_reopen=True` returns a humanized reason and a route back to the same war/nation context without mutation.
...
- Only after Slice F passes may manual Godot smoke begin. Required smoke:
  1. F1/Diplomacy button -> Britain -> Open Settlement -> `settlement_confirm` with war label, acceptance band/top blocker, warnings, terms, allies, and no raw enums -> Ratify Settlement -> required result feedback -> ledger Recent Settlements expanded/expandable.
  ...
  4. Incoming AI settlement offer if feasible -> mailbox/offer route opens a settlement review surface and accept promotes to `settlement_confirm` without mutation; if full generation remains Slice G, document the generation gap but keep the UI route guarded.
