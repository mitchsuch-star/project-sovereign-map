...
- Add settlement endpoint/dialogue contracts: no-terms `GET /diplomatic_preview`, draft-terms `POST /diplomatic_preview`, `settlement_preview`, mandatory hard-stop `settlement_confirm`, typed `POST /respond_to_diplomatic_dialogue` actions for `confirm_settlement`, `back_out_settlement`, and `revise_settlement_terms`, and the exact no-mutation response shapes from the spec.
...
- Pick one settlement dialogue action schema and test it. `settlement_confirm` should use the typed `options[].action` values that Godot popups already emit (`confirm_settlement`, `revise_settlement_terms`, `back_out_settlement`) and the backend response handler must accept those exact ids. Delete the legacy `actions=["confirm", ...]` field from settlement dialogues, or rename it to an explicitly debug-only field that Godot source guards prove is ignored. A live player payload must not expose two contradictory action schemas.
- Add a concrete stale/revise route-back contract. Any `must_reopen=True` response and `revise_settlement_terms` response must carry `error_display` / `message_display` plus `reopen_target = {"surface": "diplomacy_wizard"|"war_detail"|"settlement_review", "nation": str, "war_id": str}`. Silent close or dumping to the nation picker fails the slice.
...
- Pin the cross-war settlement collision rule. Only one settlement hard-stop may own the active popup at a time. An incoming offer for war B while a settlement-confirm for war A is active must queue/defer with a humanized "resolve current settlement first" beat, not preempt silently or overwrite the active dialogue. SUPERSEDED BY SETTLEMENT_UI_CLEANUP_SPEC.md SC-26 - cross-war restaging now rejects with `cross_war_settlement_collision` and is not queued through the mailbox; AI retry behavior is bounded by the cleanup spec if SC-5 is reversed.
...
- Add a read-only settlement-entry probe with an explicit contract:
  `probe_settlement_eligibility(world, *, player_nation, target_nation, war_id=None) -> {"available": bool, "war_id": str | None, "would_backfill": bool, "error": str | None, "disabled_reason_display": str, "debug_reason": str | None}`.
  The probe mirrors `resolve_or_backfill_war_instance_for_settlement(...)` but must not allocate a `war_instance`, attach pairs, open contribution episodes, invalidate caches, stage dialogue, or mutate any settlement/dialogue state. It is not a thin wrapper around `evaluate_open_settlement_eligibility(...)`, because that helper requires an existing `war_id` and treats `war_id=None` / missing instances as failure. Default-start `Britain|France = WAR` with no `war_instance` must show Open Settlement as available with `would_backfill=True`; the actual command click remains the only path that mutates by lazy backfill. Non-WAR pairs return `error="not_at_war"` with a display string such as "You are not at war with Spain." Stale detail/coalition `war_id` values return a humanized changed-state reason, not a raw `inactive_war_instance` string.
...
- Define stale-state and revise behavior. Slice F implements one path: `revise_settlement_terms` returns `reopen_target` for the same nation/war, preserves the previous draft `settlement_terms` in the response or a restageable dialogue payload, and reopens the wizard/review at the selected war context. It must not dump to the nation picker. Confirm-time `must_reopen=True` failures must produce `error_display` and `reopen_target` back to the same nation/war context. A no-op or generic "click Open Settlement again" message fails the slice.
- War-detail surfaces must offer a settlement affordance. `war_detail_popup.gd::show_war()` exposes `Open Settlement` when the row has `war_instance_id` or probe eligibility says `would_backfill=True`. `show_coalition()` exposes `Open Whole-War Settlement` only when visible member rows share one non-empty `war_instance_id`; when they do not, hide the whole-war CTA and show a compact explainer such as "This coalition spans multiple wars; settle each separately." Existing Target/Negotiate buttons remain secondary and mean bilateral/separate-peace intent. Add tooltips: `Negotiate Peace` means bilateral peace with this enemy only, while `Open Settlement` / `Open Whole-War Settlement` means war-wide common peace.
- Add a war-scoped entry contract for detail/coalition launches. New Godot signal/payload should be explicit, e.g. `settlement_clicked(war_id: String, target_nation: String)`, and `main.gd` must connect it. Backend settlement opening should accept either and prefer `war_id` when provided. This prevents a coalition/detail click from reopening the wrong war when pair-based resolution is ambiguous. Sketch/update the button layout before adding CTAs: war detail needs both `Negotiate Peace` and `Open Settlement`; coalition detail needs existing Target buttons plus a whole-war button, so a single non-wrapping `HBox` is likely to overflow in 4+ member coalitions.
...
- Thread settlement review route identity far enough that notification/dispatch review clicks can open the ledger settlement section; exact row auto-expand by `route_id` is preferred in F but may be documented as post-F polish if the functional path is otherwise complete. Even if auto-expand is deferred, the same non-empty `route_id` value must flow through confirmation, event log, dispatch, notification meta, and ledger row metadata in F. Legacy or synthetic settlement events without route ids should receive deterministic fallback ids before they reach Godot. `notification_bar.gd` must widen its review signal or otherwise pass `review_target`, `route_id`, and `war_id` to `main.gd`; a review button that emits only `review_target` cannot satisfy the continuity contract.
...
Gate:
- Add backend tests proving default-start `Britain|France = WAR` exposes enabled `open_settlement` in the wizard with `would_backfill=True`, `dp_cost=0`, no mutation to `world.war_instances`, and no cache invalidation/dialogue staging from preview/probe.
- Add backend tests proving repeated probes do not backfill, while executing `propose common peace with Britain` backfills exactly one `war_instance` and stages `settlement_confirm`. Include a property-style probe test that 100 probes leave `world.war_instances`, `world.war_contribution_scores`, `world.diplomatic_states`, `world.event_log`, `world.next_war_instance_id`, dialogue stack/queue, and cache invalidation counters unchanged.
...
- Add source-level Godot tests proving `diplomacy_wizard.gd` maps `open_settlement`, `proposal_confirm_popup.gd` has a `settlement_confirm` content path, war-detail/coalition detail expose distinct settlement CTAs/signals, `card_clicked` carries `war_instance_id`, row/detail routing preserves `war_instance_id`, and `main.gd` routes `incoming_settlement_offer` from both pending-envoy and mailbox-activation paths. SUPERSEDED BY SETTLEMENT_UI_CLEANUP_SPEC.md SC-5 - incoming-offer Godot routing must be hidden/no-exposure by default unless SC-5 is explicitly reversed.
...
- Add cross-war settlement collision tests: stage `settlement_confirm` on war A, activate or generate `incoming_settlement_offer` for war B, assert the active hard-stop is not overwritten, the offer queues/defers through the documented settlement-owned route, and a humanized "resolve current settlement first" beat is surfaced. SUPERSEDED BY SETTLEMENT_UI_CLEANUP_SPEC.md SC-5 - incoming-offer collision applies only if SC-5 is explicitly reversed. SUPERSEDED BY SETTLEMENT_UI_CLEANUP_SPEC.md SC-26 - outgoing cross-war collision rejects with `cross_war_settlement_collision` rather than queue/defer.
- Add stale-state tests proving confirm-time and incoming-offer promote-time `must_reopen=True` returns a humanized reason and `reopen_target` back to the same war/nation context without mutation. Revise tests must prove prior `settlement_terms` are preserved for restaging.
...
- Add notification review-route tests proving settlement notification metadata carries `review_target`, `route_id`, and `war_id` through `notification_bar.gd` into `main.gd`; exact ledger auto-expand may remain post-F, but the data flow must be present.
...
- Only after Slice F passes may manual Godot smoke begin. Required smoke:
  1. F1/Diplomacy button -> Britain -> Open Settlement at `DP=0` -> `settlement_confirm` with awe banner if applicable, war label, whole/separate badge, covered-enemy chips, side/outcome rows, acceptance band as score/threshold, top blocker, warnings with severity-count overflow, terms, and no raw enums -> Ratify Settlement -> required result feedback -> ledger Recent Settlements expanded/expandable.
