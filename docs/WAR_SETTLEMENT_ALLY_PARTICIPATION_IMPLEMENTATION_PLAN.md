...
> **Status:** v1.15 READY FOR SLICE A1 FOUNDATION GATE - v1.18 mapped-nation A1 simplification applied
> **Last Updated:** April 29, 2026
> **Source spec:** `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.18
...
- Add empty-safe `war_instances_by_leader` and `war_instances_by_participant` lookup/cache helpers plus a single dirty-flag invalidation hook. In A1 these helpers may return empty indexes, but they must establish the API later slices use and must not rebuild more than once per turn phase.
...
- Implement formula edge cases exactly: sum `leader_own_losses` sub-components before clamping to `[-25, 5]`; `direct_score >= 20` adds `0` burdened-participant penalty unless another burden rule applies; `war_objective_alignment` includes the `-15` unrelated-harsh-terms-dominates column for all five WPS objective types; and `war_exhaustion` intentionally uses the existing per-nation cumulative `enemy_leader_war_exhaustion // 3` rather than per-war partitioning or `round()`.
