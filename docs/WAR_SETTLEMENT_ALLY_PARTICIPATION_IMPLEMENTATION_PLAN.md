...
> **Status:** v1.20 READY FOR SLICE A1 FOUNDATION GATE - v1.23 review sharpening applied
> **Last Updated:** April 30, 2026
> **Source spec:** `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.23
...
- Before coding A2, introduce a small `CascadeContext` dataclass or equivalent transaction object if threading `war_id` would otherwise push `_process_war_cascade(...)` into a ten-argument positional signature. The context should carry `war_id`, `root_episode_id`, `war_entry_entries`, `ally_entry_decisions`, and any existing cascade transaction metadata that already travels together.
...
- Add deterministic Slice C tuning gate fixtures before locking constants: Pressburg-style accepting-leader losses, Tilsit-style non-leader burden, coalition split, decisive French win without total victory, total-victory harsh terms, minor-power limited common peace, mixed-strength partial-vs-full coverage, narrow-vs-full-vs-serial settlement incentive comparison, a heavily tilted 6+ participant coalition war, Britain-led defense using current mapped holdings/home-region scoring, mapped-home/capital cession/restoration cases, a live-code Britain current-map case where `NATION_CAPITALS["Britain"] == "Netherlands"` is treated as configured scenario data rather than a separate settlement identity, a full-Europe harshness calibration fixture for plausible 5-8 region packages, a multi-forced-alliance package that projects `+30` or more threat and names crossed coalition thresholds, and an AI-defender package where the only WPS objective is `defense` and `war_objective_alignment <= +5`. Pin the Pressburg-style worked example from the spec as one fixture seed.
...
- Manual comprehension gate before D1: run a complete common-peace settlement review with at least 4 participants using a reviewer who has not read the settlement spec. Within roughly 30 seconds, the spec-naive reviewer must be able to identify the top blocker or political cost, the main affected ally/enemy, and the available next action. If not, revise payload density, copy, default ordering, or sectioning before D1 reaction work starts.
