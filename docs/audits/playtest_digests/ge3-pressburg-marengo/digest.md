# Playtest digest — ge3-pressburg-marengo
...
- played: board `The Third Coalition, 1805` · map `europe` (126 provinces) · France from turn 28 · campaign seed `historical` · dice `marengo`
...
- ⚠ WARNING — two seeds: the world plays campaign seed `historical` (the save's); the module dice follow `marengo`. World jitter and tiebreaks follow the first and every die the driver reseeds follows the second — a hybrid, a replay of neither.
