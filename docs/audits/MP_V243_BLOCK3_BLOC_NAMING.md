...
>
> **April 22 re-deferral note:** the original decision list below ("Defer member badges / ledger-row bloc stamps") is now the live contract again. Badges were briefly promoted into v2.4.3 scope on April 21 (as Slice E-Cards) and then re-deferred on April 22 after a second audit pass established the concentrated four-surface naming layer (headline, threshold beats, warnings, declaration contrast) carries the mechanic on its own and that adding row stamps spreads the naming layer across the whole Nations tab in a way that weakens the headline's dramatic ownership and blurs "hegemon bloc" vs "formal coalition." The live owner docs as of April 22 reflect this: `COMMITMENTS_PRESENTATION_SPEC.md` §8.1a.4 marks stamps DEFERRED, `RELIABILITY_COMMITMENTS_SPEC.md` §11.1 routes stamp scope to Slice D3, `RELIABILITY_IMPLEMENTATION_PLAN.md` replaces Slice E-Cards with Slice D3 under Slice D Deferred follow-up, `DIPLOMAT_VOICE_BIBLE.md` scope note reads stamps as deferred, and `STATUS.md` top entry documents the re-deferral. This audit doc remains superseded for the bloc-naming contract itself (D1-D10 folded into CPS §8.1a); the badge-deferral line in the decision list below is aligned with the live contract again.
>
