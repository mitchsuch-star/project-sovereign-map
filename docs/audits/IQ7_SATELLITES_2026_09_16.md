...
- **AMBIENT (historical):** Switzerland's turn-6 relief petition is refused by the unattended policy and Switzerland **defects to Britain at turn 15** (VS-6) instead of rebelling at turn 22 as on HEAD; Holland stays; France ends on **5** provinces — the same 5 as HEAD's ambient arm, so the passive-France guard (±1) holds.
  - **⚠ Re-read by the review round (September 18, 2026), FOR USER CONFIRMATION.** The turn-40 reading on one seed cannot see a guard failure on another, so the round re-ran the unattended arm on seven seeds, shipped levers against the four `vassal` levers down (`scratchpad/iq7/review/probes/r1_ambient_run*.py`; the shipped historical run reproduces the archived `iq7-ambient` LEDGER rows byte for byte). **The contract's ±1 guard is BREACHED on eylau: France ends turn 40 on 3 provinces shipped against 6 levers-down** (t38 5/6, t39 3/6, t40 3/6 — Archduke John takes Ile-de-France and Ardennes and Archduke Charles Normandy on t38–39 only on the shipped arm; deterministic on a re-run). On historical the shipped arm is 1–2 provinces behind from t23 to t28 (13/15, 10/12, 7/9, 6/7) before both reach 5; austerlitz 8/8, marengo 0/0, ulm / jena / wagram identical on every turn. The shipped arm is never AHEAD on provinces and behind on two of seven seeds. The contract's own rule is that a worse figure blocks landing pending the user's ruling; the row landed on the three-seed reading and the eylau breach is put to the user here rather than the band widened silently. **The directional effect the three-seed reading missed:** an unattended refused or lapsed petition breaks Switzerland 3–12 turns EARLIER on every seed (shipped vs down: historical 15 vs 21, austerlitz 17 vs 27, marengo 19 vs 22, ulm 20 vs 32, jena 19 vs 27, wagram 18 vs 28, eylau 18 vs 26; mean ≈ 8) — the same ~10-turn acceleration the contract cited when it declined IQ7-D2, now measured on the shipped design. WHO takes the satellite (Britain, Russia, Austria, or an independent rebellion) is seed-dependent in both arms (shipped → a great power on 2 of 7, down on 1 of 7; ulm reverses it) and is not a new outcome class — VS-6 defection to a great power predates IQ-7. Attribution: the petitions-only arm (riders down) reproduces the shipped province series exactly; the riders-only arm reproduces the down series.
