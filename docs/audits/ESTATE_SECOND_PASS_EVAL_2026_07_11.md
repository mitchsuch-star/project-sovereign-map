...
**F3 (initially ROUTED; ✅ FIXED same day at the user's direction) — battles_won seams UNIFIED.** The solo combat path counted only DECISIVE outcomes toward `battles_won`; the coordination path has always counted tactical victories too (`combat_executor.py:3629`) — same battle, different bookkeeping depending on whether allies marched, and a starved Cost-of-Success on-ramp in defender-grind wars (8 live turns vs Mack/John = zero French expectation). Fix: the solo tactical arms in `combat.py` now increment `battles_won`/`battles_lost` identically to the coordination sets; stalemate/mutual-destruction unchanged. Tuning note: expectation accrues faster — E5's "caps ~turn 15–20" guidance should be re-measured at the next band check (`REP_STEP` stays in-band tunable). The "copy disagrees with classification" sub-claim dissolved on inspection: the classifier reads side-total casualties while the one-liner shows the primary pair — coherent. Pins: `TestUnifiedWinSemantics` (seed-scanned, both arms + stalemate).

**F4 (initially ROUTED; ✅ FIXED same day at the user's direction) — the guessed-target guard.** Root cause was live-LLM target substitution (the BUG-CA-3 family), not region redirection: the mock parser correctly refuses "Venetia"; the anthropic tier substituted Archduke John/Tyrol for the unknown name and the executor faithfully attacked. Fix: the raw text rides specific command dicts, and `_execute_attack` refuses — before auto-war-declaration — when the raw text names neither the parsed target nor anything the resolution produced (enemy name/location/nation, resolved region; raw or humanized), listing the fog-legal visible enemies instead. AI/strategic/muster-reissue paths carry no raw text and bypass. Pins: `TestGuessedTargetGuard` (4).
