...
**Deferred (surfaced to user, GR9):** move-substitution NOTE (fuzzy-match is usually a *helpful* correction → high false-note risk; move is reversible + already names destination) and orphan-scene file deletion (`coalition_declaration_popup`/`proposal_result_popup` `.tscn/.gd` verified referenced only in comments — destructive, cosmetic-only). **Refuted by the sweep:** sticky-interrupt keyword hijack (popup is modal, blocks input) and the war-purpose-on-attack wall (France boots AT WAR with Austria/Britain/Russia, so it's never reached for early Austrians).
