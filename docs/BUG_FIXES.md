...
>
> **The headline pair, both hand-reproduced on the shipped mock-default build and
> both live in `--llm anthropic` because the fast parser never escalates:**
> `Davout, attack next turn` ENDS THE TURN (FA-6) and `Ney, delay the attack`
> FIGHTS NOW (FA-7). **The single highest-leverage row is FA-26** — the ES-7
