...
#### Files
1. `diplomatic_templates.py` — `_build_base_terms()` (jitter on demands AND sweeteners per AM-25.2, desire profile bias, `deterministic` param per AM-25.3, signature extension for diplomat param). **AM-25.4:** Grep for `_build_base_terms(` before implementing — verify only one call-site in `generate_suggested_terms`. Use `**kwargs` for new params to avoid breaking undocumented callers. `generate_suggested_terms()` (situational line via event API). **AM-25.5:** All event/battle_history checks must use safe access (`if world.battle_history and ...`). Default fallthrough to `TALLEYRAND_COMMENTARY[target_nation]`.
