...
| ⛔ **NPC-2** | **P1/P2** ‡ | **Re-ordering a marshal who holds an order-bound interrupt leaves the stale question armed.** Measured: after `Ney, march to Frankfurt` replaces the Swabia order, `ney.strategic_order.target == 'Frankfurt'` while `ney.pending_interrupt` still holds `{contact_bad_odds, enemy: Mack, location: Swabia}` — which also freezes the new order at step 0a, and is the ammunition NPC-1 needs. **Root cause corrected by the refutation pass:** it is not "implemented at 1 of ~38 seams" — TUT-F4a's clear IS written, and is **unreachable for strategic orders** because `executor.py:873` excludes `is_strategic_command` before it can run | single-source it: a `Marshal.clear_strategic_order()` (or WorldState helper) nulling the order, `holding_position`/`hold_region` and the `ORDER_BOUND_INTERRUPT_TYPES` interrupt together — and make it reachable from the strategic path |
| ⛔ **NPC-3** | **P1** | **`Ney, attack Archduke John` silently attacks Archduke Charles** (with John fogged). The guessed-target guard's substring grounding (`combat_executor.py:134-136 _named_in_raw`) is disarmed by the shared title — "Archduke" matches both — and, per both refuters, the substitution originates one layer up in **the mock parser's world-blind legacy bare-`archduke` alias** in `llm_client.py`. The muster header names Charles, the battle is fought against Charles, and the word "John" never appears | retire/scope the bare-title alias, and ground `_named_in_raw` on the uniqueness rule `llm_client.unique_name_tokens` already implements |
...
convention. **The pursuit rows were judged last, and they are the reason this note
changed twice:** ~~NPC-4~~ was KILLED (2/2), ~~NPC-22~~ was KILLED (2/2),
NPC-5 was CONFIRMED P1 (2/2), and NPC-16 was confirmed with its severity
split into evidence-P1 / player-P3. **Final tally: 39 claims judged, FOUR
killed** — and two of the four were this session's own headline claims, which
is the argument for running refuters against your own conclusions and not
