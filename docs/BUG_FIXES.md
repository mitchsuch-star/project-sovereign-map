...
0. **Precondition:** If `demands` is empty (only sweeteners), skip pen nudge entirely — proposal is already generous, show normal confirm dialogue
1. Find the demand with the highest harshness contribution (ties broken by order: territory > gold > manpower > AP)
2. If numeric (gold_per_turn, gold_lump, manpower_*): reduce value by 20% (round to int). If reduction yields value ≤ 0, remove the demand entirely
3. If territory with populated `regions` list: remove the last-added region. If territory with only `value` shape (no regions list, from modify_harsh): reduce value by 1; if value reaches 0, remove the demand entirely
4. If only AP demands remain (no gold/territory/manpower): leave AP unchanged, add one sweetener (100 gold/turn to target, or 25 gold/turn if France income < 500). AP is exempt from rule 2 reduction
5. Recalculate harshness and re-run `_validate_economic_feasibility()` on the modified terms. Show the updated term summary so the player sees what changed

**Scope note:** Pen nudge applies at the confirm dialogue step, not during multi-step ultimatum wizard steps. Once a demand is approved in the ultimatum flow, it is subject to pen nudge only at the final confirm. Pen nudge modifies only ONE demand (the harshest) — no cascading. One pen nudge per proposal (no re-evaluation after [Accept]/[Insist]).
...
**AM-23.1 — Empty demands guard:** `modify_generous` sets `demands = []` (line 1510). If demands are empty, `roll_drafting_pushback()` returns `False` immediately — no roll, no pushback. Pen nudge cannot operate on an empty demands list, so firing the roll produces a broken UX (pushback dialogue with [Accept his version] showing identical terms). Talleyrand's pushback is about softening demands; with no demands there is nothing to soften.

**Note:** The authority < 50 "second-guessing generous terms" scenario is deferred. If a future design wants Talleyrand to *add* demands to generous proposals (e.g. "Sire, we could ask for *something*"), that's a separate R-item with its own pen nudge rules, not PL-23 scope.
...
Implementation: set `context["objection_resolved"] = True` on the dialogue context when PL-23 fires (both [Accept] and [Insist] paths — the roll fired either way). In `execute_proposal` (diplomatic_executor.py:1005-1017), check `dialogue.get("context", {}).get("objection_resolved")` BEFORE the `calculate_diplomatic_defiance_chance` call and skip the entire defiance block if truthy.

##### DP timing fix

DP is deducted only when the proposal actually departs — at the end of `execute_proposal` after all conversation is resolved, not before. If the player cancels at any point (including after PL-23 pushback), no DP is spent. Move the `world.diplomatic_points -= cost` block (currently at diplomatic_executor.py:1003, BEFORE the defiance check at line 1005) to after the defiance check, just before setting Talleyrand in transit (~line 1070). If §3a sabotage fires and replaces the proposal, DP is still spent (the proposal departed, just modified).
...
**What gets removed:**
- `trust` field on `DiplomaticRepresentative` (diplomat.py) — remove from `__init__`, `to_dict`, `from_dict`
...
- Trust reads/writes in confrontation handler (diplomatic_defiance.py:443 area — `talleyrand.trust -= 10`)
- Trust reads/writes in overlook handler (diplomatic_defiance.py — `talleyrand.trust += 3`, replace with authority -3)
- Any UI display of Talleyrand trust (diplomatic ledger Talleyrand tab)
- **Talleyrand redemption event entirely** (see below)

**Serialization note:** Removing `trust` from `DiplomaticRepresentative.to_dict()`/`from_dict()` means old saves will have a stale `trust` key. `from_dict()` should silently ignore it (use `.get()` pattern, don't error on unknown keys). Run `test_serialization_enforcement.py` after.
...
When PL-23's roll fires and the player picks [Cancel], the dialogue must return to the **same confirm step with the same context**.

**AM-23.2 — Pushback fires BEFORE modify_count increment.** The roll happens at the top of `modify_harsh`/`modify_generous`, before `modify_count` is incremented. If pushback fires, the pushback dialogue carries the *current* (un-incremented) modify_count. This means:
- **[Cancel]:** Returns to confirm dialogue with modify_count unchanged. The player is back exactly where they were — no modification round consumed. Prevents the exploit of losing a modify round to pushback.
- **[Accept his version]:** Handler increments modify_count by 1 (the modification happened, Talleyrand just altered it), then builds return dialogue.
- **[Insist on original]:** Handler increments modify_count by 1 (the modification happened as requested), then builds return dialogue with authority -3.

Do NOT pop the dialogue back to wizard start — that would reset modify_count and let the player re-roll indefinitely.

**Implementation detail:** `dialogue_manager.replace()` overwrites `self._current` — there is no stack or history. The old dialogue is destroyed. Do NOT try to stash and restore the pre-pushback dialogue. Instead, **reconstruct** the confirm dialogue from context fields (terms, modify_count, proposal_type, target_nation) which are already carried in `context`. [Cancel] handler builds a fresh confirm dialogue from `context` using the same path as modify_harsh/modify_generous's normal (non-pushback) return, with `context["modify_count"]` preserved and terms unchanged (since modify_count was never incremented, buttons remain available).
...
| S8 | [Insist] authority -3 can cross tier boundaries (e.g. 42→39 crosses <40 tier, giving Talleyrand +15% base) | Low | By design — insisting when authority is marginal should feel risky. Implementers: do NOT add additional penalties at tier boundaries; the curve's natural step is sufficient |
...
| S9 | Pen nudge on territory demand with value-shape (no regions list, from modify_harsh) — rule 3 "remove last region" silently no-ops | Medium | Fixed in pen nudge rules above: reduce value by 1 instead; remove demand if value reaches 0 |
| S10 | Pen nudge gold reduction can violate `_validate_economic_feasibility()` floor (e.g. 30→24, below floor of 25) | Medium | Fixed: rule 5 now re-runs feasibility post-nudge |
| S11 | Empty demands list (all-sweetener proposal from multiple "More generous" clicks) crashes pen nudge search loop | Medium | Fixed: rule 0 precondition skips pen nudge if demands=[] |
| S12 | Two harshness functions (`calculate_proposal_harshness` vs `calculate_treaty_harshness`) evaluate identical proposals differently | Low | Documented in PL-24. Sweetener weighting asymmetry deferred to design refinement |

##### Creative audit findings (Apr 9, 2026)

16 findings from adversarial edge-case audit. 3 Medium (must resolve before implementation), 13 Low/None (implementation-time checks or documentation).

| # | Bug | Finding | Severity | Resolution |
|---|-----|---------|----------|------------|
| AM-23.1 | PL-23 | **Pushback on `modify_generous` with empty demands — pen nudge no-ops.** `modify_generous` sets `demands = []` (line 1510). If the roll fires, rule 0 skips pen nudge (empty demands). What shows? A pushback dialogue with [Accept his version] offering identical terms to the original — no visible change. Player confusion. | **Medium** | **Skip the roll entirely when demands are empty after modification.** Add guard in `roll_drafting_pushback()`: if `demands` is empty, return `False` (no pushback). Rationale: Talleyrand's pushback is about softening demands — with no demands there is nothing to soften. The authority < 50 "second-guessing generous terms" scenario from §Generous Direction is interesting in theory but produces a broken UX when pen nudge can't act. If a future design wants Talleyrand to add demands to generous proposals, that's a separate R-item with its own pen nudge rules. |
| AM-23.2 | PL-23 | **[Cancel] returns with incremented `modify_count`, costing the player a modification round.** `modify_harsh` increments `modify_count` at line 1132-1133 *before* pushback fires. [Cancel] preserves this incremented count (per spec). Result: player at modify_count=1 triggers pushback, clicks [Cancel] to keep their terms, but now modify_count=2 and the "Even harsher" button is gone (war/coercive cap=2). The pushback consumed a modify round the player didn't use. | **Medium** | **Roll pushback BEFORE incrementing `modify_count`.** Move the `roll_drafting_pushback()` call to the top of `modify_harsh`/`modify_generous`, before the `modify_count = context.get("modify_count", 0) + 1` line. If the roll fires, build the pushback dialogue with the *current* (un-incremented) modify_count in context. If [Accept] or [Insist], increment modify_count in their handlers before building the return dialogue. If [Cancel], modify_count stays unchanged — the player returns to exactly where they were. Same fix for `modify_generous` and `_build_ultimatum_confirm_step`. |
| AM-23.3 | PL-23 | **`"insist"` keyword missing from `_DIALOGUE_ONLY_KEYWORDS`.** Spec adds `"insist"` to `_DIALOGUE_RESPONSE_KEYWORDS` (line ~617) but doesn't mention adding to `_DIALOGUE_ONLY_KEYWORDS` (line ~653). Typing "insist" with no active dialogue would fall through to the executor, likely returning "Unknown action." | Low | Add `"insist"` to `_DIALOGUE_ONLY_KEYWORDS` alongside `"accept"`, `"reject"`, etc. Also add the three new actions: `"accept_nudge"`, `"insist_original"`, `"cancel_pushback"` (as option label substrings like `"nudge"`, `"insist"` cover these). |
| AM-23.4 | PL-23 | **`objection_resolved` context flag must survive save/load.** If pushback fires and the player saves/loads before sending, the flag must persist. Flag lives in dialogue context dict, serialized through `DialogueManager.to_dict()`. | Low | R12 guarantees dialogue context serialization. Verify in test: create pushback dialogue with `objection_resolved=True`, round-trip through `to_dict()`/`from_dict()`, confirm flag persists. |
| AM-23.5 | PL-23 | **Trust removal: `trust` parameter must be removed from `__init__` signature, not just the field.** If only the field is removed but the `__init__` param stays with a default, stale callers silently set a dead attribute. | Low | Remove `trust` from `__init__` params (line 25), the `self.trust = trust` assignment (line 32), and all `trust=` keyword args in `STARTING_DIPLOMATS` (lines 68, 76, 84, 92, 100) and `create_starting_diplomats` (line 115). Any test constructing `DiplomaticRepresentative(trust=X)` will fail — intended, forces cleanup. `from_dict()` should silently ignore `trust` key via `.get()` pattern (don't pass to constructor). |
| AM-23.6 | PL-23 | **Redemption removal blast radius larger than spec documents.** Missing from file list: `cooldown_manager.py` (lines 141, 153 — remove from `PRIORITY_ORDER` and `RESPONSE_KEYS`), `dialogue_manager.py` (line 34 — remove from priority dict), `world_state.py` (lines 638-643 — remove `talleyrand_redemption_popup` property + setter; line 3165 — remove from `to_dict`; line 3417 — remove from `from_dict`), `turn_manager.py` (line 276 — remove from `popup_fields` list), `talleyrand_redemption_popup.tscn` (scene file — delete alongside `.gd`), `main.gd` (lines 77, 201-203, 736-738, 2814-2819 — variable declaration, registration, signal connection, response check, handler function). | Low | Add all 6 missing files to the file list. Full removal list is now 18 files, not 12. |
| AM-23.7 | PL-23 | **Pushback timing: modify_count increment on pushback [Accept]/[Insist].** Per AM-23.2, if the roll fires before modify_count increment, then [Accept] and [Insist] handlers must increment modify_count themselves before building the return dialogue. Otherwise the player gets an extra modify round. | Low | Both `accept_nudge` and `insist_original` handlers: `context["modify_count"] = context.get("modify_count", 0) + 1` before building the confirm dialogue. Follows naturally from AM-23.2. |
| AM-23.8 | PL-23 | **DP timing + mutual exclusion interaction is clean.** After DP moves past the defiance check, and `objection_resolved` skips §3a entirely, the DP deduction happens unconditionally before `talleyrand_state = "IN_TRANSIT"`. No gap. | None | No change needed. Documented for completeness. |
| AM-23.9 | PL-23 | **`"cancel"` keyword routes to `cancel_mission` / `dismiss`, NOT `cancel_pushback`.** The action_map in `handle_diplomatic_dialogue_response` (diplomatic_executor.py:872) maps `"cancel"` → `["cancel_mission", "dismiss"]`. If the player types "cancel" during a pushback_confirm dialogue, the keyword matcher tries `cancel_mission` (no such option in pushback), then `dismiss` (also absent). Falls through to "I don't understand" error. The Godot popup uses integer indices (not keywords) so this only affects terminal/text input. | **Medium** | Add `cancel_pushback` to the `"cancel"` keyword list: `"cancel": ["cancel_pushback", "cancel_mission", "dismiss"]`. The matcher tries each in order and selects the first that matches an available option — so `cancel_pushback` is tried first, and only falls through to `cancel_mission`/`dismiss` for non-pushback dialogues. Safe. |
| AM-23.10 | PL-23 | **`_get_talleyrand_trust_label()` used as `talleyrand_state` response field in 3 places.** `main.py:172, 497, 828` set `"talleyrand_state": _get_talleyrand_trust_label(world)`. After trust removal, this function must be replaced — but with what? The field name says "state" but sends a trust label. Godot doesn't read it (no `.talleyrand_state` references in Godot scripts). | Low | Replace `_get_talleyrand_trust_label()` with an authority-based label function (e.g. `_get_talleyrand_authority_label(w)` returning "Obedient"/"Independent"/"Defiant" based on authority tiers). Or remove the field entirely since Godot ignores it. Implementer's choice — either way, remove the trust-based function. |
| AM-23.11 | PL-23 | **`set_talleyrand_trust` cheat command in meta_executor.py:1993-2002.** Directly sets `talleyrand.trust`. Must be removed or converted to `set_authority` (which already exists). | Low | Remove the `set_talleyrand_trust` cheat block entirely (lines 1993-2002). Authority cheat already exists. Add `meta_executor.py` to the files list. |
| AM-23.12 | PL-23 | **`diplomatic_ledger.py:527, 672` — trust read AND serialization.** Lines 527-538 read `talleyrand.trust` and compute `trust_label`. Line 672 serializes `"trust": trust` in the API response. Both must change. Spec says "lines 527-538, replace with authority" but doesn't mention line 672. | Low | After trust removal: replace lines 527-538 with authority-based label (using `world.authority_tracker.authority`). Remove `"trust"` and `"trust_label"` keys from the return dict at line 672-673. |
| AM-23.13 | PL-23 | **`pushback_confirm` dtype falls to generic `_build_content()` in proposal_confirm_popup.gd.** The popup's match statement (lines 22-41) has no case for `"pushback_confirm"`. Falls to default `_build_content()` which shows terms + harshness + acceptance — functional but doesn't show "before vs after" comparison of original vs nudged terms.| Low | Acceptable for initial implementation — generic content renderer shows the nudged terms clearly. Optionally add `"pushback_confirm"` case with `_build_pushback_content()` that shows original → nudged diff. Not blocking. |
| AM-23.14 | PL-23 | **Keyword action_map needs entries for new actions.** `accept_nudge`, `insist_original`, `cancel_pushback` have no keyword mappings. Terminal users can't trigger them by typing. Godot uses integer indices, so Godot users are unaffected. | Low | Add to action_map: `"nudge": ["accept_nudge"]`, `"insist": ["insist_original"]`. `cancel_pushback` covered by AM-23.9 above. |
| AM-23.15 | PL-23 | **`roll_drafting_pushback()` has no guard against re-rolling after first pushback.** Spec says "One pen nudge per proposal" and sets `objection_resolved=True` on both [Accept] and [Insist] paths, but this flag is only checked at `execute_proposal` (§3a skip). If the player [Accepts] nudged terms then clicks "Harsher terms" again (modify rounds permitting), `roll_drafting_pushback()` would roll again — violating the one-per-proposal rule. | **Medium** | Add `if context.get("objection_resolved"): return False` at the top of `roll_drafting_pushback()`, before the empty-demands guard. One line. |

##### Second-pass audit findings for PL-24 (Apr 9, 2026)

| # | Bug | Finding | Severity | Resolution |
|---|-----|---------|----------|------------|
| AM-24.4 | PL-24 | **`manpower_cavalry`, `manpower_artillery` never auto-generated but player wizard CAN create them.** Ultimatum wizard f-string at diplomatic_executor.py:1433 creates `manpower_{unit_type}` demands. `DEMAND_VALUES` defines them. `calculate_treaty_harshness` scores them. `calculate_proposal_harshness` does NOT. PL-24 spec already notes this — confirming it's complete. | None | Already in PL-24 spec. No additional action. |
| AM-24.5 | PL-24 | **`infantry_manpower` naming variant (diplomatic_templates.py:1590) produces sweeteners, not demands.** This is a sweetener type created in `_build_base_terms`. `calculate_proposal_harshness` only iterates `proposal.get("demands", [])` — sweeteners are scored separately as flat -0.1. So the naming inconsistency doesn't affect demand harshness scoring, only sweetener identification. PL-24 rename still needed for consistency. | Low | PL-24 rename is correct (line 1590 `"infantry_manpower"` → `"manpower_infantry"`). Sweetener harshness is flat -0.1 regardless of type, so the name mismatch has no scoring impact. But the canonical name matters for PL-25's jitter and desire-profile bias which need to match sweetener types. |

##### Creative audit findings for PL-24 (Apr 9, 2026)

| # | Bug | Finding | Severity | Resolution |
|---|-----|---------|----------|------------|
| AM-24.1 | PL-24 | **`gold_lump` demands not capped by `_validate_economic_feasibility()`.** Feasibility caps `gold_per_turn` demands (line 1656) but not `gold_lump`. A wizard-generated gold_lump of 10000 passes uncapped. PL-23 rule 5 re-runs feasibility post-nudge, but feasibility has nothing to enforce on gold_lump demands — re-run is a no-op. | Low | Out of PL-24 scope. Add a DR item: "Cap gold_lump demands in `_validate_economic_feasibility()` to e.g. `target_gold * 0.5`." PL-23 rule 5 re-run is still correct (it handles gold_per_turn nudges). Document the gap. |
| AM-24.2 | PL-24 | **`unit_trade` dead code — decision needed.** Spec says "remove or keep as forward-compatible stub." | Trivial | Remove from formula. Dead code that scores a phantom type confuses future implementers. If `unit_trade` is ever implemented, it gets its own harshness rule at that time. |
| AM-24.3 | PL-24 | **Harshness indistinguishable above 0.7.** `min(1.0)` clamp means a proposal with harshness 0.71 and one with harshness 2.5 both read as the same to PL-23's probability curve (15% base). Pen nudge only reduces the harshest demand by 20% — insufficient to bring a 2.5 proposal below 0.7. | Low | Acceptable for PL-24. The pen nudge is a light touch, not a hard correction — Talleyrand isn't trying to fix an absurd proposal, he's expressing mild pushback. If harshness is extreme (>1.5 pre-clamp), the player will [Insist] or the terms will still be rejected by the target nation's acceptance formula. No change needed. |

##### Creative audit findings for PL-25 (Apr 9, 2026)

| # | Bug | Finding | Severity | Resolution |
|---|-----|---------|----------|------------|
| AM-25.1 | PL-25 | **Schemer pen nudge swap can create duplicate demand types.** Spec says schemer swaps demand types (territory → gold). If the proposal already has a `gold_per_turn` demand and schemer swaps a territory demand to gold, there are now two `gold_per_turn` demands. Acceptance formula may double-count. | **Medium** | **Merge on collision, fallback on failure.** If swapping territory → gold_per_turn and a gold_per_turn demand already exists: merge (add the swap value to existing demand, remove the territory demand). If ALL swap targets already exist (gold + AP + manpower all present), fall back to loyalist-style 20% reduction on the territory demand. Implementation: try swap targets in order (gold_per_turn → gold_lump → ap_per_turn), merge if collision, reduce if all occupied. |
| AM-25.2 | PL-25 | **Jitter should apply to sweetener values too.** Spec says jitter on "gold/manpower demand values" but sweeteners from `_build_base_terms()` are equally formulaic. Talleyrand offers identical sweetener gold every time. | Low | Apply `random.uniform(0.8, 1.2)` to sweetener `gold_per_turn` and `gold_lump` values too. Same deterministic override (1.0) for testing. |
| AM-25.3 | PL-25 | **Deterministic mode detection for jitter.** `_build_base_terms()` doesn't check `LLM_MODE`. Need a mechanism to disable jitter in tests. | Low | Accept optional `deterministic=False` param on `_build_base_terms()`. When `True`, use multiplier 1.0. Call-site in `generate_suggested_terms` passes `deterministic=False` by default. Tests pass `deterministic=True`. Avoids env var coupling. |
| AM-25.4 | PL-25 | **`_build_base_terms` call-site audit.** Spec says "one call-site update in `generate_suggested_terms`." May have test callers. | Low | Grep for `_build_base_terms(` before implementing. Use `**kwargs` to pass new params without breaking any undocumented callers. |
| AM-25.5 | PL-25 | **Empty event list for situational flavor (turn 1).** `world.get_latest_events(n)` returns `[]` on turn 1. `world.battle_history` is `[]`. Flavor condition checks must not crash on empty lists or missing keys. | Low | All 4-6 condition checks must use safe access: `if world.battle_history and ...`. Default fallthrough to `TALLEYRAND_COMMENTARY[target_nation]` entry. Add test: call `generate_suggested_terms` on turn 1 with empty event/battle history — verify no crash, verify default flavor line appears. |
| AM-25.6 | PL-25 | **`battle_history` is a zombie field — never populated.** `world_state.py:285` initializes `battle_history: List[Dict] = []`, serializes/deserializes it, but **no code ever appends to it**. PL-25 spec §4 says to read `world.battle_history` for "recent battle victory over target" flavor. This will always be empty. | **Medium** | **Use `world.get_events_by_type("battle")` instead of `world.battle_history`.** The event log (`world.event_log`) IS populated by combat resolution via `world.log_event()`. Battle events have `type="battle"` or similar. Grep for `log_event.*battle` to find the exact event type string. Alternatively, use `world.get_events_since_turn(world.current_turn - 3)` and filter for combat events. Do NOT rely on `battle_history` — it's dead. |
| AM-25.7 | PL-25 | **Hawk and dove pen nudge behavior undefined.** PL-25 spec only defines schemer (swap) and loyalist (20% reduction). But `DIPLOMAT_PERSONALITIES` includes hawk and dove. France starts with schemer, and can be replaced with loyalist (via redemption — but redemption is being CUT). After PL-23, the only personality change path is gone. Are hawk/dove ever assigned to France's diplomat? | Low | **No action needed.** France always starts as schemer. The only personality change mechanism (redemption → loyalist) is being cut by PL-23. No other code path changes France's diplomat personality. Hawk/dove are AI-only diplomat personalities. If a future design allows diplomat personality changes (R-item "Replace Talleyrand"), hawk/dove pen nudge behavior should be defined at that time. For PL-25, only schemer and loyalist cases matter. |
| AM-25.8 | PL-25 | **`_build_base_terms` confirmed single call-site.** Grep found exactly one caller: `generate_suggested_terms` at line 1403. No test files call it directly. Signature extension is safe. | None | Confirmed. `**kwargs` approach not needed — direct param addition is fine. |
| AM-25.9 | PL-25 | **Desire profile bias (§3) lacks concrete formulas.** Spec says Talleyrand "biases sweetener selection toward protection offers" for Saxony and "territory demands get higher harshness weight" for Prussia — design directions, not implementable formulas. An implementer must decide specific weights. | Low | Add concrete multipliers: if `values_territory == "high"` for target, multiply territory demand harshness contribution by 1.5 in Talleyrand's pen-nudge target selection (makes territory the nudge target even if gold is numerically harsher). If `weakness` matches a demand category (e.g. `"isolation"` → diplomatic demands, `"overextension"` → territory), that demand becomes the pen nudge target regardless of harshness rank. Sweetener bias: if `diplomatic_lever == "trade"`, prefer gold sweeteners; if `"stability"`, prefer non-aggression sweeteners. |

##### Files requiring changes

1. `diplomatic_defiance.py` — Replace `evaluate_pre_proposal_objection()` with `roll_drafting_pushback()` (authority-only probability curve). **AM-23.15:** Guard — return `False` immediately if `context.get("objection_resolved")` (one pushback per proposal). **AM-23.1:** Guard — return `False` immediately if demands list is empty. Add `apply_pen_nudge()` (deterministic term softening). Remove trust modifiers from `calculate_diplomatic_defiance_chance()` (lines 69-77, 115-123). Update confrontation handler (line ~440 area: remove `talleyrand.trust -= 10`). Update overlook handler (remove `talleyrand.trust += 3`, replace with `authority -3`). Remove redemption functions entirely (`check_talleyrand_redemption` lines 474-498, `build_redemption_dialogue` lines 501-550, `apply_redemption_choice` lines 553-615). Keep `calculate_proposal_harshness()` as-is (fix territory bug separately in PL-24).
2. `diplomatic_executor.py` — **AM-23.2:** Call `roll_drafting_pushback()` BEFORE incrementing `modify_count` in `modify_harsh` (line 1079), `modify_generous` (line 1466), and `_build_ultimatum_confirm_step`. On fire: build `"pushback_confirm"` dtype dialogue with options `[accept_nudge / insist_original / cancel_pushback]`, carrying `original_terms`, `nudged_terms`, and current (un-incremented) `modify_count` in context. **AM-23.7:** `accept_nudge` and `insist_original` handlers increment `modify_count` before building the return dialogue. On insist: authority -3, set `objection_resolved` flag. [Cancel]: reconstruct pre-pushback confirm from context using same path as modify_harsh/generous normal return, with `modify_count` unchanged. Add three new elif blocks in `_process_dialogue_choice` for the three new actions. **AM-23.9:** Add `cancel_pushback` to `"cancel"` keyword list in action_map (line 872): `"cancel": ["cancel_pushback", "cancel_mission", "dismiss"]`. **AM-23.14:** Add keyword mappings: `"nudge": ["accept_nudge"]`, `"insist": ["insist_original"]`. Move DP deduction in `execute_proposal` to after defiance check (~line 1070, just before `talleyrand_state = "IN_TRANSIT"`). Check `objection_resolved` to skip §3a defiance. Remove redemption action handlers (`redemption_apologize`, `redemption_replace`, `redemption_continue`).
...
4. `diplomat.py` — **AM-23.5:** Remove `trust` parameter from `__init__` signature entirely (not just the field). Remove from `to_dict` (line 41). `from_dict` (line 52): silently ignore stale `trust` key (use `.get()`, don't pass to constructor). Remove `trust=` from all `STARTING_DIPLOMATS` entries (lines 68, 76, 84, 92, 100) and `create_starting_diplomats` (line 115).
5. `diplomatic_ledger.py` — **AM-23.12:** Remove trust read (line 527), trust_label computation (lines 530-538), AND `"trust"`/`"trust_label"` keys from return dict (lines 672-673). Replace with authority-based label using `world.authority_tracker.authority`.
6. `diplomatic_advisory.py` — Remove trust from advisory text (line 121: `f"skill {int(diplomat.skill)}, trust {int(diplomat.trust)}"` → remove trust portion).
7. `main.py` — **AM-23.10:** Remove `_get_talleyrand_trust_label()` function (lines 113-127) and replace all 3 call sites (lines 172, 497, 828) with authority-based label or remove the `"talleyrand_state"` key entirely (Godot doesn't read it). **AM-23.3:** Add `"insist"` to both `_DIALOGUE_RESPONSE_KEYWORDS` (line ~617) AND `_DIALOGUE_ONLY_KEYWORDS` (line ~653).
8. `dispatch.py` — Remove `talleyrand_redemption` initialization (line 81), redemption check block entirely (lines 889-907: import, check, dialogue push, popup data).
9. `world_state.py` — Remove `last_redemption_turn` field (line 476, to_dict line 3138, from_dict line 3380). **AM-23.6:** Remove `talleyrand_redemption_popup` property + setter (lines 638-643), `to_dict` entry (line 3165), `from_dict` entry (line 3417).
10. `main.gd` — Add `"pushback_confirm"` to dtype whitelist (line ~700). **AM-23.6:** Remove redemption: variable declaration (line 77), registration + signal connection (lines 201-203), response check (lines 736-738), handler function `_on_talleyrand_redemption_choice` (lines 2814-2819).
11. `diplomatic_ledger.gd` — Trust label display (lines 550-562) will auto-resolve when backend stops sending trust data.
12. `talleyrand_redemption_popup.gd` — Delete (no longer needed).
13. **AM-23.6:** `talleyrand_redemption_popup.tscn` — Delete (scene file, alongside `.gd`).
14. **AM-23.6:** `cooldown_manager.py` — Remove `"talleyrand_redemption_popup"` from `PRIORITY_ORDER` (line 141) and `RESPONSE_KEYS` (line 153).
15. **AM-23.6:** `dialogue_manager.py` — Remove `"talleyrand_redemption"` from priority dict (line 34).
16. **AM-23.6:** `turn_manager.py` — Remove `"talleyrand_redemption_popup"` from `popup_fields` list (line 276).
17. **AM-23.11:** `meta_executor.py` — Remove `set_talleyrand_trust` cheat command (lines 1993-2002).
18. Tests — Probability curve unit tests, pen nudge deterministic tests (incl. edge cases: empty demands, territory value-shape, AP-only, gold below feasibility floor, **AM-25.1** schemer swap with duplicate collision), mutual exclusion integration tests (**AM-23.4** round-trip `objection_resolved` through save/load), DP-on-cancel tests, **AM-23.2** modify_count preservation on [Cancel], **AM-23.9** terminal "cancel" keyword routes to `cancel_pushback` during pushback dialogue, serialization update tests, redemption removal regression tests. Update diplomat trust assertions in: `test_phase1_critical_wiring.py`, `test_session6_diplomacy.py`, `test_session8a_ledger_debug.py`, `test_session8b_ledger_ui.py`, `test_systems_audit_session11.py`, `test_deep_audit_session9.py` (lines 154-191: DiplomaticRepresentative construction with trust param).
...
| `gold_lump` | Not scored | Lump-sum gold demands contribute zero harshness. Produced by: `generate_ultimatum_terms` (diplomatic_templates.py:1337), `_build_ultimatum_confirm_step` (diplomatic_executor.py:2808 floor demand), `_build_base_terms` (diplomatic_templates.py:1625 armistice sweetener) | Add `+0.1 * (value / 500)` (lower weight than per-turn since one-time) |
| `manpower_infantry` | Not scored | Produced by `generate_ultimatum_terms` (diplomatic_templates.py:1371) and ultimatum wizard (diplomatic_executor.py:1433). 500-5000 unit demands score zero | Add `+0.15 * (value / 1000)` with floor of 0.15 |
| `manpower_cavalry` | Not scored | Produced by ultimatum wizard (diplomatic_executor.py:1433, f-string `manpower_{unit_type}`) | Same formula as manpower_infantry |
| `manpower_artillery` | Not scored | Same producer as cavalry | Same formula as manpower_infantry |
| `ap_per_turn` | +0.3 flat | 1 AP/turn = 3 AP/turn = same harshness | Scale: `+0.3 * max(1, value)` |
| `unit_trade` | +0.15 flat | **Dead code** — `unit_trade` demand type is never produced by any code path. Harshness formula scores it but nothing generates it | **AM-24.2:** Remove from formula. Dead code that scores a phantom type confuses future implementers. If `unit_trade` is ever implemented, it gets its own harshness rule at that time |

**Note:** `calculate_treaty_harshness()` in `diplomatic_templates.py:1874` is a separate function that DOES score manpower types. `calculate_proposal_harshness()` in `diplomatic_defiance.py` does NOT — they are independent formulas. PL-24 fixes only `calculate_proposal_harshness`.

#### Harshness function asymmetry

The codebase has two harshness functions that evaluate identical proposals differently:

| Function | Location | Used By | Scores gold_lump | Scores manpower | Sweetener handling |
|---|---|---|---|---|---|
| `calculate_proposal_harshness` | diplomatic_defiance.py:132 | Talleyrand defiance/pushback | **No** (gap) | **No** (gap) | Flat -0.1 per sweetener |
| `calculate_treaty_harshness` | diplomatic_templates.py:1874 | Acceptance formula | Yes | Yes | Weighted by SWEETENER_VALUES |

After PL-24, `calculate_proposal_harshness` will score the missing demand types. However, **sweetener handling remains asymmetric**: defiance harshness treats all sweeteners as -0.1 flat regardless of type/value, while treaty harshness uses proportional weights. This means a proposal with a massive gold sweetener (200 gold/turn) reduces defiance harshness by the same -0.1 as a token sweetener (25 gold/turn). This is acceptable for PL-24 scope — sweetener weighting is a design refinement item, not a bug. Document for future reference.

#### Naming inconsistency

`_build_base_terms()` (diplomatic_templates.py:1590) produces sweeteners with type `"infantry_manpower"` (reversed), while all other code uses `"manpower_infantry"`. Fix: rename line 1590 to `"manpower_infantry"` for consistency. Otherwise the harshness formula (even after PL-24) won't match this variant.

#### `manpower_cavalry` / `manpower_artillery` — never produced

These demand types are defined in `DEMAND_VALUES` (diplomacy.py:164-165) and can be created via the ultimatum wizard's f-string (`manpower_{unit_type}`), but **no automated code path** (AI proposals, templates, generate_ultimatum_terms) ever produces them. Only player-driven wizard interaction creates them. Score them in PL-24 anyway — the wizard can generate them.

#### Files
- `backend/commands/diplomatic_defiance.py` — `calculate_proposal_harshness` (lines 132-162)
- `backend/game_logic/diplomatic_templates.py` — rename `"infantry_manpower"` → `"manpower_infantry"` (line 1590)
...
Current `_build_base_terms()` in `diplomatic_templates.py` ignores: diplomat personality, nation desire profiles, recent events, and any randomization. All data sources exist (`NATION_DESIRE_PROFILES`, `TALLEYRAND_COMMENTARY`, diplomat personality field on `DiplomaticRepresentative`) but none feed into term generation.

**Signature gap:** `_build_base_terms(target_nation, proposal_type, world)` has no diplomat parameter. Diplomat personality must be accessed via `world.diplomats.get("France")` inside the function, or the signature must be extended to `_build_base_terms(target_nation, proposal_type, world, talleyrand=None)` with one call-site update in `generate_suggested_terms` (line ~1403).
...
##### 1. Amount jitter (±20%)
Add `random.uniform(0.8, 1.2)` multiplier to gold/manpower demand values AND sweetener gold values (**AM-25.2**: sweeteners are equally formulaic — Talleyrand shouldn't offer identical gold every time). Terms will vary between proposals even for identical game states. **AM-25.3 — Deterministic mode:** Accept optional `deterministic=False` param on `_build_base_terms()`. When `True`, use multiplier 1.0. Call-site in `generate_suggested_terms` passes `deterministic=False` by default. Tests pass `deterministic=True`. Avoids env var coupling.
...
**AM-25.1 — Schemer swap collision rule:** If swapping would create a duplicate demand type (e.g. territory → gold_per_turn, but a gold_per_turn demand already exists), **merge** the values (add swap value to existing demand, remove the swapped demand). If ALL swap targets already exist (gold_per_turn, gold_lump, ap_per_turn all present), fall back to loyalist-style 20% reduction on the original demand. Try swap targets in priority order: gold_per_turn → gold_lump → ap_per_turn. Merge on collision, reduce on exhaustion.
...
**Available profile fields** (all 4 nations in `NATION_DESIRE_PROFILES`, diplomatic_templates.py:1205-1238):
- `covets_regions` (list) — already used in Stage 2 of `generate_suggested_terms`
- `values_gold` ("low"/"medium"/"high") — already used in Stage 2
- `values_ap` ("low"/"medium"/"high") — already used in Stage 2
- `diplomatic_lever` (str: "ambition"/"stability"/"trade"/"survival") — already used in Stage 2
- `values_territory` ("low"/"high") — **exists but never read** in any code path. Wire into `_build_base_terms` for territory demand aggressiveness.
- `weakness` (str) — **exists but never read**. Wire for pen nudge target selection (Talleyrand exploits known weaknesses).
...
Read from `world.get_latest_events(n)`, `world.get_events_since_turn(turn)`, `world.get_events_by_type("battle")`, or `world.diplomatic_history`. **AM-25.6: Do NOT use `world.battle_history`** — it is initialized but never populated (zombie field). Use `world.get_events_by_type("battle")` or filter `world.get_events_since_turn(world.current_turn - 3)` for combat events instead. `world.diplomatic_history` is populated and always a list (rolling window of 20). **Note:** `world.turn_events` does not exist — use the event API methods on WorldState (`get_latest_events`, `get_events_by_type`, `get_events_since_turn`, all defined in world_state.py:794-806). 4-6 condition checks, one line of flavor. No new systems.

#### Pre-requisite: PL-24 naming fix

`_build_base_terms()` line 1590 produces `"infantry_manpower"` sweeteners while all other code uses `"manpower_infantry"`. PL-24 fixes this naming inconsistency. PL-25's jitter and desire-profile bias must use the canonical `"manpower_infantry"` form.
