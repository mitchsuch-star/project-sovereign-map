...
| **WO-35** | P2 | **Two more pieces of blocking state survive a save and raise nothing at load** (found by slice 15's response-key census, not folded in). `pending_objection` round-trips through `to_dict`/`from_dict`, and `/load` never re-attaches it — worse, `_response_has_objection_route` requires `success == true` while the executor's objection block returns `success: False`, so that modal is unreachable by construction on the recovery path too. `pending_interrupt` (marshal-level) has the same load-side silence. Strictly worse than WO-30 was, because neither has WO-30's one-refused-command self-heal narrative. | `main.py` `/load`; `world_state.to_dict/from_dict`; `main.gd:1863-1867` | **OPEN** — declared in slice 15's census pin as a KNOWN gap, so the pin does not go green by pretending |
| **WO-36** | P3 | **`redemption_event` is the third member of the same family** — `world.pending_redemption` survives the save and no `/load` path re-attaches it, while `main.gd` gates the redemption modal on the response key. | `main.py` `/load`; `world_state.py` `pending_redemption` | **OPEN** — declared in slice 15's census pin as a KNOWN gap |
