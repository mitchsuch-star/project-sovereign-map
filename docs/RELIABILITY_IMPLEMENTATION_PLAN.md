...
- **Named pacing acceptance**: `test_hegemony_pacing_50pct_share_reaches_brewing_by_turn_16` starts from `threat_level = 0` with deterministic `50%` hegemon share, no competing event-based threat, and shipped decay math, then asserts the board reaches `BREWING` (`threat_level >= 60`) by turn 16
