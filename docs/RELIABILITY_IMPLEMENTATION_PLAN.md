...
**Bloc naming (adopted for v2.4.3).** Deterministic bloc naming is the presentation contract for the hegemony layer. The authoritative contract lives in `COMMITMENTS_PRESENTATION_SPEC.md` §8.1a (terminology guard, `33 / 50 / 60` activation gate, hegemon→label taxonomy with fallback, required surface owners: Balance-of-Europe headline + `balance_of_europe_shifted` threshold beats + proposal-preview hegemony warnings + coalition-declaration contrast copy). **Per-row bloc stamps (member badges) ship as Slice E-Cards** — the final slice in this plan, fully scoped with tests and acceptance gates in the Slice E-Cards section below. No "deferred" / "un-deferred" / "in-scope-but-last" framing; the slice order simply ends with E-Cards so the headline label contract is proven in playtest before per-row stamping layers on top. Voice register beats per band live in `DIPLOMAT_VOICE_BIBLE.md` Minimum cast coverage (`hegemony_beat_*_{noticed,alarming,crisis}`). Implementation helper is `describe_hegemon_bloc(world, hegemon, share)` in this plan's B-Hegemony slice — no new serialized save surface.
...
**Phase placement:** Slice E-Cards is the final slice in this plan and ships AFTER all of B-Hegemony, B-B1-lite, B-B3, B-B7, Slice C-lite, and B-B4 have merged. The ordering is headline first, stamps last — the headline contract (§11.1 top block) and the bloc-naming taxonomy (`COMMITMENTS_PRESENTATION_SPEC.md` §8.1a) prove out in playtest before per-row stamping layers on top. The slice is fully scoped below with scope, tests, and acceptance gates — not "deferred", not "conditional", not an optional layer. Closing Memory and Pressure requires closing this slice.
