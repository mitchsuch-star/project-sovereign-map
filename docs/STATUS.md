...
> **Last Updated:** April 19, 2026 (Map Readiness Closure Pass — §4.3 color-map validator is COMPLETE. `tools/validate_province_map.py` ships as a standalone offline acceptance gate for commissioned art deliveries: registry-only checks (sentinel collision, duplicate lookup colors) plus image-side checks (size mismatch, missing province, insufficient coverage at default 50 pixels, tiny pixel islands at default <5 pixels as warnings, unmapped colors with sample coords). Six failure codes with two severities (error / warning); CLI exit 0/1/2; `--json` for CI; `--strict` to promote warnings; pure-Python PNG decoder supports 8-bit RGB + RGBA with all five PNG scanline filters and rejects interlaced / unsupported formats with clear errors. 22 new tests in `tests/test_province_map_validator.py` pin registry checks, image checks, CLI exit codes, JSON mode, strict mode, and PNG-decoder behavior. The shipped placeholder `session8_placeholder_provinces.json` is pinned to always pass registry-only checks. Full Python-suite baseline now `8480 passed, 2 skipped` (was `8453`; +27 = 22 new validator tests + 5 elsewhere). Ruff clean on the new files. Next: §4.4 unwired province support → commissioned art-backed renderer cutover (still art-blocked).)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8480 passed, 2 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Map Readiness Closure Pass — Phase 4.3 COMPLETE (April 19, 2026).** Phase 3 (§§3.1-3.4), Phase 4.1 (province registry schema v2), Phase 4.2 (external bitmap loading), and Phase 4.3 (offline color-map validator at `tools/validate_province_map.py` with six failure codes, JSON/strict CLI modes, and a pure-Python PNG decoder covering RGB/RGBA + all five scanline filters) are all landed. Phase 2 remains COMPLETE. Next: §4.4 unwired province support. |
| **Blockers** | Real map readiness is **not** just "wait for art." The raw `enemy_ai.py` scan pass, prompt/parser/adjacency drift, nation config factory, province registry schema, renderer bitmap-ingest seam, and offline art-acceptance validator are all closed. Active non-art blocker narrows to §4.4 (unwired grey-tint rendering). Commissioned art integration and final renderer smoke remain blocked on art assets. |
...
5. **Map Readiness Closure Pass — §4.3 Color-Map Validator COMPLETE (April 19, 2026).**
   ...
   - **CLI contract.** `--registry` required; `--visual` + `--lookup` paired or omitted (registry-only mode is the cheapest gate, runs without any PNG). `--min-coverage-pixels` and `--tiny-island-threshold` tune the two thresholds per delivery. `--json` emits `{ok, error_count, warning_count, failures[]}` for CI; `--strict` promotes warnings to a non-zero exit. Exit codes: `0` pass / `1` validation failure / `2` bad input.
   - **PNG decoder scope.** Pure-Python (`zlib` + `struct`); supports 8-bit RGB (color_type 2) + RGBA (color_type 6) with all five PNG scanline filters (None / Sub / Up / Average / Paeth). Rejects interlaced PNGs, non-zero compression/filter methods, and unsupported bit depths with a clear `PNGDecodeError`. Pillow intentionally NOT a dep — stays inside the project's existing `requirements.txt`.
   - **Test coverage (22 tests, `tests/test_province_map_validator.py`).** Registry checks (placeholder passes; sentinel collision; duplicate colors; loader rejects malformed inputs); image checks (clean baseline; size mismatch; missing province; insufficient coverage; tiny islands fire alongside `UNMAPPED_COLOR`; high-pixel-count unmapped stays error-only; sentinel pixels ignored; RGB-only PNGs decode); CLI exit codes 0/1/2 + `--json` + `--strict`; PNG decoder rejects interlaced + non-PNG signatures and round-trips a 5-row RGBA fixture exercising all five filter types.
   - **Verification.** Focused map suite: **58 passed** (`tests/test_map_renderer_cutover.py tests/test_map_placeholder_assets.py tests/test_map_bitmap_contract.py tests/test_province_map_validator.py -q`). Full Python suite: **8480 passed, 2 skipped** (was `8453`). Ruff clean on the new files. The shipped placeholder JSON passes registry-only checks (pinned by `test_placeholder_registry_passes_registry_only_checks`).
...
6. **Memory and Pressure — Slice A (rivalry seed).** This remains the next diplomacy feature track, but it is not part of the map-readiness gate. See `docs/RELIABILITY_IMPLEMENTATION_PLAN.md`.
...
6. **Bitmap validation exists and runs before art integration.** ✅ DONE (April 19, 2026 — §4.3).
   - `tools/validate_province_map.py` checks dimension match, unknown colors, missing provinces, sentinel misuse, duplicate lookup colors, insufficient coverage, and tiny pixel islands. CLI emits human-readable or `--json` reports with exit code `0`/`1`/`2`; `--strict` promotes warnings.
...
**Next bug-owned implementation slice:** Map Readiness Closure Pass — §4.4 unwired province support.
...
**Current Session 8 progress:** Cutover slices 1-3 + §§4.1-4.3 COMPLETE. The map renderer now builds shared scene-node layers behind `map_renderer_base.gd`, keeps the existing `map.gd` data wrapper and `update_all_regions(map_data)` contract stable for `main.gd`, includes a placeholder province-definition asset plus hidden color-map lookup path for the current 19-region map, isolates the map world inside a `SubViewport` with native `Camera2D` zoom/pan plus world-bound clamping, carries the production province-registry schema (`province_id`, per-feature anchors, `wired`, `interactive`), can ingest imported bitmap assets through an exported-build-safe loader with runtime lookup-color validation plus circle fallback, and ships an offline color-map validator (`tools/validate_province_map.py`, 22 tests) that gates commissioned-art deliveries on sentinel collisions, duplicate colors, dimension mismatches, missing/insufficient province coverage, tiny pixel islands, and unmapped colors. Remaining non-art Session 8 work narrows to §4.4 (unwired-province runtime/visual contract). Once §4.4 lands, the remaining art-dependent work is the commissioned art-backed renderer swap and final Godot runtime smoke validation.
