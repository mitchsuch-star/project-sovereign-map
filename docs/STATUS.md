...
> **Last Updated:** April 19, 2026 (Map Readiness Closure Pass — §4.3 color-map validator is COMPLETE. `tools/validate_province_map.py` ships as a standalone offline acceptance gate for commissioned art deliveries: registry-only checks (sentinel collision, duplicate lookup colors, empty-registry rejection) plus image-side checks (size mismatch, missing province, insufficient coverage at default 50 pixels, connected tiny pixel islands at default <5 pixels as warnings, unmapped colors with sample coords). Six failure codes with two severities (error / warning); CLI exit 0/1/2; `--json` for CI; `--strict` to promote warnings; the visual PNG size path is header-only while the lookup PNG decoder remains pure-Python and supports 8-bit RGB + RGBA with all five PNG scanline filters, malformed IHDR / corrupt IDAT wrapping, and clear unsupported-format errors. 29 tests in `tests/test_province_map_validator.py` pin registry checks, image checks, CLI exit codes, JSON mode, strict mode, and PNG-decoder behavior. The shipped placeholder `session8_placeholder_provinces.json` is pinned to always pass registry-only checks. Full Python-suite baseline now `8487 passed, 2 skipped` (was `8453`; +34 = 29 validator tests + 5 elsewhere). Ruff clean on the new files. Next: §4.4 unwired province support → commissioned art-backed renderer cutover (still art-blocked).)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8487 passed, 2 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Map Readiness Closure Pass — Phase 4.3 COMPLETE (April 19, 2026).** Phase 3 (§§3.1-3.4), Phase 4.1 (province registry schema v2), Phase 4.2 (external bitmap loading), and Phase 4.3 (offline color-map validator at `tools/validate_province_map.py` with six failure codes, JSON/strict CLI modes, connected tiny-island detection, and a pure-Python PNG decoder covering RGB/RGBA + all five scanline filters) are all landed. Phase 2 remains COMPLETE. Next: §4.4 unwired province support. |
