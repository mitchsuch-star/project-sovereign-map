...
> **Last Updated:** April 28, 2026 (**IMPERIAL SETTLEMENT SPEC v1.8 SYNTHESIS CLOSURE READY + AUDIT CLARIFICATIONS** - `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` are aligned for Slice A coding after the Codex/Claude full-audit synthesis and follow-up clarification patch. Closure hardening now includes explicit ARMISTICE pair-status lifecycle inside `war_instance`, common peace locked to PEACE/treaty outcomes rather than armistice, all direct WAR-entry paths attached to `war_instance`, concrete `settlement_confirm` / `incoming_settlement_offer` dialogue contracts, Open Settlement eligibility, deterministic bargain settlement classification, settlement event routing/fog contracts, a Pressburg-style worked acceptance example, mandatory Slice C C1/C2 split and tuning gate with `0.6` side-pressure scaling as the first adjustment knob, AI-vs-AI settlement dispatch/log visibility, AI offer anti-spam, and deterministic settlement-memory cleanup/archive retention.)
...
| **Current Phase** | **Peace Deals — Ally Participation spec v1.8 / implementation plan v1.5 READY FOR SLICE A (April 28, 2026).** BPH-A through BPH-D, WPS-A through WPS-D, WB-A, WB-B, WB-C, and WB-D all landed. `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` now own the full-Europe handoff after combined Codex/Claude synthesis closure; coding may start with Slice A. Map-readiness non-art closure remains complete and art-blocked only. |
...
**Next diplomacy workflow:** **Ally Participation / Common Peace Slice A.** BPH-A through BPH-D + WPS-A through WPS-D + WB-A/WB-B/WB-C/WB-D all landed. Gate 1 passed. `threat_coalition` merged into `balance_of_europe`. War Bargains track fully closed. The historical Peace Deals umbrella remains for cross-cutting context; active settlement work starts from `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md`.
...
| 4 | Ally Participation + Common Peace | ally beneficiaries, contribution / consultation rights, common peace routing, settlement fallout | Post-bargains track. `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.8 and implementation plan v1.5 own the full-Europe handoff; next work is Slice A. |
...
**Peace Deals spec gate (Apr 28):** BPH-A through BPH-D, WPS-A through WPS-D, and WB-A through WB-D are landed. Ally Participation/Common Peace now has `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.8 and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.5 covering full-Europe scale after combined Codex/Claude synthesis closure; next is Slice A coding.
