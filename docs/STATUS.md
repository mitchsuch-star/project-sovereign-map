...
> **Last Updated:** April 26, 2026 (**WB-A COMPLETE** — War Bargains data model + creation + validation + acceptance modifiers + region_observer activation. `diplomatic_commitments` + `next_commitment_id` added to WorldState with full serialization. `create_war_bargain_commitment()` factory builds spec §8.6 records. `validate_war_bargain()` enforces 8 rules: source treaty, opposition target, claim holder, ally-held guard, duplicate beneficiary+enemy cap, duplicate claim_region cap, contradiction guard, cooldown. `get_bargain_opposition_pairs()` derives targets from WAR states, coalition members, and live bargain conflicts. `bargain_value_mod` (+10 DEF_ALLIANCE / +15 ALLIANCE) and `bargain_conflict_penalty` (-8) wired into `calculate_acceptance()` political subtotal before -60 floor clamp. Region_observer witness scope activated. `war_bargain` clause processing in `_ratify_treaty()`. 23 new tests in `tests/test_wb_a_bargain_model.py`. Verification: full suite `9095 passed, 1 skipped`; ruff green. **Next: WB-B.**)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `9071 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend tests`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals — Gate 1 passed, `threat_coalition` retired (April 26, 2026).** BPH-A through BPH-D + WPS-A through WPS-D all landed. `threat_coalition` merged into `balance_of_europe`. Next: WB-A. Map-readiness non-art closure remains complete and art-blocked only. |
...
**Next diplomacy workflow:** **Phase A COMPLETE + `threat_coalition` retired.** BPH-A through BPH-D + WPS-A through WPS-D all landed. Gate 1 passed. `threat_coalition` merged into `balance_of_europe`. Next is **WB-A** — first War Bargains coding slice. Follow `docs/PEACE_DEALS_UMBRELLA_SPEC.md` slice order.
