...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `9743 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Focused Final Gate bundle is `622 passed`; ruff is green for `backend/ tests/`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals - Imperial Settlement Slice E presentation LANDED (May 4, 2026); Final Gate is next.** BPH-A through BPH-D, WPS-A through WPS-D, WB-A through WB-D, A1/A2/A3, B1/B2/B3, C1a, C1b, C2 preview foundation, C2 ratification mutation, D1/D2 settlement & cross-war reaction routing, and **E presentation, ledger, and logs** are all landed. Slice E ships the new `backend/game_logic/settlement_presentation.py` module owning settlement-route metadata (`SETTLEMENT_ROUTES` distinct from `COMMITMENTS_ROUTES` per spec §11.6 line 1290), spec §11.6 fog rule (`is_settlement_event_visible`), top-four primary-beat cap + digest overflow (`cap_settlement_dispatch_lines` + `_enforce_settlement_primary_beat_cap` in dispatch), spec §16.3 warning cap (`apply_warning_cap`), spec §16.2 sectioned settlement review at compact / medium / verbose density (`build_settlement_review`), spec §20.8 awe set-piece tagging (`detect_awe_set_pieces`), notification rail spotlight metadata (`settlement_notification_meta`), recent-settlements ledger payload (`recent_settlement_summaries`), and top-N contribution-share rows (`build_contribution_share_rows`). `dispatch.py` consumes settlement events through their own fog filter + formatter and applies the per-ratification cap; `campaign_log.py` filters `settlement_summary` / `settlement_digest` through the same fog rule; `diplomatic_ledger.py` exposes `recent_settlements` in the ledger payload; `war_status.py` decorates each war entry with `contribution_share[]` + `contribution_overflow_count` + `war_instance_id`. Godot side: `top_bar.gd` wires the `ledger_settlements` review target into `open_diplomatic_ledger_review`; `diplomatic_ledger.gd` adds `open_to_settlements()` plus a Recent Settlements block on the Treaties tab; `main.gd` routes `settlement_review` / `diplomatic_ledger` / `ledger_settlements` notification targets to the layer-50 ledger; `war_status_panel.gd` extends the war tooltip with the standing list. Voice Bible §16.1 settlement families remain the contract for final committed cast copy authored in the Final Gate. Verification: 65 new tests in `tests/test_settlement_presentation.py` (settlement routes + priority / review-target / fog filter / one-liner formatter / top-four cap / warning cap / sectioned review / awe tagging / notification meta / recent-settlements / dispatch wiring / campaign-log filter / war-status contribution rows). Settlement focused suite: `tests/test_settlement_presentation.py tests/test_settlement_reactions_d1_d2.py tests/test_settlement_balance_projection.py tests/test_war_settlement_foundation.py tests/test_war_settlement_instances.py tests/test_war_settlement_merge.py tests/test_settlement_side_pressure.py tests/test_common_peace_acceptance.py tests/test_common_peace_harshness.py tests/test_war_status.py` = `304 passed`. Full suite `9736 passed, 1 skipped` (was `9671`, +65 net). Ruff clean across `backend/`. Map-readiness non-art closure remains complete and art-blocked only. **Next: Final Gate (`docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` line 400) — full settlement test bundle + WB/WPS/BPH bundle re-runs + manual Godot smoke on settlement review / war status / notification / ledger routes; per-cast committed copy passes for `settlement_acceptance_*` / `settlement_rejection_*` / `settlement_sold_out_by_leader_*` / `settlement_rewarded_ally_*` / `settlement_excluded_ally_*` author against Voice Bible §16.1 inside that gate.** No Slice F is currently defined. |
| **Final Gate Update** | Backend/copy gates now include committed Imperial Settlement cast templates in `backend/game_logic/diplomatic_templates.py`, compact warning cap correction, settlement fog membership payloads, and notification rail review metadata wiring. `tests/test_settlement_presentation.py` now covers 72 focused cases including defensive-settlement copy and serial-peace fallout legibility. Godot runtime smoke remains manual-only unless a Godot executable is available. |
