...
> **Last Updated:** April 20, 2026 (Memory and Pressure v2.4.3 Block 1 doc cleanup is complete. Canonical docs now agree on the routing join-table, Balance-of-Europe / hegemony signal contract, Make Amends visibility, DG-4 notice ownership, save-format alias policy, and the next-work handoff. Fresh diplomacy sessions should now start with `docs/audits/MP_V243_BLOCK2_SUBSTRATE.md`, then `docs/audits/MP_V243_BLOCK3_BLOC_NAMING.md`, before resuming the remaining implementation slices.)
...
7. **Memory and Pressure v2.4.3 — audit blocks still gate the next diplomacy feature pass.** `docs/audits/MP_V243_BLOCK1_DOC_CLEANUP.md` is complete. The next diplomacy feature track is now `docs/audits/MP_V243_BLOCK2_SUBSTRATE.md`, then `docs/audits/MP_V243_BLOCK3_BLOC_NAMING.md`, followed by the remaining B-Hegemony / B-B1-lite / B-B3 / B-B4 / B-B7 / C-lite implementation in `docs/RELIABILITY_IMPLEMENTATION_PLAN.md`. This is not part of the map-readiness gate.
...
**Next bug-owned implementation slice:** None. All non-art map-readiness items are closed. The remaining map-readiness work (commissioned art-backed renderer cutover + final Godot smoke validation) is art-blocked and sits in the Blocked list above. For a fresh session on diplomacy, the next work is Memory and Pressure v2.4.3 audit-block closure: `docs/audits/MP_V243_BLOCK2_SUBSTRATE.md` first, then `docs/audits/MP_V243_BLOCK3_BLOC_NAMING.md`, then the remaining implementation slices.
...
|-------|------------|------------------------|------|
| 1 | Memory and Pressure v2.4.3 | `R160`, `R119` | First diplomacy follow-up implementation target. Substrate is shipped; Block 1 doc cleanup is complete, so the next pass is `docs/audits/MP_V243_BLOCK2_SUBSTRATE.md`, then `docs/audits/MP_V243_BLOCK3_BLOC_NAMING.md`, then the remaining B-Hegemony / B-B1-lite / B-B3 / B-B4 / B-B7 / C-lite work in `docs/RELIABILITY_IMPLEMENTATION_PLAN.md` v2.4.3 and `docs/COMMITMENTS_PRESENTATION_SPEC.md` v0.5.1. |
...
- Block 2 substrate fixes are next: `docs/audits/MP_V243_BLOCK2_SUBSTRATE.md` owns the remaining code / serialization / test hardening before the broader feature slice continues.
- Block 3 bloc naming / alignment legibility is after Block 2: `docs/audits/MP_V243_BLOCK3_BLOC_NAMING.md` is the queued design gate that decides whether the hegemony layer should gain proper bloc naming before the remaining slices harden in code.
