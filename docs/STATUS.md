...
> **Current Settlement Gate (May 7, 2026):** `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.19 is SPEC READINESS GO / IMPLEMENTATION NO-GO. Full Treaty Settlement remains approved: author concrete war-scoped terms, preview consequences and acceptance, revise/re-author, preserve incoming-offer packages if offers ship, and ratify only after acceptance/hard-stop gates pass. v0.19 adds SC-4b, the explicit resource-cost contract: settlement entry, preview, Submit, Revise Terms, Back Out, failed ratification, stale recovery, hard-stop refusal, and successful ratification default to 0 DP/AP/gold unless a later product decision records a non-zero `confirm_settlement` cost in both spec and STATUS. Gate 2 remains ordered as Foundation (SC-1 through SC-4b), Entry Safety, Continuity, Incoming Offers, then Presentation and Metadata. Slice G remains blocked until the full treaty cleanup is implemented, behavior-tested, and manually smoked.
> **Last Updated:** May 7, 2026 (**SETTLEMENT UI CLEANUP SPEC v0.19 RESOURCE-COST CONTRACT FOLD-IN COMPLETE** - `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` now carries the post-commit readiness tightenings plus SC-4b, an explicit settlement resource-cost contract. Settlement entry, preview, Submit, Revise Terms, Back Out, failed ratification, stale recovery, hard-stop refusal, and successful ratification default to 0 DP/AP/gold unless a later product decision records a non-zero `confirm_settlement` cost in both spec and STATUS. Implementation remains NO-GO until Gate 1 doc scans pass, Gate 2 behavior tests close, and manual smoke passes.)
