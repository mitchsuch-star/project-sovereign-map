...
> **Last Updated:** April 19, 2026 (Map Readiness Closure Pass — §4.1 province registry schema landed. `godot-client/project-sovereign/assets/maps/session8_placeholder_provinces.json` now ships `schema_version: 2`: every region entry carries a stable `province_id`, four per-feature anchors (`unit_anchor` / `label_anchor` / `garrison_anchor` / `building_anchor`), and boolean `wired` / `interactive` flags. `map_renderer_base.gd` `_build_province_shapes()` reads all new fields into `province_shapes`; `_lookup_region_from_color_map()` and the `_refresh_hover_state()` distance-fallback both return `""` for non-interactive regions; `update_all_regions()` filters out unwired provinces before touching `region_full_data` / `region_controllers` / `region_visibility` / `region_marshals` / `region_fogged_forces` / `region_garrisons`; `update_region()` short-circuits on unwired regions to keep the singular path consistent. `tests/test_map_placeholder_assets.py` grew from 4 → 12 tests pinning schema version, province_id presence + uniqueness, all four anchor fields, wired/interactive flags, and renderer-side consumption of the new schema. `tests/test_map_renderer_cutover.py` now also pins the actual non-interactive distance-fallback skip, the unwired gameplay-state erasures inside `update_all_regions()`, and the unwired short-circuit inside `update_region()`. Placeholder behavior is unchanged for the current 19 regions (all wired/interactive); actual per-feature anchor offsets and unwired-province grey-tint rendering remain §4.4 work. Full Python suite green: 8449 passed, 2 skipped. Next: §4.2 external bitmap loading.)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8449 passed, 2 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Map Readiness Closure Pass — Phase 4.1 COMPLETE (April 19, 2026).** Phase 3 (§§3.1-3.4) and Phase 4.1 (province registry schema v2) are all landed suite-green. Phase 2 remains COMPLETE. 8 new placeholder-asset tests plus 4 §4.1-specific renderer guardrails now pin the runtime contract. Next: §4.2 external bitmap loading → §4.3 color-map validator → §4.4 unwired province support. |
| **Blockers** | Real map readiness is **not** just "wait for art." The raw `enemy_ai.py` scan pass, prompt/parser/adjacency drift, nation config factory, and province registry schema work are now all closed. Active non-art blockers narrow to §§4.2-4.4 (bitmap loading, color-map validator, unwired grey-tint rendering). Commissioned art integration and final renderer smoke remain blocked on art assets. |
...
3. **Map Readiness Closure Pass — §4.1 Province Registry Schema COMPLETE (April 19, 2026).**
   ...
   - **Test coverage.** `tests/test_map_placeholder_assets.py` grew from 4 → 12 tests pinning schema version, `province_id` presence + uniqueness, all four anchor fields, `wired` / `interactive` flag presence + types, current-roster invariants, and renderer-side consumption of both the new flags and the new anchor fields. Four §4.1-specific tests in `tests/test_map_renderer_cutover.py` now pin `region_full_data = wired_data` plus the `wired` gate inside `update_all_regions()`, the unwired-dict erasure block inside `update_all_regions()`, the unwired early return inside `update_region()`, and the non-interactive skip in `_refresh_hover_state()`'s distance fallback. Full Python suite: 8449 passed, 2 skipped.
   - **Not yet covered — these remain §4.4 work:** grey-tint rendering for unwired provinces, "(not yet in play)" hover copy, and authoring-side per-feature anchor offsets. §4.1 only delivers the schema + plumbing.
   - **Suggested next cold-start sequencing:** §4.2 (external bitmap loading) → §4.3 (color-map validator) → §4.4 (unwired province support, including the grey-tint + hover-copy semantics for unwired entries now that the schema carries the flag). A fresh session only needs CLAUDE.md + this STATUS.md + `SCALE_READINESS_PLAN.md` §4.
4. **Memory and Pressure — Slice A (rivalry seed).** This remains the next diplomacy feature track, but it is not part of the map-readiness gate. See `docs/RELIABILITY_IMPLEMENTATION_PLAN.md`.
...
**Next bug-owned implementation slice:** Map Readiness Closure Pass — §4.2 external bitmap loading.
...
**Current Session 8 progress:** Cutover slices 1-3 + §4.1 COMPLETE. The map renderer now builds shared scene-node layers behind `map_renderer_base.gd`, keeps the existing `map.gd` data wrapper and `update_all_regions(map_data)` contract stable for `main.gd`, includes a placeholder province-definition asset plus hidden color-map lookup path for the current 19-region map, isolates the map world inside a `SubViewport` with native `Camera2D` zoom/pan plus world-bound clamping, and now carries the production province-registry schema (`province_id`, per-feature anchors, `wired`, `interactive`). Remaining Session 8 work is **not only** "wait for art": before commissioned Europe integration the project still needs the §4.2 external bitmap-loading path, the §4.3 bitmap validator, and the §4.4 unwired-province runtime/visual contract. Next non-art slice: §4.2 external bitmap loading. Once those non-art prerequisites land, the remaining art-dependent work is the commissioned art-backed renderer swap and final Godot runtime smoke validation.
