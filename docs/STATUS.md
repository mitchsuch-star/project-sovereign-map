...
> **Last Updated:** May 3, 2026 (**IMPERIAL SETTLEMENT SLICE C1b COMMON-PEACE ACCEPTANCE FORMULA LANDED** — the C1b sub-gate of `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` §6.acceptance lines 1095-1147 closed. New pipeline `calculate_common_peace_acceptance(world, *, war_id, war_instance, proposer_side, accepting_side, accepting_leader, covered_enemy_participants, settlement_terms, ...)` in `backend/game_logic/settlement_scoring.py` ships the spec **nine-component table** (CLAUDE.md "Up Next" listed eight components but the spec's Pressburg worked example at line 1162 totals 58 only when all nine — including `settlement_tier_legitimacy` at line 1099 — are summed; verified by `test_tuning_gate_pressburg_worked_example` reproducing components `+46 / +10 / -11 / 0 / -10 / +15 / -5..-10 / +13 / 0`). Each component has a dedicated pure helper: `calculate_base_side_pressure` (`round(score * 0.65)` half-away-from-zero, clamp `[-50, 60]`), `calculate_term_harshness_penalty` (`-min(45, round((min(raw, 1.5)/1.5) * 45))`), `calculate_settlement_tier_legitimacy` (tier from existing `diplomacy.get_settlement_tier()`, base `+15/+10/+5/0/-10`, `-10` mismatch when raw harshness exceeds tier ceiling per spec line 1188-1196, white-peace-with-any-non-trivial-term auto-mismatch, clamp `[-20, 15]`), `calculate_leader_own_losses` (sub-components summed BEFORE `[-25, 5]` clamp, lost-mapped-holdings sub-cap `-10` per spec line 1186, treats `forced_alliance` against the leader as capital-equivalent sovereignty loss), `calculate_burdened_participant_penalty` (per-burden table `-30 / -15 / +0` × major-uncovered `-10` extra, aggregate cap `-30 * min(burdened_count, 2)` floored at `-60`, accepting leader excluded per spec line 1125), `calculate_war_objective_alignment` (5-WPS-objective table from spec line 1176-1183 + selection chain per line 1174 — leader-vs-accepting → leader-vs-covered → all-proposers WPS-priority `subjugation/forced_alliance/conquest/liberation/defense` → oldest `created_turn`, `0` when no live objective per spec line 1174), `calculate_war_exhaustion_component` (intentional **floor** division `min(20, raw // 3)` per spec line 1120 — explicitly distinct from the `round()` used everywhere else; `raw_per_nation_exhaustion` + `relevant_exhaustion` + `unrelated_exhaustion` always exposed in debug per plan line 222 so the war-exhaustion exploit fixture can prove whether the relevance cap needs to be adopted), `calculate_abandoned_by_ally_mod` (`+5/defector` last 3 turns per `war_instance.separate_peaced`, capped `+15`). New `project_balance_after_settlement(world, *, war_id, settlement_terms)` is the pure projection helper for `projected_hegemony_mod` per spec line 1200-1217 — applies term deltas in-memory only (deterministic order: liberation → forced_alliance → territory → vassalage), recomputes max-bloc-share against the projected snapshot, returns canonical shape `{pre_hegemon, post_hegemon, pre_share, post_share, crossed_band, deepened_band, hegemon_swap, modifier}` with modifier `-20/-10/-5` per crossed/deepened `60%/50%/33%` band and `+10` for de-escalation, clamp `[-20, 10]`. Three no-mutation regression tests pin that `world.regions` / `world.diplomatic_states` / `world.vassals` are never touched. New `compute_forced_alliance_threat_preview(world, *, settlement_terms)` exposes the `+15/clause` projected delta + crossed coalition thresholds (60 brewing / 80 instant / 90 cooldown-override) per spec line 1273. Acceptance threshold `>= 50` accept, `35-49` near_acceptable with structured top-2-component `feedback`, `< 35` hard reject. `calculate_raw_treaty_harshness(treaty)` now lands in `backend/game_logic/diplomatic_templates.py` alongside the existing 1.0-clamped `calculate_treaty_harshness()` (bilateral callers unchanged); both share the same `_accumulate_raw_treaty_harshness()` clause/demand iteration. Britain current-map proxy (`NATION_CAPITALS["Britain"] == "Netherlands"`) is treated as configured scenario data per spec line 1186, not as a separate settlement identity. Pipeline reuses C1a `compute_side_pressure_score` + memoized `direct_scores` map so `get_war_score_for(...)` is called at most once per evaluation. Hard stops bubble up unchanged: `no_covered_enemy_participants` / `no_direct_war_score_for_covered_enemy` return `score=None, verdict="reject"` with the originating `hard_stops` list. Test surface: **57 new tests in `tests/test_common_peace_acceptance.py`** (5 component-unit + 14 deterministic tuning-gate fixtures: Pressburg worked example, Tilsit non-leader burden, coalition split via separate-peace, decisive-victory-without-total-victory, total-victory-harsh-terms, minor-power limited common peace, mixed-strength partial-vs-full coverage, full-Europe narrow-vs-full-vs-serial comparison, 6+ participant coalition, Britain-led defense with `Netherlands` home, mapped-home / capital-cession / restoration variants, multi-forced-alliance threat preview projecting +30 with crossed thresholds, AI-defender alignment ≤ +5 reaching evaluation, war-exhaustion exploit exposing inputs); **3 raw-vs-clamped harshness tests in `tests/test_common_peace_harshness.py`**; **7 projection / no-mutation tests in `tests/test_settlement_balance_projection.py`**. Tuning escalation order documented per spec line 1172 (apply only if Slice C2 stress fixtures fail design targets, one knob per rerun, recorded in `SYSTEMS_REFERENCE.md`): move `BASE_SIDE_PRESSURE_SCALE` ±0.05 → lower `ACCEPTANCE_THRESHOLD` 50→40 → halve `projected_hegemony_mod` for `total_victory` only → bounded `military_supremacy_bonus` → `common_peace_coverage_bonus +3/extra capped +12`. C1b is pure (no mutation, no ratification, no live wiring into `diplomacy.py` / `diplomatic_templates.py` yet). Verification: full suite **`9607 passed, 1 skipped`** (was `9538`; +69 net); ruff clean on every C1b-touched file. **C1b acceptance formula now locked. Remaining: C2 endpoints / dialogue / advisory / Godot routing, then D1/D2 reactions, then E presentation surface.**)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `9607 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend/ tests/`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals - Imperial Settlement Slice C1b common-peace acceptance formula LANDED (May 3, 2026); C2 is next.** BPH-A through BPH-D, WPS-A through WPS-D, WB-A, WB-B, WB-C, WB-D, Slice A1 foundation, A2 war-entry threading, A3 merge/archive/leader invariants, B1 contribution store/standing math, B2 ordering guard/emitter wiring/non-battle emitters, B3 lifecycle/staying-power/archive compaction, C1a side-pressure foundation, and **C1b common-peace acceptance formula** are all landed. C1b ships the spec §6.acceptance nine-component pipeline (`base_side_pressure`, `settlement_tier_legitimacy`, `term_harshness_penalty`, `burdened_participant_penalty`, `leader_own_losses`, `war_objective_alignment`, `projected_hegemony_mod`, `war_exhaustion`, `abandoned_by_ally_acceptance_mod`) plus `project_balance_after_settlement()` pure projection helper, `calculate_raw_treaty_harshness()` extension in `diplomatic_templates.py`, and forced-alliance threat preview. Pipeline is pure — no mutation, no ratification, no live wiring into `diplomacy.py`. Active settlement work is now C2: settlement endpoints / dialogue / advisory / Godot routing wiring against the C1b helper. Map-readiness non-art closure remains complete and art-blocked only. |
| **Blockers** | No active blocker. Commissioned-art renderer smoke remains blocked on art assets. |
