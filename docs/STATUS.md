...
> **Current Settlement Gate (May 7, 2026):** `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.18 is SPEC READINESS GO / IMPLEMENTATION NO-GO with the full treaty-settlement path approved and tenth-pass, May 7 synthesis, and post-commit editor/readiness tightenings folded in. Code that cleanup spec before any Slice G / AI-ally settlement agency work. Quality beats schedule: this must become a player-usable settlement system, not a "wired but not usable" route milestone, and visible broken/misleading behavior cannot be deferred without explicit product approval in status/spec text. The cleanup spec now requires Gate 2 to proceed in ordered slices: Foundation (SC-1 through SC-4, including inline edit mode on the existing settlement review popup, editor `EDIT`/`REVIEW` mode transitions, editor-mode and review-mode ratify controls bound to the same acceptance/hard-stop gate, required first-slice live clause floor, canonical clause schema, executor `settlement_terms` and `selected_target_nation` forwarding, submit revalidation, caller-kind plus player-actor authority, POST schema/taxonomy/conflict matrix, clause-edit commit preview cadence, preview pending/failure recovery, control-state matrix, empty-draft rejection, empty-draft Back Out behavior, Back Out from review draft preservation, draft storage/serialization/end-turn discard, pending-draft migration on war merge, structured editor controls, editor inline validation, GET preview non-staging, fresh ratification-time acceptance scoring, symmetric proposer/accepting leader rescore behavior, absent ratify option, zero-option close payload, rescore-failure UX, threshold payload contract, failed-ratification no-history behavior, and hard-stop catalog), Entry Safety (SC-8/8b/10/11/11b/12/13, including cached eligibility invalidated by `invalidate_war_instance_indexes()`, one-to-one bilateral/armistice substitute CTAs, selected-target entry write matrix, stale-save selected-target fallback, selected-target diagnostics, pinned `evaluate_open_settlement_eligibility(...)` probe identity, executor/staging selected-target persistence, and focused-row coalition per-war buttons including focused-ineligible fallback), Continuity (SC-14/14b/14c/14d/14e/7b/26, including mounted-state definition, same-war restage eligibility, backend `stage_settlement_confirm(...)` collision protection, active live-route precedence, click-time active-vs-archived re-resolution, dispatch active-vs-archived routing, non-empty reopen targets, dual-empty reopen fallback, pinned reopen attempt cap with per-turn reset, `settlement:{war_id}:{turn}:{seq}` route ids, deterministic merge-conflict handling, AI cross-war retry bounds if SC-5 reverses, and same-war/cross-war settlement-family collision protection), Incoming Offers (SC-5/6/7 default defer + hide unless explicitly reversed, with backend handler no-pop/no-stage defensive deferral, enumerated no-exposure coverage across notification rail / popup queue / pending-envoy / mailbox-activate / dispatch / Godot routes / producer soak and offer identity/package preservation plus schema pinning for accept/reject/request if implemented), then Presentation and Metadata (SC-15/15b/16/17/19/20/23/24/25, including pinned `applied_clauses_preview[]` shape, preview/mutation equivalence, third-party reaction preview, ratification-time acceptance snapshots, failed-ratification producer guard, backend blocked-acceptance payload suppression, settlement vocabulary surface/file classifier, reverse-chronological merged peace/settlement history, canonical clause humanization across all eight clause types, Voice Bible section 16.1 authored settlement families with trigger/surface routing, bilateral history producer namespace, old-save vocabulary/legacy-term policy, and raw + clamped harshness fields with raw-reader consumers). Gate 1 now also requires mandatory audit-history consolidation or non-normative relabel/index before implementation checklists may cite the spec. `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` carries inline supersession markers for old route-id, incoming-offer Slice F, and cross-war queue/defer collision instructions per SC-27, with the v0.18 doc-scan token list. Slice G remains blocked until the full treaty cleanup is implemented, behavior-tested, and manually smoked.
> **Last Updated:** May 7, 2026 (**SETTLEMENT UI CLEANUP SPEC v0.18 POST-COMMIT READINESS FOLD-IN COMPLETE** - `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` now carries post-commit tightenings for preview pending/failure UX, EDIT/REVIEW control-state matrix, two-way negotiation deferral framing, required first-slice clause-floor wording, mandatory audit-history consolidation/indexing, and STATUS current-gate alignment. The spec is ready for implementation planning, but implementation remains NO-GO until Gate 1 doc scans pass and Gate 2 Slice 1 starts from the ordered Foundation path with behavior tests before any presentation polish or Slice G work.)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Last confirmed backend baseline:** `9755 passed, 1 skipped`; `ruff check backend/` clean after Imperial Settlement audit fixes. Current changes are docs-only and have not rerun the suite. Godot runtime smoke remains manual-only unless a Godot executable is available. |
| **Current Phase** | **Peace Deals - Settlement UI Cleanup v0.18.** Slice F UI reachability exists, but player-readiness is now blocked on the cleanup spec: full treaty authoring, edit/review mode controls, acceptance and hard-stop mutation gates, route safety, incoming-offer hide-or-complete behavior, presentation/metadata cleanup, behavior tests, and manual smoke. Do not start Slice G AI/ally settlement agency until this cleanup is implemented, behavior-tested, and smoked. |
