...
| **Current Phase** | **Peace Deals - Imperial Settlement final closure.** A1-E backend/presentation work is landed, but final player-readiness now requires Slice F UI/UX closure before smoke: wizard `open_settlement` routing, read-only lazy-backfill availability, free opener cost, humanized disabled reasons/terms/standing, readable `settlement_confirm` popup, war-detail/coalition-detail CTAs, war-status no-instance placeholder, ledger focus/humanization, and focused tests. After Slice F smoke, Slice G owns AI/ally settlement agency: AI war-leader `incoming_settlement_offer` plus non-leader ally petitions/advisories without veto or treaty authorship. |
