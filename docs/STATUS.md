...
> **Last Updated:** April 28, 2026 (**IMPERIAL SETTLEMENT SPEC v1.9 HANDOFF PRECISION CLOSURE READY** - `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` are aligned for Slice A coding after the Codex/Claude full-audit synthesis, follow-up clarification patch, and implementation-seam audit. Closure hardening now explicitly names the live battle writer `diplomacy.py::record_battle()`, `WorldState._ratify_treaty()` support emission, 9-parameter cascade `war_id` threading, current-turn/mailbox `incoming_settlement_offer` taxonomy, common-peace cooldown namespace, bargain `dormant` vs `at_risk`, contribution retention/compaction, and synthetic full-Europe fixture requirements.)
...
| **Current Phase** | **Peace Deals — Ally Participation spec v1.9 / implementation plan v1.6 READY FOR SLICE A (April 28, 2026).** BPH-A through BPH-D, WPS-A through WPS-D, WB-A, WB-B, WB-C, and WB-D all landed. `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` now own the full-Europe handoff after combined Codex/Claude synthesis and implementation-seam closure; coding may start with Slice A. Map-readiness non-art closure remains complete and art-blocked only. |
...
| 4 | Ally Participation + Common Peace | ally beneficiaries, contribution / consultation rights, common peace routing, settlement fallout | Post-bargains track. `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.9 and implementation plan v1.6 own the full-Europe handoff; next work is Slice A. |
...
**Peace Deals spec gate (Apr 28):** BPH-A through BPH-D, WPS-A through WPS-D, and WB-A through WB-D are landed. Ally Participation/Common Peace now has `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.9 and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.6 covering full-Europe scale after combined Codex/Claude synthesis and implementation-seam closure; next is Slice A coding.
