...
> **Current Settlement Gate (May 7, 2026):** G2-Slice-1 Foundation, G2-Slice-2 Entry Safety, and the Codex audit-fix passes are LANDED on `codex/settlement-spec-readiness-tightening` through commit `32b868c` (`Fix settlement entry safety audit gaps`). This is a backend Gate 2 cleanup GO for foundation + entry safety, not full Settlement UI Cleanup closure. Slice G / AI-ally settlement agency remains blocked until the remaining Gate 2 cleanup slices are implemented, behavior-tested, Godot-wired where required, and manually smoked. Full Treaty Settlement remains approved: author concrete war-scoped terms, preview consequences and acceptance, revise/re-author, preserve incoming-offer packages only if offers ship, and ratify only after acceptance/hard-stop gates pass. **Rejected-settlement / losing-side peace review is now folded into `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` and keeps implementation at NO-GO:** the disabled-Ratify patch alone still leaves the popup with only `Back Out`; the root cause is that the prior `Revise Terms` fix solved command fallback but never shipped editor authoring or a disabled placeholder, then hiding Revise removed the only possible next-step affordance. Before coding this UX, consume the new P0/P1 fold-in: visible disabled Ratify but absent from `available_action_ids[]`, cause-specific rejected/blocked next-step taxonomy, no `Wait for Enemy Offer` button while SC-5 is deferred, an explicit armistice fallback bridge before showing that button, concession as authoring direction over canonical clauses, blocked/rejected voice and numeric-score cleanup, debug-copy removal, row-collapse coverage, and a rejected/losing smoke-start variant. SC-4b remains the active resource-cost contract: settlement entry, preview, Submit, Revise Terms, Back Out, failed ratification, stale recovery, hard-stop refusal, and successful ratification default to 0 DP/AP/gold unless a later product decision records a non-zero `confirm_settlement` cost in both spec and STATUS.
...
> **Last Updated:** May 7, 2026 (**REJECTED-SETTLEMENT / LOSING-SIDE PEACE REVIEW FOLDED IN — IMPLEMENTATION NO-GO** — the dedicated review found the contract direction correct but too thin to implement safely. `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` now records the concrete NO-GO findings and fixes: disabled-Ratify must not leave only Back Out; Revise Terms missed UI because the previous work fixed routing rather than authoring; rejected and blocked failures need different next-step advice; armistice fallback needs a target/draft-preservation bridge; AI-offer waiting is non-actionable while SC-5 is deferred; concession is clause direction/presentation over the canonical schema, not a fake new type; blocked/rejected Godot copy must stop saying "Will they accept?", suppress `0 / 50` hard-stop score copy, and remove developer payload text; row collapse and rejected/losing smoke coverage are now required follow-up blockers. Previous Gate 2 implementation status remains: commits `0761284` and `32b868c` landed G2-Slice-2 entry safety and audit fixes; verification was `tests/test_settlement_entry_safety_slice.py` `25 passed`, settlement foundation/UI behavior bundle `62 passed`, full suite `9858 passed, 1 skipped`, and `ruff check backend tests` clean.)
