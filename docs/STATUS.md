...
> **Last Updated:** May 2, 2026 (**IMPERIAL SETTLEMENT SLICE B2 BATTLE EMITTER WIRING REVIEW FIXES LANDED** - B2 battle call-site wiring is now closed. `detect_battle_theater(...)` preserves same-nation secondary marshal theater strength by replacing only the named primary combatant's post-battle strength with captured pre-battle strength; `_post_combat_pipeline()`, `_execute_attack()` inline, and `_process_reckless_cavalry_turn_start()` pass primary marshal names plus the four theater payload fields into `record_battle()`. Test coverage now includes helper multi-marshal stacking, glorious-charge pipeline, garrison pipeline, bombardment pipeline, theater-`None` legacy fallback, distant-front exclusion, inline attack, and auto-dispatch charge. Verification: full suite `9457 passed, 1 skipped`; ruff clean. **Next:** finish the remaining B2 non-battle emitters and support attribution: occupation events, treaty-clause support emission, British coalition subsidy support emission, support-event ingestion/dedupe, and related fixtures. **After that:** B3 lifecycle / retention / full-Europe fixtures.)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `9457 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend/ tests/`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals - Ally Participation Slice B2 battle emitter wiring LANDED / remaining B2 emitters NEXT (May 2, 2026).** BPH-A through BPH-D, WPS-A through WPS-D, WB-A, WB-B, WB-C, WB-D, Slice A1 foundation, A2 war-entry threading, A3 merge/archive/leader invariants, B1 contribution store/standing math, B2 ordering guard, and B2 battle emitter call-site wiring are landed. Active settlement work is the rest of B2: occupation events, support events, treaty-clause support, British coalition subsidy support attribution, support dedupe, and related fixtures before B3 lifecycle / retention / full-Europe fixtures. Map-readiness non-art closure remains complete and art-blocked only. |
...
**Next diplomacy workflow:** **Remaining Slice B2 non-battle emitters and support attribution.** Slices A1 + A2 + A3 + B1 + B2 ordering guard + B2 battle emitter call-site wiring are landed. Active settlement work continues from `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` Slice B "B2 event emitters and theater attribution" bullets and `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` §9.2 / §9.4. The battle side is closed: `record_battle()` accrues before the 1000-casualty gate, and `_post_combat_pipeline()` / `_execute_attack()` inline / auto-dispatch charge now pass one-hop theater participants, `war_id`, and primary marshal pre-battle strength into contribution accrual. Remaining B2 work: occupation events (enemy region capture, enemy capital capture, allied/liberated restoration), treaty-clause support events, British coalition subsidy support events, support-event ingestion/dedupe, and related fixtures. B3 wires per-turn lifecycle, exit stamping, retention, and full-Europe fixtures after remaining B2 is green.
