...
> **Last Updated:** April 19, 2026 (Scale Readiness Phase 2 CLOSED. Plan §2.1's required 100-region synthetic benchmark now ships as `tools/benchmark_distance_cache.py` and is pinned by three new tests in `test_scale_readiness_phase2.py` (28 tests total, suite green). Measured speedup on a 10x10 4-neighbor grid is ~34x cached vs. uncached, with the adjacency-topology-only invariance contract verified (controller change non-invalidating, explicit invalidation clears). Phase 2 §§2.1-2.3 are now code + correctness + benchmark complete; 0 raw marshal scans remain in `backend/ai/enemy_ai.py`. `STATUS.md` continues to separate "Phase 2 done" from the broader "real map readiness" gate: Phase 3 data pipeline (nation factory, shared topology, color centralization, prompt/parser hardcoding) and Phase 4 map-art pipeline (province registry schema, external bitmap loading, color-map validator, unwired-province support) remain untouched on HEAD and are the next non-art prerequisites for commissioned Europe-map integration.)
...
| **Current Phase** | **Scale Readiness Phase 2 — COMPLETE (April 19, 2026).** Distance caching, indexed AI seams, live AI fog, indexed autonomous evaluation, live-visibility caching, direct marshal-scan cleanup pass, and plan §2.1's 100-region synthetic benchmark (`tools/benchmark_distance_cache.py`, ~34x cached speedup, invariance asserted) are all in. Next track: non-art Real Map Readiness closure — Phase 3 data pipeline + Phase 4 map-art pipeline — per the gate below. |
...
2. **Map readiness closure pass — land the non-art prerequisites for a real Europe map.** Phase 2 completion is necessary but not sufficient. The full pre-art gate captured below is untouched on HEAD: Phase 3 data/config hardcoding closure (factory pattern, shared topology, color centralization, prompt/parser fallback), Phase 4 production province registry + bitmap loader + color-map validator + unwired-province support, and final art-backed smoke once assets exist. See `docs/SCALE_READINESS_PLAN.md` §§3.1-3.4, 4.1-4.4 and `docs/SCALE_READYNESS.md` §§9 + 11.
   - **Suggested sequencing for a cold-start session:** start with the cheap items 3.3 (centralize nation colors) + 3.4 (prompt/parser fallback) as a warm-up bundle, then 3.2 (shared topology endpoint) so Phase 4 can build on a single source of truth, then 3.1 (nation config factory — biggest data-model change), then 4.1 (province registry schema) → 4.2 (external bitmap loading) → 4.3 (color-map validator) → 4.4 (unwired province support). Plan each item against its own spec section before coding; do not bundle 3.1 or 4.1 with anything else. A fresh session only needs CLAUDE.md + this STATUS.md + `SCALE_READINESS_PLAN.md` §§3-4.
