...
> **Last Updated:** April 27, 2026 (**WB-C COMPLETE** — War-entry integration, Bargain Review, AI rules, and `repudiate_bargain` action. `compute_war_entry_score` implements the full §9.4 formula (base 20, treaty depth +10/+18, defensive honor +18, hostility, opposition +6, France relation, betrayal strikes -8 each capped -24, reliability, bargain bonus +25, war load -8/-18). Hard blocks gate on armistice, at_war, anti-coalition, hard_reject posture, and participation path. `build_join_opportunity` / `resolve_join_opportunity` manage ally-entry decisions with reroll determinism via input-hash comparison. `build_declaration_preview` intercepts war declarations to surface opportunities and bargain warnings. Counter-bargain flow handles the 25-49 band with `generate_counter_bargain` / `accept_counter_bargain`. `build_bargain_review` provides forecast band, decisive flag, and contradiction warnings. `repudiate_bargain` action wired through all 10+ files per CLAUDE.md pattern. AI rules: `ai_should_propose_bargain` with feasibility gates + anti-spam, `ai_evaluate_war_entry` with hard-block integration. Diplomatic ledger gains a `war_bargains` section via `get_live_bargains_for_ledger`. 56 new tests in `tests/test_wb_c_war_entry.py`. Verification: full suite `9230 passed, 1 skipped`; ruff green. **Next: WB-D.**)
...
| **Current Phase** | **Peace Deals — War Bargains WB-C complete (April 27, 2026).** BPH-A through BPH-D, WPS-A through WPS-D, WB-A, WB-B, and WB-C all landed. `threat_coalition` merged into `balance_of_europe`. Next: WB-D bargain presentation. Map-readiness non-art closure remains complete and art-blocked only. |
...
**Next diplomacy workflow:** **War Bargains WB-C COMPLETE.** BPH-A through BPH-D + WPS-A through WPS-D + WB-A/WB-B/WB-C all landed. Gate 1 passed. `threat_coalition` merged into `balance_of_europe`. Next is **WB-D** — bargain presentation extension (response routes, bargain icons, fulfillment/breach badges). Follow `docs/PEACE_DEALS_UMBRELLA_SPEC.md` slice order.
...
| 3.5 | War Bargains | `R151`, full `war_bargain` mechanic split out of `Reliability + Commitments` v1.0 in April 16 rescope | The bilateral named-enemy promise mechanic. `docs/WAR_BARGAIN_SPEC.md` v1.0. Slices WB-A (data + creation), WB-B (lifecycle), and WB-C (war-entry contract + AI rules + `repudiate_bargain`) are landed; next is WB-D (presentation extension). **Depends on items 1-3.** ~130 tests. |
...
**Peace Deals spec gate (Apr 26):** CLOSED by Run #5 GO. The dedicated Peace Deals umbrella now stitches together Bilateral Peace Hardening, War Purpose + Score Semantics, `WAR_BARGAIN_SPEC.md`, and Ally Participation/Common Peace. **BPH-A through BPH-D and WPS-A audit follow-up landed; next is WPS-B READY.**
