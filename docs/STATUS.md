...
> **Last Updated:** May 2, 2026 (**IMPERIAL SETTLEMENT SLICE B2 ORDERING GUARD REVIEW FIXES LANDED** - follow-up fixes for the `record_battle()` settlement-accrual ordering guard now make `accrue_battle_contribution(...)` store normalized 40-point battle-bucket values per spec §9.2, filter explicit theater participants to active same-side war participants before computing side strength, and enforce `joined_turn <= event.turn <= exited_turn` before mutating episode buckets. `tests/test_war_contribution_scores.py` adds regressions for weighted bucket storage, malformed explicit participant lists, and closed-episode turn-window skips. The `record_battle()` call remains AFTER the `is_at_war` precondition and BEFORE the 1000-casualty war-score early return. Verification: full suite `9441 passed, 1 skipped`; ruff clean. **B2 emitter call-site wiring remains the next sub-gate** — theater-aware updates to `_post_combat_pipeline()` / `_execute_attack()` inline path / auto-dispatch charge path / glorious-charge pipeline, plus occupation events, treaty-clause / British-subsidy support events, and one-hop adjacency participant detection.)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `9350 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend/ tests/`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals - Ally Participation Slice A2 LANDED / A3 NEXT (May 2, 2026).** BPH-A through BPH-D, WPS-A through WPS-D, WB-A, WB-B, WB-C, WB-D, Slice A1 foundation, and Slice A2 war-entry threading are landed. A2 threads `war_id` ownership through inventoried WAR-entry seams and review follow-up commit `497791d` closes the remaining hard-stop gaps. Active settlement work is Slice A3: connected-component merge, absorbed-reference rewrites, leader replacement, elimination exits, terminal retention/archive compaction, and always-on post-merge invariants. Map-readiness non-art closure remains complete and art-blocked only. |
...
**Next diplomacy workflow:** **Slice B2 — emitter call-site wiring (post-ordering-guard).** Slices A1 + A2 + A3 + B1 + B2 ordering guard are landed. Active settlement work continues from `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` Slice B "B2 event emitters and theater attribution" bullets and `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` §9.2 / §9.4. The ordering guard now pins `accrue_battle_contribution(...)` BEFORE the 1000-casualty war-score early return in `backend/game_logic/diplomacy.py::record_battle()`. Remaining B2 work: theater-aware updates to the three diplo `record_battle` callers (`backend/commands/combat_executor.py::_post_combat_pipeline()`, `_execute_attack()` inline path, and `backend/models/world_state.py` auto-dispatch charge path) so they pass `attacker_participants` / `defender_participants` / `nation_theater_strength` derived from one-hop adjacency; occupation events (enemy region capture, enemy capital capture, allied/liberated restoration); treaty-clause and British coalition subsidy support events; and the glorious-charge pipeline fixture. The B1 module already exposes the canonical store, episode lifecycle, share math, old-record adapter, and standing classifier; B3 wires per-turn lifecycle and exit stamping.
