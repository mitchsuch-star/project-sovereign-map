...
> **Last Updated:** April 22, 2026 (Memory and Pressure v2.4.3 combined §7.3 + §11.1 audit closeout. After folding the latest external pass into the local doc/code-backed audit, recommendation is **KEEP AS WRITTEN** for the audited sections. No new doc-level must-fixes: the `33 / 50 / 60` ladder/beat drama, four-surface naming scope, D3 re-deferral, non-France-hegemon suppress-only honesty rules, three-step voice fallback, same-band hegemon-swap beat, and declaration-turn bloc-vs-coalition contrast remain coherent across `RELIABILITY_COMMITMENTS_SPEC.md`, `COMMITMENTS_PRESENTATION_SPEC.md`, `RELIABILITY_IMPLEMENTATION_PLAN.md`, `DIPLOMAT_VOICE_BIBLE.md`, and `COALITION_SPEC.md`. Remaining risks are implementation-side and already owned by the plan/tests: atomic replacement of the legacy `threat_coalition` / `NATION OVERVIEW` / anonymous clue substrate, bloc-member cache invalidation on all five seams, end-of-turn-only relaxation dedupe, recursive vassal walk cycle safety, and the B-B1-lite ↔ B-B4 merge-ordering gate. **Repo reality is still pre-migration:** `backend/game_logic/diplomatic_ledger.py` still returns `threat_coalition`; `backend/game_logic/coalition.py` still emits `Diplomatic Tension` / `European Courts Concerned`; `backend/game_logic/diplomacy.py` still computes legacy `threat_modifier` / `coalition_penalty` while `hegemony_pressure` is only a display rename. Start implementation with the planned substrate swap (B-Hegemony → B-B1-lite → B-B3/B-B7/B-B4 → C-lite), not with further spec tightening.)
