...
> **Last Updated:** April 19, 2026 (Map-readiness cross-file drift-guard hardening pass. Four non-behavioral follow-ups from the whole-of-map-readiness audit (commit `da9ee21`) now that §§3.1-3.4 + §§4.1-4.4 are all complete: (1) `tests/test_map_placeholder_assets.py::test_every_backend_region_is_wired_in_registry` asserts every region in `backend.models.region.REGIONS_DATA` has `wired: true` in `session8_placeholder_provinces.json` — the existing set-equality test wouldn't catch a future `wired: false` flip that would let backend AI / ledger / dispatch treat a region as in-play while the renderer §4.4 gate blocks clicks on it. (2) `tests/test_map_consistency.py::test_regions_adjacency_is_bidirectional` iterates `REGIONS_DATA` and reports every asymmetric adjacency edge in a single failure message (distinct from the existing `test_adjacency_is_bilateral` which bails on the first mismatch). Does NOT auto-fix `REGIONS_DATA`; the user triages. (3) `docs/ROADMAP.md` §4 item #8 "Greyed-out unplayable provinces" no longer reads "Shader desaturates/dims them" — rewritten to describe the CPU-side `Image.lerp` overlay that §4.4 actually shipped in `_apply_unwired_grey_overlay()` (no shader). (4) `docs/ADDING_CONTENT.md` §3 "Expanding the Map" gained a new "Province Registry (schema v2)" subsection documenting `session8_placeholder_provinces.json`, the `schema_version: 2` field contract, a minimum valid entry, registry-only + full-delivery validator invocations, the two art-acceptance thresholds (`--min-coverage-pixels=50` and `--tiny-island-threshold=5`), and a cross-reference to `test_every_backend_region_is_wired_in_registry`. Full Python suite now `8508 passed, 2 skipped` (was `8506`; +2 new tests). Ruff clean on all modified files.)
...
|-------|------------|------------------------|------|
| 1 | Memory and Pressure v2.4.3 | `R160`, `R119` | First diplomacy follow-up implementation target. Substrate is shipped; the next pass is the audit-block closure set in `docs/audits/MP_V243_BLOCK1_DOC_CLEANUP.md`, `docs/audits/MP_V243_BLOCK2_SUBSTRATE.md`, and `docs/audits/MP_V243_BLOCK3_BLOC_NAMING.md`, then the remaining B-Hegemony / B-B1-lite / B-B3 / B-B7 / C-lite work in `docs/RELIABILITY_IMPLEMENTATION_PLAN.md` v2.4.3 and `docs/COMMITMENTS_PRESENTATION_SPEC.md` v0.5.1. |
...
- Block 3 bloc naming / alignment legibility is third: `docs/audits/MP_V243_BLOCK3_BLOC_NAMING.md` is the queued design gate that decides whether the hegemony layer should gain proper bloc naming before the remaining slices harden in code.
- After the three audit blocks land, the live implementation order is the v2.4.3 plan surface: B-Hegemony, B-B1-lite, B-B3, B-B7, C-lite (trimmed), with B-B4 parallel where called out in `docs/RELIABILITY_IMPLEMENTATION_PLAN.md`.
