...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8572 passed, 2 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Memory and Pressure v2.4.3 — B-Hegemony COMPLETE (April 22, 2026).** Map-readiness non-art closure remains complete and art-blocked only. Next diplomacy work is **B-B1-lite** (`hegemony_target_mod`, acceptance decoupling, proposal-preview `hegemony` warnings), then `B-B3 / B-B7 / B-B4`, then Slice `C-lite`. |
| **Blockers** | No active blocker on `B-B1-lite` beyond normal slice sequencing. The live gates are (a) the `B-B1-lite ↔ B-B4` merge-ordering rule, (b) legacy ledger/UI payloads that remain on `threat_coalition` until `C-lite`, and (c) commissioned-art renderer smoke, which remains blocked on art assets. |
...
1. **Memory and Pressure v2.4.3 — B-Hegemony COMPLETE (April 22, 2026).** The substrate slice is live: bloc geometry, `power_score` / `bloc_power`, `describe_hegemon_bloc`, the `33 / 50 / 60` surfaced-band memory, same-turn `balance_of_europe_shifted` beats, non-France-hegemon threat suppression, AI bandwagon at `50%+`, coalition leadership `bloc_share_against`, and transient/save-field wiring are all in. Review-driven hardening also closed the stale cache bug on vassal removal / nation elimination and strengthened the direct regressions in `tests/test_hegemony_engine.py`. Verification: `51 passed` in the hegemony suite, unchanged ledger/UI compatibility tests still green, full suite `8572 passed, 2 skipped`.
2. **Memory and Pressure v2.4.3 — B-B1-lite NEXT.** Ship `hegemony_target_mod` + acceptance decoupling + proposal-preview `hegemony` warnings on top of the now-live helpers/state, without leaking B-B4's grievance/composite-floor work. After `B-B1-lite`, continue with `B-B3 / B-B7 / B-B4` per the merge gate, then Slice `C-lite` for the `Balance of Europe` ledger/payload swap.
