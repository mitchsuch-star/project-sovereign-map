...
**Next diplomacy workflow:** **Ally Participation / Common Peace Slice A3 merge/archive/leader invariants.** BPH-A through BPH-D + WPS-A through WPS-D + WB-A/WB-B/WB-C/WB-D + Slice A1 foundation + Slice A2 war-entry threading all landed. Gate 1 passed. `threat_coalition` merged into `balance_of_europe`. War Bargains track fully closed. A2 now threads `war_id` across all inventoried WAR-entry seams and hard-stops merge-required cases. Active settlement work continues from `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` Slice A "A3 merge/archive/leader invariants" bullets: connected-component merge, absorbed-reference rewrites against current containers plus no-op-safe future hooks, leader replacement, elimination exits, terminal retention/archive compaction, and always-on post-merge invariants.
