...
> **Current Settlement Gate (May 8, 2026):** Settlement UI Cleanup implementation commits exist as referenced objects (`b699f4c`, `4310903` / `0761284`, `052c5d0`, `4884e93`, and `b2a4d7b`), but the current `codex/settlement-smoke-start` branch is a v0.22 docs/smoke-start repair branch and `b2a4d7b` is not an ancestor of `HEAD`. The active gate is therefore **spec v0.22 Codex-Claude UX synthesis verification after the latest re-audit fixes and Deferred Work Landing Ledger, SC-27 doc-scan across the implementation plan and parent settlement spec, SC-28 / SC-28b rejected/losing recovery verification, branch-target reconciliation, and a recorded Pre-Implementation Spec Review Gate result before Gate 4 manual smoke**. Do not restart old G2-Slice-1 work from this docs branch; first choose the smoke/integration target where the landed implementation commits are reachable or explicitly record that they must be merged/cherry-picked. Slice G / AI-ally settlement agency remains blocked until the updated `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.22 checks are consumed, the rejected/losing-side smoke path passes, and SC-29 / SC-30 / SC-31 / SC-32 landing ownership remains tracked rather than open-ended.
> **Manual settlement smoke shortcuts:** before Gate 4 manual settlement UI smoke, set `SOVEREIGN_SMOKE_START=settlement_multilateral` for the shared France vs Britain + Prussia war. Use `SOVEREIGN_SMOKE_START=settlement_losing` or `settlement_rejected` for the v0.22 blocked-popup, SC-28 / SC-28b recovery, scoped-draft, no-direct-pair-action, and concession path; if neither variant is implemented, Gate 4 remains NO-GO and must record the missing fixture instead of substituting the happy path.
> **Last Updated:** May 8, 2026 (**SETTLEMENT SPEC v0.22 CODEX-CLAUDE UX SYNTHESIS — RE-AUDIT FIXES + DEFERRED WORK LANDING LEDGER FOLDED / SPEC READINESS NO-GO UNTIL REVIEW TRACEABILITY + VERIFICATION + SMOKE** — the latest Codex and Claude review items plus the re-audit fixes are folded into canonical rules: blocked ratification omits `confirm_settlement` from `options[]` and `available_action_ids[]` rather than rendering disabled Ratify; direct `Seek Armistice Instead`, `Seek Bilateral Peace`, `Wait for Enemy Offer`, `Ask for terms`, and `Surrender terms` controls remain absent only while their concrete landing rows SC-29 / SC-30 / SC-31 / SC-32 are unshipped; valid below-threshold drafts can Submit into blocked REVIEW but cannot Ratify; War Detail recovery is a live-context route, not a promise of pair actions; scoped drafts use a stable `draft_key` rather than raw `war_id`; preview/review payload schemas now require concrete player-comprehension sections; forced-alliance is demand-only in the first cleanup slice; losing-side concessions are player-authored canonical clauses with deterministic Talleyrand baseline; SC-27 doc scans now include rejected-settlement false-affordance phrases, incoming-offer smoke-branch wording, and backlog-copy ownership. Next: record the Pre-Implementation Spec Review Gate result for v0.22, verify doc scans, reconcile the implementation branch target, and run Gate 4 manual smoke including the rejected/losing variant.)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Last confirmed backend baseline in this status file:** `9755 passed, 1 skipped`; `ruff check backend/` clean after Imperial Settlement audit fixes. Later implementation commits are referenced above but are not reachable from this docs branch, so their recorded test baseline must be verified on the chosen integration/smoke target. Current v0.22 changes are docs-only and have not rerun the suite. Godot runtime smoke remains manual-only unless a Godot executable is available. |
| **Current Phase** | **Peace Deals - Settlement UI Cleanup v0.22 Codex-Claude UX synthesis + re-audit fix verification + Deferred Work Landing Ledger + branch-target reconciliation + Gate 4 smoke.** The next work is to consume the v0.22 spec fixes, record the Pre-Implementation Spec Review Gate result with review-session reference, verify STATUS/SC-27 scans across both settlement plan/spec docs, choose or create the branch where the implementation commits are reachable, verify SC-28 / SC-28b rejected/losing recovery, and run manual smoke including the rejected/losing concession path. Do not restart old G2-Slice-1 work from this docs branch, and do not start Slice G AI/ally settlement agency until v0.22 is reconciled, Gate 4 smoke evidence is recorded, and SC-29 / SC-30 / SC-31 / SC-32 ownership remains explicit rather than open-ended. |
