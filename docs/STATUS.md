...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8849 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend tests`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals Phase A — BPH-B COMPLETE + REVIEW FOLLOW-UP CLOSED (April 26, 2026).** Term ownership/display labels and the peace preview/war-context snapshot are landed and verified. Memory and Pressure v2.4.3 fully closed. Next coding slice is **BPH-C READY** (Fallout preview + commitment conflicts). Map-readiness non-art closure remains complete and art-blocked only. |
...
**Next diplomacy workflow:** **BPH-B COMPLETE + review follow-up closed.** Next is **BPH-C READY: Fallout preview + commitment conflicts (~20 tests)** — separate-peace ally fallout warnings, `get_peace_commitment_conflicts()` interface, strategic-order cancellation preview, Section 3 warning rendering in the peace preview, and ratification relation penalties. Follow `docs/PEACE_DEALS_UMBRELLA_SPEC.md` slice order. BPH and WPS slices can be interleaved.
...
**Peace Deals spec gate (Apr 26):** CLOSED by Run #5 GO. The dedicated Peace Deals umbrella now stitches together Bilateral Peace Hardening, War Purpose + Score Semantics, `WAR_BARGAIN_SPEC.md`, and Ally Participation/Common Peace. **BPH-A and BPH-B landed; next is BPH-C READY.**
