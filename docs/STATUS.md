...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8625 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Memory and Pressure v2.4.3 — B-Hegemony + B-B1-lite + B-B3 COMPLETE (April 23, 2026).** Map-readiness non-art closure remains complete and art-blocked only. Next diplomacy work is **B-B7** (Make Amends active-redemption verb, standard variant only), then `B-B4` (DG-4 follow-through, merge-gated AT or AFTER B-B1-lite — now satisfied), then Slice `C-lite`. |
| **Blockers** | No active blocker on `B-B7`. The live gates are (a) the `B-B1-lite ↔ B-B4` merge-ordering rule (B-B1-lite is landed so B-B4 is now unblocked on that axis), (b) legacy ledger/UI payloads that remain on `threat_coalition` until `C-lite`, and (c) commissioned-art renderer smoke, which remains blocked on art assets. |
...
1. **Memory and Pressure v2.4.3 — B-Hegemony + B-B1-lite + B-B3 COMPLETE (April 22-23, 2026).** B-Hegemony substrate is live (bloc geometry, `power_score` / `bloc_power`, `describe_hegemon_bloc`, `33 / 50 / 60` surfaced-band memory, `balance_of_europe_shifted` beats, non-France-hegemon threat suppression, AI bandwagon at `50%+`, coalition leadership `bloc_share_against`, transient/save-field wiring). B-B1-lite (commits `5a5a265` + `ad3b582` + `341b22a`) shipped `hegemony_target_mod` + acceptance decoupling + proposal-preview `hegemony` warnings without leaking B-B4's grievance/composite-floor work. B-B3 retired the last alliance_paradox surface seams: deleted legacy `alliance_paradox_popup.{tscn,gd,gd.uid}` from Godot, updated `dialog_manager.gd:21` layer-registry comment, cleared the obsolete `TestM10AllianceParadoxTodo` skip stub, and added 9 focused regressions in `tests/test_commitment_paradox_rename.py` covering rename smoke (canonical dialogue type + popup field emit), alias load gate (legacy save key migration + legacy dialogue type hard-stop replay), and no-double-emit (single dialogue, canonical-only popup slot, no legacy key in `to_dict`). Full suite `8625 passed, 1 skipped`. Ruff clean on touched files.
2. **Memory and Pressure v2.4.3 — B-B7 NEXT.** Ship the Make Amends active-redemption verb (standard strike-clearing variant only; grievance variant stays with B-B4 per spec §8.6.1a). Adds `make_amends` to `VALID_ACTIONS`, `_execute_make_amends` in `diplomatic_executor.py` (200g + 1 DP, oldest-strike-first removal, reliability +2, relation +5, 10-turn shared cooldown, named-diplomat acknowledgment on success, Talleyrand-voiced refusal on fail), parser keywords, `AMENDS_REFUSAL_DISPLAY` + `ACTION_DISPLAY` maps, `reparations_cooldown: Dict[str, int]` field with round-trip. ~8 tests per plan §B-B7. After `B-B7`, continue with `B-B4` (DG-4 follow-through, now unblocked — B-B1-lite merge gate is satisfied), then Slice `C-lite` for the `Balance of Europe` ledger/payload swap.
