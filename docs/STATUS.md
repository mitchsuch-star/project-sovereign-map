...
> **Current Settlement Gate (May 11, 2026):** Settlement UI Cleanup implementation commits exist as referenced objects (`b699f4c`, `4310903` / `0761284`, `052c5d0`, `4884e93`, and `b2a4d7b`), but the current `codex/settlement-smoke-start` branch is a v0.23 docs/smoke-start repair branch and `b2a4d7b` is not an ancestor of `HEAD`. The active gate is therefore **combined Codex + Claude pre-implementation review synthesis for `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.23, SC-27 doc-scan across the implementation plan and parent settlement spec, executable Godot evidence for SC-1 / SC-10b / SC-28 before smoke, SC-28 / SC-28b rejected/losing recovery verification, branch-target reconciliation, and a recorded Pre-Implementation Spec Review Gate result before Gate 4 manual smoke**. Do not restart old G2-Slice-1 work from this docs branch; first choose the smoke/integration target where the landed implementation commits are reachable or explicitly record that they must be merged/cherry-picked. Slice G / AI-ally settlement agency remains blocked until the updated v0.23 checks are consumed, the rejected/losing-side smoke path passes, and SC-29 / SC-30 / SC-31 / SC-32 / SC-33 landing ownership remains tracked rather than open-ended.
> **Manual settlement smoke shortcuts:** before Gate 4 manual settlement UI smoke, set `SOVEREIGN_SMOKE_START=settlement_multilateral` for the shared France vs Britain + Prussia war. Use `SOVEREIGN_SMOKE_START=settlement_losing` or `settlement_rejected` for the v0.23 blocked-popup, SC-10b / SC-28 / SC-28b recovery, scoped-draft, no-direct-pair-action, concession-baseline, and Covered Enemies picker path; if neither variant is implemented, Gate 4 remains NO-GO and must record the missing fixture instead of substituting the happy path.
> **Last Updated:** May 11, 2026 (**SETTLEMENT SPEC v0.23 CODEX + CLAUDE REVIEW SYNTHESIS PATCHED / SPEC READINESS STILL NO-GO UNTIL SC-10b + DOC SCANS + EXECUTABLE VERIFICATION + SMOKE** — Codex and Claude findings are folded into canonical rules: War Detail pair-scoped Bilateral Peace / Armistice controls are a new SC-10b blocker before `Open War Detail` can be treated as blocked-review recovery; blocked ratification omits `confirm_settlement`; direct `Seek Armistice Instead`, `Seek Bilateral Peace`, `Wait for Enemy Offer`, `Ask for terms`, and `Surrender terms` controls remain absent while SC-29 / SC-30 / SC-31 / SC-32 are unshipped; `gold_per_turn` is hidden under SC-33; dependency clauses have SC-31 interim-hide artifacts; `can_edit_terms`, `settlement_reopen_attempts`, bilateral `participants_signature`, Replace-this-clause, conflict-matrix, POST-preview trigger, selected-target invalidation, and incoming-offer deferred-dialogue behavior are now pinned; parent fixed-action, typed-command, and incoming-offer plan lines are superseded. Scores remain below gate due unresolved P0/P1s. Next: run the v0.23 SC-27 doc scan, reconcile the implementation branch target, record per-slice SC-22 Godot evidence or NO-GO reopen status, prove SC-10b before using `Open War Detail` recovery, and run Gate 4 manual smoke only after pre-smoke executable evidence is current.)
> **Pre-Implementation Spec Review Gate Result:** May 11, 2026 combined Codex + Claude synthesis, target branch `codex/settlement-smoke-start` at docs branch state before this commit, verdict **NO-GO**. Blocking minimum edits now folded into v0.23 were SC-10b War Detail pair substitutes, `can_edit_terms` source of truth, reopen-attempt world storage, concession baseline trigger, Replace-this-clause contract, bilateral `participants_signature`, `open_war_detail` handler/fallback, named conflict matrix, hidden canonical-clause artifacts, `gold_per_turn` landing ownership, typed-command supersession, pending incoming-offer defensive drop, selected-target invalidation, and STATUS/SC-27 v0.23 alignment. Remaining gate work is verification, branch reconciliation, executable evidence, and smoke, not implementation restart from this docs branch.
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Last confirmed backend baseline in this status file:** `9755 passed, 1 skipped`; `ruff check backend/` clean after Imperial Settlement audit fixes. Later implementation commits are referenced above but are not reachable from this docs branch, so their recorded test baseline must be verified on the chosen integration/smoke target. Current v0.23 changes are docs-only and have not rerun the suite. Godot runtime smoke remains manual-only unless a Godot executable is available. |
| **Current Phase** | **Peace Deals - Settlement UI Cleanup v0.23 combined Codex-Claude pre-implementation synthesis + SC-27 doc-scan + executable pre-smoke verification + branch-target reconciliation + expanded Gate 4 smoke.** The next work is to consume the May 11, 2026 spec fixes, record the Pre-Implementation Spec Review Gate result with review-session reference, verify STATUS/SC-27 scans across both settlement plan/spec docs, choose or create the branch where the implementation commits are reachable, verify SC-1 editor floor plus SC-10b / SC-28 / SC-28b rejected/losing recovery with executable evidence rather than tooling-block notes alone, and run manual smoke including the rejected/losing concession path. Do not restart old G2-Slice-1 work from this docs branch, and do not start Slice G AI/ally settlement agency until v0.23 is reconciled, Gate 4 smoke evidence is recorded, and SC-29 / SC-30 / SC-31 / SC-32 / SC-33 ownership remains explicit rather than open-ended. |
...
| **Blockers** | Settlement UI Cleanup remains NO-GO until the combined v0.23 Pre-Implementation Spec Review Gate result, SC-27 scans, SC-1 / SC-10b / SC-28 executable evidence, per-slice SC-22 Godot parse/load or explicit NO-GO tooling-block accounting, SC-28 / SC-28b recovery evidence, Deferred Work Landing Ledger ownership check, branch-target reconciliation, and expanded 18-step Gate 4 smoke, including rejected/losing smoke, are recorded. Commissioned-art renderer smoke remains blocked on art assets. |
