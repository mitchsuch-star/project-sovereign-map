...
> **Current Settlement Gate (May 7, 2026):** G2-Slice-1 Foundation, G2-Slice-2 Entry Safety, and the Codex audit-fix passes are LANDED on `codex/settlement-spec-readiness-tightening` through commit `32b868c` (`Fix settlement entry safety audit gaps`). This is a backend Gate 2 cleanup GO for foundation + entry safety, not full Settlement UI Cleanup closure. Slice G / AI-ally settlement agency remains blocked until the remaining Gate 2 cleanup slices are implemented, behavior-tested, Godot-wired where required, and manually smoked. Full Treaty Settlement remains approved: author concrete war-scoped terms, preview consequences and acceptance, revise/re-author, preserve incoming-offer packages only if offers ship, and ratify only after acceptance/hard-stop gates pass. Rejected settlement confirmation must not strand the player on a lone `Back Out`: blocked ratification needs a visible disabled reason and a real next step such as working term revision, valid armistice fallback, or clearly deferred AI-offer/concession routes. Losing a war must still permit peace-seeking agency through concessionary/surrender/ask-for-terms packages once treaty authoring ships; "not winning" cannot mean "no peace UI." SC-4b remains the active resource-cost contract: settlement entry, preview, Submit, Revise Terms, Back Out, failed ratification, stale recovery, hard-stop refusal, and successful ratification default to 0 DP/AP/gold unless a later product decision records a non-zero `confirm_settlement` cost in both spec and STATUS.
> **Manual settlement smoke shortcut:** before Gate 4 manual settlement UI smoke, set `SOVEREIGN_SMOKE_START=settlement_multilateral` when you want a fresh new game to start directly in one shared France vs Britain + Prussia `war_instance` with Britain as defender leader. Leave it unset for the canonical campaign start.
> **Last Updated:** May 7, 2026 (**G2-SLICE-2 ENTRY SAFETY + AUDIT FIXES LANDED / NEXT GATE 2 CLEANUP SLICE READY** — commits `0761284` and `32b868c` bring in Settlement UI Cleanup G2-Slice-2 and close the follow-up Codex audit findings. Implemented: multi-war diplomacy-wizard disambiguation, typed/free-text no-`war_id` multi-war rejection, explicit `war_id` bypass, active hostile-pair settlement eligibility for war-status rows with `ignore_active_dialogue=True`, fail-closed war-status eligibility exception handling, per-row `settlement_eligibility` plus disabled reason fields for Godot coalition/focused-row gating, `selected_target_nation` threading through staging/reopen/executor paths, selected-target validation against covered enemy scope, no-empty-covered-scope fail-closed staging, partial-settlement one-active-pair CTA hiding, and humanized display strings for new settlement refusal codes. Verification: `tests/test_settlement_entry_safety_slice.py` `25 passed`; `tests/test_settlement_foundation_slice.py tests/test_settlement_ui_slice_f_behavior.py` `62 passed`; full suite `9858 passed, 1 skipped`; `ruff check backend tests` clean. Confidence: high for backend foundation + entry safety; medium for total Gate 2 cleanup because incoming-offer deferral, active-vs-archived route continuity, settlement-family fallback safety, Godot coalition/detail focused-row UI, presentation/metadata cleanup, and manual smoke remain. **Next:** continue Gate 2 cleanup before Slice G: SC-5/SC-7 incoming-offer defer-and-hide no-leak closure, SC-14 active-vs-archived route continuity, SC-18 settlement-family fallback safety, then Godot coalition/detail focused-row pass for SC-11/SC-11b/SC-12 and presentation/metadata cleanup plus manual smoke.)
...
| **Current Phase** | **Peace Deals - Settlement UI Cleanup v0.19 coding is up next.** Begin with Gate 1 doc scans/consolidation, then G2-Slice-1 Foundation. Slice F UI reachability exists, but player-readiness is now blocked on the cleanup spec: full treaty authoring, edit/review mode controls, acceptance and hard-stop mutation gates, SC-4b resource-cost contract, route safety, incoming-offer hide-or-complete behavior, presentation/metadata cleanup, behavior tests, and manual smoke. Do not start Slice G AI/ally settlement agency until this cleanup is implemented, behavior-tested, and smoked. |
