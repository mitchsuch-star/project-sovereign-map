...
> **Current Settlement Gate (May 6, 2026):** `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.9 is NO-GO audit-complete with the full treaty-settlement path approved and Codex + Claude SPEC READINESS findings folded in. Code that cleanup spec before any Slice G / AI-ally settlement agency work. Quality beats schedule: this must become a player-usable settlement system, not a "wired but not usable" route milestone, and visible broken/misleading behavior cannot be deferred without explicit product approval in status/spec text. This gate was added after the Slice F closeout exposed a player-facing no-op affordance: `Revise Terms` reopened settlement review without any real term-editing flow. The approved target is now real treaty authoring, preview, negotiation/revision, incoming-offer package preservation, acceptance/hard-stop ratification gating, and route-safe review. The cleanup spec now requires Gate 2 to proceed in ordered slices: Foundation (SC-1 through SC-4, including the dispatch empty-terms masquerade at `settlement_presentation.py:494`), Entry Safety (SC-8/8b/10/11/11b/12/13), Continuity (SC-14/14b/14c/14d/14e/7b), Incoming Offers (SC-5/6/7 implement-or-hide), then Presentation and Metadata (SC-15/15b/16/17/19/20/23/24). The cleanup spec tracks adjacent settlement UI gaps: misleading buttons, command-box fallback, generic mailbox fallback, stale popup state, lost `war_id` / `route_id`, one-to-one war common-peace affordances, incoming-offer scaffolding and offer-identity loss, route-id collisions, active-vs-archived review routing, recent-row cap/focus loss, coalition target identity, raw/debug copy, blocked-acceptance copy, dead live-awe preview, settlement history acceptance loss, ledger/dispatch settlement-peace precedence, treaty harshness metadata, source-string-only tests, term-editor vocabulary, GET-vs-POST authoring enforcement, selected-target source of truth, accepting-leader rescore behavior, interim-hide reversal gates, and legacy test inversions. Slice G remains blocked until the full treaty cleanup is implemented, behavior-tested, and manually smoked.
> **Last Updated:** May 6, 2026 (**SETTLEMENT UI CLEANUP SPEC READINESS FOLD-IN COMPLETE** - `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.9 now carries Claude's SPEC READINESS findings and the prior Codex blockers as binding doc requirements. The spec remains NO-GO for implementation until Gate 2 Slice 1 starts from the ordered Foundation path, not presentation polish or Slice G.)
