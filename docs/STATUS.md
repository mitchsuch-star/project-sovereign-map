...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8446 passed, 2 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Map Readiness Closure Pass — Phase 4.1 COMPLETE (April 19, 2026).** Phase 3 (§§3.1-3.4) and Phase 4.1 (province registry schema v2) are all landed suite-green. Phase 2 remains COMPLETE. 8 new placeholder-asset tests + 1 updated renderer-cutover test from §4.1. Next: §4.2 external bitmap loading → §4.3 color-map validator → §4.4 unwired province support. |
...
3. **Map Readiness Closure Pass — §4.1 Province Registry Schema COMPLETE (April 19, 2026).**
   ...
   - **Test coverage.** `tests/test_map_placeholder_assets.py` grew from 4 → 12 tests pinning schema version, `province_id` presence + uniqueness, all four anchor fields, `wired` / `interactive` flag presence + types, current-roster invariants, and renderer-side consumption of both the new flags and the new anchor fields. `tests/test_map_renderer_cutover.py::test_renderer_base_preserves_update_all_regions_contract` was updated for the new `region_full_data = wired_data` assignment and the presence of a `wired` gate inside `update_all_regions()`. Full Python suite: 8446 passed, 2 skipped.
