...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `9162 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend tests`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals — War Bargains WB-B complete (April 27, 2026).** BPH-A through BPH-D, WPS-A through WPS-D, WB-A, and WB-B audit follow-up all landed. `threat_coalition` merged into `balance_of_europe`. Next: WB-C war-entry contract. Map-readiness non-art closure remains complete and art-blocked only. |
...
**Next diplomacy workflow:** **War Bargains WB-B COMPLETE.** BPH-A through BPH-D + WPS-A through WPS-D + WB-A/WB-B all landed. Gate 1 passed. `threat_coalition` merged into `balance_of_europe`. Next is **WB-C** — war-entry contract (`join_opportunity`, declaration preview intercept, offensive ally requests, counter-bargains, coalition overlap, and `repudiate_bargain`). Follow `docs/PEACE_DEALS_UMBRELLA_SPEC.md` slice order.
