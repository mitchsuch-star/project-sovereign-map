...
**Current gate (May 11, 2026):** `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.23 remains **SPEC READINESS NO-GO / IMPLEMENTATION NO-GO** until the follow-up review edits are re-scored, SC-27 doc scans pass on the implementation plan and parent settlement spec, executable Godot evidence exists for SC-1 / SC-10b / SC-28, rejected/losing recovery is verified, branch-target reconciliation is recorded, and Gate 4 manual smoke passes.

**Canonical follow-up edits now folded:** rejected multi-party recovery must offer `re_author_with_concessions` when edit/baseline recovery is available; attempt-4 stale recovery must dismiss or replace the active hard-stop popup so recovery is clickable; Covered Enemies remains a required editor control; discard-confirm has two named actions and outer-cancel keeps the draft; EDIT uses `Submit for Review`, not bare `Submit`; `Open Settlement History` must visually land in Settlement History / `PEACE & SETTLEMENT HISTORY`; settlement summary and digest share the same staged route id; parent leader-change, lazy-backfill, queue/defer, and digest-route instructions are superseded.

## Last Recorded Spec Review Result

**May 11, 2026 follow-up synthesis:** Codex + Claude recommendations folded into the cleanup spec, implementation plan, parent settlement spec, and this STATUS file. Verdict remains **NO-GO** until a new scored review confirms all categories are at least 8/10 and no P0/P1 blockers remain. The previous blocker list folded into v0.23 included SC-10b War Detail pair substitutes, `can_edit_terms`, reopen-attempt world storage, concession baseline trigger, Replace-this-clause, bilateral `participants_signature`, `open_war_detail` handler/fallback, named conflict matrix, hidden canonical-clause artifacts, `gold_per_turn` landing ownership, typed-command supersession, pending incoming-offer defensive drop, selected-target invalidation, and STATUS/SC-27 alignment.
...
Slice G / AI-ally settlement agency remains blocked until the updated v0.23 checks are consumed, the rejected/losing-side smoke path passes, SC-29 / SC-30 / SC-31 / SC-32 / SC-33 landing ownership remains tracked rather than open-ended, SC-27 doc scans pass on current text, SC-22 Godot evidence is recorded per slice, and SC-10b is proven before `Open War Detail` recovery is treated as a valid blocked-review escape.
