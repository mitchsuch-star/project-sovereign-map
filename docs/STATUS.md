...
> **Current Settlement Gate (May 6, 2026):** `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.10 is NO-GO audit-complete with the full treaty-settlement path approved and third-pass SPEC READINESS findings folded in. Code that cleanup spec before any Slice G / AI-ally settlement agency work. Quality beats schedule: this must become a player-usable settlement system, not a "wired but not usable" route milestone, and visible broken/misleading behavior cannot be deferred without explicit product approval in status/spec text. The cleanup spec now requires Gate 2 to proceed in ordered slices: Foundation (SC-1 through SC-4, including clause vocabulary, POST schema, empty-draft rejection, Back Out discard-confirm, acceptance-threshold gating, hard-stop catalog), Entry Safety (SC-8/8b/10/11/11b/12/13, including cached eligibility and selected-target diagnostics), Continuity (SC-14/14b/14c/14d/14e/7b, including dispatch active-vs-archived routing), Incoming Offers (SC-5/6/7 default defer + hide unless explicitly reversed), then Presentation and Metadata (SC-15/15b/16/17/19/20/23/24/25, including settlement vocabulary, merged peace/settlement history, and raw + clamped harshness fields). Slice G remains blocked until the full treaty cleanup is implemented, behavior-tested, and manually smoked.
> **Last Updated:** May 6, 2026 (**SETTLEMENT UI CLEANUP SPEC v0.10 THIRD-PASS READINESS FOLD-IN COMPLETE** - `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` now carries the threshold, clause vocabulary, empty-draft, Back Out, incoming-offer deferral, selected-target, family registration, slice closure, vocabulary, ledger/history, and harshness-scale product decisions as binding doc requirements. The spec remains NO-GO for implementation until Gate 2 Slice 1 starts from the ordered Foundation path, not presentation polish or Slice G.)
