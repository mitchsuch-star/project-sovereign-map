...
> **Current Settlement Gate (May 8, 2026):** Settlement UI Cleanup implementation commits exist as referenced objects (`b699f4c`, `4310903` / `0761284`, `052c5d0`, `4884e93`, and `b2a4d7b`), but the current `codex/settlement-smoke-start` branch is a v0.21 docs/smoke-start repair branch and `b2a4d7b` is not an ancestor of `HEAD`. The active gate is therefore **spec v0.21 repair verification, SC-27 doc-scan across the implementation plan and parent settlement spec, SC-28 rejected/losing recovery verification, and branch-target reconciliation before Gate 4 manual smoke**. Do not restart old G2-Slice-1 work from this docs branch; first choose the smoke/integration target where the landed implementation commits are reachable or explicitly record that they must be merged/cherry-picked. Slice G / AI-ally settlement agency remains blocked until the updated `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.21 checks are consumed, the rejected/losing-side smoke path passes, and smoke evidence is recorded.
> **Manual settlement smoke shortcuts:** before Gate 4 manual settlement UI smoke, set `SOVEREIGN_SMOKE_START=settlement_multilateral` for the shared France vs Britain + Prussia war. Use `SOVEREIGN_SMOKE_START=settlement_losing` or `settlement_rejected` for the v0.21 blocked-popup, SC-28 recovery, and concession path; if neither variant is implemented, Gate 4 remains NO-GO and must record the missing fixture instead of substituting the happy path.
> **Last Updated:** May 8, 2026 (**SETTLEMENT SPEC v0.21 REPAIR CONSOLIDATED — SPEC READINESS NO-GO / IMPLEMENTATION NO-GO UNTIL VERIFICATION + SMOKE** — Claude/Codex review findings were folded into canonical rules without adding audit-history prose: blocked ratification omits `confirm_settlement` from `options[]` and `available_action_ids[]` rather than rendering disabled Ratify; no disabled `Revise Terms` placeholder before an edit-capable route exists; enemy-offer waiting controls remain hidden while SC-5 is deferred; failed settlement review is now owned by SC-28 with real edit / War Detail / Settlement History / terminal close routes; losing-side concessions are owned by SC-1 offer-mode on the standard treaty editor with a deterministic Talleyrand baseline, direction labels, and concession scoring; Voice Bible 16.1 now owns required rejected/loss recovery families. Next: verify v0.21 doc scans, reconcile the implementation branch target, and run Gate 4 manual smoke including the rejected/losing variant.)
...
| **Blockers** | Settlement UI Cleanup remains NO-GO until v0.21 spec checks, SC-27 scans, SC-28 recovery evidence, branch-target reconciliation, and Gate 4 smoke, including rejected/losing smoke, are recorded. Commissioned-art renderer smoke remains blocked on art assets. |
