...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `9311 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend tests`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals - Ally Participation Slice A1 FOUNDATION GATE LANDED (May 1, 2026).** BPH-A through BPH-D, WPS-A through WPS-D, WB-A, WB-B, WB-C, WB-D, and now Slice A1 foundation all landed. A1 ships foundation-only scaffolding per spec v1.24 §0/§7 and implementation plan v1.21: `WorldState.next_war_instance_id`/`war_instances`/`archived_war_instances` containers with `1`/`{}`/`[]` defaults; empty-safe `get_war_instances_by_leader()` + `get_war_instances_by_participant()` with single dirty-flag invalidation hook; cached-helper refactor of the touched `_is_nation_eliminated` path; `assert_war_instance_invariants(...)` in `backend/game_logic/settlement_helpers.py`; 20-active-`war_instance` synthetic fixture in `tests/helpers/full_europe_settlement_fixtures.py` covering the canonical 13 DG-1 roster + 6+ participant coalition side; 17 foundation tests including invariant pass + bad-fixture failure coverage. **No behavioral settlement code shipped** — no declaration/cascade `war_instance` creation, no contribution accrual, no common-peace scoring, no settlement reactions. Slice A2 war-entry threading is the next gate. Map-readiness non-art closure remains complete and art-blocked only. |
...
**Next diplomacy workflow:** **Ally Participation / Common Peace Slice A2 war-entry threading.** BPH-A through BPH-D + WPS-A through WPS-D + WB-A/WB-B/WB-C/WB-D + Slice A1 foundation all landed. Gate 1 passed. `threat_coalition` merged into `balance_of_europe`. War Bargains track fully closed. A1 shipped foundation-only scaffolding (containers, save/load defaults, empty-safe leader/participant indexes, dirty-flag invalidation hook, cached-helper elimination refactor, invariant assertion, 20-instance synthetic fixture, 13 new foundation tests). A2 owns the WAR-entry seam inventory: `ensure_war_instance_for_pair(...)`, `attach_pair_to_war_instance(...)` / `attach_participant_to_war_instance(...)`, `validate_war_declaration(...)`, `_process_war_cascade(...)` `war_id` threading via a `CascadeContext` if signature pressure warrants it, and the live `backend/game_logic/vassal.py` rebellion call site. Active settlement work continues from `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.21 Slice A "A2 war-entry threading" bullets.
...
| 4 | Ally Participation + Common Peace | ally beneficiaries, contribution / consultation rights, common peace routing, settlement fallout | Post-bargains track. `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.24 and implementation plan v1.21 own the full-Europe handoff. **Slice A1 foundation gate landed May 1, 2026.** Next work is Slice A2 war-entry threading. |
...
**Peace Deals spec gate (May 1):** BPH-A through BPH-D, WPS-A through WPS-D, and WB-A through WB-D are landed. Ally Participation/Common Peace has `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.24 and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.21 covering full-Europe scale. **Slice A1 foundation gate landed May 1, 2026:** mapped-nation capital helper safety (verified pre-existing), settlement containers (`next_war_instance_id`/`war_instances`/`archived_war_instances`), save/load defaults of `1`/`{}`/`[]`, empty-safe `war_instances_by_leader` / `war_instances_by_participant` index helpers with single dirty-flag invalidation, cached-helper refactor of the touched `_is_nation_eliminated` path, `assert_war_instance_invariants(...)` in `backend/game_logic/settlement_helpers.py`, and a synthetic 20-active-`war_instance` cache/index fixture covering the canonical 13 DG-1 roster + 6+ participant coalition side. 17 foundation tests pass; full pytest suite at `9311 passed, 1 skipped`. **No behavioral settlement code shipped under A1.** Next is Slice A2 war-entry threading: `ensure_war_instance_for_pair(...)`, direct-entry helpers, cascade `war_id` threading via `CascadeContext`, and the inventoried WAR-entry seams.
