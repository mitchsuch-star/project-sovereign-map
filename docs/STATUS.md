...
> **Current Settlement Gate (May 6, 2026):** `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.11 is SPEC READINESS GO / IMPLEMENTATION NO-GO with the full treaty-settlement path approved and fourth-pass SPEC READINESS blockers folded in. Code that cleanup spec before any Slice G / AI-ally settlement agency work. Quality beats schedule: this must become a player-usable settlement system, not a "wired but not usable" route milestone, and visible broken/misleading behavior cannot be deferred without explicit product approval in status/spec text. The cleanup spec now requires Gate 2 to proceed in ordered slices: Foundation (SC-1 through SC-4, including the shared editor host, executor `settlement_terms` forwarding, POST schema/taxonomy, empty-draft rejection, Back Out discard-confirm, fresh ratification-time acceptance scoring, absent ratify option, accepting-leader rescoring, and hard-stop catalog), Entry Safety (SC-8/8b/10/11/11b/12/13, including cached eligibility invalidated by `invalidate_war_instance_indexes()`, selected-target diagnostics, and focused-row coalition per-war buttons), Continuity (SC-14/14b/14c/14d/14e/7b/26, including dispatch active-vs-archived routing, non-empty reopen targets, and cross-war settlement collision protection), Incoming Offers (SC-5/6/7 default defer + hide unless explicitly reversed, with offer identity/package preservation if implemented), then Presentation and Metadata (SC-15/15b/16/17/19/20/23/24/25, including backend blocked-acceptance payload suppression, settlement vocabulary, merged peace/settlement history, and raw + clamped harshness fields). Slice G remains blocked until the full treaty cleanup is implemented, behavior-tested, and manually smoked.
> **Last Updated:** May 6, 2026 (**SETTLEMENT UI CLEANUP SPEC v0.11 FOURTH-PASS READINESS FOLD-IN COMPLETE** - `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` now carries the fourth-pass requirements for accepting-side rescoring, fresh ratification acceptance gates, editor host, outgoing stale recovery, cross-war collision SC-26, executor term forwarding, POST validation taxonomy, absent ratify option, blocked-acceptance payload suppression, empty-draft error path, coalition CTA precedence, exact test inversion enumeration, whole-war vocabulary, and eligibility cache invalidation. The spec is ready for implementation planning, but implementation remains NO-GO until Gate 2 Slice 1 starts from the ordered Foundation path and closes behavior tests before any presentation polish or Slice G work.)
