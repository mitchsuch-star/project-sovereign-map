...
> **Last Updated:** April 19, 2026 (Map Readiness Closure Pass — §3.1 nation config factory landed. `backend/nation_config.py` now exposes `DEFAULT_NATION_DEFAULTS` (gold/actions/authority fallback) so new nations only override what differs from the baseline; `validate_runtime_nation_support()` honors defaults + overrides and still requires capital + diplomat per nation. `backend/models/marshal.py` now has `create_marshal_from_data()` / `create_marshals_from_data()` factories fed from new `FRENCH_MARSHALS_DATA` / `ENEMY_MARSHALS_DATA` constants; `create_starting_marshals()` and `create_enemy_marshals()` are one-line wrappers preserving the exact legacy roster, skills, abilities, biographies, and cross-nation relationship matrix (including original Uxbridge ↔ Austrian non-entries). `backend/models/diplomat.py` now has `DIPLOMAT_DEFINITIONS` + `create_diplomat_from_data()`; `STARTING_DIPLOMATS` + `create_starting_diplomats()` remain as factory-derived wrappers. New `tests/test_nation_config_factory.py` (15 tests) pins factory behavior, default fallback semantics, validator acceptance of default-only new nations, and preservation of the current roster's explicit overrides. This closes Phase 3 of the Map Readiness Closure Pass. Full Python suite green: 8438 passed, 2 skipped. Next: Phase 4 map-art pipeline (§§4.1-4.4) — the final non-art prerequisites before commissioned Europe-map integration.)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8438 passed, 2 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Map Readiness Closure Pass — Phase 3 COMPLETE (April 19, 2026).** §3.1 (nation config factory — `DEFAULT_NATION_DEFAULTS` + marshal/diplomat factories) + §3.2 (shared topology endpoint via `GET /map_topology`) + §3.3 (centralize nation colors) + §3.4 (prompt/parser fallback de-hardcoding) all complete. 15 new factory tests + 10 drift-prevention / parity tests from earlier in Phase 3. Phase 2 remains COMPLETE. Next: Phase 4 map-art pipeline items (§§4.1-4.4). |
| **Blockers** | Real map readiness is **not** just "wait for art." The raw `enemy_ai.py` scan pass, prompt/parser/adjacency drift, and nation config factory work are now all closed. Active non-art blockers narrow to the Phase 4 province-registry / bitmap-loading / validator / unwired-province pipeline. Commissioned art integration and final renderer smoke remain blocked on art assets. |
...
2. **Map Readiness Closure Pass — Phase 3 COMPLETE (April 19, 2026).**
   ...
   - **Remaining Phase 4 items are still open.** Full pre-art gate still requires §§4.1-4.4 map-art pipeline items. See `docs/SCALE_READINESS_PLAN.md` §§4.1-4.4 and `docs/SCALE_READYNESS.md` §§9 + 11.
   - **Suggested next cold-start sequencing:** §4.1 (province registry schema) → §4.2 (external bitmap loading) → §4.3 (color-map validator) → §4.4 (unwired province support). A fresh session only needs CLAUDE.md + this STATUS.md + `SCALE_READINESS_PLAN.md` §4.
