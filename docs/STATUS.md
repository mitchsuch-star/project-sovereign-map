...
**Current gate (May 13, 2026):** `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.27 is **GO-pending-verification** on integration branch `codex/settlement-v027-integration`. This branch was created from `codex/g2-slice5-audit-fixes`, where G2-Slice-1 through G2-Slice-5 implementation commits are reachable through `b2a4d7b`, then merged with the v0.27 docs/smoke-start branch `codex/settlement-smoke-start`. Branch-target reconciliation, focused backend/source verification, and the SC-27/v0.27 compatibility scan are now recorded. The spec contract is binding, but implementation remains blocked until the open compatibility rows below are closed, executable Godot evidence exists for SC-1 / SC-10b / SC-28, both rejected and losing recovery fixtures are verified, and Gate 4 manual smoke passes.

**Next work focus (May 13, 2026):** audit and implementation attention should target the active Settlement UI Cleanup work, not the completed Peace Deals mechanics. The next review should score only the remaining cleanup/runtime scope: open G2-Slice-6/editor recovery rows, SC-1 / SC-10b / SC-28 executable evidence, rejected and losing recovery fixtures, expanded Gate 4 smoke, and the explicitly deferred landing rows SC-29 / SC-30 / SC-31 / SC-32 / SC-33. Treat BPH, WPS, WB, and WSA A1-E mechanics as background dependencies unless the cleanup spec points to a live conflict.
...
Settlement UI Cleanup implementation commits exist as reachable ancestors on the chosen target `codex/settlement-v027-integration`: `b699f4c`, `4310903` / `0761284`, `052c5d0`, `4884e93`, and `b2a4d7b`, with Slice 5 audit follow-up commits also present. `codex/settlement-smoke-start` remains a docs/smoke-start repair branch; do not restart old G2-Slice-1 work from it. Future cleanup verification should run from `codex/settlement-v027-integration` unless this branch is explicitly merged into another smoke/integration target and that target is recorded here.
...
Slice G / AI-ally settlement agency remains blocked until both rejected and losing smoke paths pass, SC-29 / SC-30 / SC-31 / SC-32 / SC-33 landing ownership remains tracked rather than open-ended, SC-22 Godot evidence is recorded per slice, and SC-10b plus the War Detail actionability helper are proven before `Open War Detail` recovery is treated as a valid blocked-review escape.
...
| Focused backend/source verification | PASS | `208 passed` across settlement foundation, entry safety, Slice F, incoming-offer deferral, continuity, G2-Slice-5, feedback, and smoke-start tests; `ruff check backend tests` clean. |
...
| False substitute CTAs before SC-29 | PASS | Source scan finds no active `Seek Armistice Instead`, `Seek Bilateral Peace`, `Ask for terms`, `Wait for Enemy Offer`, or `Surrender terms` labels in backend/Godot/tests. |
| Incoming-offer no-exposure | PASS | Existing deferral/no-leak behavior suite remains in the focused pass; positive incoming-offer producer/mailbox work stays post-cleanup Slice G1. |
| Smoke-start `settlement_multilateral` | PASS | `tests/test_smoke_start_settlement_multilateral.py` `4 passed`. |
| War Detail actionability helper | OPEN | No executable helper/test evidence found yet; this remains G2-Slice-6 / SC-10b work. |
| Attempt-4 stale recovery actionability | OPEN | Recovery must run the same actionability probe before `Open War Detail` is treated as usable. |
| Clause control schema and hidden clause labels | OPEN | `ClauseControlSchema` is still a spec contract, not a backend source-of-truth implementation. |
| Forced-alliance Continental toggle differential | PARTIAL | Threat-preview coverage exists; explicit Balance/threat cost-differential evidence for the editor toggle remains open. |
| Losing-side concession baseline | OPEN | `settlement_losing` fixture and no-silent-overwrite concession baseline evidence remain required. |
| Same-war off-editor merge conflict | PARTIAL | Same-war merge/conflict behavior tests pass, but route-to-preserved-draft recovery evidence remains part of SC-28/G2-Slice-6. |
| Draft-discard notice clear flag | OPEN | `pending_settlement_draft_notices` remains a planned save field unless an equivalent one-shot notice path lands. |
| Gate 4 fixture variants | OPEN | `settlement_rejected`, `settlement_losing`, and `settlement_multiwar_ambiguity` still need executable/manual smoke evidence. |
...
|----|--------|--------|-------|-----------------|-----------------------|--------------------------|-------|
| DWL-SET-SC5 | WSA `incoming_settlement_offer` / cleanup SC-5 | ACTIVE_DEFERRED | `SETTLEMENT_UI_CLEANUP_SPEC.md` SC-5 + post-cleanup Slice G appendix | v0.27 cleanup checks and Gate 4 smoke are reconciled, then SC-5 is explicitly reversed in cleanup spec + STATUS | Producer, payload, mailbox/pending-envoy UI, package-preserving accept/reject/revision actions, incoming voice, route identity, and behavior tests ship together | Until reversal: no mailbox count, route, notification, dispatch, popup, Godot branch, waiting control, or turn blocking. After reversal: incoming-offer route/action tests | Current no-exposure branch is intentional, not an open-ended placeholder |
| DWL-SET-SC29 | Direct-pair substitute CTAs (`Seek Armistice Instead`, `Seek Bilateral Peace`) | ACTIVE_DEFERRED | Settlement UI Cleanup SC-29 / G2-Slice-7 | v0.27 repair verified and direct-pair recovery route is implemented | Blocked settlement review can route to real bilateral peace / armistice authoring only when actionability probe passes | SC-29 route/actionability tests plus rejected-review smoke | Hidden until implemented; clickable copy-only controls forbidden |
| DWL-SET-SC30 | Enemy-offer waiting (`Wait for Enemy Offer`, `Ask for terms`) | ACTIVE_DEFERRED | Settlement UI Cleanup SC-30 / Slice G1 | SC-5/incoming-offer producer exists or the control is explicitly removed | Waiting/ask controls either open a real enemy-offer flow or are absent | SC-30 behavior tests proving no copy-only waiting control | Depends on incoming-offer agency |
| DWL-SET-SC31 | `Surrender terms` | ACTIVE_DEFERRED | Settlement UI Cleanup SC-31 / G2-Slice-8 | Losing-side concession baseline is verified and surrender authoring is designed | Surrender terms open a real concessionary treaty editor path or remain absent | SC-31 losing-side behavior tests | Separate from normal losing-side concession authoring |
| DWL-SET-SC32 | Broader settlement agency | ACTIVE_DEFERRED | Settlement UI Cleanup SC-32 / Slice G2 | SC-29 through SC-31 are resolved or explicitly cut | Remaining hidden settlement affordances either land with behavior tests or are removed from UI/spec promises | SC-32 ledger/no-open-affordance scan | Prevents indefinite hidden controls |
| DWL-SET-SC33 | Recurring gold payments (`gold_per_turn`) | ACTIVE_DEFERRED | Settlement UI Cleanup SC-33 / G2-Slice-9 | Dependency/surrender restoration closes or recurring-payment restoration is explicitly selected | `gold_per_turn` authors preview, validates amount/duration/affordability, mutates ratification, processes over turns, and records history/dispatch/ledger/voice, or remains absent | SC-33 absence and recurring-payment behavior tests | Hidden until implemented; disabled/greyed recurring-payment labels forbidden |
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Current integration branch verification:** settlement reconciliation focused suite `208 passed`; smoke-start `settlement_multilateral` suite `4 passed`; `ruff check backend tests` clean. Full suite and Godot runtime/manual smoke were not rerun in this pass. |
| **Current Phase** | **Peace Deals - Settlement UI Cleanup v0.27 GO-pending-verification after branch reconciliation + SC-27/v0.27 compatibility scan.** The next work is to close the open compatibility rows: SC-1 editor floor plus SC-10b / SC-28 / SC-28b rejected/losing recovery with executable evidence, War Detail actionability helper, clause control schema/editor recovery, losing/rejected/multiwar smoke fixtures, and expanded Gate 4 manual smoke. Do not start Slice G AI/ally settlement agency until Gate 4 smoke evidence is recorded and SC-29 / SC-30 / SC-31 / SC-32 / SC-33 ownership remains explicit rather than open-ended. |
...
| **Blockers** | Settlement UI Cleanup implementation remains blocked until the open v0.27 compatibility rows are closed, SC-1 / SC-10b / SC-28 executable evidence exists, per-slice SC-22 Godot parse/load or explicit tooling-block accounting keeps evidence rows open, SC-28 / SC-28b recovery evidence lands, and expanded 18-step Gate 4 smoke records rejected, losing, and multiwar ambiguity fixtures. Commissioned-art renderer smoke remains blocked on art assets. |
