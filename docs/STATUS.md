...
**Peace Deals spec gate (Apr 29):** BPH-A through BPH-D, WPS-A through WPS-D, and WB-A through WB-D are landed. Ally Participation/Common Peace now has `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.18 and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.15 covering full-Europe scale after combined Codex/Claude synthesis through the mapped-nation A1 simplification. Next is Slice A1 foundation coding: mapped-nation capital helper safety, settlement containers, old-save defaults, empty index/cache scaffolding, cache invalidation hooks, and no off-map nation model before any A2/B/C/D behavior.
