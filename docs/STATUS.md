...
> **Last Updated:** April 21, 2026 (Memory and Pressure v2.4.3 doc follow-up. Closed the remaining doc seams from the latest §7.3 / §11.1 audit: `COMMITMENTS_PRESENTATION_SPEC.md` §11 now owns the `nations[*].bloc_stamp` payload contract directly instead of leaving it only in the implementation plan; `RELIABILITY_COMMITMENTS_SPEC.md` now says `50%` / `60%` re-entries within one epoch do not re-fire beats and tightens the quiet Case-2 flavor line so low-band tension reads alive rather than inert; `COALITION_SPEC.md` now restates that `70%+` is scalar-only intensification (no fourth naming band) and that brewing copy stays anchored to `world.player_nation` while foreign-hegemon pressure sub-lines remain suppressed in v0.1; `RELIABILITY_IMPLEMENTATION_PLAN.md` now treats Slice E-Cards as live helper reuse, names the pacing acceptance test `test_hegemony_pacing_50pct_share_reaches_brewing_by_turn_16`, and makes retirement of the legacy anonymous `Diplomatic Tension` / `European Courts Concerned` emit sites a B-Hegemony acceptance requirement rather than implied cleanup. The historical Block 3 audit doc now explicitly marks its old badge-deferral line as superseded. British generic-fallback cleanup remains owned by Slice C-lite: no hardcoded `Castlereagh` fallback in generic hegemony / advisory templates once the resolver lands. Repo reality is still on the older substrate until B-Hegemony / C-lite / E-Cards are coded. **Closeout rule unchanged:** evaluate deferred items after we code the spec.)
> **Previously (April 21, 2026):** Memory and Pressure v2.4.3 audit follow-through. The live docs now make the pacing contract explicit and consistent: `50%` is the proper-noun reveal, while `60%+` is the first guaranteed-hardening band. The named-speaker fallback chain remains authoritative across the three owners (named envoy → Talleyrand advisory → chancery). The 30-33% private-tally band remains mechanically backed through the shared `_identify_max_bloc_share(world)` helper, the no-hegemon coalition-target fallback remains `world.player_nation`, the Balance-of-Europe payload keeps its COOLDOWN fields, and non-France-hegemon Case 3 / Case 4 still suppress the anti-France scalar sub-line. **Member badges / per-row bloc stamps are now live v2.4.3 scope** alongside the Balance of Europe headline, `balance_of_europe_shifted` threshold beats, proposal-preview warnings, and coalition-declaration contrast copy. Repo reality is still on the older substrate: `backend/game_logic/diplomatic_ledger.py` still ships the legacy `threat_coalition` payload while the Nations tab still renders `NATION OVERVIEW`; `backend/game_logic/coalition.py` still emits the old anonymous `Diplomatic Tension` / `European Courts Concerned` notices; `backend/game_logic/diplomacy.py` + `backend/display_names.py` still expose the generic `hegemony_pressure` fallback label path. B-Hegemony/C-lite/E-Cards therefore replace those seams rather than layering on top of them. Older British template text also still hardcodes `Castlereagh` in generic advice strings; that is not the default 1805 British fallback contract for the new hegemony surfaces, which should stay scenario-authored envoy first and office/chancery language otherwise. **Closeout rule:** evaluate deferred items after we code the spec. The agent who lands the last implementation session for this spec must explicitly review the deferred-item closeout ledger in `RELIABILITY_IMPLEMENTATION_PLAN.md` and record keep / re-open decisions here rather than leaving any remaining defer implicit.)
...
> **Previously (April 20, 2026):** Memory and Pressure v2.4.3 Block 2 substrate alignment complete and pushed. Deterministic bloc naming adopted for v2.4.3 on the constrained headline/beat/warning/declaration surfaces; member badges deferred.
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8508 passed, 2 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Map Readiness Closure Pass — Phase 4.4 COMPLETE (April 19, 2026).** Phase 3 (§§3.1-3.4), Phase 4.1 (province registry schema v2), Phase 4.2 (external bitmap loading), Phase 4.3 (offline color-map validator), and Phase 4.4 (unwired province support: grey-tint overlay on both circle + bitmap paths, `"(not yet in play)"` tooltip, click gate) are all landed. Phase 2 remains COMPLETE. Next: commissioned art integration + final renderer smoke (art-blocked). |
| **Blockers** | Real map readiness is **not** just "wait for art." The raw `enemy_ai.py` scan pass, prompt/parser/adjacency drift, nation config factory, province registry schema, renderer bitmap-ingest seam, offline art-acceptance validator, AND unwired province rendering are all closed. No active non-art blocker remains. Commissioned art integration and final renderer smoke remain blocked on art assets. |
...
2. **Map Readiness Closure Pass — Phase 3 COMPLETE (April 19, 2026).**
...
3. **Map Readiness Closure Pass — §4.1 Province Registry Schema COMPLETE (April 19, 2026).**
...
4. **Map Readiness Closure Pass — §4.2 External Bitmap Loading COMPLETE (April 19, 2026).**
...
5. **Map Readiness Closure Pass — §4.3 Color-Map Validator COMPLETE (April 19, 2026).**
...
6. **Map Readiness Closure Pass — §4.4 Unwired Province Support COMPLETE (April 19, 2026).**
...
7. **Memory and Pressure v2.4.3 — Block 3 folded, implementation slices ready to open.** `docs/audits/MP_V243_BLOCK1_DOC_CLEANUP.md` and `docs/audits/MP_V243_BLOCK2_SUBSTRATE.md` are complete. `docs/audits/MP_V243_BLOCK3_BLOC_NAMING.md` is SUPERSEDED — its bloc-naming contract was folded into `COMMITMENTS_PRESENTATION_SPEC.md` §8.1a (terminology guard, `33 / 50 / 60` activation gate, hegemon→label taxonomy with fallback, required surfaces, worked-copy examples, playtest feel gates), its per-band voice contract was folded into `DIPLOMAT_VOICE_BIBLE.md` ("Bloc-naming voice contract" subsection), and its CF1-CF4 closure items were folded into their parent slices in `RELIABILITY_IMPLEMENTATION_PLAN.md` (CF1 → C-lite, CF2 → B-B7, CF3 → B-B4, CF4 → B-B4 per the B-B1-lite merge gate). No separate Block 3 consumption pass is required. Remaining work: B-Hegemony / B-B1-lite / B-B3 / B-B4 / B-B7 / C-lite can open directly per the dependency table in `RELIABILITY_IMPLEMENTATION_PLAN.md`, with the B-B1-lite ↔ B-B4 merge-ordering gate still in force. This is not part of the map-readiness gate.
...
**Next bug-owned implementation slice:** None. All non-art map-readiness items are closed. The remaining map-readiness work (commissioned art-backed renderer cutover + final Godot smoke validation) is art-blocked and sits in the Blocked list above. For a fresh session on diplomacy, the next work is Memory and Pressure v2.4.3 implementation — Block 3 is folded (see §7 above), so slices open directly against `RELIABILITY_IMPLEMENTATION_PLAN.md` (B-Hegemony first, then B-B1-lite / B-B3 / B-B7 / C-lite; B-B4 parallel slice with the B-B1-lite merge gate).
...
|-------|------------|------------------------|------|
| 1 | Memory and Pressure v2.4.3 | `R160`, `R119` | First diplomacy follow-up closure block. Blocks 1 and 2 are complete; Block 3 is SUPERSEDED — its bloc-naming contract was folded into `COMMITMENTS_PRESENTATION_SPEC.md` §8.1a, its per-band voice contract was folded into `DIPLOMAT_VOICE_BIBLE.md`, and its CF1-CF4 closure items (presentation follow-through, defensive-refusal surfacing, apology-loop follow-through, composite-floor tests) were folded into their parent slices in `RELIABILITY_IMPLEMENTATION_PLAN.md` (C-lite / B-B4 / B-B7). Next: remaining B-Hegemony / B-B1-lite / B-B3 / B-B4 / B-B7 / C-lite work continues directly in `RELIABILITY_IMPLEMENTATION_PLAN.md` v2.4.3 and `COMMITMENTS_PRESENTATION_SPEC.md` v0.5.2. |
...
- Live implementation order is the v2.4.3 plan surface: B-Hegemony, B-B1-lite, B-B3, B-B4, B-B7, C-lite (trimmed) — slices may open directly per the dependency table in `RELIABILITY_IMPLEMENTATION_PLAN.md`.
