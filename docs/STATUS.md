...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `9062 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend tests`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals — WPS-D complete (April 26, 2026).** BPH-A through BPH-D + WPS-A through WPS-D all landed. WPS phase complete. Next: Gate 1 verification or WB-A. Map-readiness non-art closure remains complete and art-blocked only. |
