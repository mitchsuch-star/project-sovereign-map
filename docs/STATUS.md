...
> **Last Updated:** May 7, 2026 (**REJECTED-SETTLEMENT / LOSING-SIDE PEACE SPEC REVIEW QUEUED** — after the settlement smoke-start manual test exposed the blocked-ratification `Back Out` dead-end, `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` now adds a dedicated rejected-settlement / losing-side peace UX contract and a pre-implementation spec review gate. This is docs/spec cleanup only, not a coding target. **Next:** run one or more spec review sessions against the new contract before any implementation: assess whether the design cleanly separates ratification, term editing, armistice fallback, concession/surrender terms, and AI incoming offers; verify that losing-side peace is active player agency rather than "win first"; and look creatively for similar settlement cleanup misses where UI exposes incomplete actions, hides strategic next steps, or conflates preview/revision/ratification/offer handling. Previous Gate 2 implementation status remains: commits `0761284` and `32b868c` landed G2-Slice-2 entry safety and audit fixes; verification was `tests/test_settlement_entry_safety_slice.py` `25 passed`, settlement foundation/UI behavior bundle `62 passed`, full suite `9858 passed, 1 skipped`, and `ruff check backend tests` clean.)
