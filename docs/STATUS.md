...
> **Current Settlement Gate (May 7, 2026):** `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.17 Gate 2 is through Slice 3 after the continuity audit follow-up. Foundation (SC-1 through SC-4), Entry Safety (SC-8/8b/10/11/11b/12/13), and Continuity (SC-14/14b/14c/14d/14e/7b/26) are landed and behavior-tested, including the follow-up fixes for live-world notification/ledger re-resolution and war-merge migration of `pending_settlement_drafts`, `settlement_route_seq`, and `settlement_reopen_attempts`. The active next implementation gate is **G2-Slice-4 Incoming Offers (SC-5/6/7 default defer + hide unless explicitly reversed)**, followed by Slice 5 Presentation and Metadata (SC-15/15b/16/17/19/20/23/24/25), manual Godot smoke, then Slice G AI/ally settlement agency. Quality beats schedule: this must become a player-usable settlement system, not a "wired but not usable" route milestone, and visible broken/misleading behavior cannot be deferred without explicit product approval in status/spec text. `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` carries inline supersession markers for old route-id, incoming-offer Slice F, and cross-war queue/defer collision instructions per SC-27, with the v0.17 widened doc-scan token list. Slice G remains blocked until the full treaty cleanup is implemented, behavior-tested, and manually smoked.
> **Last Updated:** May 7, 2026 (**SETTLEMENT UI CLEANUP G2-SLICE-3 CONTINUITY AUDIT FOLLOW-UP LANDED** — original Slice 3 closed SC-14, SC-14b, SC-14c, SC-14d, SC-14e, SC-7b, and SC-26 per `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` §G2-Slice-3. Follow-up audit found and fixed two continuity gaps: `settlement_notification_meta(event, *, world=...)`, `_queue_settlement_notification(...)`, and `recent_settlement_summaries(...)` now thread live world state through `settlement_review_target(...)` so notification and ledger rows re-resolve active-vs-archived at click/build time; `merge_war_instances(...)` now migrates absorbed-war `pending_settlement_drafts`, folds `settlement_route_seq` counters by turn, and migrates `settlement_reopen_attempts`. Test surface increased to 34 Slice 3 continuity tests plus a war-merge regression for settlement continuity state. Verification: focused settlement/inversion suite `223 passed`; full suite `9885 passed, 1 skipped`; `ruff check backend tests` clean. **Next:** Slice 4 (Incoming Offers — SC-5/6/7 default defer + hide), then Slice 5 (Presentation & Metadata), then manual Godot smoke, then Slice G AI/ally settlement agency.)
> **Previously:** May 6, 2026 (**SETTLEMENT UI CLEANUP SPEC v0.17 TENTH-PASS READINESS FOLD-IN COMPLETE** - `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` now carries tenth-pass requirements for same-war restage eligibility, backend collision change-site protection, AI cross-war retry bounds, incoming-offer handler defensive deferral, substitute bilateral/armistice CTAs when Settlement is hidden, stale-save selected-target fallback, editor authority, POST preview write cadence, canonical clause schema, SC-27 collision-rule supersession scanning, pending-draft migration on war merge, zero-option close payload, voice surface mapping, settlement-family type mirroring, vocabulary scan file enumeration, legacy term rendering, eligibility cache shape, per-path selected-target tests, manual-smoke replacement evidence, applied-clause preview shape, route-id all-consumer continuity, `available_action_ids` shape, 1080p editor reachability, collision audit artifact policy, leader-change consent semantics, and consolidated empty/authored-peace contract. The spec is ready for implementation planning, but implementation remains NO-GO until Gate 1 doc scans pass and Gate 2 Slice 1 starts from the ordered Foundation path with behavior tests before any presentation polish or Slice G work.)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Current backend baseline:** full suite `9885 passed, 1 skipped`; `ruff check backend tests` clean after Slice 3 audit follow-up. Godot runtime smoke remains manual-only unless a Godot executable is available. |
| **Current Phase** | **Peace Deals - Settlement UI Cleanup Gate 2.** Slices 1-3 are landed and audit-followed-up; the active next gate is G2-Slice-4 Incoming Offers (SC-5/6/7 default defer + hide unless explicitly reversed), then Slice 5 Presentation & Metadata, manual Godot smoke, and only then Slice G AI/ally settlement agency. |
