...
> **Current Settlement Gate (May 7, 2026):** G2-Slice-1 Foundation plus the Codex audit-fix pass are LANDED on `codex/settlement-spec-readiness-tightening` at commit `97a5fc2` (`Fix settlement foundation audit gaps`). This is a backend foundation GO, not full Settlement UI Cleanup closure. Slice G / AI-ally settlement agency remains blocked until the remaining Gate 2 cleanup slices are implemented, behavior-tested, and manually smoked. Full Treaty Settlement remains approved: author concrete war-scoped terms, preview consequences and acceptance, revise/re-author, preserve incoming-offer packages only if offers ship, and ratify only after acceptance/hard-stop gates pass. SC-4b remains the active resource-cost contract: settlement entry, preview, Submit, Revise Terms, Back Out, failed ratification, stale recovery, hard-stop refusal, and successful ratification default to 0 DP/AP/gold unless a later product decision records a non-zero `confirm_settlement` cost in both spec and STATUS.
> **Last Updated:** May 7, 2026 (**G2-SLICE-1 FOUNDATION + AUDIT FIXES LANDED / NEXT GATE 2 CLEANUP SLICE READY** — commit `97a5fc2` brings in the Settlement UI Cleanup G2-Slice-1 foundation and closes the five Codex audit findings. Implemented: canonical clause schema validation for all 8 clause types with no unknown/alias keys, empty-draft rejection, non-dict rejection, max-8 rejection, unauthorized actor/proposer-side mismatch checks, conflict matrix, POST preview validation before preview, submitted-term revalidation before staging, `settlement_terms` forwarding from top-level and `diplomatic_data`, draft persistence/clearing, hidden Revise Terms, Back Out draft discard, acceptance-gated Ratify options, live-world rescore before mutation, hard-stop fail-closed behavior, pending draft and route sequence serialization, same-war same-turn sequence route ids, and route-id continuity into settlement reaction events. Verification: focused settlement/reaction suite `106 passed`; full suite `9833 passed, 1 skipped`; `ruff check backend tests` clean. Confidence: high for the backend foundation/audit surface, medium-high for the overall cleanup because frontend/editor, entry-safety, continuity, incoming-offer deferral, and presentation slices remain. **Next:** continue Gate 2 cleanup before Slice G: SC-5/SC-7 incoming-offer defer-and-hide no-leak closure, SC-10/SC-12 settlement entry safety and one-to-one substitutes, SC-13 selected-target continuity/stale recovery, SC-14 active-vs-archived route continuity, SC-18 settlement-family fallback safety, then presentation/metadata cleanup and manual smoke.)
