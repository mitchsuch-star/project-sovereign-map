...
> **Last Updated:** May 8, 2026 (**SETTLEMENT SPEC v0.20 REPAIR CONSOLIDATED — SPEC READINESS NO-GO / IMPLEMENTATION NO-GO UNTIL VERIFICATION + SMOKE** — Claude/Codex review findings were folded into canonical rules without adding audit-history prose: blocked ratification omits `confirm_settlement` from `options[]` and `available_action_ids[]`; no disabled `Revise Terms` placeholder before an edit-capable route exists; enemy-offer waiting controls remain hidden while SC-5 is deferred; failed settlement review routes to `Open War Detail` for existing Bilateral Peace / Armistice tools instead of inventing a popup armistice action; losing-side concessions are owned by SC-1 offer-mode on the standard treaty editor with concession scoring and Talleyrand baseline copy; Voice Bible 16.1 now owns required settlement voice families. Next: verify v0.20 doc scans and run Gate 4 manual smoke, including the rejected/losing variant.)
