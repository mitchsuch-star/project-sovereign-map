...
> **Last Updated:** April 19, 2026 (Map Readiness Closure Pass — §4.2 external bitmap loading is complete, including the post-landing hardening in `d67992b`. `map_renderer_base.gd` now exposes two subclass hooks, `_get_map_visual_bitmap_path()` + `_get_map_lookup_bitmap_path()`, both defaulting to `""`; and `_load_map_images() -> bool` now loads imported `res://` bitmap assets through `ResourceLoader` / `Texture2D.get_image()` instead of `Image.load(res://...)`, so exported builds keep the bitmap path. The loader still runs before circle generation and still falls back cleanly when paths are empty or loading fails, but it now also latches bitmap-load errors once, rejects visual/lookup size mismatches, and validates that lookup-map colors round-trip against `province_color_lookup` before enabling bitmap mode. The existing `_lookup_region_from_color_map()` hit-test path (`province_lookup_image.get_pixel()` keyed into `province_color_lookup`) is unchanged. §4.3 is now narrowed to offline asset validation that the runtime loader does not already cover well: sentinel misuse, anti-alias / stray-color detection, tiny-island detection, and minimum pixel-coverage checks before accepting art. Focused map verification after the hardening follow-up: `36 passed` (`tests/test_map_renderer_cutover.py`, `tests/test_map_placeholder_assets.py`, `tests/test_map_bitmap_contract.py`). Last full Python-suite baseline remains `8453 passed, 2 skipped`. Next: §4.3 color-map validator → §4.4 unwired province support.)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8453 passed, 2 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Map Readiness Closure Pass — Phase 4.2 COMPLETE (April 19, 2026).** Phase 3 (§§3.1-3.4), Phase 4.1 (province registry schema v2), and Phase 4.2 (external bitmap loading with exported-build-safe resource loading, runtime lookup-color validation, and circle fallback) are all landed. Phase 2 remains COMPLETE. Focused map verification after the §4.2 hardening follow-up is green (`36 passed`). Next: §4.3 color-map validator → §4.4 unwired province support. |
| **Blockers** | Real map readiness is **not** just "wait for art." The raw `enemy_ai.py` scan pass, prompt/parser/adjacency drift, nation config factory, province registry schema, and renderer bitmap-ingest seam work are now all closed. Active non-art blockers narrow to §§4.3-4.4 (color-map validator, unwired grey-tint rendering). Commissioned art integration and final renderer smoke remain blocked on art assets. |
...
4. **Map Readiness Closure Pass — §4.2 External Bitmap Loading COMPLETE (April 19, 2026).**
   ...
   - **Test coverage.** `tests/test_map_renderer_cutover.py` now pins the hook declarations, exported-build-safe resource loading, runtime lookup-color validation, failure latching, and loader ordering inside `_build_map_textures()`. `tests/test_map_placeholder_assets.py` still pins that placeholder `map.gd` never opts into bitmap mode. New `tests/test_map_bitmap_contract.py` adds fixture-driven behavioral coverage for missing-file fallback, lookup-color rejection, size mismatch rejection, and hit-test round-trips with `map_origin = Vector2.ZERO`. Focused map verification: **36 passed** (`tests/test_map_renderer_cutover.py tests/test_map_placeholder_assets.py tests/test_map_bitmap_contract.py -q`).
   - **Not yet covered — these remain §4.3/§4.4 work:** offline asset validation (sentinel misuse, anti-alias artifacts, tiny pixel islands, minimum coverage thresholds, CI-time acceptance checks) lives in §4.3; grey-tint rendering for unwired provinces and "(not yet in play)" hover copy live in §4.4. §4.2 now includes runtime size and lookup-color validation, so §4.3 should not duplicate those checks blindly.
   - **Suggested next cold-start sequencing:** §4.3 (color-map validator) → §4.4 (unwired province support, including the grey-tint + hover-copy semantics for unwired entries now that the schema carries the flag). A fresh session only needs CLAUDE.md + this STATUS.md + `SCALE_READINESS_PLAN.md` §4.
5. **Memory and Pressure — Slice A (rivalry seed).** This remains the next diplomacy feature track, but it is not part of the map-readiness gate. See `docs/RELIABILITY_IMPLEMENTATION_PLAN.md`.
...
**Next bug-owned implementation slice:** Map Readiness Closure Pass — §4.3 color-map validator.
...
**Current Session 8 progress:** Cutover slices 1-3 + §§4.1-4.2 COMPLETE. The map renderer now builds shared scene-node layers behind `map_renderer_base.gd`, keeps the existing `map.gd` data wrapper and `update_all_regions(map_data)` contract stable for `main.gd`, includes a placeholder province-definition asset plus hidden color-map lookup path for the current 19-region map, isolates the map world inside a `SubViewport` with native `Camera2D` zoom/pan plus world-bound clamping, carries the production province-registry schema (`province_id`, per-feature anchors, `wired`, `interactive`), and can ingest imported bitmap assets through an exported-build-safe loader with runtime lookup-color validation plus circle fallback. Remaining Session 8 work is **not only** "wait for art": before commissioned Europe integration the project still needs the §4.3 bitmap validator and the §4.4 unwired-province runtime/visual contract. Next non-art slice: §4.3 color-map validator. Once those non-art prerequisites land, the remaining art-dependent work is the commissioned art-backed renderer swap and final Godot runtime smoke validation.
