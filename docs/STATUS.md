...
> **Last Updated:** April 19, 2026 (Map Readiness Closure Pass — §4.4 unwired province support is COMPLETE. Renderer gained three coordinated seams: (1) a single `_apply_unwired_grey_overlay(visual_image, lookup_image)` helper that lerps `UNWIRED_GREY_COLOR (0.32, 0.32, 0.34, 1.0)` over every visual pixel whose lookup color belongs to an unwired province, with `UNWIRED_GREY_BLEND = 0.7`; invoked from BOTH the circle-fallback (after `province_lookup_image = color_map`) AND the bitmap-loader path (after `province_lookup_image = lookup_image`), so unwired tint applies uniformly regardless of art source. (2) `_is_region_wired(region_name)` + `_unwired_lookup_keys()` helpers route the gate through `province_shapes` and default to wired when a region is unknown, preserving legacy callsites. (3) A dedicated `_draw_unwired_region_tooltip()` renders `"<Province Name>" / "(not yet in play)"` against a visually distinct panel color so authors can never mistake an unwired province for a fogged one. `_draw()` evaluates the unwired branch BEFORE `region_full_data.has(hovered_region)` because `update_all_regions()` excludes unwired regions from `region_full_data` by §4.1 design. The `MOUSE_BUTTON_LEFT` click handler now short-circuits on `_is_region_wired(clicked_region) == false`, so unwired provinces never emit `region_clicked`. Hover path stays unchanged — `_lookup_region_from_color_map()` keeps its `interactive`-only gate, so unwired-but-interactive provinces still produce hover hits and flow into the placeholder tooltip. 16 new tests: 10 in `tests/test_map_renderer_cutover.py` pinning constants, helper declarations, overlay/texture ordering in both paths, click-gate precedence, tooltip dispatch order, and the negative assertion that `_lookup_region_from_color_map()` never short-circuits on `wired`; 6 in new `tests/test_map_unwired_overlay.py` behaviorally mirroring the overlay math in Python and proving the sentinel is excluded by construction. Full Python-suite baseline now `8503 passed, 2 skipped` (was `8487`; +16). Ruff clean. Next: commissioned art-backed renderer cutover remains the last map-readiness gate, and it is still art-blocked.)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8503 passed, 2 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
