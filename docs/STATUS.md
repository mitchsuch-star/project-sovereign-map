...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `9522 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend/ tests/`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals - Ally Participation Slice B3 lifecycle LANDED + review fix LANDED (May 3, 2026); Slice C is next.** BPH-A through BPH-D, WPS-A through WPS-D, WB-A, WB-B, WB-C, WB-D, Slice A1 foundation, A2 war-entry threading, A3 merge/archive/leader invariants, B1 contribution store/standing math, B2 ordering guard/emitter wiring/non-battle emitters, and B3 lifecycle/staying-power/archive compaction are landed. Review follow-up `be93a32` closes the debug `set_diplo_state` WAR-exit seam: debug WAR to PEACE resolves pairs and closes contribution episodes, while debug WAR to ARMISTICE suspends the pair without closing episodes. Active settlement work is Slice C: common-peace term legitimacy, settlement standing readers, and dispatch / ledger / paradox routing consuming contribution lifecycle data. Map-readiness non-art closure remains complete and art-blocked only. |
