...
| **Current Phase** | **Peace Deals - Ally Participation spec v1.23 / implementation plan v1.20 READY FOR SLICE A1 FOUNDATION GATE (April 30, 2026).** BPH-A through BPH-D, WPS-A through WPS-D, WB-A, WB-B, WB-C, and WB-D all landed. `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` now own the full-Europe handoff after combined Codex/Claude synthesis through mapped-nation A1 simplification, future-nation onboarding hardening, active-roster fixture closure, C2 settlement-dialogue serialization closure, explicit common-peace tuning escalation, a spec-naive manual pre-D1 settlement-review comprehension gate, A3 post-merge invariant assertions, pre-A2 `CascadeContext` guidance, a B2 `record_battle()` ordering guard, concrete `request_revision` behavior, and D2 cross-war profiling. Coding may start with Slice A1 only by first landing the foundation gate: mapped-nation capital helper safety, settlement containers, old-save defaults, empty index/cache scaffolding, cache invalidation hooks, 20-`war_instance` cache/index proof, and no behavioral common-peace work. Map-readiness non-art closure remains complete and art-blocked only. |
...
| 4 | Ally Participation + Common Peace | ally beneficiaries, contribution / consultation rights, common peace routing, settlement fallout | Post-bargains track. `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.23 and implementation plan v1.20 own the full-Europe handoff; next work is Slice A1 foundation gate. |
...
**Peace Deals spec gate (Apr 30):** BPH-A through BPH-D, WPS-A through WPS-D, and WB-A through WB-D are landed. Ally Participation/Common Peace now has `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.23 and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.20 covering full-Europe scale after combined Codex/Claude synthesis through the mapped-nation A1 simplification, final scale-audit hardening, future-nation onboarding hardening, active-roster fixture closure, C2 settlement-dialogue serialization closure, explicit common-peace tuning escalation, a spec-naive manual pre-D1 settlement-review comprehension gate, A3 post-merge invariant assertions, pre-A2 `CascadeContext` guidance, a B2 `record_battle()` ordering guard, concrete `request_revision` behavior, and D2 cross-war profiling. Next is Slice A1 foundation coding: mapped-nation capital helper safety, settlement containers, old-save defaults, empty index/cache scaffolding, cache invalidation hooks, and 20-`war_instance` cache/index proof before any A2/B/C/D behavior.
