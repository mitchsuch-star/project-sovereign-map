...
**Next diplomacy workflow:** **Slice B2 — battle / occupation / support event emitters.** Slices A1 + A2 + A3 + B1 are landed. Active settlement work continues from `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` Slice B "B2 event emitters and theater attribution" bullets and `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` §9.2 / §9.4. B2 must extend `backend/game_logic/diplomacy.py::record_battle()` (NOT `WorldState.record_battle`) so settlement contribution accrual runs BEFORE the existing 1000-casualty war-score early return; add a regression guard for that ordering before B2 starts. Cover battle, occupation, support, treaty-support, British subsidy, sub-1000 battle contribution, `_execute_attack()` inline diplo records, auto-dispatch charge records, and the glorious-charge pipeline. The B1 module (`backend/game_logic/war_contribution.py`) already exposes the canonical store, episode lifecycle, share math, old-record adapter, and standing classifier — B2 wires the emitter call sites; B3 wires per-turn lifecycle and exit stamping.
