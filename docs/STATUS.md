...
> **Last Updated:** April 17, 2026 (Scale Readiness Phase 0 COMPLETE + implementation-closure doc pass. All 7/7 design gates resolved and sharpened to be implementation-ready: DG-1 locks 13 nations with 20+ nation architecture headroom; DG-2 keeps diplomacy bilateral with force-expand salience rules, a weighted-sum salience score, and a locked 5-row expanded cap; DG-3 defers supply lines; DG-4 locks direct-only bilateral call-to-arms with no transitive cascade, an explicit qualifying-treaty list (`ALLIANCE`, `DEFENSIVE_ALLIANCE`, vassal), and a concrete refusal event contract (`call_to_arms_refused` commitment_paradox episode); DG-5 defines raw-count hegemony victory with a locked qualifying-concession list and explicit indemnity / protectorate thresholds; DG-6 locks scenario-configured pacing with `scenario_schema_version: 1`, a hybrid `base_ap` (`by_nation` + `by_tier_default`), and structured `victory_profile` / `cascade_profile` sub-schemas; DG-7 uses categorized dispatch with priority escalation. Cross-cutting taxonomy is canonical: `power_tier` is authored scenario data with enum `major / secondary / minor`, never runtime-derived; any future numeric strength signal uses a separate `power_score` field. Old names `great_power / secondary_power / minor_power` in `DESIGN_REFINEMENT.md`, `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` §8.3 / §11.2, and `RELIABILITY_COMMITMENTS_SPEC.md` §12.3 are marked superseded. Scale Readiness Phase 1 also remains COMPLETE — 11 new tests, all hardcoded `== 19` removed, validator derives from NATION_CAPITALS.)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **17 targeted renderer regression tests** passed in this slice (`tests/test_map_consistency.py`, `tests/test_map_renderer_cutover.py`, `tests/test_map_placeholder_assets.py`). Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
...
- **Scale Readiness Phase 0 — Design Gates + Implementation-Closure Pass.** COMPLETE (April 17, 2026). `DG-1` 13 independent nations with 20+ headroom; `DG-2` bilateral diplomacy with force-expand salience rules, weighted-sum salience score, and locked 5-row expanded cap; `DG-3` supply lines deferred; `DG-4` direct-only bilateral call-to-arms with no transitive cascade, qualifying-treaty list locked (`ALLIANCE`, `DEFENSIVE_ALLIANCE`, vassal), concrete `call_to_arms_refused` refusal-event contract; `DG-5` raw-count hegemony victory with locked qualifying-concession list and explicit indemnity / protectorate thresholds; `DG-6` scenario-configured pacing with `scenario_schema_version: 1`, hybrid `base_ap.by_nation` + `base_ap.by_tier_default`, structured `victory_profile` / `cascade_profile` sub-schemas; `DG-7` categorized dispatch with priority escalation. Cross-cutting taxonomy is now canonical and single-sourced to `SCALE_READINESS_PLAN.md` §"Phase 0 Cross-Cutting Taxonomy": `power_tier` is authored scenario data with enum `major / secondary / minor`, never runtime-derived; runtime numeric strength signals use a separate `power_score` field. Older `great_power / secondary_power / minor_power` language in `DESIGN_REFINEMENT.md`, `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md`, and `RELIABILITY_COMMITMENTS_SPEC.md` carries supersede markers pointing to the canonical section.
...
- **Scale Readiness implementation-closure clarification pass.** COMPLETE (April 17, 2026). `SCALE_READINESS_PLAN.md` now explicitly requires authored scenario records (`player_nation`, `home_capital`, `player_sphere`, `nations`, `counts_for_strategic_power`), defines `protectorate` as a DG-2 / DG-5 subject state but not a DG-4 auto-entry state, locks DG-2's shared salience helper + tiebreak order, aligns DG-4 with a later surfaced attacker-side ally-entry seam, and moves DG-5 peace-hegemony implementation behind minimum peace-clause metadata from the Peace Deals track.
