...
> RULED + BUILT.** Suite **18,095 / 3**, ruff clean; `BASELINE_SERIES`
...
>
> Bug rows: ⚠ **WIN-H1's production half was TRIED AND REVERTED** — the
> fix NPC-16 routes is unsafe as written: `pending_interrupt` is a
> registered client route matcher consulted BEFORE the strategic-reports
> branch, so promoting the key makes the client skip the report summary
> that narrates the turn. NPC-16's row is amended with the reason and the
> two ways a future session could do it safely. WIN-1's dead option now arrives
...
> `test_tutorial_school_fixes_2026_08_08.py` (21):**
...
>   `test_tutorial_scenario.py` (21) pins every beat precondition as
