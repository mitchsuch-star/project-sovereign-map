...
> **Last Updated:** April 29, 2026 (**IMPERIAL SETTLEMENT SPEC v1.18 MAPPED-NATION A1 READY** - `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.18 and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.15 are aligned for Slice A1 foundation coding. A1 no longer owns an off-map Britain/Russia model; it starts with mapped-nation capital helper safety, settlement containers, old-save defaults, empty index/cache scaffolding, and cache invalidation hooks. Future nations such as Russia enter settlement only when scenario/map data adds them.)
> **Previously:** April 29, 2026 (**IMPERIAL SETTLEMENT SPEC v1.17 PRE-A1 FOUNDATION GATE READY** - `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.17 and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.14 are aligned for Slice A1 foundation coding after Codex/Claude reconciliation. Closure hardening pinned A1 to off-map helper APIs, settlement containers, old-save defaults, empty index/cache scaffolding, and off-map elimination protection before any A2 `war_id` threading, B contribution tracking, C common-peace scoring, or D reaction work; superseded by v1.18 mapped-nation simplification.)
> **Previously:** April 29, 2026 (**IMPERIAL SETTLEMENT SPEC v1.16 FULL-EUROPE AUDIT SYNTHESIS READY** - `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.16 and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.13 are aligned for Slice A1 after the Run 3 / cross-audit synthesis. Closure hardening now owns side-scoped leader-source metadata, off-map elimination exemption, proposer-side participant and beneficiary confirm revalidation, material-share zero guards, corrected combat call-site inventory, per-war-instance access/supply caps, cache invalidation timing, canonical full-Europe fixture ids, and final stress fixtures.)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `9230 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend tests`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals - Ally Participation spec v1.18 / implementation plan v1.15 READY FOR SLICE A1 FOUNDATION GATE (April 29, 2026).** BPH-A through BPH-D, WPS-A through WPS-D, WB-A, WB-B, WB-C, and WB-D all landed. `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` now own the full-Europe handoff after combined Codex/Claude synthesis through v1.18 mapped-nation A1 simplification. Coding may start with Slice A1 only by first landing the foundation gate: mapped-nation capital helper safety, settlement containers, old-save defaults, empty index/cache scaffolding, cache invalidation hooks, and no behavioral common-peace work. Map-readiness non-art closure remains complete and art-blocked only. |
...
**Next diplomacy workflow:** **Ally Participation / Common Peace Slice A1 foundation gate.** BPH-A through BPH-D + WPS-A through WPS-D + WB-A/WB-B/WB-C/WB-D all landed. Gate 1 passed. `threat_coalition` merged into `balance_of_europe`. War Bargains track fully closed. The historical Peace Deals umbrella remains for cross-cutting context; active settlement work starts from `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.15. A1 must begin with the mapped-nation foundation gate before any A2 war-entry threading or later settlement behavior.
...
| 4 | Ally Participation + Common Peace | ally beneficiaries, contribution / consultation rights, common peace routing, settlement fallout | Post-bargains track. `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.18 and implementation plan v1.15 own the full-Europe handoff; next work is Slice A1 foundation gate. |
...
**Peace Deals spec gate (Apr 29):** BPH-A through BPH-D, WPS-A through WPS-D, and WB-A through WB-D are landed. Ally Participation/Common Peace now has `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` v1.19 and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` v1.16 covering full-Europe scale after combined Codex/Claude synthesis through the mapped-nation A1 simplification and final scale-audit hardening. Next is Slice A1 foundation coding: mapped-nation capital helper safety, settlement containers, old-save defaults, empty index/cache scaffolding, cache invalidation hooks, and no off-map nation model before any A2/B/C/D behavior.
