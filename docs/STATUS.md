...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8901 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend tests`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals Phase A — BPH-D COMPLETE (April 26, 2026).** BPH-A through BPH-D all landed. Memory and Pressure v2.4.3 fully closed. Next coding slice is **WPS-A** (War Purpose popup at declaration time). Map-readiness non-art closure remains complete and art-blocked only. |
...
**Next diplomacy workflow:** **BPH-D COMPLETE.** All four BPH slices (A through D) are landed and verified. Next is **WPS-A: War Purpose popup at declaration time** — `war_objectives` on WorldState, War Purpose popup (CanvasLayer 110, modal), purpose selection at war declaration. Follow `docs/PEACE_DEALS_UMBRELLA_SPEC.md` slice order. BPH and WPS slices can be interleaved.
