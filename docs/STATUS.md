...
> **Last Updated:** May 8, 2026 (**SETTLEMENT SPEC v0.22 CODEX-CLAUDE UX SYNTHESIS — SPEC READINESS NO-GO / IMPLEMENTATION NO-GO UNTIL REVIEW TRACEABILITY + VERIFICATION + SMOKE** — the latest Codex and Claude review items are folded into canonical rules: blocked ratification omits `confirm_settlement` from `options[]` and `available_action_ids[]` rather than rendering disabled Ratify; direct `Seek Armistice Instead`, `Seek Bilateral Peace`, `Wait for Enemy Offer`, `Ask for terms`, and `Surrender terms` controls remain absent unless future owning rows ship producer/handoff/payload/UI/voice/tests together; War Detail recovery is a live-context route, not a promise of pair actions; scoped drafts use `draft_key` rather than raw `war_id`; payload schemas are pinned for preview/submit/review; forced-alliance is demand-only in the first cleanup slice; losing-side concessions are player-authored canonical clauses with deterministic Talleyrand baseline; SC-27 doc scans now include rejected-settlement false-affordance phrases and future-work copy. Next: record the Pre-Implementation Spec Review Gate result for v0.22, verify doc scans, reconcile the implementation branch target, and run Gate 4 manual smoke including the rejected/losing variant.)
