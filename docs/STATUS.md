...
**Current gate (May 12, 2026):** `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.25 is **SPEC READINESS GO / IMPLEMENTATION NO-GO** after Codex review session `codex-2026-05-12-settlement-ux-v0.25` on target branch `codex/settlement-smoke-start` from reviewed base `b5af5f2`. Implementation remains NO-GO until SC-27 doc scans pass on the implementation plan and parent settlement spec, executable Godot evidence exists for SC-1 / SC-10b / SC-28, rejected/losing recovery is verified on the chosen integration/smoke target, branch-target reconciliation is recorded, and Gate 4 manual smoke passes.

**Canonical follow-up edits now folded:** rejected multi-party recovery must offer `re_author_with_concessions` only when a material concession baseline exists; attempt-4 stale recovery must dismiss or replace the active hard-stop popup so recovery is clickable and preserve or invalidate drafts with a player signal; Covered Enemies remains a required editor control; discard-confirm has two named actions and outer-cancel keeps the draft; EDIT uses `Submit for Review`, not bare `Submit`; `Open Settlement History` must visually land in Settlement History / `PEACE & SETTLEMENT HISTORY`; settlement summary and digest share the same staged route id; parent leader-change, lazy-backfill, queue/defer, and digest-route instructions are superseded; Voice Bible 16.1 recovery families now include the observer blocked-ratification family; incoming-offer stale-accept coverage is conditional on SC-5 reversal or replaced by defensive no-exposure/drop tests; first-slice `forced_alliance` requires threat / Balance preview in G2-Slice-1 or it is interim-hidden; parent settlement docs now point active structured-payload and cleanup-readiness language at v0.25 rather than stale v0.24 wording; `gold_per_turn`, `vassalage`, `subjugation`, and `liberation` are interim-hidden from `available_clause_types[]`, Add Clause controls, concession baselines, presets, mailbox/request-terms payloads, and generated player copy until SC-33 / SC-31 close.

## Last Recorded Spec Review Result

**May 12, 2026 Codex v0.25 iterative synthesis:** Review session `codex-2026-05-12-settlement-ux-v0.25` re-scored the cleanup spec after folding the v0.24 / follow-up findings. Scores: Fun 8/10, Clarity 8/10, Work Segmentation 8/10, Contradiction-Freedom 8/10, Completeness 8/10. Verdict: **SPEC READINESS GO / IMPLEMENTATION NO-GO**. No unresolved P0/P1 spec blockers remain after the observer voice family, incoming-offer conditionalization, material concession baseline gate, forced-alliance political-cost slice move, and v0.25 status-check alignment. Implementation remains blocked on branch reconciliation, executable Godot evidence, SC-27 scans, pre-smoke verification, and expanded Gate 4 smoke.
...
Slice G / AI-ally settlement agency remains blocked until the updated v0.25 checks are consumed, the rejected/losing-side smoke path passes, SC-29 / SC-30 / SC-31 / SC-32 / SC-33 landing ownership remains tracked rather than open-ended, SC-27 doc scans pass on current text, SC-22 Godot evidence is recorded per slice, and SC-10b is proven before `Open War Detail` recovery is treated as a valid blocked-review escape.
