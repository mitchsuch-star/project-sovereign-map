...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `9669 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend/ tests/`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals - Imperial Settlement Slice D1/D2 settlement & cross-war reaction routing LANDED (May 4, 2026); Slice E presentation is next.** BPH-A through BPH-D, WPS-A through WPS-D, WB-A through WB-D, A1/A2/A3, B1/B2/B3, C1a, C1b, C2 preview foundation, C2 ratification mutation, and **D1/D2 settlement & cross-war reaction routing** are all landed. D1/D2 ships the new `backend/game_logic/settlement_reactions.py` module driving post-ratification settlement memories (`they_chose_us`, `settlement_gratitude`, `sold_out_by_war_leader`, `settlement_context`), `settlement_shut_out` grievance flags through the existing `_add_grievance_flag(..., source_episode_type="settlement_reaction")` machinery, sold-out-by-leader memory + relation deltas for material burdens (territory / vassalage / forced alliance / liberation), WB-B bargain breach detection when a French claim region is awarded to a different beneficiary, bounded cross-war reaction scan via `world.get_war_instances_by_participant(...)` writing `rival_strengthened` grievance flags through the same `grievance_modifier` path, and `settlement_summary` + `settlement_digest` events with spec §11.6 settlement route metadata. The orchestrator is wired into `ratify_settlement_confirm` AFTER cache invalidation but BEFORE the dialogue is popped, satisfying spec §11 line 1239. New `settlement_gratitude_mod()` acceptance hook `+5` is live in `calculate_acceptance` between `political_subtotal_clamped` and `deal_balance`. Active settlement work is now Slice E: settlement review payload / war status panel rows / Talleyrand voice copy / advisory warnings / ledger surfacing for the new memories + reaction summaries. Map-readiness non-art closure remains complete and art-blocked only. |
