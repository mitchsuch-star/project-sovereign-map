...
> **Current Settlement Gate (May 6, 2026):** `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.13 is SPEC READINESS GO / IMPLEMENTATION NO-GO with the full treaty-settlement path approved and sixth-pass SPEC READINESS blockers folded in. Code that cleanup spec before any Slice G / AI-ally settlement agency work. Quality beats schedule: this must become a player-usable settlement system, not a "wired but not usable" route milestone, and visible broken/misleading behavior cannot be deferred without explicit product approval in status/spec text. The cleanup spec now requires Gate 2 to proceed in ordered slices: Foundation (SC-1 through SC-4, including the shared editor host, executor `settlement_terms` forwarding, POST schema/taxonomy/conflict matrix, empty-draft rejection, Back Out discard-confirm, draft storage/serialization/end-turn discard, write-on-preview semantics, structured editor controls, editor inline validation, GET preview non-staging, fresh ratification-time acceptance scoring, absent ratify option, accepting-leader rescoring, rescore-failure UX, threshold payload contract, and hard-stop catalog), Entry Safety (SC-8/8b/10/11/11b/12/13, including cached eligibility invalidated by `invalidate_war_instance_indexes()`, selected-target entry write matrix, selected-target diagnostics, pinned `evaluate_open_settlement_eligibility(...)` probe identity, and focused-row coalition per-war buttons including focused-ineligible fallback), Continuity (SC-14/14b/14c/14d/14e/7b/26, including active live-route precedence, dispatch active-vs-archived routing, non-empty reopen targets, pinned reopen attempt cap, `settlement:{war_id}:{turn}:{seq}` route ids, and same-war/cross-war settlement collision protection), Incoming Offers (SC-5/6/7 default defer + hide unless explicitly reversed, with offer identity/package preservation if implemented), then Presentation and Metadata (SC-15/15b/16/17/19/20/23/24/25, including ratification-time acceptance snapshots, backend blocked-acceptance payload suppression, settlement vocabulary scope, reverse-chronological merged peace/settlement history, old-save vocabulary policy, voice-family routing, and raw + clamped harshness fields with raw-reader consumers). `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` now carries inline supersession markers for old route-id and incoming-offer Slice F instructions per SC-27. Slice G remains blocked until the full treaty cleanup is implemented, behavior-tested, and manually smoked.
> **Last Updated:** May 6, 2026 (**SETTLEMENT UI CLEANUP SPEC v0.13 SIXTH-PASS READINESS FOLD-IN COMPLETE** - `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` now carries the sixth-pass requirements for same-war refresh/merge behavior, active live-route precedence, implementation-plan supersession SC-27, POST preview conflict matrix, authored peace display, draft write-on-preview, threshold scan scope, ratification-time acceptance snapshot, required voice families, editor authority, structured per-clause editor controls, pinned route-id format, end-to-end flow, settlement vocabulary scope, and eligibility probe identity. `docs/WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` lines for old route-id and incoming-offer Slice F behavior carry inline supersession markers. The spec is ready for implementation planning, but implementation remains NO-GO until Gate 2 Slice 1 starts from the ordered Foundation path and closes behavior tests before any presentation polish or Slice G work.)
