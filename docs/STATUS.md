...
> **Last Updated:** April 12, 2026 (Session 8 renderer cutover slice 1 plus audit follow-up complete: `godot-client/project-sovereign/scenes/map.gd` now delegates to a shared node-based renderer shell while preserving `update_all_regions(map_data)` for `main.gd`, the current 19-region placeholder map, and tooltip coverage. Audit follow-up replaced the mixed scaled `Control`/`Node2D` connection path with a Control-based `map_connection_layer.gd`, restored backward-compatible `update_region()` payload handling, and expanded source-level guardrails for redraw/layer compatibility. 12 targeted regression tests passed across `tests/test_map_consistency.py`, `tests/test_map_renderer_cutover.py`, and `tests/test_popup_routing_registry.py`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment.)
...
**Current Session 8 progress:** Cutover slice 1 COMPLETE, plus immediate audit hardening. The map renderer now builds scene-node layers (`WorldLayer`, connection/region/force/garrison layers) behind a new shared `map_renderer_base.gd` shell while keeping the existing `map.gd` data wrapper and `update_all_regions(map_data)` contract stable for `main.gd`. Audit follow-up moved connection rendering to a Control-based layer so zoom/pan no longer rely on a mixed scaled `Control`/`Node2D` subtree, and `update_region()` now accepts legacy string payloads as well as array/dict data. Remaining Session 8 work is the art-backed bitmap/color-map renderer, Camera2D/input cutover, and final Godot runtime smoke validation.
