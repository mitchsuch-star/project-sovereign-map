...
> **Current Settlement Gate (May 7, 2026):** Settlement UI Cleanup G2-Slice-1 through G2-Slice-5 have landed across `b699f4c`, `4310903` / `0761284`, `052c5d0`, `4884e93`, and `b2a4d7b`, with later audit/smoke-start docs on `codex/settlement-smoke-start`. The active gate is now **spec v0.20 readiness repair plus Gate 4 manual smoke**, not another G2-Slice-1 implementation start. Slice G / AI-ally settlement agency remains blocked until the updated `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.20 checks are consumed, the rejected/losing-side smoke path passes, and smoke evidence is recorded.
> **Manual settlement smoke shortcuts:** before Gate 4 manual settlement UI smoke, set `SOVEREIGN_SMOKE_START=settlement_multilateral` for the shared France vs Britain + Prussia war. Use `SOVEREIGN_SMOKE_START=settlement_losing` or `settlement_rejected` for the v0.20 blocked-popup and concession path; if neither variant is implemented, Gate 4 remains NO-GO and must record the missing fixture instead of substituting the happy path.
> **Last Updated:** May 7, 2026 (**SETTLEMENT SPEC v0.20 CONSOLIDATION — SPEC READINESS NO-GO / IMPLEMENTATION NO-GO UNTIL REPAIR + SMOKE** — review-history prose was removed from `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` and the live Claude/Codex findings were folded into canonical rules: no `Wait for Enemy Offer` control while SC-5 is deferred; no disabled `Revise Terms` placeholder before an edit-capable route exists; `Seek Armistice with <target>` must be a target-specific bridge that preserves drafts and is absent when ineligible; losing-side concessions are part of the first editor floor; rejected-state controls are pinned by a state table; reopen-cap recovery needs clickable War Detail / Settlement History routes; rescore failure needs delta payloads; editor layout, clause vocabulary, empty-picker behavior, save/load draft signals, same-war merge conflict UI, and losing-side smoke coverage are now normative. Next: update/verify v0.20 checks and run Gate 4 manual smoke, including the rejected/losing variant.)
...
| **Current Phase** | **Peace Deals - Settlement UI Cleanup v0.20 repair + Gate 4 smoke.** G2-Slice-1 through G2-Slice-5 are already landed; do not route implementers back to G2-Slice-1. The next work is to consume the v0.20 spec fixes, verify STATUS/SC-27 scans, and run manual smoke including the rejected/losing concession path. Do not start Slice G AI/ally settlement agency until v0.20 is reconciled and Gate 4 smoke evidence is recorded. |
...
| **Blockers** | Settlement UI Cleanup remains NO-GO until v0.20 spec checks are reconciled and Gate 4 smoke, including rejected/losing smoke, is recorded. Commissioned-art renderer smoke remains blocked on art assets. |
