...
> **Current Settlement Gate (May 5, 2026):** `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.5 is audit-complete with the Codex + Claude settlement cleanup findings folded in; code that cleanup spec before any Slice G / AI-ally settlement agency work. Quality beats schedule: this must become a player-usable settlement system, not a "wired but not usable" route milestone, and visible broken/misleading behavior cannot be deferred without explicit approval in status/spec text. This gate was added after the Slice F closeout exposed a player-facing no-op affordance: `Revise Terms` reopened settlement review without any real term-editing flow. The cleanup spec now starts with that blocker and tracks adjacent settlement UI gaps: misleading buttons, command-box fallback, generic mailbox fallback, stale popup state, lost `war_id` / `route_id`, one-to-one war common-peace affordances, incoming-offer scaffolding, route-id collisions, active-vs-archived review routing, raw/debug copy, and source-string-only tests. Slice G remains blocked until the cleanup spec is implemented, behavior-tested, and manually smoked.
