...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `9004 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend tests`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals — WPS-B audit follow-up complete (April 26, 2026).** BPH-A through BPH-D + WPS-A + WPS-B all landed. Next coding slice is **WPS-C** (forced alliance + liberation). Map-readiness non-art closure remains complete and art-blocked only. |
