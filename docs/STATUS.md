...
> **Last Updated:** April 28, 2026 (**IMPERIAL SETTLEMENT SPEC v1.8 SYNTHESIS CLOSURE READY** - `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` and `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` are aligned for Slice A coding after the Codex/Claude full-audit synthesis. Closure hardening now includes explicit ARMISTICE pair-status lifecycle inside `war_instance`, common peace locked to PEACE/treaty outcomes rather than armistice, all direct WAR-entry paths attached to `war_instance`, concrete `settlement_confirm` dialogue response contracts, a Pressburg-style worked acceptance example, mandatory Slice C C1/C2 split and tuning gate with `0.6` side-pressure scaling as the first adjustment knob, AI-vs-AI settlement dispatch/log visibility, and deterministic settlement-memory cleanup/archive retention.)
