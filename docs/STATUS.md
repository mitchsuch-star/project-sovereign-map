...
> **Last Updated:** April 25, 2026 (Peace Deals **BPH-A COMPLETE** — Term ownership + display labels. `annotate_peace_terms()` in `diplomatic_templates.py` annotates every clause/sweetener/demand with `from_nation`, `to_nation`, `term_direction`, `display_label` per BPH §7.1/§7.2. `peace_ratified` campaign log event emitted on WAR/ARMISTICE→less-hostile ratification with full annotated terms payload. Annotated terms carried through dialogue payload to Godot; `proposal_confirm_popup.gd` groups terms by direction (France demands / France concedes / Mutual) with color-coded bullets. Dispatch template + HIGH priority registered. 22 new tests in `tests/test_bph_a_term_ownership.py`. Verification: full suite `8800 passed, 1 skipped`; ruff green. **Next coding slice: BPH-B** (Peace preview panel + war context).)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8800 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend tests`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals Phase A — BPH-A COMPLETE (April 25, 2026).** Term ownership annotation + display labels + `peace_ratified` event + Godot annotated-terms rendering landed. Memory and Pressure v2.4.3 fully closed. Next coding slice is **BPH-B** (Peace preview panel + war context). Map-readiness non-art closure remains complete and art-blocked only. |
...
**Next diplomacy workflow:** **BPH-A COMPLETE.** Next is **BPH-B: Peace preview panel + war context (~18 tests)** — war context snapshot at proposal time, `GET /diplomatic_preview` extension for peace-class proposals, Godot Peace Preview Panel (war summary + terms review + empty consequences section). Follow `docs/PEACE_DEALS_UMBRELLA_SPEC.md` slice order. BPH and WPS slices can be interleaved.
...
**Peace Deals spec gate (Apr 26):** CLOSED by Run #5 GO. The dedicated Peace Deals umbrella now stitches together Bilateral Peace Hardening, War Purpose + Score Semantics, `WAR_BARGAIN_SPEC.md`, and Ally Participation/Common Peace. **BPH-A landed; next is BPH-B.**
