...
> **Last Updated:** April 21, 2026 (Memory and Pressure v2.4.3 second-pass audit fixes. Seven live-doc fixes applied: **D1 pacing honesty** — `60%+` is the declared first guaranteed-hardening band in `RELIABILITY_COMMITMENTS_SPEC.md` §7.8.4; the `50%` beat is the proper-noun reveal, not a hardening promise (B-Hegemony closes on `60%+` behaving correctly, not on `50-59%` being net-positive). **D2/E8 voice fallback chain made authoritative across all three owners** — named envoy → Talleyrand advisory → chancery, pinned in `COMMITMENTS_PRESENTATION_SPEC.md` §8.1 table + note + `DIPLOMAT_VOICE_BIBLE.md` bloc-naming contract; the three docs do not disagree. **D3/E9 "deferred" / "un-deferred" hedging language purged** — per-row bloc badges ship as Slice E-Cards, the final slice of this plan, fully scoped with tests and acceptance gates; no "deferred", "conditional", or "in-scope-but-last" framing remains. Slice E-Cards is a schedule constraint (headline first, stamps last) rather than a deferral; closing Memory and Pressure requires closing the slice. **E5 acceptance formula decoupled** — new shared `_identify_max_bloc_share(world)` helper in §7.3 returns `(hegemon, share)` regardless of threshold, read by both `_calculate_hegemony_pressure` (gated at 33%+ for threat accrual) and `hegemony_target_mod` in §9.1 (gated at 30%+ for per-pair friction). The 30-33% private-tally preview-warning band in §11.2 is now mechanically backed rather than decorative. **E6 §7.4 coalition target fallback** — "coalition target = current hegemon" now falls back to `world.player_nation` when `_calculate_hegemony_pressure` returns `{}`, so event-threat brewing against France still has a named target on turns where France's bloc temporarily recedes below 33%. **E7 Balance-of-Europe payload COOLDOWN fields** — `cooldown_turns_remaining: Optional[int]` and `residual_pressure_active: bool` added to the payload contract in `COMMITMENTS_PRESENTATION_SPEC.md` §11 so the Case 5 COOLDOWN render has the data it needs without implementer improvisation. **E9 non-France-hegemon suppression** — `RELIABILITY_COMMITMENTS_SPEC.md` §11.1's "suppress OR retarget" choice is now a flat SUPPRESS rule; rendering *"Coalition pressure against the Russian Alignment: 64"* while `64` is still the anti-France scalar is a visible lie, so Case 3 / Case 4 with a non-France hegemon render only the hegemon line + flavor. Doc-only edits — B-Hegemony remains the next implementation slice.)
