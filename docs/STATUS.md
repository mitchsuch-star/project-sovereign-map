...
| **Current Phase** | **Memory and Pressure v2.4.3 — DG-4 reliability, C-lite presentation closure, and post-closeout smoke follow-up COMPLETE (April 25, 2026).** Direct-only call-to-arms, DG-4 presentation routing, defensive-refusal coalition memory snapshots, Balance of Europe ledger payload/copy, Godot notification hooks, commitment popup smoke path, vassalizable-marker gating, and witness anti-spam are live. D3 per-row bloc stamps/member badges remain deferred polish. Peace Deals is spec-gated. Map-readiness non-art closure remains complete and art-blocked only. |
...
**Next diplomacy workflow:** `Memory and Pressure` v2.4.3 core is closed. Optional follow-up is D3 Balance/ledger row clarity (per-row bloc stamps/member badges) if we want more smoke-tested Balance legibility before the next feature phase. Do **not** begin Peace Deals implementation from this status file alone: write and approve a dedicated Peace Deals implementation spec first. `docs/WAR_BARGAIN_SPEC.md` is valid for the War Bargains sub-slice, but the broader Peace Deals phase still needs an umbrella spec covering Bilateral Peace Hardening, War Purpose + Score Semantics, WB-A through WB-D sequencing, Ally Participation/Common Peace boundaries, save/load contracts, UI smoke gates, and test coverage.

**Peace Deals deferred carry-forward gate:** the Peace Deals umbrella spec is not considered ready for approval until it includes a named "Deferred Carry-Forward Checklist." That checklist must account for every deferred item routed to Peace Deals below: WB-D bargain presentation, response routes, bargain icons, fulfillment/breach badges, region-observer witness scope, scope-branched witness copy, and any callback/aftermath architecture needed for bargain breach, fulfillment, or settlement fallout. Each item must be assigned to a concrete Peace Deals slice, explicitly deferred again with rationale, or marked out-of-scope. Do not let these items disappear into generic "future polish" language.

**Deferred-item routing plan (Apr 25):**

| Deferred item | When to do it | Why / gate |
|---------------|---------------|------------|
| D3 per-row bloc stamps / member badges | Optional pre-Peace polish, only after a targeted Balance-of-Europe playtest or smoke pass says the Nations tab still lacks at-a-glance camp legibility. Otherwise leave it deferred until after the Peace Deals umbrella spec is written. | Small, bounded UI/data-contract slice. Reuse `describe_hegemon_bloc(...)`; add transient `nations[*].bloc_stamp`; never serialize it; keep the priority `[Coalition Member] > proper bloc name > descriptive bloc > Vassal > Neutral`. |
...
|-------|------------|------------------------|------|
| 1 | Memory and Pressure v2.4.3 | `R160`, `R119` | First diplomacy follow-up closure block. Blocks 1 and 2 are complete; Block 3 is SUPERSEDED; DG-4 reliability, C-lite presentation closure, and the April 25 post-closeout smoke fixes are now live. Remaining work is optional D3 Balance/ledger row polish only, plus commissioned-art smoke outside this diplomacy gate. |
...
**Peace Deals spec gate (Apr 25):** do not start Peace Deals implementation until a dedicated spec stitches together Bilateral Peace Hardening, War Purpose + Score Semantics, `WAR_BARGAIN_SPEC.md`, and Ally Participation/Common Peace. War Bargains has a spec; Peace Deals as a whole does not yet.
