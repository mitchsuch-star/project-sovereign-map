...
> **Last Updated:** April 20, 2026 (Memory and Pressure v2.4.3 Block 2 substrate alignment is complete and pushed. The next diplomacy gate is now `docs/audits/MP_V243_BLOCK3_BLOC_NAMING.md`, and the remaining major design decisions are now locked: v2.4.3 adopts deterministic bloc naming on the constrained headline/beat/warning/declaration surfaces, member badges stay deferred, and Block 3 owns the specific post-Block-2 follow-through items that were never actually finished.)
...
7. **Memory and Pressure v2.4.3 — audit blocks still gate the next diplomacy pass.** `docs/audits/MP_V243_BLOCK1_DOC_CLEANUP.md` and `docs/audits/MP_V243_BLOCK2_SUBSTRATE.md` are complete. The next diplomacy gate is now `docs/audits/MP_V243_BLOCK3_BLOC_NAMING.md`, which is the post-Block-2 closure block. The key design calls are already made: bloc naming is adopted for v2.4.3, the live surface set is Balance headline + threshold beats + proposal warnings + coalition-declaration contrast, member badges remain deferred, and Block 3 owns the specific deferred follow-through items that surfaced out of Block 2 (`C-lite` presentation follow-through, `B-B4` defensive-refusal surfacing, `B-B7` apology-loop follow-through, and composite-floor regression coverage). After Block 3 closes, the remaining B-Hegemony / B-B1-lite / B-B3 / B-B4 / B-B7 / C-lite work continues in `docs/RELIABILITY_IMPLEMENTATION_PLAN.md`. This is not part of the map-readiness gate.
...
**Next bug-owned implementation slice:** None. All non-art map-readiness items are closed. The remaining map-readiness work (commissioned art-backed renderer cutover + final Godot smoke validation) is art-blocked and sits in the Blocked list above. For a fresh session on diplomacy, the next work is Memory and Pressure v2.4.3 Block 3 closure: `docs/audits/MP_V243_BLOCK3_BLOC_NAMING.md` first. No further design-go/no-go pass is needed there: Block 3 now executes the adopted bloc-naming contract plus the specifically deferred post-Block-2 follow-through items, and only after that closes should the remaining implementation slices resume.
...
|-------|------------|------------------------|------|
| 1 | Memory and Pressure v2.4.3 | `R160`, `R119` | First diplomacy follow-up closure block. Blocks 1 and 2 are complete, so the next pass is `docs/audits/MP_V243_BLOCK3_BLOC_NAMING.md`, which now executes an already-adopted bloc-naming contract and also owns the specific deferred post-Block-2 work (`C-lite` presentation follow-through, `B-B4` defensive-refusal surfacing, `B-B7` apology-loop follow-through, composite-floor tests). After Block 3 closes, the remaining B-Hegemony / B-B1-lite / B-B3 / B-B4 / B-B7 / C-lite work continues in `docs/RELIABILITY_IMPLEMENTATION_PLAN.md` v2.4.3 and `docs/COMMITMENTS_PRESENTATION_SPEC.md` v0.5.1. |
...
- Block 3 bloc naming / post-Block-2 closure is now next: `docs/audits/MP_V243_BLOCK3_BLOC_NAMING.md` is the queued closure block. The bloc-naming call is already locked in its favor, the surface scope is constrained, member badges are deferred, and the block now owns the specific deferred post-Block-2 items (`C-lite`, `B-B4`, `B-B7`, composite-floor tests) that were never actually finished elsewhere.
- After the remaining audit blocks land, the live implementation order is the v2.4.3 plan surface: B-Hegemony, B-B1-lite, B-B3, B-B4, B-B7, C-lite (trimmed).
