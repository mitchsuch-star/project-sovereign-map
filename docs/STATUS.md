...
> **Last Updated:** April 26, 2026 (Peace Deals **WPS-B COMPLETE** — Vassalage power cap. `calculate_national_power()` now sums controlled-region income + 50% vassal-region income + British naval income (min(300, 150+50*coastal)). `check_vassalage_power_cap()` hard-gates vassalage at 50% power ratio. `project_power_after_terms()` projects post-cession power without mutating WorldState. Gates wired into: `create_vassal_treaty`, `create_vassal_conquest`, wizard `_proposal_action` for VASSAL, and `get_available_war_objectives` subjugation availability. Starting powers: France 1100, Austria 650, Britain 400, Prussia 400, Saxony 250 — Austria (59%) is the only nation above the 50% cap at game start. Spec §8.5 starting-power table corrected to reflect actual naval formula. 24 new tests in `tests/test_wpsb_power_cap.py`. Verification: focused WPS-B suite `24 passed`; full suite `8993 passed, 1 skipped`; ruff green. **Next coding slice: WPS-C** (forced alliance + liberation).)
...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8993 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend tests`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals — WPS-B COMPLETE (April 26, 2026).** BPH-A through BPH-D + WPS-A + WPS-B all landed. Next coding slice is **WPS-C** (forced alliance + liberation). Map-readiness non-art closure remains complete and art-blocked only. |
...
**Next diplomacy workflow:** **WPS-A AUDIT FOLLOW-UP COMPLETE.** BPH-A through BPH-D and WPS-A are landed. Next is **WPS-B: vassalage power cap gating** — enforce the 50% national-power cap in settlement production/legibility and keep subjugation availability aligned with the war-purpose validation path. Follow `docs/PEACE_DEALS_UMBRELLA_SPEC.md` slice order.
