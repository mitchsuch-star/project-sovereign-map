...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8775 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend tests`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Memory and Pressure v2.4.3 — DG-4 reliability, C-lite presentation closure, post-closeout smoke follow-up, and D3 Nations-tab bloc stamps COMPLETE (April 25, 2026).** Direct-only call-to-arms, DG-4 presentation routing, defensive-refusal coalition memory snapshots, Balance of Europe ledger payload/copy, Godot notification hooks, commitment popup smoke path, vassalizable-marker gating, witness anti-spam, and per-row `bloc_stamp` badges are live. Peace Deals is spec-gated. Map-readiness non-art closure remains complete and art-blocked only. |
