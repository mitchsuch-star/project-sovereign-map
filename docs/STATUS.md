...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8808 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend tests`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Peace Deals Phase A — BPH-A COMPLETE + AUDIT FOLLOW-UP CLOSED (April 26, 2026).** Term ownership annotation + display labels + `peace_ratified` event + Godot annotated-terms rendering landed and follow-up fixes are verified. Memory and Pressure v2.4.3 fully closed. Next coding slice is **BPH-B** (Peace preview panel + war context). Map-readiness non-art closure remains complete and art-blocked only. |
