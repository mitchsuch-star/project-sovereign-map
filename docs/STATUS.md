...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8751 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Ruff is green for `backend tests`. Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Memory and Pressure v2.4.3 — DG-4 reliability and C-lite presentation closure COMPLETE (April 25, 2026).** Direct-only call-to-arms, DG-4 presentation routing, defensive-refusal coalition memory snapshots, Balance of Europe ledger payload, Godot notification hooks, and witness anti-spam are live. Map-readiness non-art closure remains complete and art-blocked only. |
| **Blockers** | No active blocker on v2.4.3 reliability/presentation code. `threat_coalition` remains as a compatibility payload while `balance_of_europe` is live; commissioned-art renderer smoke remains blocked on art assets. |
...
2. **Memory and Pressure v2.4.3 — DG-4 + C-lite closure COMPLETE.** DG-4 call-to-arms is now direct-only with root attacker/defender allies and direct vassals only; `war_entry_ledger` records honored/refused/vassal paths; all three call-to-arms families emit CRITICAL notification rail entries from the shared §8.1 routing table; defensive-refusal coalition memory reads refusal-moment treaty snapshots; witness dispatch presentation collapses same-episode witness strikes; `balance_of_europe` now ships beside the legacy `threat_coalition` compatibility payload and Godot renders the Balance headline.
...
|-------|------------|------------------------|------|
| 1 | Memory and Pressure v2.4.3 | `R160`, `R119` | First diplomacy follow-up closure block. Blocks 1 and 2 are complete; Block 3 is SUPERSEDED; DG-4 reliability and the C-lite presentation closure items covered by the April 25 audit pass are now live. Remaining work is follow-up polish only, plus commissioned-art smoke outside this diplomacy gate. |
