...
> **Current Settlement Gate (May 6, 2026):** `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.15 is SPEC READINESS GO / IMPLEMENTATION NO-GO with the full treaty-settlement path approved and eighth-pass SPEC READINESS tightenings folded in. Code that cleanup spec before any Slice G / AI-ally settlement agency work. Quality beats schedule: this must become a player-usable settlement system, not a "wired but not usable" route milestone, and visible broken/misleading behavior cannot be deferred without explicit product approval in status/spec text. The cleanup spec now requires Gate 2 to proceed in ordered slices: Foundation (SC-1 through SC-4, including inline edit mode on the existing settlement review popup, editor `EDIT`/`REVIEW` mode transitions, editor-mode and review-mode ratify controls bound to the same acceptance/hard-stop gate, executor `settlement_terms` forwarding, POST schema/taxonomy/conflict matrix, empty-draft rejection, empty-draft Back Out behavior, Back Out discard-confirm, draft storage/serialization/end-turn discard, write-on-preview semantics, structured editor controls, editor inline validation, GET preview non-staging, fresh ratification-time acceptance scoring, absent ratify option, accepting-leader rescoring, rescore-failure UX, threshold payload contract, failed-ratification no-history behavior, and hard-stop catalog), Entry Safety (SC-8/8b/10/11/11b/12/13, including cached eligibility invalidated by `invalidate_war_instance_indexes()`, selected-target entry write matrix, selected-target diagnostics, pinned `evaluate_open_settlement_eligibility(...)` probe identity, and focused-row coalition per-war buttons including focused-ineligible fallback), Continuity (SC-14/14b/14c/14d/14e/7b/26, including active live-route precedence, click-time active-vs-archived re-resolution, dispatch active-vs-archived routing, non-empty reopen targets, dual-empty reopen fallback, pinned reopen attempt cap, `settlement:{war_id}:{turn}:{seq}` route ids, merge-conflict handling, AI-vs-player draft protection for later Slice G, and same-war/cross-war settlement collision protection), Incoming Offers (SC-5/6/7 default defer + hide unless explicitly reversed, with notification rail / popup queue / pending-envoy / mailbox-activate takedown coverage and offer identity/package preservation for accept/reject/request if implemented), then Presentation and Metadata (SC-15/15b/16/17/19/20/23/24/25, including ratification-time acceptance snapshots, backend blocked-acceptance payload suppression, settlement vocabulary scope, reverse-chronological merged peace/settlement history, canonical clause humanization across all eight clause types, Voice Bible section 16.1 authored settlement families, bilateral history producer namespace, old-save vocabulary policy, voice-family routing, and raw + clamped harshness fields with raw-reader consumers). `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` carries inline supersession markers for old route-id and incoming-offer Slice F instructions per SC-27. Slice G remains blocked until the full treaty cleanup is implemented, behavior-tested, and manually smoked.
> **Last Updated:** May 6, 2026 (**SETTLEMENT UI CLEANUP SPEC v0.15 EIGHTH-PASS READINESS FOLD-IN COMPLETE** - `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` now carries eighth-pass requirements for editor-mode ratify gating, editor `EDIT`/`REVIEW` transitions, authored peace dispatch wording, failed-ratification history behavior, incoming-offer notification/popup/pending-envoy/mailbox takedown surfaces, all-clause humanization tests, Voice Bible authored settlement families, and AI-vs-player draft protection for later Slice G. The spec is ready for implementation planning, but implementation remains NO-GO until Gate 2 Slice 1 starts from the ordered Foundation path and closes behavior tests before any presentation polish or Slice G work.)
...
| **Current Phase** | **Peace Deals - Imperial Settlement final closure.** A1-E backend/presentation work is landed, but final player-readiness now requires Slice F UI/UX closure before smoke: wizard `open_settlement` routing, explicit read-only lazy-backfill probe, free non-DP-gated opener, humanized disabled reasons/terms/standing/warnings/acceptance bands, readable `settlement_confirm` popup with war label/review sections/top-blocker/stale-state behavior/result feedback, one typed action schema, concrete `reopen_target`, staging-time stable `route_id`, war-detail/coalition-detail CTAs with war-scoped `war_id` entry, `card_clicked(..., war_instance_id)` routing, war-status no-instance placeholder, ledger focus/humanization, minimal `incoming_settlement_offer` backend handlers + Godot routing, cross-war settlement collision handling, notification route metadata continuity, raw-enum scans, and focused tests. After Slice F smoke, Slice G owns full AI/ally settlement agency: AI war-leader offer generation plus non-leader ally petitions/advisories without veto or treaty authorship. |
| **Next Roadmap Gate** | After Slice F, final smoke, Slice G, and final settlement gate, run the new `ROADMAP.md` **8.EVAL Pre-8.5 War LLM + Diplomacy Refinement Evaluation**. It must recover/triage buried war-LLM improvements, battle/war narration and creative-command war uses, LLM cost/toggle implications, and `DESIGN_REFINEMENT.md` diplomacy candidates before Phase 8.5 begins. |
