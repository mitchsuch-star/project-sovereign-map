...
**Next diplomacy workflow:** `Memory and Pressure` v2.4.3 core plus D3 Balance/ledger row clarity are closed. Peace Deals passed Run #5 audit with GO on April 26, 2026. Begin implementation with **BPH-A: Term ownership + display labels**, following `docs/PEACE_DEALS_UMBRELLA_SPEC.md` slice order and gate checks.
...
**Peace Deals spec gate (Apr 26):** CLOSED by Run #5 GO. The dedicated Peace Deals umbrella now stitches together Bilateral Peace Hardening, War Purpose + Score Semantics, `WAR_BARGAIN_SPEC.md`, and Ally Participation/Common Peace. Start coding next with **BPH-A**.
