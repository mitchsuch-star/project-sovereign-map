...
> **Last Updated:** March 29, 2026 (R16 DialogManager + Layer Subdivision COMPLETE — created dialog_manager.gd (DialogManager class_name: register/get_dialog/is_any_modal_open/hide_all), subdivided Layer 100 into 18 unique layers (101-118) across all popup .tscn files, pause_menu→120, refactored main.gd _ready() from ~275 lines boilerplate to ~140 lines via DialogManager.register(), simplified _is_modal_dialog_open() from ~45 lines to 2-line delegation, main.gd 3157→2982 lines (-175), 7755 tests passing)
...
1. **Playtest Audit (March 29, 2026)** — 3 automated playtests found 1 CRITICAL + 3 MAJOR + 2 MINOR bugs + 1 design discussion. 2 required sessions + 1 design gate. See `docs/PLAYTEST_AUDIT_2026_03_29.md`.
   - **Session 1:** PT-1 turn skip (CRITICAL, known C3 — idempotency guard redesign), PT-2 mock parser "status", PT-3 cavalry emoji encoding.
   - **Session 2:** PT-4 armistice "Unknown target" attack error, PT-5 pursue AP waste during armistice, PT-6 AP warning on end turn.
   - **Design discussion:** Aggressive play balance (defensive turtle dominant, needs momentum/shock/blitz mechanics).
1. **Architecture Refactoring** — **IN PROGRESS.** 20 R-items, 22 sessions, 7 phases. Session 1 (R3 conftest) COMPLETE — created `tests/conftest.py` with MarshalFactory/WorldFactory + 25 validation tests, migrated 5 test files (253 tests). Session 2A (R1 characterization tests) COMPLETE — 25 tests pinning current post-combat behavior of all 5 combat paths. Session 2B (R1 pipeline extraction + 7 bug fixes) COMPLETE — created `_post_combat_pipeline()` (14-step centralized method), wired all 5 combat paths, fixed 7 bugs (decisive_victory casualty check, war score inflation, garrison/charge missing steps, coordination clear, bombardment diplo, pursuit floor, FORCED_RETREAT_THRESHOLD dedup), 30 enforcement tests, 6 characterization tests updated. Session 3 (R2 war-state helper layer) COMPLETE — added 5 helpers to WorldState (`are_allies`, `can_interact_diplomatically`, `get_hostile_marshals_in_region`, `get_friendly_marshals_in_region`, `get_nations_at_war_with`), created centralized `set_diplomatic_state()` in diplomacy.py with automatic bookkeeping (war_start_turns, armistice cleanup, treaty removal), migrated all 23 direct diplomatic_states[] writes across 5 files, 47 new tests. Session 4 (R4 response pipeline) COMPLETE — created `build_base_response()` and `_build_result_response()` in main.py, migrated all 8 gameplay POST endpoints + /load + /notifications/dismiss + /cancel_order. Structurally guarantees popup passthroughs (eliminates "Bug 5" pattern), diplomatic top-bar fields in every response (previously missing from objection/redemption/capture/etc.), notifications. Removed 26 manual `_include_popup_passthroughs()` calls, 7 "Bug 5" comments. 45 new tests. Session 5 (R5 fog-filtered data access) COMPLETE — added `get_visible_enemies()` to WorldState and `visibility_at_least()` to RegionIntel, fixed 4 fog leaks (strategic parser 3 sites + main.py map_data), migrated 4 additional callers in executor.py (2 new fog guards + 2 inline simplifications), audited all 26 `get_enemies_of_nation()` call sites (8 migrated, 5 omniscient mechanics, 1 debug, 12 AI). 42 new tests. Session 6 (R7+R8 display names + campaign log) COMPLETE — created `backend/display_names.py` (single source of truth: 11 display maps consolidated from 5 files + 4 new maps), migrated all 7 import sites across 6 files, added `_display` fields to 3 endpoints (pending_objection, respond_to_objection, debug diplomatic_states), added 16 previously-silent event types to CAMPAIGN_LOG_TYPES (45 total) with format strings + fog rules + categories, 99 enforcement tests in `test_campaign_log_enforcement.py`. Session 7 (R6 CooldownManager + PopupQueue) COMPLETE — created `backend/models/cooldown_manager.py` with CooldownManager (5 registered dict/scalar cooldowns, `decrement_all()`) and PopupQueue (7 popup types, priority-ordered `pop_highest()`). Added 12 backward-compatible properties on WorldState (5 cooldowns + 7 popups). Replaced 4 `_decrement_*` methods + inline talleyrand decrement with single `decrement_all()` call. Simplified `_include_popup_passthroughs()` to use `PopupQueue.pop_highest()`. 64 new tests (20 characterization + 44 unit/enforcement). Session 8 (R9+R20 Scaling Index + advance_turn Guard) COMPLETE — added `_marshals_by_region` transient index to WorldState with `_build_marshal_index()` (O(N) rebuild, O(1) lookup), `_get_marshals_in_region_indexed()` for internal hot paths. Optimized 2 Tier-1 hot spots: `calculate_visibility()` and `process_supply_attrition()` (O(R*N) → O(R+N)). Index auto-rebuilt at `__init__`, `_advance_turn_internal()` start + before final visibility calc, and `from_dict()`. Public `get_marshals_in_region()` unchanged (linear scan for backward compat). Added `_last_advanced_turn` idempotency guard: stamps at start of `_advance_turn_internal()`, rejects double-calls for same turn. Serialized in `to_dict()`/`from_dict()`, defaults to 0 for old saves. 28 new tests (8 index + 5 internal methods + 3 rebuild timing + 12 idempotency + 2 serialization). Session 9 (R18 Test Enforcement Suite) COMPLETE — created `tests/test_enforcement_suite.py` with 7 test classes (21 tests) enforcing cross-module consistency: VALID_ACTIONS ↔ _action_costs, VALID_ACTIONS ↔ ACTION_DISPLAY, VALID_ACTIONS ↔ parser ↔ executor dispatch, proposal types ↔ PROPOSAL_TYPE_DISPLAY, diplomatic states ↔ STATE_DISPLAY, free_actions list sync, campaign log completeness. Found and fixed 11 latent gaps: 6 missing explicit AP costs (drill/fortify/unfortify/charge/restrain/cancel), 4 missing display names (charge/restrain/cancel/economy), 1 missing proposal type display (harsh_peace). Session 10A (R10A Executor Split: Combat Core) COMPLETE — created `backend/commands/combat_executor.py` (CombatExecutor class, ~3,700 lines) extracted from executor.py (14,802→11,101 lines). Extracted 21 combat methods: _execute_attack (~1,760 lines), _execute_bombardment (~375 lines), _execute_glorious_charge, _execute_charge, _execute_form_square, _execute_break_square, _resolve_garrison_combat, _post_combat_pipeline, _handle_forced_retreat, _apply_forced_retreat_or_break, _distribute_casualties, _get_casualty_participants, _apply_battle_effects_to_region, _log_battle_event, _process_combat_notifications, _attempt_region_capture, _apply_plunder/_apply_secure, respond_to_glorious_charge. CombatExecutor.__init__(parent_executor) stores _executor ref + combat_resolver. Methods referencing non-combat executor methods use self._executor.X (12 remapped: _auto_break_square, _fuzzy_match_*, _calculate_coordination/reinforcements/overwatch/movement_attrition, _clear_coordination_fields, _execute_strategic_command, _execute_wait, _has_depot_supply_bonus, fuzzy_matcher). `__getattr__` backward compat on CommandExecutor for test code. 3 fixes: monkeypatch target (CombatExecutor class), enforcement suite scans both files, _current_sortie reads from _executor. Session 10B (R10B Audit 10A + Coordination Extraction) COMPLETE — Phase 1 audit of 10A extraction all clean (10 delegated methods verified, no circular imports, dispatch routing correct, full test suite green). Phase 2: extracted 13 coordination methods (_count_unit_types, _get_combined_arms_bonus, _calculate_per_ally_coordination, _count_adjacent_allies, _calculate_coordination_context, _has_dedicated_support, _is_reinforcement_eligible, _calculate_arrival_score, _calculate_reinforcements, _COORDINATION_FIELDS, _clear_coordination_fields, _calculate_overwatch, _RELATIONSHIP_SCALING) + 6 auto-dispatch methods (_execute_general_attack, _execute_general_attack_combat, _execute_auto_assign_attack, _execute_auto_assign_bombardment, _execute_general_retreat, _execute_general_defensive) from executor.py to combat_executor.py. executor.py 11,101→10,153 lines (-948), combat_executor.py 3,752→4,713 lines (+961). Updated _COMBAT_DELEGATED set (20→31 methods), dispatch routing (5 command_types → self._combat.X). 2 test fixes (class attribute references: _COORDINATION_FIELDS, _RELATIONSHIP_SCALING). Updated self._executor.X references for now-local methods (4 call sites). 7707 tests passing (no new tests — pure restructuring). Session 11 (R11 Executor Split Phase 2: Diplomatic + Strategic) COMPLETE — created `backend/commands/strategic_executor.py` (StrategicExecutor class, ~1,833 lines, 10 methods: _execute_strategic_command, _handle_strategic_objection_response, _handle_first_step_blocked, _execute_cancel, _resolve_generic_target, _find_nearest_enemy, _build_clarification, _handle_strategic_objection_from_endpoint, _generate_mild_concern_message, _generate_objection_message) and `backend/commands/diplomatic_executor.py` (DiplomaticExecutor class, ~2,303 lines, 19 methods: _execute_diplomatic router, 8 diplomatic action handlers, _apply_diplomatic_trust_reactions, handle_diplomatic_dialogue_response, _process_dialogue_choice, 4 dialogue helpers, 3 AI proposal handlers). executor.py 10,153→6,148 lines (-4,005). Added _STRATEGIC_DELEGATED (10 methods) and _DIPLOMATIC_DELEGATED (19 methods) sets. Updated __getattr__ for 3-way delegation (combat/strategic/diplomatic). Updated dispatch routing: 4 explicit self._strategic/self._diplomatic calls. 7,707 tests passing (no new tests — pure restructuring). Session 12A (R12A DialogueManager Foundation) COMPLETE — created `backend/models/dialogue_manager.py` (DialogueManager class: push/replace/pop/peek/is_blocking/clear_stale/promote_if_empty/remove_matching, DIALOGUE_PRIORITY dict, QUEUE_CAP=20, BLOCKING_TIMEOUT_TURNS=2, serialization with deepcopy). Integrated into WorldState with transparent backward-compat properties (pending_diplomatic_dialogue getter/setter → _dialogue_manager._current, pending_dialogue_queue getter/setter → _dialogue_manager._queue, dialogue_manager property). Updated to_dict() (writes both dialogue_manager key + legacy flat keys) and from_dict() (reads new format first, falls back to legacy). 48 new tests: 13 characterization (set/clear/queue/auto-pop/blocking/timeout/serialization/paired popup/dispatch conditional) + 35 unit (push/replace/pop/peek/is_blocking/clear_stale/promote_if_empty/remove_matching/priority/serialization). 7,755 tests passing. Session 12B (R12 Core Migration: diplomatic_executor.py) COMPLETE — migrated all 65 `pending_diplomatic_dialogue` write operations (22 `replace()`, 41 `pop()`, 2 deleted). Session 12C (R12 Consolidation) COMPLETE — migrated 9 remaining backend files (world_state.py, main.py, dispatch.py, turn_manager.py, vassal.py, coalition.py, ai_diplomacy.py, diplomacy.py, executor.py). Removed duplicate auto-pop in main.py (14 lines) and `_pop_dialogue_queue()` + `_DIALOGUE_TYPE_PRIORITY` in turn_manager.py (20 lines) → replaced with `dialogue_manager.promote_if_empty()`. Consolidated non-blocking auto-dismiss + blocking safety valve in world_state.py → `dialogue_manager.clear_stale()`. Converted counter-offer SET to `push()` (fixes latent overwrite bug). `coalition.py` and `vassal.py` queue filters → `remove_matching()`. Migrated 285 operations across 29 test files. Removed `pending_diplomatic_dialogue` setter and `pending_dialogue_queue` property. Zero setter calls remain in backend or tests. 7,755 tests passing. Session 13A (R13A Executor Split Phase 3a) COMPLETE — created 4 sub-executors: `vassal_executor.py` (VassalExecutor, 147 lines, 4 methods), `capture_executor.py` (CaptureExecutor, 94 lines, 1 method), `economy_executor.py` (EconomyExecutor, 804 lines, 6 methods + 2 helpers + 5 class constants), `tactical_executor.py` (TacticalExecutor, 715 lines, 9 methods). executor.py 6,148→4,483 lines (-1,665). Added 4 delegation sets + `__getattr__` wiring. Class-level constant delegation for backward compat (GARRISON_*, WATCHTOWER_*). Enforcement suite updated to scan all 8 executor files. 7,755 tests passing (no new tests — pure restructuring). Session 13B (R13B Executor Split Phase 3b) COMPLETE — created `backend/commands/movement_executor.py` (MovementExecutor class, ~680 lines, 6 methods: _has_depot_supply_bonus, _calculate_movement_attrition, _execute_move, _execute_scout, _execute_auto_assign_scout, _execute_retreat_action) and `backend/commands/meta_executor.py` (MetaExecutor class, ~1,900 lines, 8 methods: _execute_end_turn, _apply_grouchy_ambiguity_buff, _execute_status, _execute_help, _execute_debug, _execute_cheat, handle_objection_response, _execute_post_objection + module-level _filter_tactical_events_by_fog, _action_display_name, _proposal_display_name). executor.py 4,483→1,554 lines (-2,929). Added _MOVEMENT_DELEGATED (6 methods) and _META_DELEGATED (8 methods) delegation sets. Removed unused imports (random, TERRAIN_DEFENSE_BONUS, List, _ACTION_DISPLAY_NAMES). Enforcement suite updated: scans 10 executor files, free_actions sync across executor+meta_executor, _execute_specific regex handles end-of-class. 1 test fix (mock patch target: executor.random→meta_executor.random). 7,755 tests passing (no new tests — pure restructuring). Session 14 (R17 HTTP Timeout + api_client Consolidation) COMPLETE — added 30s timeout to HTTPRequest, created `_get()`/`_post()` generic helpers, refactored all 22 endpoint methods to thin 1-line delegators, added timeout + non-success result handling in `_on_request_completed()`, improved error messages (includes HTTP status code). api_client.gd 348→122 lines (-65%). All 34 caller sites across 7 .gd files verified unchanged. 7,755 tests passing. Session 15 (R15 utils.gd + PopupBase) COMPLETE — created `utils.gd` (class_name Utils: 19 COLOR_ constants, NATION_COLORS dict, bbcode_color/format_number helpers) and `popup_base.gd` (class_name PopupBase: close_popup, _disable_all_buttons, _apply_standard_theme). Migrated 3 popups to extend PopupBase (coalition_declaration, alliance_paradox, interrupt). Replaced color constants across 12 .gd files with Utils.COLOR_* references (496 refs total, ~75 local const lines removed). 7,755 tests passing. Session 16 (R16 DialogManager + Layer Subdivision) COMPLETE — created `dialog_manager.gd` (DialogManager class_name: register/get_dialog/is_any_modal_open/hide_all, centralized dialog registry with modal tracking). Subdivided Layer 100 into 18 unique layers (101-118) across all popup .tscn files, pause_menu 101→120. Refactored main.gd _ready() from ~275 lines of load/instantiate/null-check/add_child/connect boilerplate to ~140 lines via DialogManager.register(). Simplified _is_modal_dialog_open() from ~45 lines (20 null-and-visible checks) to 2-line DialogManager delegation. Screen instantiation (5 screens) consolidated to config-driven loop. main.gd 3,157→2,982 lines (-175). 7,755 tests passing. See `docs/ARCHITECTURE_REFACTORING_PLAN.md`.
...
2. ~~**Systems Audit V2 Fix Plan**~~ — **ALL 7 SESSIONS COMPLETE.** 56 bugs fixed, 7040 tests. See `docs/SYSTEMS_AUDIT_V2_FIX_PLAN.md`.
   ...
   - **Session 7 (P3 — optional): COMPLETE.** Test quality hardening. Deleted 5 pass-placeholder tests, added assertions to ~35 zero-assertion tests, rewrote 8 print-only tests, fixed 5 conditional assertions, wrote 8 new coverage tests (fog/trust save-load/multi-cavalry), loosened 3 fragile string assertions, removed docstring tests, rewrote futility tests to use production code. V2-41 (shared conftest) deferred. +8 net tests (7040 total).
   - **IMPORTANT: After each session, update this STATUS.md to mark it done and indicate which session is next.**
