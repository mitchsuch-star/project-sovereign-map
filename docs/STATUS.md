...
**Current gate (May 11, 2026):** `docs/SETTLEMENT_UI_CLEANUP_SPEC.md` v0.24 remains **SPEC READINESS NO-GO / IMPLEMENTATION NO-GO** until this combined Codex + Claude iterative review is re-scored, SC-27 doc scans pass on the implementation plan and parent settlement spec, executable Godot evidence exists for SC-1 / SC-10b / SC-28, rejected/losing recovery is verified, branch-target reconciliation is recorded, and Gate 4 manual smoke passes.

**Canonical follow-up edits now folded:** rejected multi-party recovery must offer `re_author_with_concessions` when edit/baseline recovery is available; attempt-4 stale recovery must dismiss or replace the active hard-stop popup so recovery is clickable and preserve or invalidate drafts with a player signal; Covered Enemies remains a required editor control; discard-confirm has two named actions and outer-cancel keeps the draft; EDIT uses `Submit for Review`, not bare `Submit`; `Open Settlement History` must visually land in Settlement History / `PEACE & SETTLEMENT HISTORY`; settlement summary and digest share the same staged route id; parent leader-change, lazy-backfill, queue/defer, and digest-route instructions are superseded; Voice Bible 16.1 recovery families are authored; `gold_per_turn`, `vassalage`, `subjugation`, and `liberation` are interim-hidden from `available_clause_types[]`, Add Clause controls, concession baselines, presets, mailbox/request-terms payloads, and generated player copy until SC-33 / SC-31 close.

## Last Recorded Spec Review Result

**May 11, 2026 combined v0.24 iterative synthesis:** Codex + Claude v0.22/v0.23 review findings are folded into the cleanup spec, Voice Bible, implementation plan, parent settlement spec, and this STATUS file. Verdict remains **NO-GO** until a new scored review confirms all categories are at least 8/10 and no P0/P1 blockers remain. The folded blocker list now includes Voice Bible 16.1 recovery families, SC-10b War Detail pair substitutes, `can_edit_terms` / `available_clause_types[]` schema alignment, reopen-attempt world storage and draft lifecycle, concession baseline trigger and placement, Replace-this-clause, bilateral `participants_signature`, `open_war_detail` handler/fallback and one-to-one no-substitute unblock copy, named conflict matrix, hidden canonical-clause artifacts, dependency-clause interim hides, `gold_per_turn` landing ownership, typed-command supersession, pending incoming-offer defensive drop, selected-target invalidation, applied-clause preview shape, coalition overflow, STATUS/SC-27 alignment, and parent-doc supersession scan patterns.
...
Before Gate 4 manual settlement UI smoke, set `SOVEREIGN_SMOKE_START=settlement_multilateral` for the shared France vs Britain + Prussia war. Use `SOVEREIGN_SMOKE_START=settlement_losing` or `settlement_rejected` for blocked-popup, SC-10b / SC-28 / SC-28b recovery, scoped-draft, no-direct-pair-action, concession-baseline, and Covered Enemies picker paths. Use `SOVEREIGN_SMOKE_START=settlement_multiwar_ambiguity` for the required same-nation multi-war ambiguity check; Gate 4 step 1 cannot close unless smoke evidence names this fixture and records the rendered disambiguation or hidden-action outcome.

## Verification Snapshot

Slice G / AI-ally settlement agency remains blocked until the updated v0.24 checks are consumed, the rejected/losing-side smoke path passes, SC-29 / SC-30 / SC-31 / SC-32 / SC-33 landing ownership remains tracked rather than open-ended, SC-27 doc scans pass on current text, SC-22 Godot evidence is recorded per slice, and SC-10b is proven before `Open War Detail` recovery is treated as a valid blocked-review escape.
...
| **Blockers** | Settlement UI Cleanup remains NO-GO until the combined v0.24 Pre-Implementation Spec Review Gate result, SC-27 scans, SC-1 / SC-10b / SC-28 executable evidence, per-slice SC-22 Godot parse/load or explicit NO-GO tooling-block accounting, SC-28 / SC-28b recovery evidence, Deferred Work Landing Ledger ownership check, branch-target reconciliation, and expanded 18-step Gate 4 smoke, including rejected/losing smoke, are recorded. Commissioned-art renderer smoke remains blocked on art assets. |
