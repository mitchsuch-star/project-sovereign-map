...
| Metric | Value |
|--------|-------|
| **Tests Passing** | **Full Python suite green:** `8720 passed, 1 skipped` (`.\\.venv\\Scripts\\python.exe -m pytest tests/ -q`). Godot runtime smoke verification is still manual-only because the engine is not installed in this environment. |
| **Current Phase** | **Memory and Pressure v2.4.3 — B-Hegemony + B-B1-lite + B-B3 + B-B7 + B-B4 COMPLETE (April 23, 2026).** Map-readiness non-art closure remains complete and art-blocked only. Next diplomacy work is **Slice C-lite** — the final v2.4.3 presentation slice (Balance of Europe ledger/payload swap + `commitments_notice_*` committed prose + named-diplomat resolver + British fallback cleanup + paradox popup scene). |
| **Blockers** | No active blocker on `C-lite`. The live gates are (a) legacy ledger/UI payloads that remain on `threat_coalition` and must be rewritten on this slice, (b) `tests/test_session8a_ledger_debug.py` + `tests/test_session8b_ledger_ui.py` still assert the legacy payloads and will need rewriting on the swap, and (c) commissioned-art renderer smoke, which remains blocked on art assets. |
...
2. **Memory and Pressure v2.4.3 — B-B4 NEXT.** Ship DG-4 call-to-arms follow-through (parallel slice, now unblocked on both axes: B-B1-lite merge gate satisfied + B-B7 substrate live). B-B4 owns: (a) the grievance-variant Make Amends path per spec §8.6.1a (`make amends with {nation} for the abandoned alliance` parser disambiguation, `400g + 2 DP` cost, oldest-grievance removal, `+3` reliability / `+8` relation, sharing the same `reparations_cooldown` + `amends_offered` emitters B-B7 just landed); (b) same-turn alliance termination on `call_to_arms_refused_defensive` with `end_reason_family = "defensive_refusal_termination"` + `END_REASON_FAMILY_DEFENSIVE_REFUSAL_TERMINATION` constant + display plumbing + bloc-cache invalidation; (c) `grievance_modifier = -30 per active grievance` (saturating at 3 per pair, max `-90`) plus the `-60` composite-floor reintroduction with debug exposure; and (d) the R9/R10/R11 playtest gates per §14. ~25-29 tests per plan §B-B4. After `B-B4`, continue with Slice `C-lite` for the `Balance of Europe` ledger/payload swap + `commitments_notice_*` committed prose.
...
9. **Memory and Pressure v2.4.3 — Block 3 folded, implementation slices in progress.** `docs/audits/MP_V243_BLOCK1_DOC_CLEANUP.md` and `docs/audits/MP_V243_BLOCK2_SUBSTRATE.md` are complete. `docs/audits/MP_V243_BLOCK3_BLOC_NAMING.md` is SUPERSEDED — its bloc-naming contract was folded into `COMMITMENTS_PRESENTATION_SPEC.md` §8.1a (terminology guard, `33 / 50 / 60` activation gate, hegemon→label taxonomy with fallback, required surfaces, worked-copy examples, playtest feel gates), its per-band voice contract was folded into `DIPLOMAT_VOICE_BIBLE.md` ("Bloc-naming voice contract" subsection), and its CF1-CF4 closure items were folded into their parent slices in `RELIABILITY_IMPLEMENTATION_PLAN.md` (CF1 → C-lite, CF2 → B-B7, CF3 → B-B4, CF4 → B-B4 per the B-B1-lite merge gate). No separate Block 3 consumption pass is required. **Current state:** B-Hegemony is complete. Remaining work: B-B1-lite next, then B-B3 / B-B7 / B-B4 per the merge gate, then C-lite. This is not part of the map-readiness gate.
...
**Next bug-owned implementation slice:** `Memory and Pressure` v2.4.3 **B-B1-lite**. B-Hegemony is complete and verified. The next handoff should open directly against `RELIABILITY_IMPLEMENTATION_PLAN.md` for `hegemony_target_mod`, acceptance decoupling, and proposal-preview `hegemony` warnings; after that, proceed `B-B3 / B-B7 / B-B4` per the merge gate, then Slice `C-lite`.
...
|-------|------------|------------------------|------|
| 1 | Memory and Pressure v2.4.3 | `R160`, `R119` | First diplomacy follow-up closure block. Blocks 1 and 2 are complete; Block 3 is SUPERSEDED — its bloc-naming contract was folded into `COMMITMENTS_PRESENTATION_SPEC.md` §8.1a, its per-band voice contract was folded into `DIPLOMAT_VOICE_BIBLE.md`, and its CF1-CF4 closure items (presentation follow-through, defensive-refusal surfacing, apology-loop follow-through, composite-floor tests) were folded into their parent slices in `RELIABILITY_IMPLEMENTATION_PLAN.md` (C-lite / B-B4 / B-B7). **Current state:** B-Hegemony is complete. Next: B-B1-lite, then B-B3 / B-B7 / B-B4 per the merge gate, then C-lite. |
...
**Godot Layer 1 â€” HUD Cards:** `war_status_panel.gd` (CanvasLayer 25). Bottom-right panel with coalition header, member rows, bilateral war cards (with score bars), armistice cards. Hides when no wars/armistices. Signals: `card_clicked(nation, status)`, `coalition_header_clicked()`.
