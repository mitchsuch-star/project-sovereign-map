...
- **Deferred (owned):** ESP-3 respect-by-treaty stays with its diplomacy-gate owner row (`DESIGN_REFINEMENT.md`); Jealousy §14's own exclusions (pride mechanic, Petition Chain/BALANCED) stand unchanged — **Council command was BUILT August 9, 2026 under CA9 row 3's ruling Q2(a), see §14**; the recruitment spec's §8 lists its own non-goals.
...
- ~~**Council command** ("to my tent" active resolution) — deferred to Phase 8 or later~~ ✅ **BUILT August 9, 2026 — CA9 row 3, ruling Q2(a).** This row was a GR9 orphan: deferred with no owner row, no landing slice and no completion definition, while the confrontation card's own body asked for exactly it (*"He requests a command worthy of his talents"*). The Aug-9 grievance audit made it the load-bearing item — escalation history and level are written only at fire time, so no petition arm could reach either and all three arms converged to the same state at different prices. The arm is **"Give him a command"**: it issues an existing **PURSUE** through the shared executor at that order's own AP price (2, or 1 for a literal), targeting `find_autonomous_attack_target`'s quarry, and the grievance then ends the way §3 says grievances end — on the field, through `check_battle_resolution`'s per-personality predicate. Honest availability mirrors every refusal the executor would raise (broken, reforming, artillery, no reachable enemy, engaged elsewhere, an identical standing order, a contested destination), re-derived at ANSWER time as well as build time. Zero new verbs, serialized fields, popup slots, campaign-log types or `.gd`. Landing record: `tests/test_ca9_row3_q2_council_command.py`.
