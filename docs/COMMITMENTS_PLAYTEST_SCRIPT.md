...
> **v0.3 scope note (Apr 16, 2026):** This script was authored against `COMMITMENTS_PRESENTATION_SPEC.md` v0.2 and includes bargain events (`bargain_ratified`, `bargain_triggered`, `bargain_fulfilled`, `bargain_breached`) that **cannot be triggered in the current build** — war bargains were rescoped out of the Memory and Pressure phase and now live in `docs/WAR_BARGAIN_SPEC.md` (Peace Deals phase). For the C3-lite baseline playtest, replace the bargain events in §2 with the three live equivalents: `diplomatic_treaty_broken` (france-as-breaker via manual break or war declaration on a treaty partner), `hard_reject_posture_triggered` (use the `cheat seed_hard_reject <nation>` command), and `commitment_paradox` (engineer two alliances + a war declaration that forces the player to choose a side). The script's debrief questions remain valid for those three events. Re-run the full bargain script when `WAR_BARGAIN_SPEC` slice WB-D ships.
