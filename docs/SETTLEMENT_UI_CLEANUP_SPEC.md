...
Losing-side peace is owned by SC-1's first editor floor, not by a future unowned "ask for terms" feature. The existing clause schema supports concessions when authored direction benefits the accepting side: France can be the `from` / payer / ceding nation, and the enemy can be the recipient. The editor presents this as an offer-mode conversation, not as a new clause type.
...
- If the war is active, include `Open War Detail` as a structured recovery route so the player can choose the existing Bilateral Peace or Armistice tools from their proper per-pair surface. Do not surface a direct armistice action from `settlement_confirm`; settlement is war-scoped and armistice is bilateral/per-pair.
...
| --- | --- | --- |
| Acceptance fails, editor available | active `Revise Terms`; `Open War Detail`; `Back Out` | Explain the current package cannot be ratified, identify the top acceptance blockers, and explain that War Detail holds bilateral peace / armistice alternatives. |
| Acceptance fails, editor unavailable | `Open War Detail`; `Back Out` | Explain no term editor is available in this build; do not show a disabled `Revise Terms` control. |
...
| Active war, no editor route | `Open War Detail`; `Back Out` | Explain that this popup cannot alter terms, and route the player to War Detail for reachable peace or armistice tools. |
...
- The recovery action id is `open_war_detail`, with `recovery_route={"surface": "war_detail", "war_id": str, "selected_target_nation": str | null, "source_route_id": str | null, "reason": str}`. Archived recovery uses `{"surface": "settlement_history", "route_id": str}`.
...
- Choosing `Open War Detail` closes or backgrounds the rejected review through Back Out semantics and preserves a non-empty unratified draft in `pending_settlement_drafts[war_id]` unless the player explicitly discards it.
- If the player completes bilateral peace or armistice from War Detail, any preserved settlement draft for that `war_id` becomes stale. Reopening Settlement must run fresh POST preview before any ratification path appears; if the war state no longer supports the draft, the player sees a stale-draft notice and may discard or re-author.
...
Required behavior tests: `test_failed_settlement_open_war_detail_preserves_draft_without_pair_action`, `test_bilateral_armistice_success_invalidates_preserved_settlement_draft_with_notice`, and `test_one_to_one_stale_settlement_routes_to_war_detail_bilateral_tools`.
...
- Losing-side treaty packages must support concessionary terms through the same war-scoped authoring/preview/ratification pipeline. `Ask for terms` and `Surrender terms` are conversational presets on the settlement editor's offer-mode baseline; they are not separate incoming-offer buttons while SC-5 is deferred.
...
| Action rail | Back Out, Preview refresh, Submit, optional Ratify-from-editor only when ratifiable | Back Out, Revise Terms only when edit-capable, Ratify Settlement only when active per SC-3/SC-4, recovery route when ratification is blocked |
...
- **SC-1:** Losing-side peace uses editor offer-mode on the canonical clause schema. `Ask for terms` / `Surrender terms` are Talleyrand-authored draft presets that populate concessionary clauses and run through POST preview; they are not incoming-offer waits, mailbox actions, or new backend command text.
...
`docs/STATUS.md` currently records G2-Slice-1 through G2-Slice-5 as referenced implementation commits, while the active `codex/settlement-smoke-start` branch is a docs/smoke-start repair branch where the latest implementation commit may not be an ancestor of `HEAD`. The next work is branch-target reconciliation plus verification and smoke, not a fresh G2-Slice-1 implementation restart from this docs branch. Before Gate 4 manual smoke can start, each row below must have current evidence from the chosen integration/smoke target or be reopened explicitly.

| Verification item | Required evidence before smoke |
| --- | --- |
| SC-1 editor floor | A current test or smoke note proves the normal Settlement CTA opens EDIT mode with `peace`, `territory_cede`, `gold_indemnity`, and `forced_alliance` controls; a losing France fixture can use the concession baseline or offer-mode controls to author player-as-payer/ceder terms. |
...
| SC-27 doc maintenance | Doc scan proves older incoming-offer and route-id instructions are removed, replaced, superseded on the same paragraph, or explicitly scoped to post-cleanup Slice G after SC-5 reversal. |
| STATUS alignment | `docs/STATUS.md` names v0.20 verification + Gate 4 smoke as the active gate, names the rejected/losing smoke fixture, and does not claim SC-27 closure until the scan above passes. |
...
7. Force an acceptance rejection and a hard stop. Confirm `confirm_settlement` is absent from `options[]` and `available_action_ids[]`, the blocked-ratification banner names the blocker, `Open War Detail` or `Open Settlement History` is present when recoverable, Back Out is not the only move for an active war, and the backend refuses mutation even if called directly.
8. Run the rejected/losing smoke variant (`SOVEREIGN_SMOKE_START=settlement_losing` or `settlement_rejected`). With France losing badly against a Britain-led coalition, open Settlement, attempt a winner-favored package and observe rejection, then use the editor's concession baseline or offer-mode controls to author a concessionary package with France as payer/ceder. Confirm acceptance moves toward accept/near-accept, clause copy reads as an offer/concession in treaty voice, `Open War Detail` is the only popup recovery route for bilateral peace/armistice choices, and no enemy-offer waiting control appears while SC-5 is deferred.
