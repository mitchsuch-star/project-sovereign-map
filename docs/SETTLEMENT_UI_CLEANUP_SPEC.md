...
> **Status:** v0.21 SPEC READINESS NO-GO / IMPLEMENTATION NO-GO - rejected/losing-side recovery is owned by SC-28; blocked ratify is absent rather than disabled; editor-mode ratification shortcuts are removed; concession baseline, draft-discard notice, parent-spec route-id supersession, and pre-smoke evidence rules tightened before Gate 4
...
| Open War Detail | `options[]` entry plus `recovery_route` | `open_war_detail` | Active `war_id` still exists or can be resolved from the mounted settlement route | Backgrounds the rejected review without discard-confirm, preserves a non-empty draft in `pending_settlement_drafts[war_id]`, opens War Detail for the exact `war_id`, and lets War Detail expose Bilateral Peace / Armistice with their own costs and disabled reasons | `settlement_open_war_detail_recovery_talleyrand` |
...
- Choosing `Open War Detail` is a background-and-preserve transition, not ordinary Back Out. It preserves a non-empty unratified draft in `pending_settlement_drafts[war_id]` without showing discard-confirm, because the player is following the spec-provided recovery route rather than abandoning the draft.
...
- Losing-side treaty packages must support concessionary terms through the same war-scoped authoring/preview/ratification pipeline. `Ask for terms` and `Surrender terms` are allowed only as Talleyrand-authored draft-preset labels that populate canonical concessionary clauses in the editor; they are not separate incoming-offer buttons, mailbox waits, or backend command text while SC-5 is deferred.
...
- **SC-1:** Losing-side peace uses editor offer-mode on the canonical clause schema. `Ask for terms` / `Surrender terms` are Talleyrand-authored draft-preset labels that populate concessionary clauses and run through POST preview; they are not incoming-offer waits, mailbox actions, new backend command text, or new clause types.
...
- **STATUS:** `docs/STATUS.md` must name this cleanup spec as v0.21 / rejected-losing recovery synthesis before the next smoke or implementation step, and must not claim SC-27 closure unless the current doc-scan passes.
...
- Verify `docs/STATUS.md` references this spec as v0.21 / rejected-losing recovery synthesis and does not claim stale seventh-pass, eighth-pass, ninth-pass-only, tenth-pass-only, v0.18-only, v0.19-only, or v0.20-only readiness as the current gate.
...
| SC-27 doc maintenance | Doc scan proves older incoming-offer and route-id instructions in both `WAR_SETTLEMENT_ALLY_PARTICIPATION_IMPLEMENTATION_PLAN.md` and `WAR_SETTLEMENT_ALLY_PARTICIPATION_SPEC.md` are removed, replaced, superseded on the same paragraph, or explicitly scoped to post-cleanup Slice G after SC-5 reversal. |
| SC-28 rejected/losing recovery | Blocked/rejected reviews follow the recovery affordance contract: absent blocked Ratify, real editor/recovery/history/terminal-close routes only, no enemy-offer wait while SC-5 is deferred, Open War Detail preserves drafts, and losing-side concession baseline is testable. |
| STATUS alignment | `docs/STATUS.md` names v0.21 verification + branch reconciliation + Gate 4 smoke as the active gate, names the rejected/losing smoke fixture, and does not claim SC-27 closure until the scan above passes. |
...
Close SC-28 and the v0.21 SC-27 parent-spec route-id extension after Slices 1-5 are reachable on the chosen integration/smoke target. This is a focused repair slice, not a restart of the earlier G2 ladder.
...
- `open_war_detail` preserves a non-empty draft without discard-confirm and opens the exact `war_id`; War Detail, not the settlement popup, owns Bilateral Peace and Armistice controls.
- No payload, option label, disabled placeholder, recovery hint, mailbox item, notification, dispatch line, or Godot branch mentions enemy-offer waiting while SC-5 is deferred.
...
- Rejected/blocked settlement reviews follow SC-28: real editor, war-detail, history, or terminal close routes only; no disabled Ratify, disabled Revise, direct popup armistice/peace, or enemy-offer wait while SC-5 is deferred.
