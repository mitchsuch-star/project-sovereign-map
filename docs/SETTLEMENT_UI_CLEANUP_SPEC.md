...
> **Status:** v0.20 SPEC READINESS NO-GO / IMPLEMENTATION NO-GO - history prose removed; rejected/losing-side controls, armistice bridge, concession path, editor edge cases, and STATUS drift fixes folded into canonical rows before smoke
> **Owner:** Project Sovereign / Ink & Iron settlement feature
> **Created:** May 5, 2026
> **Last spec update:** May 7, 2026
...
Root-cause diagnosis from the rejected-settlement review:
...
- The interim safety decision then hid `Revise Terms` rather than rendering it disabled. That was valid for SC-2 in isolation, but it created the rejected-popup dead end once `Ratify Settlement` became disabled: the only available player action became `Back Out`.
- Tests pinned the presence/absence and backend response shape around ratification and revise handling, but no behavior test asserted the player-facing rejected-popup action set. A rejected `settlement_confirm` payload with disabled Ratify plus only Back Out therefore passed.
...
| `forced_alliance` | `Nation forced into alliance`, `Alliance imposed by`, `Continental System inclusion` | "Imposed on <them>" or "Offered alignment" depending on beneficiary |
...
18. **Failed settlement confirmation must offer an understandable next step.** A rejected or blocked `settlement_confirm` is only a ratification gate, not a treaty editor, not a surrender menu, and not an incoming-offer mailbox. It must not collapse to disabled `Ratify Settlement` plus lone available `Back Out` unless the state-control table below explicitly says no editor or armistice route exists. If `Ratify Settlement` is unavailable, the popup keeps the action visible but disabled with a humanized reason, omits it from `available_action_ids[]`, and renders the required control set for the failure cause. `Revise Terms` is visible only when it opens a real editor/re-author route. `Seek Armistice Instead` may appear only after the bridge contract below is implemented and must name the selected target in the label. `Wait for Enemy Offer` is forbidden as a control while SC-5 remains default-defer-and-hide; foreign-court offers may be mentioned only as non-actionable body copy until a real producer, mailbox payload, and package-preserving action path ship together.
...
- Keep `Ratify Settlement` visible but disabled when acceptance or hard stops block it; show the exact blocked reason in the popup body and button tooltip. The option remains in `options[]` with `available=false`, but is absent from `available_action_ids[]`.
...
- If the player has a valid armistice/bilateral fallback and the bridge contract below is implemented, surface it as an explicit alternative rather than making `Back Out` look like the only possible move.
- If AI settlement offers are deferred, do not imply that waiting will produce one. `Wait for Enemy Offer` must not be a button while SC-5 is deferred; use only non-actionable future-work copy if needed.

Failure-state control table:

| State | Required visible controls, in order | Required body copy |
| --- | --- | --- |
| Acceptance fails, editor available | disabled `Ratify Settlement`; active `Revise Terms`; `Seek Armistice with <selected_target_nation>` when eligible; `Back Out` | Explain the current package cannot be ratified and identify the top acceptance blockers. |
| Acceptance fails, editor unavailable | disabled `Ratify Settlement`; `Seek Armistice with <selected_target_nation>` when eligible; `Back Out` | Explain no term editor is available in this build; do not show a disabled `Revise Terms` control. |
| Hard stop active | disabled `Ratify Settlement`; `Seek Armistice with <selected_target_nation>` when eligible; `Back Out` | Explain the hard stop and why revision cannot ratify this package until scope/state changes. |
| No editor and no armistice eligibility | disabled `Ratify Settlement`; `Back Out` | Explain that no current peace path exists from this popup and direct the player to War Detail if a recovery route exists. |
| Stale or leader-change rescore failure | disabled `Ratify Settlement`; `Revise Terms` only if editor route still valid; `Open War Detail` or `Open Settlement History` recovery route; `Back Out` | Show previous score/verdict, current score/verdict, and the top components or hard stops that changed. |

Required behavior test: `test_rejected_settlement_popup_renders_required_control_set_for_each_blocked_state`.

Armistice fallback bridge contract:

- `Seek Armistice Instead` is not allowed until this bridge exists end to end.
- From `settlement_confirm`, the fallback targets only `dialogue.selected_target_nation`. The button label is `Seek Armistice with <selected_target_nation>` so the bilateral scope is explicit. If that pair is not armistice-eligible, the button is absent; do not render a vague disabled control. If the selected target is absent, ambiguous, or only a collapsed-war leader default, open a target picker listing every eligible covered enemy rather than silently choosing the war leader.
- Choosing the fallback pops the settlement popup through Back Out semantics, preserving the unratified draft in `pending_settlement_drafts[war_id]` unless the player explicitly discards it.
- The bridge opens the diplomacy wizard or equivalent proposal flow on the selected target with `propose_armistice` preselected or directly previewed. The originating `war_id`, selected target, and prior settlement route id travel as context so return/reopen paths are stable. Armistice costs are the normal bilateral armistice costs, not the SC-4b free settlement contract, and must be displayed before confirmation.
- Behavior tests must prove the fallback does not mutate settlement state, does not lose a non-empty draft, and does not route a multi-party settlement into the wrong bilateral pair.
Required behavior tests: `test_rejected_multiparty_settlement_seek_armistice_targets_selected_target_only_and_preserves_draft` and `test_seek_armistice_button_absent_when_pair_armistice_ineligible`.
...
- Losing-side treaty packages must support concessionary terms such as lighter demands, offered concessions, asking for terms, or surrender framing through the same war-scoped authoring/preview/ratification pipeline.
...
- The first editor slice must include a discoverable concession baseline or suggestion affordance for losing players, such as a one-click "Generate concession baseline" or Talleyrand suggestion using existing MVP clauses (`territory_cede`, `gold_indemnity`, `peace`). Without that, losing-side peace is technically possible but practically invisible.
...
Required behavior test: `test_losing_side_authored_concession_draft_can_reach_accept_band_with_realistic_war_state`, using a losing France fixture with player-authored gold/territory concessions to Britain and asserting the preview reaches `accept` or `near_acceptable`.

Rejected-popup presentation co-landings:

- `proposal_confirm_popup.gd::_build_settlement_content` must not render the outgoing "Will <leader> accept this settlement?" header when `can_ratify=false`. Blocked/rejected reviews render a refusal header such as "This settlement cannot be ratified now:" followed by `ratify_blocked_reason`.
...
| `REVIEW` | Ratify Settlement, Revise Terms when edit-capable, Back Out | Ratify Settlement remains visible but disabled when the fresh verdict rejects, score is below `acceptance.threshold`, or hard stops exist; it is absent only when stale/malformed metadata prevents rendering a ratification decision at all. Revise Terms is visible only when it returns the staged package to an edit-capable route and is absent otherwise. Back Out pops review while preserving same-turn draft state under SC-2. |
...
| Action rail | Back Out, Preview refresh, Submit, optional Ratify-from-editor | Back Out, Revise Terms only when edit-capable, Ratify Settlement visible-disabled or active per SC-3/SC-4 |
...
- **STATUS:** `docs/STATUS.md` must name this cleanup spec as v0.20 / rejected-losing control synthesis before the next smoke or implementation step, and must not claim SC-27 closure unless the current doc-scan passes.
...
- Verify `docs/STATUS.md` references this spec as v0.20 / rejected-losing control synthesis and does not claim stale seventh-pass, eighth-pass, ninth-pass-only, tenth-pass-only, v0.18-only, or v0.19-only readiness as the current gate.
...
- Verify no implementation checklist cites removed review-history prose. Implementation checklists must cite SC rows, gate bullets, required tests, or required inversions only.
...
Close SC-1, SC-2, SC-3, SC-4, and SC-4b first, including the Claude SC-1 empty-dispatch one-liner anchor at `settlement_presentation.py:494`.
...
- **G2-1c Ratification gates:** fresh acceptance rescore, `threshold`/`accept_threshold` reconciliation, hard-stop refusal, disabled ratify option in `options[]` plus absence from `available_action_ids[]`, SC-4b resource-cost contract, and direct backend no-mutation enforcement.
...
- `Ratify Settlement` remains visible but disabled in `options[]` on rejection or hard stops, is absent from `available_action_ids[]`, and direct backend ratification refuses mutation under the same fresh acceptance/hard-stop gate.
...
- Disabled-ratify popup render test proving blocked/rejected payloads show "This settlement cannot be ratified now:", keep `Ratify Settlement` visible but disabled, omit it from `available_action_ids[]`, and suppress outgoing acceptance framing.
...
7. Force an acceptance rejection and a hard stop. Confirm `Ratify Settlement` remains visible but disabled with exact reason copy, is absent from `available_action_ids[]`, the popup offers an honest next step or explicitly approved temporary no-next-step deferral, and the backend refuses mutation even if called directly.
8. Run the rejected/losing smoke variant (`SOVEREIGN_SMOKE_START=settlement_losing` or `settlement_rejected`). With France losing badly against a Britain-led coalition, open Settlement, attempt a winner-favored package and observe rejection, then author a concessionary package with France as payer/ceder. Confirm acceptance moves toward accept/near-accept, `Seek Armistice with <target>` appears only when the selected bilateral pair is eligible, and no `Wait for Enemy Offer` button appears while SC-5 is deferred.
9. Trigger stale state before confirm. Confirm the popup shows a humanized stale reason, includes an actionable recovery route such as `Open War Detail` or `Open Settlement History`, and does not strand the player in an invisible hard stop.
10. Trigger incoming settlement offer handling if implemented. Confirm the offer is naturally produced, mailbox/pending-envoy routes render settlement review, and accept/reject/request revision avoid generic fallback.
11. Ratify an accepted settlement. Confirm result feedback names mutations, unresolved pairs, beneficiaries/ignored parties, and routes active wars to war detail while archived settlements focus the ledger row.

If any smoke step fails, the spec returns to NO-GO and re-enters the relevant Gate 2 slice. Smoke pass requires all 11 steps green.
...
- The ratify option remains in `options[]` as visible-disabled whenever the backend would reject for acceptance or hard stops, and is absent only from `available_action_ids[]`.
