...
| **GEV-D1** | **"The Road to Forty-Five" — the reach gate.** GE-V measured the played road from the 1805 boot to the Congress on two hand-played openings and three scripted arms (memo `docs/audits/GE_V_PLAYED_CAMPAIGN_2026_09_25.md`): every arm stalled at **35 titled by turn 12** with the army at about half its boot strength (best reach 41), so §2.8's rule fired and `hold_titled` is 45 — but the blocker is the campaign layer, not the number. Six measured mechanics: the muster preview overstates arrival by about a third ("51,719 if all march" fought with two of five corps); four action points for eight corps; the 37,500 supply cap taxes concentration (13,408 men lost in three turns at Munich before a battle); the marshals' own systems — Murat's autonomous charge (−9,891), a "trust" answer to Ney's fortify objection that sent him to attack and break, Davout's cautious auto-fortify — cost a third of the army by turn 8; 3–5k British landings take undefended homeland provinces (Wellesley's 3,782 men took nine); a losing lord's satellites bleed −6 a turn and fall under the loyalty-40 titled line. | **Recommend, not built (a balance ruling — the user's):** the two cheapest levers, in order — **(a)** the muster preview quotes the EXPECTED arrival (the relationship/personality-weighted figure the engine already computes) instead of "if all march", so a player can plan on the number the field will show; **(b)** a garrison-detachment default (3,000) on homeland coast provinces against the raids. Then re-measure on the same chunked method — the archived `gev-played-*` digests are the baseline. The larger levers (an AP for every corps, a supply cap that scales with ally soil, the objection arms never executing an order the player did not give) are each a gate of their own. | **Owner:** the Victory & Objectives pass (ROADMAP 12–13), which owns the ending's balance. **Landing slice:** VP-R1 "The Road to Forty-Five", first slice after the release build's Update 1. **Completion:** a hand-played arm on the `historical` seed reaches 45 titled by turn 40 through typed orders alone; the muster preview's quoted figure is within 20% of the arrival on 8 of 10 driven strikes. **STATUS line:** "GEV-D1 The Road to Forty-Five — RECOMMENDED Sept 25, 2026, awaiting the user's ruling; lands as VP-R1." **Behaviour test:** `tests/test_vp_r1_the_road_to_forty_five.py` — `test_the_muster_preview_quotes_the_expected_arrival`, `test_a_played_arm_reaches_forty_five_by_turn_forty`. **Re-open** the 45 line itself only after (a) lands: if the re-measured reach still falls short, the number moves again; if it does not, the number stands. |
