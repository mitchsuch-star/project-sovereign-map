...
|---|---|---|---|
| **SR-L-D** L-D "the boolean road" | **APPROVED.** Keyless delegation gated on the deterministic `DelegationMatch` (not the parse), keeping the action-only, objection-first single-modal and personality pre-flight guardrails (`COMMAND_ROBUSTNESS_SPEC.md` §6.3). Confirms the September 23 delegated ruling (`LOCAL_PARSER_FEASIBILITY_2026_09_20.md` §6.3), whose §6.4 `done_when` is the contract. | Chunk 3's remaining slice, after SR-2e | `COMMAND_ROBUSTNESS_SPEC.md` §12 (the SR-3a row) |
| **SR-D3** the AP and DP gaps | **Premise corrected:** the AI is NOT exempt from action points — `enemy_ai` spends `world.nation_actions[nation]` (majors 4, minors 2–3) through the same cost table, with the same military free verbs and at most two free actions a turn; every enemy corps moves because the 1805 rosters are 1–3 corps against those budgets, while France fields 8 against 4. **RULED (c) first:** SR-2e "standing orders reliable" (SUPPORT at 1 action; a march keeps its tail through a reinforcement; SR-4b folded in). **(a) free AP scaling REJECTED.** Points are minted only by SR-D1's Staff reform through the existing `calculate_max_actions` → `bonus_actions` seam, purchasable by BOTH sides at the same price (GR5). Q0 (39 of 45) is re-measured after SR-2e and after SR-D1; 45 is not moved. Q3 (DP) and Q5 (the admin pool) ride SR-D1's gate. | SR-2e next; the rest rides SR-D1 | `SCORE_MANDATE_PLAN.md` §4 SR-D3 |
| **SR-D1** "Reforms, not research" | **Re-slotted to Chunk 5's gate** — the economy's peacetime sink AND the AP lever. Unbuilt until ruled. | Chunk 5's gate | `SCORE_MANDATE_PLAN.md` §4 SR-D1 |
| **SR-D2** asymmetry and national flavor | **Re-slotted to Chunk 7's gate, with HC-6 seasons folded in.** Unbuilt until ruled. | Chunk 7's gate | `SCORE_MANDATE_PLAN.md` §4 SR-D2; `SEASONS_WEATHER_SPEC.md` |
| **SR-G7** the standing alarm floor | **Gate question added to Chunk 7:** a standing alarm floor derived from France's share of Europe, so the long peace ends by machinery (PB-D1). **Recommended default: Europe stays at Brewing while France holds ≥ 40% of the map**; the numbers escalate to the gate. | Chunk 7's gate (SR-7b measures first) | `SCORE_MANDATE_PLAN.md` §2 Chunk 7; PB-D1 below |
| **SR-5c** the Descent's second throw | **Added to Chunk 5:** the window gets a second throw through readiness; the Continental System shut-out arc is PLAYED at the session exit covering Chunk 5. | Chunk 5 | `SCORE_MANDATE_PLAN.md` §2 Chunk 5 |
| **SR-M** measurement | **ONE exit per SESSION** over every slice it landed (one memo named for the date, one residue slice for what it found); **§1 re-scored ONCE, fully, at the end of the mandate** — the user: *"a full rescore at end instead of after every chunk, to save resources"*. Why: Chunks 1 and 2 stopped 0.25–0.5 short because their exits ran last and their residues were banked instead of fixed. A chunk is CLOSED when its last slice has been through a session exit. The Chunk 3 exit re-score was NOT run; its slices are covered by the next session's exit. | every session | `SCORE_MANDATE_PLAN.md` §5 |

---

## Creative AAR — design items (AAR-D) — filed September 25, 2026 (evening) (**ALL OPEN, every one owned**; memo `docs/audits/PLAYTEST_CREATIVE_AAR_2026_09_25.md`; defects `BUG_FIXES.md` §Creative AAR Playtest)

| Row | Item | Recommendation | Owner |
|---|---|---|---|
| **AAR-D1** | **The client's war is the lord's war.** The Treaty of Vienna (turn 9) left `Austria\|KingdomOfItaly` at war; Austria, at peace with France, took Piedmont, Tyrol and Milan and eliminated the client by turn 18. The client's contingent is the lord's roster (VS-4), so it has no defender of its own; the AI's "Redeem Italy" design was satisfied by eating a court whose lord it had just signed with; the offer surface labelled the contradiction HARD_STOP and stopped nothing (BUG_FIXES AAR-1, AAR-28). | Rule it: (a) a lord's peace or armistice covers its satellites' pairs with the same court — the common-peace path already resolves N pairs — OR (b) it does not, and the AI's war council treats a fresh peace with the lord as a restraint on the client's pair (`FRESH_PEACE_FLOOR`, one hop down); either way (c) the acceptance surface lists "still at war after this peace: …". Historically Pressburg bound the Kingdom of Italy with France; (a) is the faithful default. | **Update 1 companion (a player report, the plan's own re-order rule)**<br>**⚑ RULED (a) + BUILT September 26, 2026 — SR-1b** (a lord's peace or truce carries its clients' pairs by the same road; the court's clients follow it too; the client's truce thaws or collapses with the lord's; landing record `SCORE_MANDATE_PLAN.md` §2 Chunk 1, rules `SYSTEMS_REFERENCE.md` §70.2). |
| **AAR-D2** | **Status quo is a cession.** After the Treaty of Vienna the Congress line still read "35 of 45 titled (4 held, unsettled: Bohemia, Hungary, Moravia, Vienna)" — a ratified status quo that RETAINS a province does not title it (`congress`: titled = homeland / ceded by treaty / a client's soil / 12 quiet turns). Uti possidetis in a signed treaty is a title everywhere in the period's practice; the road to 45 is four provinces shorter than the count says. | Count a province retained by a ratified status-quo clause as "ceded by treaty" (it was signed for). Re-measure GEV-D1's road with the rule: the hand-played campaign would have stood at 36 titled at turn 9 and 32 → 33 at turn 18 with Vienna titled. | Victory & Objectives gate (ROADMAP 12) / the GEV-D1 owner<br>**⚑ BUILT September 26, 2026 — SR-1a** (uti possidetis at the ONE state setter on every signed road, both directions; a retained title counts and breaks like a signature but is never reconciled — recognition is still bought at the table; the AAR road re-measured in SR-1e; landing record `SCORE_MANDATE_PLAN.md` §2 Chunk 1, rules `SYSTEMS_REFERENCE.md` §70.1). |
| **AAR-D3** | **The jealousy cadence.** Nine petitions in eighteen turns (Bernadotte ×3, Davout ×2, Soult ×3, one rivalry confrontation); Bernadotte envious after ONE battle (turn 2); the crown passed to Oudinot for beating a 4,000-man raid with 4,500 men; "Ney is no longer the army's most celebrated commander — the laurels have passed" while every marshal sat at 0 glory; "There is no one left to envy" when two corps broke. The voices are the best writing in the game; the ladder needs a floor under what counts as a laurel and a cap on how often a man may ask. | A laurel floor (a battle of `battle_scale.is_a_battle` size, or a decisive exchange) for crown and envy; a per-marshal petition cooldown longer than the 2–3 turns measured; F4's fuse for the reward curve as the model. | The Jealousy gate (JEALOUSY_SPEC §0 re-open) |
| **AAR-D4** | **Dispersion is taxed and dispersion is punished.** Three corps in any province bleed to the crowding tax (~2%/turn each) and the fourth starves; the muster then rewards concentration; the Russian AI's ninth night (a free counter-punch + a river crossing + three attacks on three isolated corps) broke exactly the corps the tax had spread. Also: a cornered corps at Tyrol could retreat only to Bohemia — Milan (a vassal's capital with Massena in it) and Munich (an ally's capital with Davout in it) were not "friendly" to the retreat scan. | A balance note for the FA-D27 owner: either the crowding threshold rises to 4 corps on major cities/capitals, or the tax stops applying to corps under an active SUPPORT/HOLD of the same lead. The retreat's friendly set should include ally and vassal soil France may enter (PC15-D1's law already knows the answer). | FA-D27 balance owner |
| **AAR-D5** | **Diplomatic points are the bottleneck of the diplomatic game.** Five a turn; two for a courtship; one for a client's petition; three for a separate peace; one for an armistice. Granting the Kingdom of Italy's petition on turn 8 cost the Austrian peace a turn (the chip was disabled for 1 DP); the Seat's +1 needs the Emperor at Paris, which the campaign never allows. | Either the separate-peace substitute is 2 DP, or DP bank one turn (cap 7), or the war-room counsel names the DP price of the road it recommends ("Press Austria alone — 3 DP; you have 2"). | EC-2 pass 2 / the diplomacy gate — *SR-D3's Q3 (DP) rides SR-D1's gate at Chunk 5 (September 26, 2026 (evening))* |
| **AAR-D6** | **The paying peace, from the player's chair.** Four British offers in seventeen turns: 1,358g on turn 4 (having lost nothing), 2,330g on turn 9, a white peace that kept Provence on turn 14, and a Russian-mediated white peace on turn 17; Talleyrand's counsel at each was "a court that pays to stop fighting has already decided". The two passive arms (`aar-fastforward`, `aar-verdict-peace`) took the turn-4 gold and sat on 81,144 and 119,401 gold with threat 0 at turns 30 and 44. | Evidence for Update 1's PR-D1b (the gate is P1's own coalition break-ranks clause). The Verdict's "an empire contested — the question of 1805, still open" is the right sentence for that board; the offers are the wrong price for it. | Update 1 (PR-D1b) |
| **AAR-D7** | **One throw of the dice is the whole naval road.** A keel under blockade "fixes nothing" (said, honestly); the Irish expedition at the quoted 56% cost 4,500 men and 24 sail, Spain 20 and Holland 6 beside, and the Trafalgar beat closed the sea for the campaign. Nothing on the Admiralty board offered a second road (pooling with Spain before the throw, waiting for a diversion window, a smaller corps at better odds). | Record as working as designed unless the Admiralty's expedition chip can name the levers that move the odds (corps size, a diversion first, a pooled escort) — the D5 instruments do this for diplomacy; the sea should too. | DEF-5 residue (NAVAL_SPEC §15) |
| **AAR-D8** | **The AI's garrison grind.** Charles and Mack spent turns 13–16 assaulting Bavaria's Franconia garrison of 47 → 24 → 12 → 6 → 3 men, losing 12, 6, 3 and 1; Archduke John attacked Deroy twice at "unfavorable odds" with a defensive stance and a cautious penalty (−30% on the second) and was captured. The AI's assault rung has no floor and no futility check. | A floor under the assault rung (no assault on a garrison under 500; a garrison under 500 surrenders to any corps that stands beside it) and the cautious kit's own odds gate on the AI's attack rungs (the player's delegation obeys one; VP-R1 gave the glory attack one). | ENEMY_AI_REFERENCE (the P4.25 rung) / the FA-D27 owner |
...
| **LV-D2** | **A minor's single corps captures three great-power homeland provinces in one AI turn** (Bavaria's Deroy → Bohemia + 2 on turn 1 of the historical seed, promoting Austria's REVANCHE against Bavaria on turn 1 and drawing Charles into a three-turn punishment). P4.5's undefended-capture rung has no per-turn cap and no strength check for a "literal" marshal (`enemy_ai.py:3835-3956`, `:5502-5513`). FA-D13 asked exactly this and was closed as a duplicate of PT-D4, which only changed the display. | **Rule on FA-D13 option (a):** cap walk-in captures per army per turn at its `movement_range` (one), and give "literal" the strength check "cautious" has. Measure the ambient series (this WILL move `BASELINE_SERIES`; re-record once with attribution). | The next AI-behaviour slice (FA-D13 re-opened) **✅ BUILT September 25, 2026 — row EP F5** (landing record `ENDGAME_PLAN.md` §1 F5, rules `SYSTEMS_REFERENCE.md` §62): one walk-in per corps per turn (`movement_range`) and the literal corps takes the cautious strength check — measured, Bavaria takes at most one Austrian province on turn 1 on three seeds and the Revanche fires on none; `BASELINE_SERIES` re-recorded once, four-arm attributed. FA-D13 stays closed. |
...
## Playtest Re-Score — design items (PR-D) — filed September 12, 2026 (PR-D1 ✅ **CLOSED by IQ-3**, September 14, 2026; PR-D1b filed by it; PR-D2 + PR-D3 ✅ **CLOSED by IQ-4**, same day; PR-D4 ✅ **CLOSED by IQ-8**, September 17, 2026 — cause unrecoverable, no archive; the rest **OPEN**)
...
|---|---|---|---|---|---|
| ~~**PR-D1**~~ ✅ **CLOSED September 14, 2026 by IQ-3** — a treaty that dissolves the league spends its alarm (`IMPROVEMENT_QUEUE_SPEC.md` §1.2); commanded arm 6–8 → 1 coalition, completion board 2 / 1 / 2 on three seeds, pinned by `tests/test_iq3_the_league_is_spent.py` (incl. PR-D1's own test: a spent league cannot re-form inside the window absent a new act of the target's) | P2 | **Coalitions are a revolving door.** Up to **ten** form in forty turns, each with eight or nine members, each dissolving when two of them make peace. The ordinal reached "Tenth" on a 40-turn commanded arm and would have printed "The 8th Austria Coalition" but for PR-2. Every re-formation tears up whatever the settlement system just achieved, so the churn is what caps the diplomacy pillar even after PR-1. The question is not the naming (fixed) but the CADENCE: should a coalition that dissolves for `insufficient_members` be able to re-form the next turn against the same target, or should the dissolution itself buy the target a breathing space the way a peace now does? | `fix-cmd-austerlitz`: 7 coalitions in 40 turns; `fix-cmd-historical`: 6, reaching "The Tenth Austrian Coalition". Pre-fix: 4 each, all of which included the courts that had just signed. | the coalition owner (`COALITION_SPEC` §3e/§4b) | a blessed cadence rule with a measured coalition count per 40 turns, and a behaviour test that a dissolved coalition cannot re-form against the same target inside it |
| **PR-D1b** | P2 | **The League Treats When Spent — every coalition sues at war-age 2.** Filed by IQ-3. The AI's multi-party settlement offer (`ai_diplomacy._settlement_offer_eligible_for_war` → `process_settlement_offer_phase`) is gated only structurally — the war is live, multi-party, includes the player, has a known leader, is at least 2 turns old and is off cooldown. It reads no exhaustion, no war score and no wants-peace term, and `effective_peace_threshold` (the P1 formula every other peace path reads) is never consulted, so every coalition war against the player ends 3–4 turns after it starts. After IQ-3 that premature white peace is also worth half of France's alarm. | IQ-3 decision fleet, commanded arm: every post-boot coalition lived exactly 3 turns (5→8, 9→12, 13→16 …) with members' WE 24–39 at the moment they signed. The design that gated it (the fleet's Design 1, H1) made leagues ~10 turns long, but flipped 10 pins (9 in `test_ai_intent_mediation.py` + the re-score floor pin), gated the player's live `request_terms` (Slice G1) and reversed the AUD-c decisive-band arm (8.EVAL Batch Q) — so it is a design row, not an IQ-3 rider | the coalition owner (`COALITION_SPEC` §7) + the settlement-offer producer (`ai_diplomacy.py`); landing slot = the first diplomacy slice after row IQ closes (tracked on STATUS ▶ NEXT UP, IQ-3 landing entry) | a coalition war against the player continues until its leader passes the P1 break-ranks predicate (single source with `effective_peace_threshold`); mediation stays ungated; the AUD-c re-bless is put to the user; the flipped pins are taken consciously; behaviour test `tests/test_pr_d1b_the_league_treats_when_spent.py` pins that no offer is produced at war-age 2 by a leader still above its peace threshold<br><br>**⚑ RULED September 23, 2026 — SCHEDULED as Update 1, after the release** (calls under the user's delegated grant; routing = `STATUS.md` ▶ NEXT UP). Based on an in-memory simulation at HEAD `289697ef`, 3 seeds × 40 turns, with no repo edit. A telemetry-only control reproduced the baseline digests byte-for-byte.<br><br>**Correction to this row.** The single source is **P1's own coalition break-ranks clause** (war score &lt; −50, or war exhaustion &gt; 80, or 8+ turns at score &lt; −60, or Pressburg), extracted as `coalition_break_ranks_reason()` so P1 behaves exactly as before. It is **not** `effective_peace_threshold`: that was transcription drift. Simulated literally, it keeps the league at war until turns 26–36 and France collapses to 9–11 provinces.<br><br>**R1 — the gate.**<br>• Applied to the offering leader of the active coalition's war against the player only.<br>• Sited in the producer loop and the two `request_terms` checkpoints, NOT in `_settlement_offer_eligible_for_war`, which mediation shares.<br>• Measured: the league's first offer moves t3 → **t10**, and the war ends t4 → **t11** on all 3 seeds.<br>• France holds 27 / 26 / 30 provinces at t40, so FA-D27 never fires.<br>• `BASELINE_SERIES` is byte-identical, the IQ-3 coalition count stays in band (1/1/1), and the volte-face still fires once (t22).<br>• Honest caveat: on this board the gate behaves as a ~10-turn floor, because Britain's exhaustion arm (WE ≥ 80 by t10) fires first.<br><br>**R2 — AUD-c re-blessed as a direction rule only.** The decisive band decides who pays; it no longer summons an offer.<br>• The turn-3 "Britain pays 1,358g" came entirely from Austria's pair score, priced onto a fresh leader's table.<br>• France now cashes a single member's collapse through that member's own P1 armistice (t9–10 on every seed).<br><br>**R3 — `request_terms` is gated by the same helper.**<br>• The button is disabled with an honest clock: "exhaustion N of 80, at most ⌈(81−N)/8⌉ turns".<br>• The resolver re-checks at answer time.<br>• **The player's own peace PROPOSAL stays open**, so a losing France can always offer terms.<br><br>**R4 — mediation stays ungated** by construction.<br><br>**R5 — PR-D1c stays a separate slice (Update 5).**<br><br>**Rider, same slice.** On 2 of 3 seeds, Austria's armistice and the league's offer arrive the same turn. The player then sees "another matter has arrived since" and "A ARMISTICE treaty would be a downgrade". The producer waits a turn while a covered member's envoy for that war is pending, and the article is fixed.<br><br>**Pins consciously flipped** (named in the commit):<br>• `test_ai_intent_mediation.py` ×2 (re-seat the leader at WE 81);<br>• `test_iq6_europe_speaks_its_mind.py` ×9 (re-derive, or run those drives with the lever down);<br>• `test_iq7_review_round.py` `turn6_snapshot` fixture (323 tests error until it is re-staged);<br>• `test_iq7_satellites_have_a_position.py::TestT7Lapse` ×1.<br><br>**Estimate:** ~1–1.5 sessions.<br><br>**✅ BUILT September 26, 2026 — SR-1d (Score Mandate Chunk 1).** As ruled: the gate is P1's own break-ranks clause extracted as `coalition_break_ranks_reason` (its `long_war` limb found dead — subsumed by `score` — kept and recorded), applied to the offering leader in the producer loop and the two `request_terms` checkpoints, never in `_settlement_offer_eligible_for_war`; the player's proposal stays open; the envoy-pending rider and the article fixed. **Measured: the league's first offer t4 → t9 / t10 / t11, France 28 / 25 / 28 at turn 40** (`prd1b-cmd-*`). The four pin families re-seated exactly as listed (IQ-6's drives lever-down via the driver's new `--lever`). `BASELINE_SERIES` byte-identical, attributed (arms 4/5). Rules `SYSTEMS_REFERENCE.md` §70.5; pins `tests/test_sr1d_the_league_treats_when_spent.py`. |
| **PR-D1c** | P3 | **The spend forgives aggression against courts outside the league.** Filed by IQ-3's review round. `league_spent_alarm` halves every non-treaty increment in the target's slot, from whatever turn it came, so declarations of war France stacks on courts OUTSIDE the league before signing the league's peace are halved with it: 70 → 100 (declaring on Prussia and Denmark) → peace → about 48, and those wars bring on no coalition. The same acts made after the peace reach 75 and form a 14-court league. A single declaration flips nothing (45 against 55). | IQ-3 review round, exploit lens plus its refuter: a probe on the 1805 board, 15 ticks in each order. The refuter showed the finder's proposed fix ("halve only the start-of-turn slot") merely moves the seam to the turn boundary, and that the 100 clamp makes any derivation from applied deltas order-dependent. | the coalition owner (`COALITION_SPEC` §7), in the same slice as PR-D1b | a rule that halves only the alarm attributable to the war the treaty ends. That needs threat rows attributed to their victim (AI-4a keys rows by ACTOR only). Measured in both orders; behaviour test `tests/test_pr_d1c_the_spend_is_the_wars_own.py` pins that declarations on outside courts survive a later peace whole<br><br>**⚑ SLOTTED September 23, 2026: Update 5 (after Update 4 / B1), together with PR-D1d.** The slice needs a serialization decision, because the alarm slot is a single number whose source rows are cleared every turn. PR-D1b barely moves the spend (79→39 at t11, against 90→45 at t4), which is why this can follow it.<br><br>**✅ BUILT September 26, 2026 — SR-2c (Score Mandate Chunk 2).** The serialization decision: the declaration's alarm is stamped on the pair it opened INSIDE the war instance (`declared_by` / `declaration_alarm` on `diplo_key_meta[pair]`, deep-copied by `to_dict`/`from_dict` — zero new top-level fields), so the rule is derived from the live war store and order-independent under the 100 clamp. `league_spent_alarm` keeps whole the standing stamps of the target's live wars against courts outside the dissolving league. The row's scenario measures 70 → 100 → **90** (was ~48). Rules `SYSTEMS_REFERENCE.md` §71.3; the row's named test `tests/test_pr_d1c_the_spend_is_the_wars_own.py` 7. |
| **PR-D1d** | P3 | **A war France declares after the peace still draws an AI offer at war-age 2.**<br>• PR-D1b's gate is scoped to the active coalition's war, deliberately: extending it to every multi-party war against the player flips about 55 producer pins, against 2.<br>• So a war France re-declares still draws an offer at war-age 2. Measured: Austria asking 4,263g at turn 13. | PR-D1b's simulation, September 23, 2026 (`STATUS.md` ▶ NEXT UP, the pre-build calls) | **Update 5**, with PR-D1c | Either (a) extend `coalition_break_ranks_reason()`'s leader gate to every multi-party war against the player and take the pin census consciously, or (b) a lighter war-age floor on the non-coalition producer path. Measure both on the driver's `--declare-war proceed` arm before choosing. Behaviour test `tests/test_pr_d1d_a_new_war_is_not_a_spent_league.py` pins that a war France declares draws no offer at war-age 2 from a court that has not broken ranks.<br><br>**✅ BUILT September 26, 2026 — SR-2c (Score Mandate Chunk 2).** Both options and a third were built behind levers and measured on the commanded arm with one French re-declaration (Austria, loop 14): on the historical seed France declares on turn 13 and the lever-down board offers on turn 15 — war-age 2, the filed defect; lever (b) 15 (INERT: the re-declaration is a fresh instance, so the pair's reopening is never later than the instance's birth); levers (a) and (c) both 22, when Austria breaks ranks. On austerlitz the lever-down board offers at 16 (declared 14) and (c) at 25; on marengo the lever-down board offers at 13 (declared 11) and then shows the revolving door the row is about — the script's next attacks re-declare at 14 and 24 with offers at 16 and 29 — while (c) holds the first war to 20 and the second to 36. Archives `prd1d-off-<seed>` (the explicit lever-down arm, which reproduces the pre-flip board line for line), `prd1d-c-<seed>`, `prd1d-a-historical`, `prd1d-b-historical`. ⚠ The two extra seeds' FIRST base arms were launched in the same message as the lever flip and ran with (c) already up — they matched the (c) arms line for line and were DISCARDED; the record's baselines are the explicit lever-down arms. **Chosen: (c)** — a war the player DECLARED (any live pair of France's carrying PR-D1c's stamp) is gated on its leader's break-ranks clause like the league's war; `war_not_spent` is named on every surface; a war declared on France stays ungated. Option (a)'s ~55-pin census is not taken; option (b)'s substrate ships (`reopened_turn`), its lever stays down (measured inert on the re-declaration arm: a fresh instance). Rules `SYSTEMS_REFERENCE.md` §71.3; the named test 24. |
| **PB-D1** | P2 (design) | **The long peace.**<br>• PR-D1b makes the opening war last about 11 turns instead of 4, but the quiet after it stays: 29 turns on the commanded-accept arm with 0 enemy attacks, and threat decays to 0.<br>• A France at peace generates no alarm. The AI's own war decision (AI-3r) measured 0 council wars on every seed.<br>• So nothing but the player restarts the war. | Pre-build review + PR-D1b simulation, September 23, 2026 (`prebuild-commanded`: 0 enemy attacks turns 5–40; with the gate simulated, 0 attacks turns 12–40) | The Victory & Objectives gate (ROADMAP 12, VP-1 "The Emperor's Designs"), with player evidence first | A France at peace after the opening war has a reason to act, and a Europe that answers her. On the commanded-accept driver arm, turns 12–40 contain either an AI-initiated war or ultimatum against France, or a French design pursued. *(September 26, 2026 (evening): the Score Mandate's Chunk 7 gate carries the standing-alarm-floor question — §Score Mandate rulings.)* The gate chooses which, with row GE's verdict tiers (`GAME_END_SPEC.md` R2) as the first pressure: they reward growth before Tilsit. Behaviour test: a driver-arm assertion in the VP-1 slice. |
| ~~**PR-D2**~~ ✅ **CLOSED September 14, 2026 by IQ-4.** The Cabinet sits on the Strategic Ledger's Orders tab with the applied figures. The rail has one row per mission with a Recall. `diplomatic_mission_ended` records every end. The help block names the five missions and their costs, and tutorial step XIV names the door (F1). Record: `IMPROVEMENT_QUEUE_SPEC.md` §1.3 | P2 | **The mission system is invisible on every surface a player actually reads.** It is absent from the Strategic Ledger, the notification rail, the campaign log's live events (only `started` and `cancelled_eliminated` have log types), the tutorial and the help text — which names "mission" once, generically, with no type, no cost and no cadence. A running mission shows on the top bar, the Talleyrand ledger tab, one LOW dispatch line per turn and a `[MISSION]` tag in the wizard. After MS-1..MS-10 the machinery is sound; what is missing is exposure. | audited across every `.gd` file and every backend payload builder: zero mission hits in `ledger.py`, `strategic_ledger.gd`, `notifications.py`, `intel_report.py`, `war_status.py`, `tutorial_1805.json`, `TUTORIAL_SCRIPT.md` | the next UX slice | a mission appears on the Strategic Ledger's Orders tab and on the notice rail when it pauses or completes, and the tutorial or help names one type with its per-turn cost |
| ~~**PR-D3**~~ ✅ **CLOSED September 14, 2026 by IQ-4 — "The Court's Favour", ⚠ RULED, FOR USER CONFIRMATION.** Every funded turn of courting adds +2, capped at +10, to the player's cooperative offers to that court. COURT becomes the quicker, dearer road: faster on 44 of 84 below-ACCEPT rungs, and uniquely best where it is no dearer. Talleyrand's counsel names it, and it ticked on all three seeds of the counsel arm. The fold stays the recorded fallback. Its re-open condition (a Franco-Prussian alliance by turn 10 on 2 of 3 seeds) did not fire. Record: spec §1.3 | P3 | **`COURT_NATION` is strictly dominated.** Per DP: `IMPROVE_RELATIONS` is +8 −1 decay = **+7 per 1 DP**; `COURT_NATION` is +8 − 0.6 expected blowback = **+7.4 per 2 DP**. Its only other mechanical read is a decay exemption. A live, reachable, authored mission type that is never the right choice. | measured against the shipped Talleyrand (skill 10, ×1.5) with the decay term at `diplomacy.py`'s relation tick and the exemption at the `COURT_NATION` arm | the diplomacy owner | either a second effect that pays for the extra DP, or the type is folded into `IMPROVE_RELATIONS` (GR9) |
| ~~**PR-D4**~~ ✅ **CLOSED September 17, 2026 by IQ-8 — cause unrecoverable, no archive.** The published 20 / 24 / 22 has no digest, no `meta.json` and no `driver_revision` anywhere; its tree, invocation and platform are unknowable, and no `meta.json` of that day could have answered the question anyway (it recorded no engine revision and no platform, and hashed the driver's raw bytes). **Measured, the hash seed does not move the board:** 12- and 40-turn commanded runs and 20-turn ambient runs at `PYTHONHASHSEED` 0, 1, 7 and 12345 carry identical treasury, province, threat and army figures, and the only difference was the position of one `strait_open` rail row inside a turn (`naval._tracked_links_for` walked a frozenset — now sorted, and the digest is byte-identical across hash seeds); every Sept-12 archive recorded seed `0`, and CPython 3.11 and 3.13 hash `str` identically under it. The re-measure's driver IS recoverable: `cmd-*` / `fix-cmd-*` stamp `9f00997bc24a`, the LF hash of the driver committed at `4094eb4a`. The AP claim was never a measurement — 160 is the script's line count, and 81 / 77 / 75 reproduce exactly from the archived end-turn warnings. The CLASS is closed instead of the instance: `meta.json` now records `requested` / `resolved` / `platform` / `engine_revision` and `ap_available` / `ap_spent` / `cmd_refused` (`BUG_FIXES.md` PR-X5), and `PLAYTESTING.md` binds every measured table to its platform, commit, hash seed, flags and archive, marking an unarchived figure UNCITABLE; the commanded table gained a row at the IQ-8 tree (`iq8-cmd-*`). | P3 | **The documented COMMANDED-arm table does not reproduce, and its AP claim is wrong.** `PLAYTESTING.md`, the SLICE 17 (Phase 4) block and the FA-S17-D6 row all record 20 / 24 / 22 provinces at turn 40 and "160 of 160 action points". Measured at the same commit on Linux, deterministically: **23 / 24 / 21** pre-fix (29 / 20 / 21 post-PR-1) and **81 / 77 / 75** of 160 AP, with an unused-action warning on 36–37 of 40 turns. `BASELINE_SERIES` passes byte-identically here, so the ambient board is platform-stable and this is not a general engine divergence; the Windows arm could not be tested from this environment, so the cause is **not isolated**. The ruling's conclusion survives either reading (0 of 3 below 20). | two independent measurements this session agree to the digit | the FA-D27 owner | the table is re-measured on the authoring platform and BOTH readings are recorded with their platform, or the divergence is root-caused |
...
| **FA-D26** | **The Ledger economy tab's Net omits the Materiel bill — the only component the applied identity needs, and the ledger screen has no row for it.** DUPLICATE-IN-SUBSTANCE of N11/PT-C4 (docs/BUG_FIXES.md:1399, tier-2-landed per :1360; docs/PLAYTEST_FIXES_SPEC.md:242 "not fully closed"), narrowed to the one surface that fix left untouched. `backend/game_logic/dispatch.py:2271-2290` already fixed the identical gap for the Morning Dispatch by calling `_build_economy` in APPLIED mode and labeling the result `treasury_delta_label = "by the accounts"`, with an explicit comment that the EC-W3 Materiel bill is "charged outside Net by design (the plunder-gold precedent)" — a deliberate, tested exclusion (`tests/test_economy_ledger_reconciliation.py… | The player opens the ledger, reads 'Net +2,131', ends the turn and finds the chest up 1,764; nothing on that screen explains the 367 (the banner does, once, in a scrolling terminal). Over a 24-turn fighting campaign the gap compounds into thousands of gold the ledger never accounted for. | `backend/game_logic/ledger.py:411` | ONE seam: _build_economy returns `materiel` (read from world.materiel_spent_this_turn in applied mode, 0 in projection mode) as a signed informational line; strategic_ledger.gd renders 'Materiel (last turn): -Ng' under Net with the note that it is charged at the battle, not in the accounts. | After one fought turn, _build_economy(world, player, income_data=applied)['materiel'] equals world.materiel_spent_this_turn[player], and treasury delta == net − materiel exactly. | ✅ **RULED + BUILT September 11, 2026** (slice 17, Phase 2 batch 2a; landing record = the boxed **SLICE 17 (Phase 2a)** block in `BUG_FIXES.md`; pins `tests/test_fa_slice17_p2a_the_rulings_read_the_board_2026_09_11.py`) — **Options:** (a) an informational Materiel row on the economy tab (dimmed, outside Net — the EC-W3 design), reading `materiel_spent_this_turn` so a mid-turn read is "spent so far"; (b) fold the bill into Net; (c) leave it. **Measurement:** the Butcher's Bill is charged at the battle, outside Net by the plunder-gold precedent, and the ledger had no row for the one component the applied identity needs. **Ruling:** (a), lever `THE_LEDGER_SHOWS_THE_MATERIEL_BILL`; Net is unchanged by the row (pinned); the client renders it dimmed after Admiralty. (b) rejected — it would break the SC-33 sum and the EC-W3 ruling. **Dissent:** none. **Re-open:** EC pass 2, if the bill is ever moved into the turn's Net. Sept-2 verdict: **NARROWED** (was NARROWED) |
| **FA-D27** | **GATE (slice-4 review round, Sept 4, 2026): does the AI now beat an unattended France, and is that wrong?** Measured by the round's balance lens (`docs/audits/fa_build_2026_09_04/REVIEW_slice4_R2_balance_measurement.md`) on the slice-4 board: an unattended France is overrun on 8/8 seeds (≤ 5 provinces by turn 23–27, threat zero by 28–33; seed means Fr@30 16.2 → 3.9, Fr@40 9.5 → 2.2) and a scripted, fighting France on 5/5 arms (turn-40 provinces {27, 29, 27, 16, 27} → {14, 8, 11, 6, 10}); the slice-4 levers act JOINTLY (B alone 6, the other eight together 27, all nine 2). The round's own eight defect fixes then moved the passive board back to France 5 with no French corps captured, touching no number. The reviewer's reading stands: nothing shows the AI "too strong" rather than "finally not wasting a third of its actions". | Balance is the user's call. Options on the table, none built: (a) leave it — the next PLAYED campaign (a human recalls corps, sues for peace, answers landings; the scripts cannot) is the measurement that matters; (b) a France-side lever (starting AP, treasury, or the CR/strategic AP cost) rather than dulling the AI; (c) an AI aggression dial keyed to difficulty. Whatever is chosen, re-run `tools/ai_v_sweep.py` and the balance lens's three scripts before and after. | `docs/audits/fa_build_2026_09_04/REVIEW_slice4_R2_balance_measurement.md` §1–§3 | ✅ **RULED September 11, 2026 — option (a), leave the military balance** (slice 17, Phase 2 ruling 1; memo of record = `docs/audits/FA_D27_BALANCE_MEASURE_2026_09_11.md`). **Measurement** (17 runs × 40 turns at `9f0681be`, the part-f driver, no constant touched): the passive, diplomacy-refusing France ends Fr@40 = 5 / 12 / 8 / 10 / 29 (mean 12.8; the slice-4 review's own "after" was 2.2 and its fixes measured 5), Paris held 17/17; a France that ANSWERS the coalition's settlement offer (`--diplomacy accept`, which slice 15's driver could not activate) keeps 27 / 28 / 28; one that PROPOSES peace keeps 29 / 28 / 27; the tyrant script (all-out attack, refuse everything) ends 10 / 11 / 2; the Emperor script holds 24 / 13 / 28 with Napoleon CAPTURED on 2 of 3 seeds and the campaign continuing (captivity is leverage). **Ruling:** the balance is decided by whether France answers the table, which is the design — nothing shows the AI "too strong" rather than "finally not wasting its actions"; options (b) and (c) are NOT built. **Re-open condition:** the Phase-3 played campaign (a France that fights AND answers diplomacy) below 20 provinces on ≥3 of 5 seeds at turn 40, or a human-played Paris loss before turn 20 with no refused peace offer on the record → re-open at option (b), a France-side lever (starting AP or treasury), never an AI-side nerf. **Dissent:** the ambient arm's 4-of-5 collapse is a heavy tax on a player who ignores the mailbox; the answer is the letter-book's legibility and the peace-route honesty of FA-D17/FA-D18 (built this batch), not a constant. **Phase-2 note (same day):** FA-D29's two levers (the muster price; the engaged-bodies casualty pool — FA-S17-1) moved the ambient SERIES board after this memo was taken: on the series seed the passive France ends turn 40 at 4 provinces (was 5) and the coalition's war changes shape (Austria 26 → 12, Britain 21 → 35). The Phase-3 arms re-measure the five shapes with the levers up, and this ruling's re-open condition applies to THAT reading. Original: **OPEN — GATE**, filed by the slice-4 review round ⚠ **RE-OPEN FIRED — measured September 11, 2026 (Phase 3), FOR USER CONFIRMATION:** on `1ae82476` the condition's own arm (a scripted France that fights AND answers — the tyrant and Emperor scripts with `--diplomacy accept`, five seeds) ends below 20 provinces on **4 of 5** seeds under the tyrant script (19, 13, 10, 8, 25) and on 2 of 5 under the Emperor script (19, 26, 18, 22, 21); Paris held on all 34 runs. Option (b) measured: France's starting treasury 800 → 2,000 changes NO outcome on the ten runs (the scripts buy nothing the gold gates); starting AP is not a lever this instrument can pull (the player's AP is the hardcoded 4 of `WorldState.calculate_max_actions`, and the scripts never spend a fourth action). **No blessed number is moved on an inert measurement: the ruling stands at (a) with the re-open recorded**; the France-side lever can only be judged by a HUMAN campaign that spends the extra gold or AP. The passive board drifted 12.8 → 10.6 provinces (Phase-1 → Phase-3 trees, no constant changed); the Emperor is captured on 3 of 31 saved runs, once on a passive board (marengo) — save-verified, the run summariser's column having over-counted. Memo of record: `docs/audits/PLAYTEST_FULL_RESCORE_2026_09_11.md` §3 and the D27 memo's §5. |
| **FA-D28** | **GATE (slice-4 review round, Sept 4, 2026): a detachment garrison costs ⌈log₂ N⌉+1 assaults and a fifth to a half of the ATTACKER however large he is.** Pre-existing; `combat_executor.py::_resolve_garrison_combat` caps `garrison_damage_ratio` at 0.50 per assault and floors the attacker's losses at 2% of his OWN strength (WO-3 fixed only the 1-man stall). Lever A (slice 4) now steers the AI into exactly these fights. Measured (`garrison_repro.py`, `PYTHONHASHSEED=0`): a 40,000-man Kutuzov vs a French detachment of 3,000 → 13 assaults, 40,000 → 29,844; of 12,000 → 15 assaults, → 24,973; of 25,000 → 16 assaults, → 16,570. A corps that outnumbers the garrison 13:1 loses more men than the garrison had; live, thirteen assaults over two turns by three corps of two nations to clear one 3,000-man detachment. | Combat-balance change → gated. Fix shape: scale the garrison damage with the odds (≥ 4:1 → the detachment falls in one assault; the 0.50 cap keeps only for a capital's own garrison) and floor the attacker's losses on the GARRISON's size, not his own. Golden Rule 1 territory: the numbers live in `_resolve_garrison_combat`, shown = applied on the garrison line. Pin: the three measured cases above as a before/after table; M1–M7 unaffected (the harness never resolves a garrison). | `backend/commands/combat_executor.py::_resolve_garrison_combat` | ✅ **RULED AND BUILT September 5, 2026 — FA slice 14 (part 1)** (landing record = the boxed SLICE 14 (part 1) block in `BUG_FIXES.md`). The ruling took the LOSS half of the fix shape and not the odds-scaling half. Built as the row's own words — the minimum-loss floor reads the GARRISON's size, not the attacker's own, same 2% rate, different base: 40,000 men against 3,000 / 12,000 / 25,000 cost **10,234 / 14,156 / 20,184 -> 2,942 / 8,180 / 15,962** (legacy Paris fixture), and "loses more men than the garrison had" is gone in all three. **Three measured corrections:** the ruling's literal wording (`max(losses, garrison)`) ANNIHILATES a 40,000-man corps in three assaults; the 2% constant is NOT inert (the proportional term is capped at 0.35, so the floor binds again at `garrison/strength > 17.5` — player- and naval-reachable, AI-unreachable, and pinned); and GR5 holds for the arithmetic but not the reachability. ⚠ **The ASSAULT-COUNT half stays OPEN on this row** — 13 / 15 / 16 unchanged, so a detachment still costs 13 AP and 13 marshal-actions, and now costs almost no blood (assaults 11 and 12 cost a 38,674-man corps zero men). Owner: this row; landing slice = a combat gate that also owns the `>= 4:1` one-assault rule; done when a 4:1 over-match clears a detachment in one assault with the 0.50 cap kept for a capital's own garrison. |
| **FA-D29** | **A 500-man stub in a garrisoned province is a P4 sink and the garrison is never assaulted while it stands** — P4 prices the province's visible field, never the CO-1 reinforcement that arrives: a 100k Wellington attacks a 500-man Ney in Paris (garrison 15k, a French corps adjacent), Davout's 48k relocates as a reinforcement, Ney loses 2 men, Wellington −1,331; next turn the same (100,000 → 88,113 over two turns, the stub at 495). With no adjacent friend the stub dies in two blows and P4.25 fires. Not a regression (the old rung's garrison assault resolved as the same field battle); FA-8's skip makes "a stub shields the garrison" absolute. | Larger than a review round: P4's field price should include what `_calculate_reinforcements` will commit from adjacent provinces — the player's muster preview already computes exactly this. Same price seam as FA-8 / CA9-N6 (`_defending_strength_in_region` at P0/P4/P3.25/P7.5). | `backend/ai/enemy_ai.py` P4 (`_find_attack_opportunity`) vs `combat_executor._calculate_reinforcements` | ✅ **RULED + BUILT September 11, 2026 — FOR USER CONFIRMATION** (slice 17, Phase 2; landing record = the boxed **SLICE 17 (Phase 2a)** block in `BUG_FIXES.md`; pins `tests/test_fa_slice17_p2_d29_the_stub_and_the_field_2026_09_11.py`). **Reproduced on the shipped board** (Moore 100,000 at Artois, Ney 500 at Paris, Davout 48,000 at Berry): P4 priced 200:1; **the row's own fix leaves that decision unchanged** — the muster-priced figure is 2.9:1 against a 1.2 floor. What the reproduction found underneath is the real defect: the RESOLVER sized the reinforced defender's casualties by the primary's 500 men while pricing the attacker's against the 48,000-man committed field (CO-1 added the reinforcers' STRENGTH to the exchange and left each side's `army_size` at the primary) — a 10:1 exchange from accounting; CO-3's decisiveness then broke the 100,000-man attacker at morale 15 and, his only road being at-war soil, **captured him whole on 3 of 5 rolls with 48,000 of his men "escaping home"**. **Options:** (a) price the muster at P0/P4/P3.25/P7.5 (the row's ask); (b) let P4.25 assault a garrison behind a stub — contradicts FA-8's written ruling, rejected; (c) size each side's casualty pool by the ENGAGED bodies on the field (the lead's corps plus each reinforcer's committed share, α-scaled like the strength it commits — over the same participants the executor distributes across); (d) leave it. **Ruling:** (a) AND (c), two levers — `P4_PRICES_THE_MUSTER` (ONE helper `_muster_price` at the four rungs, reading the executor's own `_defender_muster` / `_committed_reinforcement_strength(expected_at=…)` as GROUND TRUTH, the player's own odds band's rule, for the ADJACENT reinforcers only — a co-located corps is already in the field sum) and `combat.CASUALTIES_FALL_ON_THE_FIELD` (`resolve_battle(attacker_bodies=, defender_bodies=)` — the executor's `_committed_bodies`, the quantity half of the committed term; the deferred projection reads the field's bodies too, so a field that keeps most of its men is a TACTICAL result, never its primary's annihilation; a solo battle byte-identical). Measured after: the same attack is an attacker tactical victory, ~2,700 against ~13,300, Davout routed to Berry with 35,000, Ney cornered. **`BASELINE_SERIES` re-recorded ONCE, six-arm attributed** (arm 0 reproduces the prior series byte-for-byte; D = FA-D24's RNG decoupling, index 10; A alone index 10, France 5 / Austria 23 / Britain 26 at t40; B alone index 7, France 2 / Austria 16 / Britain 36; C inert; ABCD = the shipped state, France 4 / Austria 12 / Britain 35 vs the base's 5 / 26 / 21). M1–M7 byte-identical because the harness passes no bodies — a fact about the harness, not evidence. **⚠ FOR USER CONFIRMATION:** the ambient board's map moves — the passive France ends turn 40 at 4 provinces (was 5) on the series seed and the coalition's own war changes shape (Austria 26 → 12, Britain 21 → 35) because a reinforced side finally bleeds by the men it commits, both sides alike; Britain's single-corps descents gain what Austria's reinforced stacks now bleed. The FA-D27 ruling's own measurement (memo `docs/audits/FA_D27_BALANCE_MEASURE_2026_09_11.md`) predates this build; Phase 3 re-runs its five arms with these levers up and the D27 re-open condition applies to THAT reading. **Dissent:** the fixture-level pin `test_an_enemy_standing_with_the_ally_is_still_struck` (slice 4) was re-sited to 100,000 — at 30,000 the honest price rightly declines a blow on a 6,000-man corps backed by 40,000 next door; the pin's intent (an enemy standing with the ally is a legitimate target) holds at odds that survive the price. **Re-open:** if the user rules the ambient collapse out of band, lever B stays and the FRANCE-side lever of FA-D27 option (b) is the remedy — never the pool. Original: **OPEN**, filed by the slice-4 review round (R1-9) |
| **FA-R3** | **A standing order is created at 0 AP when the sentence's BASE action is a free verb.** Pre-existing root, measured by the slice-7 review round (R3-2) on the PARENT tree: `Davout, hold Rhineland and wait` → AP 4→4 with a HOLD order standing and "(2 AP — a standing strategic order…)" in the reply; `march to Lorraine and wait there` → marched + MOVE_TO at 0 AP. The mock chain's WAIT arm sits above hold/move (it must stay above the reinforce arm — "wait for reinforcements"), so the BASE action is `wait`, and the executor charges the strategic cost off the base action's `free_actions` membership. Slice 7 had WIDENED this with `stay put` / `rest your men` (`hold Rhineland and stay put` 4→2 → 4→4); the review round relocated stay-put below the order verbs, which removes the widening and leaves the root. | The player gets a two-turn standing order for nothing whenever the sentence also says "wait"; the reply prices it at 2 AP. | `backend/commands/executor.py` (the strategic pre-gate reads `free_actions`) / `backend/ai/llm_client.py` (the wait arm above hold/move) | Charge the strategic cost from the ORDER in `_execute_strategic_command`, or refuse the strategic upgrade when the base action is free; pin AP 4→2 for "hold Rhineland and wait". A balance-adjacent change (AP economy) — measure before landing. | `hold Rhineland and wait` → AP 4→2 and a HOLD order; `march to Lorraine and wait there` → 4→2; `wait for reinforcements` still WAITS (free). | ✅ **FIXED September 5, 2026 — FA slice 14 (part 1) "The Rulings" (landing record = the boxed SLICE 14 (part 1) block in `BUG_FIXES.md` §Final Whole-Game Audit).** **Three categories wider than filed.** Not a 2-AP leak but a TOTAL BYPASS of the AP pre-gate — measured at AP 0, five standing orders created and four marshals marched a province each. Not a trailing suffix — a LEADING wait fires, and so do `wait for orders` and `stand by`; seven typed shapes. And `Marshal.strategic_order_ap` was never consulted, so the literal's and the sovereign's 1-AP discount was bypassed too. ⛔ **The naive shape ships a regression and this slice shipped it before catching it**: `retreat` is free by design and six phrasings parse `retreat` WITH a strategic type, so `withdraw from the alliance` went 4->3 and was REFUSED at 0 AP — retreat made unavailable exactly when retreat is for. The rule now states its exception. |
| **FA-R4** | **`Berthier, end turn` / `Sire, end turn` shrug while `Berthier, status` and `Berthier, help` work.** Slice 7's `_DESK_ADDRESS_RE` strips the chief-of-staff address on the two exact-match desk routes but deliberately NOT on `is_bare_end_turn`: the client's lapse-confirm gate (`main.gd::_is_end_turn_phrasing`) mirrors the backend's bare end-turn vocabulary word for word, so widening only the backend would advance the turn behind the client's unanswered-envoys confirm (the UX23 soft-lock class). | A player who addresses Berthier to end the turn is shrugged at, once, and types `end turn`. | `backend/ai/clause_guards.py` (`is_bare_end_turn`) + `godot-client/project-sovereign/scripts/main.gd` (`_is_end_turn_phrasing`) | Widen BOTH gates from ONE vocabulary (the `.gd` gate's comment already demands parity) in a slice that boots the engine. | `Berthier, end turn` ends the turn on the server AND arms the client's lapse confirm; a `.gd`-touching slice. | ✅ **FIXED September 5, 2026 — FA slice 14 (part 1) "The Rulings" (landing record = the boxed SLICE 14 (part 1) block in `BUG_FIXES.md` §Final Whole-Game Audit).** Wider than filed: `Berthier, next turn` and `Berthier: end turn` shrug too, and fall out of the same rule. The desk-address vocabulary moved DOWN into `clause_guards` (which `llm_client` already imports) so there is ONE source; the client mirrors it with a two-directional parity pin. The phrasing vocabulary is untouched — `end the turn` still shrugs and every FA-6 narrowing holds. **Two hazards found by the reproduction fleet:** `main.gd`'s function ORDER is load-bearing for six pins that slice the gate's body (a comment containing a forbidden word, and later one quoting the pin's own anchor, each redded it), and **the parity harness was about to go INERT for the exact behaviour this row adds** — it harvested equality needles only, so it agreed whether or not the `.gd` carried the strip. It is strip-aware now and carries eight addressed fixtures. |
| **FA-S7-D1** | **The live parser reads "Ney, fix bayonets" and "Ney, cover the retreat" as a cavalry CHARGE.** Measured once on the live Anthropic parser while landing FA-73's `live_only` twins (both pass — they pin `not_action: repair` / `retreat`): the model maps an infantry bayonet order and a screening order to `charge`, a real order the executor can run (recklessness-gated). The prompt has no rule for a deed no action models (the mock refuses both; PS18-5's harm was the fast parser's RETREAT). | On the live parser, "fix bayonets" may launch a Glorious Charge the player never ordered, when recklessness allows. | `backend/ai/prompt_builder.py` (Valid Actions rules) | One prompt line under Valid Actions: "an order naming a deed no listed action models (screening, drill, fixing bayonets, restoring order) → unknown"; then flip the two live twins to `action: unknown`. CR-6/CR-8's gate owns the model-side vocabulary. | Both live twins parse `unknown` on the live parser (a deliberate two-call live eval). | ✅ **FIXED September 5, 2026 — FA slice 14 (part 1) "The Rulings" (landing record = the boxed SLICE 14 (part 1) block in `BUG_FIXES.md` §Final Whole-Game Audit).** One line under Valid Actions in `prompt_builder`. **The ruling's own expectation shape was corrected by the measurement**: the live provider now answers `action=unknown` (verified in the provider log), but the pipeline treats an unknown as a non-parse and falls back to the fast parser, so what the harness and the player see is a REFUSAL — `success False`, `action None`, Berthier asking what was meant. The twins pin `success: false`, falsifiable against the `charge` slice 7 measured. Four live calls, not the ruling's two: two rows x two worlds. |
| **FA-S9-D1** | **The `administrative_role` redemption answer promises a return to the field that no production verb can grant.** The arm freezes the man's troops (`administrative_strength`), zeroes his strength and location, buys +1 AP, and says *"their N troops await future assignment"*; the only restore is the debug cheat `admin <marshal>` (`meta_executor`). Until the slice-9 review round the attrition sweep then DESTROYED him a turn later (fixed, `ADMINISTRATIVE_EXEMPT_FROM_ATTRITION`); he now sits frozen forever, an extra action his only legacy. | A player who takes the arm in good faith loses a marshal for the campaign while the copy says otherwise. | `backend/commands/disobedience.py` (`handle_redemption_response`, administrative arm) + `backend/commands/meta_executor.py` (the `admin` cheat, the only restore) | Gate: either an admin action `recall <marshal>` (1 admin AP, restores `administrative_strength` at the capital or richest home province, returns the bonus action, cooldown before he may be asked again) with a corpus row and card chip — or retire the arm and re-word the redemption dialog to two choices. The copy must state whichever is true. | The promise in the arm's message is a verb the player can type, or the promise is gone. | ✅ **RULED AND BUILT September 5, 2026 — FA slice 14 (part 2a)** (landing record = the boxed SLICE 14 (part 2a) block in `BUG_FIXES.md`; the rules are `SYSTEMS_REFERENCE.md` §7). The verb, not the retirement. ⚠ **The gate could not be built as asked until a P2 nobody had filed was fixed first**: `administrative`, `administrative_strength` and `administrative_location` were serialized NOWHERE, so `recall` on a loaded campaign would restore 0 men at `None` — and, before any verb existed, one save already defeated the slice-9 attrition exemption (the sweep DESTROYED the frozen marshal) and re-opened the max-one-admin gate, making save/freeze/load an unbounded +1-military-action farm. The gate's own destination wording is built and FA-71's is not: the row's `administrative_location`/capital rule is the measured hazard (the debug cheat's `or 'Paris'` could restore 22,000 men into enemy-held soil), so the cheat now delegates to the verb. |
| **FA-S9-D2** | **A marshal with a standing redemption question keeps taking orders.** Measured (slice-9 review R1-7): Murat asked at trust 15, `Murat, attack Mack` fought and moved him, `Murat, move to Orleanais` succeeded, the question still standing. The checker never gated orders; slice 9 widened the moments a question stands (any write at ≤ 20, re-raised at end turn) so the window is now ordinary. The client intercepts the answer tokens before sending, so this bites headless drivers and the text fallback most. | The fiction says a man is demanding an audience; the executor says he marches. | `backend/commands/executor.py` (no gate) | Design call: (a) gate orders for the asking marshal with an honest refusal ("X awaits your answer, Sire — grant autonomy, an administrative role, or dismiss him first") and a corpus row, or (b) record that he serves while the question stands (the current behaviour) in `SYSTEMS_REFERENCE.md` §7. | One of the two, stated and pinned. | ✅ **RULED September 5, 2026 — option (b), RECORDED AS DESIGN in `SYSTEMS_REFERENCE.md` §7, not built** (FA slice 14 part 2a). He serves while the question stands. Option (a) was declined with the reason on the record: the man is demanding an audience, not mutinying; slice 9 made the window ordinary, so gating his orders would idle a corps for a whole turn on a modal the client intercepts before it is even sent; and the answer's own consequences are what change his standing. Pinned so a later reader does not "fix" it. |
...
| **IQ6-D4** | **The AI-V scripted arm's scene-4 positive rests on the retired arm.** `tools/ai_v_sweep.py` `_turn_11` stages the Tilsit reversal through hand-written exhaustion and a +125 relation jump, which V3 retires; the assurance pin was re-worded to assert the retired arm no longer fires, so `AI_V_SWEEP_2026_08_01.md` §7a scene 4 has no in-run positive today. | A harness edit, not an engine one — the scripted arm needs a SOIL mark at turn 11 (hand Lithuania's controller to France, for example) so the door opens on the map. | IQ-8 "The Harness Tells the Truth" (the instrument row) | **✅ CLOSED by IQ-8 item 9 (September 18, 2026).** `tools/ai_v_sweep.py` gains the arm `--script france_soil` — the `france` schedule plus, at `_turn_11`, Lithuania (a non-capital Russian homeland province; Vilna is the capital on this board, and a lost capital would promote Revanche and shut the door) handed to France, the reading `emergent_designs._lost_homeland` makes; the `france` arm plays the same game it did (its watch note gains a `soil=` reading) so the re-worded negative stays. **Re-verified in a run on all three scripted seeds:** receptive t11 on `soil=['Lithuania']`, the courier t12 (`decision_reason: volte_face`, an alliance), signed through the §5b.3 conflict confirm, `volte_face` t13 aimed at `gulf_and_straits` (Russia's intent after the signing: `gulf_and_straits` against Sweden) — where the retired WE staging had fired on historical alone. Positive pin restored beside the negative: `tests/test_ai_intent_assurance.py::TestArmBScriptedFrance::test_volte_face_signed_and_aimed_at_a_third_party` (fixture `scripted_soil`); memo addendum `AI_V_SWEEP_2026_08_01.md` §11; `run_all` now runs both scripted arms. |

## IQ-7 "The Satellites Have a Position" — routed, not built (filed September 16, 2026)

> Filed by row IQ-7 (landing record `IMPROVEMENT_QUEUE_SPEC.md` §1.6; rules `SYSTEMS_REFERENCE.md` §46; the design fleet's contract in the session scratchpad ruled "The Client's Petition" over two other designs). FA-S17-D7 above is CLOSED by this row. Each item names an owner and a completion (Golden Rule 9).

| id | item | why it is not this row's | owner | completion |
|---|---|---|---|---|
| **IQ7-D1** | **The Contingent — "the satellite pays in blood."** A loyal satellite raises an income-and-loyalty-scaled contingent in a shared war, marches under a French host, bleeds loyalty by its dead, comes home crowned or decimated, and walks out on a break. Design 2 of the IQ-7 fleet; it states what a loyal vassal is FOR more vividly than the petition. | Its decisions did not change the outcome on the board that failed (peace-time drift untouched: rebellions within ±1 turn of HEAD on the prototype), its delivery on the real transport is unverified (auto-accepted at the tick), and it mutates marshal strength with recall hooks at four exits — a slice of its own. | `VASSAL_DEEPENING_SPEC.md` §9 row **VD-C** (gate-ready with the prototype evidence) | A commanded 3-seed run in which a contingent is called in a shared war and marches home, and R1's regiments clause becomes true for the boot satellites; `tests/test_vassal_contingent.py`. |
| **IQ7-D2** | **The Suitor at the Gate — DECLINED on measurement.** Rival courts court a satellite even while France is strong, the pull scaling with France's threat to Europe, so the Emperor must spend to keep what he holds. Design 3 of the fleet. | Measured on the prototype: it accelerates every UNATTENDED collapse by ~10 turns and moves ambient Holland from loyal to lost (the passive-France balance FA-D27 cannot afford it), puts the decision on the rail where it can be missed, and needs a new verb through the full checklist plus a coupling to threat and an AI rung. | the FA-D27 balance owner (the user) | ⚠ **Re-open condition:** a played or commanded 40-turn campaign after IQ-7 shows a BONDED web silent for more than 20 consecutive turns while France's `threat_by_target` stays ≥ 40. No player-facing promise is made. |
...
## Generals' Mortality — design item (GE-D) — filed September 25, 2026 (memo `docs/audits/GENERALS_DEATH_ODDS_2026_09_25.md`; ⚠ AWAITING THE USER'S RULING — nothing built)

| Row | Item | Recommendation | Owner |
|---|---|---|---|
| **GE-D1** | **The Generals' Mortality.** A general has no personal death roll. He falls only when his corps is removed at `WorldState.destroy_marshal` (`world_state.py:3096`); the one other removal site is `_eliminate_nation`'s direct pop of a prisoner (`:4586`). The census is `battle`, `charge`, **`bombardment`** (missing from the original list), `attrition`, `interned`, `dismissed` and `nation_eliminated`. The Emperor never dies: `death_guard` converts his removal into a capture. **Measured** (25 seeded 40-turn driver campaigns, 884 battles, master `1d3f2535`): 11 field deaths (France 8, AI 3), all of them defenders on the losing side; 6 were remnants finished by the 50-man rubble floor (`marshal.py:1555`), and Britain's Paget made 6 of the 8 French kills. 19 removals by nation elimination, every one Deroy with 4,730–16,767 men. 0 by attrition, internment, dismissal or bombardment. 156 captures (France 120, released 2), so capture outnumbers death about 15 to 1 for France. A beaten defender died 2.2% of the time and was captured 21.9%. The commanded arm lost 0 French marshals to death or captivity across 40 turns on three seeds. The September 11 "four destroyed between turns 30 and 37" does not reproduce. The Emperor's corps was destroyed in 12 of 25 runs, always as a 55–255-man remnant after 3–6 Guard tolls, and the "spent Guard" question fired 0 times. | **(a) YES, bounded — "The Fortunes of War":** only the LEADING marshal of the LOSING side rolls, and only in a real battle (`battle_scale.is_a_battle`) where his corps lost at least 25%. One draw through `campaign_variance`'s seeded helpers: **killed 1%, wounded 8%** (in-band). A wound puts him out 3 turns on ONE serialized field `wounded_until_turn`; his corps stays, defends and moves, but cannot attack or take a strategic order. On a death his corps survives: its men pass to the nearest friendly corps within 3 regions (the dismissal transfer) or disperse, and the tombstone cause is `killed_in_action`. The sovereign is excluded (GE-1 owns his death). Expected per French campaign: about 1.0 wound and 0.13 deaths, which matches history's roughly 1.5% per marshal-year. **(b) No death off the field:** nation elimination tombstones him as `exiled` (never narrated as a death); attrition and internment return him alive to the capital depot (the `administrative` state, exempt from the sweep); a non-sovereign corps rubbled in battle or charge goes through the W6-7 fate check (capture 40% / escape 60%) instead of automatic death. **(c) Interplay:** the Marshalate bench is the replacement path, and the PT-J4 bench note rides the new beats. The glory ladder must drop the dead man and resolve envy aimed at him; a wounded man gains no glory and is not counted as sidelined. ES-7 ends a dead man's rente and returns his estates. GE-1's exile story must branch on tombstone `cause`, and should consider a real "Guard spent" floor so the player is asked before the fatal battle. GR5: one roll on both boards. `BASELINE_SERIES` is re-recorded once with a flip attribution; M1–M7 stay byte-identical. | **Owner:** the Victory & Objectives pass (ROADMAP positions 12–13), which takes ROADMAP Phase 10's planned "Marshal Death — casualties in battle (% chance per major defeat)" row (`ROADMAP.md:823`). This row specifies and supersedes it. **Landing slice:** **VP-M1 "The Fortunes of War"**, the pass's first slice after GE-V. GE-1 carries only the exile-story constraint in (c): read `fallen_marshals` by cause. **Completion:** 1. The roll is behind the flip lever `THE_GENERALS_ARE_MORTAL`, at the post-battle seam, seeded. 2. (b)'s three roads keep the man alive, behind their own levers. 3. On the driver (ambient, war-long and commanded arms, seeds historical / ulm / austerlitz, 40 turns): French field deaths fall within 0.05–0.5 per campaign; wounds fall within 0.5–2.0; no marshal is tombstoned as fallen by `nation_eliminated`, `attrition` or `interned`. 4. `BASELINE_SERIES` is re-recorded once, attributed. 5. The wound and death beats reach the dispatch and the Moniteur. 6. `SYSTEMS_REFERENCE.md` and `SAVE_FORMAT_REFERENCE.md` are updated. **STATUS line:** "GE-D1 The Generals' Mortality — RECOMMENDED Sept 25, 2026 (memo GENERALS_DEATH_ODDS_2026_09_25.md), awaiting the user's ruling; lands as VP-M1." **Behaviour test:** `tests/test_ge_d1_generals_mortality.py`. Its pins are `test_winners_and_stalemates_never_roll`, `test_losing_leader_rolls_once_through_campaign_seed`, `test_skirmish_never_rolls`, `test_wound_keeps_corps_and_blocks_attack_until_turn`, `test_death_passes_men_to_nearest_corps`, `test_nation_elimination_exiles_not_fallen`, `test_rubbled_corps_commander_takes_fate_check`, `test_sovereign_excluded_from_general_roll` and `test_gr5_ai_marshal_same_odds`. **Re-open** (if declined or deferred) when any one of these is measured: a human session or the GE-V pass reports a named general dying to the 50-man floor, and it reads as arbitrary; OR the war-long driver arm shows more than 1 French field death, or more than 5 French captures with 0 releases, per campaign; OR GE-1's Emperor-death arm lands on a seam a general roll must share. **⚑ RULED (a) AND BUILT September 25, 2026 in GE-V under the user's grant ("make these decisions") as VP-M1 "The Fortunes of War" — `backend/game_logic/fortunes_of_war.py`, rules `SYSTEMS_REFERENCE.md` §67, the nine named pins + the surfaces in `tests/test_ge_d1_generals_mortality.py` (22). Built as specified: the losing lead of a real battle at ≥ 25% lost, one seeded draw (killed 1% / wounded 8%), the wound on ONE field `wounded_until_turn` (3 turns; no attack, charge, bombard, pursue or PURSUE/HOLD; the corps stands, defends and marches; the AI stands its man down; not sidelined), the death through `destroy_marshal(cause="killed_in_action")` with the men passed to the nearest corps within 3 or dispersed, the sovereign excluded, both boards. `BASELINE_SERIES` byte-identical with the lever up (`tools/_vpm1_series_arms.py` — 39 seams reached, no qualifying draw fired in forty ambient turns), so no re-record; M1–M7 byte-identical; the log type 167 → 168 flipped consciously. Not built from (c): the exile story does not yet branch on a general's `killed_in_action` (it reads the sovereign's death only) — filed as the copy pass's item.** |

## Game End GE-1 — routed, not built (filed September 25, 2026)

| Row | Item | Ruling | Owner |
|---|---|---|---|
| **GE-D2** | **"The Guard is Spent."** The generals' death-odds memo (§3.8, §5(c)(iii)) measured the Emperor's "Guard is spent" question firing **0 times in 80 sovereign fate checks**: the escape toll (`GUARD_ESCAPE_TOLL` 0.30) pays the corps down to ~56 men (`GUARD_RUBBLE_FLOOR` 50) and the NEXT defeat annihilates it before the question can be asked (a zeroed corps never reaches `_check_marshal_fate`). Since GE-1 the annihilation is where "The Eagle Falls" rolls (15%), so the player meets the Emperor's death without ever having been asked to fight to the last or cut his way out. | **NOT BUILT in GE-1, by ruling:** raising the floor changes the NP-4 peril's measured shape and every NP pin that reads it, and GE-1's death roll is the ask the user made — the floor is a legibility question the played campaign should answer, not a probe. Proposed shape if taken: a `GUARD_SPENT_FLOOR` (e.g. 1,000 — `rout_survivors`' own floor) below which the toll is refused and the player is ASKED, so the death roll only ever meets a corps the player chose to keep fighting with. | **Owner:** GE-V (the played campaign, `ENDGAME_PLAN.md` §6). **Landing slice:** GE-V, or GE-2 if GE-2's driver arm reaches the Eagle-Falls ending with the question never having been asked. **Completion:** on the driver arms that reach "The Eagle Falls", the player was asked the Guard's question at least once before the fatal battle, OR the ruling is recorded that the unasked death is intended. **STATUS line:** "GE-D2 The Guard is Spent — routed to GE-V." **Behaviour test:** `tests/test_ge1_the_verdict_and_the_fall.py::TestTheEagleFalls` gains `test_the_guard_asks_before_the_last_battle` when built. **Re-open:** a human session reports the Emperor dying with no question asked and it reads as arbitrary. **⚑ GE-2 MEASURED the landing condition (Sept 25, 2026):** the archived `ge2-eagle-falls` arm (`docs/audits/playtest_digests/ge2-eagle-falls/digest.md`) reaches "The Eagle Falls" on turn 31 — the Emperor's corps annihilated at Burgundy by Britain — with NO Guard question asked (Massena and Ney received their `last_stand` asks; the sovereign never does). GE-2 did NOT build the floor: it is a mechanic (NP-4's measured shape) outside the client slice, and the ruling — the floor, or "the unasked death is intended" — is the user's; it stays with GE-V. **⚑ RULED (the floor) AND BUILT September 25, 2026 in GE-V under the user's grant:** `CombatExecutor.GUARD_SPENT_FLOOR = 1000` — the toll is refused and the Emperor ASKED when paying it would leave the Guard under a thousand men; the breakout copy reads the same floor; the ask's report line and rail row say "the Guard is SPENT", not "ENCIRCLED". The row's named pin `test_the_guard_asks_before_the_last_battle` is green (`tests/test_gev_played_campaign.py::TestTheGuardIsSpent`); the FA-S17-7 pin that staged a 1,000-man Guard "that can pay" was flipped consciously to 3,000. Rules `SYSTEMS_REFERENCE.md` §67.1. The archived `ge2-eagle-falls` arm predates the floor and was not re-driven. |
| ~~**GE-D3**~~ ✅ **BUILT in GE-1 (Sept 25, 2026)** — the full suite's log census (`test_no_silent_drops`) refused the unlisted type, so the line landed in-slice: `campaign_ending` in `CAMPAIGN_LOG_TYPES` (category `command`, one-liner "THE VERDICT OF HISTORY — … (An Empire Contested)"), the fourteen count pins 165 → 166 flipped consciously. | **The chronicle line for an ending.** `record_ending` logs a `campaign_ending` event row, but the type is NOT in `CAMPAIGN_LOG_TYPES` (adding it flips the twelve `len(CAMPAIGN_LOG_TYPES)` pins — 165), so the campaign log does not yet show "THE VERDICT OF HISTORY — …" as a line. The end screen (GE-2) is the ending's surface; the chronicle line is its record. | Routed to GE-2 with the client: the log type + one-liner + category land with the end screen that renders the same record, the twelve count pins flipped consciously there. | **Owner:** GE-2. **Landing slice:** GE-2. **Completion:** a Verdict, a Humbled Peace and a Fall each leave one chronicle line; the count pins read 166. **STATUS line:** "GE-D3 the ending's chronicle line — GE-2." **Behaviour test:** `tests/test_ge2_*.py::test_every_ending_leaves_a_chronicle_line`. |
| **GEV-D1** | **"The Road to Forty-Five" — the reach gate.** GE-V measured the played road from the 1805 boot to the Congress on two hand-played openings and three scripted arms (memo `docs/audits/GE_V_PLAYED_CAMPAIGN_2026_09_25.md`): every arm stalled at **35 titled by turn 12** with the army at about half its boot strength (best reach 41), so §2.8's rule fired and `hold_titled` is 45 — but the blocker is the campaign layer, not the number. Six measured mechanics: the muster preview overstates arrival by about a third ("51,719 if all march" fought with two of five corps); four action points for eight corps; the 37,500 supply cap taxes concentration (13,408 men lost in three turns at Munich before a battle); the marshals' own systems — Murat's autonomous charge (−9,891), a "trust" answer to Ney's fortify objection that sent him to attack and break, Davout's cautious auto-fortify — cost a third of the army by turn 8; 3–5k British landings take undefended homeland provinces (Wellesley's 3,782 men took nine); a losing lord's satellites bleed −6 a turn and fall under the loyalty-40 titled line. | **Recommend, not built (a balance ruling — the user's):** the two cheapest levers, in order — **(a)** the muster preview quotes the EXPECTED arrival (the relationship/personality-weighted figure the engine already computes) instead of "if all march", so a player can plan on the number the field will show; **(b)** a garrison-detachment default (3,000) on homeland coast provinces against the raids. Then re-measure on the same chunked method — the archived `gev-played-*` digests are the baseline. The larger levers (an AP for every corps, a supply cap that scales with ally soil, the objection arms never executing an order the player did not give) are each a gate of their own. | **✅ LANDED as VP-R1 "The Road to Forty-Five", September 25, 2026 (taken ahead of the release build by the user's direction). Landing record — probes memo `docs/audits/VP_R1_PROBES_2026_09_25.md`, rules `SYSTEMS_REFERENCE.md` §68, pins `tests/test_vp_r1_the_road_to_forty_five.py` (33), series attribution `tools/_vpr1_series_arms.py` + `.json`.** **Three probes before any code, two overriding the brief:** (P1) the muster's committed figure was ALREADY the honest expectation — within 20% of the Monte-Carlo mean on 74 of 74 supported strikes — and what lied was the label ("if all march" is neither the ceiling nor a promise; GE-V compared the CEILING to the field) and the rows (35 of 100 strikes printed WILL JOIN for a corps priced at 0%, every one departing from mountains); (P2) 60% = 16 of 26 ports was unreachable from play (the Pressburg shape minus the fixture's gifts closes 11; every signatory in the System 15; 16 needs a war on the Pope or Denmark) → **`cs_shutout_pct` 60 → 50 moved FIRST**; (P3) two of three autonomous charges fired at `unfavorable` (0.198, 0.279) below the floor a delegation-inferred attack obeys — a defect. **Built, each behind a lever:** (a) `MUSTER_ROWS_NAME_THEIR_ODDS` — "expect about X with the corps likely to arrive, up to Y if all march", every WILL JOIN row's `arrival_odds`/`arrival_note` off the resolver's own sum with the SUPPORT lever named at zero; (b) `RAIDING_PARTY_HOLDS_NO_HOMELAND` — a corps under `MARCH_HALTS_AT_GARRISON` (5,000; the brief's 3,000 would have stopped neither measured raid) holds no province that is its holder's homeland, at the walk-in, the attack's undefended exit, the landing and the AI's rungs (GR5); (c) `GLORY_ATTACK_OBEYS_THE_ODDS` — ONE predicate over `CombatExecutor.muster_odds`, the player processor and the AI's P3.9 rung, the WO-28 beat names the odds; (d) `describe_alternative` stamps "attack Archduke Charles at Tyrol" on the trust option, the message and the digest. **Completion:** item 2 (the preview within 20% on 8 of 10 driven strikes) **MET** by the existing arithmetic and pinned; item 1 (45 by turn 40 on typed orders) **NOT MET** — opening B 40 at turn 8 (archive 39) then 36 at turn 40; opening A 41 at turn 7, 39 at 13 (archive 35), then the Emperor and four marshals captured by turn 14 and the Fall. **The number is NOT moved a second time. The deeper gate, the user's ruling (each a gate of its own):** four action points for eight corps; the 37,500 supply cap that scatters the army into pieces Charles beats one at a time; the marshals' capture cascade; the harness's `--diplomacy accept` signing every armistice mid-conquest. `test_a_played_arm_reaches_forty_five_by_turn_forty` is a strict xfail over the archived digests — it turns red the day a road reaches 45. **Filed, not fixed: VP-R1-X1** (`BUG_FIXES.md`) — the objection's attack alternative can name an ALLY's corps in range, which the executor then refuses. `BASELINE_SERIES` re-recorded ONCE, four-arm attributed (the coast lever the sole mover; the passive France ends turn 40 with 17 provinces instead of 4 — a harness fact, not a balance claim). |
