...
| **WO-D6** | **The game cannot narrate your own catastrophe.** The fall of **Paris** — capital, Emperor's seat — is narrated with the exact template Brittany gets three turns later; `capital_stormed` (weight 92) exists only for France taking an ENEMY capital, with no symmetric class for losing your own. Meanwhile `own_mauled` led a briefing with a **26-man** casualty on a turn a vassal defected, a homeland province fell, and the army stood at 24% of boot strength. | Kingmaker + Tyrant arms; memo §4 | A `capital_lost` headline class at or above 92, and a re-look at whether the relative-proportion classes should outrank absolute-scale catastrophes in the weight table. Cheap, and it lands on the pillar the project has scored lowest for months. |
...
| " | **near-term mitigation — ✅ DISCHARGED August 22, 2026** | row WO **slice 5**, LANDED (`WEIRD_OUTCOMES_SPEC.md` §3 slice 5 = the landing record) | Berthier names the ACCEPT-able peace, so a contactless war is at least *visible* as endable. **Measured on the row's own seed:** at t16 of the `austerlitz` reproduction the war room named Russia — pair score 0, both courts past the exhaustion floor, and the scorer rating her plain peace ACCEPT at 54 — where before it named Britain's design and never mentioned Russia at all. **Does not fix the billing**, which stays carried to EC-2 pass 2 above, untouched. | `tests/test_wo_slice5_berthier_names_the_peace.py` (56); mutation sweep `tools/_sweep_wo5.json` 23/23 killed |
