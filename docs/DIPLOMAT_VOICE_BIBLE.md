...
> **v0.5.2 scope note (2026-04-20, updated v2.4.3-final):** Cast coverage requirement now includes the live hegemony and repair surfaces, not only the breach/paradox beats. `COMMITMENTS_PRESENTATION_SPEC.md` v0.5.2 requires (a) the core breach / hard-reject leads, (b) one `balance_of_europe_shifted` warning family per likely warning court, with `noticed` / `alarming` / `crisis` variants aligned to the `33 / 50 / 60` activation gate in presentation spec §8.1a, and (c) one `amends_offered` acknowledgment line per foreign court. Bloc naming is adopted for v2.4.3. **Per-row bloc stamps (member badges) are now in-scope** as the final slice of the Memory and Pressure track (Slice E-Cards per `RELIABILITY_IMPLEMENTATION_PLAN.md`); stamp copy itself is minimal microcopy (deterministic labels routed through `describe_hegemon_bloc`) and does NOT require new authored-diplomat lines — the voiced scope remains headline / threshold beats / warnings / amends, and stamps reuse those labels without new cast work. Bargain-era callbacks and extra witness variants still defer to `docs/WAR_BARGAIN_SPEC.md` slice WB-D.
...
The adopted bloc-naming contract (`COMMITMENTS_PRESENTATION_SPEC.md` §8.1a.4) means authors do **not** need to prepare separate voiced member-badge or ledger-stamp lines for named blocs. Per-row stamps in Slice E-Cards reuse the same deterministic labels from `describe_hegemon_bloc` that the headline and threshold beats already use; stamp copy is a short label taxonomy, not a new cast surface. The required voiced surfaces remain the headline-adjacent threshold beats, warnings, breach/paradox lines, and amends acknowledgments — authors can ignore stamps for register purposes.
