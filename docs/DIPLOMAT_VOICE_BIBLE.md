...
The adopted bloc-naming contract (`COMMITMENTS_PRESENTATION_SPEC.md` §8.1a.4) means authors do **not** need to prepare separate voiced member-badge or ledger-stamp lines for named blocs. If a later stamp pass is re-opened, it should reuse the same deterministic labels from `describe_hegemon_bloc` that the headline and threshold beats already use; stamp copy would be a short label taxonomy, not a new cast surface. The required voiced surfaces remain the headline-adjacent threshold beats, warnings, breach/paradox lines, and amends acknowledgments — authors can ignore stamps for register purposes.
