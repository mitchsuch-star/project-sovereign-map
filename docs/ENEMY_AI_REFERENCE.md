...
| P2.5 | Square Formation | — | Threat gone → break square (cooldown 2). Cavalry threat + no artillery → the square is WANTED here and formed only after every strike rung (P3.25 … P4.5) declines; P3's fortify / defensive stance yield to it; a formed square ends the corps' phase; a square broken by the corps' own action takes the cooldown (FA slice 4, Sept 4 2026) An administrative act (the admin recruit, a garrison detachment) neither breaks the square from the pick nor stamps the cooldown (slice-4 review round R1-3) |
| P3 | Threat Response / Attack | 75 | Meets personality threshold |
| P3.25 | Counter-punch | — | Cautious: free attack after defense — priced against the FIELD at the target's province, the mood threshold as a floor, drawn only when a target exists (FA slice 4) — never priced BELOW the target himself: a retreated target is not his neighbours (slice-4 review round R1-1) |
...
| **P-Intent** (AI-2 Stage C, BEFORE P3 — the design outranks the threat-shelter ask) | At peace with the player · a live non-survival design (deckless courts fall through byte-identically, pin 18). Three arms, first match: **(1) design purchase** — obstacle IS France, price `ask`/`buy` → demands the coveted province, pays gold at `buy` (accept cedes + satisfies; reject writes the pin-8 refusal record); **(2) sell-neutrality** — at war in a war France is NOT in, France looming (relation < 0 or the §3.5 mirror reads this court), price below `coerce`, obstacle NOT France → gold lump for a non-aggression pact whose acceptance mints the §3.3 neutrality compact; **(3) alignment ask** — obstacle is a third court France is hostile to, price `align`/`bandwagon` → the upgrade pact | `design_purchase` / `sell_neutrality` / upgrade types; every envelope tagged `intent_ask` and delivered under the §4.2c budget (`INTENT_ASK_BUDGET_PER_TURN = 2`, separate from the bandwagon cap; ultimatums exempt from both). The §4.2c opportunism valve: a design-family ask while the obstacle fights 2+ wars bypasses the NATION cooldown (type cooldown stands) |
| P3 | Threat > 60 AND not allied | Hegemony ask (live since W6-10; **AI-2c: statecraft reorders the candidates** — Prussia leads with gold, Austria with pacts — under the NA-2 design-advancing front-move) |
...
| P-Bandwagon | Player-hegemon arm unchanged; **AI-2 widened arm:** any NON-player hegemon at ≥50% share + this court's own intent price reads `bandwagon` → recipient-addressed upgrade to that hegemon through the converged AI-2a transport (boot-dormant on 1805 — France leads at 0.396) | Upgrade toward the hegemon |

**Court-to-court (AI-AI) additions, Stage C** (`_evaluate_ai_ai_proposal` trigger 0 + phase-level passes):

- **Trigger 0a — the design ask:** a court at `ask`/`buy` whose obstacle is the OTHER court asks it; never satisfiable by agreement in Stage C (territorial AI-AI transfer lands with AI-4's settlements) — the refusal IS the deliverable (§5 pin 8 record + the fog-visible `ai_ai_proposal_refused` event AI-3 escalates from).
- **Trigger 0b — the alignment ask:** a court at `align`+ courts the enemies of its obstacle (defensive-alliance ask; ratifiable).
- **The haggle arm (AI-2c):** a HAGGLING recipient (statecraft: Metternich/Hardenberg/Castlereagh, never Alexander) whose refusal fell in [35, 50) counters one rung down the upgrade ladder; dual consent ratifies the downgraded type (`countered: True` on the event). Non-hagglers keep accept-or-refuse byte-identically.
- **The sponsor branch (`_process_ai_sponsorships`):** a sponsor-first court (statecraft: Russia, Britain) with treasury ≥ 1,500 funds another court's live design against its own obstacle — the same AI-2b directed record the player mints, bond included; ≤ 1 new patronage per turn.
- **The allegiance auction (`process_allegiance_auctions`, §12.6):** a minor whose intent crests at `bandwagon` has its flip announced IN PLAY (3-turn window, serialized); resolved to the best suitor by relations + standing sponsorship (10g/turn = 1 lean point); a player win arrives as a refusable mailbox offer; a passed crest lapses.
