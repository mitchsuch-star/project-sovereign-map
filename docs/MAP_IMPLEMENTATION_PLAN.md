...
**Also flagged (not gate-blocking, decide disposition):** the Flanders↔London sea link is walkable both ways with no naval layer (Moore can invade France; France can rush London — garrison/strength tuning is the only current control; differentiate capital garrisons majors 20–25k vs minors 8–10k); compressed cross-Baltic **land** adjacencies in the registry (Scania↔Berlin, East Prussia↔Finland, Posen↔Finland, Livonia↔Pomerania/Brandenburg/Posen, Hanover↔Jutland/Oslo) will read as bugs in play — consider a registry pass demoting them to sea links or cutting them; `is_coastal` is image-derived and over-true (Franche-Comte/Munich/Bern/Tyrol/Milan flagged coastal) — do not use for naval logic without review.
