...
| **B1** | **F1 The Antechamber** — backend tier/routing/notification + `GET /marshal_petition`; then the chip/badge/dialog-open client half. **⚑ SLOTTED September 23, 2026 (calls under the user's delegated grant, `STATUS.md` ▶ NEXT UP): Update 4, after the release.** It jumps ahead if players flag modal overload. Measured at HEAD `289697ef`: the first end turn stacks about six modal surfaces, including a Murat grievance card on turn 1, and one petition arrives every 1–2 turns. B2–B5 follow B1 in order. | **clear — Q1(a)/Q2(a) RULED** | backend + `.gd` |
