...
> **Status:** DRAFT v0.1 — July 2, 2026. **Needs user scope/sequencing blessing before implementation.** Slice CR-0 is a defect fix (no design gate); CR-6 (Conversational Objection Negotiation) and anything coupling `strategic_score` to outcomes REQUIRE their own design gates per the ROADMAP LLM Mechanical Expansion rules.
...
| Slice | Scope | Gate |
|-------|-------|------|
| **CR-0 (P0 defect fix)** | Dynamize parser rosters from the live world: `valid_marshals` from `world.marshals` (mirror the `_get_known_enemies(world)` pattern), `known_regions` from `world.regions`, mock target ladder from game_state. Closes the 5/7-marshal gap + the BUG_FIXES bare-command entry in one pass. Behavior tests over BOTH worlds (legacy fixture + `europe_1805.json`). | None (defect) |
