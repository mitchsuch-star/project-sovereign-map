...
>
> CRT-2's other rows (CQ-17, CQ-29) and CRT-4 … CRT-11 follow, in this order. Player reports may re-order them. **Score Mandate ruling (September 26, 2026 (evening)): CRT-4 and CRT-5 ride SR-2e "standing orders reliable" (SR-4b folded in), built next; the rest follow in Chunk 3b.**
...
**Not built here, with reasons:** the client's non-question half of CX-X1 (`what the hell, declare war on Prussia` is sent) stays the recorded P4; the door's rule is a mirror of the parser's subject rule, not a classifier. **L-D "the boolean road"** (the plan's SR-3a rider) was SKIPPED here: it sat behind a user gate (`LOCAL_PARSER_FEASIBILITY_2026_09_20.md` §6.3) that this session's prompt left unfilled. **✅ APPROVED by the user September 26, 2026 (evening)** — keyless delegation is gated on the deterministic `DelegationMatch` (not the parse), keeping the action-only, objection-first single-modal and personality pre-flight guardrails (§6.3); the memo's §6.4 `done_when` is the contract. **Not yet built — it is Chunk 3's remaining slice, after SR-2e** (`SCORE_MANDATE_PLAN.md` §2).
...
### §12.11 L-1 LANDING RECORD — "The prompt turns around", with IQ9-X1 + IQ9-X3 ✅ LANDED September 26, 2026 (Score Mandate Chunk 3, SR-3c)

**Re-measured first, on the shipped 1805 boot, through the production-shaped prompt state (`parser_eval.build_llm_game_state`, pinned equal to `main.get_llm_game_state`).** Two parse prompts for the SAME order on boards one battle apart shared **84 characters** (the memo measured 95 on an older board): the board — our marshals, the enemy, the per-marshal compass lines inside the directions rules — sat at the top, so nothing above the command could be reused. IQ9-X1 reproduced (`hunt down mack`: one live retry spent and discarded, "Did you mean 'Davout'? ('down' not found)"); IQ9-X3 reproduced ("flurble the wibble": one live parse, the response stamped as the offline road).

**L-1 — the prompt turns around** (`prompt_builder.THE_PROMPT_IS_STATIC_FIRST`). The ONE prompt f-string is cut at its own headers into sections that are the shipped text byte for byte (generated, not retyped) and assembled in either order: up = every rule, the output contract and the board-independent examples FIRST, then the board (our marshals, the order's addressee, the enemy, the map orientation, the examples that name an enemy), then the order; down = the pre-slice prompt — **42 of 42 prompts byte-identical to HEAD's builder** (six boards incl. the cold state × seven orders). The dead `marshal_name` / `personality` params are wired: `addressed_marshal` reads the parser's own address token and the personality the marshals block prints, both providers pass them, and an addressed order carries "## The Order Is Addressed To / Ney (aggressive)" on the request the replay receives. The personality rule's "above" reads "below" in the new order only.

**The examples had to become board-independent — found by measuring past the contract.** The contract's case (one battle apart) passed at once (16,150 shared), but over twelve ambient turns the prefix fell to **13,280 on turn 3 and 12,199 on turn 8** — below the ~4,096-token floor the contract names — because the examples named the first VISIBLE enemy and his CURRENT province. The static examples now name our first two marshals, the first court at war with us (sorted — public) and its capital; the two templates that name an enemy commander ride after the board, still naming only an enemy in view (never a fogged name the model could echo into a player-facing suggestion — IQ9-X2's principle). Each template is taught once (the sweep's one INERT found that invariant unpinned; it is pinned now).

**Measured after:** same order, one battle apart **84 → 15,771 shared characters**; across twelve turns of play ≥ 15,921 every turn; the invariant prefix across orders and boards 15,771 characters ≈ **4,381 tokens** (an ESTIMATE at the memo's measured 3.6 chars/token — no local tokenizer exists); the prompt grows ~130 characters. **Prompt caching stays OFF**: its recorded rejection (STATUS) rested on the volatile board at the top of the prompt; that premise is gone, and turning caching on is a fresh decision for the user, not taken here.

**The IQ-9 cassettes: 16 parse cassettes RE-STAMPED + the MANIFEST, attributed exactly** — with the lever DOWN every cassette's live fingerprint equals the one it carried (prompt sha AND chars), the system prompt is unchanged in both arms, and the recovery cassette does not move; the before/after record is `tests/data/l1_prompt_restamp.json` and the attribution is pinned both ways per cassette. Replay keys are `(kind, utterance, world)`, so every cassette still replays; `parser_eval --replay` 6/6, no drift. Never `--refresh-drifted` (it re-records against the user's key).

**IQ9-X1 — the retry reads the marshal** (`parser.THE_RETRY_READS_THE_MARSHAL`). The fast pass's marshal WORD SCAN (a heuristic that runs when a parse names no marshal) read "down" as a typo of Davout; the CR-2 retry's live answer (no marshal named, pursue Mack) was re-run through the same scan and discarded. On the retry only, the fuzzy pass gets `trust_marshal_reading=True`: the live model's "no marshal named" stands, a marshal it NAMES is still validated, and the primary road is unchanged. At the wire `hunt down mack` now answers exactly as the fast road's own `pursue mack` does — the pursuit handed to the same marshal by the existing nearest-marshal rule — instead of a question about a name nobody typed.

**IQ9-X3 — a failure names its road** (`parser.A_FAILURE_NAMES_ITS_ROAD` + the transient `ParseResult.live_consulted`, mirroring `llm_error`). Every parser failure dict carries `mode`: the live provider when a live call was made (a retry consulted, the result's own live mode, a live answer discarded by either client fallback, or a call failed at the API), else "mock". The offline fallback keeps mode "mock" — whose reading it is; CR-5's guardrail (e) reads that — so the marker is separate. A SUCCESSFUL parse still names whose reading it acts on (unchanged, pinned). Both client fallback branches are pinned with a stub provider.

**Two IQ-9 pins flipped consciously, as their own docstrings asked:** `test_retry_cannot_rescue_the_word_scan_family_today` → `test_the_retry_rescues_the_word_scan_family`; `test_a_live_road_failure_still_stamps_mock_provenance` → `test_a_live_road_failure_names_the_live_road`.

**Found by the families, fixed:** the FA-15b provenance census (every line naming the response's parse-provenance keys must live in `main.py`) caught the IQ9-X3 lever's own COMMENT naming the key — reworded, the census untouched.

**Tests:** `tests/test_l1_the_prompt_turns_around.py` (29, incl. the 16-cassette attribution) + `tests/test_cr6_retry_rescues_the_word_scan.py` (14). **Sweep:** `tools/_sweep_sr3c.json` 18/18 killed, 0 INERT at close — one first INERT (removing the static examples' enemy filter left honesty and stability green, because the stable names carry no commander), resolved by pinning the invariant the filter actually keeps: each template is taught once. **Gates:** corpus 791/791 mock + replay 6/6; the parser / command / prompt / live-road families incl. the baseline-series assurance and the combat harness (5,671 across 103 files); `BASELINE_SERIES` + M1–M7 byte-identical (structural — the AI never parses text); ruff clean; zero `.gd`.
